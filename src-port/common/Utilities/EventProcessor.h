...
private:
  ...
  AbortState m_abortState{AbortState::STATE_RUNNING}; // set by externals when the event is aborted,
                                                      // aborted events don't execute
...
template <typename T> class LambdaBasicEvent : public BasicEvent
{
public:
...
public:
  ...
  template <typename T> is_lambda_event<T> AddEvent(T&& event, Milliseconds e_time, bool set_addtime = true)
  {
  ...
  {
    AddEvent(event, CalculateTime(randtime(offset, offset2).count()), true, false);
  }
  template <typename T> is_lambda_event<T> AddEventAtOffset(T&& event, Milliseconds offset, uint8 eventGroup)
  {
    AddEvent(new LambdaBasicEvent<T>(std::move(event)), CalculateTime(offset.count()), true, eventGroup);
  };
  template <typename T>
  is_lambda_event<T> AddEventAtOffset(T&& event, Milliseconds offset, Milliseconds offset2, uint8 eventGroup)
  {
    AddEvent(new LambdaBasicEvent<T>(std::move(event)), CalculateTime(randtime(offset, offset2).count()), true,
             eventGroup);
  };
  template <typename T> is_lambda_event<T> AddEventAtOffset(T&& event, Milliseconds offset)
  {
    AddEventAtOffset(new LambdaBasicEvent<T>(std::move(event)), offset);
  }
  template <typename T> is_lambda_event<T> AddEventAtOffset(T&& event, Milliseconds offset, Milliseconds offset2)
  {
    AddEventAtOffset(new LambdaBasicEvent<T>(std::move(event)), offset, offset2);
  }
