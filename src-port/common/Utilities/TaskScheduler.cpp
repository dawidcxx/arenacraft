...
{
  _task_holder.RemoveIf([group](TaskContainer const& task) -> bool { return task->IsInGroup(group); });
  return *this;
}
...
bool TaskScheduler::IsGroupScheduled(group_t const group) { return _task_holder.IsGroupQueued(group); }

Milliseconds TaskScheduler::GetNextGroupOcurrence(group_t const group) const
{
  return std::chrono::duration_cast<std::chrono::milliseconds>(_task_holder.GetNextGroupOcurrence(group) -
                                                               clock_t::now());
}

void TaskScheduler::TaskQueue::Push(TaskContainer&& task) { container.insert(task); }
...
auto TaskScheduler::TaskQueue::First() const -> TaskContainer const& { return *container.begin(); }
...
bool TaskContext::IsInGroup(TaskScheduler::group_t const group) const { return _task->IsInGroup(group); }
...
TaskScheduler::repeated_t TaskContext::GetRepeatCounter() const { return _task->_repeated; }

TaskScheduler::timepoint_t TaskContext::GetNextOcurrence() const { return _task->_end; }
...
TaskContext& TaskContext::CancelAll() { return Dispatch(std::mem_fn(&TaskScheduler::CancelAll)); }
