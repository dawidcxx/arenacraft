...
template <typename T> constexpr bool IsEnumFlag(T) { return false; }
...
{
template <typename T> using IsFlag = std::integral_constant<bool, IsEnumFlag(T{})>;
}
...
{
  static_assert(EnumTraits::IsFlag<T>::value, "EnumFlag must be used only with enums that are specify "
                                              "EnumFlag::IsFlag");
...
public:
  ...
  constexpr friend EnumFlag operator&(EnumFlag left, EnumFlag right) { return left &= right; }
  ...
  constexpr EnumFlag operator~() const { return static_cast<T>(~static_cast<std::underlying_type_t<T>>(_value)); }
  ...
  constexpr bool HasAllFlags(T flags) const { return (_value & flags) == flags; }
