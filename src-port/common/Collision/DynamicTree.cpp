...
struct DynamicTreeAreaInfoCallback
{
  ...
  void operator()(G3D::Vector3 const& p, GameObjectModel const& obj) { obj.IntersectPoint(p, _areaInfo, _phaseMask); }
...
bool DynamicMapTree::GetIntersectionTime(const uint32 phasemask, const G3D::Ray& ray, const G3D::Vector3& endPos,
                                         float& maxDist) const
{
...
bool DynamicMapTree::GetObjectHitPos(const uint32 phasemask, const G3D::Vector3& startPos, const G3D::Vector3& endPos,
...
bool DynamicMapTree::isInLineOfSight(float x1, float y1, float z1, float x2, float y2, float z2, uint32 phasemask,
...
bool DynamicMapTree::GetAreaInfo(float x, float y, float& z, uint32 phasemask, uint32& flags, int32& adtId,
...
void DynamicMapTree::GetAreaAndLiquidData(float x, float y, float z, uint32 phasemask, uint8 reqLiquidType,
                                          VMAP::AreaAndLiquidData& data) const
{
  ...
  if (intersectionCallBack.GetLocationInfo().hitModel)
  {
    ...
    float  liquidLevel;
    if (!reqLiquidType ||
        (dynamic_cast<VMAP::VMapMgr2*>(VMAP::VMapFactory::createOrGetVMapMgr())->GetLiquidFlagsPtr(liquidType) &
         reqLiquidType))
      if (intersectionCallBack.GetHitModel()->GetLiquidLevel(v, intersectionCallBack.GetLocationInfo(), liquidLevel))
        data.liquidInfo.emplace(liquidType, liquidLevel);

    data.areaInfo.emplace(0, intersectionCallBack.GetLocationInfo().rootId,
