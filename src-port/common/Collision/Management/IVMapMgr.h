...
public:
  ...
  /**
  test if we hit an object. return true if we hit one. rx, ry, rz will hold the
  hit position or the dest position, if no intersection was found return a
  ...
  */
  ...
  It is enabled by default. If it is enabled in mid game the maps have to loaded
  manualy
  */
  ...
  */
  virtual bool GetAreaInfo(uint32 pMapId, float x, float y, float& z, uint32& flags, int32& adtId, int32& rootId,
  ...
  virtual bool GetLiquidLevel(uint32 pMapId, float x, float y, float z, uint8 ReqLiquidType, float& level, float& floor,
                              uint32& type, uint32& mogpFlags) const = 0;
  // get both area + liquid data in a single vmap lookup
  virtual void GetAreaAndLiquidData(uint32 mapId, float x, float y, float z, uint8 reqLiquidType,
