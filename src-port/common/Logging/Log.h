...
public:
  ...
  inline void outMessage(std::string const& filter, LogLevel const level, Acore::FormatString<Args...> fmt,
                         Args&&... args)
  {
    _outMessage(filter, level, Acore::StringFormat(fmt, std::forward<Args>(args)...));
  }

  template <typename... Args> void outCommand(uint32 account, Acore::FormatString<Args...> fmt, Args&&... args)
  {
...
#define LOG_EXCEPTION_FREE(filterType__, level__, ...)                                                                 \
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      sLog->outMessage("server", LogLevel::LOG_LEVEL_ERROR, "Wrong format occurred ({}) at '{}:{}'", e.what(),         \
                       __FILE__, __LINE__);                                                                            \
    }                                                                                                                  \
  }
