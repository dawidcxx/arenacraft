...
AppenderFile::AppenderFile(uint8 id, std::string const& name, LogLevel level, AppenderFlags flags,
    ...
    : Appender(id, name, level, flags), logfile(nullptr), _logDir(sLog->GetLogsDir()), _maxFileSize(0), _fileSize(0)
{
  if (args.size() < 4)
  {
    throw InvalidAppenderArgsException(
        Acore::StringFormat("Log::CreateAppenderFromConfig: Missing file name for appender {}", name));
  }
  ...
  if (5 < args.size())
  {
    ...
    {
      throw InvalidAppenderArgsException(
          Acore::StringFormat("Log::CreateAppenderFromConfig: Invalid size '{}' for appender {}", args[5], name));
    }
  }
