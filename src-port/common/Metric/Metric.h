...
public:
  ...
  template <class T> void LogValue(std::string const& category, T value, std::vector<MetricTag> tags)
  {
...
template <typename LoggerType> class MetricStopWatch
{
public:
...
template <typename LoggerType> MetricStopWatch<LoggerType> MakeMetricStopWatch(LoggerType&& loggerFunc)
{
...
#define METRIC_TIMER(category, ...)                                                                                    \
  MetricStopWatch METRIC_UNIQUE_NAME(__ac_metric_stop_watch) = MakeMetricStopWatch(                                    \
      [&](TimePoint start) { sMetric->LogValue(category, std::chrono::steady_clock::now() - start, {__VA_ARGS__}); });
...
#define METRIC_DETAILED_TIMER(category, ...)                                                                           \
  MetricStopWatch METRIC_UNIQUE_NAME(__ac_metric_stop_watch) = MakeMetricStopWatch(                                    \
      ...
      {                                                                                                                \
        int64 duration =                                                                                               \
            int64(std::chrono::duration_cast<Milliseconds>(std::chrono::steady_clock::now() - start).count());         \
