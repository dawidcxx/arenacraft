...
void Metric::Initialize(std::string const& realmName, Acore::Asio::IoContext& ioContext,
...
bool Metric::Connect()
{
  ...
  if (error)
  {
    LOG_ERROR("metric", "Error connecting to '{}:{}', disabling Metric. Error message: {}", _hostname, _port,
              error.message());
...
void Metric::LoadFromConfigs()
{
  ...
  {
    LOG_ERROR("metric", "'Metric.OverallStatusInterval' config set to {}, overriding to 1.",
...
std::string Metric::FormatInfluxDBValue(bool value) { return value ? "t" : "f"; }

template <class T> std::string Metric::FormatInfluxDBValue(T value) { return std::to_string(value) + 'i'; }
...
std::string Metric::FormatInfluxDBValue(char const* value) { return FormatInfluxDBValue(std::string(value)); }

std::string Metric::FormatInfluxDBValue(double value) { return std::to_string(value); }

std::string Metric::FormatInfluxDBValue(float value) { return FormatInfluxDBValue(double(value)); }
