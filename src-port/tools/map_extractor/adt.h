...
public:
  ...
  int8 GetLiquidDepth(adt_liquid_instance const* h, int32 pos) const
  {
    ...
    case LiquidVertexFormatType::HeightDepth:
      return ((int8 const*)((int8 const*)this + 8 + h->OffsetVertexData +
                            (h->GetWidth() + 1) * (h->GetHeight() + 1) * 4))[pos];
    case LiquidVertexFormatType::HeightTextureCoord:
      return 0;
    case LiquidVertexFormatType::Depth:
  ...
  uint16 const* GetLiquidTextureCoordMap(adt_liquid_instance const* h, int32 pos) const
  {
    ...
    case LiquidVertexFormatType::Depth:
      return nullptr;
    case LiquidVertexFormatType::HeightTextureCoord:
      return (uint16 const*)((uint8 const*)this + 8 + h->OffsetVertexData +
                             4 * ((h->GetWidth() + 1) * (h->GetHeight() + 1) + pos));
    default:
      break;
    }
    return nullptr;
...
public:
  bool      prepareLoadedData();
  adt_MCIN* getMCIN() { return reinterpret_cast<adt_MCIN*>(reinterpret_cast<uint8*>(&flags) + offsMCIN); }
  adt_MH2O* getMH2O()
  {
    if (offsMH2O)
