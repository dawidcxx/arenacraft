...
{
  auto arenaInstance = sBattlegroundMgr->CreateNewBattleground(BATTLEGROUND_AA, constants::SOLOQ_DIFFICULTY_ENTRY,
                                                               ArenaType::ARENA_TYPE_5v5, true);
...
bool CreateSoloqTeam(Player* player)
{
  ...
  if (arenaTeam->Create(player->GetGUID(), ARENA_TYPE_5v5, teamNameBuilder.str(), 0, 0, 0, 0,
                        0 // black bordered team banner
                        ))
  {
...
bool AddPlayerToSoloQueue(Player* player)
{
  ...
  GroupQueueInfo* ginfo =
      bgQueue.AddGroup(player,
                       nullptr, // no group
                       BattlegroundTypeId::BATTLEGROUND_AA, constants::SOLOQ_DIFFICULTY_ENTRY, ARENA_TYPE_5v5,
                       true, // isRated
                       false, 0, 0, 0, 0);
...
bool HasSoloTeam(Player* player) { return GetArenaTeam(player) != nullptr; }
