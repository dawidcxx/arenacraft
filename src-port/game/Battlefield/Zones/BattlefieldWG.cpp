...
bool BattlefieldWG::SetupBattlefield()
{
  ...
  InitStalker(BATTLEFIELD_WG_NPC_STALKER, WintergraspStalkerPos[0], WintergraspStalkerPos[1], WintergraspStalkerPos[2],
  ...
  {
    // Horde npc
    if (Creature* creature = SpawnCreature(WGKeepNPC[i].entryHorde, WGKeepNPC[i].x, WGKeepNPC[i].y, WGKeepNPC[i].z,
                                           WGKeepNPC[i].o, TEAM_HORDE))
      KeepCreature[TEAM_HORDE].insert(creature->GetGUID());
    // Alliance npc
    if (Creature* creature = SpawnCreature(WGKeepNPC[i].entryAlliance, WGKeepNPC[i].x, WGKeepNPC[i].y, WGKeepNPC[i].z,
                                           WGKeepNPC[i].o, TEAM_ALLIANCE))
  ...
  for (uint8 i = 0; i < WG_OUTSIDE_ALLIANCE_NPC; i++)
    if (Creature* creature = SpawnCreature(WGOutsideNPC[i].entryHorde, WGOutsideNPC[i].x, WGOutsideNPC[i].y,
  ...
  for (uint8 i = WG_OUTSIDE_ALLIANCE_NPC; i < WG_MAX_OUTSIDE_NPC; i++)
    if (Creature* creature = SpawnCreature(WGOutsideNPC[i].entryAlliance, WGOutsideNPC[i].x, WGOutsideNPC[i].y,
  ...
  {
    GameObject*             go = SpawnGameObject(WGGameObjectBuilding[i].entry, WGGameObjectBuilding[i].x,
                                                 WGGameObjectBuilding[i].y, WGGameObjectBuilding[i].z, WGGameObjectBuilding[i].o);
    BfWGGameObjectBuilding* b  = new BfWGGameObjectBuilding(this);
    b->Init(go, WGGameObjectBuilding[i].type, WGGameObjectBuilding[i].WorldState, WGGameObjectBuilding[i].damageText,
  ...
  {
    GameObject* go = SpawnGameObject(WGPortalDefenderData[i].entry, WGPortalDefenderData[i].x,
                                     WGPortalDefenderData[i].y, WGPortalDefenderData[i].z, WGPortalDefenderData[i].o);
  ...
  for (uint8 i = 0; i < PVP_TEAMS_COUNT; ++i)
    for (GuidUnorderedSet::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
      if (Player* player = ObjectAccessor::FindPlayer(*itr))
      {
        ...
        player->SendUpdateWorldState(BATTLEFIELD_WG_WORLD_STATE_MAX_VEHICLE_A,
...
void BattlefieldWG::OnBattleEnd(bool endByTimer)
{
  ...
  for (Workshop::const_iterator itr = WorkshopsList.begin(); itr != WorkshopsList.end(); ++itr)
  {
    (*itr)->GiveControlTo((*itr)->workshopId == BATTLEFIELD_WG_WORKSHOP_SE ||
  ...
  if (!endByTimer) // win alli/horde
  {
    ...
    SendWarning((GetDefenderTeam() == TEAM_ALLIANCE) ? BATTLEFIELD_WG_TEXT_WIN_KEEP
  ...
  for (Workshop::const_iterator itr = WorkshopsList.begin(); itr != WorkshopsList.end(); ++itr)
  {
    if (WGWorkshop* workshop = (*itr))
    {
      if (workshop->workshopId == workshopId)
      {
        ...
        // Sending neutral team at start to set normal capture points by workshop->teamControl, TEAM_NEUTRAL is ignored
        // at first call
    ...
    if (victim->IsPlayer() && victim->HasAura(SPELL_LIEUTENANT))
    {
      ...
      for (auto& playerGuid : m_PlayersInWar[killerTeam])
      {
        if (Player* player = ObjectAccessor::FindPlayer(playerGuid))
        {
          ...
          {
            player->KilledMonsterCredit(killerTeam == TEAM_HORDE ? NPC_QUEST_PVP_KILL_ALLIANCE
...
{
  // Remove all aura from WG /// @todo: false we can go out of this zone on retail and keep Rank buff, remove on end of
  // WG
...
void BattlefieldWG::BrokenWallOrTower(TeamId /*team*/)
{
  ...
  /*    if (team == GetDefenderTeam())
      {
          for (GuidUnorderedSet::const_iterator itr = m_PlayersInWar[GetAttackerTeam()].begin(); itr !=
  ...
  if (team == GetAttackerTeam())
  {
    ...
    for (GuidUnorderedSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin();
      ...
      if (Player* player = ObjectAccessor::FindPlayer(*itr))
      {
        ...
        player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_LEANING_TOWER_ACHIEVEMENT, 0,
                                          0);
      }
...
void BattlefieldWG::AddUpdateTenacity(Player* player) { m_updateTenacityList.insert(player->GetGUID()); }
