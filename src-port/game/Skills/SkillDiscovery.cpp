...
void LoadSkillDiscoveryTable()
{
  ...
  do
  {
          ...
          !reqSpellInfo->IsExplicitDiscovery())
      {
        if (reportedReqSpells.find(absReqSkillOrSpell) == reportedReqSpells.end())
        {
          LOG_ERROR("sql.sql",
                    ...
                    " and not 100% chance random discovery ability but listed for spellId {} (and maybe more) in "
    ...
    else if (reqSkillOrSpell == 0) // skill case
    {
      ...
      {
        LOG_ERROR("sql.sql",
                  "Spell (ID: {}) not listed in `SkillLineAbility.dbc` but listed with `reqSpell`=0 in "
                  "`skill_discovery_template` table",
                  spellId);
        continue;
      }
    ...
    {
      LOG_ERROR("sql.sql", "Spell (ID: {}) have negative value in `reqSpell` field in `skill_discovery_template` table",
                spellId);
      continue;
    }
  ...
  if (!ssNonDiscoverableEntries.str().empty())
    LOG_ERROR("sql.sql",
              "Some items can't be successfully discovered: have in chance field value < 0.000001 in "
  ...
  for (uint32 spell_id = 1; spell_id < sSpellMgr->GetSpellInfoStoreSize(); ++spell_id)
  {
    ...
    if (SkillDiscoveryStore.find(int32(spell_id)) == SkillDiscoveryStore.end())
      LOG_ERROR("sql.sql",
                "Spell (ID: {}) is 100% chance random discovery ability but not have data in "
                "`skill_discovery_template` table",
                spell_id);
  }
...
uint32 GetExplicitDiscoverySpell(uint32 spellId, Player* player)
{
  ...
  for (SkillDiscoveryList::const_iterator item_iter = tab->second.begin(); item_iter != tab->second.end(); ++item_iter)
  {
    ...
    if (item_iter->chance > roll)
    {
      ...
      if (spellId != 64323)
        player->UpdateGatherSkill(SKILL_INSCRIPTION, player->GetPureSkillValue(SKILL_INSCRIPTION),
