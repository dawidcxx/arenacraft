...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket& recvData)
{
  ...
  if (!questGiver)
  {
    LOG_DEBUG("network.opcode", "Error in CMSG_QUESTGIVER_STATUS_QUERY, called for not found questgiver ({})",
              guid.ToString());
    return;
  }
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket& recvData)
{
  ...
  if (!creature)
  {
    LOG_DEBUG("network", "WORLD: HandleQuestgiverHelloOpcode - Unit ({}) not found or you can't interact with him.",
              guid.ToString());
    return;
  }
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_ACCEPT_QUEST npc {}, quest = {}, unk1 = {}", guid.ToString(),
            questId, unk1);
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
  ...
  uint8      unk1;
  recvData >> guid >> questId >> unk1;
  LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_QUERY_QUEST npc {}, quest = {}, unk1 = {}", guid.ToString(),
            questId, unk1);
  ...
  if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
  {
    ...
    if (quest->IsAutoComplete() || !quest->GetQuestMethod())
      _player->PlayerTalkClass->SendQuestGiverRequestItems(quest, object->GetGUID(),
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& recvData)
{
  ...
  if (reward >= QUEST_REWARD_CHOICES_COUNT)
  {
    LOG_ERROR("network.opcode",
              "Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player {} ({}) tried to get invalid reward ({}) (probably "
              "packet hacking)",
              _player->GetName(), _player->GetGUID().ToString(), reward);
    return;
  }

  LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_CHOOSE_REWARD npc {}, quest = {}, reward = {}", guid.ToString(),
            questId, reward);
...
void WorldSession::HandleQuestgiverCancel(WorldPacket& /*recvData*/) { _player->PlayerTalkClass->SendCloseGossip(); }
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recvData)
{
  ...
  if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
  {
    ...
    {
      LOG_ERROR("network.opcode",
                "Possible hacking attempt: Player {} [{}] tried to complete quest [entry: {}] without being in "
                "possession of the quest!",
                _player->GetName(), _player->GetGUID().ToString(), questId);
      return;
    }
    ...
    if (_player->GetQuestStatus(questId) != QUEST_STATUS_COMPLETE)
    {
      if (quest->IsRepeatable())
        _player->PlayerTalkClass->SendQuestGiverRequestItems(quest, guid, _player->CanCompleteRepeatableQuest(quest),
                                                             false);
      else
  ...
  if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
  {
    if (Group* group = _player->GetGroup())
    {
      for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
      {
        ...
        if (quest->IsAutoComplete() || !quest->GetQuestMethod())
          player->PlayerTalkClass->SendQuestGiverRequestItems(quest, _player->GetGUID(),
