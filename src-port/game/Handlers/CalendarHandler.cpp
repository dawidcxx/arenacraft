...
void WorldSession::HandleCalendarGetCalendar(WorldPacket& /*recvData*/)
{
  ...
  uint32 relationTime =
      ...
      sWorld->getIntConfig(CONFIG_INSTANCE_RESET_TIME_HOUR) * HOUR; // set point in time (default 29.12.2005) + X hours
...
void WorldSession::HandleCalendarGuildFilter(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "CMSG_CALENDAR_GUILD_FILTER: Min level [{}], Max level [{}], Min rank [{}]", minLevel, maxLevel,
            minRank);
}
...
void WorldSession::HandleCalendarAddEvent(WorldPacket& recvData)
{
  ...
  CalendarEvent* calendarEvent =
      new CalendarEvent(sCalendarMgr->GetFreeEventId(), guid, 0, CalendarEventType(type), dungeonId,
  ...
  if (calendarEvent->IsGuildAnnouncement())
  {
    ...
    CalendarInvite* invite = new CalendarInvite(0, calendarEvent->GetEventId(), ObjectGuid::Empty, guid, 946684800,
  ...
  else
  {
    ...
    for (uint32 i = 0; i < inviteCount && i < CALENDAR_MAX_INVITES; ++i)
    {
      ...
      CalendarInvite* invite =
          new CalendarInvite(sCalendarMgr->GetFreeInviteId(), calendarEvent->GetEventId(), invitee[i], guid, 946684800,
                             CalendarInviteStatus(status[i]), CalendarModerationRank(rank[i]), "");
...
void WorldSession::HandleCalendarUpdateEvent(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network",
            ...
            guid.ToString(), eventId, inviteId, title, description, type, repetitionType, maxInvites, dungeonId,
...
void WorldSession::HandleCalendarCopyEvent(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "CMSG_CALENDAR_COPY_EVENT [{}], EventId [{}] inviteId [{}] Time: {}", guid.ToString(), eventId,
            inviteId, eventTime);
...
void WorldSession::HandleCalendarEventInvite(WorldPacket& recvData)
{
  ...
  // xinef: sync query
  if (QueryResult result = CharacterDatabase.Query("SELECT flags FROM character_social WHERE guid = {} AND friend = {}",
                                                   inviteeGuid.GetCounter(), playerGuid.GetCounter()))
  {
  ...
  if (!isPreInvite)
  {
    if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId))
    {
      ...
      // 946684800 is 01/01/2000 00:00:00 - default response time
      CalendarInvite* invite = new CalendarInvite(sCalendarMgr->GetFreeInviteId(), eventId, inviteeGuid, playerGuid,
  ...
  else
  {
    ...
    // 946684800 is 01/01/2000 00:00:00 - default response time
    CalendarInvite* invite = new CalendarInvite(inviteId, 0, inviteeGuid, playerGuid, 946684800,
...
void WorldSession::HandleCalendarEventSignup(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "CMSG_CALENDAR_EVENT_SIGNUP [{}] EventId [{}] Tentative {}", guid.ToString(), eventId,
            tentative);
  ...
  if (CalendarEvent* calendarEvent = sCalendarMgr->GetEvent(eventId))
  {
    ...
    CalendarInvite*      invite = new CalendarInvite(sCalendarMgr->GetFreeInviteId(), eventId, guid, guid,
                                                     GameTime::GetGameTime().count(), status, CALENDAR_RANK_PLAYER, "");
    sCalendarMgr->AddInvite(calendarEvent, invite);
...
void WorldSession::HandleCalendarEventRsvp(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "CMSG_CALENDAR_EVENT_RSVP [{}] EventId [{}], InviteId [{}], status {}", guid.ToString(), eventId,
            inviteId, status);
...
void WorldSession::HandleCalendarComplain(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "CMSG_CALENDAR_COMPLAIN [{}] EventId [{}] guid [{}]", guid.ToString(), eventId,
...
void WorldSession::SendCalendarRaidLockoutUpdated(InstanceSave const* save, bool isExtended)
{
  ...
  data << uint32(save->GetMapId());
  data << uint32(save->GetDifficulty());
  data << uint32(resetTimeOp >= currTime ? resetTimeOp - currTime
