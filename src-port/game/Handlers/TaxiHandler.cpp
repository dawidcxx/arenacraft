...
void WorldSession::SendTaxiStatus(ObjectGuid guid)
{
  ...
  // find taxi node
  uint32 nearest = sObjectMgr->GetNearestTaxiNode(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ(),
                                                  unit->GetMapId(), player->GetTeamId());
  if (!nearest)
  {
    return;
  }
...
void WorldSession::HandleTaxiQueryAvailableNodes(WorldPacket& recvData)
{
  ...
  if (!unit)
  {
    LOG_DEBUG("network", "WORLD: HandleTaxiQueryAvailableNodes - Unit ({}) not found or you can't interact with him.",
              guid.ToString());
    return;
  }
...
void WorldSession::SendTaxiMenu(Creature* unit)
{
  ...
  if (unit->GetEntry() == 29480)
    GetPlayer()->SetTaxiCheater(true); // Grimwing in Ebon Hold, special case. NOTE: Not perfect, Zul'Aman should not be
                                       // included according to WoWhead, and I think taxicheat includes it.
...
void WorldSession::HandleActivateTaxiExpressOpcode(WorldPacket& recvData)
{
  ...
  if (!npc)
  {
    LOG_DEBUG("network", "WORLD: HandleActivateTaxiExpressOpcode - Unit ({}) not found or you can't interact with it.",
              guid.ToString());
...
void WorldSession::HandleActivateTaxiOpcode(WorldPacket& recvData)
{
  ...
  if (!npc)
  {
    LOG_DEBUG("network", "WORLD: HandleActivateTaxiOpcode - Unit ({}) not found or you can't interact with it.",
              guid.ToString());
    SendActivateTaxiReply(ERR_TAXITOOFARAWAY);
    return;
  }
