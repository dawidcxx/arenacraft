...
void WorldSession::HandleAutostoreLootItemOpcode(WorldPacket& recvData)
{
  ...
  if (lguid.IsGameObject())
  {
    ...
    // if (player->GetGroup() && player->GetGroup()->GetLootMethod() == MASTER_LOOT && player->GetGUID() !=
  ...
  if (lguid.IsGameObject())
  {
    ...
    else if (loot->isLooted() || go->GetGoType() == GAMEOBJECT_TYPE_FISHINGNODE)
    {
      ...
      else
      {
        ...
        // Xinef: prevents exploits with just opening GO and spawning bilions of npcs, which can crash core if you know
        // what you're doin ;)
        if (go->GetGoType() == GAMEOBJECT_TYPE_CHEST && go->GetGOInfo()->chest.eventId)
        {
          LOG_DEBUG("spells.aura", "Chest ScriptStart id {} for GO {}", go->GetGOInfo()->chest.eventId,
                    go->GetSpawnId());
...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "WorldSession::HandleLootMasterGiveOpcode (CMSG_LOOT_MASTER_GIVE, 0x02A3) Target = [{}].",
            target->GetName());
  ...
  if (!_player->IsInRaidWith(target))
  {
    ...
    // LOG_DEBUG("network", "MasterLootItem: Player {} tried to give an item to ineligible player {} !",
    // GetPlayer()->GetName(), target->GetName());
    return;
  }
  ...
  {
    LOG_DEBUG("loot", "MasterLootItem: Player {} might be using a hack! (slot {}, size {})", GetPlayer()->GetName(),
              slotid, (unsigned long)loot->items.size());
    return;
  }
