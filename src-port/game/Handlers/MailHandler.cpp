...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
  ...
  if (!receiverGuid)
  {
    LOG_DEBUG("network.opcode",
              "Player {} is sending mail to {} (GUID: not existed!) with subject {} and body {} includes {} items, {} "
              "copper and {} COD copper with unk1 = {}, unk2 = {}",
  ...
  LOG_DEBUG("network.opcode",
            "Player {} is sending mail to {} ({}) with subject {} and body {} includes {} items, {} copper and {} COD "
            "copper with unk1 = {}, unk2 = {}",
            player->GetGUID().ToString(), receiver, receiverGuid.ToString(), subject, body, items_count, money, COD,
            unk1, unk2);
  ...
  if (items_count > 0 || money > 0)
  {
    if (items_count > 0)
    {
      for (uint8 i = 0; i < items_count; ++i)
      {
        ...
        item->SaveToDB(trans); // recursive and not have transaction guard into self, item not in inventory and can be
                               // save standalone
    ...
    if (money >= 10 * GOLD)
    {
      ...
      CharacterDatabase.Execute(
          "INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", {}, \"{}\", NOW(), {})", GetAccountId(),
          player->GetGUID().GetCounter(), player->GetName(), player->GetSession()->GetRemoteAddress(), rc_account,
          receiver, money, subject, 5);
    }
  }
  ...
  draft.AddMoney(money).AddCOD(COD).SendMailTo(
      trans, MailReceiver(receive, receiverGuid.GetCounter()), MailSender(player),
      body.empty() ? MAIL_CHECK_MASK_COPIED : MAIL_CHECK_MASK_HAS_BODY, deliver_delay);
...
void WorldSession::HandleMailReturnToSender(WorldPacket& recvData)
{
  ...
  if (m->HasItems())
  {
    for (MailItemInfoVec::iterator itr = m->items.begin(); itr != m->items.end(); ++itr)
    {
      ...
      if (item && !sScriptMgr->CanSendMail(player, ObjectGuid(HighGuid::Player, m->sender), mailbox, m->subject,
                                           m->body, m->money, m->COD, item))
      {
        player->SendMailResult(mailId, MAIL_RETURNED_TO_SENDER, MAIL_ERR_INTERNAL_ERROR);
        return;
      }
  ...
  else if (!sScriptMgr->CanSendMail(player, ObjectGuid(HighGuid::Player, m->sender), mailbox, m->subject, m->body,
                                    m->money, m->COD, nullptr))
  {
    player->SendMailResult(mailId, MAIL_RETURNED_TO_SENDER, MAIL_ERR_INTERNAL_ERROR);
    return;
  }
  ...
  if (msg == EQUIP_ERR_OK)
  {
    ...
    if (m->COD > 0) // if there is COD, take COD money from player and send them to sender by mail
    {
      ...
      if (sender || sender_accId)
      {
        MailDraft(m->subject, "")
            .AddMoney(m->COD)
            .SendMailTo(trans, MailReceiver(sender, m->sender), MailSender(MAIL_NORMAL, m->receiver),
        ...
        if (m->COD >= 10 * GOLD)
        {
          ...
          CharacterDatabase.Execute(
              "INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", {}, \"{}\", NOW(), {})", GetAccountId(),
              player->GetGUID().GetCounter(), player->GetName(), player->GetSession()->GetRemoteAddress(), sender_accId,
              senderName, m->COD, subj, 1);
        }
      }
