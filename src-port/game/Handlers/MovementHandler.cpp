...
void WorldSession::HandleMoveWorldportAck()
{
  ...
  {
    LOG_ERROR("network.opcode", "Player (Name {}) is still in world when teleported from map {} to new map {}",
              GetPlayer()->GetName(), oldMap->GetId(), loc.GetMapId());
  ...
  if (!GetPlayer()->m_InstanceValid && !mInstance)
  {
    ...
    // pussywizard: m_InstanceValid can be false only by leaving a group in an instance => so remove temp binds that
    // could not be removed because player was still on the map!
    if (!sInstanceSaveMgr->PlayerIsPermBoundToInstance(GetPlayer()->GetGUID(), oldMap->GetId(),
  ...
  {
    LOG_ERROR("network.opcode", "Map {} could not be created for player {}, porting player to homebind", loc.GetMapId(),
              GetPlayer()->GetGUID().ToString());
    GetPlayer()->TeleportTo(GetPlayer()->m_homebindMapId, GetPlayer()->m_homebindX, GetPlayer()->m_homebindY,
                            GetPlayer()->m_homebindZ, GetPlayer()->GetOrientation());
    return;
  }
...
void WorldSession::HandleMoveTeleportAck(WorldPacket& recvData)
{
  ...
  if (Pet* pet = plMover->GetPet())
  {
    if (!pet->IsWithinDist3d(plMover, plMover->GetMap()->GetVisibilityRange() - 5.0f))
      pet->NearTeleportTo(plMover->GetPositionX(), plMover->GetPositionY(), plMover->GetPositionZ(),
                          pet->GetOrientation());
  }
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
  ...
  if (plrMover && ((movementInfo.flags & MOVEMENTFLAG_SWIMMING) != 0) != plrMover->IsInWater())
  {
    ...
    plrMover->SetInWater(!plrMover->IsInWater() ||
                         plrMover->GetMap()->IsUnderWater(
                             plrMover->GetPhaseMask(), movementInfo.pos.GetPositionX(), movementInfo.pos.GetPositionY(),
                             movementInfo.pos.GetPositionZ(), plrMover->GetCollisionHeight()));
  }
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket& recvData)
{
  ...
  static char const* move_type_name[MAX_MOVE_TYPE] = {"Walk",     "Run",    "RunBack",    "Swim",     "SwimBack",
...
void WorldSession::HandleTimeSyncResp(WorldPacket& recvData)
{
  ...
  // SMSG_TIME_SYNC_REQUEST packet. clientTime:  time that was displayed on the clock of the CLIENT at the moment when
  // the client processed the SMSG_TIME_SYNC_REQUEST packet.

  // Once clockDelta has been computed, we can compute the time of an event on server clock when we know the time of
  // that same event on the client clock, using the following relation: serverTime = clockDelta + clientTime
