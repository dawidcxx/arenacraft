...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recv_data)
{
  ...
  if (!code.empty())
  {
    if (unit)
    {
      ...
      if (!sScriptMgr->OnGossipSelectCode(_player, unit, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
    ...
    else if (go)
    {
      ...
      sScriptMgr->OnGossipSelectCode(_player, go, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
                                     _player->PlayerTalkClass->GetGossipOptionAction(gossipListId), code.c_str());
    }
    else if (item)
    {
      sScriptMgr->OnGossipSelectCode(_player, item, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
                                     _player->PlayerTalkClass->GetGossipOptionAction(gossipListId), code.c_str());
    }
    else
    {
      sScriptMgr->OnGossipSelectCode(_player, menuId, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
                                     _player->PlayerTalkClass->GetGossipOptionAction(gossipListId), code.c_str());
    }
  }
  else
  {
    if (unit)
    {
      ...
      if (!sScriptMgr->OnGossipSelect(_player, unit, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
    ...
    else if (go)
    {
      ...
      if (!sScriptMgr->OnGossipSelect(_player, go, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
                                      _player->PlayerTalkClass->GetGossipOptionAction(gossipListId)))
        _player->OnGossipSelect(go, gossipListId, menuId);
    }
    else if (item)
    {
      sScriptMgr->OnGossipSelect(_player, item, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
                                 _player->PlayerTalkClass->GetGossipOptionAction(gossipListId));
    }
    else
    {
      sScriptMgr->OnGossipSelect(_player, menuId, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId),
                                 _player->PlayerTalkClass->GetGossipOptionAction(gossipListId));
    }
  }
}
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
  ...
  for (auto const& target : sWhoListCacheMgr->GetWhoList())
  {
    ...
    for (uint32 i = 0; i < strCount; ++i)
    {
      if (!str[i].empty())
      {
        if (wideguildname.find(str[i]) != std::wstring::npos || wideplayername.find(str[i]) != std::wstring::npos ||
...
void WorldSession::HandleTogglePvP(WorldPacket& recv_data)
{
  ...
  else if (!GetPlayer()->pvpInfo.IsHostile && GetPlayer()->IsPvP()) // pussywizard: in pvp mode, but doesn't want to be
...
void WorldSession::HandleUpdateAccountData(WorldPacket& recv_data)
{
  ...
  if (uncompress(dest.contents(), &realSize, recv_data.contents() + recv_data.rpos(),
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recv_data)
{
  ...
  {
    if (ActionBar != 0)
      LOG_ERROR("network.opcode", "WorldSession::HandleSetActionBarToggles in not logged state with value: {}, ignored",
                uint32(ActionBar));
    return;
  }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recv_data)
{
  ...
  {
    LOG_DEBUG("network", "Player '{}' ({}) in flight, ignore worldport command.", GetPlayer()->GetName(),
              GetPlayer()->GetGUID().ToString());
    return;
  }

  LOG_DEBUG("network", "CMSG_WORLD_TELEPORT: Player = {}, Time = {}, map = {}, x = {}, y = {}, z = {}, o = {}",
            GetPlayer()->GetName(), time, mapid, PositionX, PositionY, PositionZ, Orientation);
...
void WorldSession::HandleComplainOpcode(WorldPacket& recv_data)
{
  ...
  LOG_DEBUG("network", "REPORT SPAM: type {}, {}, unk1 {}, unk2 {}, unk3 {}, unk4 {}, message {}", spam_type,
            spammer_guid.ToString(), unk1, unk2, unk3, unk4, description);
}
...
void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
  ...
  if (apply)
  {
    LOG_DEBUG("network", "Added FarSight {} to player {}", _player->GetGuidValue(PLAYER_FARSIGHT).ToString(),
              _player->GetGUID().ToString());
    if (WorldObject* target = _player->GetViewpoint())
      _player->SetSeer(target);
    else
      LOG_DEBUG("network.opcode", "Player {} requests non-existing seer {}", _player->GetName(),
  ...
  else
  {
    ...
    {
      LOG_DEBUG("network", "Player {} set vision to old farsight {}", _player->GetGUID().ToString(),
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket& recv_data)
{
  ...
  if (group)
  {
    if (group->IsLeader(_player->GetGUID()))
    {
      ...
      if (uint32 preventionTime = group->GetDifficultyChangePreventionTime())
      {
        ...
        case DIFFICULTY_PREVENTION_CHANGE_BOSS_KILLED:
          ChatHandler(this).PSendSysMessage(
              "Raid was in combat recently and may not change difficulty again for {} sec.", preventionTime);
          break;
        case DIFFICULTY_PREVENTION_CHANGE_RECENTLY_CHANGED:
        default:
          ChatHandler(this).PSendSysMessage(
              "Raid difficulty has changed recently, and may not change again for {} sec.", preventionTime);
          break;
        }
      ...
      for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
      {
            ...
            (uint32(mode % 2) == uint32(_player->GetRaidDifficulty() % 2)) && group->isRaidGroup())
        {
          ...
          if (!groupGuy->IsAlive() || groupGuy->IsInCombat() || groupGuy->GetVictim() ||
              ...
              !groupGuy->Satisfy(sObjectMgr->GetAccessRequirement(groupGuy->GetMap()->GetId(), Difficulty(mode)),
      ...
      if (currMap)
        for (Map::PlayerList::const_iterator itr = currMap->GetPlayers().begin(); itr != currMap->GetPlayers().end();
             ++itr)
          if (Player* p = itr->GetSource())
            if (p->GetGroup() != group)
            {
              if (!p->IsInWorld() || !p->IsAlive() || p->IsInCombat() || p->GetVictim() || p->m_mover != p ||
      ...
      for (std::map<Player*, Position>::iterator itr = playerTeleport.begin(); itr != playerTeleport.end(); ++itr)
      {
        ...
        if (!itr->first->TeleportTo(*(foundMaps.begin()), itr->second.GetPositionX(), itr->second.GetPositionY(),
                                    itr->second.GetPositionZ(), itr->second.GetOrientation()))
