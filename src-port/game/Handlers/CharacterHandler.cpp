...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
  ...
  if (raceEntry->expansion > Expansion())
  {
    ...
    LOG_ERROR("network.opcode", "Expansion {} account:[{}] tried to Create character with expansion {} race ({})",
              Expansion(), GetAccountId(), raceEntry->expansion, createInfo->Race);
    return;
  }
  ...
  if (classEntry->expansion > Expansion())
  {
    ...
    LOG_ERROR("network.opcode", "Expansion {} account:[{}] tried to Create character with expansion {} class ({})",
              Expansion(), GetAccountId(), classEntry->expansion, createInfo->Class);
    return;
  }
              ...
              [this, createInfo](QueryCallback& queryCallback, PreparedQueryResult result)
              {
                if (result)
                {
                  ...
                  createInfo->CharCount = uint8(
                      fields[0]
                          .Get<uint64>()); // SQL's COUNT() returns uint64 but it will always be less than uint8.Max
                    ...
                    [this, createInfo](PreparedQueryResult result)
                {
                  ...
                  uint32 heroicReqLevel =
                      sWorld->getIntConfig(CONFIG_CHARACTER_CREATING_MIN_LEVEL_FOR_HEROIC_CHARACTER);
                  ...
                  AddTransactionCallback(CharacterDatabase.AsyncCommitTransaction(characterTransaction))
                      .AfterComplete(
                          [this, newChar = std::move(newChar)](bool success)
                          {
                            if (success)
                            {
                              LOG_INFO("entities.player.character", "Account: {} (IP: {}) Create Character: {} {}",
                                       GetAccountId(), GetRemoteAddress(), newChar->GetName(),
                                       newChar->GetGUID().ToString());
                              sScriptMgr->OnPlayerCreate(newChar.get());
                              sCharacterCache->AddCharacterCacheEntry(
                                  newChar->GetGUID(), GetAccountId(), newChar->GetName(), newChar->getGender(),
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
  ...
  LOG_INFO("entities.player.character", "Account: {}, IP: {} deleted character: {}, {}, Level: {}", accountId,
           GetRemoteAddress(), name, guid.ToString(), level);

  // To prevent hook failure, place hook before removing reference from DB
  sScriptMgr->OnPlayerDelete(guid, initAccountId); // To prevent race conditioning, but as it also makes sense, we hand
                                                   // the accountId over for successful delete.
  ...
  if (WorldSession* sess = sWorld->FindOfflineSession(GetAccountId()))
  {
    ...
    else
    {
      ...
      uint8 limitA = 10, limitB = 10,
            limitC = 10; // pussywizard: this somehow froze (probably, ahh crash logs ...), and while (far) have never
                         // frozen in LogoutPlayer o_O maybe it's the combination of while(far); while(near);
...
void WorldSession::HandlePlayerLoginFromDB(LoginQueryHolder const& holder)
{
  ...
  // "GetAccountId() == db stored account id" checked in LoadFromDB (prevent login not own character using cheating
  // tools)
  if (!pCurrChar->LoadFromDB(playerGuid, holder))
  {
    SetPlayer(nullptr);
    KickPlayer(
        "WorldSession::HandlePlayerLogin Player::LoadFromDB failed"); // disconnect client, player no set to session and
                                                                      // it will not deleted or saved at kick
  ...
  if (!pCurrChar->GetMap()->AddPlayerToMap(pCurrChar) || !pCurrChar->CheckInstanceLoginValid())
  {
    ...
    if (at)
      ...
      pCurrChar->TeleportTo(pCurrChar->m_homebindMapId, pCurrChar->m_homebindX, pCurrChar->m_homebindY,
                            pCurrChar->m_homebindZ, pCurrChar->GetOrientation());

    // Probably a hackfix, but currently the best workaround to prevent character names showing as Unknown after
    // teleport out from instances at login.
  ...
  if (MapDifficulty const* mapDiff =
    ...
    if (mapDiff->resetTime)
      if (time_t timeReset =
              sInstanceSaveMgr->GetResetTimeFor(pCurrChar->GetMap()->GetId(), pCurrChar->GetMap()->GetDifficulty()))
      {
        uint32 timeleft = uint32(timeReset - GameTime::GetGameTime().count());
        pCurrChar->SendInstanceResetWarning(pCurrChar->GetMap()->GetId(), pCurrChar->GetMap()->GetDifficulty(),
                                            timeleft, true);
      }
  ...
  if (pCurrChar->m_deathState != DeathState::Alive)
  {
    ...
    if (pCurrChar->getRace() == RACE_NIGHTELF)
      pCurrChar->CastSpell(
          ...
          0); // auras SPELL_AURA_INCREASE_SPEED(+speed in wisp form), SPELL_AURA_INCREASE_SWIM_SPEED(+swim speed in
  ...
  LOG_INFO("entities.player", "Account: {} (IP: {}) Login Character:[{}] ({}) Level: {}", GetAccountId(), IP_str,
           pCurrChar->GetName(), pCurrChar->GetGUID().ToString(), pCurrChar->GetLevel());
  ...
  // pussywizard: on login it's not possible to go back to arena as a spectator, HandleMoveWorldportAckOpcode is not
  // sent, so call it here
...
void WorldSession::HandlePlayerLoginToCharInWorld(Player* pCurrChar)
{
  ...
  if (MapDifficulty const* mapDiff =
    ...
    if (mapDiff->resetTime)
      ...
      {
        uint32 timeleft = uint32(timeReset - GameTime::GetGameTime().count());
        GetPlayer()->SendInstanceResetWarning(pCurrChar->GetMap()->GetId(), pCurrChar->GetMap()->GetDifficulty(),
                                              timeleft, true);
      }
...
void WorldSession::HandlePlayerLoginToCharOutOfWorld(Player* /*pCurrChar*/) { ABORT(); }
...
void WorldSession::HandleCharRenameCallBack(std::shared_ptr<CharacterRenameInfo> renameInfo, PreparedQueryResult result)
{
  ...
  LOG_INFO("entities.player.character", "Account: {} (IP: {}), Character [{}] (guid: {}) Changed name to: {}",
           GetAccountId(), GetRemoteAddress(), oldName, guidLow, renameInfo->Name);
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
  ...
  if (!IsLegitCharacterForAccount(customizeInfo->Guid))
  {
    LOG_ERROR("entities.player.cheat",
              "Account {}, IP: {} tried to customise {}, but it does not belong to their account!", GetAccountId(),
              GetRemoteAddress(), customizeInfo->Guid.ToString());
    recvData.rfinish();
                                               ...
                                               PreparedQueryResult                     result)
{
  ...
  LOG_INFO("entities.player.character", "Account: {} (IP: {}), Character[{}] ({}) Customized to: {}", GetAccountId(),
           GetRemoteAddress(), oldName, customizeInfo->Guid.ToString(), customizeInfo->Name);
}
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
  ...
  _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
      std::bind(&WorldSession::HandleCharFactionOrRaceChangeCallback, this, factionChangeInfo, std::placeholders::_1)));
}
                                                         ...
                                                         PreparedQueryResult                         result)
{
  ...
  // xinef: update global data
  sCharacterCache->UpdateCharacterData(factionChangeInfo->Guid, factionChangeInfo->Name, factionChangeInfo->Gender,
  ...
  if (oldRace != factionChangeInfo->Race)
  {
    ...
    if (factionChangeInfo->FactionChange)
    {
      ...
      for (auto const& [reputation_alliance, reputation_horde] : sObjectMgr->FactionChangeReputation)
      {
        ...
        // new base reputation
        int32 newBaseRep = sObjectMgr->GetBaseReputationOf(sFactionStore.LookupEntry(newReputation),
                                                           factionChangeInfo->Race, playerClass);
  ...
  LOG_DEBUG("entities.player", "{} (IP: {}) changed race from {} to {}", GetPlayerInfo(), GetRemoteAddress(), oldRace,
            factionChangeInfo->Race);
