...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket& recvData)
{
  ...
  if (!creature)
  {
    LOG_DEBUG("network", "WORLD: HandlePetitionBuyOpcode - Unit ({}) not found or you can't interact with him.",
              guidNPC.ToString());
    return;
  }
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& recvData)
{
  ...
  // solve (possible) some strange compile problems with explicit use petition low guid at some GCC versions (wrong code
  // optimization in compiler?)
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& recvData)
{
  ...
  if (!petition)
  {
    LOG_ERROR("network.opcode", "Petition {} is not found for player {} (Name: {})", petitionGuid.ToString(),
              GetPlayer()->GetGUID().ToString(), GetPlayer()->GetName());
    return;
  }
  ...
  if (type != GUILD_CHARTER_TYPE)
  {
    ...
    if (_player->GetLevel() < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
    {
      SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", _player->GetName().c_str(),
    ...
    {
      SendArenaTeamCommandResult(ERR_ARENA_TEAM_INVITE_SS, "", _player->GetName().c_str(),
  ...
  LOG_DEBUG("network", "PETITION SIGN: {} by player: {} ({}, Account: {})", petitionGuid.ToString(), _player->GetName(),
            playerGuid.ToString(), GetAccountId());
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket& recvData)
{
  ...
  if (petition->petitionType != GUILD_CHARTER_TYPE)
  {
    ...
    if (player->GetLevel() < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
    {
      ...
      SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, player->GetName().c_str(), "",
                                 ERR_ARENA_TEAM_TARGET_TOO_LOW_S);
      return;
    }
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket& recvData)
{
  ...
  else
  {
    ...
    if (signs)
      for (SignatureMap::const_iterator itr = signatureCopy.begin(); itr != signatureCopy.end(); ++itr)
      {
        LOG_DEBUG("network", "PetitionsHandler: Adding arena team (guid: {}) member {}", arenaTeam->GetId(),
                  itr->first.ToString());
...
void WorldSession::SendPetitionShowList(ObjectGuid guid)
{
  ...
  if (!creature)
  {
    LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - Unit ({}) not found or you can't interact with him.",
              guid.ToString());
    return;
  }
