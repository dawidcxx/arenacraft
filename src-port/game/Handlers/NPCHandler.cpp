...
void WorldSession::SendTrainerList(ObjectGuid guid, const std::string& strTitle)
{
  ...
  if (!unit)
  {
    LOG_DEBUG("network", "WORLD: SendTrainerList - Unit ({}) not found or you can not interact with him.",
              guid.ToString());
    return;
  }
  ...
  uint32 count = 0;
  for (TrainerSpellMap::const_iterator itr = trainer_spells->spellList.begin(); itr != trainer_spells->spellList.end();
       ++itr)
  {
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvData)
{
  ...
  if (!unit)
  {
    LOG_DEBUG("network", "WORLD: HandleTrainerBuySpellOpcode - Unit ({}) not found or you can not interact with him.",
              guid.ToString());
    return;
  }
...
void WorldSession::HandleGossipHelloOpcode(WorldPacket& recvData)
{
  ...
  if (!unit)
  {
    LOG_DEBUG("network", "WORLD: HandleGossipHelloOpcode - Unit ({}) not found or you can not interact with him.",
              guid.ToString());
    return;
  }
...
/*void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket & recvData)
{
    ...
    if (!unit)
    {
        LOG_DEBUG("network.opcode", "WORLD: HandleGossipSelectOptionOpcode - Unit ({}) not found or you can't interact
with him.", guid.ToString()); return;
    }
    ...
    if (!code.empty())
    {
        if (!Script->GossipSelectWithCode(_player, unit, _player->PlayerTalkClass->GossipOptionSender (option),
_player->PlayerTalkClass->GossipOptionAction(option), code.c_str())) unit->OnGossipSelect (_player, option);
    }
    else
    {
        if (!Script->OnGossipSelect (_player, unit, _player->PlayerTalkClass->GossipOptionSender (option),
_player->PlayerTalkClass->GossipOptionAction (option))) unit->OnGossipSelect (_player, option);
    }
}*/
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket& recvData)
{
  ...
  if (!unit)
  {
    LOG_DEBUG("network", "WORLD: HandleBinderActivateOpcode - Unit ({}) not found or you can not interact with him.",
              npcGUID.ToString());
    return;
  }
...
void WorldSession::HandleUnstablePet(WorldPacket& recvData)
{
  ...
  auto stabledPet =
      std::find_if(petStable->StabledPets.begin(), petStable->StabledPets.end(),
  ...
  if (oldPet)
  {
    ...
    _player->RemovePet(
        oldPet, PetSaveMode(PET_SAVE_FIRST_STABLE_SLOT + std::distance(petStable->StabledPets.begin(), stabledPet)));
  }
...
void WorldSession::HandleRepairItemOpcode(WorldPacket& recvData)
{
  ...
  if (!unit)
  {
    LOG_DEBUG("network", "WORLD: HandleRepairItemOpcode - Unit ({}) not found or you can not interact with him.",
              npcGUID.ToString());
    return;
  }
