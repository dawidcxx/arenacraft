...
void World::LoadConfigSettings(bool reload)
{
  ...
  {
    LOG_ERROR("server.loading", "Rate.Rage.Loss ({}) must be > 0. Using 1 instead.",
  ...
  {
    LOG_ERROR("server.loading", "Rate.RunicPower.Loss ({}) must be > 0. Using 1 instead.",
  ...
  {
    LOG_ERROR("server.loading", "DurabilityLoss.OnDeath ({}) must be >=0. Using 0.0 instead.",
  ...
  {
    LOG_ERROR("server.loading", "DurabilityLoss.OnDeath ({}) must be <= 100. Using 100.0 instead.",
              _rate_values[RATE_DURABILITY_LOSS_ON_DEATH]);
    _rate_values[RATE_DURABILITY_LOSS_ON_DEATH] = 0.0f;
  }
  ...
  {
    LOG_ERROR("server.loading", "DurabilityLossChance.Damage ({}) must be >=0. Using 0.0 instead.",
  ...
  {
    LOG_ERROR("server.loading", "DurabilityLossChance.Absorb ({}) must be >=0. Using 0.0 instead.",
  ...
  {
    LOG_ERROR("server.loading", "DurabilityLossChance.Parry ({}) must be >=0. Using 0.0 instead.",
  ...
  {
    LOG_ERROR("server.loading", "DurabilityLossChance.Block ({}) must be >=0. Using 0.0 instead.",
  ...
  {
    LOG_ERROR("server.loading", "Compression level ({}) must be in range 1..9. Using default compression level (1).",
  ...
  {
    LOG_ERROR("server.loading", "MapUpdateInterval ({}) must be greater {}. Use this minimal value.",
  ...
  {
    LOG_ERROR("server.loading", "MinPlayerName ({}) must be in range 1..{}. Set to 2.",
  ...
  {
    LOG_ERROR("server.loading", "MinCharterName ({}) must be in range 1..{}. Set to 2.",
  ...
  {
    LOG_ERROR("server.loading", "MinPetName ({}) must be in range 1..{}. Set to 2.", _int_configs[CONFIG_MIN_PET_NAME],
              MAX_PET_NAME);
  ...
  {
    LOG_ERROR("server.loading", "CharactersPerRealm ({}) must be in range 1..10. Set to 10.",
  ...
  {
    LOG_ERROR("server.loading", "CharactersPerAccount ({}) can't be less than CharactersPerRealm ({}).",
  ...
  {
    LOG_ERROR("server.loading", "HeroicCharactersPerRealm ({}) must be in range 0..10. Set to 1.",
  ...
  {
    LOG_ERROR("server.loading", "SkipCinematics ({}) must be in range 0..2. Set to 0.",
  ...
  if (reload)
  {
    ...
    if (val != _int_configs[CONFIG_MAX_PLAYER_LEVEL])
      LOG_ERROR("server.loading", "MaxPlayerLevel option can't be changed at config reload, using current value ({}).",
  ...
  {
    LOG_ERROR("server.loading", "MaxPlayerLevel ({}) must be in range 1..{}. Set to {}.",
              _int_configs[CONFIG_MAX_PLAYER_LEVEL], MAX_LEVEL, MAX_LEVEL);
  ...
  {
    LOG_ERROR("server.loading", "StartPlayerLevel ({}) must be in range 1..MaxPlayerLevel({}). Set to 1.",
              _int_configs[CONFIG_START_PLAYER_LEVEL], _int_configs[CONFIG_MAX_PLAYER_LEVEL]);
  ...
  {
    LOG_ERROR("server.loading", "StartPlayerMoney ({}) must be in range 0..{}. Set to {}.",
              _int_configs[CONFIG_START_PLAYER_MONEY], MAX_MONEY_AMOUNT, 0);
  ...
  {
    LOG_ERROR("server.loading", "StartHeroicPlayerMoney ({}) must be in range 0..{}. Set to {}.",
              _int_configs[CONFIG_START_HEROIC_PLAYER_MONEY], MAX_MONEY_AMOUNT, 2000);
  ...
  {
    LOG_ERROR("server.loading", "MaxHonorPoints ({}) can't be negative. Set to 0.",
  ...
  {
    LOG_ERROR("server.loading", "MaxHonorPointsMoneyPerPoint ({}) can't be negative. Set to 0.",
              _int_configs[CONFIG_MAX_HONOR_POINTS_MONEY_PER_POINT]);
  ...
  {
    LOG_ERROR("server.loading", "MaxArenaPoints ({}) can't be negative. Set to 0.",
  ...
  {
    LOG_ERROR("server.loading", "MinPetitionSigns ({}) must be in range 0..9. Set to 9.",
  ...
  if (_int_configs[CONFIG_START_GM_LEVEL] < _int_configs[CONFIG_START_PLAYER_LEVEL])
  {
    LOG_ERROR("server.loading", "GM.StartLevel ({}) must be in range StartPlayerLevel({})..{}. Set to {}.",
              _int_configs[CONFIG_START_GM_LEVEL], _int_configs[CONFIG_START_PLAYER_LEVEL], MAX_LEVEL,
  ...
  {
    LOG_ERROR("server.loading", "GM.StartLevel ({}) must be in range 1..{}. Set to {}.",
              _int_configs[CONFIG_START_GM_LEVEL], MAX_LEVEL, MAX_LEVEL);
  ...
  {
    LOG_ERROR("server.loading", "UpdateUptimeInterval ({}) must be > 0, set to default 10.",
  ...
  {
    LOG_ERROR("server.loading", "LogDB.Opt.ClearInterval ({}) must be > 0, set to default 10.",
  ...
  // note: disable value (-1) will assigned as 0xFFFFFFF, to prevent overflow at calculations limit it to max possible
  ...
  {
    LOG_ERROR("server.loading", "Battleground.Random.ResetHour ({}) can't be load. Set to 6.",
  ...
  {
    LOG_ERROR("server.loading", "Calendar.DeleteOldEventsHour ({}) can't be load. Set to 6.",
              _int_configs[CONFIG_CALENDAR_DELETE_OLD_EVENTS_HOUR]);
  ...
  {
    LOG_ERROR("server.loading", "Battleground.ReportAFK ({}) must be >0. Using 3 instead.",
  ...
  {
    LOG_ERROR("server.loading", "Battleground.ReportAFK ({}) must be <10. Using 3 instead.",
              _int_configs[CONFIG_BATTLEGROUND_REPORT_AFK]);
    _int_configs[CONFIG_BATTLEGROUND_REPORT_AFK] = 3;
  }
  ...
  {
    LOG_ERROR("server.loading", "Battleground.PlayerRespawn ({}) must be >2. Using 30 instead.",
  ...
  {
    LOG_ERROR("server.loading", "Battleground.RestorationBuffRespawn ({}) must be > 0. Using 20 instead.",
              _int_configs[CONFIG_BATTLEGROUND_RESTORATION_BUFF_RESPAWN]);
  ...
  {
    LOG_ERROR("server.loading", "Battleground.BerserkingBuffRespawn ({}) must be > 0. Using 120 instead.",
              _int_configs[CONFIG_BATTLEGROUND_BERSERKING_BUFF_RESPAWN]);
  ...
  {
    LOG_ERROR("server.loading", "Battleground.SpeedBuffRespawn ({}) must be > 0. Using 150 instead.",
              _int_configs[CONFIG_BATTLEGROUND_SPEED_BUFF_RESPAWN]);
  ...
  {
    LOG_ERROR("server.loading", "WaterBreath.Timer ({}) must be > 0. Using 180000 instead.",
  ...
  {
    LOG_ERROR("server.loading", "Visibility.Distance.Continents can't be less max aggro radius {}",
  ...
  {
    LOG_ERROR("server.loading", "Visibility.Distance.Instances can't be less max aggro radius {}",
              45 * sWorld->getRate(RATE_CREATURE_AGGRO));
  ...
  {
    LOG_ERROR("server.loading", "Visibility.Distance.BGArenas can't be less max aggro radius {}",
              45 * sWorld->getRate(RATE_CREATURE_AGGRO));
  ...
  if (!enableHeight)
    LOG_ERROR("server.loading", "VMap height checking disabled! Creatures movements and other various things WILL be "
                                "broken! Expect no support.");
  ...
  LOG_INFO("server.loading", "WORLD: VMap support included. LineOfSight:{}, getHeight:{}, indoorCheck:{} PetLOS:{}",
...
void World::SetInitialWorldSettings()
{
  ...
  if (!sConfigMgr->isDryRun())
  {
    ...
    if (!MapMgr::ExistMapAndVMap(0, -6240.32f, 331.033f) || !MapMgr::ExistMapAndVMap(0, -8949.95f, -132.493f) ||
        ...
        (_int_configs[CONFIG_EXPANSION] &&
         (!MapMgr::ExistMapAndVMap(530, 10349.6f, -6357.29f) || !MapMgr::ExistMapAndVMap(530, -3961.64f, -13931.2f))))
    {
      exit(1);
    }
  }
  ...
  LoginDatabase.Execute("UPDATE realmlist SET icon = {}, timezone = {} WHERE id = '{}'", server_type, realm_zone,
  ...
  // sObjectMgr->SetDBCLocaleIndex(GetDefaultDbcLocale());        // Get once for all the locale index of DBC language
  // (console/broadcasts) LOG_INFO("server.loading", ">> Localization Strings loaded in {} ms",
  // GetMSTimeDiffToNow(oldMSTime)); LOG_INFO("server.loading", " ");
  ...
  CharacterDatabase.Execute("UPDATE mail m LEFT JOIN mail_items mi ON m.id = mi.mail_id SET m.has_items=0 WHERE "
                            "m.has_items<>0 AND mi.mail_id IS NULL");
  ...
  LOG_INFO("server.loading",
           "Initializing SpellInfo Precomputed Data..."); // must be called after loading items, professions, spells and
  ...
  _timers[WUPDATE_PINGDB].SetInterval(getIntConfig(CONFIG_DB_PING_INTERVAL) * MINUTE *
...
void World::DetectDBCLang()
{
  ...
  LOG_INFO("server.loading", "Using {} DBC Locale As Default. All Available DBC locales: {}",
           localeNames[GetDefaultDbcLocale()], availableLocalsStr.empty() ? "<none>" : availableLocalsStr);
  LOG_INFO("server.loading", " ");
}
...
void World::ShutdownMsg(bool show, Player* player, const std::string& reason)
{
      ...
      (_shutdownTimer > 12 * HOUR && (_shutdownTimer % (12 * HOUR)) == 0))      // > 12 h ; every 12 h
  {
    ...
    LOG_DEBUG("server.worldserver", "Server is {} in {}",
              (_shutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"), str);
  }
}
...
void World::ShutdownCancel()
{
  ...
  LOG_DEBUG("server.worldserver", "Server {} cancelled.",
...
void World::SendServerMessage(ServerMessageType messageID, std::string stringParam /*= ""*/,
...
{
  {
    METRIC_DETAILED_NO_THRESHOLD_TIMER("world_update_time", METRIC_TAG("type", "Add sessions"),
...
void World::RemoveOldCorpses() { _timers[WUPDATE_CORPSES].SetCurrent(_timers[WUPDATE_CORPSES].GetInterval()); }
...
bool World::IsFFAPvPRealm() const { return getIntConfig(CONFIG_GAME_TYPE) == REALM_TYPE_FFA_PVP; }
...
CliCommandHolder::CliCommandHolder(void* callbackArg, char const* command, Print zprint,
