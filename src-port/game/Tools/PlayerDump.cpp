...
BaseTable const BaseTables[] = {{"character_pet", "id", "owner", GUID_TYPE_PET},
...
{
  return std::find_if(tableStruct.TableFields.begin(), tableStruct.TableFields.end(),
...
inline void MarkDependentColumn(TableStruct& tableStruct, std::string const& columnName, GuidType dependentType)
{
  ...
  if (itr == tableStruct.TableFields.end())
  {
    LOG_FATAL("server.loading",
              "Column `{}` declared in table `{}` marked as dependent but doesn't exist, PlayerDump will not work "
              "properly, please update table definitions",
...
{
  auto itr =
      std::find_if(CharacterTables.begin(), CharacterTables.end(), [baseTable](TableStruct const& tableStruct) -> bool
...
void PlayerDump::InitializeTables()
{
  ...
  for (DumpTable const& dumpTable : DumpTables)
  {
    ...
    case DTT_PET_TABLE:
      ...
      break;
    default:
      LOG_FATAL("server.loading", "Wrong dump table type {}, probably added a new table type without updating code",
  ...
  for (TableStruct const& tableStruct : CharacterTables)
  {
    ...
    {
      LOG_FATAL("server.loading", "Table `{}` defined in player dump doesn't have a WHERE query field",
...
inline bool ValidateFields(TableStruct const& ts, std::string const& str, std::size_t lineNumber)
{
  ...
  do
  {
    ...
    {
      LOG_ERROR("misc",
                "LoadPlayerDump: (line {}) unknown column name `{}` for table `{}`, aborting due to incompatible DB "
                "structure.",
                lineNumber, column, ts.TableName);
      return false;
    }
...
inline bool ChangeColumn(TableStruct const& ts, std::string& str, std::string const& column, std::string const& with,
...
bool PlayerDumpWriter::AppendTable(StringTransaction& trans, ObjectGuid::LowType guid, TableStruct const& tableStruct,
...
{
  dump = "IMPORTANT NOTE: THIS DUMPFILE IS MADE FOR USE WITH THE 'PDUMP' COMMAND ONLY - EITHER THROUGH INGAME CHAT OR "
         "ON CONSOLE!\n";
  dump += "IMPORTANT NOTE: DO NOT apply it directly - it will irreversibly DAMAGE and CORRUPT your database! You have "
          "been warned!\n\n";
...
DumpReturn PlayerDumpReader::LoadDump(std::istream& input, uint32 account, std::string name, ObjectGuid::LowType guid)
{
  ...
  sCharacterCache->AddCharacterCacheEntry(ObjectGuid(HighGuid::Player, guid), account, name, gender, race, playerClass,
                                          level);

  sObjectMgr->GetGenerator<HighGuid::Item>().Set(sObjectMgr->GetGenerator<HighGuid::Item>().GetNextAfterMaxUsed() +
                                                 items.size());
...
DumpReturn PlayerDumpReader::LoadDumpFromString(std::string const& dump, uint32 account, std::string name,
...
DumpReturn PlayerDumpReader::LoadDumpFromFile(std::string const& file, uint32 account, std::string name,
                                              ObjectGuid::LowType guid)
{
