...
constexpr float VisibilityDistances[AsUnderlyingType(VisibilityDistanceType::Max)] = {
    ...
    VISIBILITY_DISTANCE_LARGE,   VISIBILITY_DISTANCE_GIGANTIC, VISIBILITY_DISTANCE_INFINITE};
...
WorldObject::~WorldObject()
{
  ...
  if (IsWorldObject() && m_currMap)
  {
    if (IsCorpse())
    {
      LOG_FATAL("entities.object", "Object::~Object Corpse {}, type={} deleted but still in map!!",
                GetGUID().ToString(), ((Corpse*)this)->GetType());
      ABORT();
    }
    ResetMap();
  }
...
void Object::BuildOutOfRangeUpdateBlock(UpdateData* data) const { data->AddOutOfRangeGUID(GetGUID()); }
...
void MovementInfo::OutDebug()
{
  ...
  if (flags & MOVEMENTFLAG_FALLING)
    LOG_INFO("movement", "j_zspeed: {} j_sinAngle: {} j_cosAngle: {} j_xyspeed: {}", jump.zspeed, jump.sinAngle,
             jump.cosAngle, jump.xyspeed);
...
WorldObject::WorldObject(bool isWorldObject)
    : WorldLocation(), LastUsedScriptID(0), m_name(""), m_isActive(false), m_visibilityDistanceOverride(),
      m_isWorldObject(isWorldObject), m_zoneScript(nullptr), _zoneId(0), _areaId(0), _floorZ(INVALID_HEIGHT),
      _outdoors(false), _liquidData(), _updatePositionData(false), m_transport(nullptr), m_currMap(nullptr),
      m_InstanceId(0), m_phaseMask(PHASEMASK_NORMAL), m_useCombinedPhases(true), m_notifyflags(0),
...
void WorldObject::Update(uint32 time_diff) { sScriptMgr->OnWorldObjectUpdate(this, time_diff); }
...
Position WorldObject::GetHitSpherePointFor(Position const& dest, Optional<float> collisionHeight,
...
bool WorldObject::IsWithinLOS(float ox, float oy, float oz, VMAP::ModelIgnoreFlags ignoreFlags,
                                   ...
                                   Optional<float> combatReach /*= { }*/) const
{
  ...
  if (IsPlayer())
  {
    ...
    GetHitSpherePointFor({obj->GetPositionX(), obj->GetPositionY(), obj->GetPositionZ() + obj->GetCollisionHeight()}, x,
                         y, z, collisionHeight, combatReach);
...
bool WorldObject::isInFront(WorldObject const* target, float arc) const { return HasInArc(arc, target); }

bool WorldObject::isInBack(WorldObject const* target, float arc) const { return !HasInArc(2 * M_PI - arc, target); }
...
void WorldObject::UpdateAllowedPositionZ(float x, float y, float& z, float* groundZ) const
{
  ...
  if (Unit const* unit = ToUnit())
  {
    if (!unit->CanFly())
    {
      ...
      {
        if (canSwim && unit->GetMap()->IsInWater(unit->GetPhaseMask(), x, y, max_z - Z_OFFSET_FIND_HEIGHT,
  ...
  if (Creature const* cObj = obj->ToCreature())
  {
    if (Player const* player = ToPlayer())
    {
      ...
      ConditionList conditions =
          sConditionMgr->GetConditionsForNotGroupedEntry(CONDITION_SOURCE_TYPE_CREATURE_VISIBILITY, cObj->GetEntry());
  ...
  if (distanceCheck)
  {
    ...
    if (Player const* thisPlayer = ToPlayer())
    {
      ...
      if (Unit const* target = obj->ToUnit())
      {
        ...
        // pussywizard: during arena preparation, don't allow to detect pets if can't see its owner (spoils enemy arena
        // frames)
    ...
    // Xinef: check reversely obj vs viewpoint, object could be a gameObject which overrides _IsWithinDist function to
    // include gameobject size
...
void WorldObject::SetMap(Map* map)
{
  ...
  if (m_currMap)
  {
    LOG_FATAL("entities.object", "WorldObject::SetMap: obj {} new map {} {}, old map {} {}", (uint32)GetTypeId(),
              map->GetId(), map->GetInstanceId(), m_currMap->GetId(), m_currMap->GetInstanceId());
    ABORT();
  }
...
void WorldObject::AddObjectToRemoveList()
{
  ...
  if (!map)
  {
    LOG_ERROR("entities.object", "Object {} at attempt add to move list not have valid map (Id: {}).",
              GetGUID().ToString(), GetMapId());
    return;
  }
...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos,
                                SummonPropertiesEntry const* properties /*= nullptr*/, uint32 duration /*= 0*/,
                                WorldObject* summoner /*= nullptr*/, uint32 spellId /*= 0*/, uint32 vehId /*= 0*/,
                                bool visibleBySummonerOnly /*= false*/)
{
  ...
  if (!summon->Create(GenerateLowGuid<HighGuid::Unit>(), this, phase, entry, vehId, pos.GetPositionX(),
...
TempSummon* WorldObject::SummonCreature(uint32 id, float x, float y, float z, float ang, TempSummonType spwtype,
                                        uint32 despwtime, SummonPropertiesEntry const* properties,
                                  ...
                                  float rotation2, float rotation3, uint32 respawnTime, bool checkTransport)
{
  ...
  if (!go->Create(GenerateLowGuid<HighGuid::GameObject>(), entry, this, PHASEMASK_NORMAL, x, y, z, ang,
                  G3D::Quat(rotation0, rotation1, rotation2, rotation3), 100, GO_STATE_READY))
  {
    delete go;
    return nullptr;
  }
...
{
  return SummonGameObject(entry, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation(),
                          rotation0, rotation1, rotation2, rotation3, respawnTime, checkTransport);
}
...
TempSummon* WorldObject::SummonCreature(uint32 entry, const Position& pos, TempSummonType spwtype, uint32 duration,
                                        uint32 /*vehId*/, SummonPropertiesEntry const* properties,
...
GameObject* WorldObject::SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0,
                                          float rotation1, float rotation2, float rotation3, uint32 respawnTime,
                                          bool checkTransport, GOSummonType summonType)
{
  if (!IsInWorld())
    return nullptr;
  ...
  Map*        map = GetMap();
  GameObject* go  = sObjectMgr->IsGameObjectStaticTransport(entry) ? new StaticTransport() : new GameObject();
  if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(), entry, map, GetPhaseMask(), x, y, z, ang,
                  G3D::Quat(rotation0, rotation1, rotation2, rotation3), 100, GO_STATE_READY))
  {
    delete go;
    return nullptr;
...
Creature* WorldObject::SummonTrigger(float x, float y, float z, float ang, uint32 duration, bool setLevel,
...
void WorldObject::SummonCreatureGroup(uint8 group, std::list<TempSummon*>* list /*= nullptr*/)
{
  ...
  std::vector<TempSummonData> const* data =
      sObjectMgr->GetSummonGroup(GetEntry(), IsGameObject() ? SUMMONER_TYPE_GAMEOBJECT : SUMMONER_TYPE_CREATURE, group);
  if (!data)
    return;
...
Creature* WorldObject::FindNearestCreature(uint32 entry, float range, bool alive) const
{
  ...
  Acore::CreatureLastSearcher<Acore::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(this, creature,
                                                                                                   checker);
...
void WorldObject::GetGameObjectListWithEntryInGrid(std::list<GameObject*>& gameobjectList, uint32 entry,
...
void WorldObject::GetCreatureListWithEntryInGrid(std::list<Creature*>& creatureList, uint32 entry,
                                                 float maxSearchRange) const
{
...
void WorldObject::GetDeadCreatureListInGrid(std::list<Creature*>& creaturedeadList, float maxSearchRange,
    ...
    class NearUsedPosDo
    {
        public:
            ...
            void add(WorldObject* u, float x, float y) const
            {
                ...
                if (!i_object.IsInRange2d(x, y, i_selector.m_dist - i_selector.m_size, i_selector.m_dist +
i_selector.m_size)) return;
...
void WorldObject::GetNearPoint2D(WorldObject const* searcher, float& x, float& y, float distance2d, float absAngle,
...
void WorldObject::MovePosition(Position& pos, float dist, float angle, bool disableWarning)
{
  ...
  {
    if (!disableWarning)
      LOG_FATAL("entities.object", "WorldObject::MovePosition invalid coordinates X: {} and Y: {} were passed!", destx,
                desty);
...
void WorldObject::MovePositionToFirstCollision(Position& pos, float dist, float angle)
{
  ...
  if (!GetMap()->CheckCollisionAndGetValidCoords(this, pos.m_positionX, pos.m_positionY, pos.m_positionZ, destx, desty,
                                                 destz, false))
    return;
    ...
    if (Unit* u = ToUnit())
    {
      if (f & NOTIFY_VISIBILITY_CHANGED)
      {
        uint32 EVENT_VISIBILITY_DELAY =
            u->FindMap() ? DynamicVisibilityMgr::GetVisibilityNotifyDelay(u->FindMap()->GetEntry()->map_type) : 1000;
...
void WorldObject::RemoveFromObjectUpdate() { GetMap()->RemoveUpdateObject(this); }
...
float WorldObject::GetMapHeight(float x, float y, float z, bool vmap /* = true*/,
...
float WorldObject::GetFloorZ() const
{
  ...
  return std::max<float>(
      _floorZ, GetMap()->GetGameObjectFloor(GetPhaseMask(), GetPositionX(), GetPositionY(),
...
void WorldObject::AddAllowedLooter(ObjectGuid guid) { _allowedLooters.insert(guid); }

void WorldObject::SetAllowedLooters(GuidUnorderedSet const looters) { _allowedLooters = looters; }
...
GuidUnorderedSet const& WorldObject::GetAllowedLooters() const { return _allowedLooters; }

void WorldObject::RemoveAllowedLooter(ObjectGuid guid) { _allowedLooters.erase(guid); }
