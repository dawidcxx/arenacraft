...
KillRewarder::KillRewarder(Player* killer, Unit* victim, bool isBattleGround)
    : // 1. Initialize internal variables to default values.
      _killer(killer), _victim(victim), _group(killer->GetGroup()), _groupRate(1.0f), _maxNotGrayMember(nullptr),
      _count(0), _aliveSumLevel(0), _sumLevel(0), _xp(0), _isFullXP(false), _maxLevel(0),
...
void KillRewarder::_RewardKillCredit(Player* player)
{
  ...
  if (!_group || player->IsAlive() || !player->GetCorpse())
    if (Creature* target = _victim->ToCreature())
    {
      ...
      player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE_TYPE, target->GetCreatureType(), 1,
                                        target);
    }
}
...
void KillRewarder::Reward()
{
  ...
  else
  {
    ...
    if (!_isBattleGround || _xp)
      // 3.2.2. Reward killer.
      if (_killer->IsInMap(_victim)) // pussywizard: killer may be on other map (crashfix), when killing in a group same
