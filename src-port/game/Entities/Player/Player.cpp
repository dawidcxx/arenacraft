...
bool Player::Create(ObjectGuid::LowType guidlow, CharacterCreateInfo* createInfo)
{
  ...
  if (!info)
  {
    LOG_ERROR("entities.player",
              "Player::Create: Possible hacking-attempt: Account {} tried creating a character named '{}' with an "
              "invalid race/class pair ({}/{}) - refusing to do so.",
  ...
  if (!cEntry)
  {
    LOG_ERROR("entities.player",
              "Player::Create: Possible hacking-attempt: Account {} tried creating a character named '{}' with an "
              "invalid character class ({}) - refusing to do so (wrong DBC-files?)",
  ...
  if (!IsValidGender(createInfo->Gender))
  {
    LOG_ERROR("entities.player",
              "Player::Create: Possible hacking-attempt: Account {} tried creating a character named '{}' with an "
              "invalid gender ({}) - refusing to do so",
  ...
  SetUInt32Value(PLAYER_BYTES, (createInfo->Skin | (createInfo->Face << 8) | (createInfo->HairStyle << 16) |
...
bool Player::StoreNewItemInBestSlots(uint32 titem_id, uint32 titem_amount)
{
  ...
  // item can't be added
  LOG_ERROR("entities.player", "STORAGE: Can't equip or store initial item {} for race {} class {}, error msg = {}",
...
uint32 Player::EnvironmentalDamage(EnviromentalDamage type, uint32 damage)
{
  ...
  case DAMAGE_SLIME:
  {
    DamageInfo dmgInfo(this, this, damage, nullptr,
                       type == DAMAGE_LAVA ? SPELL_SCHOOL_MASK_FIRE : SPELL_SCHOOL_MASK_NATURE, DIRECT_DAMAGE);
...
int32 Player::getMaxTimer(MirrorTimerType timer)
{
  ...
  case BREATH_TIMER:
  {
    if (!IsAlive() || HasWaterBreathingAura() ||
...
void Player::setDeathState(DeathState s, bool /*despawn = false*/)
{
  ...
  if (s == DeathState::JustDied)
  {
    if (!cur)
    {
      LOG_ERROR("entities.player", "setDeathState: attempt to kill a dead player {} ({})", GetName(),
                GetGUID().ToString());
      return;
    }
    ...
    // FIXME: is pet dismissed at dying or releasing spirit? if second, add setDeathState(DeathState::Dead) to
...
{
  //             0               1                2                3                 4                  5 6 7
  //    "SELECT characters.guid, characters.name, characters.race, characters.class, characters.gender, characters.skin,
  //    characters.face, characters.hairStyle,
  //     8                     9                       10              11               12              13 14 15
  //    characters.hairColor, characters.facialStyle, character.level, characters.zone, characters.map,
  //    characters.position_x, characters.position_y, characters.position_z, 16                    17 18 19 20 21 22 23
  //    guild_member.guildid, characters.playerFlags, characters.at_login, character_pet.entry, character_pet.modelid,
  //    character_pet.level, characters.equipmentCache, character_banned.guid, 24                      25
  ...
  *data << uint32(petFamily);
  ...
  for (uint8 slot = 0; slot < INVENTORY_SLOT_BAG_END; ++slot)
  {
    ...
    if (!proto)
    {
      if (!itemId || *itemId)
      {
        LOG_WARN("entities.player.loading",
                 "Player {} has invalid equipment '{}' in `equipmentcache` at index {}. Skipped.", guid.ToString(),
                 (visualBase < equipment.size()) ? equipment[visualBase] : "<none>", visualBase);
      }
    ...
    {
      LOG_WARN("entities.player.loading",
               "Player {} has invalid enchantment info '{}' in `equipmentcache` at index {}. Skipped.", guid.ToString(),
               ((visualBase + 1) < equipment.size()) ? equipment[visualBase + 1] : "<none>", visualBase + 1);
                        ...
                        Unit* target /*= nullptr*/, bool newInstance /*= false*/)
{
  ...
  {
    LOG_ERROR("entities.player",
              "TeleportTo: invalid map ({}) or invalid coordinates (X: {}, Y: {}, Z: {}, O: {}) given when teleporting "
              "player ({}, name: {}, map: {}, X: {}, Y: {}, Z: {}, O: {}).",
              mapid, x, y, z, orientation, GetGUID().ToString(), GetName(), GetMapId(), GetPositionX(), GetPositionY(),
  ...
  {
    LOG_ERROR("entities.player", "Player ({}, name: {}) tried to enter a forbidden map {}", GetGUID().ToString(),
              GetName(), mapid);
  ...
  {
    LOG_DEBUG("maps", "Player {} using client without required expansion tried teleport to non accessible map {}",
              GetName(), mapid);
...
void Player::RemoveFromWorld()
{
  ...
  if (m_uint32Values)
  {
    ...
    {
      LOG_FATAL("entities.player", "Player {} has viewpoint {} {} when removed from world", GetName(),
...
void Player::RegenerateAll()
{
  ...
  if (m_regenTimerCount >= 2000)
  {
    ...
    if (!IsInCombat() || IsPolymorphed() || m_baseHealthRegen || HasRegenDuringCombatAura() ||
  ...
  // According to sniffs there is a background timer going on that repeats independed from the time window where the
  // aura applies. That's why we dont need to reset the timer on apply. In sniffs I have seen that the first call for
  // the spell visual is totally random, then after 5 seconds over and over again which confirms my theory that we have
  // a independed timer.
...
void Player::Regenerate(Powers power)
{
  ...
  case POWER_MANA:
  {
    ...
    if (recentCast) // Trinity Updates Mana in intervals of 2s, which is correct
      addvalue +=
          GetFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER) * ManaIncreaseRate * 0.001f * m_regenTimer;
    else
...
Creature* Player::GetNPCIfCanInteractWith(ObjectGuid guid, uint32 npcflagmask)
{
  ...
  // pussywizard: many npcs have missing conditions for class training and rogue trainer can for eg. train dual wield to
  // a shaman :/ too many to change in sql and watch in the future pussywizard: this function is not used when talking,
  // but when already taking action (buy spell, reset talents, show spell list)
...
GameObject* Player::GetGameObjectIfCanInteractWith(ObjectGuid guid, GameobjectTypes type) const
{
  if (GameObject* go = GetMap()->GetGameObject(guid))
  {
    if (go->GetGoType() == type)
    {
      ...
      LOG_DEBUG("maps",
                "IsGameObjectOfTypeInRange: GameObject '{}' [{}] is too far away from player {} [{}] to be used by him "
                "(distance={}, maximal 10 is allowed)",
                go->GetGOInfo()->name, go->GetGUID().ToString(), GetName(), GetGUID().ToString(),
                go->GetDistance(this));
    }
  }
  return nullptr;
...
void Player::RemoveFromGroup(Group* group, ObjectGuid guid, RemoveMethod method /* = GROUP_REMOVEMETHOD_DEFAULT*/,
                             ObjectGuid kicker /* = ObjectGuid::Empty */, const char* reason /* = nullptr */)
{
  if (group)
  {
...
void Player::InitStatsForLevel(bool reapplyMods)
{
  ...
  RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_NOT_ATTACKABLE_1 |
...
void Player::SendInitialSpells()
{
  ...
  WorldPacket data(SMSG_INITIAL_SPELLS,
                   (1 + 2 + 4 * m_spells.size() + 2 + m_spellCooldowns.size() * (4 + 2 + 2 + 4 + 4)));
  data << uint8(0);
...
{
  // pussywizard: spells learnt from talents are added as TEMPORARY, so not saved to db (only the talent itself is
  // saved)
  SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
...
bool Player::addSpell(uint32 spellId, uint8 addSpecMask, bool updateActive, bool temporary /*= false*/,
...
{
  // pussywizard: this can be called to OVERWRITE currently existing spell params! usually to set active = false for
  // lower ranks of a spell
  ...
  if (temporary && spellInfo->HasEffect(SPELL_EFFECT_LEARN_SPELL))
  {
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
      if (spellInfo->Effects[i].IsEffect())
      {
        if (spellInfo->Effects[i].Effect != SPELL_EFFECT_LEARN_SPELL)
        {
          LOG_INFO("entities.player", "TRYING TO LEARN SPELL WITH EFFECT LEARN: {}, PLAYER: {}", spellId,
                   GetGUID().ToString());
          return false;
          // ABORT();
        }
  ...
  // pussywizard: return true to fix active for ranks, this condition is true only at loading, so no problems with
  // learning packets
  ...
  if (spellInfo->HasEffect(SPELL_EFFECT_LEARN_SPELL))
  {
    LOG_INFO("entities.player", "TRYING TO LEARN SPELL WITH EFFECT LEARN 2: {}, PLAYER: {}", spellId,
             GetGUID().ToString());
  ...
  else
  {
         ...
         ++_spell_idx)
    {
      ...
      /// @todo confirm if rogues start wth lockpicking skill at level 1 but only recieve the spell to use it at level
      /// 16
      // Added for runeforging, it is confirmed via sniff that this happens when death knights learn the spell, not on
      // character creation.
...
void Player::learnSpell(uint32 spellId, bool temporary /*= false*/, bool learnFromSkill /*= false*/)
{
  ...
  {
    // pussywizard: lookup next rank in m_spells (the only talents on m_spella are for example pyroblast, that have all
    // ranks restored upon learning rank 1) pussywizard: next ranks must not be in current spec (otherwise no need to
    // learn already learnt)
  ...
  // xinef: if we learn new spell, check all spells requiring this spell, if we have such a spell, and it is not in
  // current spec - learn it
...
void Player::removeSpell(uint32 spell_id, uint8 removeSpecMask, bool onlyTemporary)
{
  ...
  // pussywizard: some more conditions needed for spells like pyroblast (shouldn't be fully removed when not available
  // in any spec, should stay in db with specMask = 0)
    ...
    if (spellInfo->HasAttribute(SPELL_ATTR4_IGNORE_DEFAULT_ARENA_RESTRICTIONS))
      RemoveSpellCooldown(itr->first, true);
             ...
             && itr->second.maxduration <
                    10 * MINUTE * IN_MILLISECONDS) // xinef: dont clear cooldowns that have maxduration > 10 minutes (eg
                                                   // item cooldowns with no spell.dbc cooldown info)
      RemoveSpellCooldown(itr->first, true);
  }
...
void Player::_LoadSpellCooldowns(PreparedQueryResult result)
{
  ...
  // QueryResult* result = CharacterDatabase.Query("SELECT spell, category, item, time FROM character_spell_cooldown
  // WHERE guid = '{}'", GetGUID().GetCounter()());
  ...
  if (result)
  {
    ...
    do
    {
      ...
      {
        LOG_ERROR("entities.player", "Player {} has unknown spell {} in `character_spell_cooldown`, skipping.",
                  GetGUID().ToString(), spell_id);
        continue;
      }
      ...
      LOG_DEBUG("entities.player.loading", "Player ({}) spell {}, item {} cooldown loaded ({} secs).",
                GetGUID().ToString(), spell_id, item_id, uint32(db_time - curTime));
    ...
    if (itr->second.end <= curMSTime + 1000)
      m_spellCooldowns.erase(itr++);
             ...
             (logout ||
              itr->second.end >
                  (curMSTime +
                   5 * MINUTE * IN_MILLISECONDS))) // not save locked cooldowns, it will be reset or set at reload
    {
      if (first_round)
      {
...
bool Player::resetTalents(bool noResetCost)
{
  ...
  // pussywizard: removed saving to db, nothing important happens and saving only spells and talents may cause data
  // integrity problems (eg. with skills saved to db)
...
void Player::DeleteFromDB(ObjectGuid::LowType lowGuid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
  ...
  case CHAR_DELETE_REMOVE:
  {
    ...
    // pet. NOW we can finally clear other DB data related to character
...
void Player::DeleteOldCharacters(uint32 keepDays)
{
  LOG_INFO("server.loading",
           "Player::DeleteOldChars: Deleting all characters which have been deleted {} days before...", keepDays);
  LOG_INFO("server.loading", " ");
...
/**
 * Items which were kept back in the database after being deleted and are now too old (see config option
 * "ItemDelete.KeepDays"), will be completely deleted.
 */
...
/**
 * Items which were kept back in the database after being deleted and are older than the specified amount of days, will
 * be completely deleted.
 */
void Player::DeleteOldRecoveryItems(uint32 keepDays)
{
  LOG_INFO("server.loading",
           "Player::DeleteOldRecoveryItems: Deleting all items which have been deleted {} days before...", keepDays);
  LOG_INFO("server.loading", " ");
...
void Player::BuildPlayerRepop()
{
  ...
  {
    LOG_ERROR("entities.player", "BuildPlayerRepop: player {} ({}) already has a corpse", GetName(),
              GetGUID().ToString());
    return;
  }
...
void Player::KillPlayer()
{
  ...
  ApplyModFlag(PLAYER_FIELD_BYTES, PLAYER_FIELD_BYTE_RELEASE_TIMER,
               !sMapStore.LookupEntry(GetMapId())->Instanceable() && !HasPreventResurectionAura());
...
void Player::SendDurabilityLoss() { SendDirectMessage(WorldPackets::Misc::DurabilityDamageDeath().Write()); }
...
void Player::SendActionButtons(uint32 state) const
{
  ...
  /*
      state can be 0, 1, 2
      0 - Looks to be sent when initial action buttons get sent, however on Trinity we use 1 since 0 had some
     difficulties 1 - Used in any SMSG_ACTION_BUTTONS packet with button data on Trinity. Only used after spec swaps on
     retail. 2 - Clears the action bars client sided. This is sent during spec swap before unlearning and before sending
     the new buttons
  */
  if (state != 2)
  {
...
bool Player::IsActionButtonDataValid(uint8 button, uint32 action, uint8 type)
{
  ...
  {
    LOG_ERROR("entities.player", "Action {} not added into button {} for player {}: button must be < {}", action,
              button, GetName(), MAX_ACTION_BUTTONS);
    return false;
  }

  if (action >= MAX_ACTION_BUTTON_ACTION_VALUE)
  {
    LOG_ERROR("entities.player", "Action {} not added into button {} for player {}: action must be < {}", action,
              button, GetName(), MAX_ACTION_BUTTON_ACTION_VALUE);
    return false;
  }
  ...
  case ACTION_BUTTON_SPELL:
    ...
    {
      LOG_ERROR("entities.player", "Spell action {} not added into button {} for player {}: spell not exist", action,
                button, GetName());
      return false;
    }
    ...
    if (!HasSpell(action))
    {
      LOG_DEBUG("entities.player.loading",
                "Player::IsActionButtonDataValid Spell action {} not added into button {} for player {}: player don't "
                "known this spell",
  ...
  case ACTION_BUTTON_ITEM:
    ...
    {
      LOG_ERROR("entities.player", "Item action {} not added into button {} for player {}: item not exist", action,
                button, GetName());
      return false;
    }
    break;
...
ActionButton* Player::addActionButton(uint8 button, uint32 action, uint8 type)
{
  ...
  LOG_DEBUG("entities.player", "Player {} Added Action {} (type {}) to Button {}", GetGUID().ToString(), action, type,
            button);
  return &ab;
}
...
void Player::SendMessageToSetInRange(WorldPacket const* data, float dist, bool self, bool includeMargin,
                                     bool ownTeamOnly, bool required3dDist) const
{
  if (self)
    SendDirectMessage(data);
...
void Player::CheckAreaExploreAndOutdoor()
{
  ...
  if (!areaEntry)
  {
    LOG_ERROR("entities.player", "Player '{}' ({}) discovered unknown area (x: {} y: {} z: {} map: {})", GetName(),
              GetGUID().ToString(), GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId());
    return;
  }
  ...
  {
    LOG_ERROR("entities.player",
              "Wrong area flag {} in map data for (X: {} Y: {}) point to field PLAYER_EXPLORED_ZONES_1 + {} ( {} must "
              "be < {} ).",
              areaEntry->flags, GetPositionX(), GetPositionY(), offset, offset, PLAYER_EXPLORED_ZONES_SIZE);
    return;
  }
...
TeamId Player::TeamIdForRace(uint8 race)
{
  if (ChrRacesEntry const* rEntry = sChrRacesStore.LookupEntry(race))
  {
    ...
    LOG_ERROR("entities.player", "Race ({}) has wrong teamid ({}) in DBC: wrong DBC files?", uint32(race),
              rEntry->TeamID);
  }
  else
...
float Player::CalculateReputationGain(ReputationSource source, uint32 creatureOrQuestLevel, float rep, int32 faction,
                                      bool noQuestBonus)
{
...
void Player::RewardReputation(Unit* victim)
{
  ...
  {
    float donerep1 =
        CalculateReputationGain(REPUTATION_SOURCE_KILL, victim->GetLevel(), static_cast<float>(Rep->RepValue1),
    ...
    if (factionEntry1)
    {
      GetReputationMgr().ModifyReputation(factionEntry1, donerep1, false,
  ...
  {
    float donerep2 =
        CalculateReputationGain(REPUTATION_SOURCE_KILL, victim->GetLevel(), static_cast<float>(Rep->RepValue2),
    ...
    if (factionEntry2)
    {
      GetReputationMgr().ModifyReputation(factionEntry2, donerep2, false,
...
void Player::RewardReputation(Quest const* quest)
{
  for (uint8 i = 0; i < QUEST_REPUTATIONS_COUNT; ++i)
  {
    ...
    {
      rep = CalculateReputationGain(REPUTATION_SOURCE_WEEKLY_QUEST, GetQuestLevel(quest), rep,
                                    quest->RewardFactionId[i], false);
    }
    else if (quest->IsMonthly())
    {
      rep = CalculateReputationGain(REPUTATION_SOURCE_MONTHLY_QUEST, GetQuestLevel(quest), rep,
                                    quest->RewardFactionId[i], false);
    }
    else if (quest->IsRepeatable())
    {
      rep = CalculateReputationGain(REPUTATION_SOURCE_REPEATABLE_QUEST, GetQuestLevel(quest), rep,
                                    quest->RewardFactionId[i], false);
    }
    else
    {
      rep =
          CalculateReputationGain(REPUTATION_SOURCE_QUEST, GetQuestLevel(quest), rep, quest->RewardFactionId[i], false);
    }
...
void Player::DuelComplete(DuelCompleteType type)
{
  ...
  LOG_DEBUG("entities.unit", "Player::DuelComplete: Player '{}' ({}), Opponent: '{}' ({})", GetName(),
            GetGUID().ToString(), opponent->GetName(), opponent->GetGUID().ToString());
...
void Player::_ApplyItemMods(Item* item, uint8 slot, bool apply)
{
  ...
  if (item->HasSocket()) // only (un)equipping of items with sockets can influence metagems, so no need to waste time
                         // with normal items
...
void Player::_ApplyItemBonuses(ItemTemplate const* proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
  ...
  for (uint8 i = 0; i < MAX_ITEM_PROTO_STATS; ++i)
  {
    ...
    if (ssv)
    {
      ...
      else
      {
        ...
        // OnCustomScalingStatValue(Player* player, ItemTemplate const* proto, uint32& statType, int32& val, uint8
        // itemProtoStatNumber, uint32 ScalingStatValue, ScalingStatValuesEntry const* ssv)
  ...
  if (ssv)
  {
    if (uint32 ssvarmor = ssv->getArmorMod(ScalingStatValue))
      if (proto->ScalingStatValue > 0 || ssvarmor < proto->Armor) // Check to avoid higher values than stat itself
...
void Player::_ApplyWeaponDependentAuraCritMod(Item* item, WeaponAttackType attackType, AuraEffect const* aura,
                                              bool apply)
{
...
void Player::_ApplyWeaponDependentAuraDamageMod(Item* item, WeaponAttackType attackType, AuraEffect const* aura,
                                                bool apply)
{
  // don't apply mod if item is broken or cannot be used
  if (item->IsBroken() || !CanUseAttackType(attackType))
...
void Player::CastItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item* item,
    ...
    for (uint8 i = 0; i < MAX_ITEM_SPELLS; ++i)
    {
      ...
      if (roll_chance_f(chance) && sScriptMgr->OnCastItemCombatSpell(this, target, spellInfo, item))
        CastSpell(target, spellInfo->Id,
                  TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD), item);
    }
  }
...
void Player::CastItemUseSpell(Item* item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex)
{
  ...
  if (proto->Spells[0].SpellId == 483 || proto->Spells[0].SpellId == 55884)
  {
    ...
    if (!spellInfo)
    {
      LOG_ERROR("entities.player", "Player::CastItemUseSpell: Item (Entry: {}) in have wrong spell id {}, ignoring ",
  ...
  for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
  {
    ...
    // Xinef: dont allow to cast such spells, it may happen that spell possess 2 spells, one for players and one for
    // items / gameobjects Xinef: if first one is cast on player, it may be deleted thus resulting in crash because
    // second spell has saved pointer to the item Xinef: there is one problem with scripts which wont be loaded at the
    // moment of call
  ...
  for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
  {
    ...
    for (uint8 s = 0; s < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS; ++s)
    {
      ...
      if (!spellInfo)
      {
        LOG_ERROR("entities.player", "Player::CastItemUseSpell Enchant {}, cast unknown spell {}", pEnchant->ID,
                  pEnchant->spellid[s]);
        continue;
      }
      ...
      else
      {
        if (recipientGroup)
        {
          if (GetGroup() == recipientGroup)
          {
            ...
            case MASTER_LOOT:
              permission =
                  recipientGroup->GetMasterLooterGuid() == GetGUID() ? MASTER_PERMISSION : RESTRICTED_PERMISSION;
              break;
            case FREE_FOR_ALL:
              permission = ALL_PERMISSION;
              break;
  ...
  else
  {
    ...
    case 139: // Eastern Plaguelands
      ...
      else
      {
        ...
        data << uint32(0x931)
             << uint32(0x1); // 38 2367 ??  grey horde not in dbc! send for consistency's sake, and to match field count
    ...
    case 3277: // Warsong Gulch
      ...
      else
      {
        ...
        data << uint32(0x922) << uint32(0x1); // 13 2338 horde (0 - hide, 1 - flag ok, 2 - flag picked up (flashing), 3
                                              // - flag picked up (not flashing)
        data << uint32(0x923) << uint32(0x1); // 14 2339 alliance (0 - hide, 1 - flag ok, 2 - flag picked up (flashing),
                                              // 3 - flag picked up (not flashing)
      }
      break;
    ...
    case 3820: // Eye of the Storm
      ...
      else
      {
        ...
        data << uint32(0xad2)
             << uint32(0x1); // 30 2770 Horde top-stats (1 - show, 0 - hide) // 02 -> horde picked up the flag
        data << uint32(0xad1)
             << uint32(0x1); // 31 2769 Alliance top-stats (1 - show, 0 - hide) // 02 -> alliance picked up the flag
        ...
        data << uint32(0xaa0) << uint32(0x0); // 35 2720 Capturing progress-bar (100 -> empty (only grey), 0 -> blue|red
                                              // (no grey), default 0)
    ...
    case 3483: // Hellfire Peninsula
      ...
      {
        data << uint32(0x9ba)
             << uint32(0x1); // 10 // add ally tower main gui icon       // maybe should be sent only on login?
        data << uint32(0x9b9)
             << uint32(0x1); // 11 // add horde tower main gui icon      // maybe should be sent only on login?
...
uint32 Player::GetXPRestBonus(uint32 xp)
{
  ...
  LOG_DEBUG("entities.player", "Player gain {} xp (+ {} Rested Bonus). Rested points={}", xp + rested_bonus,
...
void Player::ResetPetTalents()
{
  ...
  if (!charmInfo)
  {
    LOG_ERROR("entities.player", "Object ({}) is considered pet-like but doesn't have a charminfo!",
      ...
      !InBattleground())
  {
    ...
    if (spellInfo)
    {
      for (uint32 i = 0; i < MAX_SPELL_REAGENTS; ++i)
      {
        ...
        {
          ItemPosCountVec dest; // for succubus, voidwalker, felhunter and felguard credit soulshard when despawn reason
...
void Player::StopCastingCharm(Aura* except /*= nullptr*/)
{
  ...
  if (GetCharmGUID())
  {
    LOG_FATAL("entities.player", "Player {} ({} is not able to uncharm unit ({})", GetName(), GetGUID().ToString(),
...
void Player::TextEmote(std::string_view text, WorldObject const* /*= nullptr*/, bool /*= false*/)
{
  ...
  SendMessageToSetInRange(&data, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), true, false,
                          !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_EMOTE), true);
}
...
void Player::Whisper(uint32 textId, Player* target, bool isBossWhisper)
{
  ...
  WorldPacket    data;
  if (isBossWhisper)
    ChatHandler::BuildChatPacket(data, CHAT_MSG_RAID_BOSS_WHISPER, LANG_UNIVERSAL, this, target,
                                 bct->GetText(locale, getGender()), 0, "", locale);
  else
    ChatHandler::BuildChatPacket(data, CHAT_MSG_WHISPER, LANG_UNIVERSAL, this, target,
                                 bct->GetText(locale, getGender()), 0, "", locale);
...
void Player::PossessSpellInitialize()
{
  ...
  if (!charmInfo)
  {
    LOG_ERROR("entities.player", "Player::PossessSpellInitialize(): charm ({}) has no charminfo!",
...
void Player::VehicleSpellInitialize()
{
  ...
  for (uint32 i = 0; i < MAX_CREATURE_SPELLS; ++i)
  {
    ...
    {
      LOG_DEBUG("condition", "VehicleSpellInitialize: conditions not met for Vehicle entry {} spell {}",
...
void Player::CharmSpellInitialize()
{
  ...
  if (!charmInfo)
  {
    LOG_ERROR("entities.player", "Player::CharmSpellInitialize(): the player's charm ({}) has no charminfo!",
              charm->GetGUID().ToString());
    return;
  }
...
void Player::ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue, Spell* spell, bool temporaryPet)
{
  ...
  {
    // xinef: temporary pets cannot use charged mods of owner, needed for mirror image QQ they should use their own
    // auras
    ...
    else if (mod->type == SPELLMOD_PCT)
    {
      ...
      // xinef: special case for backdraft gcd reduce with backlast time reduction, dont affect gcd if cast time was not
      // applied
...
template AC_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, int32& basevalue, Spell* spell,
                                                bool temporaryPet);
template AC_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, uint32& basevalue, Spell* spell,
                                                bool temporaryPet);
template AC_GAME_API void Player::ApplySpellMod(uint32 spellId, SpellModOp op, float& basevalue, Spell* spell,
                                                bool temporaryPet);
...
void Player::SetSpellModTakingSpell(Spell* spell, bool apply)
{
  ...
  {
    LOG_INFO("misc", "Player::SetSpellModTakingSpell (A1) - {}, {}", spell->m_spellInfo->Id,
  ...
  else if (!apply)
  {
    ...
    {
      LOG_INFO("misc", "Player::SetSpellModTakingSpell (C1) - {}, {}", spell->m_spellInfo->Id,
               m_spellModTakingSpell->m_spellInfo->Id);
      return;
    }
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= nullptr*/, uint32 spellid /*= 1*/)
{
  ...
  // not let cheating with start flight in time of logout process || while in combat || has type state: stunned || has
  // type state: root
...
void Player::SendTaxiNodeStatusMultiple()
{
  for (auto itr = m_clientGUIDs.begin(); itr != m_clientGUIDs.end(); ++itr)
  {
    ...
    uint32 nearestNode = sObjectMgr->GetNearestTaxiNode(creature->GetPositionX(), creature->GetPositionY(),
                                                        creature->GetPositionZ(), creature->GetMapId(), GetTeamId());
    if (!nearestNode)
    {
      continue;
    }
...
void Player::InitDisplayIds()
{
  ...
  if (!info)
  {
    LOG_ERROR("entities.player", "Player {} has incorrect race/class pair. Can't init display ids.",
              GetGUID().ToString());
    return;
  }
...
inline bool Player::_StoreOrEquipNewItem(uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot,
                                         int32 price, ItemTemplate const* pProto, Creature* pVendor,
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag,
                                   uint8 slot)
{
  ...
  if (!creature)
  {
    LOG_DEBUG("network", "WORLD: BuyItemFromVendor - Unit ({}) not found or you can't interact with him.",
              vendorguid.ToString());
  ...
  {
    // LOG_DEBUG("condition", "BuyItemFromVendor: conditions not met for creature entry {} item {}",
  ...
  if (crItem->ExtendedCost)
  {
    ...
    if (!iece)
    {
      LOG_ERROR("entities.player", "Item {} have wrong ExtendedCost field value {}", pProto->ItemId,
                crItem->ExtendedCost);
      return false;
    }
      ...
      pProto->BuyPrice > 0) // Assume price cannot be negative (do not know why it is int32)
  {
    ...
    {
      LOG_ERROR("entities.player", "Player {} tried to buy {} item id {}, causing overflow", GetName(), (uint32)count,
                pProto->ItemId);
...
void Player::AddSpellAndCategoryCooldowns(SpellInfo const* spellInfo, uint32 itemId, Spell* spell,
  ...
  if (cat && catrec > 0)
  {
    ...
    if (i_scstore != sSpellsByCategoryStore.end())
    {
           ...
           ++i_scset)
      {
        ...
        _AddSpellCooldown(i_scset->second, cat, itemId, catrecTime,
                               ...
                               bool forceSendToSpectator)
{
  ...
  if (end_time >= SPECTATOR_COOLDOWN_MIN * IN_MILLISECONDS && end_time <= SPECTATOR_COOLDOWN_MAX * IN_MILLISECONDS)
  {
    if (NeedSendSpectatorData() && forceSendToSpectator && (itemid || HasActiveSpell(spellid)))
    {
      ...
      ArenaSpectator::SendCommand_Cooldown(FindMap(), GetGUID(), "ACD", spellid, end_time / IN_MILLISECONDS,
...
void Player::SendCooldownEvent(SpellInfo const* spellInfo, uint32 itemId /*= 0*/, Spell* spell /*= nullptr*/,
...
void Player::LearnDefaultSkill(uint32 skillId, uint16 rank)
{
  ...
  LOG_DEBUG("entities.player.loading", "PLAYER (Class: {} Race: {}): Adding initial skill, id = {}", uint32(getClass()),
            uint32(getRace()), skillId);
...
void Player::learnSkillRewardedSpells(uint32 skill_id, uint32 skill_value)
{
  ...
  for (SkillLineAbilityEntry const* pAbility : GetSkillLineAbilitiesBySkillLine(skill_id))
  {
    ...
    {
      // used to avoid double Seal of Righteousness on paladins, it's the only player spell which has both spell and
      // forward spell in auto learn
...
void Player::SetBattlegroundId(uint32 id, BattlegroundTypeId bgTypeId, uint32 queueSlot, bool invited, bool isRandom,
                               TeamId teamId)
{
...
void Player::SummonIfPossible(bool agree, ObjectGuid summoner_guid)
{
  ...
  TeleportTo(m_summon_mapid, m_summon_x, m_summon_y, m_summon_z, GetOrientation(), 0,
...
bool Player::HasItemFitToSpellRequirements(SpellInfo const* spellInfo, Item const* ignoreItem) const
{
  ...
  default:
    LOG_ERROR("entities.player", "HasItemFitToSpellRequirements: Not handled spell requirement for item class {}",
...
void Player::SetMover(Unit* target)
{
  ...
  {
    LOG_INFO("misc", "Player::SetMover (A1) - {}, {}, {}, {}, {}, {}, {}, {}", GetGUID().ToString(), GetMapId(),
             GetInstanceId(), FindMap()->GetId(), IsInWorld() ? 1 : 0, IsDuringRemoveFromWorld() ? 1 : 0,
             IsBeingTeleported() ? 1 : 0, isBeingLoaded() ? 1 : 0);
    LOG_INFO("misc", "Player::SetMover (A2) - {}, {}, {}, {}, {}, {}, {}, {}", target->GetGUID().ToString(),
             target->GetMapId(), target->GetInstanceId(), target->FindMap()->GetId(), target->IsInWorld() ? 1 : 0,
             target->IsDuringRemoveFromWorld() ? 1 : 0,
             (target->ToPlayer() && target->ToPlayer()->IsBeingTeleported() ? 1 : 0), target->isBeingLoaded() ? 1 : 0);
    LOG_INFO("misc", "Player::SetMover (A3) - {}, {}, {}, {}, {}, {}, {}, {}",
             target->m_movedByPlayer->GetGUID().ToString(), target->m_movedByPlayer->GetMapId(),
             target->m_movedByPlayer->GetInstanceId(), target->m_movedByPlayer->FindMap()->GetId(),
             target->m_movedByPlayer->IsInWorld() ? 1 : 0, target->m_movedByPlayer->IsDuringRemoveFromWorld() ? 1 : 0,
  ...
  {
    LOG_INFO("misc", "Player::SetMover (B1) - {}, {}, {}, {}, {}, {}, {}, {}", GetGUID().ToString(), GetMapId(),
             GetInstanceId(), FindMap()->GetId(), IsInWorld() ? 1 : 0, IsDuringRemoveFromWorld() ? 1 : 0,
             IsBeingTeleported() ? 1 : 0, isBeingLoaded() ? 1 : 0);
    LOG_INFO("misc", "Player::SetMover (B2) - {}, {}, {}, {}, {}, {}, {}, {}", target->GetGUID().ToString(),
             target->GetMapId(), target->GetInstanceId(), target->FindMap()->GetId(), target->IsInWorld() ? 1 : 0,
             target->IsDuringRemoveFromWorld() ? 1 : 0,
             (target->ToPlayer() && target->ToPlayer()->IsBeingTeleported() ? 1 : 0), target->isBeingLoaded() ? 1 : 0);
  }
...
bool Player::isUsingLfg() { return sLFGMgr->GetState(GetGUID()) != lfg::LFG_STATE_NONE; }
...
void Player::SetBattlegroundOrBattlefieldRaid(Group* group, int8 subgroup)
{
  ...
  {
    LOG_INFO("misc", "Player::SetBattlegroundOrBattlefieldRaid - current group is {} group!",
...
void Player::SetViewpoint(WorldObject* target, bool apply)
{
  if (apply)
  {
    LOG_DEBUG("maps", "Player::CreateViewpoint: Player {} create seer {} (TypeId: {}).", GetName(), target->GetEntry(),
              target->GetTypeId());
...
uint32 Player::GetRuneBaseCooldown(uint8 index, bool skipGrace)
{
  ...
  if (!skipGrace)
    cooldown -=
        GetGracePeriod(index) < 250
            ? 0
            : GetGracePeriod(index) - 250; // xinef: reduce by grace period, treat first 250ms as instant use of rune
...
LootItem* Player::StoreLootItem(uint8 lootSlot, Loot* loot, InventoryResult& msg)
{
  ...
  // Xinef: exploit protection, dont allow to loot normal items if player is not master loot and not below loot
  // threshold Xinef: only quest, ffa and conditioned items
...
{
  //                                                           0      1      2
  // SetQuery(PLAYER_LOGIN_QUERY_LOADSKILLS,          "SELECT skill, value, max FROM character_skills WHERE guid =
  // '{}'", m_guid.GetCounter());
  ...
  if (result)
  {
    do
    {
      ...
      if (value == 0)
      {
        LOG_ERROR("entities.player", "Character {} has skill {} with value 0. Will be deleted.", GetGUID().ToString(),
                  skill);
  ...
  // Learn skill rewarded spells after all skills have been loaded to prevent learning a skill from them before its
  // loaded with proper value from DB
...
InventoryResult Player::CanEquipUniqueItem(ItemTemplate const* itemProto, uint8 except_slot, uint32 limit_count) const
{
  ...
  if (itemProto->ItemLimitCategory)
  {
    ...
    // there is an equip limit on this item
    if (HasItemOrGemWithLimitCategoryEquipped(itemProto->ItemLimitCategory, limitEntry->maxCount - limit_count + 1,
                                              except_slot))
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
  ...
  if (z_diff >= 14.57f && !isDead() && !IsGameMaster() && !GetCommandStatus(CHEAT_GOD) && !HasHoverAura() &&
      !HasFeatherFallAura() && !HasFlyAura())
  {
    ...
    if (damageperc > 0 && !IsImmunedToDamageOrSchool(SPELL_SCHOOL_MASK_NORMAL))
    {
      ...
      LOG_DEBUG("entities.player", "FALLDAMAGE mZ={} z={} fallTime={} damage={} SF={}", movementInfo.pos.GetPositionZ(),
                GetPositionZ(), movementInfo.fallTime, damage, safe_fall);
    }

    // recheck alive, might have died of EnvironmentalDamage, avoid cases when player die in fact like Spirit of
    // Redemption case
...
void Player::CheckAllAchievementCriteria() { m_achievementMgr->CheckAllAchievementCriteria(); }
...
void Player::ResetAchievementCriteria(AchievementCriteriaCondition condition, uint32 value,
                                      bool evenIfCriteriaComplete /* = false*/)
{
  m_achievementMgr->ResetAchievementCriteria(condition, value, evenIfCriteriaComplete);
}

void Player::CompletedAchievement(AchievementEntry const* entry) { m_achievementMgr->CompletedAchievement(entry); }
...
void Player::SetEquipmentSet(uint32 index, EquipmentSet eqset)
{
  if (eqset.Guid != 0)
  {
    ...
    {
      LOG_ERROR("entities.player",
                "Player {} tried to save equipment set {} (index {}), but that equipment set not found!", GetName(),
                eqset.Guid, index);
      return;
    }
  }
...
{
  // SetQuery(PLAYER_LOGIN_QUERY_LOADTALENTS, "SELECT spell, specMask FROM character_talent WHERE guid = '{}'",
  // m_guid.GetCounter());
  if (result)
  {
    do
...
void Player::ActivateSpec(uint8 spec)
{
  ...
  // pussywizard: remove spells that are in previous spec, but are not present in new one (or are in new spec, but not
  // in the old one)
  for (PlayerSpellMap::iterator itr = m_spells.begin(); itr != m_spells.end(); ++itr)
  {
...
std::string const& Player::GetGuildName() { return sGuildMgr->GetGuildById(GetGuildId())->GetName(); }
      ...
      if (spellInfo->Effects[i].Effect == SPELL_EFFECT_SCHOOL_DAMAGE)
      {
        ...
        if ((int32)damage_type[offset] < dmg)
        {
          if (!m_charmAISpells[SPELL_INSTANT_DAMAGE + offset] ||
              !spellInfo->IsHighRankOf(sSpellMgr->GetSpellInfo(m_charmAISpells[SPELL_INSTANT_DAMAGE + offset])) ||
              urand(0, 1))
        ...
        {
          if (m_charmAISpells[SPELL_INSTANT_DAMAGE + offset] &&
              sSpellMgr->GetSpellInfo(m_charmAISpells[SPELL_INSTANT_DAMAGE + offset])->IsHighRankOf(spellInfo) &&
              urand(0, 1))
            continue;
...
void Player::AddRefundReference(ObjectGuid itemGUID) { m_refundableItems.insert(itemGUID); }
...
void Player::_LoadPetStable(uint8 petStableSlots, PreparedQueryResult result)
{
  ...
  //         14       15
  // SELECT id, entry, modelid, level, exp, Reactstate, slot, name, renamed, curhealth, curmana, curhappiness, abdata,
  // savetime, CreatedBySpell, PetType FROM character_pet WHERE owner = ?
  if (result)
  {
    do
    {
...
Optional<float> Player::GetFarSightDistance() const { return _farSightDistance; }
