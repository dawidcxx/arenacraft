...
void Player::SavePositionInDB(WorldLocation const& loc, uint16 zoneId, ObjectGuid guid,
...
void Player::SendItemRetrievalMail(uint32 itemEntry, uint32 count) { SendItemRetrievalMail({{itemEntry, count}}); }
...
void Player::SendItemRetrievalMail(std::vector<std::pair<uint32, uint32>> mailItems)
{
  ...
  auto AddMailItem = [&allItems](uint32 itemEntry, uint32 itemCount)
  {
    ...
    {
      LOG_ERROR("entities.player.items", "> SendItemRetrievalMail: Incorrect item count ({}) for item id {}", itemEntry,
                itemCount);
      return;
    }
  ...
  for (auto const& items : allItems)
  {
    ...
    MailDraft  draft("Recovered Item",
                     "We recovered a lost item in the twisting nether and noted that it was yours.$B$BPlease find said "
                      "object enclosed."); // This is the text used in Cataclysm, it probably wasn't changed.
