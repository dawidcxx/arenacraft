...
void Player::Update(uint32 p_time)
{
  ...
  if (!IsPositionValid()) // pussywizard: will crash below at eg. GetZoneAndAreaId
  {
    LOG_INFO("misc", "Player::Update - invalid position ({:0.1f}, {:0.1f}, {:0.1f})! Map: {}, MapId: {}, {}",
             GetPositionX(), GetPositionY(), GetPositionZ(), (FindMap() ? FindMap()->GetId() : 0), GetMapId(),
             GetGUID().ToString());
  ...
  if (m_deathTimer > 0 && !GetMap()->Instanceable() && !HasPreventResurectionAura())
  {
...
{
  LOG_DEBUG("entities.player.skills", "UpdateGatherSkill(SkillId {} SkillLevel {} RedLevel {})", SkillId, SkillValue,
            RedLevel);

  uint32 gathering_skill_gain = sWorld->getIntConfig(CONFIG_SKILL_GAIN_GATHERING);
  sScriptMgr->OnUpdateGatheringSkill(this, SkillId, SkillValue, RedLevel + 100, RedLevel + 50, RedLevel + 25,
  ...
  for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first; _spell_idx != bounds.second; ++_spell_idx)
  {
    if (_spell_idx->second->SkillLine)
    {
      ...
      return UpdateSkillPro(
          ...
          SkillGainChance(SkillValue, _spell_idx->second->TrivialSkillLineRankHigh,
                          (_spell_idx->second->TrivialSkillLineRankHigh + _spell_idx->second->TrivialSkillLineRankLow) /
                              2,
...
bool Player::UpdateFishingSkill()
{
   ...
   */
  return UpdateSkillPro(SKILL_FISHING, static_cast<int32>(getProbabilityOfLevelUp(SkillValue)) * 10,
...
static const std::size_t bonusSkillLevelsSize = sizeof(bonusSkillLevels) / sizeof(uint32);
...
void Player::UpdateCombatSkills(Unit* victim, WeaponAttackType attType, bool defence, Item* item /*= nullptr*/)
{
  ...
  LOG_DEBUG(
      ...
      defence, playerLevel, moblevel, currentSkillValue, currentSkillMax, chance);
...
void Player::UpdateVisibilityForPlayer(bool mapChange)
{
  ...
  Acore::VisibleNotifier notifierNoLarge(*this, mapChange,
...
{
  // Prevent updating visibility if player is not in world (example: LoadFromDB sets drunkstate which updates
  // invisibility while player is not in map)
  if (!IsInWorld())
    return;
...
void Player::UpdateTriggerVisibility()
{
  ...
  for (GuidUnorderedSet::iterator itr = m_clientGUIDs.begin(); itr != m_clientGUIDs.end(); ++itr)
  {
    if ((*itr).IsCreatureOrVehicle())
    {
      ...
      if (!creature ||
          (!creature->IsTrigger() && !creature->HasTransformAura() && !creature->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE)))
        continue;
...
void Player::UpdateAchievementCriteria(AchievementCriteriaTypes type, uint32 miscValue1 /*= 0*/,
