...
void CinematicMgr::BeginCinematic()
{
  ...
  if (std::vector<FlyByCamera> const* flyByCameras = GetFlyByCameras(m_activeCinematicCameraId))
  {
    ...
    if (camitr != m_cinematicCamera->end())
    {
      ...
      m_CinematicObject = player->SummonCreature(VISUAL_WAYPOINT, pos.m_positionX, pos.m_positionY, pos.m_positionZ,
                                                 0.0f, TEMPSUMMON_TIMED_DESPAWN, 120000);
...
void CinematicMgr::UpdateCinematicLocation(uint32 /*diff*/)
{
  ...
  if (m_CinematicObject && interPosition.IsPositionValid())
    m_CinematicObject->MonsterMoveWithSpeed(interPosition.m_positionX, interPosition.m_positionY,
                                            interPosition.m_positionZ, 500.0f);
