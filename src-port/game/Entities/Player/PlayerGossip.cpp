...
void Player::PrepareGossipMenu(WorldObject* source, uint32 menuId /*= 0*/, bool showQuests /*= false*/)
{
  ...
  for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
  {
    ...
    if (Creature* creature = source->ToCreature())
    {
      ...
      case GOSSIP_OPTION_VENDOR:
      {
        ...
        {
          VendorItemData const* vendorItems = itr->second.ActionMenuID
                                                  ? sObjectMgr->GetNpcVendorItemList(itr->second.ActionMenuID)
                                                  : creature->GetVendorItems();
          if (!vendorItems || vendorItems->Empty())
          {
            LOG_ERROR("sql.sql", "Creature {} have UNIT_NPC_FLAG_VENDOR but have empty trading item list.",
      ...
      case GOSSIP_OPTION_OUTDOORPVP:
        ...
        break;
      default:
        LOG_ERROR("sql.sql", "Creature entry {} has unknown OptionType {} for menu {}", creature->GetEntry(),
                  itr->second.OptionType, itr->second.MenuID);
        canTalk = false;
        break;
      }
    }
    ...
    if (canTalk)
    {
      ...
      if (optionBroadcastText)
        ObjectMgr::GetLocaleString(getGender() == GENDER_MALE ? optionBroadcastText->MaleText
                                                              : optionBroadcastText->FemaleText,
                                   locale, strOptionText);

      if (boxBroadcastText)
        ObjectMgr::GetLocaleString(
            getGender() == GENDER_MALE ? boxBroadcastText->MaleText : boxBroadcastText->FemaleText, locale, strBoxText);
      ...
      menu->GetGossipMenu().AddMenuItem(itr->second.OptionID, itr->second.OptionIcon, strOptionText, 0,
                                        itr->second.OptionType, strBoxText, itr->second.BoxMoney, itr->second.BoxCoded);
      menu->GetGossipMenu().AddGossipMenuItemData(itr->second.OptionID, itr->second.ActionMenuID,
  ...
  if (sWorld->getIntConfig(CONFIG_INSTANT_TAXI) == 2 && npcflags & UNIT_NPC_FLAG_FLIGHTMASTER)
    menu->GetGossipMenu().AddMenuItem(
        -1, GOSSIP_ICON_INTERACT_1, GetSession()->GetAcoreString(LANG_TOGGLE_INSTANT_FLIGHT), 0,
        GOSSIP_ACTION_TOGGLE_INSTANT_FLIGHT, "", 0, false); // instant flight toggle option
}
...
void Player::OnGossipSelect(WorldObject* source, uint32 gossipListId, uint32 menuId)
{
  ...
  if (source->IsGameObject())
  {
    ...
    {
      LOG_ERROR("entities.player", "Player guid {} request invalid gossip option for GameObject entry {}",
                GetGUID().ToString(), source->GetEntry());
      return;
    }
  }
  ...
  case GOSSIP_OPTION_BATTLEFIELD:
  {
    ...
    {
      LOG_ERROR("entities.player", "A user ({}) requested battlegroundlist from a npc who is no battlemaster",
                GetGUID().ToString());
      return;
    }
...
void Player::ToggleInstantFlight() { m_isInstantFlightOn = !m_isInstantFlightOn; }
