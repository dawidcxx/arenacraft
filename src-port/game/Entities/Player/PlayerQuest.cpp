...
void Player::SendPreparedQuest(ObjectGuid guid)
{
  ...
  if (questMenu.GetMenuItemCount() == 1)
  {
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
      ...
      {
        Object* object =
            ObjectAccessor::GetObjectByTypeMask(*this, guid, TYPEMASK_UNIT | TYPEMASK_GAMEOBJECT | TYPEMASK_ITEM);
...
{
  return !DisableMgr::IsDisabledFor(DISABLE_TYPE_QUEST, quest->GetQuestId(), this) && SatisfyQuestStatus(quest, msg) &&
         SatisfyQuestExclusiveGroup(quest, msg) && SatisfyQuestClass(quest, msg) && SatisfyQuestRace(quest, msg) &&
         SatisfyQuestLevel(quest, msg) && SatisfyQuestSkill(quest, msg) && SatisfyQuestReputation(quest, msg) &&
         SatisfyQuestPreviousQuest(quest, msg) && SatisfyQuestTimed(quest, msg) && SatisfyQuestNextChain(quest, msg) &&
         SatisfyQuestPrevChain(quest, msg) && SatisfyQuestDay(quest, msg) && SatisfyQuestWeek(quest, msg) &&
         SatisfyQuestMonth(quest, msg) && SatisfyQuestSeasonal(quest, msg) && SatisfyQuestConditions(quest, msg);
}
...
bool Player::CanRewardQuest(Quest const* quest, bool msg)
{
  ...
  if (quest->HasSpecialFlag(QUEST_SPECIAL_FLAGS_DELIVER))
  {
    for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
    {
      if (quest->RequiredItemCount[i] != 0 && GetItemCount(quest->RequiredItemId[i]) < quest->RequiredItemCount[i])
      {
        if (msg)
...
bool Player::CanRewardQuest(Quest const* quest, uint32 reward, bool msg)
{
  ...
  if (quest->GetRewChoiceItemsCount() > 0)
  {
    ...
    {
      InventoryResult res = CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, quest->RewardChoiceItemId[reward],
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce, bool isLFGReward)
{
  ...
  {
    sScriptMgr->OnGivePlayerXP(this, XP, nullptr,
                               isLFGReward ? PlayerXPSource::XPSOURCE_QUEST_DF : PlayerXPSource::XPSOURCE_QUEST);
  ...
  if (uint32 mail_template_id = quest->GetRewMailTemplateId())
  {
    ...
    if (quest->GetRewMailSenderEntry() != 0)
      MailDraft(mail_template_id)
          .SendMailTo(trans, this, quest->GetRewMailSenderEntry(), MAIL_CHECK_MASK_HAS_BODY,
  ...
  for (; range.first != range.second; ++range.first)
  {
    ...
    if (GetQuestStatus(exclude_Id) != QUEST_STATUS_NONE ||
        (!(qInfo->IsRepeatable() && Nquest->IsRepeatable()) && !Nquest->IsSeasonal() &&
         IsQuestRewarded(
             exclude_Id))) // pussywizard: added !Nquest->IsSeasonal() because seasonal quests are considered rewarded
                           // only if finished this year, this is checked above in SatisfyQuestSeasonal
    {
      if (msg)
        SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
      return false;
...
QuestGiverStatus Player::GetQuestDialogStatus(Object* questgiver)
{
  ...
  default:
    ...
    // LOG_ERROR("entities.player.quest", "GetQuestDialogStatus called for unexpected type {}",
...
void Player::KilledMonsterCredit(uint32 entry, ObjectGuid guid)
{
  ...
  UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, real_entry, addkillcount,
...
void Player::KilledPlayerCredit(uint16 count)
{
  for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
  {
    ...
    if (q_status.Status == QUEST_STATUS_INCOMPLETE &&
        (!GetGroup() || !GetGroup()->isRaidGroup() || qInfo->IsAllowedInRaid(GetMap()->GetDifficulty())))
    {
      // Xinef: PvP Killing quest require player to be in same zone as quest zone (only 2 quests so no doubt, can be
      // extended to conditions in cata ;s)
      if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAGS_PLAYER_KILL) &&
...
bool Player::HasQuestForItem(uint32 itemid, uint32 excludeQuestId /* 0 */, bool turnIn /* false */,
  ...
  for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
  {
    ...
    if ((q_status.Status == QUEST_STATUS_INCOMPLETE) || (turnIn && q_status.Status == QUEST_STATUS_COMPLETE))
    {
      ...
      if (GetGroup() && GetGroup()->isRaidGroup() && !qinfo->IsAllowedInRaid(GetMap()->GetDifficulty()))
      {
        if (!InBattleground() &&
            !GetGroup()
                 ->isBFGroup()) // there are two ways.. we can make every bg-quest a raidquest, or add this code here..
                                // i don't know if this can be exploited by other quests, but i think all other quests
                                // depend on a specific area.. but keep this in mind, if something strange happens later
        {
          continue;
        }
      }
...
void Player::SendQuestUpdateAddCreatureOrGo(Quest const* quest, ObjectGuid guid, uint32 creatureOrGO_idx,
                                            uint16 old_count, uint16 add_count)
{
