...
enum class GameObjectActions : uint32
{
  ...
  OpenAndUnlock,  // Open + Unlock                    // Sets GO_STATE_ACTIVE and
                  // resets GO_FLAG_LOCKED
...
{
  GO_NOT_READY,
  GO_READY, // can be ready but despawned, and then not possible activate until
            // spawn
  GO_ACTIVATED,
  GO_JUST_DEACTIVATED
};
...
public:
  ...
  virtual bool Create(ObjectGuid::LowType guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y,
  ...
  {
    return m_respawnDelayTime == 0 || (m_respawnTime > 0 && !m_spawnedByDefault) ||
  ...
  // sets GameObject type 33 destruction flags and optionally default health for
  // that state
...
protected:
  ...
  ObjectGuid::LowType m_spawnId; ///< For new or temporary gameobjects is 0 for
                                 ///< saved it is lowguid
