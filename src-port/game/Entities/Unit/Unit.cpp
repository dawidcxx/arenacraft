...
DamageInfo::DamageInfo(DamageInfo const& dmg1, DamageInfo const& dmg2)
    : m_attacker(dmg1.m_attacker), m_victim(dmg1.m_victim), m_damage(dmg1.m_damage + dmg2.m_damage),
      m_spellInfo(dmg1.m_spellInfo), m_schoolMask(SpellSchoolMask(dmg1.m_schoolMask | dmg2.m_schoolMask)),
      m_damageType(dmg1.m_damageType), m_attackType(dmg1.m_attackType), m_absorb(dmg1.m_absorb + dmg2.m_absorb),
...
DamageInfo::DamageInfo(CalcDamageInfo const& dmgInfo, uint8 damageIndex)
    : m_attacker(dmgInfo.attacker), m_victim(dmgInfo.target), m_damage(dmgInfo.damages[damageIndex].damage),
      m_spellInfo(nullptr), m_schoolMask(SpellSchoolMask(dmgInfo.damages[damageIndex].damageSchoolMask)),
      m_damageType(DIRECT_DAMAGE), m_attackType(dmgInfo.attackType), m_absorb(dmgInfo.damages[damageIndex].absorb),
      m_resist(dmgInfo.damages[damageIndex].resist), m_block(dmgInfo.blocked_amount), m_cleanDamage(dmgInfo.cleanDamage)
{
}

DamageInfo::DamageInfo(SpellNonMeleeDamage const& spellNonMeleeDamage, DamageEffectType damageType)
    : m_attacker(spellNonMeleeDamage.attacker), m_victim(spellNonMeleeDamage.target),
      m_damage(spellNonMeleeDamage.damage), m_spellInfo(spellNonMeleeDamage.spellInfo),
      m_schoolMask(SpellSchoolMask(spellNonMeleeDamage.schoolMask)), m_damageType(damageType),
...
uint32 DamageInfo::GetUnmitigatedDamage() const { return m_damage + m_cleanDamage + m_absorb + m_resist; }

ProcEventInfo::ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget, uint32 typeMask, uint32 spellTypeMask,
                             uint32 spellPhaseMask, uint32 hitMask, Spell const* spell, DamageInfo* damageInfo,
                             HealInfo* healInfo, SpellInfo const* triggeredByAuraSpell, int8 procAuraEffectIndex)
    : _actor(actor), _actionTarget(actionTarget), _procTarget(procTarget), _typeMask(typeMask),
      _spellTypeMask(spellTypeMask), _spellPhaseMask(spellPhaseMask), _hitMask(hitMask), _spell(spell),
      _damageInfo(damageInfo), _healInfo(healInfo), _triggeredByAuraSpell(triggeredByAuraSpell),
...
Unit::Unit(bool isWorldObject)
    : WorldObject(isWorldObject), m_movedByPlayer(nullptr), m_lastSanctuaryTime(0), IsAIEnabled(false),
      NeedChangeAI(false), m_ControlledByPlayer(false), m_CreatedByPlayer(false),
...
void Unit::Update(uint32 p_time)
{
  ...
  {
    // Check UNIT_STATE_MELEE_ATTACKING or UNIT_STATE_CHASE (without UNIT_STATE_FOLLOW in this case) so pets can reach
    // far away targets without stopping half way there and running off. These flags are reset after target dies or
...
bool Unit::GetRandomContactPoint(Unit const* obj, float& x, float& y, float& z, bool force) const
{
  ...
  GetNearPoint(
      obj, x, y, z,
      isMoving() ? (obj->GetCombatReach() > 7.75f ? obj->GetCombatReach() - 7.5f : 0.25f) : obj->GetCombatReach(), 0.0f,
...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage,
                        DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto,
                        bool durabilityLoss, bool /*allowGM*/, Spell const* damageSpell /*= nullptr*/)
{
  ...
  {
    // interrupting auras with AURA_INTERRUPT_FLAG_DAMAGE before checking !damage (absorbed damage breaks that type of
    // auras)
    if (spellProto)
    {
    ...
    for (AuraEffectList::iterator i = vCopyDamageCopy.begin(); i != vCopyDamageCopy.end(); ++i)
    {
      ...
      {
        attacker->SendSpellNonMeleeDamageLog(shareDamageTarget, spell, shareDamage, damageSchoolMask, shareAbsorb,
                                             shareResist, damagetype == DIRECT_DAMAGE, 0, false, true);
      }

      Unit::DealDamage(attacker, shareDamageTarget, shareDamage, cleanDamage, NODAMAGE, damageSchoolMask, spellProto,
                       false, false, damageSpell);
    }
  }
  ...
  if (attacker && attacker != victim)
    ...
    {
      // pussywizard: don't allow GMs to deal damage in normal way (this leaves no evidence in logs!), they have
      // commands to do so
      // if (!allowGM && killer->GetSession()->GetSecurity() && killer->GetSession()->GetSecurity() <=
      // SEC_ADMINISTRATOR)
      //  return 0;
      ...
      if (Battleground* bg = killer->GetBattleground())
      {
        ...
        killer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_DAMAGE_DONE, damage, 0,
                                          victim); // pussywizard: InBattleground() optimization
      }
      // killer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_DEALT, damage); // pussywizard:
      // optimization
    }
...
SpellCastResult Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo,
                                CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem,
...
SpellCastResult Unit::CastSpell(Unit* victim, uint32 spellId, bool triggered, Item* castItem,
                                AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
  return CastSpell(victim, spellId, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura,
                   originalCaster);
}
...
SpellCastResult Unit::CastSpell(Unit* victim, SpellInfo const* spellInfo, bool triggered, Item* castItem /*= nullptr*/,
                                ...
                                ObjectGuid        originalCaster /*= ObjectGuid::Empty*/)
{
  return CastSpell(victim, spellInfo, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura,
                   originalCaster);
}
...
SpellCastResult Unit::CastCustomSpell(Unit* target, uint32 spellId, int32 const* bp0, int32 const* bp1,
                                      int32 const* bp2, bool triggered, Item* castItem,
                                      AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
  ...
  return CastCustomSpell(spellId, values, target, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem,
...
SpellCastResult Unit::CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* target,
                                      TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura,
                                ...
                                AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
  ...
  return CastSpell(targets, spellInfo, nullptr, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem,
                   triggeredByAura, originalCaster);
}
...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss, Spell const* spell /*= nullptr*/)
{
  ...
  Unit::DealDamage(this, victim, damageInfo->damage, &cleanDamage, SPELL_DIRECT_DAMAGE,
                   SpellSchoolMask(damageInfo->schoolMask), spellProto, durabilityLoss, false, spell);
}
                                ...
                                const bool sittingVictim)
{
  ...
  for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
  {
    ...
    if (CanApplyResilience())
    {
      ...
      Unit::ApplyResilience(victim, nullptr, &resilienceReduction, (damageInfo->hitOutCome == MELEE_HIT_CRIT),
                            CR_CRIT_TAKEN_MELEE);
    ...
    if (damageInfo->damages[i].damage > 0)
    {
      ...
      {
        tmpHitInfo[i] |= (damageInfo->damages[i].damage - damageInfo->damages[i].absorb == 0 ? HITINFO_FULL_ABSORB
                                                                                             : HITINFO_PARTIAL_ABSORB);
      }

      if (damageInfo->damages[i].resist)
      {
        tmpHitInfo[i] |= (damageInfo->damages[i].damage - damageInfo->damages[i].resist == 0 ? HITINFO_FULL_RESIST
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
  ...
  for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
  {
    ...
    Unit::DealDamage(this, victim, damageInfo->damages[i].damage, &cleanDamage, DIRECT_DAMAGE,
                     SpellSchoolMask(damageInfo->damages[i].damageSchoolMask), nullptr, durabilityLoss);
  }
...
uint32 Unit::CalcArmorReducedDamage(Unit const* attacker, Unit const* victim, const uint32 damage,
...
void Unit::CalcAbsorbResist(DamageInfo& dmgInfo, bool Splited)
{
  ...
  if (!(schoolMask & SPELL_SCHOOL_MASK_NORMAL) && (!(schoolMask & SPELL_SCHOOL_MASK_HOLY) || victim->IsCreature()) &&
      (!spellInfo ||
       (!spellInfo->HasAttribute(SPELL_ATTR0_CU_BINARY_SPELL) && !spellInfo->HasAttribute(SPELL_ATTR4_NO_CAST_LOG))))
  {
       ...
       (itr != vSchoolAbsorbCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
  {
    ...
    absorbAurEff->GetBase()->CallScriptEffectAbsorbHandlers(absorbAurEff, aurApp, dmgInfo, tempAbsorb,
                                                            defaultPrevented);
       ...
       (itr != vManaShieldCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
  {
    ...
    absorbAurEff->GetBase()->CallScriptEffectManaShieldHandlers(absorbAurEff, aurApp, dmgInfo, tempAbsorb,
                                                                defaultPrevented);
    currentAbsorb = tempAbsorb;
    ...
    if (float manaMultiplier = absorbAurEff->GetSpellInfo()->Effects[absorbAurEff->GetEffIndex()].CalcValueMultiplier(
  ...
  if (/*victim != attacker &&*/ !Splited)
  {
    ...
    for (AuraEffectList::iterator itr = vSplitDamageFlatCopy.begin();
         (itr != vSplitDamageFlatCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
      // Check if aura was removed during iteration - we don't need to work on such auras
      ...
      caster->ProcDamageAndSpellFor(true, attacker, procVictim, procEx, BASE_ATTACK, spellInfo, splitted, nullptr, -1,
                                    nullptr, &splittedDmgInfo);

      if (attacker)
      {
        attacker->SendSpellNonMeleeDamageLog(caster, (*itr)->GetSpellInfo(), splitted, schoolMask, splitted_absorb,
                                             splitted_resist, false, 0, false, true);
      }

      CleanDamage cleanDamage = CleanDamage(splitted, 0, BASE_ATTACK, MELEE_HIT_NORMAL);
      Unit::DealDamage(attacker, caster, splitted, &cleanDamage, DIRECT_DAMAGE, schoolMask, (*itr)->GetSpellInfo(),
                       false);
    }
    ...
    for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(), next;
         (itr != vSplitDamagePctCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
      // Check if aura was removed during iteration - we don't need to work on such auras
      ...
      if (attacker)
      {
        attacker->SendSpellNonMeleeDamageLog(caster, splitSpellInfo, splitted, splitSchoolMask, splitted_absorb,
                                             splitted_resist, false, 0, false, true);
      }
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType /*= BASE_ATTACK*/, bool extra /*= false*/,
                               bool ignoreCasting /*= false*/)
{
  ...
  if (meleeAttack)
  {
    ...
    Unit::ProcDamageAndSpell(damageInfo.attacker, damageInfo.target, damageInfo.procAttacker, damageInfo.procVictim,
                             damageInfo.procEx, dmgInfo.GetDamage(), damageInfo.attackType, nullptr, nullptr, -1,
                             nullptr, &dmgInfo);

    if (IsPlayer())
      LOG_DEBUG("entities.unit",
                "AttackerStateUpdate: (Player) {} attacked {} for {} dmg, absorbed {}, blocked {}, resisted {}.",
                GetGUID().ToString(), victim->GetGUID().ToString(), dmgInfo.GetDamage(), dmgInfo.GetAbsorb(),
                dmgInfo.GetBlock(), dmgInfo.GetResist());
    else
      LOG_DEBUG("entities.unit",
...
bool Unit::GetMeleeAttackPoint(Unit* attacker, Position& pos)
{
  ...
  for (auto const& otherAttacker : attackers)
  {
    ...
    if (!otherAttacker || otherAttacker->GetGUID() == attacker->GetGUID() || !otherAttacker->IsWithinMeleeRange(this) ||
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(Unit const* victim, WeaponAttackType attType) const
{
  ...
  float miss_chance = MeleeSpellMissChance(
      victim, attType, int32(GetWeaponSkillValue(attType, victim)) - int32(victim->GetMaxSkillValueForLevel(this)), 0);
  ...
  // LOG_DEBUG("entities.unit", "MELEE OUTCOME: miss {} crit {} dodge {} parry {} block {}", miss_chance, crit_chance,
  // dodge_chance, parry_chance, block_chance);

  return RollMeleeOutcomeAgainst(victim, attType, int32(crit_chance * 100), int32(miss_chance * 100),
                                 int32(dodge_chance * 100), int32(parry_chance * 100), int32(block_chance * 100));
}
                                              ...
                                              int32 block_chance) const
{
  ...
  sScriptMgr->OnBeforeRollMeleeOutcomeAgainst(this, victim, attType, attackerMaxSkillValueForLevel,
  ...
  // Max 40% chance to score a glancing blow against mobs that are higher level (can do only players and pets and not
  // with ranged weapon)
...
{
  // pussywizard: calling SendMeleeAttackStop without clearing UNIT_STATE_MELEE_ATTACKING and then AttackStart the same
  // player may spoil npc rotating! pussywizard: this happens in some boss scripts, just add clearing here
  ...
  if (victim)
    LOG_DEBUG("entities.unit", "{} {} stopped attacking {} {}", (IsPlayer() ? "Player" : "Creature"),
              GetGUID().ToString(), (victim->IsPlayer() ? "player" : "creature"), victim->GetGUID().ToString());
  else
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
  ...
  bool canBlock =
      spellInfo->HasAttribute(SPELL_ATTR3_COMPLETELY_BLOCKED) && !spellInfo->HasAttribute(SPELL_ATTR0_CU_DIRECT_DAMAGE);
  ...
  if (attType == RANGED_ATTACK)
  {
    ...
    if (!victim->HasUnitState(UNIT_STATE_STUNNED) &&
        (victim->HasInArc(M_PI, this) || victim->HasIgnoreHitDirectionAura()))
    {
  ...
  for (AuraEffectList::const_iterator i = ignore.begin(); i != ignore.end(); ++i)
  {
    ...
    case MELEE_HIT_PARRY:
      ...
      break;
    default:
      LOG_DEBUG("entities.unit", "Spell {} SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state {}", (*i)->GetId(),
                (*i)->GetMiscValue());
      break;
    }
  }
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
  ...
  // xinef: imo it should completly ignore all calculations, eg: 14792. Hits 80 level players on blizz without any
  // problems
  ...
  if (!spellInfo->IsPositive() && !spellInfo->HasAttribute(SPELL_ATTR4_NO_CAST_LOG))
  {
    ...
    if (bNegativeAura)
    {
      tmp += victim->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(spellInfo->Dispel)) *
             100;
      tmp += victim->GetMaxNegativeAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(spellInfo->Dispel)) *
             100;
    }
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spell, bool CanReflect)
{
  ...
  if (CanReflect)
  {
    ...
    if (reflectchance > 0 && roll_chance_i(reflectchance))
    {
      ...
      // ProcDamageAndSpell(victim, PROC_FLAG_NONE, PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, 1,
      // BASE_ATTACK, spell);
...
uint32 Unit::GetWeaponSkillValue(WeaponAttackType attType, Unit const* target) const
{
  ...
  if (Player const* player = ToPlayer())
  {
    ...
    // in PvP use full skill instead current skill value
    value = (target && target->IsControlledByPlayer()) ? player->GetMaxSkillValue(skill) : player->GetSkillValue(skill);
...
void Unit::_UpdateSpells(uint32 time)
{
  ...
  // m_auraUpdateIterator can be updated in indirect called code at aura remove to skip next planned to update but
  // removed auras
...
bool Unit::IsNonMeleeSpellCast(bool withDelayed, bool skipChanneled, bool skipAutorepeat, bool isAutoshoot,
                               bool skipInstant) const
{
      ...
      (withDelayed || m_currentSpells[CURRENT_GENERIC_SPELL]->getState() != SPELL_STATE_DELAYED))
  {
    ...
    {
      if (!isAutoshoot ||
          !m_currentSpells[CURRENT_GENERIC_SPELL]->m_spellInfo->HasAttribute(SPELL_ATTR2_DO_NOT_RESET_COMBAT_TIMERS))
        return true;
    }
  }
  ...
  {
    if (!isAutoshoot ||
        !m_currentSpells[CURRENT_CHANNELED_SPELL]->m_spellInfo->HasAttribute(SPELL_ATTR2_DO_NOT_RESET_COMBAT_TIMERS))
      return true;
  }
...
bool Unit::isInAccessiblePlaceFor(Creature const* c) const
{
  ...
  else if (c->GetMapId() == 631) // Icecrown Citadel
  {
    ...
    // special handling for ICC (map 631), for non-flying pets in Gunship Battle, for trash npcs this is done via
    // CanAIAttack
...
void SafeUnitPointer::UnitDeleted()
{
  ...
  if (defaultValue)
  {
    ...
    {
      LOG_INFO("misc", "SafeUnitPointer::UnitDeleted (A1) - {}, {}, {}, {}, {}, {}, {}, {}", p->GetGUID().ToString(),
               p->GetMapId(), p->GetInstanceId(), p->FindMap()->GetId(), p->IsInWorld() ? 1 : 0,
               p->IsDuringRemoveFromWorld() ? 1 : 0, p->IsBeingTeleported() ? 1 : 0, p->isBeingLoaded() ? 1 : 0);
      if (ptr)
...
bool Unit::IsInWater() const { return (GetLiquidData().Status & MAP_LIQUID_STATUS_SWIMMING) != 0; }

bool Unit::IsUnderWater() const { return GetLiquidData().Status == LIQUID_MAP_UNDER_WATER; }
                                                 ...
                                                 bool       periodicReset /*= false*/)
{
  ...
  if (!newAura->IsMultiSlotAura())
  {
    ...
    if (Aura* foundAura = GetOwnedAura(
            newAura->Id, newAura->HasAttribute(SPELL_ATTR0_CU_SINGLE_AURA_STACK) ? ObjectGuid::Empty : casterGUID,
...
void Unit::_AddAura(UnitAura* aura, Unit* caster)
{
  ...
  aura->SetIsSingleTarget(caster &&
                          (aura->GetSpellInfo()->IsSingleTarget() || aura->HasEffectType(SPELL_AURA_CONTROL_VEHICLE)));
...
Aura* Unit::GetOwnedAura(uint32 spellId, ObjectGuid casterGUID, ObjectGuid itemCasterGUID, uint8 reqEffMask,
                         Aura* except) const
{
...
void Unit::RemoveAurasDueToSpellByDispel(uint32 spellId, uint32 dispellerSpellId, ObjectGuid casterGUID,
                                         Unit* dispeller, uint8 chargesRemoved /*= 1*/)
{
  AuraMapBoundsNonConst range = m_ownedAuras.equal_range(spellId);
  for (AuraMap::iterator iter = range.first; iter != range.second;)
  {
    ...
    if (aura->GetCasterGUID() == casterGUID)
    {
        ...
        if (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x1000)
        {
          if (Unit* caster = aura->GetCaster())
          {
            if (AuraEffect* aureff =
                    caster->GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS, SPELLFAMILY_HUNTER, 3521, 1))
            {
      ...
      case SPELLFAMILY_DEATHKNIGHT:
      {
        ...
        {
          if (AuraEffect* aureff =
                  GetAuraEffect(SPELL_AURA_MOD_DECREASE_SPEED, SPELLFAMILY_DEATHKNIGHT, 0, 0x40000, 0, casterGUID))
    ...
    if (aura->GetCasterGUID() == casterGUID)
    {
      ...
      else
      {
        ...
        if (!curAura || (!curAura->IsPermanent() && curAura->GetDuration() < (int32)dur))
          if (Aura* newAura = Aura::TryRefreshStackOrCreate(aura->GetSpellInfo(), effMask, stealer, nullptr,
                                                            &baseDamage[0], nullptr, aura->GetCasterGUID()))
          {
            ...
            newAura->SetLoadedState(aura->GetMaxDuration(), int32(dur), stealCharge ? 1 : aura->GetCharges(), 1,
...
void Unit::RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3,
...
void Unit::DelayOwnedAuras(uint32 spellId, ObjectGuid caster, int32 delaytime)
{
  ...
  for (; range.first != range.second; ++range.first)
  {
    ...
    if (!caster || aura->GetCasterGUID() == caster)
    {
      ...
      LOG_DEBUG("spells.aura", "Aura {} partially interrupted on unit {}, new duration: {} ms", aura->GetId(),
                GetGUID().ToString(), aura->GetDuration());
    }
  }
}
...
AuraApplication* Unit::GetAuraApplication(uint32 spellId, ObjectGuid casterGUID, ObjectGuid itemCasterGUID,
                                          uint8 reqEffMask, AuraApplication* except) const
{
  AuraApplicationMapBounds range = m_appliedAuras.equal_range(spellId);
  for (; range.first != range.second; ++range.first)
  {
...
AuraApplication* Unit::GetAuraApplicationOfRankedSpell(uint32 spellId, ObjectGuid casterGUID, ObjectGuid itemCasterGUID,
                                                       uint8 reqEffMask, AuraApplication* except) const
{
  uint32 rankSpell = sSpellMgr->GetFirstSpellInChain(spellId);
  while (rankSpell)
...
Aura* Unit::GetAuraOfRankedSpell(uint32 spellId, ObjectGuid casterGUID, ObjectGuid itemCasterGUID,
                                  ...
                                  SpellInfo const* dispelSpell)
{
  ...
  for (Unit::VisibleAuraMap::const_iterator itr = visibleAuras->begin(); itr != visibleAuras->end(); ++itr)
  {
    ...
    if (aura->GetSpellInfo()->GetDispelMask() & dispelMask)
    {
      ...
      // Ie: A dispel on a target with 5 stacks of Winters Chill and a Polymorph has 1 / (1 + 1) -> 50% chance to
      // dispell Polymorph instead of 1 / (5 + 1) -> 16%.
...
void Unit::SendSpellNonMeleeReflectLog(SpellNonMeleeDamage* log, Unit* attacker)
{
  ...
  // If we are in cheat mode we swap absorb with damage and set damage to 0, this way we can still debug damage but our
  ...
  data << uint8(log->physicalLog); // if 1, then client show spell name (example: %s's ranged shot hit %s for %u school
...
void Unit::SendSpellNonMeleeDamageLog(SpellNonMeleeDamage* log)
{
  ...
  data << uint8(log->HitInfo &
                (SPELL_HIT_TYPE_CRIT_DEBUG | SPELL_HIT_TYPE_HIT_DEBUG | SPELL_HIT_TYPE_ATTACK_TABLE_DEBUG));
...
void Unit::ProcDamageAndSpell(Unit* actor, Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procExtra,
                              uint32 amount, WeaponAttackType attType, SpellInfo const* procSpellInfo,
                              SpellInfo const* procAura, int8 procAuraEffectIndex, Spell const* procSpell,
                              DamageInfo* damageInfo, HealInfo* healInfo, uint32 procPhase)
{
  // Not much to do if no flags are set.
  if (procAttacker && actor)
    actor->ProcDamageAndSpellFor(false, victim, procAttacker, procExtra, attType, procSpellInfo, amount, procAura,
                                 procAuraEffectIndex, procSpell, damageInfo, healInfo, procPhase);
  // Now go on with a victim's events'n'auras
  // Not much to do if no flags are set or there is no victim
  if (victim && victim->IsAlive() && procVictim)
    victim->ProcDamageAndSpellFor(true, actor, procVictim, procExtra, attType, procSpellInfo, amount, procAura,
                                  procAuraEffectIndex, procSpell, damageInfo, healInfo, procPhase);
}
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
  ...
  {
    // IF we are in cheat mode we swap absorb with damage and set damage to 0, this way we can still debug damage but
    // our hp bar will not drop
...
void Unit::SendAttackStateUpdate(uint32 HitInfo, Unit* target, uint8 /*SwingType*/, SpellSchoolMask damageSchoolMask,
                                 uint32 Damage, uint32 AbsorbDamage, uint32 Resist, VictimState TargetState,
                               ...
                               uint32 procFlag, uint32 procEx, uint32 cooldown, ProcEventInfo const& eventInfo)
{
  ...
  case SPELLFAMILY_MAGE:
  {
    ...
    case 56375:
    {
      if (!target)
        return false;
      target->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE, ObjectGuid::Empty,
  ...
  case SPELLFAMILY_DRUID:
  {
    ...
    case 54815:
    {
      ...
      if (AuraEffect const* AurEff =
              target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, 0x00800000, 0x0, 0x0, GetGUID()))
      {
    ...
    [[fallthrough]]; /// @todo: Not sure whether the fallthrough was a mistake (forgetting a break) or intended. This
  ...
  case SPELLFAMILY_ROGUE:
  {
    ...
    case 56800:
    {
      if (victim)
        if (AuraEffect* aurEff =
                victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x100000, 0, 0, GetGUID()))
    ...
    case 2236: // Thrill of the Hunt
    {
      ...
      if (procSpell->SpellFamilyFlags[2] & 0x200)
      {
        ...
        if (AuraEffect const* pEff =
                victim->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY, SPELLFAMILY_HUNTER, 0x0, 0x80000000, 0x0, GetGUID()))
          basepoints0 =
              pEff->GetSpellInfo()->CalcPowerCost(this, SpellSchoolMask(pEff->GetSpellInfo()->SchoolMask)) * 4 / 10 / 3;
      }
      else
  ...
  case SPELLFAMILY_PALADIN:
  {
    ...
    if (dummySpell->Id == 53651)
    {
      ...
      victim->CastCustomSpell(beaconTarget, triggered_spell_id, &basepoints0, nullptr, nullptr, true, 0,
                              triggeredByAura, victim->GetGUID());
      return true;
    }
  ...
  case SPELLFAMILY_SHAMAN:
  {
    ...
    case 70817:
    {
      ...
      if (AuraEffect const* aurEff =
              target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_SHAMAN, 0x10000000, 0x0, 0x0, GetGUID()))
      {
  ...
  if (!triggerEntry)
  {
    LOG_ERROR("entities.unit", "Unit::HandleDummyAuraProc: Spell {} has non-existing triggered spell {}",
  ...
  case SPELLFAMILY_MAGE:
  {
    ...
    case 31658:
    {
      *handled = true;
      ...
      CastCustomSpell(this, 67545, &bp0, nullptr, nullptr, true, nullptr, triggeredByAura->GetEffect(EFFECT_0),
                      GetGUID());
      return true;
    }
    }
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell,
                                  uint32 procFlags, uint32 procEx, uint32 cooldown, uint32 procPhase,
                                  ProcEventInfo& eventInfo)
{
    ...
    case SPELLFAMILY_MAGE:
      if (auraSpellInfo->SpellIconID == 2127) // Blazing Speed
      {
        ...
        case 31642: // Rank 2
          ...
          break;
        default:
          LOG_ERROR("entities.unit", "Unit::HandleProcTriggerSpell: Spell {} miss posibly Blazing Speed",
                    auraSpellInfo->Id);
          return false;
        }
      }
    ...
    case SPELLFAMILY_HUNTER:
    {
      if (auraSpellInfo->SpellIconID == 3247) // Piercing Shots
      {
        ...
        case 53238: // Rank 3
          ...
          break;
        default:
          LOG_ERROR("entities.unit", "Unit::HandleProcTriggerSpell: Spell {} miss posibly Piercing Shots",
                    auraSpellInfo->Id);
          return false;
        }
    ...
    case SPELLFAMILY_PALADIN:
    {
      ...
      default:
        ...
        if (auraSpellInfo->SpellIconID == 241)
        {
          ...
          int32 cost =
              int32(originalSpell->ManaCost + CalculatePct(GetCreateMana(), originalSpell->ManaCostPercentage));
  ...
  if (!triggerEntry)
  {
    ...
    LOG_ERROR(
        ...
        auraSpellInfo->Id, triggeredByAura->GetEffIndex(), trigger_spell_id);
    return false;
  }
  ...
  case 12867:
  {
    ...
    if (procFlags & PROC_FLAG_DONE_OFFHAND_ATTACK)
      basepoints0 =
          int32((GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE) + GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE)) / 2.0f);
    else
  ...
  case 58679:
  {
    // Proc only from healing part of Death Coil. Check is essential as all Death Coil spells have 0x2000 mask in
    // SpellFamilyFlags
    if (!procSpell ||
        !(procSpell->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && procSpell->SpellFamilyFlags[0] == 0x80002000))
      return false;
    break;
  }
  // Glyph of Death Grip
...
bool Unit::HandleOverrideClassScriptAuraProc(Unit* victim, uint32 /*damage*/, AuraEffect* triggeredByAura,
                                             SpellInfo const* procSpell, uint32 cooldown)
{
  ...
  if (!triggerEntry)
  {
    LOG_ERROR("entities.unit", "Unit::HandleOverrideClassScriptAuraProc: Spell {} triggering for class script id {}",
...
void Unit::setPowerType(Powers new_powertype)
{
  ...
  if (Player const* player = ToPlayer())
    if (player->NeedSendSpectatorData())
    {
      ...
      ArenaSpectator::SendCommand_UInt32Value(FindMap(), GetGUID(), "MPW",
                                              new_powertype == POWER_RAGE || new_powertype == POWER_RUNIC_POWER
                                                  ? GetMaxPower(new_powertype) / 10
                                                  : GetMaxPower(new_powertype));
      ArenaSpectator::SendCommand_UInt32Value(FindMap(), GetGUID(), "CPW",
                                              new_powertype == POWER_RAGE || new_powertype == POWER_RUNIC_POWER
...
FactionTemplateEntry const* Unit::GetFactionTemplateEntry() const
{
  ...
  if (!entry)
  {
    ...
    {
      if (Player const* player = ToPlayer())
        LOG_ERROR("entities.unit", "Player {} has invalid faction (faction template id) #{}", player->GetName(),
                  GetFaction());
      else if (Creature const* creature = ToCreature())
        LOG_ERROR("entities.unit", "Creature (template id: {}) has invalid faction (faction template id) #{}",
                  creature->GetCreatureTemplate()->Entry, GetFaction());
      else
        LOG_ERROR("entities.unit", "Unit (name={}, type={}) has invalid faction (faction template id) #{}", GetName(),
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
  ...
  // if (HasUnitFlag(UNIT_FLAG_PACIFIED)) // pussywizard: why having this flag prevents from entering combat? it should
  // just prevent melee attack
  //     return false;
  ...
  if (HasOffhandWeaponForAttack() && isAttackReady(OFF_ATTACK))
    setAttackTimer(OFF_ATTACK, std::max(getAttackTimer(OFF_ATTACK),
                                        getAttackTimer(BASE_ATTACK) +
                                            int32(CalculatePct(GetFloatValue(UNIT_FIELD_BASEATTACKTIME), 50))));
  ...
  if (apply)
  {
    if (IsPlayer())
    {
      if (!AddGuidValue(UNIT_FIELD_CHARM, charm->GetGUID()))
        LOG_FATAL("entities.unit", "Player {} is trying to charm unit {}, but it already has a charmed unit {}",
                  GetName(), charm->GetEntry(), GetCharmGUID().ToString());
    ...
    if (!charm->AddGuidValue(UNIT_FIELD_CHARMEDBY, GetGUID()))
      LOG_FATAL("entities.unit", "Unit {} is being charmed, but it already has a charmer {}", charm->GetEntry(),
  ...
  else
  {
    if (IsPlayer())
    {
      if (!RemoveGuidValue(UNIT_FIELD_CHARM, charm->GetGUID()))
        LOG_FATAL("entities.unit", "Player {} is trying to uncharm unit {}, but it has another charmed unit {}",
                  GetName(), charm->GetEntry(), GetCharmGUID().ToString());
    }

    if (!charm->RemoveGuidValue(UNIT_FIELD_CHARMEDBY, GetGUID()))
      LOG_FATAL("entities.unit", "Unit {} is being uncharmed, but it has another charmer {}", charm->GetEntry(),
                charm->GetCharmerGUID().ToString());
...
int32 Unit::DealHeal(Unit* healer, Unit* victim, uint32 addhealth)
{
  ...
  if (Player* player = unit->ToPlayer())
  {
    ...
    // player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEAL_CASTED, addhealth); // pussywizard:
    // optimization
  }

  /*if (Player* player = victim->ToPlayer())
  {
      //player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_TOTAL_HEALING_RECEIVED, gain); // pussywizard:
  optimization
...
Unit* Unit::GetMagicHitRedirectTarget(Unit* victim, SpellInfo const* spellInfo)
{
  ...
  for (Unit::AuraEffectList::const_iterator itr = magnetAuras.begin(); itr != magnetAuras.end(); ++itr)
  {
    if (Unit* magnet = (*itr)->GetBase()->GetUnitOwner())
      ...
      {
        // Xinef: We should choose minimum between flight time and queue time as in reflect, however we dont know flight
        // time at this point, use arbitrary small number
        magnet->m_Events.AddEvent(new RedirectSpellEvent(*magnet, victim->GetGUID(), *itr),
...
void Unit::RemoveAllControlled(bool onDeath /*= false*/)
{
  ...
  while (!m_Controlled.empty())
  {
    ...
    {
      LOG_ERROR("entities.unit", "Unit {} is trying to release unit {} which is neither charmed nor owned by it",
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
  ...
  // reserve place for players and pets because resizing vector every unit push is unefficient (vector is reallocated
  // then)
...
float Unit::SpellPctDamageModsDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype)
{
  ...
  {
    // prevent apply mods from weapon specific case to non weapon specific spells (Example: thunder clap and two-handed
                                ...
                                WeaponAttackType attackType, bool skipEffectCheck) const
{
  ...
  case SPELL_DAMAGE_CLASS_MAGIC:
  {
    ...
    // For other schools
      ...
      crit_chance =
          GetFloatValue(static_cast<uint16>(PLAYER_SPELL_CRIT_PERCENTAGE1) + GetFirstSchoolInMask(schoolMask));
    else
    {
                                      ...
                                      Unit const* victim)
{
  ...
  if (caster)
  {
    ...
    if (victim)
      crit_mod +=
          caster->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_CRIT_PERCENT_VERSUS, victim->GetCreatureTypeMask());
...
uint32 Unit::SpellCriticalHealingBonus(Unit const* caster, SpellInfo const* spellProto, uint32 damage,
                                       Unit const* victim)
{
  // Calculate critical bonus
  int32 crit_bonus;
  ...
  if (caster)
  {
    if (victim)
    {
      uint32 creatureTypeMask = victim->GetCreatureTypeMask();
      crit_bonus              = int32(
          crit_bonus * caster->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_CRIT_PERCENT_VERSUS, creatureTypeMask));
    }
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount,
                                    DamageEffectType damagetype, uint32 stack)
{
  float TakenTotalMod = 1.0f;
  float minval        = 0.0f;
  ...
  if (spellInfo->Id != 42292 && spellInfo->Id != 59752 && spellInfo->Id != 19574 && spellInfo->Id != 34471)
  {
    ...
    if (spellSchoolMask != SPELL_SCHOOL_MASK_NONE)
    {
      ...
      for (SpellImmuneList::const_iterator itr = schoolList.begin(); itr != schoolList.end(); ++itr)
      {
        ...
        if (((itr->type & spellSchoolMask) == spellSchoolMask) && (!immuneSpellInfo || immuneSpellInfo->IsPositive()) &&
            !spellInfo->IsPositive() && !spellInfo->CanPierceImmuneAura(immuneSpellInfo))
        {
          return true;
        }
      }
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
  ...
  {
    if (itr->type == effect &&
        (itr->spellId != 62692 || (spellInfo->Effects[index].MiscValue == POWER_MANA && !CanRestoreMana(spellInfo))))
    {
      return true;
    }
  }
  ...
  if (uint32 aura = spellInfo->Effects[index].ApplyAuraName)
  {
    ...
    {
      if (itr->type == aura &&
          (itr->spellId != 64848 || (spellInfo->Effects[index].MiscValue == POWER_MANA && !CanRestoreMana(spellInfo))))
      {
        if (!spellInfo->HasAttribute(SPELL_ATTR3_ALWAYS_HIT))
        {
                                   ...
                                   SpellSchoolMask  damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
  ...
  if (attType != RANGED_ATTACK)
  {
    ...
    for (AuraEffectList::const_iterator i = mModMeleeDamageTakenPercent.begin(); i != mModMeleeDamageTakenPercent.end();
         ++i)
      AddPct(TakenTotalMod, (*i)->GetAmount());
  }
  else
...
void Unit::ApplySpellImmune(uint32 spellId, uint32 op, uint32 type, bool apply, SpellImmuneBlockType blockType)
{
  if (apply)
  {
    ...
    if (spellId == 0 && std::find_if(m_spellImmune[op].begin(), m_spellImmune[op].end(),
...
float Unit::GetPPMProcChance(uint32 WeaponSpeed, float PPM, SpellInfo const* spellProto) const
{
  ...
  return std::floor((WeaponSpeed * PPM) /
                    600.0f); // result is chance in percents (probability = Speed_in_sec * (PPM / 60))
}
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, uint32 duration)
{
  ...
  if (Creature* creature = ToCreature())
  {
    ...
    if (!(creature->GetCreatureTemplate()->type_flags & CREATURE_TYPE_FLAG_ALLOW_MOUNTED_COMBAT))
      Dismount();
    if (!IsStandState()) // pussywizard: already done in CombatStart(target, initialAggro) for the target, but when
                         // aggro'ing from MoveInLOS CombatStart is not called!
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
  ...
  if (m_vehicle)
    if (IsOnVehicle(target) || m_vehicle->GetBase()->IsOnVehicle(target))
      if (!IsHostileTo(target)) // pussywizard: actually can attack own vehicle or passenger if it's hostile to us -
                                // needed for snobold in Gormok encounter
        return false;

  // can't attack invisible (ignore stealth for aoe spells) also if the area being looked at is from a spell use the
  // dynamic object created instead of the casting unit.
  ...
  if ((!bySpell || !bySpell->HasAttribute(SPELL_ATTR6_IGNORE_PHASE_SHIFT)) &&
      (obj ? !obj->CanSeeOrDetect(target, bySpell && bySpell->IsAffectingArea())
           : !CanSeeOrDetect(target, (bySpell && bySpell->IsAffectingArea()) ||
                                         (target->IsPlayer() && target->HasStealthAura() && target->IsInCombat() &&
  ...
  if (target->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_TAXI_FLIGHT | UNIT_FLAG_NOT_ATTACKABLE_1 |
      ...
      // check if this is a world trigger cast - GOs are using world triggers to cast their spells, so we need to ignore
      // their immunity flag here, this is a temp workaround, needs removal when go cast is implemented properly
  ...
  // Not all neutral creatures can be attacked (even some unfriendly faction does not react aggresive to you, like
  // Sporaggar)
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index,
      ...
      if (Unit* owner = GetOwner())
        if (Player* player = owner->ToPlayer())
        {
          if (player->NeedSendSpectatorData() && pet->GetCreatureTemplate()->family)
            ArenaSpectator::SendCommand_UInt32Value(player->FindMap(), player->GetGUID(), "PHP",
...
uint32 Unit::GetCreatePowers(Powers power) const
{
  ...
  case POWER_RAGE:
    return 1000;
  case POWER_FOCUS:
    return (IsPlayer() || !((Creature const*)this)->IsPet() || ((Pet const*)this)->getPetType() != HUNTER_PET ? 0
                                                                                                              : 100);
  case POWER_ENERGY:
    return 100;
  case POWER_HAPPINESS:
  ...
  if (IsInWorld())
  {
    ...
    if (Unit* owner = GetOwner())
    {
      if (owner->m_Controlled.find(this) != owner->m_Controlled.end())
      {
        ...
        LOG_INFO("entities.unit", "Unit {} is in controlled list of {} when removed from world", GetEntry(),
                 owner->GetEntry());
        // ABORT();
      }
    }
...
void Unit::CleanupBeforeRemoveFromMap(bool finalCleanup)
{
  ...
  m_Events.KillAllEvents(false); // non-delatable (currently casted spells) will not deleted now but it will deleted at
                                 // call in Map::RemoveAllObjectsInRemoveList
  CombatStop();
  ClearComboPoints();
...
struct ProcTriggeredData
{
  ...
  bool operator==(const uint32 spellId) const { return aura->GetId() == spellId; }
};
...
void createProcFlags(SpellInfo const* spellInfo, WeaponAttackType attackType, bool positive, uint32& procAttacker,
                     uint32& procVictim)
{
  if (spellInfo)
  {
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra,
                                 WeaponAttackType attType, SpellInfo const* procSpellInfo, uint32 damage,
                                 SpellInfo const* procAura, int8 procAuraEffectIndex, Spell const* procSpell,
                                 DamageInfo* damageInfo, HealInfo* healInfo, uint32 procPhase)
{
  ...
  if (procFlag & MELEE_BASED_TRIGGER_MASK && target && procPhase == PROC_SPELL_PHASE_HIT)
  {
    ...
    if (IsPlayer() && !target->IsCharmedOwnedByPlayerOrPlayer())
    {
      ...
      // Update defence if player is victim and we block - TODO: confirm that blocked attacks only have a chance to
      // increase defence skill
  ...
  ProcEventInfo eventInfo = ProcEventInfo(actor, actionTarget, target, procFlag, 0, procPhase, procExtra, procSpell,
                                          damageInfo, healInfo, procAura, procAuraEffectIndex);
  ...
  for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr != GetAppliedAuras().end(); ++itr)
  {
    ...
    bool isTriggeredAtSpellProcEvent = IsTriggeredAtSpellProcEvent(target, triggerData.aura, attType, isVictim, active,
                                                                   triggerData.spellProcEvent, eventInfo);
    ...
    {
      // If there is aura that triggers another proc aura, make sure that the triggered one is going to be proccessed on
      // top of it
      if (hasTriggeredProc)
      {
  ...
  for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
  {
    ...
    if (!handled && HandleAuraProc(target, damage, i->aura, procSpellInfo, procFlag, procExtra, cooldown, &handled))
    {
      ...
      LOG_DEBUG("spells.aura", "ProcDamageAndSpell: casting spell {} (triggered with value by {} aura of spell {})",
    ...
    if (!handled)
      for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
      {
        ...
        case SPELL_AURA_PROC_TRIGGER_SPELL:
        {
          LOG_DEBUG("spells.aura", "ProcDamageAndSpell: casting spell {} (triggered by {} aura of spell {})",
        ...
        case SPELL_AURA_PROC_TRIGGER_DAMAGE:
        {
          ...
          triggeredByAura->HandleProcTriggerDamageAuraProc(aurApp,
        ...
        case SPELL_AURA_DUMMY:
        {
          LOG_DEBUG("spells.aura", "ProcDamageAndSpell: casting spell id {} (triggered by {} dummy aura of spell {})",
                    spellInfo->Id, (isVictim ? "a victim's" : "an attacker's"), triggeredByAura->GetId());
        ...
        case SPELL_AURA_MOD_MELEE_HASTE:
          LOG_DEBUG("spells.aura", "ProcDamageAndSpell: casting spell id {} (triggered by {} aura of spell {})",
                    spellInfo->Id, isVictim ? "a victim's" : "an attacker's", triggeredByAura->GetId());
          takeCharges = true;
          break;
        ...
        case SPELL_AURA_TRANSFORM:
        {
          ...
          // Xinef: Or when the aura is at full duration (assume that such auras should be added at the end, skipping
          // all damage procs etc.)
          if (procSpellInfo)
...
void Unit::TriggerAurasProcOnEvent(CalcDamageInfo& damageInfo)
{
  ...
  TriggerAurasProcOnEvent(nullptr, nullptr, damageInfo.target, damageInfo.procAttacker, damageInfo.procVictim, 0, 0,
                                   ...
                                   HealInfo* healInfo)
{
  ...
  ProcEventInfo myProcEventInfo = ProcEventInfo(this, actionTarget, actionTarget, typeMaskActor, spellTypeMask,
                                                spellPhaseMask, hitMask, spell, damageInfo, healInfo);
  std::list<AuraApplication*> myAurasTriggeringProc;
  GetProcAurasTriggeredOnEvent(myAurasTriggeringProc, myProcAuras, myProcEventInfo);

  // prepare data for target trigger
  ProcEventInfo targetProcEventInfo = ProcEventInfo(this, actionTarget, this, typeMaskActionTarget, spellTypeMask,
                                                    spellPhaseMask, hitMask, spell, damageInfo, healInfo);
...
MovementGeneratorType Unit::GetDefaultMovementType() const { return IDLE_MOTION_TYPE; }
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect* triggeredByAura)
{
  ...
  {
    LOG_DEBUG("spells.aura", "Unit::HandleAuraRaidProcFromChargeWithValue, received not handled spell: {}",
              spellProto->Id);
    return false;
  }
      ...
      if (player)
      {
        ...
        if (!nearMembers.empty())
        {
          ...
          if (Unit* target = nearMembers.front())
          {
            ...
            CastCustomSpell(target, spellProto->Id, &heal, nullptr, nullptr, true, nullptr, triggeredByAura,
                            caster_guid);
                ...
                SpellInfo const* spellProto, Spell const* spell /*= nullptr*/)
{
  ...
  // Do KILL and KILLED procs. KILL proc is called only for the unit who landed the killing blow (and its owner - for
  // pets and totems) regardless of who tapped the victim
  if (killer && (killer->IsPet() || killer->IsTotem()))
    if (Unit* owner = killer->GetOwner())
    {
      Unit::ProcDamageAndSpell(owner, victim, PROC_FLAG_KILL, PROC_FLAG_NONE, PROC_EX_NONE, 0, attackType, spellProto,
                               nullptr, -1, spell);
  ...
  if (killer != victim)
  {
    Unit::ProcDamageAndSpell(killer, victim, killer ? PROC_FLAG_KILL : 0, PROC_FLAG_KILLED, PROC_EX_NONE, 0, attackType,
                             spellProto, nullptr, -1, spell);
  }

  // Proc auras on death - must be before aura/combat remove
  Unit::ProcDamageAndSpell(victim, nullptr, PROC_FLAG_DEATH, PROC_FLAG_NONE, PROC_EX_NONE, 0, attackType, spellProto,
                           nullptr, -1, spell);
      ...
      !victim->ToPlayer()->HasPlayerFlag(PLAYER_FLAGS_IS_OUT_OF_BOUNDS))
  {
    if (AuraEffect* aurEff = victim->GetAuraEffectDummy(20711))
    {
      ...
      {
        /*LOG_INFO("misc", "Player ({}) died with spirit of redemption. Killer (Entry: {}, Name: {}), Map: {}, x: {}, y:
        {}, z: {}", victim->GetGUID().ToString(), killer ? killer->GetEntry() : 1, killer ? killer->GetName() : "",
        victim->GetMapId(), victim->GetPositionX(), victim->GetPositionY(), victim->GetPositionZ());
  ...
  if (Player* plrVictim = victim->ToPlayer())
  {
    ...
    {
      LOG_DEBUG("entities.unit", "We are dead, losing {} percent durability",
  ...
  // battleground things (do this at the end, so the death state flag will be properly set to handle in the
  // bg->handlekill)
  if (player)
    if (Battleground* bg = player->GetBattleground())
    {
  ...
  if (killer && victim->IsPlayer())
  {
    if (killer->IsCreature())
      ...
      victim->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_PLAYER, 1,
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
  ...
  LOG_DEBUG("entities.unit", "SetCharmedBy: charmer {} ({}), charmed {} ({}), type {}.", charmer->GetEntry(),
            charmer->GetGUID().ToString(), GetEntry(), GetGUID().ToString(), uint32(type));

  if (this == charmer)
  {
    LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Unit {} ({}) is trying to charm itself!", GetEntry(),
              GetGUID().ToString());
    return false;
  }
  ...
  {
    LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Player on transport is trying to charm {} ({})", GetEntry(),
              GetGUID().ToString());
    return false;
  }
  ...
  if (GetCharmerGUID())
  {
    LOG_FATAL("entities.unit",
              "Unit::SetCharmedBy: {} ({}) has already been charmed but {} ({}) is trying to charm it!", GetEntry(),
              GetGUID().ToString(), charmer->GetEntry(), charmer->GetGUID().ToString());
    return false;
  }
      ...
      if (IsCreature() && charmer->IsClass(CLASS_WARLOCK, CLASS_CONTEXT_PET_CHARM))
      {
        ...
        if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
        {
          ...
          if (GetCharmInfo())
            ...
            LOG_ERROR("entities.unit", "Aura::HandleModCharm: target={} has a charm aura but no charm info!",
                      GetGUID().ToString());
        }
      }
      break;
  ...
  if (group)
  {
    ...
    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
      ...
      if (Target && Target->IsInMap(owner) && Target->GetSubGroup() == subgroup && !IsHostileTo(Target))
      {
        ...
        for (Unit::ControlSet::iterator iterator = Target->m_Controlled.begin(); iterator != Target->m_Controlled.end();
             ++iterator)
        {
...
float Unit::MeleeSpellMissChance(Unit const* victim, WeaponAttackType attType, int32 skillDiff, uint32 spellId) const
{
  ...
  // Check if dual wielding, add additional miss penalty - when mainhand has on next swing spell, offhand doesnt suffer
  // penalty
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
  ...
  for (SpellClickInfoContainer::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
  {
    ...
    if (seatId > -1)
    {
      ...
      if (!valid)
      {
        LOG_ERROR("sql.sql", "Spell {} specified in npc_spellclick_spells is not a valid vehicle enter aura!",
                  itr->second.spellId);
        continue;
      }
...
{
  CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED, SPELLVALUE_BASE_POINT0, seatId + 1, base,
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
  ...
  if (m_vehicle)
  {
    if (m_vehicle == vehicle)
    {
      ...
      {
        LOG_DEBUG("vehicles", "EnterVehicle: {} leave vehicle {} seat {} and enter {}.", GetEntry(),
                  m_vehicle->GetBase()->GetEntry(), GetTransSeat(), seatId);
        ChangeSeat(seatId);
      }
    ...
    {
      LOG_DEBUG("vehicles", "EnterVehicle: {} exit {} and enter {}.", GetEntry(), m_vehicle->GetBase()->GetEntry(),
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
  ...
  else
  {
    ...
    if (seatAddon)
    {
      if (seatAddon->ExitParameter == VehicleExitParameters::VehicleExitParamOffset)
        pos.RelocateOffset({seatAddon->ExitParameterX, seatAddon->ExitParameterY, seatAddon->ExitParameterZ,
                            seatAddon->ExitParameterO});
      else if (seatAddon->ExitParameter == VehicleExitParameters::VehicleExitParamDest)
        pos.Relocate({seatAddon->ExitParameterX, seatAddon->ExitParameterY, seatAddon->ExitParameterZ,
                      seatAddon->ExitParameterO});
    }
  }
  ...
  // xinef: move fall, should we support all creatures that exited vehicle in air? Currently Quest Drag and Drop only,
  // Air Assault quest
  ...
  if ((!player || !(player->GetDelayedOperations() & DELAYED_VEHICLE_TELEPORT)) &&
    ...
    if (((Minion*)vehicleBase)->GetOwner() == this)
    {
      ...
      {
        vehicle->Uninstall();
        vehicleBase->m_Events.AddEvent(new VehicleDespawnEvent(*vehicleBase, 2000),
...
{
  // Mirror client behavior, if this method returns false then client will not use swimming animation and for players
  // will apply gravity as if there was no water
                          ...
                          bool withPet /*= false*/, bool removeTransport /*= false*/)
{
  NearTeleportTo(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation(), casting,
                 vehicleTeleport, withPet, removeTransport);
}
                          ...
                          bool vehicleTeleport /*= false*/, bool withPet /*= false*/, bool removeTransport /*= false*/)
{
  DisableSpline();
  if (IsPlayer())
    ToPlayer()->TeleportTo(GetMapId(), x, y, z, orientation,
                           TELE_TO_NOT_LEAVE_COMBAT | (removeTransport ? 0 : TELE_TO_NOT_LEAVE_TRANSPORT) |
                               TELE_TO_NOT_UNSUMMON_PET | (casting ? TELE_TO_SPELL : 0) |
                               (vehicleTeleport ? TELE_TO_NOT_LEAVE_VEHICLE : 0) | (withPet ? TELE_TO_WITH_PET : 0));
  else
  {
...
void Unit::OutDebugInfo() const
{
  ...
  LOG_INFO("entities.unit", "OwnerGUID {}, MinionGUID {}, CharmerGUID {}, CharmedGUID {}", GetOwnerGUID().ToString(),
           GetMinionGUID().ToString(), GetCharmerGUID().ToString(), GetCharmGUID().ToString());
...
{
public:
  AuraMunchingQueue(Unit& owner, ObjectGuid targetGUID, int32 basePoints, uint32 spellId, AuraEffect* aurEff,
                    AuraType auraType)
      : _owner(owner), _targetGUID(targetGUID), _basePoints(basePoints), _spellId(spellId), _aurEff(aurEff),
        _auraType(auraType)
  {
  }
  ...
  bool Execute(uint64 /*eventTime*/, uint32 /*updateTime*/) override
  {
    if (_owner.IsInWorld() && _owner.FindMap())
      if (Unit* target = ObjectAccessor::GetUnit(_owner, _targetGUID))
      {
        ...
        _owner.CastCustomSpell(_spellId, SPELLVALUE_BASE_POINT0, _basePoints, target,
                               TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_NO_PERIODIC_RESET), nullptr, _aurEff,
                               _owner.GetGUID());
      }
                                              ...
                                              uint8 effectIndex)
{
  ...
  if (this == caster || !sWorld->getBoolConfig(CONFIG_MUNCHING_BLIZZLIKE))
    caster->CastCustomSpell(spellId, SPELLVALUE_BASE_POINT0, addAmount, this,
                            TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_NO_PERIODIC_RESET), nullptr, aurEff,
                            caster->GetGUID());
  else
...
int32 Unit::CalculateAOEDamageReduction(int32 damage, uint32 schoolMask, Unit* caster) const
{
  ...
  if (caster && caster->IsCreature())
    damage = int32(float(damage) *
                   GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_CREATURE_AOE_DAMAGE_AVOIDANCE, schoolMask));
...
void Unit::ExecuteDelayedUnitRelocationEvent()
{
    ...
    for (SharedVisionList::const_iterator itr = this->GetSharedVisionList().begin();
      ...
      if (Player* player = (*itr))
      {
        if (player->IsOnVehicle(this) || !player->IsInWorld() ||
            player->IsDuringRemoveFromWorld()) // players on vehicles have their own event executed (due to passenger
                                               // relocation)
          continue;
        ...
        if (Unit* active = viewPoint->ToUnit())
        {
          ...
          // active->m_last_notify_position.Relocate(active->GetPositionX(), active->GetPositionY(),
          // active->GetPositionZ());
        }
  ...
  if (Player* player = this->ToPlayer())
  {
    ...
    Acore::PlayerRelocationNotifier relocateNoLarge(*player,
...
bool Unit::SetHover(bool enable, bool /*packetOnly = false*/, bool /*updateAnimationTier = true*/)
{
  ...
  else
  {
    ...
    SendMovementFlagUpdate(); // pussywizard: needed for falling after death (instead of falling onto air at hover
                              // height)
  }
...
bool Unit::IsInCombatWith(Unit const* who) const
{
  ...
  for (ThreatContainer::StorageType::const_iterator i = m_ThreatMgr.GetThreatList().begin();
       i != m_ThreatMgr.GetThreatList().end(); ++i)
  {
...
float Unit::GetCollisionWidth() const
{
  ...
  // if the objectSize is the default value or the creature is mounted and we have a DBC value, then we can retrieve DBC
  // value instead
...
float Unit::GetCollisionHeight() const
{
  ...
  if (IsMounted())
  {
            ...
            sCreatureDisplayInfoStore.LookupEntry(GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID)))
    {
      if (CreatureModelDataEntry const* mountModelData = sCreatureModelDataStore.LookupEntry(mountDisplayInfo->ModelId))
      {
        collisionHeight = scaleMod * (mountModelData->MountHeight +
...
void Unit::Whisper(std::string_view text, Language language, Player* target, bool isBossWhisper /*= false*/)
{
  ...
  WorldPacket    data;
  ChatHandler::BuildChatPacket(data, isBossWhisper ? CHAT_MSG_RAID_BOSS_WHISPER : CHAT_MSG_MONSTER_WHISPER, language,
                               this, target, text, 0, "", locale);
...
void Unit::Whisper(uint32 textId, Player* target, bool isBossWhisper /*= false*/)
{
  ...
  LocaleConstant locale = target->GetSession()->GetSessionDbLocaleIndex();
  WorldPacket    data;
  ChatHandler::BuildChatPacket(data, isBossWhisper ? CHAT_MSG_RAID_BOSS_WHISPER : CHAT_MSG_MONSTER_WHISPER,
                               LANG_UNIVERSAL, this, target, bct->GetText(locale, getGender()), 0, "", locale);
  target->SendDirectMessage(&data);
}
