...
enum Gossip_Option
{
  ...
  GOSSIP_OPTION_LEARNDUALSPEC = 18,     // UNIT_NPC_FLAG_TRAINER             (16)
                                        // (bonus option for GOSSIP_OPTION_TRAINER)
  GOSSIP_OPTION_OUTDOORPVP    = 19,     // added by code (option for outdoor pvp creatures)
  GOSSIP_OPTION_DUALSPEC_INFO = 20,     // UNIT_NPC_FLAG_TRAINER             (16)
                                        // (bonus option for GOSSIP_OPTION_TRAINER)
  GOSSIP_OPTION_MAX
};
...
public:
  ...
  void AddMenuItem(int32 menuItemId, uint8 icon, std::string const& message, uint32 sender, uint32 action,
  ...
  [[nodiscard]] GossipMenuItemContainer const& GetMenuItems() const { return _menuItems; }
...
public:
  ...
  [[nodiscard]] uint8 GetMenuItemCount() const { return _questMenuItems.size(); }

  [[nodiscard]] bool Empty() const { return _questMenuItems.empty(); }

  [[nodiscard]] bool HasItem(uint32 questId) const;

  [[nodiscard]] QuestMenuItem const& GetItem(uint16 index) const { return _questMenuItems[index]; }
