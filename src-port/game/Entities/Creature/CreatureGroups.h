...
enum class GroupAIFlags : uint16
{
  ...
  GROUP_AI_FLAG_SUPPORTED = GROUP_AI_FLAG_ASSIST_MASK | GROUP_AI_FLAG_EVADE_MASK |
                            GROUP_AI_FLAG_DONT_RESPAWN_LEADER_ON_EVADE | GROUP_AI_FLAG_FOLLOW_LEADER |
                            GROUP_AI_FLAG_ACQUIRE_NEW_TARGET_ON_EVADE
};

struct FormationInfo
{
  FormationInfo() : leaderGUID(0), follow_dist(0.0f), follow_angle(0.0f), groupAI(0), point_1(0), point_2(0) {}
...
private:
  Creature* m_leader; // Important do not forget sometimes to work with pointers
                      // instead synonims :D:D
