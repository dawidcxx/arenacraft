...
void FormationMgr::AddCreatureToGroup(uint32 groupId, Creature* member)
{
  ...
  {
    LOG_DEBUG("entities.unit", "Group found: {}, inserting creature {}, Group InstanceID {}", groupId,
              member->GetGUID().ToString(), member->GetInstanceId());
...
{
  LOG_DEBUG("entities.unit", "Deleting member pointer to spawnId: {} from group {}", member->GetSpawnId(),
            group->GetId());
...
void FormationMgr::LoadCreatureFormations()
{
  ...
  // Get group data
  QueryResult result = WorldDatabase.Query("SELECT leaderGUID, memberGUID, dist, angle, groupAI, point_1, point_2 FROM "
                                           "creature_formations ORDER BY leaderGUID");
  if (!result)
  {
  ...
  do
  {
    ...
    if (group_member.leaderGUID != memberGUID)
    {
      ...
      {
        LOG_ERROR("sql.sql",
                  "creature_formations table leader guid {} and member guid {} has unsupported GroupAI flag value "
                  "({}). Skipped",
                  group_member.leaderGUID, memberGUID, group_member.groupAI);
        continue;
      }

      if (!group_member.HasGroupFlag(std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_FOLLOW_LEADER)) &&
          (follow_dist > 0.0f || follow_angle > 0.0f))
      {
        LOG_ERROR("sql.sql",
                  "creature_formations table member guid {} and leader guid {} cannot have follow distance or follow "
                  "angle because don't have GROUP_AI_FLAG_FOLLOW_LEADER flag. Values are not gonna be used",
                  memberGUID, group_member.leaderGUID);
    ...
    {
      LOG_ERROR("sql.sql", "creature_formations table leader guid {} incorrect (does not exist). Skipped",
  ...
  for (auto const& itr : m_members)
  {
    ...
    if (member && member != m_leader && member->IsAlive())
    {
      if (initMotionMaster)
      {
        ...
        LOG_DEBUG("entities.unit", "Set {} movement for member {}", dismiss ? "default" : "idle",
...
void CreatureGroup::LeaderMoveTo(float x, float y, float z, uint32 move_type)
{
  ...
  for (auto const& itr : m_members)
  {
    ...
    if (member == m_leader || !member->IsAlive() || member->GetVictim() ||
        !pFormationInfo.HasGroupFlag(std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_FOLLOW_LEADER)))
      continue;
        ...
        static_cast<float>(M_PI) * 0.5f)
    {
      ...
      // if (m_leader->GetCurrentWaypointID()+1 == pFormationInfo->point_1 || m_leader->GetCurrentWaypointID()+1 ==
      // itr->second->point_2)
      followAngle = Position::NormalizeOrientation(pFormationInfo.follow_angle +
                                                   static_cast<float>(M_PI)); //(2 * M_PI) - itr->second->follow_angle;
    }
    ...
    // pussywizard: setting the same movementflags is not enough, spline decides whether leader walks/runs, so spline
    // param is now passed as "run" parameter to this function
