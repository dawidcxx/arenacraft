...
Unit* Minion::GetOwner() const { return ObjectAccessor::GetUnit(*this, m_owner); }
  ...
  if (s == DeathState::JustDied && IsGuardianPet())
    if (Unit* owner = GetOwner())
      if (owner->IsPlayer() && owner->GetMinionGUID() == GetGUID())
        for (Unit::ControlSet::const_iterator itr = owner->m_Controlled.begin(); itr != owner->m_Controlled.end();
             ++itr)
...
void Puppet::InitSummon()
{
  ...
  if (!SetCharmedBy(GetOwner(), CHARM_TYPE_POSSESS))
  {
    if (Player* p = GetOwner())
      LOG_INFO("misc", "Puppet::InitSummon (A1) - {}, {}, {}, {}, {}, {}, {}, {}", p->GetGUID().ToString(),
...
Player* Puppet::GetOwner() const { return ObjectAccessor::GetPlayer(*this, m_owner); }
