...
Map* MapInstanced::CreateInstanceForPlayer(const uint32 mapId, Player* player)
{
  ...
  else
  {
    ...
    if (destInstId)
    {
      ...
      {
        if (player->isBeingLoaded()) // pussywizard: crashfix (assert(passengers.empty) fail in ~transport), could be
                                     // added to a transport during loading from db
          return nullptr;

        if (!map->AllTransportsEmpty())
          map->AllTransportsRemovePassengers(); // pussywizard: gameobjects / summons (assert(passengers.empty) fail in
                                                // ~transport)
...
InstanceMap* MapInstanced::CreateInstance(uint32 InstanceId, InstanceSave* save, Difficulty difficulty)
{
  ...
  LOG_DEBUG("maps", "MapInstanced::CreateInstance: {} map instance {} for {} created with difficulty {}",
            save ? "" : "new ", InstanceId, GetId(), difficulty ? "heroic" : "normal");
