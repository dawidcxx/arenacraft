...
public:
  ...
  [[nodiscard]] uint32 GetAreaId(uint32 phaseMask, uint32 mapid, Position const& pos) const
  {
    return GetAreaId(phaseMask, mapid, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
  }
  [[nodiscard]] uint32 GetAreaId(uint32 phaseMask, WorldLocation const& loc) const
  {
  ...
  [[nodiscard]] uint32 GetZoneId(uint32 phaseMask, uint32 mapid, Position const& pos) const
  {
    return GetZoneId(phaseMask, mapid, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
  }
  [[nodiscard]] uint32 GetZoneId(uint32 phaseMask, WorldLocation const& loc) const
  {
  ...
  {
    return IsValidMapCoord(loc.GetMapId(), loc.GetPositionX(), loc.GetPositionY(), loc.GetPositionZ(),
                           loc.GetOrientation());
  }
  ...
  template <typename Worker> void DoForAllMaps(Worker&& worker);

  template <typename Worker> void DoForAllMapsWithMapId(uint32 mapId, Worker&& worker);
...
template <typename Worker> void MapMgr::DoForAllMaps(Worker&& worker)
{
...
template <typename Worker> inline void MapMgr::DoForAllMapsWithMapId(uint32 mapId, Worker&& worker)
{
  std::lock_guard<std::mutex> guard(Lock);
