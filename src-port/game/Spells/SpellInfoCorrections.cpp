...
void SpellMgr::LoadSpellInfoCorrections()
{
  ...
  ApplySpellFix(
      {
          ...
          63137  // Force Cast (HACK: Target shouldn't be changed; summon position should be untied from spell
                 // destination)
      },
  ...
  ApplySpellFix(
      ...
      [](SpellInfo* spellInfo)
      {
        spellInfo->AttributesEx3 |= SPELL_ATTR3_CAN_PROC_FROM_PROCS; // Entries were not updated after spell effect
                                                                     // change, we have to do that manually
      });
  ...
  ApplySpellFix({29214, 54836},
                [](SpellInfo* spellInfo)
                {
                  // target allys instead of enemies, target A is src_caster, spells with effect like that have ally
                  // target this is the only known exception, probably just wrong data
  ...
  ApplySpellFix({20424}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 &= ~SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });
  ...
  ApplySpellFix({53353}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
  ...
  // Honor Among Thieves
  ApplySpellFix({51698, 51700, 51701}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TriggerSpell = 51699; });
  ...
  // Pestilence
  ApplySpellFix({50842}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].TargetA = TARGET_DEST_TARGET_ENEMY; });
  ...
  ApplySpellFix({49576, 49560},
                [](SpellInfo* spellInfo)
                {
                  ...
                  //  should fix taunt on bosses and not break the pull protection at the same time (no aura provides
                  //  immunity to grip mechanic)
  ...
  ApplySpellFix({51560, 51561}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].MiscValue = SPELLMOD_DAMAGE; });
  ...
  ApplySpellFix({52752}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
  ...
  // Ritual Enslavement
  ApplySpellFix({22987},
                [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_CHARM; });
  ...
  ApplySpellFix({70691}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
  ...
  // Luck of the Draw
  ApplySpellFix({72221}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
  ...
  // Energy Feedback
  ApplySpellFix({44335}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
  ...
  ApplySpellFix({32111},
                [](SpellInfo* spellInfo)
                {
                  spellInfo->CastTimeEntry =
                      sSpellCastTimesStore.LookupEntry(0); // We only need the animation, no damage
                });

  // Flame Breath, catapult spell
  ApplySpellFix({50989},
                [](SpellInfo* spellInfo) { spellInfo->Attributes &= ~SPELL_ATTR0_SCALES_WITH_CREATURE_LEVEL; });
  ...
  // Krik'thir - Mind Flay
  ApplySpellFix({52586, 59367},
                [](SpellInfo* spellInfo) { spellInfo->ChannelInterruptFlags |= AURA_INTERRUPT_FLAG_MOVE; });
  ...
  ApplySpellFix(
      {
          ...
          56438, // Arcane Overload (-50% dmg taken) - this is to prevent apply -> unapply -> apply ... dunno whether
                 // it's correct
      },
      [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
  ...
  // Vortex (freeze anim)
  ApplySpellFix({55883}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
  ...
  // Black Hole
  ApplySpellFix({62168, 65250, 62169},
                [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_AURA_IS_DEBUFF; });
  ...
  ApplySpellFix({17647}, [](SpellInfo* spellInfo) { spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(37); });
  ...
  ApplySpellFix({17731, 69294},
                [](SpellInfo* spellInfo)
                {
                  ...
                  spellInfo->Effects[EFFECT_1].RadiusEntry = sSpellRadiusStore.LookupEntry(
                      EFFECT_RADIUS_18_YARDS); // 18yd instead of 13yd to make sure all cracks erupt
                });

  // Onyxia's Lair, Onyxia, Breath
  ApplySpellFix({18576, 18578, 18579, 18580, 18581, 18582, 18583, 18609, 18611, 18612, 18613, 18614, 18615, 18616,
                 18584, 18585, 18586, 18587, 18588, 18589, 18590, 18591, 18592, 18593, 18594, 18595, 18564, 18565,
                 18566, 18567, 18568, 18569, 18570, 18571, 18572, 18573, 18574, 18575, 18596, 18597, 18598, 18599,
                 18600, 18601, 18602, 18603, 18604, 18605, 18606, 18607, 18617, 18619, 18620, 18621, 18622, 18623,
                 18624, 18625, 18626, 18627, 18628, 18618, 18351, 18352, 18353, 18354, 18355, 18356, 18357, 18358,
                 18359, 18360, 18361, 17086, 17087, 17088, 17089, 17090, 17091, 17092, 17093, 17094, 17095, 17097,
                 22267, 22268, 21132, 21133, 21135, 21136, 21137, 21138, 21139},
                [](SpellInfo* spellInfo)
                {
  ...
  ApplySpellFix({59086},
                [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(1); });

  // Halls of Lightning, Arcing Burn
  ApplySpellFix({52671, 59834},
                [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
  ...
  ApplySpellFix({67705, 67715},
                [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_ALLOW_DEAD_TARGET; });
  ...
  ApplySpellFix({66258, 67901},
                [](SpellInfo* spellInfo)
                {
                  spellInfo->DurationEntry =
                      sSpellDurationStore.LookupEntry(85); // summon for 18 seconds, 15 not enough
                });
  ...
  // Explosive Barrage
  ApplySpellFix({69263},
                [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_STUN; });
  ...
  ApplySpellFix({73035, 70719}, [](SpellInfo* spellInfo) { spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); });
  ...
  ApplySpellFix({72347}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 &= ~SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });

  // Award Reputation - Boss Kill
  ApplySpellFix(
      {73843, 73844, 73845, 73846}, [](SpellInfo* spellInfo)
      { spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_50000_YARDS); });
  ...
  ApplySpellFix({72723},
                [](SpellInfo* spellInfo)
                {
                  // this spell initially granted Shadow damage immunity, however it was removed but the data was left
                  // in client
                  spellInfo->Effects[EFFECT_2].Effect = 0;
                });
  ...
  ApplySpellFix({72255, 72444, 72445, 72446},
                [](SpellInfo* spellInfo)
                {
                  // Patch 3.3.2 (2010-01-02): Deathbringer Saurfang will no longer gain blood power from Mark of the
                  // Fallen Champion. prevented in script, effect needed for Prayer of Mending
                  spellInfo->AttributesEx3 &= ~SPELL_ATTR3_SUPPRESS_CASTER_PROCS;
                });
  ...
  ApplySpellFix({71274},
                [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(6); });
  ...
  ApplySpellFix({70402, 72511, 72512, 72513},
  ...
  ApplySpellFix({71266, 72890},
                [](SpellInfo* spellInfo)
                {
                  spellInfo->AreaGroupId =
                      0; // originally, these require area 4522, which is... outside of Icecrown Citadel
                });
  ...
  // Harvest Soul
  ApplySpellFix({73655}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
  ...
  ApplySpellFix({73261}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
  ...
  ApplySpellFix({70227, 70232},
                ...
                {
                  spellInfo->AreaGroupId =
                      2452; // Whole icc instead of Crimson Halls only, remove when area calculation is fixed
                });
  ...
  ApplySpellFix(
      ...
      [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].TargetA = SpellImplicitTargetInfo(TARGET_UNIT_CASTER); });
  ...
  ApplySpellFix({46432}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 &= ~SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; });
  ...
  ApplySpellFix({8219, 8220, 8221, 8222},
                [](SpellInfo* spellInfo) { spellInfo->Attributes &= ~SPELL_ATTR0_NO_IMMUNITIES; });
  ...
  ApplySpellFix({16097}, [](SpellInfo* spellInfo) { spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(16); });
  ...
  // Focused Assault
  // Brutal Assault
  ApplySpellFix({46392, 46393},
                [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
  ...
  ApplySpellFix({21895},
                [](SpellInfo* spellInfo) { spellInfo->Effects[0].SpellClassMask = flag96(0x0603E000, 0x00200100); });
  ...
  // Feed Pet
  ApplySpellFix({1539, 51284}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_ALLOW_WHILE_SITTING; });

  // Judgement (Paladin T2 8P Bonus)
  // Battlegear of Eternal Justice
  ApplySpellFix({23591, 26135},
                [](SpellInfo* spellInfo) { spellInfo->ProcFlags = PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS; });
  ...
  ApplySpellFix({53444}, [](SpellInfo* spellInfo) { spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(27); });
  ...
  ApplySpellFix({35475, 35476, 35477, 35478}, [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 51120; });
  ...
  // Seed of Corruption
  ApplySpellFix({27285, 47833, 47834},
                [](SpellInfo* spellInfo) { spellInfo->AttributesEx |= SPELL_ATTR1_NO_REFLECTION; });

  // Turn the Tables
  ApplySpellFix({51627, 51628, 51629},
                [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
  ...
  // Poultryized!
  ApplySpellFix({30504},
                [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_TAKE_DAMAGE; });
  ...
  // Spotlight
  ApplySpellFix({29683, 32214},
                [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_DO_NOT_DISPLAY_DURATION; });
  ...
  ApplySpellFix({12043},
                [](SpellInfo* spellInfo)
                {
                  // It should not share cooldown mods with category[1151] spells (Arcane Power [12042], Decimate
                  // [47271])
                  spellInfo->AttributesEx6 |= SPELL_ATTR6_NO_CATEGORY_COOLDOWN_MODS;
                });
  ...
  ApplySpellFix(
      {26655, 26656}, [](SpellInfo* spellInfo)
      { spellInfo->AuraInterruptFlags &= ~(AURA_INTERRUPT_FLAG_CHANGE_MAP | AURA_INTERRUPT_FLAG_TELEPORTED); });

  // Summon Cyclone
  ApplySpellFix({43112}, [](SpellInfo* spellInfo) { spellInfo->RequiresSpellFocus = 0; });
