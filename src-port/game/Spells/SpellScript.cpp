...
bool _SpellScript::_Validate(SpellInfo const* entry)
{
  if (!Validate(entry))
  {
    LOG_ERROR("spells.scripts",
              "_SpellScript::_Validate: Spell `{}` did not pass Validate() function of script `{}` - script will not "
...
void SpellScript::CastHandler::Call(SpellScript* spellScript) { (spellScript->*pCastHandlerScript)(); }
...
void SpellScript::HitHandler::Call(SpellScript* spellScript) { (spellScript->*pHitHandlerScript)(); }
...
bool SpellScript::_Validate(SpellInfo const* entry)
{
  for (std::list<EffectHandler>::iterator itr = OnEffectLaunch.begin(); itr != OnEffectLaunch.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectLaunch` of SpellScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectHandler>::iterator itr = OnEffectHit.begin(); itr != OnEffectHit.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectHit` of SpellScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectHandler>::iterator itr = OnEffectHitTarget.begin(); itr != OnEffectHitTarget.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectHitTarget` of SpellScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
...
void SpellScript::_PrepareScriptCall(SpellScriptHookType hookType) { m_currentScriptState = hookType; }

void SpellScript::_FinishScriptCall() { m_currentScriptState = SPELL_SCRIPT_STATE_NONE; }

bool SpellScript::IsInCheckCastHook() const { return m_currentScriptState == SPELL_SCRIPT_HOOK_CHECK_CAST; }
...
void SpellScript::SetExplTargetDest(WorldLocation& loc) { m_spell->m_targets.SetDst(loc); }

WorldObject* SpellScript::GetExplTargetWorldObject() { return m_spell->m_targets.GetObjectTarget(); }

Unit* SpellScript::GetExplTargetUnit() { return m_spell->m_targets.GetUnitTarget(); }

GameObject* SpellScript::GetExplTargetGObj() { return m_spell->m_targets.GetGOTarget(); }

Item* SpellScript::GetExplTargetItem() { return m_spell->m_targets.GetItemTarget(); }
...
Unit* SpellScript::GetHitUnit()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitUnit was called, but function has no effect in "
              "current hook!",
...
Creature* SpellScript::GetHitCreature()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitCreature was called, but function has no effect "
              "in current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return nullptr;
  }
...
Player* SpellScript::GetHitPlayer()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitPlayer was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return nullptr;
  }
...
Item* SpellScript::GetHitItem()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitItem was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return nullptr;
  }
...
GameObject* SpellScript::GetHitGObj()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitGObj was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return nullptr;
  }
...
WorldLocation* SpellScript::GetHitDest()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitDest was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return nullptr;
  }
...
int32 SpellScript::GetHitDamage()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitDamage was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return 0;
  }
...
void SpellScript::SetHitDamage(int32 damage)
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::SetHitDamage was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return;
  }
...
int32 SpellScript::GetHitHeal()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitHeal was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return 0;
  }
...
void SpellScript::SetHitHeal(int32 heal)
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::SetHitHeal was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return;
  }
...
Aura* SpellScript::GetHitAura()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetHitAura was called, but function has no effect in "
              "current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return nullptr;
  }
...
void SpellScript::PreventHitAura()
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::PreventHitAura was called, but function has no effect "
              "in current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return;
  }
...
void SpellScript::PreventHitEffect(SpellEffIndex effIndex)
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::PreventHitEffect was called, but function has no effect "
              "in current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return;
  }
...
void SpellScript::PreventHitDefaultEffect(SpellEffIndex effIndex)
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::PreventHitDefaultEffect was called, but function has no "
...
int32 SpellScript::GetEffectValue() const
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::GetEffectValue was called, but function has no effect "
              "in current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return 0;
  }
...
void SpellScript::SetEffectValue(int32 value)
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::SetEffectValue was called, but function has no effect "
              "in current hook!",
              m_scriptName->c_str(), m_scriptSpellId);
    return;
  }
...
void SpellScript::CreateItem(uint32 effIndex, uint32 itemId) { m_spell->DoCreateItem(effIndex, itemId); }

SpellInfo const* SpellScript::GetTriggeringSpell() { return m_spell->m_triggeredByAuraSpell.spellInfo; }
...
void SpellScript::SetCustomCastResultMessage(SpellCustomErrors result)
{
  ...
  {
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}`: function SpellScript::SetCustomCastResultMessage was called while spell not "
              "in check cast phase!",
              m_scriptName->c_str(), m_scriptSpellId);
    return;
  }
...
bool AuraScript::_Validate(SpellInfo const* entry)
{
  for (std::list<CheckAreaTargetHandler>::iterator itr = DoCheckAreaTarget.begin(); itr != DoCheckAreaTarget.end();
    ...
    if (!entry->HasAreaAuraEffect() && !entry->HasEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA))
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have area aura effect - handler bound to hook `DoCheckAreaTarget` "
  ...
  for (std::list<AuraDispelHandler>::iterator itr = OnDispel.begin(); itr != OnDispel.end(); ++itr)
    if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasAreaAuraEffect())
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have apply aura effect - handler bound to hook `OnDispel` of "
  ...
  for (std::list<AuraDispelHandler>::iterator itr = AfterDispel.begin(); itr != AfterDispel.end(); ++itr)
    if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasAreaAuraEffect())
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have apply aura effect - handler bound to hook `AfterDispel` of "
                "AuraScript won't be executed",
                entry->Id, m_scriptName->c_str());
  ...
  for (std::list<EffectApplyHandler>::iterator itr = OnEffectApply.begin(); itr != OnEffectApply.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectApply` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectApplyHandler>::iterator itr = OnEffectRemove.begin(); itr != OnEffectRemove.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectRemove` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectApplyHandler>::iterator itr = AfterEffectApply.begin(); itr != AfterEffectApply.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`AfterEffectApply` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectApplyHandler>::iterator itr = AfterEffectRemove.begin(); itr != AfterEffectRemove.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`AfterEffectRemove` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectPeriodicHandler>::iterator itr = OnEffectPeriodic.begin(); itr != OnEffectPeriodic.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectPeriodic` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectUpdatePeriodicHandler>::iterator itr = OnEffectUpdatePeriodic.begin();
    ...
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectUpdatePeriodic` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectAbsorbHandler>::iterator itr = OnEffectAbsorb.begin(); itr != OnEffectAbsorb.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectAbsorb` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectAbsorbHandler>::iterator itr = AfterEffectAbsorb.begin(); itr != AfterEffectAbsorb.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`AfterEffectAbsorb` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectSplitHandler>::iterator itr = OnEffectSplit.begin(); itr != OnEffectSplit.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectSplit` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());

  for (std::list<CheckProcHandler>::iterator itr = DoCheckProc.begin(); itr != DoCheckProc.end(); ++itr)
    if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasAreaAuraEffect())
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have apply aura effect - handler bound to hook `DoCheckProc` of "
                "AuraScript won't be executed",
                entry->Id, m_scriptName->c_str());
  ...
  for (std::list<AfterCheckProcHandler>::iterator itr = DoAfterCheckProc.begin(); itr != DoAfterCheckProc.end(); ++itr)
    if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasAreaAuraEffect())
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have apply aura effect - handler bound to hook `DoAfterCheckProc` "
                "of AuraScript won't be executed",
                entry->Id, m_scriptName->c_str());
  ...
  for (std::list<AuraProcHandler>::iterator itr = DoPrepareProc.begin(); itr != DoPrepareProc.end(); ++itr)
    if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasAreaAuraEffect())
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have apply aura effect - handler bound to hook `DoPrepareProc` of "
                "AuraScript won't be executed",
                entry->Id, m_scriptName->c_str());
  ...
  for (std::list<AuraProcHandler>::iterator itr = OnProc.begin(); itr != OnProc.end(); ++itr)
    if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasAreaAuraEffect())
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have apply aura effect - handler bound to hook `OnProc` of "
                "AuraScript won't be executed",
                entry->Id, m_scriptName->c_str());
  ...
  for (std::list<AuraProcHandler>::iterator itr = AfterProc.begin(); itr != AfterProc.end(); ++itr)
    if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasAreaAuraEffect())
      LOG_ERROR("spells.scripts",
                "Spell `{}` of script `{}` does not have apply aura effect - handler bound to hook `AfterProc` of "
                "AuraScript won't be executed",
                entry->Id, m_scriptName->c_str());
  ...
  for (std::list<EffectProcHandler>::iterator itr = OnEffectProc.begin(); itr != OnEffectProc.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`OnEffectProc` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
  ...
  for (std::list<EffectProcHandler>::iterator itr = AfterEffectProc.begin(); itr != AfterEffectProc.end(); ++itr)
    if (!(*itr).GetAffectedEffectsMask(entry))
      LOG_ERROR("spells.scripts",
                "Spell `{}` Effect `{}` of script `{}` did not match dbc effect data - handler bound to hook "
                "`AfterEffectProc` of AuraScript won't be executed",
                entry->Id, (*itr).ToString(), m_scriptName->c_str());
...
AuraScript::EffectPeriodicHandler::EffectPeriodicHandler(AuraEffectPeriodicFnType _pEffectHandlerScript,
...
void AuraScript::EffectCalcAmountHandler::Call(AuraScript* auraScript, AuraEffect const* aurEff, int32& amount,
...
void AuraScript::EffectCalcPeriodicHandler::Call(AuraScript* auraScript, AuraEffect const* aurEff, bool& isPeriodic,
                                                 int32& periodicTimer)
{
  (auraScript->*pEffectHandlerScript)(aurEff, isPeriodic, periodicTimer);
}

AuraScript::EffectCalcSpellModHandler::EffectCalcSpellModHandler(AuraEffectCalcSpellModFnType _pEffectHandlerScript,
                                                                 uint8 _effIndex, uint16 _effName)
    : AuraScript::EffectBase(_effIndex, _effName)
{
  pEffectHandlerScript = _pEffectHandlerScript;
...
void AuraScript::EffectApplyHandler::Call(AuraScript* auraScript, AuraEffect const* _aurEff,
...
void AuraScript::EffectAbsorbHandler::Call(AuraScript* auraScript, AuraEffect* aurEff, DamageInfo& dmgInfo,
                                           uint32& absorbAmount)
{
...
void AuraScript::EffectManaShieldHandler::Call(AuraScript* auraScript, AuraEffect* aurEff, DamageInfo& dmgInfo,
                                               uint32& absorbAmount)
{
  (auraScript->*pEffectHandlerScript)(aurEff, dmgInfo, absorbAmount);
}
...
void AuraScript::EffectSplitHandler::Call(AuraScript* auraScript, AuraEffect* aurEff, DamageInfo& dmgInfo,
                                          uint32& splitAmount)
{
...
bool AuraScript::AfterCheckProcHandler::Call(AuraScript* auraScript, ProcEventInfo& eventInfo,
...
AuraScript::EffectProcHandler::EffectProcHandler(AuraEffectProcFnType effectHandlerScript, uint8 effIndex,
                                                 uint16 effName)
...
void AuraScript::PreventDefaultAction()
{
  ...
  default:
    LOG_ERROR("spells.scripts",
              "Script: `{}` Spell: `{}` AuraScript::PreventDefaultAction called in a hook in which the call won't have "
              "effect!",
              m_scriptName->c_str(), m_scriptSpellId);
    break;
  }
...
SpellInfo const* AuraScript::GetSpellInfo() const { return m_aura->GetSpellInfo(); }
...
DynamicObject* AuraScript::GetDynobjOwner() const { return m_aura->GetDynobjOwner(); }

void AuraScript::Remove(uint32 removeMode) { m_aura->Remove((AuraRemoveMode)removeMode); }
...
void AuraScript::SetDuration(int32 duration, bool withMods) { m_aura->SetDuration(duration, withMods); }
...
void AuraScript::SetMaxDuration(int32 duration) { m_aura->SetMaxDuration(duration); }
...
bool AuraScript::DropCharge(AuraRemoveMode removeMode) { return m_aura->DropCharge(removeMode); }
...
bool AuraScript::IsDeathPersistent() const { return m_aura->IsDeathPersistent(); }

bool AuraScript::HasEffect(uint8 effIndex) const { return m_aura->HasEffect(effIndex); }

AuraEffect* AuraScript::GetEffect(uint8 effIndex) const { return m_aura->GetEffect(effIndex); }

bool AuraScript::HasEffectType(AuraType type) const { return m_aura->HasEffectType(type); }
...
AuraApplication const* AuraScript::GetTargetApplication() const { return m_auraApplication; }
