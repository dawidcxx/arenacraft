...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS] = {
    ...
    &Spell::EffectTitanGrip, // 155 SPELL_EFFECT_TITAN_GRIP Allows you to equip two-handed axes, maces and swords in one
                             // hand, but you attack $49152s1% slower than normal.
    &Spell::EffectEnchantItemPrismatic, // 156 SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC
    &Spell::EffectCreateItem2,  // 157 SPELL_EFFECT_CREATE_ITEM_2            create item or create item template and
                                // replace by some randon spell loot item
...
void Spell::EffectNULL(SpellEffIndex /*effIndex*/) { LOG_DEBUG("spells.aura", "WORLD: Spell Effect DUMMY"); }
...
void Spell::EffectResurrectNew(SpellEffIndex effIndex)
{
  ...
  target->setResurrectRequestData(m_caster->GetGUID(), m_caster->GetMapId(), m_caster->GetPositionX(),
...
void Spell::EffectInstaKill(SpellEffIndex /*effIndex*/)
{
  ...
  Unit::DealDamage(m_caster, unitTarget, unitTarget->GetHealth(), nullptr, NODAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr,
                   false);
}
...
void Spell::EffectEnvironmentalDMG(SpellEffIndex /*effIndex*/)
{
  ...
  else
  {
    ...
    m_caster->SendSpellNonMeleeDamageLog(unitTarget, m_spellInfo, damage, m_spellInfo->GetSchoolMask(), absorb, resist,
                                         false, 0);
  }
}
  ...
  if (unitTarget && unitTarget->IsAlive())
  {
    ...
    case SPELLFAMILY_WARRIOR:
    {
      ...
      if (m_spellInfo->SpellFamilyFlags[1] & 0x200 && m_spellInfo->GetCategory() == 1209)
      {
        ...
        uint32 block_value =
            m_caster->GetShieldBlockValue(uint32(float(level) * 24.5f * limit), uint32(float(level) * 34.5f * limit));
      ...
      else if (m_spellInfo->TargetAuraState == AURA_STATE_CONFLAGRATE)
      {
        ...
        if (aura)
        {
          ...
          m_spellValue->EffectBasePoints[1] =
              m_spellInfo->Effects[EFFECT_1].CalcBaseValue(int32(CalculatePct(pdamage * baseTotalTicks, pct_dot)));
      ...
      if (m_spellInfo->SpellFamilyFlags[1] & 0x00000008)
      {
        if (Player* player = m_caster->ToPlayer())
        {
          ...
          if (uint32 combo = player->GetComboPoints())
          {
            ...
            if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE,
                                                                     0x00010000, 0, 0, m_caster->GetGUID()))
            {
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
  ...
  if (!spellInfo)
  {
    LOG_DEBUG("spells.aura", "Spell::EffectTriggerSpell spell {} tried to trigger unknown spell {}", m_spellInfo->Id,
              triggered_spell_id);
    return;
  }
  ...
  m_caster->CastSpell(targets, spellInfo, &values, TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_NO_PERIODIC_RESET),
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
  ...
  if (!spellInfo)
  {
    LOG_DEBUG("spells.aura", "Spell::EffectTriggerMissileSpell spell {} tried to trigger unknown spell {}",
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
  ...
  if (!spellInfo)
  {
    LOG_ERROR("spells.effect", "Spell::EffectForceCast of spell {}: triggering unknown spell id {}", m_spellInfo->Id,
              triggered_spell_id);
    return;
  }
  ...
  if (m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_FORCE_CAST && damage)
  {
    ...
    case 52349: // Overtake
      unitTarget->CastCustomSpell(unitTarget, spellInfo->Id, &damage, nullptr, nullptr, true, nullptr, nullptr,
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffIndex effIndex)
{
  ...
  if (!spellInfo)
  {
    LOG_ERROR("spells.effect", "EffectTriggerRitualOfSummoning of spell {}: triggering unknown spell id {}",
              m_spellInfo->Id, triggered_spell_id);
    return;
  }
...
void Spell::CalculateJumpSpeeds(uint8 i, float dist, float& speedXY, float& speedZ)
{
  ...
  float minHeight =
      m_spellInfo->Effects[i].MiscValue ? m_spellInfo->Effects[i].MiscValue / 10.0f : 0.5f; // Lower bound is blizzlike
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex)
{
  ...
  LOG_DEBUG("spells.aura", "Spell: Player {} has unlearned spell {} from Npc: {}", player->GetGUID().ToString(),
            spellToUnlearn, m_caster->GetGUID().ToString());
}
...
void Spell::EffectSendEvent(SpellEffIndex effIndex)
{
  ...
  LOG_DEBUG("spells.aura", "Spell ScriptStart {} for spellid {} in EffectSendEvent ",
...
void Spell::EffectHeal(SpellEffIndex effIndex)
{
  ...
  if (unitTarget && unitTarget->IsAlive() && damage >= 0)
  {
             ...
             unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
    {
      ...
      if (!targetAura)
      {
        LOG_ERROR("spells.effect", "Target({}) has aurastate AURA_STATE_SWIFTMEND but no matching aura.",
    ...
    {
      addhealth = caster->SpellHealingBonusDone(unitTarget, m_spellInfo, int32(caster->CountPctFromMaxHealth(damage)),
                                                HEAL, effIndex);
...
void Spell::EffectHealPct(SpellEffIndex effIndex)
{
  ...
  uint32 heal = m_originalCaster->SpellHealingBonusDone(unitTarget, m_spellInfo,
                                                        unitTarget->CountPctFromMaxHealth(damage), HEAL, effIndex);
  ...
  if (!m_spellAura)
  {
    ...
    if (!caster->IsInWorld() || !caster->FindMap() ||
        !ObjectAccessor::GetUnit(*caster,
                                 caster->GetGUID())) // pussywizard: temporary crash fix (FindMap and GetUnit are mine)
      return;
    DynamicObject* dynObj = new DynamicObject(false);
    if (!dynObj->CreateDynamicObject(caster->GetMap()->GenerateLowGuid<HighGuid::DynamicObject>(), caster,
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
  ...
  if (m_spellInfo->Id == 45051)
  {
    ...
    if (!availableElixirs.empty())
    {
      ...
      m_caster->CastSpell(unitTarget, Acore::Containers::SelectRandomContainerElement(availableElixirs), true,
                          m_CastItem);
    }
  }
}
...
void Spell::SendLoot(ObjectGuid guid, LootType loottype)
{
  ...
  if (gameObjTarget)
  {
    ...
    {
      LOG_ERROR("spells.effect",
                "Possible hacking attempt: Player {} [{}] tried to loot a gameobject [{}] which is on respawn time "
...
void Spell::EffectSummonChangeItem(SpellEffIndex effIndex)
{
  ...
  for (uint8 j = PERM_ENCHANTMENT_SLOT; j <= TEMP_ENCHANTMENT_SLOT; ++j)
    if (m_CastItem->GetEnchantmentId(EnchantmentSlot(j)))
      pNewItem->SetEnchantment(EnchantmentSlot(j), m_CastItem->GetEnchantmentId(EnchantmentSlot(j)),
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
  ...
  case SUMMON_CATEGORY_UNK:
    ...
    case SUMMON_TYPE_VEHICLE2:
      summon = m_caster->GetMap()->SummonCreature(entry, *destTarget, properties, duration, m_originalCaster,
                                                  m_spellInfo->Id, 0, personalSpawn);
      break;
    ...
    case SUMMON_TYPE_MINIPET:
    {
      ...
      if (properties->Type != SUMMON_TYPE_JEEVES)
      {
        ...
        summon->GetMotionMaster()->MoveFollow(m_originalCaster, PET_FOLLOW_DIST, summon->GetFollowAngle(),
                                              MOTION_SLOT_ACTIVE);
      }
      break;
    }
  ...
  case SUMMON_CATEGORY_VEHICLE:
    ...
    summon = m_originalCaster->GetMap()->SummonCreature(entry, *destTarget, properties, duration, m_caster,
                                                        m_spellInfo->Id, 0, personalSpawn);
    ...
    // xinef: i think this is wrong, found only 2 vehicles with faction override and one of them should inherit caster
    // faction...
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex)
{
  ...
  LOG_DEBUG("spells.aura", "Spell: Player {} has learned spell {} from Npc {}", player->GetGUID().ToString(),
            spellToLearn, m_caster->GetGUID().ToString());
}
...
void                                             Spell::EffectDispel(SpellEffIndex effIndex)
{
  ...
  for (DispelChargesList::iterator itr = success_list.begin(); itr != success_list.end(); ++itr)
  {
    ...
    unitTarget->RemoveAurasDueToSpellByDispel(itr->first->GetId(), m_spellInfo->Id, itr->first->GetCasterGUID(),
                                              m_caster, itr->second);
  }
...
void Spell::EffectAddFarsight(SpellEffIndex effIndex)
{
  ...
  if (!dynObj->CreateDynamicObject(m_caster->GetMap()->GenerateLowGuid<HighGuid::DynamicObject>(), m_caster,
                                   m_spellInfo->Id, *destTarget, radius, DYNAMIC_OBJECT_FARSIGHT_FOCUS))
  {
    delete dynObj;
    return;
  }
...
void Spell::EffectTeleUnitsFaceCaster(SpellEffIndex /*effIndex*/)
{
  ...
  if (m_targets.HasDst())
  {
    unitTarget->NearTeleportTo(destTarget->GetPositionX(), destTarget->GetPositionY(), destTarget->GetPositionZ(),
                               destTarget->GetAbsoluteAngle(m_caster), unitTarget == m_caster);
  }
}
...
void Spell::EffectLearnSkill(SpellEffIndex effIndex)
{
  ...
  unitTarget->ToPlayer()->SetSkill(skillid, m_spellInfo->Effects[effIndex].CalcValue(), skillval ? skillval : 1,
                                   damage * 75);
}
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex)
{
  ...
  // support only enchantings with add socket in this slot
  {
    ...
    if (!add_socket)
    {
      LOG_ERROR(
          "spells.effect",
          "Spell::EffectEnchantItemPrismatic: attempt apply enchant spell {} with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC "
          "({}) but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET ({}), not suppoted yet.",
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
  ...
  if (!enchant_id)
  {
    LOG_ERROR("spells.effect", "Spell {} Effect {} (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have 0 as enchanting id",
  ...
  itemTarget->SetEnchantment(TEMP_ENCHANTMENT_SLOT, enchant_id, duration * 1000, pEnchant->charges,
                             m_caster->GetGUID());
           ...
           (i == CURRENT_CHANNELED_SPELL && curSpellInfo->ChannelInterruptFlags & CHANNEL_INTERRUPT_FLAG_INTERRUPT)))
      {
        if (m_originalCaster)
        {
          int32 duration = m_originalCaster->ModSpellDuration(
              m_spellInfo, unitTarget, m_originalCaster->CalcSpellDuration(m_spellInfo), false, 1 << effIndex);
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
  ...
  if (!pGameObj->Create(map->GenerateLowGuid<HighGuid::GameObject>(), gameobject_id, map, m_caster->GetPhaseMask(), x,
                        y, z, target->GetOrientation(), G3D::Quat(), 100, GO_STATE_READY))
  {
    delete pGameObj;
    return;
  }
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex)
{
  ...
  case SPELLFAMILY_GENERIC:
  {
    ...
    case 41931:
    {
        ...
        if (bag)
        {
            if (m_caster->ToPlayer()->GetItemByPos(bag, slot)->GetCount() == 1) m_caster->ToPlayer()->RemoveItem(bag,
    slot, true); else m_caster->ToPlayer()->GetItemByPos(bag, slot)->SetCount(m_caster->ToPlayer()->GetItemByPos(bag,
    slot)->GetCount()-1);
    ...
    case 47770:
    {
      ...
      if (m_caster->getGender() > 0)
        gender = "her";
      snprintf(buf, sizeof(buf), "%s rubs %s [Decahedral Dwarven Dice] between %s hands and rolls. One %u and one %u.",
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
  ...
  if (!pGameObj->Create(map->GenerateLowGuid<HighGuid::GameObject>(), gameobject_id, map, m_caster->GetPhaseMask(),
                        ...
                        m_caster->GetPositionZ(), m_caster->GetOrientation(), G3D::Quat(), 0, GO_STATE_READY))
  {
    delete pGameObj;
    return;
  }
...
void Spell::EffectActivateObject(SpellEffIndex effIndex)
{
  ...
  case GameObjectActions::UseArtKit3:
  {
    ...
    if (artKitValue == 0)
      LOG_ERROR("sql.sql", "GameObject {} hit by spell {} needs `artkit{}` in `gameobject_template_addon`",
  ...
  case GameObjectActions::None:
    ...
    break;
  default:
    LOG_ERROR("spell", "Spell {} has unhandled action {} in effect {}", m_spellInfo->Id, int32(action),
              int32(effIndex));
    break;
  }
}
    ...
    if (GlyphPropertiesEntry const* glyphEntry = sGlyphPropertiesStore.LookupEntry(glyph))
    {
      ...
      if (uint32 oldGlyph = player->GetGlyph(m_glyphIndex))
        if (GlyphPropertiesEntry const* oldGlyphEntry = sGlyphPropertiesStore.LookupEntry(oldGlyph))
        {
          ...
          player->SendLearnPacket(oldGlyphEntry->SpellId,
                                  false); // Send packet to properly handle client-side spell tooltips
        }

      player->SendLearnPacket(glyphEntry->SpellId, true); // Send packet to properly handle client-side spell tooltips
      player->CastSpell(
          m_caster, glyphEntry->SpellId,
...
void Spell::EffectSummonObject(SpellEffIndex effIndex)
{
  ...
  Map* map = m_caster->GetMap();
  if (!pGameObj->Create(map->GenerateLowGuid<HighGuid::GameObject>(), gameobjectId, map, m_caster->GetPhaseMask(), x, y,
                        z, m_caster->GetOrientation(), G3D::Quat(), 0, GO_STATE_READY))
  {
    delete pGameObj;
    return;
  }
...
void Spell::EffectLeap(SpellEffIndex /*effIndex*/)
{
  ...
  unitTarget->NearTeleportTo(dstpos.GetPositionX(), dstpos.GetPositionY(), dstpos.GetPositionZ(),
                             dstpos.GetOrientation(), unitTarget == m_caster);
}
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/)
{
  ...
  for (Unit::AttackerSet::const_iterator itr = m_caster->getAttackers().begin(); itr != m_caster->getAttackers().end();
       ++itr)
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/)
{
  if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
  {
    ...
    {
      Position pos =
          unitTarget->GetFirstCollisionPosition(unitTarget->GetCombatReach(), unitTarget->GetRelativeAngle(m_caster));
      m_caster->GetMotionMaster()->MoveCharge(pos.m_positionX, pos.m_positionY, pos.m_positionZ, speed, EVENT_CHARGE,
                                              nullptr, false, 0.0f, targetGUID);
    }
    else
    {
...
void Spell::EffectPullTowards(SpellEffIndex effIndex)
{
  ...
  else // if (m_spellInfo->Effects[i].Effect == SPELL_EFFECT_PULL_TOWARDS)
  {
    ...
    pos.Relocate(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ() + 1.0f,
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
  ...
  if (!goinfo)
  {
    LOG_ERROR("sql.sql", "Gameobject (Entry: {}) not exist and not created at spell (ID: {}) cast", name_id,
              m_spellInfo->Id);
    return;
  }
  ...
  if (!pGameObj->Create(cMap->GenerateLowGuid<HighGuid::GameObject>(), name_id, cMap, m_caster->GetPhaseMask(), fx, fy,
                        fz, m_caster->GetOrientation(), G3D::Quat(), 100, GO_STATE_READY))
  {
    delete pGameObj;
    return;
  }
...
void Spell::EffectSpiritHeal(SpellEffIndex /*effIndex*/)
{
  ...
  //unitTarget->ToPlayer()->setResurrect(m_caster->GetGUID(), unitTarget->GetPositionX(), unitTarget->GetPositionY(),
  unitTarget->GetPositionZ(), unitTarget->GetMaxHealth(), unitTarget->GetMaxPower(POWER_MANA));
...
void Spell::EffectGameObjectSetDestructionState(SpellEffIndex effIndex)
{
  ...
  gameObjTarget->SetDestructibleState(GameObjectDestructibleState(m_spellInfo->Effects[effIndex].MiscValue), player,
                                      true);
}
                           ...
                           bool personalSpawn)
{
  ...
  summonLevel = std::min<uint8>(sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL) +
