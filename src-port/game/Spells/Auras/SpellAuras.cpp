...
AuraApplication::AuraApplication(Unit* target, Unit* caster, Aura* aura, uint8 effMask)
    : _target(target), _base(aura), _removeMode(AURA_REMOVE_NONE), _slot(MAX_AURAS), _flags(AFLAG_NONE),
      _effectsToApply(effMask), _needClientUpdate(false), _disableMask(0)
{
  ...
  if (GetBase()->CanBeSentToClient())
  {
    ...
    if (AuraApplication* foundAura = GetTarget()->GetAuraApplication(GetBase()->GetId(), GetBase()->GetCasterGUID(),
    ...
    if (slot < MAX_AURAS)
    {
      ...
      SetNeedClientUpdate();
      LOG_DEBUG("spells.aura", "Aura: {} Effect: {} put to unit visible auras slot: {}", GetBase()->GetId(),
                GetEffectMask(), slot);
    }
    else
    {
      LOG_ERROR("spells.aura", "Aura: {} Effect: {} could not find empty unit visible slot", GetBase()->GetId(),
                GetEffectMask());
    }
  }
...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
  ...
  LOG_DEBUG("spells.aura", "AuraApplication::_HandleEffect: {}, apply: {}: amount: {}", aurEff->GetAuraType(), apply,
            aurEff->GetAmount());
      ...
      if (Aura* aura = GetBase())
        if (plr->NeedSendSpectatorData() &&
          ...
          ArenaSpectator::SendCommand_Aura(
              plr->FindMap(), plr->GetGUID(), "AUR", aura->GetCasterGUID(), aura->GetSpellInfo()->Id,
              aura->GetSpellInfo()->IsPositive(), aura->GetSpellInfo()->Dispel, aura->GetDuration(),
              aura->GetMaxDuration(), (aura->GetCharges() > 1 ? aura->GetCharges() : aura->GetStackAmount()), remove);
                                    ...
                                    bool periodicReset /*= false*/)
{
  ...
  if (!effMask)
    return nullptr;
  if (Aura* foundAura = owner->ToUnit()->_TryStackingOrRefreshingExistingAura(spellproto, effMask, caster, baseAmount,
                                                                              castItem, casterGUID, periodicReset))
  {
...
void Aura::_UnapplyForTarget(Unit* target, Unit* caster, AuraApplication* auraApp)
{
  ...
  if (itr == m_applications.end())
  {
    LOG_ERROR("spells.aura",
              ...
              target->GetGUID().ToString(), caster ? caster->GetGUID().ToString() : "",
...
void Aura::UpdateTargetMap(Unit* caster, bool apply)
{
  ...
  for (ApplicationMap::iterator appIter = m_applications.begin(); appIter != m_applications.end(); ++appIter)
  {
    ...
    else
    {
      ...
      // (dbcs do not have auras which apply on same type of targets but have different radius, so this is not really
      // needed)
  ...
  for (std::map<Unit*, uint8>::iterator itr = targets.begin(); itr != targets.end();)
  {
    ...
    if (addUnit)
    {
      ...
      else // (GetType() == UNIT_AURA_TYPE)
      {
        ...
        if (itr->first != GetOwner())
        {
          ...
          // this one prevents unwanted usefull buff loss because of stacking and prevents overriding auras periodicaly
          // by 2 near area aura owners
    ...
    else
    {
      ...
      if (!GetOwner()->IsSelfOrInSameMap(itr->first))
      {
        ...
        LOG_FATAL("spells.aura", "Aura {}: Owner {} (map {}) is not in the same map as target {} (map {}).",
                  GetSpellInfo()->Id, GetOwner()->GetName(),
                  GetOwner()->IsInWorld() ? GetOwner()->GetMap()->GetId() : uint32(-1), itr->first->GetName(),
...
void Aura::RefreshTimersWithMods()
{
  ...
  // xinef: we should take ModSpellDuration into account, but none of the spells using this function is affected by
  // contents of ModSpellDuration
  RefreshDuration();
...
void Aura::UnregisterSingleTarget()
{
  ...
  if (!caster)
  {
    LOG_INFO("spells", "Aura::UnregisterSingleTarget: (A1) - {}, {}, {}, {}", GetId(), GetOwner()->GetTypeId(),
             GetOwner()->GetEntry(), GetOwner()->GetName());
...
void Aura::SetLoadedState(int32 maxduration, int32 duration, int32 charges, uint8 stackamount, uint8 recalculateMask,
                          int32* amount)
{
      ...
      case 12043: // Presence of Mind
        ...
        if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, 0))
        {
          ...
          case 31572:
            ...
            break;
          default:
            LOG_ERROR("spells.aura", "Aura::HandleAuraSpecificMods: Unknown rank of Arcane Potency ({}) found",
                      aurEff->GetId());
          }
          if (spellId)
      ...
      if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2)
      {
        ...
        if (aurEff)
        {
          ...
          case 49032:
            ...
            break;
          default:
            LOG_ERROR("spells.aura", "Aura::HandleAuraSpecificMods: Unknown rank of Crypt Fever/Ebon Plague ({}) found",
                      aurEff->GetId());
          }
        ...
        if (sSpellMgr->GetSpellGroup(GetId()) == 1) /*Elixirs*/
        {
          if (caster->HasSpell(GetSpellInfo()->Effects[EFFECT_0].TriggerSpell))
          {
            for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
              if (GetEffect(i))
                GetEffect(i)->SetAmount(
                    CalculatePct(GetEffect(i)->GetAmount(), 100 + sSpellMgr->GetSpellMixologyBonus(GetId())));
  ...
  else
  {
      ...
      if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400)
      {
        if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, 0))
        {
          ...
          case 53754:
            ...
            break;
          default:
            LOG_ERROR("spells.aura", "Aura::HandleAuraSpecificMods: Unknown rank of Improved Fear ({}) found",
                      aurEff->GetId());
          }
          if (spellId)
    ...
    case SPELLFAMILY_PRIEST:
      ...
      if (removeMode == AURA_REMOVE_BY_ENEMY_SPELL &&
          (GetSpellInfo()->SpellFamilyFlags[0] & 0x00008000 || GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400))
      {
        // Shadow Affinity
...
void Aura::LoadScripts()
{
  ...
  for (std::list<AuraScript*>::iterator itr = m_loadedScripts.begin(); itr != m_loadedScripts.end();)
  {
    ...
    LOG_DEBUG("spells.aura", "Aura::LoadScripts: Script `{}` for aura `{}` is loaded now",
...
bool Aura::CallScriptEffectApplyHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp,
...
bool Aura::CallScriptEffectRemoveHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp,
                                          AuraEffectHandleModes mode)
{
  bool preventDefault = false;
  for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
...
void Aura::CallScriptAfterEffectApplyHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp,
                                              AuraEffectHandleModes mode)
{
  for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
  {
...
void Aura::CallScriptAfterEffectRemoveHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp,
                                               AuraEffectHandleModes mode)
{
  for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
  {
...
void Aura::CallScriptEffectUpdatePeriodicHandlers(AuraEffect* aurEff)
{
  for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
  {
    ...
    std::list<AuraScript::EffectUpdatePeriodicHandler>::iterator effEndItr = (*scritr)->OnEffectUpdatePeriodic.end(),
...
void Aura::CallScriptEffectAfterAbsorbHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo& dmgInfo,
                                               uint32& absorbAmount)
{
  for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
  {
...
void Aura::CallScriptEffectAfterManaShieldHandlers(AuraEffect* aurEff, AuraApplication const* aurApp,
...
void Aura::CallScriptEffectSplitHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo& dmgInfo,
                                         uint32& splitAmount)
{
  for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
  {
...
bool Aura::CallScriptAfterCheckProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo,
...
void Aura::CallScriptAfterEffectProcHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp,
                                             ProcEventInfo& eventInfo)
{
  for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
  {
...
SpellInfo const* Aura::GetTriggeredByAuraSpellInfo() const { return m_triggeredByAuraSpellInfo; }
  ...
  for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
  {
    ...
    else
    {
      ...
      if (!GetUnitOwner()->HasUnitState(UNIT_STATE_ISOLATED))
      {
        ...
        {
          targetList.push_back(GetUnitOwner());
          Acore::AnyGroupedUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius,
                                                          GetSpellInfo()->Effects[effIndex].Effect ==
                                                              SPELL_EFFECT_APPLY_AREA_AURA_RAID);
          Acore::UnitListSearcher<Acore::AnyGroupedUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList,
                                                                                    u_check);
        ...
        case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
        {
          ...
          Acore::UnitListSearcher<Acore::AnyFriendlyUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList,
                                                                                     u_check);
          Cell::VisitAllObjects(GetUnitOwner(), searcher, radius);
          break;
        }
        ...
        {
          Acore::AnyAoETargetUnitInObjectRangeCheck                          u_check(GetUnitOwner(), GetUnitOwner(),
                                                                                     radius); // No GetCharmer in searcher
          Acore::UnitListSearcher<Acore::AnyAoETargetUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList,
                                                                                      u_check);
          Cell::VisitAllObjects(GetUnitOwner(), searcher, radius);
          break;
        }
...
void DynObjAura::FillTargetMap(std::map<Unit*, uint8>& targets, Unit* /*caster*/)
{
  ...
  for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
  {
    ...
    // pussywizard: TARGET_DEST_DYNOBJ_NONE is supposed to search for both friendly and unfriendly targets, so for any
    // unit what about EffectImplicitTargetA?
    else if (spellInfo->Effects[effIndex].TargetB.GetTarget() == TARGET_DEST_DYNOBJ_NONE)
    {
      Acore::AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster,
                                                                                radius);
    ...
    else
    {
      ...
      Acore::UnitListSearcher<Acore::AnyAoETargetUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList,
                                                                                  u_check);
      Cell::VisitAllObjects(GetDynobjOwner(), searcher, radius);
    }
    ...
    for (UnitList::iterator itr = targetList.begin(); itr != targetList.end(); ++itr)
    {
      ...
      if (!spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) &&
          ...
          !GetDynobjOwner()->IsWithinLOSInMap(target, VMAP::ModelIgnoreFlags::Nothing, LINEOFSIGHT_ALL_CHECKS,
                                              collisionHeight, combatReach))
      {
        continue;
      }
