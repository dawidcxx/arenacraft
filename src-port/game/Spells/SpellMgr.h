...
enum ProcFlags
{
  ...
  PROC_FLAG_DONE_TRAP_ACTIVATION = 0x00200000, // 21 On trap activation (possibly needs name change to
...
#define MELEE_BASED_TRIGGER_MASK                                                                                       \
  (PROC_FLAG_DONE_MELEE_AUTO_ATTACK | PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK | PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS |       \
   PROC_FLAG_TAKEN_SPELL_MELEE_DMG_CLASS | PROC_FLAG_DONE_RANGED_AUTO_ATTACK | PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK |    \
   PROC_FLAG_DONE_SPELL_RANGED_DMG_CLASS | PROC_FLAG_TAKEN_SPELL_RANGED_DMG_CLASS)
...
enum ProcFlagsExLegacy
{
  ...
  PROC_EX_EX_ONE_TIME_TRIGGER = 0x0020000, // If set trigger always but only one
...
{
  uint32 schoolMask;      // if nonzero - bit mask for matching proc condition based
  ...
  flag96 spellFamilyMask; // if nonzero - for matching proc condition based on
                          // candidate spell's SpellFamilyFlags  (like auras 107
                          // and 108 do)
  uint32 procFlags;       // bitmask for matching proc event
  uint32 procEx;          // proc Extend info (see ProcFlagsEx)
  uint32 procPhase;       // proc phase (see ProcFlagsSpellPhase)
  float  ppmRate;         // for melee (ranged?) damage spells - proc rate per minute. if
                          // zero, falls back to flat chance from Spell.dbc
...
struct SpellProcEntry
{
  ...
  uint32 ProcFlags;       // if nonzero - owerwrite procFlags field for given
                          // Spell.dbc entry, bitmask for matching proc condition, see
                          // enum ProcFlags
  uint32 SpellTypeMask;   // if nonzero - bitmask for matching proc condition
                          // based on candidate spell's damage/heal effects, see
                          // enum ProcFlagsSpellType
  uint32 SpellPhaseMask;  // if nonzero - bitmask for matching phase of a spellcast
                          // on which proc occurs, see enum ProcFlagsSpellPhase
  ...
  float  ProcsPerMinute;  // if nonzero - chance to proc is equal to value * aura
                          // caster's weapon speed / 60
...
enum SpellGroupSpecialFlags
{
  ...
  SPELL_GROUP_SPECIAL_FLAG_FORCED_STRONGEST    = 0x010, // xinef: specially helpful flag if some spells have different
...
typedef std::map<std::pair<uint32 /*spell_id*/, SpellEffIndex /*effIndex*/>, SpellTargetPosition>
...
public:
  ...
  [[nodiscard]] bool IsRemovedOnChangePet() const { return removeOnChangePet; }
...
struct SpellArea
{
  ...
  uint32 questStart;       // quest start (quest must be active or rewarded for spell
                           // apply)
  ...
  uint32 questStartStatus; // QuestStatus that quest_start must have in order to
                           // keep the spell
  uint32 questEndStatus;   // QuestStatus that the quest_end must have in order to
                           // keep the spell (if the quest_end's status is
                           // different than this, the spell will be dropped)
  bool autocast;           // if true then auto applied at area enter, in other case just
                           // allowed to cast
...
typedef std::pair<SpellAreaForQuestMap::const_iterator, SpellAreaForQuestMap::const_iterator>
...
typedef std::pair<SpellsRequiringSpellMap::const_iterator, SpellsRequiringSpellMap::const_iterator>
...
public:
  ...
  SpellGroupStackFlags CheckSpellGroupStackRules(SpellInfo const* spellInfo1, SpellInfo const* spellInfo2, bool remove,
                                                 bool areaAura) const;
  void                 GetSetOfSpellsInSpellGroupWithFlag(uint32 group_id, SpellGroupSpecialFlags flag,
                                                          std::set<uint32>& availableElixirs) const;

  // Spell proc event table
  [[nodiscard]] SpellProcEventEntry const* GetSpellProcEvent(uint32 spellId) const;
  bool IsSpellProcEventCanTriggeredBy(SpellInfo const* spellProto, SpellProcEventEntry const* spellProcEvent,
  ...
  [[nodiscard]] SpellInfo const* CheckSpellInfo(uint32 spellId) const
  {
    ...
    {
      LOG_ERROR("server", "spellId {} is not lower than GetSpellInfoStoreSize() ({})", spellId,
...
private:
  ...
  PetDefaultSpellsMap        mPetDefaultSpellsMap; // only spells not listed in related
