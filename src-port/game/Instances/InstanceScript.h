...
#define OUT_SAVE_INST_DATA                                                                                             \
  LOG_DEBUG("scripts.ai", "Saving Instance Data for Instance {} (Map {}, Instance Id {})", instance->GetMapName(),     \
...
#define OUT_LOAD_INST_DATA_COMPLETE                                                                                    \
  LOG_DEBUG("scripts.ai",                                                                                              \
            "Instance Data Load for Instance {} (Map {}, Instance Id: {}) is "                                         \
            "complete.",                                                                                               \
            instance->GetMapName(), instance->GetId(), instance->GetInstanceId())
#define OUT_LOAD_INST_DATA_FAIL                                                                                        \
  LOG_ERROR("scripts.ai",                                                                                              \
            "Unable to load Instance Data for Instance {} (Map {}, Instance "                                          \
            "Id: {}).",                                                                                                \
            instance->GetMapName(), instance->GetId(), instance->GetInstanceId())
...
enum EncounterFrameType
{
  ...
  ENCOUNTER_FRAME_REFRESH_FRAMES    = 7, // Xinef: can be used to refresh frames after unit was destroyed from
...
struct DoorInfo
{
  explicit DoorInfo(BossInfo* _bossInfo, DoorType _type) : bossInfo(_bossInfo), type(_type) {}
  BossInfo* bossInfo;
  DoorType  type;
};
...
public:
  ...
  // instance scripts use HandleGameObject(GUID, boolen, nullptr); in any other
  // script
  ...
  virtual bool CheckAchievementCriteriaMeet(uint32 /*criteria_id*/, Player const* /*source*/,
  ...
  // Allows executing code using all creatures registered in the instance script
  // as minions
...
protected:
  ...
  // This is used to handle summons that are not directly controlled by the
  // summoner. Summoner creature must be loaded in the instance data
  // (LoadObjectData).
