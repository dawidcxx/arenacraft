...
void LFGQueue::RemoveFromQueue(ObjectGuid guid, bool partial)
{
  ...
  for (LfgQueueDataContainer::iterator itr = QueueDataStore.begin(); itr != QueueDataStore.end(); ++itr)
  {
    if (itr->first != guid)
    {
      ...
      {
        LOG_DEBUG("lfg", "CLEAR bestCompatible: {}, because of: {}", itr->second.bestCompatible.toString(),
                  guid.ToString());
    ...
    {
      LOG_DEBUG("lfg", "CLEAR bestCompatible SELF: {}, because of: {}", itr->second.bestCompatible.toString(),
                guid.ToString());
      // itr->second.bestCompatible.clear(); // don't clear here, because UpdateQueueTimers will try to find with every
      // diff update
      itDelete = itr;
    }
  }
  ...
  if (!partial && itDelete != QueueDataStore.end())
  {
    ...
    LOG_DEBUG("lfg", "ERASE QueueDataStore for: {}, itDelete: {},{},{}", guid.ToString(), itDelete->second.dps,
              itDelete->second.healers, itDelete->second.tanks);
...
uint8 LFGQueue::FindGroups()
{
  ...
  if (!newToQueueStore.empty())
  {
    ...
    bool       pushCompatiblesToFront =
        (std::find(restoredAfterProposal.begin(), restoredAfterProposal.end(), newGuid) != restoredAfterProposal.end());
...
LfgCompatibility LFGQueue::FindNewGroups(const ObjectGuid& newGuid)
{
  ...
  // first 16 bits of the mask are for marking if such combination was found once, second 16 bits for marking second
  // occurence of that combination, etc
  uint64 foundMask  = 0;
  uint32 foundCount = 0;
...
LfgCompatibility LFGQueue::CheckCompatibility(Lfg5Guids const& checkWith, const ObjectGuid& newGuid, uint64& foundMask,
                                              uint32& foundCount, const std::set<Lfg5Guids>& currentCompatibles)
{
  ...
  for (uint8 i = 0; i < 5 && !(guid = check.guids[i]).IsEmpty() && numLfgGroups < 2 && numPlayers <= MAXGROUPSIZE; ++i)
  {
    ...
    // Store group so we don't need to call Mgr to get it later (if it's player group will be 0 otherwise would have
    // joined as group)
  ...
  if (!sLFGMgr->IsTesting() && check.size() == 1 && numPlayers < MAXGROUPSIZE)
  {
    ...
    itQueue->second.bestCompatible.clear(); // this may be left after a failed proposal (not cleared, because
                                            // UpdateQueueTimers would try to generate it with every update)
    // UpdateBestCompatibleInQueue(itQueue, strGuids);
    AddToCompatibles(strGuids);
    if (roleCheckResult && roleCheckResult <= 15)
      foundMask |= ((((uint64)1) << (roleCheckResult - 1)) | (((uint64)1) << (16 + roleCheckResult - 1)) |
  ...
  // If it's single group no need to check for duplicate players, ignores, bad roles or bad dungeons as it's been
  ...
  if (check.size() > 1)
  {
    for (uint8 i = 0; i < 5 && check.guids[i]; ++i)
    {
      ...
      for (LfgRolesMap::const_iterator itRoles = roles.begin(); itRoles != roles.end(); ++itRoles)
      {
        ...
        for (itPlayer = proposalRoles.begin(); itPlayer != proposalRoles.end(); ++itPlayer)
        {
          ...
          {
            // pussywizard: LFG this means that this player was in two different LfgQueueData (in QueueDataStore), and
            // at least one of them is a group guid, because we do checks so there aren't 2 same guids in current CHECK
            // LOG_ERROR("lfg", "LFGQueue::CheckCompatibility: ERROR! Player multiple times in queue! [{}]",
    ...
    for (uint8 i = 1; i < 5 && check.guids[i]; ++i)
    {
      ...
      std::set_intersection(proposalDungeons.begin(), proposalDungeons.end(), dungeons.begin(), dungeons.end(),
...
void LFGQueue::UpdateQueueTimers(uint32 diff)
{
  ...
  for (LfgQueueDataContainer::iterator itQueue = QueueDataStore.begin(); itQueue != QueueDataStore.end(); ++itQueue)
  {
    ...
    LfgQueueStatusData queueData(dungeonId, waitTime, wtAvg, wtTank, wtHealer, wtDps, queuedTime, queueinfo.tanks,
...
time_t LFGQueue::GetJoinTime(ObjectGuid guid) { return QueueDataStore[guid].joinTime; }
