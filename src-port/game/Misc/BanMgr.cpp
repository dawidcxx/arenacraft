...
BanReturn BanMgr::BanAccount(std::string const& AccountName, std::string const& Duration, std::string const& Reason,
                             std::string const& Author)
{
  ...
  if (banresult && ((*banresult)[0].Get<uint32>() == (*banresult)[1].Get<uint32>() ||
  ...
  if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
  {
    ...
    if (!IsPermanetly)
      ChatHandler(nullptr).SendWorldText(LANG_BAN_ACCOUNT_YOUBANNEDMESSAGE_WORLD, Author, AccountName,
                                         secsToTimeString(TimeStringToSecs(Duration), true), Reason);
    else
...
BanReturn BanMgr::BanAccountByPlayerName(std::string const& CharacterName, std::string const& Duration,
...
BanReturn BanMgr::BanIP(std::string const& IP, std::string const& Duration, std::string const& Reason,
                        std::string const& Author)
{
  ...
  if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
  {
    ...
    if (IsPermanetly)
      ...
      ChatHandler(nullptr).SendWorldText(LANG_BAN_IP_YOUBANNEDMESSAGE_WORLD, Author, IP,
                                         secsToTimeString(TimeStringToSecs(Duration), true), Reason);
  }
...
BanReturn BanMgr::BanCharacter(std::string const& CharacterName, std::string const& Duration, std::string const& Reason,
                               std::string const& Author)
{
  ...
  if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
  {
    ...
    if (!IsPermanetly)
      ChatHandler(nullptr).SendWorldText(LANG_BAN_CHARACTER_YOUBANNEDMESSAGE_WORLD, Author, CharacterName,
                                         secsToTimeString(TimeStringToSecs(Duration), true), Reason);
    else
