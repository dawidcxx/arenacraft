...
bool BattlegroundTemplate::IsArena() const { return BattlemasterEntry->type == MAP_ARENA; }
...
BattlegroundMgr::BattlegroundMgr()
    : m_ArenaTesting(false), m_Testing(false), m_NextAutoDistributionTime(0), m_AutoDistributionTimeChecker(0),
...
void BattlegroundMgr::Update(uint32 diff)
{
  ...
  // update using scheduled tasks (used only for rated arenas, initial opponent search works differently than periodic
  // queue update)
  if (!m_QueueUpdateScheduler.empty())
  {
    ...
    for (uint8 i = 0; i < scheduled.size(); i++)
    {
      ...
      m_BattlegroundQueues[bgQueueTypeId].BattlegroundQueueUpdate(diff, bgTypeId, bracket_id, arenaType,
                                                                  arenaMMRating > 0, arenaMMRating);
    ...
    for (uint32 qtype = BATTLEGROUND_QUEUE_2v2; qtype < MAX_BATTLEGROUND_QUEUE_TYPES; ++qtype)
    {
      for (uint32 bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
      {
        m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(
            m_NextPeriodicQueueUpdateTime, BATTLEGROUND_AA, BattlegroundBracketId(bracket),
    ...
    if (m_AutoDistributionTimeChecker < diff)
    {
      if (GameTime::GetGameTime() > m_NextAutoDistributionTime)
      {
        ...
        m_NextAutoDistributionTime =
            GameTime::GetGameTime() + Seconds(DAY * sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_INTERVAL_DAYS));
...
void BattlegroundMgr::BuildBattlegroundStatusPacket(WorldPacket* data, Battleground* bg, uint8 QueueSlot,
                                                    uint8 StatusID, uint32 Time1, uint32 Time2, uint8 arenatype,
                                                    TeamId teamId, bool isRated, BattlegroundTypeId forceBgTypeId)
{
  // pussywizard:
...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId        originalBgTypeId,
                                                     PvPDifficultyEntry const* bracketEntry, uint8 arenaType,
                                                     bool isRated)
{
...
void BattlegroundMgr::LoadBattlegroundTemplates()
{
  ...
  QueryResult result = WorldDatabase.Query(
      "SELECT ID, MinPlayersPerTeam, MaxPlayersPerTeam, MinLvl, MaxLvl, AllianceStartLoc, AllianceStartO, "
      "HordeStartLoc, HordeStartO, StartMaxDist, Weight, ScriptName FROM battleground_template");
  ...
  do
  {
    ...
    if (!bl)
    {
      LOG_ERROR("bg.battleground", "Battleground ID {} not found in BattlemasterList.dbc. Battleground not created.",
                bgTypeId);
      continue;
    }
    ...
    {
      LOG_ERROR("sql.sql",
                "Table `battleground_template` for id {} contains bad values for MinPlayersPerTeam ({}) and "
                "MaxPlayersPerTeam({}).",
    ...
    if (bgTemplate.Id != BATTLEGROUND_AA && bgTemplate.Id != BATTLEGROUND_RB)
    {
      ...
      {
        LOG_ERROR("sql.sql",
                  "Table `battleground_template` for id {} contains a non-existing WorldSafeLocs.dbc id {} in field "
                  "`AllianceStartLoc`. BG not created.",
                  bgTemplate.Id, startId);
        continue;
      }
  ...
  LOG_INFO("server.loading", ">> Loaded {} battlegrounds in {} ms", _battlegroundTemplates.size(),
...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket* data, ObjectGuid guid, Player* player,
                                                  BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
  if (!player)
    return;
    ...
    *data << uint32(0); // number of bg instances
    ...
    if (it != bgDataStore.end())
    {
      ...
      if (PvPDifficultyEntry const* bracketEntry =
              GetBattlegroundBracketByLevel(it->second._Battlegrounds.begin()->second->GetMapId(), player->GetLevel()))
      {
        uint32                          count     = 0;
...
void BattlegroundMgr::SendToBattleground(Player* player, uint32 instanceId, BattlegroundTypeId bgTypeId)
{
  if (Battleground* bg = GetBattleground(instanceId, bgTypeId))
  {
    ...
    LOG_DEBUG("bg.battleground", "BattlegroundMgr::SendToBattleground: Sending {} to map {}, {} (bgType {})",
              player->GetName(), mapid, pos->ToString(), bgTypeId);
    if (!player->TeleportTo(mapid, pos->GetPositionX(), pos->GetPositionY(), pos->GetPositionZ(),
                            pos->GetOrientation()))
    {
      LOG_ERROR("bg.battleground", "Failed to teleport player to Battleground map");
    }
  }
  else
  {
    LOG_ERROR(
        ...
        instanceId, bgTypeId, player->GetName());
  }
}
...
void BattlegroundMgr::LoadBattleMastersEntry()
{
  ...
  do
  {
    ...
    {
      LOG_ERROR("sql.sql", "Table `battlemaster_entry` contain entry {} for not existed battleground type {}, ignored.",
                entry, bgTypeId);
      continue;
    }
...
void BattlegroundMgr::CheckBattleMasters()
{
  ...
  for (CreatureTemplateContainer::const_iterator itr = ctc->begin(); itr != ctc->end(); ++itr)
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "CreatureTemplate (Entry: {}) has UNIT_NPC_FLAG_BATTLEMASTER but no data in `battlemaster_entry` "
                "table. Removing flag!",
                itr->second.Entry);
...
std::unordered_map<int, BattlegroundTypeId> BattlegroundMgr::queueToBg = {
    {BATTLEGROUND_QUEUE_NONE, BATTLEGROUND_TYPE_NONE}, {BATTLEGROUND_QUEUE_AV, BATTLEGROUND_AV},
    {BATTLEGROUND_QUEUE_WS, BATTLEGROUND_WS},          {BATTLEGROUND_QUEUE_AB, BATTLEGROUND_AB},
    {BATTLEGROUND_QUEUE_EY, BATTLEGROUND_EY},          {BATTLEGROUND_QUEUE_SA, BATTLEGROUND_SA},
    {BATTLEGROUND_QUEUE_IC, BATTLEGROUND_IC},          {BATTLEGROUND_QUEUE_RB, BATTLEGROUND_RB},
    {BATTLEGROUND_QUEUE_2v2, BATTLEGROUND_AA},         {BATTLEGROUND_QUEUE_3v3, BATTLEGROUND_AA},
    {BATTLEGROUND_QUEUE_5v5, BATTLEGROUND_AA},
};

std::unordered_map<int, Battleground*> BattlegroundMgr::bgtypeToBattleground = {
    {BATTLEGROUND_AV, new BattlegroundAV}, {BATTLEGROUND_WS, new BattlegroundWS}, {BATTLEGROUND_AB, new BattlegroundAB},
    {BATTLEGROUND_NA, new BattlegroundNA}, {BATTLEGROUND_BE, new BattlegroundBE}, {BATTLEGROUND_EY, new BattlegroundEY},
    {BATTLEGROUND_RL, new BattlegroundRL}, {BATTLEGROUND_SA, new BattlegroundSA}, {BATTLEGROUND_DS, new BattlegroundDS},
    {BATTLEGROUND_RV, new BattlegroundRV}, {BATTLEGROUND_IC, new BattlegroundIC}, {BATTLEGROUND_AA, new Battleground},
...
std::unordered_map<int, bgTypeRef> BattlegroundMgr::getBgFromTypeID = {
    {BATTLEGROUND_RB, [](WorldPacket* data, Battleground::BattlegroundScoreMap::const_iterator itr2, Battleground* bg)
     {
...
std::unordered_map<uint32, ArenaType> BattlegroundMgr::QueueToArenaType = {{BATTLEGROUND_QUEUE_2v2, ARENA_TYPE_2v2},
