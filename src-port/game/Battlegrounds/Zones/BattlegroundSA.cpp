...
bool BattlegroundSA::ResetObjs()
{
  ...
  {
    if (!AddObject(i, BG_SA_ObjEntries[i], BG_SA_ObjSpawnlocs[i][0], BG_SA_ObjSpawnlocs[i][1], BG_SA_ObjSpawnlocs[i][2],
  ...
  for (uint8 i = BG_SA_BOAT_ONE; i < BG_SA_SIGIL_1; i++)
  {
    ...
    if (!AddObject(i, boatid, BG_SA_ObjSpawnlocs[i][0], BG_SA_ObjSpawnlocs[i][1],
                   BG_SA_ObjSpawnlocs[i][2] + (Attackers ? -3.750f : 0), BG_SA_ObjSpawnlocs[i][3], 0, 0, 0, 0,
                   RESPAWN_ONE_DAY))
      return false;
  }
  ...
  {
    if (!AddCreature(BG_SA_NpcEntries[i], i, BG_SA_NpcSpawnlocs[i][0], BG_SA_NpcSpawnlocs[i][1],
  ...
  {
    AddObject(i, (BG_SA_ObjEntries[i] - (Attackers == TEAM_ALLIANCE ? 1 : 0)), BG_SA_ObjSpawnlocs[i][0],
              BG_SA_ObjSpawnlocs[i][1], BG_SA_ObjSpawnlocs[i][2], BG_SA_ObjSpawnlocs[i][3], 0, 0, 0, 0,
              RESPAWN_ONE_DAY);
  ...
  {
    AddObject(i, BG_SA_ObjEntries[i], BG_SA_ObjSpawnlocs[i][0], BG_SA_ObjSpawnlocs[i][1], BG_SA_ObjSpawnlocs[i][2],
              BG_SA_ObjSpawnlocs[i][3], 0, 0, 0, 0, RESPAWN_ONE_DAY);
  ...
  {
    AddObject(i, BG_SA_ObjEntries[BG_SA_BOMB], BG_SA_ObjSpawnlocs[i][0], BG_SA_ObjSpawnlocs[i][1],
              BG_SA_ObjSpawnlocs[i][2], BG_SA_ObjSpawnlocs[i][3], 0, 0, 0, 0, RESPAWN_ONE_DAY);
    GetBGObject(i)->SetUInt32Value(GAMEOBJECT_FACTION, atF);
  }
...
void BattlegroundSA::DefendersPortalTeleport(GameObject* portal, Player* plr)
{
  ...
  plr->TeleportTo(plr->GetMapId(), SOTADefPortalDest[portal_num][0], SOTADefPortalDest[portal_num][1],
                  SOTADefPortalDest[portal_num][2], SOTADefPortalDest[portal_num][3], TELE_TO_SPELL);
}
...
void BattlegroundSA::CaptureGraveyard(BG_SA_Graveyards i, Player* Source)
{
  ...
  AddSpiritGuide(i + static_cast<uint16>(BG_SA_MAXNPC), sg->x, sg->y, sg->z, BG_SA_GYOrientation[i],
                 GraveyardStatus[i]);
  uint32 npc  = 0;
  uint32 flag = 0;
  ...
  case BG_SA_LEFT_CAPTURABLE_GY:
    ...
    AddObject(flag, (BG_SA_ObjEntries[flag] - (Source->GetTeamId() == TEAM_ALLIANCE ? 0 : 1)),
              BG_SA_ObjSpawnlocs[flag][0], BG_SA_ObjSpawnlocs[flag][1], BG_SA_ObjSpawnlocs[flag][2],
              BG_SA_ObjSpawnlocs[flag][3], 0, 0, 0, 0, RESPAWN_ONE_DAY);

    npc = BG_SA_NPC_RIGSPARK;
    AddCreature(BG_SA_NpcEntries[npc], npc, Attackers, BG_SA_NpcSpawnlocs[npc][0], BG_SA_NpcSpawnlocs[npc][1],
                BG_SA_NpcSpawnlocs[npc][2], BG_SA_NpcSpawnlocs[npc][3]);

    for (uint8 j = BG_SA_DEMOLISHER_7; j <= BG_SA_DEMOLISHER_8; j++)
    {
      AddCreature(BG_SA_NpcEntries[j], j, BG_SA_NpcSpawnlocs[j][0], BG_SA_NpcSpawnlocs[j][1], BG_SA_NpcSpawnlocs[j][2],
  ...
  case BG_SA_RIGHT_CAPTURABLE_GY:
    ...
    AddCreature(BG_SA_NpcEntries[npc], npc, BG_SA_NpcSpawnlocs[npc][0], BG_SA_NpcSpawnlocs[npc][1],
                BG_SA_NpcSpawnlocs[npc][2], BG_SA_NpcSpawnlocs[npc][3]);
      ...
      if (Creature* Demolisher = GetBGCreature(i))
      {
        if (Demolisher->isDead())
        {
          ...
          else
          {
            ...
            {
              Demolisher->Relocate(BG_SA_NpcSpawnlocs[i][0], BG_SA_NpcSpawnlocs[i][1], BG_SA_NpcSpawnlocs[i][2],
