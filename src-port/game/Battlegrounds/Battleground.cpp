...
inline void Battleground::_CheckSafePositions(uint32 diff)
{
  ...
  if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL)
  {
    ...
    for (auto const& [playerGuid, player] : GetPlayers())
    {
      ...
      {
        LOG_DEBUG("bg.battleground", "BATTLEGROUND: Sending {} back to start location (map: {}) (possible exploit)",
                  player->GetName(), GetMapId());
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
  ...
  if (!(m_Events & BG_STARTING_EVENT_1))
  {
    ...
    if (!FindBgMap())
    {
      LOG_ERROR("bg.battleground", "Battleground::_ProcessJoin: map (map id: {}, instance id: {}) is not created!",
    ...
    if (isArena())
    {
      sScriptMgr->OnArenaInvitesSent(this);
    }
  }
  ...
  else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
  {
    ...
    if (isArena())
    {
      ...
      for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        ...
        {
          WorldPacket status;
          sBattlegroundMgr->BuildBattlegroundStatusPacket(&status, this, player->GetCurrentBattlegroundQueueSlot(),
          ...
          for (Unit::AuraApplicationMap::iterator iter = auraMap.begin(); iter != auraMap.end();)
          {
            ...
            if (!aura->IsPermanent() && aura->GetDuration() <= 30 * IN_MILLISECONDS &&
                aurApp->IsPositive()
                // && (!aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_NO_IMMUNITIES)) Xinef: condition, ALL buffs
                // should be removed
    ...
    else
    {
      ...
      if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
        ChatHandler(nullptr).SendWorldText(LANG_BG_STARTED_ANNOUNCE_WORLD, GetName(),
                                           std::min(GetMinLevel(), (uint32)80), std::min(GetMaxLevel(), (uint32)80));
...
void Battleground::PlaySoundToAll(uint32 soundID) { SendPacketToAll(WorldPackets::Misc::Playsound(soundID).Write()); }
...
void Battleground::EndBattleground(PvPTeamId winnerTeamId)
{
  ...
  for (auto const& [playerGuid, player] : m_Players)
  {
    ...
    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, this, player->GetCurrentBattlegroundQueueSlot(),
                                                    STATUS_IN_PROGRESS, TIME_TO_AUTOREMOVE, GetStartTime(),
                                                    GetArenaType(), player->GetBgTeamId());
...
void Battleground::BlockMovement(Player* player)
{
  player->SetClientControl(
      player, 0); // movement disabled NOTE: the effect will be automatically removed by client when the player is
                  // teleported from the battleground, so no need to send with uint8(1) in RemovePlayerAtLeave()
}
...
void Battleground::RemovePlayerAtLeave(Player* player)
{
  ...
  if (participant)
  {
    ...
    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, this, player->GetCurrentBattlegroundQueueSlot(), STATUS_NONE,
                                                    0, 0, 0, TEAM_NEUTRAL);
    player->GetSession()->SendPacket(&data);
...
void Battleground::Init()
{
  ...
  if (m_BgInvitedPlayers[TEAM_ALLIANCE] > 0 || m_BgInvitedPlayers[TEAM_HORDE] > 0)
  {
    LOG_ERROR("bg.battleground",
              "Battleground::Reset: one of the counters is not 0 (alliance: {}, horde: {}) for BG (map: {}, instance "
              "id: {})!",
              m_BgInvitedPlayers[TEAM_ALLIANCE], m_BgInvitedPlayers[TEAM_HORDE], m_MapId, m_InstanceID);
    ABORT();
  }
...
void Battleground::StartBattleground()
{
  ...
  // this must be done here, because we need to have already invited some players when first Battleground::Update()
  // method is executed
  sBattlegroundMgr->AddBattleground(this);

  if (m_IsRated)
    LOG_DEBUG("bg.arena", "Arena match type: {} for Team1Id: {} - Team2Id: {} started.", m_ArenaType,
...
void Battleground::AddOrSetPlayerToCorrectBgGroup(Player* player, TeamId teamId)
{
  ...
  {
    LOG_INFO("misc", "Battleground::AddOrSetPlayerToCorrectBgGroup - player is already in {} group!",
...
uint32 Battleground::GetFreeSlotsForTeam(TeamId teamId) const
{
  ...
  // if CONFIG_BATTLEGROUND_INVITATION_TYPE == BG_QUEUE_INVITATION_TYPE_NO_BALANCE, invite everyone unless the BG is
  // full
  ...
  // if BG is already started or CONFIG_BATTLEGROUND_INVITATION_TYPE != BG_QUEUE_INVITATION_TYPE_NO_BALANCE, do not
  // allow to join too many players of one faction
                             ...
                             float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/, GOState goState)
{
  ...
  // and when loading it (in go::LoadFromDB()), a new guid would be assigned to the object, and a new object would be
  // created So we must create it specific for this instance
  GameObject* go = sObjectMgr->IsGameObjectStaticTransport(entry) ? new StaticTransport() : new GameObject();
  if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(), entry, GetBgMap(), PHASEMASK_NORMAL, x, y, z, o,
                  G3D::Quat(rotation0, rotation1, rotation2, rotation3), 100, goState))
  {
    LOG_ERROR("sql.sql",
              "Battleground::AddObject: cannot create gameobject (entry: {}) for BG (map: {}, instance id: {})!", entry,
              m_MapId, m_InstanceID);
    LOG_ERROR("bg.battleground",
              "Battleground::AddObject: cannot create gameobject (entry: {}) for BG (map: {}, instance id: {})!", entry,
              m_MapId, m_InstanceID);
  ...
  /*
      ...
      // iirc that was changed, so adding to go data map is no longer required if that was the only function using
     godata from GameObject without checking if it existed GameObjectData& data = sObjectMgr->NewGOData(spawnId);
...
GameObject* Battleground::GetBGObject(uint32 type)
{
  ...
  if (!obj)
    LOG_ERROR("bg.battleground",
              "Battleground::GetBGObject: gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!", type,
              BgObjects[type].ToString(), m_MapId, m_InstanceID);
  return obj;
}
...
Creature* Battleground::GetBGCreature(uint32 type)
{
  ...
  if (!creature)
    LOG_ERROR("bg.battleground",
              "Battleground::GetBGCreature: creature (type: {}, {}) not found for BG (map: {}, instance id: {})!", type,
              BgCreatures[type].ToString(), m_MapId, m_InstanceID);
  return creature;
}
                                    ...
                                    MotionTransport* transport)
{
  ...
  {
    LOG_ERROR("bg.battleground",
              "Battleground::AddCreature: cannot create creature (entry: {}) for BG (map: {}, instance id: {})!", entry,
              m_MapId, m_InstanceID);
    delete creature;
    return nullptr;
  }
...
bool Battleground::DelCreature(uint32 type)
{
  ...
  LOG_ERROR("bg.battleground",
            "Battleground::DelCreature: creature (type: {}, {}) not found for BG (map: {}, instance id: {})!", type,
            BgCreatures[type].ToString(), m_MapId, m_InstanceID);
...
bool Battleground::DelObject(uint32 type)
{
  ...
  LOG_ERROR("bg.battleground",
            "Battleground::DelObject: gameobject (type: {}, {}) not found for BG (map: {}, instance id: {})!", type,
            BgObjects[type].ToString(), m_MapId, m_InstanceID);
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, TeamId teamId)
{
  ...
  LOG_ERROR("bg.battleground",
            "Battleground::AddSpiritGuide: cannot create spirit guide (type: {}, entry: {}) for BG (map: {}, instance "
            "id: {})!",
...
int32 Battleground::GetObjectType(ObjectGuid guid)
{
  ...
  LOG_ERROR("bg.battleground",
            "Battleground::GetObjectType: player used gameobject ({}) which is not in internal data for BG (map: {}, "
            "instance id: {}), cheating?",
