...
public:
  CreatureTextBuilder(WorldObject* obj, uint8 gender, ChatMsg msgtype, uint8 textGroup, uint32 id, uint32 language,
      ...
      : _source(obj), _gender(gender), _msgType(msgtype), _textGroup(textGroup), _textId(id), _language(language),
        _target(target)
  {
  }
...
public:
  PlayerTextBuilder(WorldObject* obj, WorldObject* speaker, uint8 gender, ChatMsg msgtype, uint8 textGroup, uint32 id,
      ...
      : _source(obj), _talker(speaker), _gender(gender), _msgType(msgtype), _textGroup(textGroup), _textId(id),
        _language(language), _target(target)
  {
  }
...
void CreatureTextMgr::LoadCreatureTexts()
{
  ...
  do
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "CreatureTextMgr: Entry {}, Group {} in table `creature_texts` using Language {} but Language does not "
                "exist.",
    ...
    {
      LOG_ERROR("sql.sql",
                "CreatureTextMgr: Entry {}, Group {} in table `creature_texts` has Type {} but this Chat Type does not "
                "exist.",
    ...
    if (temp.BroadcastTextId)
    {
      ...
      {
        LOG_ERROR("sql.sql",
                  "CreatureTextMgr: Entry {}, Group {}, Id {} in table `creature_text` has non-existing or "
                  "incompatible BroadcastTextId {}.",
  ...
  LOG_INFO("server.loading", ">> Loaded {} Creature Texts For {} Creatures in {} ms", textCount, mTextMap.size(),
...
void CreatureTextMgr::LoadCreatureTextLocales()
{
  ...
  LOG_INFO("server.loading", ">> Loaded {} Creature Text Locale in {} ms", uint32(mLocaleTextMap.size()),
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server.loading", " ");
}
                                 ...
                                 Player* srcPlr /*= nullptr*/)
{
  ...
  if (srcPlr)
  {
    PlayerTextBuilder builder(source, finalSource, finalSource->getGender(), finalType, iter->group, iter->id,
                              finalLang, target);
    SendChatPacket(finalSource, builder, finalType, target, range, teamId, gmOnly);
  }
  else
  {
    CreatureTextBuilder builder(finalSource, finalSource->getGender(), finalType, iter->group, iter->id, finalLang,
                                target);
    SendChatPacket(finalSource, builder, finalType, target, range, teamId, gmOnly);
  }
...
void CreatureTextMgr::SendNonChatPacket(WorldObject* source, WorldPacket const* data, ChatMsg msgType,
                                        WorldObject const* target, CreatureTextRange range, TeamId teamId,
                                        bool gmOnly) const
{
...
void CreatureTextMgr::SetRepeatId(Creature* source, uint8 textGroup, uint8 id)
{
  ...
  if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
    ...
    LOG_ERROR("sql.sql", "CreatureTextMgr: TextGroup {} for Creature {} ({}), id {} already added", uint32(textGroup),
              source->GetName(), source->GetGUID().ToString(), uint32(id));
}
...
bool CreatureTextMgr::TextExist(uint32 sourceEntry, uint8 textGroup)
{
  ...
  if (itr == textHolder.end())
  {
    LOG_DEBUG("entities.unit", "CreatureTextMgr::TextExist: Could not find TextGroup {} for Creature (entry {}).",
...
std::string CreatureTextMgr::GetLocalizedChatString(uint32 entry, uint8 gender, uint8 textGroup, uint32 id,
