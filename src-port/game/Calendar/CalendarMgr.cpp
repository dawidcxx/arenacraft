...
CalendarInvite::CalendarInvite()
    : _inviteId(1), _eventId(0), _statusTime(GameTime::GetGameTime().count()), _status(CALENDAR_STATUS_INVITED),
      _rank(CALENDAR_RANK_PLAYER), _text("")
{
}
  ...
  if (QueryResult result = CharacterDatabase.Query(
    ...
    do
    {
      ...
      CalendarEvent* calendarEvent =
          new CalendarEvent(eventId, creatorGUID, guildId, type, dungeonId, time_t(eventTime), flags,
                            time_t(timezoneTime), title, description);
...
void CalendarMgr::RemoveEvent(CalendarEvent* calendarEvent, ObjectGuid remover)
{
  ...
  for (std::size_t i = 0; i < eventInvites.size(); ++i)
  {
    ...
    // When an event is deleted, all invited (accepted/declined? - verify) guildies are notified via in-game mail.
    // (wowwiki)
    if (remover && invite->GetInviteeGUID() != remover)
      mail.SendMailTo(trans, MailReceiver(invite->GetInviteeGUID().GetCounter()), calendarEvent,
...
CalendarInviteStore const& CalendarMgr::GetEventInvites(uint64 eventId) { return _invites[eventId]; }
...
{
  WorldPacket data(SMSG_CALENDAR_EVENT_UPDATED_ALERT, 1 + 8 + 4 + 4 + 4 + 1 + 4 + calendarEvent.GetTitle().size() +
...
void CalendarMgr::SendCalendarEventInviteRemove(CalendarEvent const& calendarEvent, CalendarInvite const& invite,
                                                uint32 flags)
{
...
void CalendarMgr::SendCalendarEventInviteRemoveAlert(ObjectGuid guid, CalendarEvent const& calendarEvent,
...
void CalendarMgr::SendPacketToAllEventRelatives(WorldPacket packet, CalendarEvent const& calendarEvent)
{
  ...
  // Send packet to all invitees if event is non-guild, in other case only to non-guild invitees (packet was broadcasted
  // for them)
