...
template <class T>
bool ChaseMovementGenerator<T>::PositionOkay(T* owner, Unit* target, Optional<float> maxDistance,
...
template <class T> bool ChaseMovementGenerator<T>::DoUpdate(T* owner, uint32 time_diff)
{
  ...
  bool forceDest =
      ...
      (owner->CanFly()); // closes "bool forceDest", that way it is more appropriate, so we can comment out crap
                         // whenever we need to
      ...
      !owner->IsWithinLOSInMap(target))
  {
    ...
    if (owner->HasUnitState(UNIT_STATE_CHASE_MOVE) || !PositionOkay(owner, target, maxTarget, angle))
    {
      ...
      else
      {
        ...
        target->GetNearPoint(owner, x, y, z, (moveToward ? maxTarget : minTarget) - chaseRange - additionalRange, 0,
...
template <class T> void ChaseMovementGenerator<T>::DoReset(T* owner) { DoInitialize(owner); }
...
static Position const PredictPosition(Unit* target)
{
  ...
  // 0.5 - it's time (0.5 sec) between starting movement opcode (e.g. MSG_MOVE_START_FORWARD) and MSG_MOVE_HEARTBEAT
  // sent by client
...
bool FollowMovementGenerator<T>::PositionOkay(Unit* target, bool isPlayerPet, bool& targetIsMoving, uint32 diff)
{
  ...
  float exactDistSq = target->GetExactDistSq(_lastTargetPosition->GetPositionX(), _lastTargetPosition->GetPositionY(),
  ...
  if (isPlayerPet)
  {
    targetIsMoving = target->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_BACKWARD |
...
template <class T> void FollowMovementGenerator<T>::DoReset(T* owner) { DoInitialize(owner); }
