...
template <class T> class RandomMovementGenerator : public MovementGeneratorMedium<T, RandomMovementGenerator<T>>
{
public:
        ...
        _currentPoint(RANDOM_POINTS_NUMBER)
  {
    ...
    for (uint8 i = 0; i < RANDOM_POINTS_NUMBER; ++i)
    {
      ...
      for (uint8 j = 0; j < RANDOM_LINKS_COUNT; ++j)
        _validPointsVector[i].push_back((i + j + RANDOM_POINTS_NUMBER / 2 - RANDOM_LINKS_COUNT / 2) %
