...
char const* ChatHandler::GetAcoreString(uint32 entry) const { return m_session->GetAcoreString(entry); }
...
void ChatHandler::SendSysMessage(uint32 entry) { SendSysMessage(GetAcoreString(entry)); }
...
std::size_t ChatHandler::BuildChatPacket(WorldPacket& data, ChatMsg chatType, Language language,
                                         ...
                                         std::string_view message, uint32 achievementId /*= 0*/,
                                         ...
                                         LocaleConstant     locale /*= DEFAULT_LOCALE*/)
{
  ...
  return BuildChatPacket(data, chatType, language, senderGUID, receiverGUID, message, chatTag, senderName, receiverName,
...
bool ChatHandler::extractPlayerTarget(char* args, Player** player, ObjectGuid* player_guid /*=nullptr*/,
...
char const* CliHandler::GetAcoreString(uint32 entry) const { return sObjectMgr->GetAcoreStringForDBCLocale(entry); }
...
std::string CliHandler::GetNameLink() const { return GetAcoreString(LANG_CONSOLE_COMMAND); }

bool CliHandler::needReportToTarget(Player* /*chr*/) const { return true; }

bool ChatHandler::GetPlayerGroupAndGUIDByName(const char* cname, Player*& player, Group*& group, ObjectGuid& guid,
                                              bool offline)
{
  player = nullptr;
...
bool CliHandler::HasSession() const { return true; }
...
{
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, CHAT_MSG_WHISPER, LANG_ADDON, GetSession()->GetPlayer(), GetSession()->GetPlayer(),
                               msg);
