...
Channel::Channel(std::string const& name, uint32 channelId, uint32 channelDBId, TeamId teamId, bool announce,
                 bool ownership)
    : _announce(announce), _ownership(ownership), _IsSaved(false), _isOwnerGM(false), _flags(0), _channelId(channelId),
...
void Channel::UnBan(Player const* player, std::string const& badname)
{
  ...
  if (_channelRights.flags & CHANNEL_RIGHT_CANT_BAN)
    LOG_GM(player->GetSession()->GetAccountId(),
           "Command: /unban {} {} (Moderator {} [{}, account: {}] unbanned {} [{}])", GetName(), badname,
           player->GetName(), player->GetGUID().ToString(), player->GetSession()->GetAccountId(), badname,
           victim.ToString());
...
void Channel::SetMode(Player const* player, std::string const& p2n, bool mod, bool set)
{
  ...
  if (!victim || !IsOn(victim) ||
      ...
      ((!player->GetSession()->IsGMAccount() || !newp->GetSession()->IsGMAccount()) &&
       player->GetTeamId() != newp->GetTeamId() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL)))
  {
    WorldPacket data;
...
void Channel::MakeNotInArea(WorldPacket* data) { MakeNotifyPacket(data, CHAT_NOT_IN_AREA_NOTICE); }

void Channel::MakeNotInLfg(WorldPacket* data) { MakeNotifyPacket(data, CHAT_NOT_IN_LFG_NOTICE); }
