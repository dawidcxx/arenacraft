...
/*static*/ void Acore::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap() { COMMAND_MAP.clear(); }
...
/*static*/ void Acore::Impl::ChatCommands::ChatCommandNode::LoadCommandMap()
{
  ...
  if (PreparedQueryResult result = WorldDatabase.Query(WorldDatabase.GetPreparedStatement(WORLD_SEL_COMMANDS)))
  {
    do
    {
      ...
      {
        LOG_WARN("sql.sql",
                 "Table `command` has permission {} for '{}' which does not match the core ({}). Overriding.", secLevel,
                 name, cmd->_permission.RequiredLevel);
...
static void LogCommandUsage(WorldSession const& session, std::string_view cmdStr)
{
  ...
  std::string logMessage = Acore::StringFormat(
      ...
      "{} ({})]",
      cmdStr, player->GetName(), player->GetGUID().ToString(), session.GetAccountId(), player->GetPositionX(),
      player->GetPositionY(), player->GetPositionZ(), player->GetMapId(),
      player->FindMap() ? player->FindMap()->GetMapName() : "Unknown", areaId, areaName, zoneId, zoneName,
      (player->GetSelectedUnit()) ? player->GetSelectedUnit()->GetName() : "", targetGuid.ToString());
...
void Acore::Impl::ChatCommands::ChatCommandNode::SendCommandHelp(ChatHandler& handler) const
{
  ...
  for (auto it = _subCommands.begin(); it != _subCommands.end(); ++it)
  {
    ...
    handler.PSendSysMessage(subCommandHasSubCommand ? LANG_SUBCMDS_LIST_ENTRY_ELLIPSIS : LANG_SUBCMDS_LIST_ENTRY,
                            it->second._name);
  }
}
...
/*static*/ bool Acore::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(ChatHandler&     handler,
  ...
  while (!oldTail.empty())
  {
    ...
    { /* ok, so it1 points at a partially matching subcommand - let's see if there are others */
      ...
      { /* there are multiple matching subcommands - print possibilities and return */
        ...
        do
        {
          handler.PSendSysMessage(it2->second.HasVisibleSubCommands(handler) ? LANG_SUBCMDS_LIST_ENTRY_ELLIPSIS
                                                                             : LANG_SUBCMDS_LIST_ENTRY,
                                  it2->first);
        } while (++it2);
...
/*static*/ void Acore::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(ChatHandler&     handler,
                                                                               std::string_view cmdStr)
{
  ChatCommandNode const*   cmd = nullptr;
  ChatSubCommandMap const* map = &GetTopLevelMap();
  ...
  else if (cmdStr.empty())
  {
    ...
    do
    {
      handler.PSendSysMessage(it->second.HasVisibleSubCommands(handler) ? LANG_SUBCMDS_LIST_ENTRY_ELLIPSIS
                                                                        : LANG_SUBCMDS_LIST_ENTRY,
                              it->second._name);
    } while (++it);
  }
...
Acore::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmdStr)
{
  std::string              path;
  ChatCommandNode const*   cmd = nullptr;
  ChatSubCommandMap const* map = &GetTopLevelMap();
