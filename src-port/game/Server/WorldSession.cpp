...
WorldSession::WorldSession(uint32 id, std::string&& name, std::shared_ptr<WorldSocket> sock, AccountTypes sec,
                           uint8 expansion, time_t mute_time, LocaleConstant locale, uint32 recruiter,
                           bool isARecruiter, bool skipQueue, uint32 TotalTime)
    : m_muteTime(mute_time), m_timeOutTime(0), _lastAuctionListItemsMSTime(0), _lastAuctionListOwnerItemsMSTime(0),
      AntiDOS(this), m_GUIDLow(0), _player(nullptr), m_Socket(sock), _security(sec), _skipQueue(skipQueue),
      _accountId(id), _accountName(std::move(name)), m_expansion(expansion), m_total_time(TotalTime), _logoutTime(0),
...
bool WorldSession::IsGMAccount() const { return GetSecurity() >= SEC_GAMEMASTER; }

std::string const& WorldSession::GetPlayerName() const { return _player ? _player->GetName() : DefaultPlayerName; }
...
ObjectGuid::LowType WorldSession::GetGuidLow() const { return GetPlayer() ? GetPlayer()->GetGUID().GetCounter() : 0; }
...
void WorldSession::SendPacket(WorldPacket const* packet)
{
  ...
  else
  {
    ...
    LOG_DEBUG("network", "Send all time packets count: {} bytes: {} avr.count/sec: {} avr.bytes/sec: {} time: {}",
              sendPacketCount, sendPacketBytes, float(sendPacketCount) / fullTime, float(sendPacketBytes) / fullTime,
              uint32(fullTime));

    LOG_DEBUG("network", "Send last min packets count: {} bytes: {} avr.count/sec: {} avr.bytes/sec: {}",
...
void WorldSession::QueuePacket(WorldPacket* new_packet) { _recvQueue.add(new_packet); }
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
  ...
  while (m_Socket && _recvQueue.next(packet, updater))
  {
    ...
    try
    {
      ...
      case STATUS_LOGGEDIN:
        if (!_player)
        {
          ...
          //! the client to be in world yet. We will re-add the packets to the bottom of the queue and process them
          //! later.
      ...
      case STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT:
        if (!_player && !m_playerRecentlyLogout) // There's a short delay between _player = null and
    ...
    {
      LOG_ERROR("network", "PacketArrayMaxCapacityException: {} while parsing {} from {}.", pamce.what(),
                GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())), GetPlayerInfo());
    }
    catch (ByteBufferException const&)
    {
      LOG_ERROR("network",
                "WorldSession::Update ByteBufferException occured while parsing a packet (opcode: {}) from client {}, "
...
{
  if (m_Socket && !m_Socket->IsOpen() && !IsKicked() && GetPlayer() && !PlayerLogout() && GetPlayer()->m_taxi.empty() &&
...
bool WorldSession::IsSocketClosed() const { return !m_Socket || !m_Socket->IsOpen(); }
...
void WorldSession::LogoutPlayer(bool save)
{
  ...
  if (_player)
  {
    ...
    // there are some positive auras from boss encounters that can be kept by logging out and logging in after boss is
    // dead, and may be used on next bosses
    _player->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_CHANGE_MAP);

    ///- If the player is in a group and LeaveGroupOnLogout is enabled or if the player is invited to a group, remove
    /// him. If the group is then only 1 person, disband the group.
    if (!_player->GetGroup() || sWorld->getBoolConfig(CONFIG_LEAVE_GROUP_ON_LOGOUT))
      _player->UninviteFromGroup();

    // remove player from the group if he is:
    // a) in group; b) not in raid group; c) logging out normally (not being kicked or disconnected) d)
    // LeaveGroupOnLogout is enabled
    ...
    LOG_INFO("entities.player", "Account: {} (IP: {}) Logout Character:[{}] ({}) Level: {}", GetAccountId(),
             GetRemoteAddress(), _player->GetName(), _player->GetGUID().ToString(), _player->GetLevel());
...
void WorldSession::KickPlayer(std::string const& reason, bool setKicked)
{
  if (m_Socket)
  {
    LOG_INFO("network.kick", "Account: {} Character: '{}' {} kicked with reason: {}", GetAccountId(),
...
bool WorldSession::DisallowHyperlinksAndMaybeKick(std::string_view str)
{
  ...
  LOG_ERROR("network", "Player {} {} sent a message which illegally contained a hyperlink:\n%.*s",
            GetPlayer()->GetName(), GetPlayer()->GetGUID().ToString(), STRING_VIEW_FMT_ARG(str));
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask)
{
  ...
  do
  {
    ...
    {
      LOG_ERROR("network", "Table `{}` have invalid account data type ({}), ignore.",
    ...
    if ((mask & (1 << type)) == 0)
    {
      LOG_ERROR("network", "Table `{}` have non appropriate for table  account data type ({}), ignore.",
                mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
      continue;
    }
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      LOG_DEBUG("entities.unit",                                                                                       \
                "WorldSession::ReadMovementInfo: Violation of MovementFlags found ({}). "                              \
                "MovementFlags: {}, MovementFlags2: {} for player {}. Mask {} will be removed.",                       \
                STRINGIZE(check), mi->GetMovementFlags(), mi->GetExtraMovementFlags(),                                 \
                          GetPlayer()->GetGUID().ToString(), maskToRemove);                                            \
...
#define REMOVE_VIOLATING_FLAGS(check, maskToRemove)                                                                    \
  ...
  */
  ...
  REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_WATERWALKING) && !GetPlayer()->HasWaterWalkAura() &&
  ...
  */
  ...
  REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_FLYING | MOVEMENTFLAG_CAN_FLY) &&
                             GetSecurity() == SEC_PLAYER && !GetPlayer()->m_mover->HasFlyAura() &&
                             !GetPlayer()->m_mover->HasIncreaseMountedFlightSpeedAura(),
...
void WorldSession::ReadAddonsInfo(ByteBuffer& data)
{
  ...
  if (uncompress(addonInfo.contents(), &uSize, data.contents() + pos, data.size() - pos) == Z_OK)
  {
    ...
    for (uint32 i = 0; i < addonsCount; ++i)
    {
      ...
      LOG_DEBUG("network", "ADDON: Name: {}, Enabled: 0x{:x}, CRC: 0x{:x}, Unknown2: 0x{:x}", addonName, enabled, crc,
                unk1);
      ...
      if (savedAddon)
      {
        ...
        if (!match)
          LOG_DEBUG("network", "ADDON: {} was known, but didn't match known CRC (0x{:x})!", addon.Name,
                    savedAddon->CRC);
        else
    ...
    if (crcpub)
    {
      ...
      if (usepk) // if CRC is wrong, add public key (client need it)
      {
        LOG_DEBUG("network",
                  ...
                  itr->CRC, itr->Name, STANDARD_ADDON_CRC);
...
void WorldSession::InitializeSession()
{
  ...
  AddQueryHolderCallback(CharacterDatabase.DelayQueryHolder(realmHolder))
      .AfterComplete(
          [this, cacheVersion](SQLQueryHolderBase const& holder)
...
{
  LoadAccountData(realmHolder.GetPreparedResult(AccountInfoQueryHolderPerRealm::GLOBAL_ACCOUNT_DATA),
                  GLOBAL_CACHE_MASK);
