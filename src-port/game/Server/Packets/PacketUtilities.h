...
template <std::size_t MaxBytesWithoutNullTerminator> struct ByteSize
{
 ...
 */
template <std::size_t MaxBytesWithoutNullTerminator, typename... Validators> class String
{
  using ValidatorList =
      std::conditional_t<!Acore::has_type<Strings::RawBytes, std::tuple<Validators...>>::value,
...
private:
  bool Validate() const { return ValidateNth(std::make_index_sequence<std::tuple_size_v<ValidatorList>>{}); }

  template <std::size_t... indexes> bool ValidateNth(std::index_sequence<indexes...>) const
  {
 ...
 * packets
 */
template <typename T, std::size_t N = 1000 /*select a sane default limit*/> class Array
{
...
public:
  ...
  CompactArray(CompactArray&& right) : _mask(right._mask), _contents(std::move(right._contents)) { right._mask = 0; }
...
template <typename T> ByteBuffer& operator<<(ByteBuffer& data, CompactArray<T> const& v)
{
...
template <typename T> ByteBuffer& operator>>(ByteBuffer& data, CompactArray<T>& v)
{
  uint32 mask;
  data >> mask;
