...
enum GuildEvents
{
  ...
  GE_SIGNED_ON                 = 12,
  GE_SIGNED_OFF                = 13,
  GE_GUILDBANKBAGSLOTS_CHANGED = 14, /// @todo: Sent when items are moved in gbank - all players with bank
                                     /// open will send tab query
...
public: // pussywizard: public class Member
  ...
  public:
    Member(uint32 guildId, ObjectGuid guid, uint8 rankId)
        : m_guildId(guildId), m_guid(guid), m_zoneId(0), m_level(0), m_class(0), m_flags(GUILDMEMBER_STATUS_NONE),
          m_accountId(0), m_rankId(rankId), receiveGuildBankUpdatePackets(false)
    {
    }
...
private:
  ...
  {
  public:
    EventLogEntry(uint32 guildId, ObjectGuid::LowType guid, GuildEventLogTypes eventType, ObjectGuid playerGuid1,
                  ObjectGuid playerGuid2, uint8 newRank)
        : LogEntry(guildId, guid), m_eventType(eventType), m_playerGuid1(playerGuid1), m_playerGuid2(playerGuid2),
          m_newRank(newRank)
    {
    }

    EventLogEntry(uint32 guildId, ObjectGuid::LowType guid, time_t timestamp, GuildEventLogTypes eventType,
                  ObjectGuid playerGuid1, ObjectGuid playerGuid2, uint8 newRank)
        : LogEntry(guildId, guid, timestamp), m_eventType(eventType), m_playerGuid1(playerGuid1),
          m_playerGuid2(playerGuid2), m_newRank(newRank)
    {
    }
  ...
  public:
    ...
    BankEventLogEntry(uint32 guildId, ObjectGuid::LowType guid, GuildBankEventLogTypes eventType, uint8 tabId,
                      ObjectGuid playerGuid, uint32 itemOrMoney, uint16 itemStackCount, uint8 destTabId)
    ...
    BankEventLogEntry(uint32 guildId, ObjectGuid::LowType guid, time_t timestamp, uint8 tabId,
                      GuildBankEventLogTypes eventType, ObjectGuid playerGuid, uint32 itemOrMoney,
  ...
  public:
    ...
    template <typename... Ts> void LoadEvent(Ts&&... args);
    // Adds new event to collection and saves it to DB
    template <typename... Ts> void AddEvent(CharacterDatabaseTransaction trans, Ts&&... args);
    uint32                         GetNextGUID();
  ...
  public:
    MoveItemData(Guild* guild, Player* player, uint8 container, uint8 slotId)
        : m_pGuild(guild), m_pPlayer(player), m_container(container), m_slotId(slotId), m_pItem(nullptr),
    ...
    // Checks splited amount against item. Splited amount cannot be more that
  ...
  private:
    Item* _StoreItem(CharacterDatabaseTransaction trans, BankTab* pTab, Item* pItem, ItemPosCount& pos,
                     bool clone) const;
...
public:
  static void SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode,
  ...
  void HandleSetRankInfo(WorldSession* session, uint8 rankId, std::string_view name, uint32 rights, uint32 moneyPerDay,
  ...
  // Broadcasts
  void BroadcastToGuild(WorldSession* session, bool officerOnly, std::string_view msg,
  ...
  void SwapItemsWithInventory(Player* player, bool toChar, uint8 tabId, uint8 slotId, uint8 playerBag,
  ...
  [[nodiscard]] bool ModifyBankMoney(CharacterDatabaseTransaction trans, const uint64& amount, bool add)
  {
...
private:
  ...
  void _LogEvent(GuildEventLogTypes eventType, ObjectGuid playerGuid1, ObjectGuid playerGuid2 = ObjectGuid::Empty,
                 uint8 newRank = 0);
  void _LogBankEvent(CharacterDatabaseTransaction trans, GuildBankEventLogTypes eventType, uint8 tabId,
                     ObjectGuid playerGuid, uint32 itemOrMoney, uint16 itemStackCount = 0, uint8 destTabId = 0);
  ...
  void _SendBankList(WorldSession* session = nullptr, uint8 tabId = 0, bool sendFullSlots = false,
                     SlotIds* slots = nullptr) const;

  void _BroadcastEvent(GuildEvents guildEvent, ObjectGuid guid = ObjectGuid::Empty,
                       Optional<std::string_view> param1 = {}, Optional<std::string_view> param2 = {},
