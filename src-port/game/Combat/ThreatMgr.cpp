...
float ThreatCalcHelper::calcThreat(Unit* hatedUnit, float threat, SpellSchoolMask schoolMask,
...
{
  // Xinef: Do not allow to modify threat by percent if threat is negative (forced to big value < 0 by spells adding
  // temporary threat) Xinef: When the temporary effect ends, temporary threat is added back which results in huge
  // additional amount of threat
  if (iThreat <= 0)
    return;
...
void HostileReference::updateOnlineStatus()
{
  ...
  if (isValid() && (!getTarget()->IsPlayer() || !getTarget()->ToPlayer()->IsGameMaster()) &&
      !getTarget()->IsInFlight() && getTarget()->IsInMap(GetSourceUnit()) && getTarget()->InSamePhase(GetSourceUnit()))
  {
...
HostileReference* ThreatContainer::SelectNextVictim(Creature* attacker, HostileReference* currentVictim) const
{
  ...
  // pussywizard: currentVictim is needed to compare if threat was exceeded by 10%/30% for melee/range targets (only
  // then switching current target)
  if (currentVictim)
  {
    Unit* cvUnit = currentVictim->getTarget();
    if (!attacker->CanCreatureAttack(cvUnit)) // pussywizard: if currentVictim is not valid => don't compare the threat
                                              // with it, just take the highest threat valid target
      currentVictim = nullptr;
    else if (cvUnit->IsImmunedToDamageOrSchool(attacker->GetMeleeDamageSchoolMask()) ||
             cvUnit->HasNegativeAuraWithInterruptFlag(
                 AURA_INTERRUPT_FLAG_TAKE_DAMAGE)) // pussywizard: no 10%/30% if currentVictim is immune to damage or
                                                   // has auras breakable by damage
      currentVictim = nullptr;
  }
  ...
  for (ThreatContainer::StorageType::const_iterator iter = iThreatList.begin(); iter != iThreatList.end() && !found;)
  {
    ...
    // pussywizard: don't go to threat comparison if this ref is immune to damage or has aura breakable on damage
    // (second choice target) pussywizard: if this is the last entry on the threat list, then all targets are second
    // choice, set bool to true and loop threat list again, ignoring this section
    ...
    if (attacker->CanCreatureAttack(target))
    {
      ...
      {
        if (currentVictim == currentRef) // pussywizard: nothing found previously was good and enough, currentRef passed
                                         // all necessary tests, so end now
        {
          found = true;
          break;
        }

        // pussywizard: implement 110% threat rule for targets in melee range and 130% rule for targets in ranged
        // distances
        ...
        else // pussywizard: nothing found previously was good and enough, this and next entries on the list have less
             // than 110% threat, and currentVictim is present and valid as checked before the loop (otherwise it's
             // nullptr), so end now
        {
