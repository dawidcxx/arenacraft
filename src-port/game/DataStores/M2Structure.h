...
struct M2Header
{
  ...
  uint32 GlobalModelFlags; // 0x0001: tilt x, 0x0002: tilt y, 0x0008: add 2
                           // fields in header, 0x0020: load .phys data (MoP+),
                           // 0x0080: has _lod .skin files (MoP?+), 0x0100: is
                           // camera related.
  ...
  uint32 ofsRibbonEmitters; // Things swirling around. See the CoT-entrance for
                            // light-trails.
  uint32 nParticleEmitters;
  uint32 ofsParticleEmitters; // Spells and weapons, doodads and loginscreens
                              // use them. Blood dripping of a blade? Particles.
  uint32 nBlendMaps;          // This has to deal with blending. Exists IFF (flags & 0x8)
