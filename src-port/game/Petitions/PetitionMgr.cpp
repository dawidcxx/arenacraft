...
void PetitionMgr::LoadPetitions()
{
  ...
  do
  {
    ...
    AddPetition(ObjectGuid::Create<HighGuid::Item>(fields[1].Get<uint32>()),
                ObjectGuid::Create<HighGuid::Player>(fields[0].Get<uint32>()), fields[2].Get<std::string>(),
                fields[3].Get<uint8>());
    ++count;
...
void PetitionMgr::LoadSignatures()
{
  ...
  {
    Field* fields = result->Fetch();
    AddSignature(ObjectGuid::Create<HighGuid::Item>(fields[0].Get<uint32>()), fields[2].Get<uint32>(),
