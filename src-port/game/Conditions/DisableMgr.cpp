...
bool IsDisabledFor(DisableType type, uint32 entry, Unit const* unit, uint8 flags)
{
  if (type > MAX_DISABLE_TYPES)
  {
    LOG_ERROR("server", "Disables::IsDisabledFor() called with unknown disable type {}!  (entry {}, flags {}).", type,
              entry, flags);
    return false;
  }
...
bool IsVMAPDisabledFor(uint32 entry, uint8 flags) { return IsDisabledFor(DISABLE_TYPE_VMAP, entry, nullptr, flags); }
