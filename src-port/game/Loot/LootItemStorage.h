...
struct StoredLootItem
{
  StoredLootItem(uint32 i, uint32 idx, uint32 c, int32 ri, uint32 rs, bool follow_loot_rules, bool freeforall,
      ...
      : itemid(i), itemIndex(idx), count(c), randomPropertyId(ri), randomSuffix(rs),
        follow_loot_rules(follow_loot_rules), freeforall(freeforall), is_blocked(is_blocked), is_counted(is_counted),
        is_underthreshold(is_underthreshold), needs_quest(needs_quest), conditionLootId(conditionLootId)
  {
  }
