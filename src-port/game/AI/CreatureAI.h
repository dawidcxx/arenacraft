...
protected:
  ...
  Creature* DoSummon(uint32 entry, Position const& pos, uint32 despawnTime = 30000,
  ...
  Creature* DoSummon(uint32 entry, WorldObject* obj, float radius = 5.0f, uint32 despawnTime = 30000,
                     TempSummonType summonType = TEMPSUMMON_CORPSE_TIMED_DESPAWN);
  Creature* DoSummonFlyer(uint32 entry, WorldObject* obj, float flightZ, float radius = 5.0f,
                          uint32 despawnTime = 30000, TempSummonType summonType = TEMPSUMMON_CORPSE_TIMED_DESPAWN);
...
public:
  ...
  enum EvadeReason
  {
    ...
    EVADE_REASON_SEQUENCE_BREAK, // this is a boss and the pre-requisite
                                 // encounters for engaging it are not defeated
                                 // yet
  ...
  explicit CreatureAI(Creature* creature)
      : UnitAI(creature), me(creature), _boundary(nullptr), _negateBoundary(false), m_MoveInLineOfSight_locked(false)
  {
  }
  ...
  // Called when the creature is target of hostile action: swing, hostile spell
  // landed, fear/etc)
