...
#define AC_SAI_IS_BOOLEAN_VALID(e, value)                                                                              \
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      LOG_ERROR("sql.sql",                                                                                             \
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses param {} of type Boolean with value {}, "  \
...
void SmartWaypointMgr::LoadFromDB()
{
  ...
  do
  {
    ...
    if (last_id != id)
      LOG_ERROR("sql.sql", "SmartWaypointMgr::LoadFromDB: Path entry {}, unexpected point id {}, expected {}.", entry,
                id, last_id);
  ...
  LOG_INFO("server.loading", ">> Loaded {} SmartAI waypoint paths (total {} waypoints) in {} ms", count, total,
...
void SmartAIMgr::LoadSmartAIFromDB()
{
  ...
  do
  {
    ...
    {
      LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: invalid source_type ({}), skipped loading.",
                uint32(source_type));
      continue;
    }
    if (temp.entryOrGuid >= 0)
    {
      ...
      case SMART_SCRIPT_TYPE_CREATURE:
      {
        ...
        {
          LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) does not exist, skipped loading.",
      ...
      case SMART_SCRIPT_TYPE_GAMEOBJECT:
      {
        ...
        {
          LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) does not exist, skipped loading.",
                    uint32(temp.entryOrGuid));
          continue;
        }
        break;
      ...
      case SMART_SCRIPT_TYPE_AREATRIGGER:
      {
        ...
        {
          LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry ({}) does not exist, skipped loading.",
                    uint32(temp.entryOrGuid));
          continue;
        }
        break;
    ...
    else
    {
      ...
      {
        LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature guid ({}) does not exist, skipped loading.",
    ...
    if (temp.action.type == SMART_ACTION_MOVE_TO_POS)
      if (temp.target.type == SMART_TARGET_SELF &&
          (std::fabs(temp.target.x) > 200.0f || std::fabs(temp.target.y) > 200.0f || std::fabs(temp.target.z) > 200.0f))
...
void SmartAIMgr::CheckIfSmartAIInDatabaseExists()
{
  ...
  for (auto const& [entry, creatureTemplate] : *sObjectMgr->GetCreatureTemplates())
  {
    ...
    if (!found)
      LOG_ERROR("sql.sql", "Creature entry ({}) has SmartAI enabled but no SmartAI entries in the database.",
  ...
  for (auto const& pair : sObjectMgr->GetAllAreaTriggerScriptData())
  {
    ...
    if (mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_AREATRIGGER)].find(pair.first) ==
      ...
      LOG_ERROR("sql.sql", "AreaTrigger entry ({}) has SmartTrigger enabled but no SmartAI entries in the database.",
                pair.first);
  }
}
...
bool SmartAIMgr::IsTargetValid(SmartScriptHolder const& e)
{
  ...
  case SMART_TARGET_CREATURE_RANGE:
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Creature entry {} as "
                "target_param1, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.target.unitDistance.creature);
      return false;
    }
    break;
  }
  ...
  case SMART_TARGET_GAMEOBJECT_RANGE:
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent GameObject entry {} as "
                "target_param1, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.target.goDistance.entry);
      return false;
    }
    break;
  }
  ...
  case SMART_TARGET_INVOKER_PARTY:
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invoker target, but event does not provide "
                "any invoker!",
  ...
  case SMART_TARGET_THREAT_LIST:
    break;
  default:
    LOG_ERROR("sql.sql", "SmartAIMgr: Not handled target_type({}), Entry {} SourceType {} Event {} Action {}, skipped.",
              e.GetTargetType(), e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
    return false;
  }
...
bool SmartAIMgr::CheckUnusedEventParams(SmartScriptHolder const& e)
{
  std::size_t paramsStructSize = [&]() -> std::size_t
  {
    ...
    default:
      LOG_WARN("sql.sql",
               "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} is using an event {} with no unused "
               "params specified in SmartAIMgr::CheckUnusedEventParams(), please report this.",
  ...
  for (std::size_t index = paramsCount; index < rawCount; index++)
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} has unused event_param{} with value "
...
bool SmartAIMgr::CheckUnusedActionParams(SmartScriptHolder const& e)
{
  std::size_t paramsStructSize = [&]() -> std::size_t
  {
    ...
    default:
      LOG_WARN("sql.sql",
               ...
               "params specified in SmartAIMgr::CheckUnusedActionParams(), please report this.",
               e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
      return sizeof(SmartAction::raw);
    }
  }();
  ...
  for (std::size_t index = paramsCount; index < rawCount; index++)
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} has unused action_param{} with value "
                "{}, it must be 0, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), index + 1, value);
      valid = false;
    }
...
bool SmartAIMgr::CheckUnusedTargetParams(SmartScriptHolder const& e)
{
  std::size_t paramsStructSize = [&]() -> std::size_t
  {
    ...
    case SMART_TARGET_INVOKER_PARTY:
      return sizeof(SmartTarget::invokerParty);
    case SMART_TARGET_PLAYER_RANGE:
    ...
    default:
      LOG_WARN("sql.sql",
               "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} is using a target {} with no unused "
               "params specified in SmartAIMgr::CheckUnusedTargetParams(), please report this.",
  ...
  for (std::size_t index = paramsCount; index < rawCount; index++)
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} has unused target_param{} with value "
                "{}, it must be 0, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), index + 1, value);
      valid = false;
    }
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
  ...
  {
    LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid event type ({}), skipped.",
  ...
  {
    LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid {}, event type {} can not be used for Script type {}", e.entryOrGuid,
              e.GetEventType(), e.GetScriptType());
    return false;
  }
  ...
  {
    LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has an invalid action type ({}), skipped.",
  ...
  case SMART_ACTION_RESPAWN_BY_SPAWNID:
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid {} using event({}) has an action type that is not yet supported on AzerothCore "
              "({}), skipped.",
              e.entryOrGuid, e.event_id, e.GetActionType());
    return false;
  default:
  ...
  {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid {} using event({}) has a target type that is not yet supported on AzerothCore "
              "({}), skipped.",
              e.entryOrGuid, e.event_id, e.GetTargetType());
    return false;
  }
  ...
  {
    LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid phase mask ({}), skipped.",
              e.entryOrGuid, e.event_id, e.event.event_phase_mask);
    return false;
  }
  if (e.event.event_flags > SMART_EVENT_FLAGS_ALL)
  {
    LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid {} using event({}) has invalid event flags ({}), skipped.",
  ...
  else
  {
      ...
      if (e.event.spellHit.spell)
      {
        ...
        if (!spellInfo)
        {
          LOG_ERROR("sql.sql",
                    ...
                    e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.spellHit.spell);
          return false;
        }
        if (e.event.spellHit.school && (e.event.spellHit.school & spellInfo->SchoolMask) != spellInfo->SchoolMask)
        {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses Spell entry {} with invalid school "
                    "mask, skipped.",
                    e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.spellHit.spell);
          return false;
        }
    ...
    case SMART_EVENT_IC_LOS:
      ...
      {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses hostilityMode with invalid value {} (max "
                  "allowed value {}), skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.los.hostilityMode,
    ...
    case SMART_EVENT_RESPAWN:
      if (e.event.respawn.type == SMART_SCRIPT_RESPAWN_CONDITION_MAP && !sMapStore.LookupEntry(e.event.respawn.map))
      {
        LOG_ERROR("sql.sql",
                  ...
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.respawn.map);
        return false;
      }
          ...
          !sAreaTableStore.LookupEntry(e.event.respawn.area))
      {
        LOG_ERROR("sql.sql",
                  ...
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.respawn.area);
        return false;
      }
      break;
    case SMART_EVENT_FRIENDLY_HEALTH:
    ...
    case SMART_EVENT_MOVEMENTINFORM:
    {
      if (e.event.movementInform.type > NULL_MOTION_TYPE)
      {
        LOG_ERROR("sql.sql",
                  ...
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.movementInform.type);
        return false;
      }
      break;
    }
    ...
    case SMART_EVENT_TEXT_OVER:
      // if (e.event.textOver.textGroupID && !IsTextValid(e, e.event.textOver.textGroupID)) return false;// 0 is a valid
      // text group!
      break;
    case SMART_EVENT_LINK:
    {
    ...
    case SMART_EVENT_EVENT_PHASE_CHANGE:
    {
      ...
      if (e.event.eventPhaseChange.phasemask > SMART_EVENT_PHASE_ALL)
      {
        LOG_ERROR("sql.sql",
                  ...
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.eventPhaseChange.phasemask);
        return false;
      }
      ...
      {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses event phasemask {} and incompatible "
                  "event_param1 {}, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.event_phase_mask,
    ...
    case SMART_EVENT_ACTION_DONE:
    {
      ...
      {
        LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid event id {}, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.doAction.eventId);
        return false;
      }
      break;
    }
    ...
    case SMART_EVENT_DISTANCE_CREATURE:
      ...
      {
        LOG_ERROR("sql.sql", "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using invalid creature guid {}, skipped.",
    ...
    case SMART_EVENT_COUNTER_SET:
      ...
      {
        LOG_ERROR("sql.sql", "SmartAIMgr: Event SMART_EVENT_COUNTER_SET using invalid counter id {}, skipped.",
                  e.event.counter.id);
        return false;
      }
      ...
      {
        LOG_ERROR("sql.sql", "SmartAIMgr: Event SMART_EVENT_COUNTER_SET using invalid value {}, skipped.",
    ...
    default:
      LOG_ERROR("sql.sql",
                ...
                e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
      return false;
    }
  }
  ...
  case SMART_ACTION_SET_FACTION:
    if (e.action.faction.factionID && !sFactionTemplateStore.LookupEntry(e.action.faction.factionID))
    {
      LOG_ERROR("sql.sql",
                ...
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.faction.factionID);
      return false;
    }
    break;
  case SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL:
    ...
    if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
    {
      if (e.action.morphOrMount.creature > 0 && !sObjectMgr->GetCreatureTemplate(e.action.morphOrMount.creature))
      {
        LOG_ERROR("sql.sql",
                  ...
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.morphOrMount.creature);
        return false;
      }
      ...
      if (e.action.morphOrMount.model)
      {
        ...
        {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has ModelID set with also set CreatureId, "
                    "skipped.",
                    e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
          return false;
        }
        else if (!sCreatureDisplayInfoStore.LookupEntry(e.action.morphOrMount.model))
        {
          LOG_ERROR("sql.sql",
                    ...
                    e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.morphOrMount.model);
          return false;
        }
      }
    }
  ...
  case SMART_ACTION_ACTIVATE_TAXI:
  {
    if (!sTaxiPathStore.LookupEntry(e.action.taxi.id))
    {
      LOG_ERROR("sql.sql",
                ...
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.taxi.id);
      return false;
    }
    break;
  }
  case SMART_ACTION_RANDOM_EMOTE:
  {
    if (std::all_of(e.action.randomEmote.emotes.begin(), e.action.randomEmote.emotes.end(),
  ...
  case SMART_ACTION_CALL_RANDOM_TIMED_ACTIONLIST:
  {
    if (std::all_of(e.action.randTimedActionList.actionLists.begin(), e.action.randTimedActionList.actionLists.end(),
  ...
  case SMART_ACTION_START_CLOSEST_WAYPOINT:
  {
    if (std::all_of(e.action.closestWaypointFromList.wps.begin(), e.action.closestWaypointFromList.wps.end(),
  ...
  case SMART_ACTION_INVOKER_CAST:
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invoker cast action, but event does not "
                "provide any invoker!",
                e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
      return false;
    }
  ...
  case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
    if (Quest const* qid = sObjectMgr->GetQuestTemplate(e.action.quest.quest))
    {
      ...
      {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry {} SourceType {} Event {} Action {} SpecialFlags for Quest entry {} does not "
                  "include FLAGS_EXPLORATION_OR_EVENT(2), skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.quest.quest);
        return false;
      }
    }
    else
  ...
  case SMART_ACTION_SET_EVENT_PHASE:
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to set phase {}. Phase mask cannot be "
                "used past phase {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.setEventPhase.phase,
  ...
  case SMART_ACTION_INC_EVENT_PHASE:
    ...
    {
      LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} is incrementing phase by 0, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to increment phase by too large value, "
                "skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
      return false;
    }
  ...
  case SMART_ACTION_RANDOM_PHASE:
  {
    if (std::all_of(e.action.randomPhase.phases.begin(), e.action.randomPhase.phases.end(),
    ...
    if (std::any_of(e.action.randomPhase.phases.begin(), e.action.randomPhase.phases.end(),
  ...
  case SMART_ACTION_SUMMON_CREATURE:
        ...
        e.action.summonCreature.type > TEMPSUMMON_MANUAL_DESPAWN)
    {
      LOG_ERROR("sql.sql",
                ...
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.summonCreature.type);
      return false;
    }
  ...
  case SMART_ACTION_SET_SHEATH:
    if (e.action.setSheath.sheath && e.action.setSheath.sheath >= MAX_SHEATH_STATE)
    {
      LOG_ERROR("sql.sql",
                ...
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.setSheath.sheath);
      return false;
    }
    break;
  case SMART_ACTION_SET_REACT_STATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SmartAIMgr: Creature {} Event {} Action {} uses invalid React State {}, skipped.",
                e.entryOrGuid, e.event_id, e.GetActionType(), e.action.react.state);
      return false;
    }
    break;
  }
  ...
  case SMART_ACTION_TELEPORT:
    if (!sMapStore.LookupEntry(e.action.teleport.mapID))
    {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Map entry {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.teleport.mapID);
      return false;
    }
    break;
  case SMART_ACTION_INSTALL_AI_TEMPLATE:
    ...
    {
      LOG_ERROR("sql.sql", "SmartAIMgr: Creature {} Event {} Action {} uses non-existent AI template id {}, skipped.",
                e.entryOrGuid, e.event_id, e.GetActionType(), e.action.installTtemplate.id);
      return false;
    }
    break;
  case SMART_ACTION_WP_STOP:
  ...
  case SMART_ACTION_WP_START:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SmartAIMgr: Creature {} Event {} Action {} uses non-existent WaypointPath id {}, skipped.",
                e.entryOrGuid, e.event_id, e.GetActionType(), e.action.wpStart.pathID);
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql", "SmartAIMgr: Creature {} Event {} Action {} uses invalid React State {}, skipped.",
                e.entryOrGuid, e.event_id, e.GetActionType(), e.action.wpStart.reactState);
      return false;
    }
  ...
  case SMART_ACTION_REMOVE_POWER:
    if (e.action.power.powerType > MAX_POWERS)
    {
      LOG_ERROR("scripts.ai.sai",
                ...
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.power.powerType);
      return false;
    }
    break;
  case SMART_ACTION_GAME_EVENT_STOP:
  {
    ...
    if (eventId < 1 || eventId >= events.size())
    {
      LOG_ERROR("scripts.ai.sai",
                ...
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.gameEventStop.id);
      return false;
    }
  ...
  case SMART_ACTION_GAME_EVENT_START:
  {
    ...
    if (eventId < 1 || eventId >= events.size())
    {
      LOG_ERROR("scripts.ai.sai",
                "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent event, eventId {}, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.gameEventStart.id);
      return false;
    }
  ...
  case SMART_ACTION_LOAD_GRID:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai", "SmartScript: SMART_ACTION_LOAD_GRID uses invalid map coords: {}, skipped.",
                e.entryOrGuid);
      return false;
    }
    break;
  ...
  case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SmartScript: action_type {} is not allowed with source_type {}. Entry {}, skipped.",
  ...
  case SMART_ACTION_SET_INST_DATA:
  {
    if (e.action.setInstanceData.type == 1)
    {
      ...
      {
        LOG_ERROR("sql.sql",
                  "SmartScript: SMART_ACTION_SET_INST_DATA with type 1 (bossState) uses invalid encounter state {}. "
                  "Source entry {}, type {}",
                  e.action.setInstanceData.data, e.entryOrGuid, e.GetScriptType());
        return false;
      }
    }
    else if (e.action.setInstanceData.type > 1)
    {
      LOG_ERROR("sql.sql",
                ...
                e.action.setInstanceData.type, e.entryOrGuid, e.GetScriptType());
      return false;
    }
    break;
  }
  ...
  case SMART_ACTION_SET_MOVEMENT_SPEED:
  {
    if (e.action.movementSpeed.movementType >= MAX_MOVE_TYPE)
    {
      LOG_ERROR("sql.sql",
                ...
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.movementSpeed.movementType);
      return false;
    }

    if (!e.action.movementSpeed.speedInteger && !e.action.movementSpeed.speedFraction)
    {
      LOG_ERROR("sql.sql", "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses speed 0, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
      return false;
    }
    break;
  ...
  default:
    LOG_ERROR("sql.sql",
              ...
              e.GetActionType(), e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id);
    return false;
  }
...
bool SmartAIMgr::IsTextValid(SmartScriptHolder const& e, uint32 id)
{
      ...
      if (e.entryOrGuid < 0)
      {
        ...
        if (!data)
        {
          LOG_ERROR(
              ...
              e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), guid);
          return false;
        }
        else
          entry = data->id1;
  ...
  if (!entry || !sCreatureTextMgr->TextExist(entry, uint8(id)))
  {
    LOG_ERROR("sql.sql",
              ...
              e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), id);
    return false;
  }
