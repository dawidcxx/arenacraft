...
npc_escortAI::npc_escortAI(Creature* creature)
    : ScriptedAI(creature), m_uiWPWaitTimer(1000), m_uiPlayerCheckTimer(0), m_uiEscortState(STATE_ESCORT_NONE),
      MaxPlayerDistance(DEFAULT_MAX_PLAYER_DISTANCE), m_pQuestForEscort(nullptr), m_bIsActiveAttacker(true),
      m_bIsRunning(false), m_bCanInstantRespawn(false), m_bCanReturnToStart(false), DespawnAtEnd(true),
      DespawnAtFar(true), ScriptWP(false), HasImmuneToNPCFlags(false)
{
}
      ...
      !HasEscortState(STATE_ESCORT_RETURNING))
  {
    if (m_uiWPWaitTimer <= diff)
    {
      if (CurrentWP == WaypointList.end())
      {
        ...
        // xinef: remove escort state, escort was finished (lack of this line resulted in skipping UpdateEscortAI calls
        // after finished escort)
                         ...
                         bool resetWaypoints /* = true */)
{
  if (me->GetVictim())
  {
    LOG_ERROR("entities.unit.ai", "ERROR: EscortAI (script: {}, creature entry: {}) attempts to Start while in combat",
              me->GetScriptName(), me->GetEntry());
    return;
  }

  if (HasEscortState(STATE_ESCORT_ESCORTING))
  {
    LOG_ERROR("entities.unit.ai", "EscortAI (script: {}, creature entry: {}) attempts to Start while already escorting",
              me->GetScriptName(), me->GetEntry());
    return;
  }
  ...
  {
    LOG_ERROR("sql.sql",
              "EscortAI (script: {}, creature entry: {}) starts with 0 waypoints (possible missing entry in "
              "script_waypoint. Quest: {}).",
  ...
  if (m_bCanReturnToStart && m_bCanInstantRespawn)
    LOG_DEBUG("scripts.ai", "EscortAI is set to return home after waypoint end and instant respawn at waypoint end. "
                            "Creature will never despawn.");
