...
public:
  ...
  template <typename T> void DespawnIf(T const& predicate) { storage_.remove_if(predicate); }

  void DoAction(int32 info, uint16 max = 0)
  {
    if (max)
      RemoveNotExisting(); // pussywizard: when max is set, non existing can be
                           // chosen and nothing will happen
  ...
  template <class Predicate> void DoAction(int32 info, Predicate&& predicate, uint16 max = 0)
  {
    ...
    // We need to use a copy of SummonList here, otherwise original SummonList
    // would be modified
    StorageType listCopy = storage_;
...
struct ScriptedAI : public CreatureAI
{
  ...
  // Called when AI is temporarily replaced or put back when possess is applied
  // or removed
  ...
  {
    DoTeleportPlayer(unit, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation());
  };
  ...
  Creature* DoSpawnCreature(uint32 entry, float offsetX, float offsetY, float offsetZ, float angle, uint32 type,
                            uint32 despawntime);
  ...
  void ScheduleTimedEvent(Milliseconds timerMin, Milliseconds timerMax, std::function<void()> exec,
                          Milliseconds repeatMin, Milliseconds repeatMax = 0s, uint32 uniqueId = 0);
  void ScheduleTimedEvent(Milliseconds timerMax, std::function<void()> exec, Milliseconds repeatMin,
                          Milliseconds repeatMax = 0s, uint32 uniqueId = 0)
  {
...
{
  HealthCheckEventData(uint8 healthPct, std::function<void()> exec, bool valid = true)
      : _healthPct(healthPct), _exec(exec), _valid(valid) {};
...
void        GetCreatureListWithEntryInGrid(std::list<Creature*>& list, WorldObject* source, uint32 entry,
                                           float maxSearchRange);
void        GetGameObjectListWithEntryInGrid(std::list<GameObject*>& list, WorldObject* source, uint32 entry,
                                             float maxSearchRange);
void        GetDeadCreatureListInGrid(std::list<Creature*>& list, WorldObject* source, float maxSearchRange,
                                      bool alive = false);
