...
void PetAI::UpdateAI(uint32 diff)
{
            ...
            !me->HasUnitState(UNIT_STATE_CASTING)))
  {
    if (me->HasReactState(REACT_AGGRESSIVE) || me->GetCharmInfo()->IsAtStay())
    {
      ...
      // All other cases (ie: defensive) - Targets are assigned by AttackedBy(), OwnerAttackedBy(), OwnerAttacked(),
      // etc.
  ...
  if (!me->HasUnitState(UNIT_STATE_CASTING))
  {
    ...
    {
      owner->ToPlayer()->GetSession()->HandlePetActionHelper(me, me->GetGUID(),
...
void PetAI::MovementInform(uint32 moveType, uint32 data)
{
  ...
  case FOLLOW_MOTION_TYPE:
  {
    ...
    if (me->GetCharmerOrOwner() && me->GetCharmInfo() && data == me->GetCharmerOrOwner()->GetGUID().GetCounter() &&
