...
void ScriptMgr::OnStartup() { CALL_ENABLED_HOOKS(WorldScript, WORLDHOOK_ON_STARTUP, script->OnStartup()); }

void ScriptMgr::OnShutdown() { CALL_ENABLED_HOOKS(WorldScript, WORLDHOOK_ON_SHUTDOWN, script->OnShutdown()); }
...
{
  CALL_ENABLED_HOOKS(WorldScript, WORLDHOOK_ON_BEFORE_FINALIZE_PLAYER_WORLD_SESSION,
...
WorldScript::WorldScript(const char* name, std::vector<uint16> enabledHooks) : ScriptObject(name, WORLDHOOK_END)
{
