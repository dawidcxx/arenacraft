...
{
  CALL_ENABLED_BOOLEAN_HOOKS(MiscScript, MISCHOOK_CAN_APPLY_SOULBOUND_FLAG,
                             !script->CanApplySoulboundFlag(item, proto));
}

bool ScriptMgr::CanItemApplyEquipSpell(Player* player, Item* item)
{
  CALL_ENABLED_BOOLEAN_HOOKS(MiscScript, MISCHOOK_CAN_ITEM_APPLY_EQUIP_SPELL,
...
{
  CALL_ENABLED_BOOLEAN_HOOKS(MiscScript, MISCHOOK_CAN_SEND_AUCTIONHELLO,
                             !script->CanSendAuctionHello(session, guid, creature));
}

void ScriptMgr::ValidateSpellAtCastSpell(Player* player, uint32& oldSpellId, uint32& spellId, uint8& castCount,
                                         uint8& castFlags)
{
  CALL_ENABLED_HOOKS(MiscScript, MISCHOOK_VALIDATE_SPELL_AT_CAST_SPELL,
                     script->ValidateSpellAtCastSpell(player, oldSpellId, spellId, castCount, castFlags));
}

void ScriptMgr::ValidateSpellAtCastSpellResult(Player* player, Unit* mover, Spell* spell, uint32 oldSpellId,
                                               uint32 spellId)
{
  CALL_ENABLED_HOOKS(MiscScript, MISCHOOK_VALIDATE_SPELL_AT_CAST_SPELL_RESULT,
...
{
  CALL_ENABLED_HOOKS(MiscScript, MISCHOOK_ON_AFTER_LOOT_TEMPLATE_PROCESS,
                     script->OnAfterLootTemplateProcess(loot, tab, store, lootOwner, personal, noEmptyError, lootMode));
}

void ScriptMgr::OnPlayerSetPhase(const AuraEffect* auraEff, AuraApplication const* aurApp, uint8 mode, bool apply,
                                 uint32& newPhase)
{
  CALL_ENABLED_HOOKS(MiscScript, MISCHOOK_ON_PLAYER_SET_PHASE,
...
MiscScript::MiscScript(const char* name, std::vector<uint16> enabledHooks) : ScriptObject(name, MISCHOOK_END)
{
