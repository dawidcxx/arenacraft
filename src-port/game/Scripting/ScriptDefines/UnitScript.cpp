...
{
  CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_MODIFY_PERIODIC_DAMAGE_AURAS_TICK,
...
{
  CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_MODIFY_SPELL_DAMAGE_TAKEN,
...
{
  CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_MODIFY_HEAL_RECEIVED,
...
void ScriptMgr::OnBeforeRollMeleeOutcomeAgainst(Unit const* attacker, Unit const* victim, WeaponAttackType attType,
                                                ...
                                                int32& dodge_chance, int32& parry_chance, int32& block_chance)
{
  CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_ON_BEFORE_ROLL_MELEE_OUTCOME_AGAINST,
                     script->OnBeforeRollMeleeOutcomeAgainst(attacker, victim, attType, attackerMaxSkillValueForLevel,
                                                             victimMaxSkillValueForLevel, attackerWeaponSkill,
                                                             victimDefenseSkill, crit_chance, miss_chance, dodge_chance,
...
bool ScriptMgr::IsNeedModSpellDamagePercent(Unit const* unit, AuraEffect* auraEff, float& doneTotalMod,
                                            SpellInfo const* spellProto)
{
  CALL_ENABLED_BOOLEAN_HOOKS(UnitScript, UNITHOOK_IS_NEEDMOD_SPELL_DAMAGE_PERCENT,
                             !script->IsNeedModSpellDamagePercent(unit, auraEff, doneTotalMod, spellProto));
}

bool ScriptMgr::IsNeedModMeleeDamagePercent(Unit const* unit, AuraEffect* auraEff, float& doneTotalMod,
                                            SpellInfo const* spellProto)
{
  CALL_ENABLED_BOOLEAN_HOOKS(UnitScript, UNITHOOK_IS_NEEDMOD_MELEE_DAMAGE_PERCENT,
                             !script->IsNeedModMeleeDamagePercent(unit, auraEff, doneTotalMod, spellProto));
}

bool ScriptMgr::IsNeedModHealPercent(Unit const* unit, AuraEffect* auraEff, float& doneTotalMod,
                                     SpellInfo const* spellProto)
{
  CALL_ENABLED_BOOLEAN_HOOKS(UnitScript, UNITHOOK_IS_NEEDMOD_HEAL_PERCENT,
...
{
  CALL_ENABLED_BOOLEAN_HOOKS(UnitScript, UNITHOOK_CAN_SET_PHASE_MASK,
                             !script->CanSetPhaseMask(unit, newPhaseMask, update));
}

bool ScriptMgr::IsCustomBuildValuesUpdate(Unit const* unit, uint8 updateType, ByteBuffer& fieldBuffer,
                                          Player const* target, uint16 index)
{
  CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
      UnitScript, UNITHOOK_IS_CUSTOM_BUILD_VALUES_UPDATE,
...
{
  CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(UnitScript, UNITHOOK_SHOULD_TRACK_VALUES_UPDATE_POS_BY_INDEX,
                                                script->ShouldTrackValuesUpdatePosByIndex(unit, updateType, index));
}

void ScriptMgr::OnPatchValuesUpdate(Unit const* unit, ByteBuffer& valuesUpdateBuf,
                                    BuildValuesCachePosPointers& posPointers, Player* target)
{
  CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_ON_PATCH_VALUES_UPDATE,
