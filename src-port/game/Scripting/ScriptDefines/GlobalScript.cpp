...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_MIRRORIMAGE_DISPLAY_ITEM,
...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_AFTER_REF_COUNT,
                     script->OnAfterRefCount(player, LootStoreItem, loot, canRate, lootMode, maxcount, store));
}

void ScriptMgr::OnAfterCalculateLootGroupAmount(Player const* player, Loot& loot, uint16 lootMode, uint32& groupAmount,
                                                LootStore const& store)
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_AFTER_CALCULATE_LOOT_GROUP_AMOUNT,
...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_BEFORE_DROP_ADD_ITEM,
                     script->OnBeforeDropAddItem(player, loot, canRate, lootMode, LootStoreItem, store));
}

bool ScriptMgr::OnItemRoll(Player const* player, LootStoreItem const* lootStoreItem, float& chance, Loot& loot,
                           LootStore const& store)
{
  CALL_ENABLED_BOOLEAN_HOOKS(GlobalScript, GLOBALHOOK_ON_ITEM_ROLL,
                             !script->OnItemRoll(player, lootStoreItem, chance, loot, store));
}

bool ScriptMgr::OnBeforeLootEqualChanced(Player const* player, LootStoreItemList equalChanced, Loot& loot,
                                         LootStore const& store)
{
  CALL_ENABLED_BOOLEAN_HOOKS(GlobalScript, GLOBALHOOK_ON_BEFORE_LOOT_EQUAL_CHANCED,
...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_INITIALIZE_LOCKED_DUNGEONS,
...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_AFTER_INITIALIZE_LOCKED_DUNGEONS,
...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_AFTER_UPDATE_ENCOUNTER_STATE,
                     script->OnAfterUpdateEncounterState(map, type, creditEntry, source, difficulty_fixed, encounters,
                                                         dungeonCompleted, updated));
}

void ScriptMgr::OnBeforeWorldObjectSetPhaseMask(WorldObject const* worldObject, uint32& oldPhaseMask,
                                                uint32& newPhaseMask, bool& useCombinedPhases, bool& update)
{
  CALL_ENABLED_HOOKS(
      GlobalScript, GLOBALHOOK_ON_BEFORE_WORLDOBJECT_SET_PHASEMASK,
      script->OnBeforeWorldObjectSetPhaseMask(worldObject, oldPhaseMask, newPhaseMask, useCombinedPhases, update));
}

bool ScriptMgr::OnIsAffectedBySpellModCheck(SpellInfo const* affectSpell, SpellInfo const* checkSpell,
                                            SpellModifier const* mod)
{
  CALL_ENABLED_BOOLEAN_HOOKS(GlobalScript, GLOBALHOOK_ON_IS_AFFECTED_BY_SPELL_MOD_CHECK,
                             !script->OnIsAffectedBySpellModCheck(affectSpell, checkSpell, mod));
}

bool ScriptMgr::OnSpellHealingBonusTakenNegativeModifiers(Unit const* target, Unit const* caster,
                                                          SpellInfo const* spellInfo, float& val)
{
  CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
      GlobalScript, GLOBALHOOK_ON_SPELL_HEALING_BONUS_TAKEN_NEGATIVE_MODIFIERS,
...
{
  CALL_ENABLED_BOOLEAN_HOOKS(GlobalScript, GLOBALHOOK_ON_ALLOWED_FOR_PLAYER_LOOT_CHECK,
...
{
  CALL_ENABLED_BOOLEAN_HOOKS(GlobalScript, GLOBALHOOK_ON_ALLOWED_TO_LOOT_CONTAINER_CHECK,
 ...
 * @details Careful checks for old- and newState are required, since it can fire multiple times and not only when combat
 * starts/ends.
 *
...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_ON_BEFORE_SET_BOSS_STATE,
...
{
  CALL_ENABLED_HOOKS(GlobalScript, GLOBALHOOK_AFTER_INSTANCE_GAME_OBJECT_CREATE,
                     script->AfterInstanceGameObjectCreate(instance, go));
}

GlobalScript::GlobalScript(const char* name, std::vector<uint16> enabledHooks) : ScriptObject(name, GLOBALHOOK_END)
{
