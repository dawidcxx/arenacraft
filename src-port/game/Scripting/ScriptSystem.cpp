...
void SystemMgr::LoadScriptWaypoints()
{
  ...
  do
  {
    ...
    if (!pCInfo)
    {
      LOG_ERROR("sql.sql", "DB table script_waypoint has waypoint for non-existant creature entry {}",
    ...
    if (!pCInfo->ScriptID)
      LOG_ERROR("sql.sql",
                "DB table script_waypoint has waypoint for creature entry {}, but creature does not have ScriptName "
                "defined and then useless.",
                temp.uiCreatureEntry);
