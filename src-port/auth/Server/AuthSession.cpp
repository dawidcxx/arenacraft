...
std::unordered_map<uint8, AuthHandler> AuthSession::InitHandlers()
{
  ...
  handlers[AUTH_LOGON_CHALLENGE]     = {STATUS_CHALLENGE, AUTH_LOGON_CHALLENGE_INITIAL_SIZE,
  ...
  handlers[AUTH_RECONNECT_CHALLENGE] = {STATUS_CHALLENGE, AUTH_LOGON_CHALLENGE_INITIAL_SIZE,
                                        &AuthSession::HandleReconnectChallenge};
  handlers[AUTH_RECONNECT_PROOF]     = {STATUS_RECONNECT_PROOF, sizeof(AUTH_RECONNECT_PROOF_C),
...
void AccountInfo::LoadResult(Field* fields)
{
  ...
  // FROM account a LEFT JOIN account_access aa ON a.id = aa.id LEFT JOIN account_banned ab ON ab.id = a.id AND
  // ab.active = 1 LEFT JOIN ip_banned ipb ON ipb.ip = ? WHERE a.username = ?
...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
  if (result)
  {
    ...
    if (banned)
    {
      ...
      SendPacket(pkt);
      LOG_DEBUG("session", "[AuthSession::CheckIpCallback] Banned ip '{}:{}' tries to login!",
                GetRemoteIpAddress().to_string(), GetRemotePort());
      return;
    }
  }
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result)
{
  ...
  {
    LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is locked to IP - '{}' is logging in from '{}'",
              _accountInfo.Login, _accountInfo.LastIP, ipAddress);
  ...
  else
  {
    ...
    {
      LOG_DEBUG("server.authserver", "[AuthChallenge] Account '{}' is locked to country: '{}' Player country is '{}'",
  ...
  if (_accountInfo.IsBanned)
  {
    if (_accountInfo.IsPermanentlyBanned)
    {
      pkt << uint8(WOW_FAIL_BANNED);
      SendPacket(pkt);
      LOG_INFO("server.authserver.banned", "'{}:{}' [AuthChallenge] Banned account {} tried to login!", ipAddress, port,
               _accountInfo.Login);
      return;
    }
    else
    {
      ...
      SendPacket(pkt);
      LOG_INFO("server.authserver.banned", "'{}:{}' [AuthChallenge] Temporarily banned account {} tried to login!",
  ...
  if (!fields[11].IsNull())
  {
    ...
    if (auto const& secret = sSecretMgr->GetSecret(SECRET_TOTP_MASTER_KEY))
    {
      ...
      if (!success)
      {
        ...
        LOG_ERROR("server.authserver", "[AuthChallenge] Account '{}' has invalid ciphertext for TOTP token key stored",
                  _accountInfo.Login);
        SendPacket(pkt);
        return;
      }
  ...
  _srp6.emplace(_accountInfo.Login, fields[12].Get<Binary, Acore::Crypto::SRP6::SALT_LENGTH>(),
  ...
  if (AuthHelper::IsAcceptedClientBuild(_build))
  {
    ...
    LOG_DEBUG("server.authserver", "'{}:{}' [AuthChallenge] account {} is using '{}' locale ({})", ipAddress, port,
              _accountInfo.Login, _localizationName, GetLocaleByName(_localizationName));
...
bool AuthSession::HandleLogonProof()
{
  ...
  if (Optional<SessionKey> K = _srp6->VerifyChallengeResponse(logonProof->A, logonProof->clientM))
  {
    ...
    if (sentToken && _totpSecret)
    {
      ...
      std::string token(
          reinterpret_cast<char*>(GetReadBuffer().GetReadPointer() + sizeof(sAuthLogonProof_C) + sizeof(size)), size);
    ...
    LOG_DEBUG("server.authserver", "'{}:{}' User '{}' successfully authenticated", GetRemoteIpAddress().to_string(),
              GetRemotePort(), _accountInfo.Login);

    // Update the sessionkey, last_ip, last login time and reset number of failed logins in the account table for this
    // account No SQL injection (escaped user name) and IP address as received by socket
      ...
      if (++_accountInfo.FailedLogins >= MaxWrongPassCount)
      {
        ...
        if (WrongPassBanType)
        {
          ...
          LOG_DEBUG("server.authserver",
                    "'{}:{}' [AuthChallenge] account {} got banned for '{}' seconds because it failed to authenticate "
                    "'{}' times",
                    GetRemoteIpAddress().to_string(), GetRemotePort(), _accountInfo.Login, WrongPassBanTime,
        ...
        else
        {
          ...
          LOG_DEBUG("server.authserver",
                    "'{}:{}' [AuthChallenge] IP got banned for '{}' seconds because account {} failed to authenticate "
                    "'{}' times",
                    GetRemoteIpAddress().to_string(), GetRemotePort(), WrongPassBanTime, _accountInfo.Login,
                    _accountInfo.FailedLogins);
        }
      }
    }
...
bool AuthSession::VerifyVersion(uint8 const* a, int32 aLength, Acore::Crypto::SHA1::Digest const& versionProof,
                                bool isReconnect)
{
