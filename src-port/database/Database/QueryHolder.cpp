...
bool SQLQueryHolderBase::SetPreparedQueryImpl(std::size_t index, PreparedStatementBase* stmt)
{
  ...
  {
    LOG_ERROR("sql.sql", "Query index ({}) out of range (size: {}) for prepared statement", uint32(index),
...
PreparedQueryResult SQLQueryHolderBase::GetPreparedResult(std::size_t index) const
{
  ...
  ASSERT(index < m_queries.size(),
         "Query holder result index out of range, tried to access index {} but there are only {} results", index,
         m_queries.size());
