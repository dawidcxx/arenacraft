...
bool ResultSet::NextRow()
{
  ...
  if (!lengths)
  {
    LOG_WARN("sql.sql", "{}:mysql_fetch_lengths, cannot retrieve value lengths. Error {}.", __FUNCTION__,
...
void ResultSet::AssertRows(std::size_t sizeRows) { ASSERT(sizeRows == _fieldCount); }
      ...
      m_metadataResult(result)
{
  ...
  {
    LOG_WARN("sql.sql", "{}:mysql_stmt_store_result, cannot bind result from MySQL server. Error: {}", __FUNCTION__,
  ...
  {
    LOG_WARN("sql.sql", "{}:mysql_stmt_bind_result, cannot bind result from MySQL server. Error: {}", __FUNCTION__,
             mysql_stmt_error(m_stmt));
    ...
    for (uint32 fIndex = 0; fIndex < m_fieldCount; ++fIndex)
    {
      ...
      if (!*m_rBind[fIndex].is_null)
      {
        ...
        case MYSQL_TYPE_VAR_STRING:
          ...
          // we cannot blindly null-terminate the data either as it may be retrieved as binary blob and not specifically
          // a string in this case using Field::GetCString will result in garbage
