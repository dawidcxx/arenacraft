...
MySQLPreparedStatement::MySQLPreparedStatement(MySQLStmt* stmt, std::string_view queryString)
    : m_stmt(nullptr), m_Mstmt(stmt), m_bind(nullptr), m_queryString(std::string(queryString))
{
...
#ifdef _DEBUG
  if (pos < m_paramCount)
    LOG_WARN("sql.sql", "[WARNING]: BindParameters() for statement {} did not bind all allocated parameters",
             stmt->GetIndex());
#endif
}
...
{
  LOG_ERROR(
      "sql.driver", "Attempted to bind parameter {}{} on a PreparedStatement {} (statement has only {} parameters)",
...
void MySQLPreparedStatement::AssertValidIndex(uint8 index)
{
  ...
  if (m_paramsSet[index])
    LOG_ERROR("sql.sql", "[ERROR] Prepared Statement (id: {}) trying to bind value on already bound index ({}).",
              m_stmt->GetIndex(), index);
}

template <typename T> void MySQLPreparedStatement::SetParameter(const uint8 index, T value)
{
...
void MySQLPreparedStatement::SetParameter(const uint8 index, bool value) { SetParameter(index, uint8(value ? 1 : 0)); }
...
std::string MySQLPreparedStatement::getQueryString() const
{
  ...
  for (PreparedStatementData const& data : m_stmt->GetParameters())
  {
    ...
    std::string replaceStr = std::visit([&](auto&& data) { return PreparedStatementData::ToString(data); }, data.data);
