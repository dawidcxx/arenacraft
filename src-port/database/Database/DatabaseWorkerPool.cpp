...
template <class T> DatabaseWorkerPool<T>::~DatabaseWorkerPool() { _queue->Cancel(); }
...
template <class T> QueryCallback DatabaseWorkerPool<T>::AsyncQuery(std::string_view sql)
{
  ...
  // Store future result before enqueueing - task might get already processed and deleted before returning from this
  // method
...
bool DatabaseIncompatibleVersion(std::string const mysqlVersion)
{
  ...
  return std::lexicographical_compare(parsedMySQLVersion.begin(), parsedMySQLVersion.end(), parsedMinVersion.begin(),
  ...
  for (uint8 i = 0; i < numConnections; ++i)
  {
    ...
    else if (DatabaseIncompatibleVersion(connection->GetServerInfo()))
    {
      LOG_ERROR("sql.driver",
                "AzerothCore does not support MySQL versions below 8.0\n\nFound server version: {}. Server compiled "
                "with: {}.",
...
template <class T> void DatabaseWorkerPool<T>::Enqueue(SQLOperation* op) { _queue->Push(op); }

template <class T> std::size_t DatabaseWorkerPool<T>::QueueSize() const { return _queue->Size(); }
