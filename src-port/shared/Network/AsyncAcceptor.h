...
public:
  ...
  AsyncAcceptor(Acore::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port)
      : _acceptor(ioContext), _endpoint(Acore::Net::make_address(bindIp), port), _socket(ioContext), _closed(false),
  ...
  bool Bind()
  {
    ...
    if (errorCode)
    {
      LOG_INFO("network", "Could not bind to {}:{} {}", _endpoint.address().to_string(), _endpoint.port(),
               errorCode.message());
      return false;
    }
    ...
    if (errorCode)
    {
      LOG_INFO("network", "Failed to start listening on {}:{} {}", _endpoint.address().to_string(), _endpoint.port(),
               errorCode.message());
      return false;
    }
  ...
  _acceptor.async_accept(_socket,
                         [this](boost::system::error_code error)
                         {
                           if (!error)
                           {
                             try
                             {
                               // this-> is required here to fix an segmentation fault in gcc 4.7.2 -
                               // reason is lambdas in a templated class
                           ...
                           // lets slap some more this-> on this so we can fix this bug with gcc 4.7.2
                           // throwing internals in yo face
                           if (!_closed)
                             this->AsyncAccept<T>();
                         });
}
