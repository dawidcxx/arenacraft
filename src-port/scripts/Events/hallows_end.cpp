...
class spell_hallows_end_candy_pirate_costume : public AuraScript
{
  ...
  void HandleEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
  {
    if (Unit* target = GetTarget())
    {
      target->CastSpell(
          target,
          target->getGender() == GENDER_MALE ? SPELL_HALLOWS_END_CANDY_3_MALE : SPELL_HALLOWS_END_CANDY_3_FEMALE, true);
    }
  }
  ...
  void Register() override
  {
    OnEffectApply += AuraEffectApplyFn(spell_hallows_end_candy_pirate_costume::HandleEffectApply, EFFECT_0,
                                       SPELL_AURA_MOD_INCREASE_SWIM_SPEED, AURA_EFFECT_HANDLE_REAL);
    OnEffectRemove += AuraEffectRemoveFn(spell_hallows_end_candy_pirate_costume::HandleEffectRemove, EFFECT_0,
                                         SPELL_AURA_MOD_INCREASE_SWIM_SPEED, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_hallows_end_base_fire : public AuraScript
{
  ...
  void Register() override
  {
    ...
    OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_hallows_end_base_fire::HandleEffectPeriodicUpdate,
                                                         EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    OnEffectApply += AuraEffectApplyFn(spell_hallows_end_base_fire::HandleEffectApply, EFFECT_0,
...
struct npc_hallows_end_soh : public ScriptedAI
{
  ...
  void EnterEvadeMode(EvadeReason /* why */) override { me->DespawnOrUnsummon(1); }
...
struct boss_headless_horseman : public ScriptedAI
{
  ...
  void JustDied(Unit* /*killer*/) override
  {
    ...
    if (!players.IsEmpty() && players.begin()->GetSource() && players.begin()->GetSource()->GetGroup())
      sLFGMgr->FinishDungeon(players.begin()->GetSource()->GetGroup()->GetGUID(), lfg::LFG_DUNGEON_HEADLESS_HORSEMAN,
                             me->FindMap());
...
struct boss_headless_horseman_pumpkin : public ScriptedAI
{
  ...
  void SpellHit(Unit* /*caster*/, SpellInfo const* spellInfo) override
  {
    ...
    {
      if (Creature* cr = me->SummonCreature(NPC_PUMPKIN_FIEND, me->GetPositionX(), me->GetPositionY(),
...
public:
  ...
  bool OnQuestReward(Player* player, GameObject* go, Quest const* /*quest*/, uint32 /*opt*/) override
  {
    ...
    if (Creature* horseman = go->SummonCreature(NPC_HEADLESS_HORSEMAN_MOUNTED, 1754.00f, 1346.00f, 17.50f, 0.0f,
  ...
  GameObjectAI* GetAI(GameObject* go) const override { return new go_loosely_turned_soilAI(go); }
};
...
void AddSC_event_hallows_end_scripts()
{
  ...
  RegisterSpellScriptWithArgs(spell_hallows_end_put_costume, "spell_hallows_end_pirate_costume",
                              SPELL_PIRATE_COSTUME_MALE, SPELL_PIRATE_COSTUME_FEMALE);
  RegisterSpellScriptWithArgs(spell_hallows_end_put_costume, "spell_hallows_end_leper_costume",
                              SPELL_LEPER_GNOME_COSTUME_MALE, SPELL_LEPER_GNOME_COSTUME_FEMALE);
  RegisterSpellScriptWithArgs(spell_hallows_end_put_costume, "spell_hallows_end_ghost_costume",
                              SPELL_GHOST_COSTUME_MALE, SPELL_GHOST_COSTUME_FEMALE);
  RegisterSpellScriptWithArgs(spell_hallows_end_put_costume, "spell_hallows_end_ninja_costume",
                              SPELL_NINJA_COSTUME_MALE, SPELL_NINJA_COSTUME_FEMALE);
