...
struct npc_winterfin_playmate : public ScriptedAI
{
  ...
  void UpdateAI(uint32 diff) override
  {
    ...
    if (timer <= diff)
    {
      ...
      {
      case 1:
        orphan->GetMotionMaster()->MovePoint(0, me->GetPositionX() + cos(me->GetOrientation()) * 5,
...
{
  npc_the_biggest_tree(Creature* creature) : ScriptedAI(creature) { me->SetDisplayId(DISPLAY_INVISIBLE); }
...
public:
  ...
  {
    npc_cw_area_triggerAI(Creature* creature) : ScriptedAI(creature) { me->SetDisplayId(DISPLAY_INVISIBLE); }
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new npc_cw_area_triggerAI(creature); }
};
...
public:
  ...
  {
    npc_grizzlemaw_cw_triggerAI(Creature* creature) : ScriptedAI(creature) { me->SetDisplayId(DISPLAY_INVISIBLE); }
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new npc_grizzlemaw_cw_triggerAI(creature); }
};
