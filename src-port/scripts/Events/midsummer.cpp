...
uint32 const GoBonfireAlliance[COUNT_GO_BONFIRE_ALLIANCE] = {
    187564, 187914, 187916, 187917, 187919, 187920, 187921, 187922, 187923, 187924, 187925, 187926, 187927, 187928,
    187929, 187930, 187931, 187932, 187933, 187934, 187935, 187936, 187937, 187938, 187939, 187940, 187941, 187942,
    187943, 187944, 187945, 187946, 194032, 194035, 194036, 194038, 194040, 194044, 194045, 194049};
uint32 const GoBonfireHorde[COUNT_GO_BONFIRE_HORDE] = {
    187559, 187947, 187948, 187949, 187950, 187951, 187952, 187953, 187954, 187955, 187956, 187957, 187958,
    187959, 187960, 187961, 187962, 187963, 187964, 187965, 187966, 187967, 187968, 187969, 187970, 187971,
    187972, 187973, 187974, 187975, 194033, 194034, 194037, 194039, 194042, 194043, 194046, 194048};
uint32 const GoBonfireCity[COUNT_GO_BONFIRE_CITY] = {181332, 181333, 181334, 181335, 181336,
                                                     181337, 188128, 188129, 188352};
  ...
  bool InitBonfire()
  {
    ...
    if ((itr != BonfireStateStore.end()) && (itr->second))
      ...
      LOG_ERROR("scripts.midsummer",
                ...
                me->GetGUID().GetEntry(), me->GetSpawnId(), me->GetMapId(), me->GetZoneId(), _teamId);
...
struct npc_midsummer_torch_target : public ScriptedAI
{
  ...
  bool CanBeSeen(Player const* seer) override { return seer->GetGUID() == playerGUID; }
...
class spell_fire_festival_fortitude : public SpellScript
{
      ...
      void SelectTargets(std::list<WorldObject*>& targets)
  {
    ...
    GetCaster()->GetMap()->DoForAllPlayers(
        [&](Player* p)
        {
  ...
  {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_fire_festival_fortitude::SelectTargets, EFFECT_0,
...
class spell_bonfires_blessing : public AuraScript
{
  ...
  void Register() override
  {
    AfterEffectApply += AuraEffectApplyFn(spell_bonfires_blessing::OnApply, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL,
...
class spell_gen_crab_disguise : public AuraScript
{
  ...
  void Register() override
  {
    AfterEffectApply += AuraEffectApplyFn(spell_gen_crab_disguise::OnApply, EFFECT_0, SPELL_AURA_FORCE_REACTION,
                                          AURA_EFFECT_HANDLE_REAL);
    AfterEffectRemove += AuraEffectRemoveFn(spell_gen_crab_disguise::OnRemove, EFFECT_0, SPELL_AURA_FORCE_REACTION,
                                            AURA_EFFECT_HANDLE_REAL);
  }
};
...
struct npc_midsummer_ribbon_pole_target : public ScriptedAI
{
  ...
  void DoFireworkChecks()
  {
    ...
    {
      scheduler
          .Schedule(500ms, [this](TaskContext /*context*/) { _bunny->CastSpell(nullptr, SPELL_RIBBON_POLE_FIREWORK); })
          .Schedule(1s, [this](TaskContext /*context*/) { _bunny->CastSpell(nullptr, SPELL_RIBBON_POLE_FIREWORK); });
    }
    if (_dancerList.size() >= THRESHOLD_FIREWORK_5)
    {
      scheduler
          .Schedule(1500ms, [this](TaskContext /*context*/) { _bunny->CastSpell(nullptr, SPELL_RIBBON_POLE_FIREWORK); })
          .Schedule(2s, [this](TaskContext /*context*/) { _bunny->CastSpell(nullptr, SPELL_RIBBON_POLE_FIREWORK); });
    }
  }
    ...
    if (_dancerList.size() >= THRESHOLD_SPEW_LAVA)
    {
      if (!_dancerList.empty())
      {
        ...
        for (uint8 i = 0; (i < MAX_COUNT_SPEW_LAVA_TARGETS) && (i < _dancerList.size()); i++)
        {
          ...
          {
            Creature* fireSpiralBunny = dancerTarget->SummonCreature(
                NPC_RIBBON_POLE_FIRE_SPIRAL_BUNNY, dancerTarget->GetPositionX(), dancerTarget->GetPositionY(),
                dancerTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 10000);
            if (fireSpiralBunny)
  ...
  void DoDancingFLameChecks()
  {
    if (_dancerList.size() >= THRESHOLD_DANCING_FLAMES)
    {
      ...
      if (crList.size() < MAX_COUNT_DANCING_FLAMES)
      {
        ...
        DoSpawnCreature(NPC_BIG_DANCING_FLAMES, spawnDist * cos(angle), spawnDist * std::sin(angle), 0, angle + M_PI,
...
class spell_midsummer_ribbon_pole_firework : public SpellScript
{
  ...
  void Register() override
  {
    OnDestinationTargetSelect += SpellDestinationTargetSelectFn(spell_midsummer_ribbon_pole_firework::ModDestHeight,
...
class spell_midsummer_ribbon_pole : public AuraScript
{
  PrepareAuraScript(spell_midsummer_ribbon_pole)
      ...
      bool Validate(SpellInfo const* /*spell*/) override
  {
    return ValidateSpellInfo({SPELL_RIBBON_POLE_XP, SPELL_TEST_RIBBON_POLE_CHANNEL_BLUE,
                              SPELL_TEST_RIBBON_POLE_CHANNEL_RED, SPELL_TEST_RIBBON_POLE_CHANNEL_PINK});
  }
  ...
  void Register() override
  {
    OnEffectApply += AuraEffectApplyFn(spell_midsummer_ribbon_pole::HandleEffectApply, EFFECT_0,
                                       SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
    OnEffectPeriodic +=
        AuraEffectPeriodicFn(spell_midsummer_ribbon_pole::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
  }
};
...
class spell_midsummer_ribbon_pole_visual : public SpellScript
{
  ...
  void Register() override
  {
    OnObjectTargetSelect +=
        SpellObjectTargetSelectFn(spell_midsummer_ribbon_pole_visual::UpdateTarget, EFFECT_0, TARGET_UNIT_NEARBY_ENTRY);
  }
};
...
class spell_midsummer_torch_quest : public AuraScript
{
  ...
  {
    Unit* ar = GetTarget();
    if (Creature* cr = ar->SummonCreature(NPC_TORCH_TOSS_TARGET_BUNNY, ar->GetPositionX(), ar->GetPositionY(),
                                          ar->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 90000))
    {
  ...
  void Register() override
  {
    OnEffectApply += AuraEffectApplyFn(spell_midsummer_torch_quest::HandleEffectApply, EFFECT_0,
                                       SPELL_AURA_DETECT_AMORE, AURA_EFFECT_HANDLE_REAL);
    OnEffectRemove += AuraEffectRemoveFn(spell_midsummer_torch_quest::HandleEffectRemove, EFFECT_0,
                                         SPELL_AURA_DETECT_AMORE, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_midsummer_fling_torch : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override
  {
    return ValidateSpellInfo({SPELL_FLING_TORCH, SPELL_TORCH_SHADOW, SPELL_MISSED_TORCH, SPELL_TORCH_CATCH_SUCCESS_A,
                              SPELL_TORCH_CATCH_SUCCESS_H, SPELL_TORCH_COUNTER});
  }
    ...
    if (Player* target = GetHitPlayer())
    {
      ...
      if (Aura* aur = target->GetAura(SPELL_TORCH_COUNTER))
      {
        ...
        if (aur->GetStackAmount() >= count)
        {
          ...
          target->CastSpell(target, (target->GetTeamId() ? SPELL_TORCH_CATCH_SUCCESS_H : SPELL_TORCH_CATCH_SUCCESS_A),
...
class spell_midsummer_juggling_torch : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override
  {
    return ValidateSpellInfo({SPELL_JUGGLE_SELF, SPELL_JUGGLE_SLOW, SPELL_JUGGLE_MED, SPELL_JUGGLE_FAST,
                              SPELL_TORCH_SHADOW_SELF, SPELL_TORCH_SHADOW_SLOW, SPELL_TORCH_SHADOW_MED,
  ...
  void Register() override { AfterCast += SpellCastFn(spell_midsummer_juggling_torch::HandleFinish); }
};
...
class spell_midsummer_torch_catch : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_GIVE_TORCH}); }
...
class spell_midsummer_summon_ahune_lieutenant : public SpellScript
{
  ...
  void HandleDummy(SpellEffIndex /*effIndex*/)
  {
    ...
    if (npcEntry)
      caster->SummonCreature(npcEntry, caster->GetPosition(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
