...
{
  SetCapturePointData(EPCapturePoints[EP_EWT].entry, EPCapturePoints[EP_EWT].map, EPCapturePoints[EP_EWT].x,
                      EPCapturePoints[EP_EWT].y, EPCapturePoints[EP_EWT].z, EPCapturePoints[EP_EWT].o,
                      EPCapturePoints[EP_EWT].rot0, EPCapturePoints[EP_EWT].rot1, EPCapturePoints[EP_EWT].rot2,
                      EPCapturePoints[EP_EWT].rot3);
  AddObject(EP_EWT_FLAGS, EPTowerFlags[EP_EWT].entry, EPTowerFlags[EP_EWT].map, EPTowerFlags[EP_EWT].x,
            EPTowerFlags[EP_EWT].y, EPTowerFlags[EP_EWT].z, EPTowerFlags[EP_EWT].o, EPTowerFlags[EP_EWT].rot0,
            EPTowerFlags[EP_EWT].rot1, EPTowerFlags[EP_EWT].rot2, EPTowerFlags[EP_EWT].rot3);
}
...
void OPvPCapturePointEP_EWT::ChangeState()
{
  ...
  case OBJECTIVESTATE_ALLIANCE:
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_EWT_A));
    break;
  case OBJECTIVESTATE_HORDE:
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_EWT_H));
    break;
...
{
  SetCapturePointData(EPCapturePoints[EP_NPT].entry, EPCapturePoints[EP_NPT].map, EPCapturePoints[EP_NPT].x,
                      EPCapturePoints[EP_NPT].y, EPCapturePoints[EP_NPT].z, EPCapturePoints[EP_NPT].o,
                      EPCapturePoints[EP_NPT].rot0, EPCapturePoints[EP_NPT].rot1, EPCapturePoints[EP_NPT].rot2,
                      EPCapturePoints[EP_NPT].rot3);
  AddObject(EP_NPT_FLAGS, EPTowerFlags[EP_NPT].entry, EPTowerFlags[EP_NPT].map, EPTowerFlags[EP_NPT].x,
            EPTowerFlags[EP_NPT].y, EPTowerFlags[EP_NPT].z, EPTowerFlags[EP_NPT].o, EPTowerFlags[EP_NPT].rot0,
            EPTowerFlags[EP_NPT].rot1, EPTowerFlags[EP_NPT].rot2, EPTowerFlags[EP_NPT].rot3);
}
...
void OPvPCapturePointEP_NPT::ChangeState()
{
  ...
  case OBJECTIVESTATE_ALLIANCE:
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_NPT_A));
    break;
  case OBJECTIVESTATE_HORDE:
    m_TowerState = EP_TS_H;
    artkit       = 1;
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_NPT_H));
    break;
  case OBJECTIVESTATE_NEUTRAL:
    m_TowerState       = EP_TS_N;
...
void OPvPCapturePointEP_NPT::SummonGO(TeamId teamId)
{
  if (m_SummonedGOSideId != teamId)
  {
    ...
    DelObject(EP_NPT_BUFF);
    AddObject(EP_NPT_BUFF, EP_NPT_LordaeronShrine.entry, EP_NPT_LordaeronShrine.map, EP_NPT_LordaeronShrine.x,
              EP_NPT_LordaeronShrine.y, EP_NPT_LordaeronShrine.z, EP_NPT_LordaeronShrine.o, EP_NPT_LordaeronShrine.rot0,
              EP_NPT_LordaeronShrine.rot1, EP_NPT_LordaeronShrine.rot2, EP_NPT_LordaeronShrine.rot3);
    Map* map    = sMapMgr->FindMap(0, 0);
...
{
  SetCapturePointData(EPCapturePoints[EP_CGT].entry, EPCapturePoints[EP_CGT].map, EPCapturePoints[EP_CGT].x,
                      EPCapturePoints[EP_CGT].y, EPCapturePoints[EP_CGT].z, EPCapturePoints[EP_CGT].o,
                      EPCapturePoints[EP_CGT].rot0, EPCapturePoints[EP_CGT].rot1, EPCapturePoints[EP_CGT].rot2,
                      EPCapturePoints[EP_CGT].rot3);
  AddObject(EP_CGT_FLAGS, EPTowerFlags[EP_CGT].entry, EPTowerFlags[EP_CGT].map, EPTowerFlags[EP_CGT].x,
            EPTowerFlags[EP_CGT].y, EPTowerFlags[EP_CGT].z, EPTowerFlags[EP_CGT].o, EPTowerFlags[EP_CGT].rot0,
            EPTowerFlags[EP_CGT].rot1, EPTowerFlags[EP_CGT].rot2, EPTowerFlags[EP_CGT].rot3);
}
...
void OPvPCapturePointEP_CGT::ChangeState()
{
  ...
  case OBJECTIVESTATE_ALLIANCE:
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_CGT_A));
    break;
  case OBJECTIVESTATE_HORDE:
    m_TowerState = EP_TS_H;
    artkit       = 1;
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_CGT_H));
    break;
  case OBJECTIVESTATE_NEUTRAL:
    m_TowerState = EP_TS_N;
    break;
...
{
  SetCapturePointData(EPCapturePoints[EP_PWT].entry, EPCapturePoints[EP_PWT].map, EPCapturePoints[EP_PWT].x,
                      EPCapturePoints[EP_PWT].y, EPCapturePoints[EP_PWT].z, EPCapturePoints[EP_PWT].o,
                      EPCapturePoints[EP_PWT].rot0, EPCapturePoints[EP_PWT].rot1, EPCapturePoints[EP_PWT].rot2,
                      EPCapturePoints[EP_PWT].rot3);
  AddObject(EP_PWT_FLAGS, EPTowerFlags[EP_PWT].entry, EPTowerFlags[EP_PWT].map, EPTowerFlags[EP_PWT].x,
            EPTowerFlags[EP_PWT].y, EPTowerFlags[EP_PWT].z, EPTowerFlags[EP_PWT].o, EPTowerFlags[EP_PWT].rot0,
            EPTowerFlags[EP_PWT].rot1, EPTowerFlags[EP_PWT].rot2, EPTowerFlags[EP_PWT].rot3);
}
...
void OPvPCapturePointEP_PWT::ChangeState()
{
  ...
  case OBJECTIVESTATE_ALLIANCE:
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_PWT_A));
    break;
  case OBJECTIVESTATE_HORDE:
    m_TowerState = EP_TS_H;
    ...
    if (_oldState != _state)
      sWorld->SendZoneText(EP_GraveYardZone, sObjectMgr->GetAcoreStringForDBCLocale(LANG_OPVP_EP_CAPTURE_PWT_H));
    break;
  case OBJECTIVESTATE_NEUTRAL:
    m_TowerState = EP_TS_N;
...
void OPvPCapturePointEP_PWT::SummonFlightMaster(TeamId teamId)
{
  if (m_FlightMasterSpawnedId != teamId)
  {
    ...
    DelCreature(EP_PWT_FLIGHTMASTER);
    AddCreature(EP_PWT_FLIGHTMASTER, EP_PWT_FlightMaster.entry, EP_PWT_FlightMaster.map, EP_PWT_FlightMaster.x,
                EP_PWT_FlightMaster.y, EP_PWT_FlightMaster.z, EP_PWT_FlightMaster.o);
    /*
...
void OutdoorPvPEP::SetControlledState(uint32 index, TeamId teamId) { EP_ControlsId[index] = teamId; }
