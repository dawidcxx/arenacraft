...
bool OutdoorPvPSI::HandleDropFlag(Player* player, uint32 spellId)
{
  ...
  {
    // if it was dropped away from the player's turn-in point, then create a silithyst mound, if it was dropped near the
    // areatrigger, then it was dispelled by the outdoorpvp, so do nothing
    ...
    case TEAM_ALLIANCE:
    {
      ...
      if (atEntry)
      {
        ...
        if (player->GetDistance(atEntry->x, atEntry->y, atEntry->z) > 5.0f + atEntry->radius)
        {
          ...
          GameObject* go =
              sObjectMgr->IsGameObjectStaticTransport(SI_SILITHYST_MOUND) ? new StaticTransport() : new GameObject();
          ...
          if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(), SI_SILITHYST_MOUND, map, player->GetPhaseMask(),
                          player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(),
                          player->GetOrientation(), G3D::Quat(), 100, GO_STATE_READY))
          {
            delete go;
            return true;
          }
