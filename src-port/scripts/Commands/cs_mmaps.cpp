...
public:
  ...
  static bool HandleMmapStatsCommand(ChatHandler* handler)
  {
    ...
    // handler->PSendSysMessage("  global mmap pathfinding is %sabled", DisableMgr::IsPathfindingEnabled(mapId) ? "en" :
    // "dis");

    MMAP::MMapMgr* manager = MMAP::MMapFactory::createOrGetMMapMgr();
    handler->PSendSysMessage(" {} maps loaded with {} tiles overall", manager->getLoadedMapsCount(),
    ...
    handler->PSendSysMessage(" {} MB of data (not including pointers)",
