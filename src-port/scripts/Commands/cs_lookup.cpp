  ...
  static bool HandleLookupAreaCommand(ChatHandler* handler, Tail namePart)
  {
    ...
    for (auto areaEntry : sAreaTableStore)
    {
      ...
      if (locale < TOTAL_LOCALES)
      {
        ...
        {
          ss << areaEntry->ID << " - |cffffffff|Harea:" << areaEntry->ID << "|h[" << name << ' ' << localeNames[locale]
             << "]|h|r";
        }
        else
        {
          ...
          if (il->Name.size() > ulocaleIndex && !il->Name[ulocaleIndex].empty())
          {
            ...
            if (Utf8FitTo(name, wNamePart))
            {
              ...
              if (handler->GetSession())
              {
                ...
                handler->PSendSysMessage(LANG_ITEM_LIST_CHAT, itemTemplate.ItemId, color.str(), itemTemplate.ItemId,
                                         name);
              }
              else
              {
      ...
      if (GameObjectLocale const* objectLocalte = sObjectMgr->GetGameObjectLocale(gameObjectTemplate.entry))
      {
        if (objectLocalte->Name.size() > localeIndex && !objectLocalte->Name[localeIndex].empty())
        {
          ...
          if (Utf8FitTo(name, wNamePart))
          {
            ...
            if (handler->GetSession())
            {
              handler->PSendSysMessage(LANG_GO_ENTRY_LIST_CHAT, gameObjectTemplate.entry, gameObjectTemplate.entry,
                                       name);
            }
            else
            {
            ...
            if (Utf8FitTo(title, wNamePart))
            {
              ...
              if (handler->GetSession())
              {
                handler->PSendSysMessage(LANG_QUEST_LIST_CHAT, qInfo->GetQuestId(), qInfo->GetQuestId(),
                                         qInfo->GetQuestLevel(), title, statusStr);
              }
              else
              {
    ...
    for (auto skillInfo : sSkillLineStore)
    {
      ...
      if (locale < TOTAL_LOCALES)
      {
        ...
        if (target && target->HasSkill(skillInfo->id))
        {
          ...
          valStr =
              Acore::StringFormat(handler->GetAcoreString(LANG_SKILL_VALUES), curValue, maxValue, permValue, tempValue);
        }

        // send skill in "id - [namedlink locale]" format
        if (handler->GetSession())
        {
          handler->PSendSysMessage(LANG_SKILL_LIST_CHAT, skillInfo->id, skillInfo->id, name, localeNames[locale],
                                   knownStr, valStr);
        }
        else
        {
  ...
  static bool HandleLookupTaxiNodeCommand(ChatHandler* handler, Tail namePart)
  {
    ...
    for (auto nodeEntry : sTaxiNodesStore)
    {
      ...
      if (locale < TOTAL_LOCALES)
      {
        ...
        if (handler->GetSession())
        {
          handler->PSendSysMessage(LANG_TAXINODE_ENTRY_LIST_CHAT, nodeEntry->ID, nodeEntry->ID, name,
                                   localeNames[locale], nodeEntry->map_id, nodeEntry->x, nodeEntry->y, nodeEntry->z);
        }
        else
        {
  ...
  static bool HandleLookupTitleCommand(ChatHandler* handler, Tail namePart)
  {
    ...
    for (auto titleInfo : sCharTitlesStore)
    {
      ...
      if (locale < TOTAL_LOCALES)
      {
        ...
        if (handler->GetSession())
        {
          handler->PSendSysMessage(LANG_TITLE_LIST_CHAT, titleInfo->ID, titleInfo->bit_index, titleInfo->ID,
                                   titleNameStr, localeNames[locale], knownStr, activeStr);
        }
        else
        {
          handler->PSendSysMessage(LANG_TITLE_LIST_CONSOLE, titleInfo->ID, titleInfo->bit_index, titleNameStr,
                                   localeNames[locale], knownStr, activeStr);
        }
    ...
    do
    {
      ...
      if (result2)
      {
        ...
        do
        {
          ...
          {
            handler->PSendSysMessage("  {} (GUID {}) - {} - {} - {}{}", name, guid, EnumUtils::ToTitle(Races(prace)),
                                     EnumUtils::ToTitle(Classes(pclass)), plevel, (online ? " - [ONLINE]" : ""));
          }
          else
          {
