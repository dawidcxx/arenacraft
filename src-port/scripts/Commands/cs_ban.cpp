...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable commandTable = {{"ban", banCommandTable},
  ...
  static bool HandleBanCharacterCommand(ChatHandler* handler, char const* args)
  {
    ...
    switch (sBan->BanCharacter(name, durationStr, reasonStr,
    ...
    case BAN_SUCCESS:
      if (atoi(durationStr) > 0)
      {
        if (!sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
          handler->PSendSysMessage(LANG_BAN_YOUBANNED, name, secsToTimeString(TimeStringToSecs(durationStr), true),
                                   reasonStr);
      }
      else
      {
  ...
  static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
  {
    ...
    {
    case BAN_ACCOUNT:
      banReturn = sBan->BanAccount(nameOrIP, durationStr, reasonStr,
                                   handler->GetSession() ? handler->GetSession()->GetPlayerName() : "");
      break;
    case BAN_CHARACTER:
      banReturn = sBan->BanAccountByPlayerName(nameOrIP, durationStr, reasonStr,
                                               handler->GetSession() ? handler->GetSession()->GetPlayerName() : "");
      break;
    case BAN_IP:
    default:
      banReturn = sBan->BanIP(nameOrIP, durationStr, reasonStr,
                              handler->GetSession() ? handler->GetSession()->GetPlayerName() : "");
      break;
    }
    ...
    case BAN_SUCCESS:
      ...
      {
        if (!sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
          handler->PSendSysMessage(LANG_BAN_YOUBANNED, nameOrIP, secsToTimeString(TimeStringToSecs(durationStr), true),
                                   reasonStr);
      }
      else
      {
  ...
  static bool HandleBanInfoHelper(uint32 accountId, char const* accountName, ChatHandler* handler)
  {
    QueryResult result = LoginDatabase.Query(
        "SELECT FROM_UNIXTIME(bandate, '%Y-%m-%d..%H:%i:%s') as bandate, unbandate-bandate, active, unbandate, "
        "banreason, bannedby FROM account_banned WHERE id = '{}' ORDER BY bandate ASC",
        accountId);
    if (!result)
    {
    ...
    do
    {
      ...
      std::string banTime =
          permanent ? handler->GetAcoreString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].Get<uint64>(), true);
      handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY, fields[0].Get<std::string>(), banTime,
  ...
  static bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
  {
    ...
    handler->PSendSysMessage(LANG_BANINFO_IPENTRY, fields[0].Get<std::string>(), fields[1].Get<std::string>(),
  ...
  static bool HandleBanListHelper(PreparedQueryResult result, ChatHandler* handler)
  {
    ...
    if (handler->GetSession())
    {
      do
      {
        ...
        QueryResult banResult = LoginDatabase.Query("SELECT account.username FROM account, account_banned WHERE "
                                                    "account_banned.id='{}' AND account_banned.id=account.id",
                                                    accountid);
        if (banResult)
        {
          ...
          do
          {
            ...
            if (fields2[0].Get<uint32>() == fields2[1].Get<uint32>())
            {
              handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|   permanent  |%-15.15s|%-15.15s|",
                                       accountName, tmBan.tm_year % 100, tmBan.tm_mon + 1, tmBan.tm_mday, tmBan.tm_hour,
                                       tmBan.tm_min, fields2[2].Get<std::string>(), fields2[3].Get<std::string>());
            }
            else
            {
              ...
              handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|%02d-%02d-%02d %02d:%02d|%-15.15s|%-15.15s|",
                                       accountName, tmBan.tm_year % 100, tmBan.tm_mon + 1, tmBan.tm_mday, tmBan.tm_hour,
                                       tmBan.tm_min, tmUnban.tm_year % 100, tmUnban.tm_mon + 1, tmUnban.tm_mday,
                                       tmUnban.tm_hour, tmUnban.tm_min, fields2[2].Get<std::string>(),
      ...
      do
      {
        ...
        if (banInfo)
        {
          ...
          do
          {
            ...
            {
              handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|   permanent  |%-15.15s|%-15.15s|",
                                       char_name, tmBan.tm_year % 100, tmBan.tm_mon + 1, tmBan.tm_mday, tmBan.tm_hour,
                                       tmBan.tm_min, banFields[2].Get<std::string>(), banFields[3].Get<std::string>());
            }
            else
            {
              ...
              handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|%02d-%02d-%02d %02d:%02d|%-15.15s|%-15.15s|",
                                       char_name, tmBan.tm_year % 100, tmBan.tm_mon + 1, tmBan.tm_mday, tmBan.tm_hour,
                                       tmBan.tm_min, tmUnban.tm_year % 100, tmUnban.tm_mon + 1, tmUnban.tm_mday,
                                       tmUnban.tm_hour, tmUnban.tm_min, banFields[2].Get<std::string>(),
  ...
  static bool HandleBanListIPCommand(ChatHandler* handler, char const* args)
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|   permanent  |%-15.15s|%-15.15s|",
                                   fields[0].Get<std::string>(), tmBan.tm_year % 100, tmBan.tm_mon + 1, tmBan.tm_mday,
                                   tmBan.tm_hour, tmBan.tm_min, fields[3].Get<std::string>(),
        ...
        else
        {
          ...
          handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|%02d-%02d-%02d %02d:%02d|%-15.15s|%-15.15s|",
                                   fields[0].Get<std::string>(), tmBan.tm_year % 100, tmBan.tm_mon + 1, tmBan.tm_mday,
                                   tmBan.tm_hour, tmBan.tm_min, tmUnban.tm_year % 100, tmUnban.tm_mon + 1,
                                   tmUnban.tm_mday, tmUnban.tm_hour, tmUnban.tm_min, fields[3].Get<std::string>(),
                                   fields[4].Get<std::string>());
        }
      } while (result->NextRow());
