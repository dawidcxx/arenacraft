...
public:
  ...
  {
    static ChatCommandTable auraCommandTable = {{"stack", HandleAuraStacksCommand, SEC_GAMEMASTER, Console::No},
  ...
  static bool HandleSkirmishCommand(ChatHandler* handler, std::vector<std::string_view> args)
  {
    ...
    Battleground* bg = sBattlegroundMgr->CreateNewBattleground(
        randomizedArenaBgTypeId, GetBattlegroundBracketById(bgt->GetMapId(), bgt->GetBracketId()),
        ArenaType(hcnt >= 2 ? hcnt : 2), false);
    if (!bg)
    {
      handler->SendErrorMessage("Couldn't create arena map!");
      return false;
  ...
  static bool HandleGPSCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    handler->PSendSysMessage(LANG_MAP_POSITION, object->GetMapId(),
                             (mapEntry ? mapEntry->name[handler->GetSessionDbcLocale()] : "<unknown>"), zoneId,
                             (zoneEntry ? zoneEntry->area_name[handler->GetSessionDbcLocale()] : "<unknown>"), areaId,
                             ...
                             object->GetPositionZ(), object->GetOrientation(), cell.GridX(), cell.GridY(), cell.CellX(),
                             cell.CellY(), object->GetInstanceId(), zoneX, zoneY, groundZ, floorZ, haveMap, haveVMap,
                             haveMMAP);
    ...
    {
      handler->PSendSysMessage(LANG_LIQUID_STATUS, liquidData.Level, liquidData.DepthLevel, liquidData.Entry,
                               liquidData.Flags, liquidData.Status);
    }

    if (object->GetTransport())
    {
      handler->PSendSysMessage(
          "Transport offset: {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}", object->m_movementInfo.transport.pos.GetPositionX(),
          object->m_movementInfo.transport.pos.GetPositionY(), object->m_movementInfo.transport.pos.GetPositionZ(),
  ...
  static bool HandleAppearCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    if (target->IsConnected())
    {
      ...
      if (map->IsBattlegroundOrArena())
      {
        ...
        _player->SetBattlegroundId(targetPlayer->GetBattlegroundId(), targetPlayer->GetBattlegroundTypeId(),
      ...
      if (_player->TeleportTo(targetPlayer->GetMapId(), targetPlayer->GetPositionX(), targetPlayer->GetPositionY(),
                              targetPlayer->GetPositionZ() + 0.25f, _player->GetOrientation(), TELE_TO_GM_MODE,
                              targetPlayer))
      {
  ...
  static bool HandleSummonCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    if (target->IsConnected())
    {
      ...
      else if (map->IsDungeon())
      {
        ...
        if (destMap->Instanceable() && destMap->GetInstanceId() != map->GetInstanceId())
        {
          sInstanceSaveMgr->PlayerUnbindInstance(target->GetGUID(), map->GetInstanceId(),
                                                 targetPlayer->GetDungeonDifficulty(), true, targetPlayer);
        }
      ...
      targetPlayer->TeleportTo(handler->GetSession()->GetPlayer()->GetMapId(), x, y, z, targetPlayer->GetOrientation(),
                               0, handler->GetSession()->GetPlayer());
    }
    else
    {
      // check offline security
      ...
      Player::SavePositionInDB(
          handler->GetSession()->GetPlayer()->GetMapId(), handler->GetSession()->GetPlayer()->GetPositionX(),
          handler->GetSession()->GetPlayer()->GetPositionY(), handler->GetSession()->GetPlayer()->GetPositionZ(),
          handler->GetSession()->GetPlayer()->GetOrientation(), handler->GetSession()->GetPlayer()->GetZoneId(),
          target->GetGUID());
    }
  ...
  static bool HandleGroupSummonCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
      ...
      player->TeleportTo(handler->GetSession()->GetPlayer()->GetMapId(), x, y, z, player->GetOrientation(), 0,
  ...
  static bool HandleDieCommand(ChatHandler* handler)
  {
    ...
    if (target->IsAlive())
    {
      ...
      {
        Unit::DealDamage(handler->GetSession()->GetPlayer(), target, target->GetHealth(), nullptr, DIRECT_DAMAGE,
  ...
  static bool HandleReviveCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    if (target->IsConnected())
    {
      ...
      targetPlayer->ResurrectPlayer(!AccountMgr::IsPlayerAccount(targetPlayer->GetSession()->GetSecurity()) ? 1.0f
                                                                                                            : 0.5f);
  ...
  static bool HandleCooldownCommand(ChatHandler* handler, Optional<SpellInfo const*> spell)
  {
    ...
    else
    {
      ...
      handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spell.value()->Id,
                               target == handler->GetSession()->GetPlayer() ? handler->GetAcoreString(LANG_YOU)
                                                                            : nameLink);
    }
    return true;
  }
  ...
  static bool HandleGetDistanceCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    handler->PSendSysMessage(LANG_DISTANCE, handler->GetSession()->GetPlayer()->GetDistance(object),
  ...
  static bool HandleRecallCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    targetPlayer->TeleportTo(targetPlayer->m_recallMap, targetPlayer->m_recallX, targetPlayer->m_recallY,
                             targetPlayer->m_recallZ, targetPlayer->m_recallO);
    return true;
  }
  ...
  // kick player
  static bool HandleKickPlayerCommand(ChatHandler* handler, Optional<PlayerIdentifier> target,
  ...
  static bool HandleUnstuckCommand(ChatHandler* handler, Optional<PlayerIdentifier> target,
                                   Optional<std::string_view> location)
  {
    ...
    if (!target || !target->IsConnected())
    {
      ...
      if (result)
      {
        ...
        WorldLocation loc(fieldsDB[0].Get<uint16>(), fieldsDB[2].Get<float>(), fieldsDB[3].Get<float>(),
                          fieldsDB[4].Get<float>(), 0.0f);
  ...
  static bool HandleSetSkillCommand(ChatHandler* handler, Variant<Hyperlink<skill>, uint32> skillId, int32 level,
                                    Optional<uint16> maxPureSkill)
  {
    ...
    handler->PSendSysMessage(LANG_SET_SKILL, skillID, skillLine->name[handler->GetSessionDbcLocale()],
                             handler->GetNameLink(target), level, max);
    return true;
  }
  ...
  static bool HandlePInfoCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    handler->PSendSysMessage(LANG_PINFO_PLAYER, playerTarget ? "" : handler->GetAcoreString(LANG_OFFLINE), nameLink,
    ...
    if (banTime >= 0)
    {
      handler->PSendSysMessage(LANG_PINFO_BANNED, banType, banReason,
    ...
    if (muteTime > 0)
    {
      handler->PSendSysMessage(LANG_PINFO_MUTED, muteReason,
                               secsToTimeString(muteTime - GameTime::GetGameTime().count(), true), muteBy);
    }
    ...
    handler->PSendSysMessage(LANG_PINFO_ACC_IP, lastIp,
                             locked ? handler->GetAcoreString(LANG_YES) : handler->GetAcoreString(LANG_NO));
    ...
    // SQL: "SELECT SUM(CASE WHEN (checked & 1) THEN 1 ELSE 0 END) AS 'readmail', COUNT(*) AS 'totalmail' FROM mail
    // WHERE `receiver` = ?"
  ...
  static bool HandleMuteCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, std::string notSpeakTime,
                                Tail muteReason)
  {
    ...
    uint32  accountId =
        target ? target->GetSession()->GetAccountId() : sCharacterCache->GetCharacterAccountIdByGuid(player->GetGUID());
    ...
    if (target)
    {
      ...
      {
        handler->SendWorldText(LANG_COMMAND_MUTEMESSAGE_WORLD, muteBy, nameLink, secsToTimeString(muteDuration, true),
                               muteReasonStr);
      }
        ...
        if (itr->second->GetSession()->GetSecurity())
          ChatHandler(itr->second->GetSession())
              .PSendSysMessage(target ? LANG_YOU_DISABLE_CHAT : LANG_COMMAND_DISABLE_CHAT_DELAYED,
                               ...
                               nameLink, secsToTimeString(muteDuration, true), muteReasonStr);
    }
  ...
  static bool HandleMuteInfoHelper(ChatHandler* handler, uint32 accountId, char const* accountName)
  {
    ...
    do
    {
      ...
      handler->PSendSysMessage(LANG_COMMAND_MUTEHISTORY_OUTPUT,
                               ...
                               fields[1].Get<uint32>(), fields[2].Get<std::string>(), fields[3].Get<std::string>());
  ...
  static bool HandleMovegensCommand(ChatHandler* handler)
  {
    ...
    handler->PSendSysMessage(LANG_MOVEGENS_LIST, (unit->IsPlayer() ? "Player" : "Creature"),
  ...
  */
  ...
  static bool HandleDamageCommand(ChatHandler* handler, uint32 damage, Optional<std::string> percent)
  {
    ...
    Unit::DealDamage(handler->GetSession()->GetPlayer(), target, damage, nullptr, DIRECT_DAMAGE,
                     SPELL_SCHOOL_MASK_NORMAL, nullptr, false, true);
  ...
  static bool HandleFreezeCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
  {
    ...
    else if (creatureTarget && creatureTarget->IsAlive())
    {
      handler->PSendSysMessage(LANG_COMMAND_FREEZE,
                               GetLocalizeCreatureName(creatureTarget, handler->GetSessionDbcLocale()));
