...
public:
  ...
  static bool HandleInstanceListBindsCommand(ChatHandler* handler)
  {
    ...
    for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
    {
      for (auto const& [mapId, bind] : sInstanceSaveMgr->PlayerGetBoundInstances(player->GetGUID(), Difficulty(i)))
      {
        ...
        handler->PSendSysMessage("map: {}, inst: {}, perm: {}, diff: {}, canReset: {}, TTR: {}{}", mapId,
                                 save->GetInstanceId(), bind.perm ? "yes" : "no", save->GetDifficulty(),
                                 save->CanReset() ? "yes" : "no", timeleft, (bind.extended ? " (extended)" : ""));
        counter++;
      }
    }
      ...
      for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin(); itr != m_boundInstances.end();)
      {
            ...
            (!difficultyArg || difficultyArg == save->GetDifficulty()))
        {
          ...
          handler->PSendSysMessage("unbinding map: {}, inst: {}, perm: {}, diff: {}, canReset: {}, TTR: {}{}",
                                   itr->first, save->GetInstanceId(), itr->second.perm ? "yes" : "no",
                                   save->GetDifficulty(), save->CanReset() ? "yes" : "no", timeleft,
  ...
  static bool HandleInstanceStatsCommand(ChatHandler* handler)
  {
    ...
    handler->PSendSysMessage("instances loaded: dungeons ({}), battlegrounds ({}), arenas ({})", dungeon, battleground,
                             arena);
    dungeon      = 0;
    battleground = 0;
    arena        = 0;
  ...
  static bool HandleInstanceSetBossStateCommand(ChatHandler* handler, uint32 encounterId, uint32 state,
  ...
  static bool HandleInstanceGetBossStateCommand(ChatHandler* handler, uint32 encounterId,
                                                Optional<PlayerIdentifier> player)
  {
    // Character name must be provided when using this from console.
    if (!player && !handler->GetSession())
