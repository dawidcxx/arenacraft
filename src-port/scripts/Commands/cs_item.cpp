...
public:
  ...
  static bool HandleItemRestoreCommand(ChatHandler* handler, uint32 restoreId, PlayerIdentifier player)
  {
    ...
    else
    {
      ...
      MailDraft  draft("Recovered Item", "We recovered a lost item in the twisting nether and noted that it was "
                                          "yours.$B$BPlease find said object enclosed.");
  ...
  static bool HandleItemRefundCommand(ChatHandler* handler, PlayerIdentifier player, uint32 itemId, uint32 extendedCost)
  {
    ...
    if (Player* target = player.GetConnectedPlayer())
    {
      ...
      if (iece->reqhonorpoints)
      {
        ...
        {
          handler->SendErrorMessage(LANG_CMD_ITEM_REFUND_MAX_HONOR, item->Name1, item->ItemId,
                                    sWorld->getIntConfig(CONFIG_MAX_HONOR_POINTS), target->GetHonorPoints(),
                                    iece->reqhonorpoints);
      ...
      if (iece->reqarenapoints)
      {
        ...
        {
          handler->SendErrorMessage(LANG_CMD_ITEM_REFUND_MAX_AP, item->Name1, item->ItemId,
                                    sWorld->getIntConfig(CONFIG_MAX_ARENA_POINTS), target->GetArenaPoints(),
                                    iece->reqarenapoints);
      ...
      if (result)
      {
        if (iece->reqhonorpoints)
        {
          ...
          if (queryResult)
          {
            ...
            {
              handler->SendErrorMessage(LANG_CMD_ITEM_REFUND_MAX_HONOR, item->Name1, item->ItemId,
                                        sWorld->getIntConfig(CONFIG_MAX_HONOR_POINTS), fields[0].Get<uint32>(),
                                        iece->reqhonorpoints);
              return false;
            }
          }
        ...
        if (iece->reqarenapoints)
        {
          ...
          if (queryResult)
          {
            ...
            {
              handler->SendErrorMessage(LANG_CMD_ITEM_REFUND_MAX_AP, item->Name1, item->ItemId,
                                        sWorld->getIntConfig(CONFIG_MAX_ARENA_POINTS), fields[0].Get<uint32>(),
                                        iece->reqarenapoints);
              return false;
            }
          }
