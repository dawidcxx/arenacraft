...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable serverCommandTable = {{"corpses", HandleServerCorpsesCommand, SEC_GAMEMASTER, Console::Yes},
  ...
  static bool HandleServerDebugCommand(ChatHandler* handler)
  {
...

    {
      ...
      if (dbPort)
        ...
        dbPortOutput =
            Acore::StringFormat("Realm Id: {} not found in `realmlist` table. Please check your setup", realm.Id.Realm);
    }

    HandleServerInfoCommand(handler);

    handler->PSendSysMessage("Using SSL version: {} (library: {})", OPENSSL_VERSION_TEXT,
                             OpenSSL_version(OPENSSL_VERSION));
    handler->PSendSysMessage("Using Boost version: {}.{}.{}", BOOST_VERSION / 100000, BOOST_VERSION / 100 % 1000,
                             BOOST_VERSION % 100);
    ...
    {
      handler->PSendSysMessage("VMAPs status: Enabled. LineOfSight: {}, getHeight: {}, indoorCheck: {}", vmapLOSCheck,
                               vmapHeightCheck, vmapIndoorCheck);
    ...
    for (std::string const& subDir : subDirs)
    {
      ...
      std::size_t folderSize = std::accumulate(std::filesystem::directory_iterator(mapPath), end, std::size_t(0),
      ...
      handler->PSendSysMessage("{} directory located in {}. Total size: {} bytes", subDir, mapPath.generic_string(),
                               folderSize);
    }
    ...
    handler->PSendSysMessage("Default DBC locale: {}.\nAll available DBC locales: {}", localeNames[defaultLocale],
                             availableLocales);
  ...
  static bool HandleServerInfoCommand(ChatHandler* handler)
  {
    ...
    if (!queuedSessionCount)
      ...
      handler->PSendSysMessage("Connected players: {}. Characters in world: {}. Queue: {}.", activeSessionCount,
                               playerCount, queuedSessionCount);
    ...
    handler->PSendSysMessage("Update time diff: {}ms. Last {} diffs summary:", sWorldUpdateTime.GetLastUpdateTime(),
    ...
    handler->PSendSysMessage("|- Percentiles (95, 99, max): {}ms, {}ms, {}ms", sWorldUpdateTime.GetPercentile(95),
                             sWorldUpdateTime.GetPercentile(99), sWorldUpdateTime.GetPercentile(100));
  ...
  static bool HandleServerIdleRestartCommand(ChatHandler* handler, std::string time, Optional<int32> exitCode,
                                             Tail reason)
  {
    std::wstring wReason   = std::wstring();
    std::string  strReason = std::string();
  ...
  static bool HandleServerIdleShutDownCommand(ChatHandler* handler, std::string time, Optional<int32> exitCode,
                                              Tail reason)
  {
    std::wstring wReason   = std::wstring();
    std::string  strReason = std::string();
  ...
  static bool HandleServerSetLogLevelCommand(ChatHandler* /*handler*/, bool isLogger, std::string const& name,
                                             int32 level)
  {
