...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable account2faCommandTable{{"setup", HandleAccount2FASetupCommand, SEC_PLAYER, Console::No},
    ...
    static ChatCommandTable commandTable = {{"account", accountCommandTable}};
  ...
  static bool HandleAccount2FASetupCommand(ChatHandler* handler, char const* args)
  {
    ...
    { // check if 2FA already enabled
      ...
      if (!result)
      {
        LOG_ERROR("misc", "Account {} not found in login database when processing .account 2fa setup command.",
                  accountId);
    ...
    auto                                                           pair = suggestions.emplace(
        std::piecewise_construct, std::make_tuple(accountId),
        std::make_tuple(Acore::Crypto::TOTP::RECOMMENDED_SECRET_LENGTH)); // std::vector 1-argument std::size_t
                                                                          // constructor invokes resize
  ...
  /// \todo This function has to be enhanced to respect the login/realm split (delete char, delete account chars in
  /// realm then delete account)
  ...
  static bool HandleAccountOnlineListCommand(ChatHandler* handler, char const* /*args*/)
  {
    ...
    do
    {
      ...
      if (resultLogin)
      {
        ...
        handler->PSendSysMessage(LANG_ACCOUNT_LIST_LINE, fieldsLogin[0].Get<std::string>(), name,
                                 fieldsLogin[1].Get<std::string>(), fieldsDB[2].Get<uint16>(),
                                 fieldsDB[3].Get<uint16>(), fieldsLogin[3].Get<uint8>(), fieldsLogin[2].Get<uint8>());
      }
      else
  ...
  static bool HandleAccountSetEmailCommand(ChatHandler* handler, AccountIdentifier account, std::string email,
