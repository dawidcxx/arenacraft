...
public:
  ...
  static bool HandleListCreatureCommand(ChatHandler* handler, Variant<Hyperlink<creature_entry>, uint32> creatureId,
                                        Optional<uint32> countArg)
  {
    ...
    if (handler->GetSession())
    {
      ...
      result = WorldDatabase.Query("SELECT guid, position_x, position_y, position_z, map, (POW(position_x - '{}', 2) + "
                                   "POW(position_y - '{}', 2) + POW(position_z - '{}', 2)) AS order_ FROM creature "
                                   "WHERE id1='{}' OR id2='{}' OR id3='{}' ORDER BY order_ ASC LIMIT {}",
                                   player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(),
                                   uint32(creatureId), uint32(creatureId), uint32(creatureId), count);
    }
    else
      result = WorldDatabase.Query("SELECT guid, position_x, position_y, position_z, map FROM creature WHERE id1='{}' "
                                   "OR id2='{}' OR id3='{}' LIMIT {}",
                                   uint32(creatureId), uint32(creatureId), uint32(creatureId), count);
                 ...
                 itr != creBounds.second;)
            {
              if (handler->GetSession())
                handler->PSendSysMessage(LANG_CREATURE_LIST_CHAT, guid, cInfo->Entry, guid, cInfo->Name, x, y, z, mapId,
                                         itr->second->GetGUID().ToString(), itr->second->IsAlive() ? "*" : " ");
              else
  ...
  static bool HandleListItemCommand(ChatHandler* handler, Variant<Hyperlink<item>, uint32> itemArg,
                                    Optional<uint32> countArg)
  {
    uint32 itemId = 0;
    uint32 count  = countArg.value_or(10);
    ...
    if (result)
    {
      do
      {
        ...
        handler->PSendSysMessage(LANG_ITEMLIST_MAIL, itemGuid, itemSenderName, itemSender, itemSenderAccountId,
                                 itemReceiverName, itemReceiver, itemReceiverAccount, itemPos);
      } while (result->NextRow());
    ...
    handler->PSendSysMessage(LANG_COMMAND_LISTITEMMESSAGE, itemId,
                             inventoryCount + mailCount + auctionCount + guildCount, inventoryCount, mailCount,
                             auctionCount, guildCount);

    return true;
  }

  static bool HandleListObjectCommand(ChatHandler* handler, Variant<Hyperlink<gameobject_entry>, uint32> gameObjectId,
                                      Optional<uint32> countArg)
  {
    ...
    {
      Player* player = handler->GetSession()->GetPlayer();
      result = WorldDatabase.Query("SELECT guid, position_x, position_y, position_z, map, id, (POW(position_x - '{}', "
                                   "2) + POW(position_y - '{}', 2) + POW(position_z - '{}', 2)) AS order_ FROM "
                                   "gameobject WHERE id = '{}' ORDER BY order_ ASC LIMIT {}",
                                   player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(),
                 ...
                 itr != goBounds.second;)
            {
              if (handler->GetSession())
                handler->PSendSysMessage(LANG_GO_LIST_CHAT, guid, entry, guid, gInfo->name, x, y, z, mapId,
                                         itr->second->GetGUID().ToString(), itr->second->isSpawned() ? "*" : " ");
              else
  ...
  static bool HandleListAllAurasCommand(ChatHandler* handler) { return ListAurasCommand(handler, {}, {}); }
  ...
  static bool ListAurasCommand(ChatHandler* handler, Optional<uint32> spellId, std::wstring namePart)
  {
    ...
    for (auto const& [aurId, aurApp] : auras)
    {
      ...
      handler->PSendSysMessage(
          LANG_COMMAND_TARGET_AURADETAIL, aura->GetId(), (handler->GetSession() ? ss_name.str() : name),
          aurApp->GetEffectMask(), aura->GetCharges(), aura->GetStackAmount(), aurApp->GetSlot(), aura->GetDuration(),
          aura->GetMaxDuration(), (aura->IsPassive() ? passiveStr : ""), (talent ? talentStr : ""),
          aura->GetCasterGUID().IsPlayer() ? "player" : "creature", aura->GetCasterGUID().ToString());
    }
    ...
    for (uint16 i = 0; i < TOTAL_AURAS; ++i)
    {
      ...
      for (AuraEffect const* effect : auraList)
      {
        ...
        handler->PSendSysMessage(LANG_COMMAND_TARGET_AURASIMPLE, effect->GetId(), effect->GetEffIndex(),
                                 effect->GetAmount());
      }
    }
