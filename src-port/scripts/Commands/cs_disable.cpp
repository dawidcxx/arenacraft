...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable disableCommandTable = {{"add", addDisableCommandTable},
                                                   {"remove", removeDisableCommandTable}};
    static ChatCommandTable commandTable        = {{"disable", disableCommandTable}};
    return commandTable;
  }

  static bool HandleAddDisables(ChatHandler* handler, uint32 entry, uint8 flags, std::string disableComment,
                                uint8 disableType)
  {
  ...
  static bool HandleAddDisableBattlegroundCommand(ChatHandler* handler, uint32 entry, uint8 flags,
                                                  std::string disableComment)
  {
    return HandleAddDisables(handler, entry, flags, disableComment, DISABLE_TYPE_BATTLEGROUND);
  }

  static bool HandleAddDisableAchievementCriteriaCommand(ChatHandler* handler, uint32 entry, uint8 flags,
                                                         std::string disableComment)
  {
    return HandleAddDisables(handler, entry, flags, disableComment, DISABLE_TYPE_ACHIEVEMENT_CRITERIA);
  }

  static bool HandleAddDisableOutdoorPvPCommand(ChatHandler* handler, uint32 entry, uint8 flags,
                                                std::string disableComment)
  {
