...
class spell_warr_mocking_blow : public SpellScript
{
  ...
  void Register() override { OnHit += SpellHitFn(spell_warr_mocking_blow::HandleOnHit); }
};
...
class spell_warr_victory_rush : public SpellScript
{
  ...
  void Register() override { AfterHit += SpellHitFn(spell_warr_victory_rush::VictoryRushHit); }
};
...
class spell_warr_improved_spell_reflection : public AuraScript
{
  ...
  void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    eventInfo.GetActor()->CastCustomSpell(SPELL_WARRIOR_IMPROVED_SPELL_REFLECTION_TRIGGER, values, eventInfo.GetActor(),
...
class spell_warr_improved_spell_reflection_trigger : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_WARRIOR_SPELL_REFLECTION}); }
...
class spell_warr_improved_spell_reflection_trigger_aura : public AuraScript
{
  ...
  void Register() override
  {
    AfterEffectRemove += AuraEffectRemoveFn(spell_warr_improved_spell_reflection_trigger_aura::HandleRemove, EFFECT_0,
                                            SPELL_AURA_REFLECT_SPELLS, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_warr_last_stand : public SpellScript
{
  ...
  void HandleDummy(SpellEffIndex /*effIndex*/)
  {
    ...
    caster->CastCustomSpell(caster, SPELL_WARRIOR_LAST_STAND_TRIGGERED, &healthModSpellBasePoints0, nullptr, nullptr,
                            true, nullptr);
  }
...
class spell_warr_deep_wounds : public SpellScript
{
  ...
  void HandleDummy(SpellEffIndex /*effIndex*/)
  {
    ...
    if (Unit* target = GetHitUnit())
    {
      ...
      target->CastDelayedSpellWithPeriodicAmount(caster, SPELL_WARRIOR_DEEP_WOUNDS_RANK_PERIODIC,
                                                 SPELL_AURA_PERIODIC_DAMAGE, damage, EFFECT_0);
...
class spell_warr_charge : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override
  {
    return ValidateSpellInfo(
        {SPELL_WARRIOR_JUGGERNAUT_CRIT_BONUS_TALENT, SPELL_WARRIOR_JUGGERNAUT_CRIT_BONUS_BUFF, SPELL_WARRIOR_CHARGE});
  }
...
class spell_warr_slam : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_WARRIOR_SLAM}); }
  ...
  {
    if (GetHitUnit())
      GetCaster()->CastCustomSpell(SPELL_WARRIOR_SLAM, SPELLVALUE_BASE_POINT0, GetEffectValue(), GetHitUnit(),
                                   TRIGGERED_FULL_MASK);
  }
...
class spell_warr_damage_shield : public AuraScript
{
  ...
  void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    GetTarget()->CastCustomSpell(SPELL_WARRIOR_DAMAGE_SHIELD_DAMAGE, SPELLVALUE_BASE_POINT0, damage,
...
class spell_warr_execute : public SpellScript
{
  ...
  void HandleEffect(SpellEffIndex effIndex)
  {
    ...
    if (Unit* target = GetHitUnit())
    {
      ...
      int32 rageUsed = std::min<int32>(300 - spellInfo->CalcPowerCost(caster, SpellSchoolMask(spellInfo->SchoolMask)),
                                       caster->GetPower(POWER_RAGE));
      int32 newRage  = std::max<int32>(0, caster->GetPower(POWER_RAGE) - rageUsed);

      // Sudden Death rage save
      if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_GENERIC,
                                                     WARRIOR_ICON_ID_SUDDEN_DEATH, EFFECT_0))
      {
      ...
      caster->CastCustomSpell(target, SPELL_WARRIOR_EXECUTE, &bp, nullptr, nullptr, true, nullptr, nullptr,
...
class spell_warr_bloodthirst : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_WARRIOR_BLOODTHIRST}); }
...
class spell_warr_overpower : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override
  {
    return ValidateSpellInfo({SPELL_WARRIOR_UNRELENTING_ASSAULT_RANK_1, SPELL_WARRIOR_UNRELENTING_ASSAULT_TRIGGER_1,
                              SPELL_WARRIOR_UNRELENTING_ASSAULT_RANK_2, SPELL_WARRIOR_UNRELENTING_ASSAULT_TRIGGER_2});
  }
...
class spell_warr_intimidating_shout : public SpellScript
{
  ...
  void FilterTargets(std::list<WorldObject*>& unitList) { unitList.remove(GetExplTargetWorldObject()); }

  void Register() override
  {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_warr_intimidating_shout::FilterTargets, EFFECT_1,
                                                              TARGET_UNIT_SRC_AREA_ENEMY);
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_warr_intimidating_shout::FilterTargets, EFFECT_2,
                                                              TARGET_UNIT_SRC_AREA_ENEMY);
  }
};
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    if (DamageInfo* damageInfo = eventInfo.GetDamageInfo())
    {
      ...
      else
      {
        ...
        GetTarget()->CastCustomSpell(_procTarget, SPELL_WARRIOR_SWEEPING_STRIKES_EXTRA_ATTACK_1, &damage, 0, 0, true,
                                     nullptr, aurEff);
      }
    }
  }
...
class spell_warr_vigilance : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override
  {
    return ValidateSpellInfo({SPELL_WARRIOR_GLYPH_OF_VIGILANCE, SPELL_WARRIOR_VIGILANCE_PROC,
                              SPELL_WARRIOR_VIGILANCE_REDIRECT_THREAT, SPELL_GEN_DAMAGE_REDUCTION_AURA,
                              SPELL_PALADIN_BLESSING_OF_SANCTUARY, SPELL_PALADIN_GREATER_BLESSING_OF_SANCTUARY,
  ...
  void HandleRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
  {
    ...
    if (target->HasAura(SPELL_GEN_DAMAGE_REDUCTION_AURA) &&
        !(target->HasAura(SPELL_PALADIN_BLESSING_OF_SANCTUARY) ||
          target->HasAura(SPELL_PALADIN_GREATER_BLESSING_OF_SANCTUARY) || target->HasAura(SPELL_PRIEST_RENEWED_HOPE)))
    {
  ...
  void Register() override
  {
    OnEffectApply += AuraEffectApplyFn(spell_warr_vigilance::HandleApply, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL,
                                       AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    AfterEffectApply += AuraEffectApplyFn(spell_warr_vigilance::HandleAfterApply, EFFECT_0,
                                          SPELL_AURA_PROC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    OnEffectRemove += AuraEffectRemoveFn(spell_warr_vigilance::HandleRemove, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL,
                                         AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
...
class spell_warr_vigilance_trigger : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_WARRIOR_TAUNT}); }
...
class spell_warr_glyph_of_sunder_armor : public AuraScript
{
  ...
  void Register() override
  {
    DoEffectCalcSpellMod += AuraEffectCalcSpellModFn(spell_warr_glyph_of_sunder_armor::HandleEffectCalcSpellMod,
...
void AddSC_warrior_spell_scripts()
{
  ...
  RegisterSpellAndAuraScriptPair(spell_warr_improved_spell_reflection_trigger,
