...
// Player Eq
bool operator==(const SoloqPlayer& lhs, const SoloqPlayer& rhs) { return lhs.playerGUID == rhs.playerGUID; }

// Player Hash
struct SoloqPlayerHash
{
  std::size_t operator()(const SoloqPlayer& player) const { return std::hash<uint64_t>{}(player.playerGUID); }
};
