...
async function setupConfigFiles() {
    for await (const file of $`ls ${DEST}/etc`.lines()) {
        if (file.endsWith('.dist')) {
            ...
            await $`cp -n ${DEST}/etc/${file} ${DEST}/etc/${newFile}`.quiet().nothrow()
        }
    }
    for await (const file of $`ls ${DEST}/etc/modules`.lines()) {
        if (file.endsWith('.dist')) {
            const newFile = file.replace('.dist', '')
            await $`cp -n ${DEST}/etc/modules/${file} ${DEST}/etc/modules/${newFile}`.quiet().nothrow()
        }
    }
}

const systemdWorldService = ['arenacraft-world', `
[Unit]
...
const systemdAuthService = ['arenacraft-auth', `
[Unit]
...
for (const [name, content] of [systemdWorldService, systemdAuthService]) {
    ...
    await Bun.file(`${process.env.HOME}/.config/systemd/user/${name}.service`).write(content)
}
