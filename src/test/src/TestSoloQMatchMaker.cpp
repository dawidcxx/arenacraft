...
TEST_CASE("MatchMaker scenario test suite")
{
  ...
  SUBCASE("The most basic example should generate a valid match")
  {
    ...
    CHECK_MESSAGE(matchmaker.GetInfo().GetTotalPlayerCount() == 6,
                  "There should be 6 players in Queue, got " << matchmaker.GetInfo().GetTotalPlayerCount());
    CHECK_MESSAGE(matchmaker.GetInfo().classToCount[arenacraft::CLASS_WARRIOR] == 1,
                  "There should be a arenacraft::CLASS_WARRIOR in queue");
    ...
    CHECK_MESSAGE(matchmaker.GetInfo().GetTotalPlayerCount() == 0,
                  "Queue needs to be cleared after calling .PopMatchups()");
    CHECK_MESSAGE(matchmaker.GetInfo().classToCount[arenacraft::CLASS_WARRIOR] == 0,
                  "arenacraft::CLASS_WARRIOR should be removed from queue");
  }
  ...
  SUBCASE("Prevent class stacking")
  {
    ...
    CHECK_MESSAGE(matchmaker.GetInfo().GetTotalPlayerCount() == 6,
                  "There should be 6 players in Queue, got " << matchmaker.GetInfo().GetTotalPlayerCount());
    CHECK_MESSAGE(matchmaker.GetInfo().classToCount[arenacraft::CLASS_PALADIN] == 3,
                  "There should be 3 paladins in queue");
  ...
  SUBCASE("Queue player mmr extension should work properly")
  {
    ...
    arenacraft::soloq::SoloqPlayer player1{1, arenacraft::CLASS_WARRIOR, 1001, 0}; // arms warrior       - melee
    arenacraft::soloq::SoloqPlayer player3{2, arenacraft::CLASS_WARLOCK, 1080, 0}; // affliction warlock - caster
    arenacraft::soloq::SoloqPlayer player2{3, arenacraft::CLASS_PALADIN, 1400,
                                           0}; // holy paladin       - healer  <= High rated player
    arenacraft::soloq::SoloqPlayer player4{4, arenacraft::CLASS_DEATH_KNIGHT, 1010, 2}; // unholy dk          - melee
    arenacraft::soloq::SoloqPlayer player6{5, arenacraft::CLASS_SHAMAN, 1002, 0};       // elemental shaman   - caster
    arenacraft::soloq::SoloqPlayer player5{6, arenacraft::CLASS_PRIEST, 1090, 1};       // holy priest        - healer
  ...
  SUBCASE("Benchmark")
  {
    ...
    std::cout << "Matchmaking " << totalPlayers << " players took " << duration.count() << "ms. "
              << "Found " << matchups.size() << " matchups" << std::endl;
