...
MySQLConnection::MySQLConnection(MySQLConnectionInfo& connInfo)
    : m_reconnecting(false), m_prepareError(false), m_Mysql(nullptr), m_queue(nullptr), m_connectionInfo(connInfo),
      m_connectionFlags(CONNECTION_SYNCH)
{
}

MySQLConnection::MySQLConnection(ProducerConsumerQueue<SQLOperation*>* queue, MySQLConnectionInfo& connInfo)
    : m_reconnecting(false), m_prepareError(false), m_Mysql(nullptr), m_queue(queue), m_connectionInfo(connInfo),
...
uint32 MySQLConnection::Open()
{
  ...
  m_Mysql = reinterpret_cast<MySQLHandle*>(
      mysql_real_connect(mysqlInit, m_connectionInfo.host.c_str(), m_connectionInfo.user.c_str(),
  ...
  {
    LOG_ERROR("sql.driver", "Could not connect to MySQL database at {}: {}", m_connectionInfo.host,
...
bool MySQLConnection::Execute(PreparedStatementBase* stmt)
{
...
#endif
  {
    ...
    if (_HandleMySQLErrno(
            lErrno,
            mysql_stmt_error(msql_STMT))) // If it returns true, an error was handled successfully (i.e. reconnection)
...
bool MySQLConnection::_Query(std::string_view sql, MySQLResult** pResult, MySQLField** pFields, uint64* pRowCount,
                             uint32* pFieldCount)
{
  if (!m_Mysql)
    return false;
...
uint32 MySQLConnection::GetServerVersion() const { return mysql_get_server_version(m_Mysql); }

std::string MySQLConnection::GetServerInfo() const { return mysql_get_server_info(m_Mysql); }

MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
  ASSERT(index < m_stmts.size(),
         "Tried to access invalid prepared statement index {} (max index {}) on database `{}`, connection type: {}",
         index, m_stmts.size(), m_connectionInfo.database,
         (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous" : "synchronous");

  MySQLPreparedStatement* ret = m_stmts[index].get();

  if (!ret)
    LOG_ERROR("sql.sql", "Could not fetch prepared statement {} on database `{}`, connection type: {}.", index,
              m_connectionInfo.database, (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous" : "synchronous");
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo, char const* err, uint8 attempts /*= 5*/)
{
  ...
  case CR_CONN_HOST_ERROR:
  {
    ...
    if (!lErrno)
    {
      ...
      LOG_INFO("sql.sql", "Successfully reconnected to {} @{}:{} ({}).", m_connectionInfo.database,
  ...
  case ER_NO_SUCH_TABLE:
    str = "Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates "
          "folders.";
    LOG_FATAL("sql.sql", "{}", str);
    std::this_thread::sleep_for(10s);
    ABORT("{}\n\n[{}] {}", str, errNo, err);
