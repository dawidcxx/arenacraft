...
template <typename T> inline void Destroy(T& t) { t.~T(); }
...
// Not using initialization lists to work around segmentation faults when compiling with clang without precompiled
// headers
...
{
  return WithChainingPreparedCallback([callback](QueryCallback& /*this*/, PreparedQueryResult result)
...
void QueryCallback::SetNextQuery(QueryCallback&& next) { MoveFrom(this, std::move(next)); }
