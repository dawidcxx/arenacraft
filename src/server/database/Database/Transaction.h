...
public:
  ...
  template <typename... Args> void Append(std::string_view sql, Args&&... args)
  {
...
template <typename T> class Transaction : public TransactionBase
{
public:
  using TransactionBase::Append;

  void Append(PreparedStatement<T>* statement) { AppendPreparedStatement(statement); }
};
