...
UpdateFetcher::UpdateFetcher(Path const& sourceDirectory, std::function<void(std::string const&)> const& apply,
                             std::function<void(Path const& path)> const&          applyFile,
                             std::function<QueryResult(std::string const&)> const& retrieve,
                             std::string const& dbModuleName, std::string_view modulesList /*= {}*/)
    : _sourceDirectory(std::make_unique<Path>(sourceDirectory)), _apply(apply), _applyFile(applyFile),
...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state,
                                            uint32 const depth) const
{
  ...
  for (directory_iterator itr(path); itr != end; ++itr)
  {
    ...
    {
      LOG_TRACE("sql.updates", "Added locale file \"{}\" state '{}'.", itr->path().filename().generic_string(),
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
  ...
  else
  {
    ...
    do
    {
      ...
      if (!is_directory(p))
      {
        LOG_WARN("sql.updates", "DBUpdater: Given update include directory \"{}\" does not exist, skipped!",
                 p.generic_string());
        continue;
      }
      ...
      LOG_TRACE("sql.updates", "Added applied file \"{}\" '{}' state from remote.", p.filename().generic_string(),
                state);
                                   ...
                                   int32 const cleanDeadReferencesMaxCount) const
{
  ...
  auto ApplyUpdateFile = [&](LocaleFileEntry const& sqlFile)
  {
    ...
    if (iter != applied.end())
    {
      ...
      // If the update is in an archived directory and is marked as archived in our database, skip redundancy checks
      // (archived updates never change).
    ...
    if (iter == applied.end())
    {
      ...
      if (hashIter != hashToName.end())
      {
        ...
        {
          LOG_INFO("sql.updates", ">> Renaming update \"{}\" to \"{}\" \'{}\'.", hashIter->second,
      ...
      {
        LOG_INFO("sql.updates", ">> Applying update \"{}\" \'{}\'...", filePath.filename().string(), hash.substr(0, 7));
      }
    }
    ...
    else
    {
      ...
      else
      {
        ...
        {
          LOG_DEBUG("sql.updates", ">> Updating the state of \"{}\" to \'{}\'...", filePath.filename().string(),
