...
bool DBUpdaterUtil::CheckExecutable()
{
  ...
  if (!is_regular_file(exe))
  {
    ...
    LOG_FATAL("sql.updates",
              "Didn't find any executable MySQL binary at \'{}\' or in path, correct the path in the *.conf "
              "(\"MySQLExecutable\").",
...
template <class T> bool DBUpdater<T>::Create(DatabaseWorkerPool<T>& pool)
{
  ...
  try
  {
    DBUpdater<T>::ApplyFile(pool, pool.GetConnectionInfo()->host, pool.GetConnectionInfo()->user,
  ...
  {
    LOG_FATAL("sql.updates",
              "Failed to create database {}! Does the user (named in *.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` "
              "and `DELETE` privileges on the MySQL server?",
...
template <class T> bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool, std::string_view modulesList /*= {}*/)
{
  ...
  if (!is_directory(sourceDirectory))
  {
    LOG_ERROR("sql.updates",
              "DBUpdater: The given source directory {} does not exist, change the path to the directory where your "
              "sql directory exists (for example c:\\source\\azerothcore). Shutting down.",
  ...
  auto CheckUpdateTable = [&](std::string const& tableName)
  {
    ...
    if (!checkTable)
    {
      ...
      {
        LOG_FATAL("sql.updates",
                  "Failed apply file to database {}! Does the user (named in *.conf) have `INSERT` and `DELETE` "
  ...
  UpdateFetcher updateFetcher(
      sourceDirectory, [&](std::string const& query) { DBUpdater<T>::Apply(pool, query); },
      [&](Path const& file) { DBUpdater<T>::ApplyFile(pool, file); }, [&](std::string const& query) -> QueryResult
      { return DBUpdater<T>::Retrieve(pool, query); }, DBUpdater<T>::GetDBModuleName(), modulesList);
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool, std::vector<std::string> const* setDirectories)
{
  ...
  UpdateFetcher updateFetcher(
      sourceDirectory, [&](std::string const& query) { DBUpdater<T>::Apply(pool, query); },
      [&](Path const& file) { DBUpdater<T>::ApplyFile(pool, file); }, [&](std::string const& query) -> QueryResult
      { return DBUpdater<T>::Retrieve(pool, query); }, DBUpdater<T>::GetDBModuleName(), setDirectories);
...
void DBUpdater<T>::ApplyFile(DatabaseWorkerPool<T>& pool, std::string const& host, std::string const& user,
                             std::string const& password, std::string const& port_or_socket,
