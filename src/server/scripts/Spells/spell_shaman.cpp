...
class spell_sha_totem_of_wrath : public SpellScript
{
  ...
  void Register() override { AfterCast += SpellCastFn(spell_sha_totem_of_wrath::HandleAfterCast); }
};
...
class spell_sha_spirit_walk : public SpellScript
{
  ...
  void Register() override { OnCheckCast += SpellCheckCastFn(spell_sha_spirit_walk::CheckCast); }
};
...
class spell_sha_t10_restoration_4p_bonus : public AuraScript
{
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    eventInfo.GetProcTarget()->CastDelayedSpellWithPeriodicAmount(GetTarget(), triggered_spell_id,
                                                                  SPELL_AURA_PERIODIC_HEAL, amount, EFFECT_0);
  }
...
class spell_sha_totemic_mastery : public AuraScript
{
  ...
  {
    OnEffectPeriodic +=
        AuraEffectPeriodicFn(spell_sha_totemic_mastery::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
  }
};
...
class spell_sha_feral_spirit_scaling : public AuraScript
{
  ...
  void Register() override
  {
    if (m_scriptSpellId == 35675)
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_feral_spirit_scaling::CalculateResistanceAmount,
                                                   EFFECT_ALL, SPELL_AURA_MOD_RESISTANCE);

    if (m_scriptSpellId == 35674)
    {
      DoEffectCalcAmount +=
          AuraEffectCalcAmountFn(spell_sha_feral_spirit_scaling::CalculateStatAmount, EFFECT_ALL, SPELL_AURA_MOD_STAT);
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_feral_spirit_scaling::CalculateAPAmount, EFFECT_ALL,
                                                   SPELL_AURA_MOD_ATTACK_POWER);
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_feral_spirit_scaling::CalculateSPAmount, EFFECT_ALL,
                                                   SPELL_AURA_MOD_DAMAGE_DONE);
    }

    OnEffectApply += AuraEffectApplyFn(spell_sha_feral_spirit_scaling::HandleEffectApply, EFFECT_ALL, SPELL_AURA_ANY,
...
class spell_sha_fire_elemental_scaling : public AuraScript
{
  ...
  void Register() override
  {
    if (m_scriptSpellId != 35665 && m_scriptSpellId != 65225)
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_fire_elemental_scaling::CalculateResistanceAmount,
                                                   EFFECT_ALL, SPELL_AURA_MOD_RESISTANCE);

    if (m_scriptSpellId == 35666 || m_scriptSpellId == 65226)
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_fire_elemental_scaling::CalculateStatAmount, EFFECT_ALL,
                                                   SPELL_AURA_MOD_STAT);

    if (m_scriptSpellId == 35665 || m_scriptSpellId == 65225)
    {
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_fire_elemental_scaling::CalculateAPAmount, EFFECT_ALL,
                                                   SPELL_AURA_MOD_ATTACK_POWER);
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_fire_elemental_scaling::CalculateSPAmount, EFFECT_ALL,
                                                   SPELL_AURA_MOD_DAMAGE_DONE);
    }

    OnEffectApply += AuraEffectApplyFn(spell_sha_fire_elemental_scaling::HandleEffectApply, EFFECT_ALL, SPELL_AURA_ANY,
                                       AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_sha_ancestral_awakening_proc : public SpellScript
{
  ...
  void HandleDummy(SpellEffIndex /*effIndex*/)
  {
    ...
    if (GetHitUnit())
      GetCaster()->CastCustomSpell(GetHitUnit(), SPELL_SHAMAN_ANCESTRAL_AWAKENING_PROC, &damage, nullptr, nullptr,
                                   true);
  }

  void Register() override
  {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_sha_ancestral_awakening_proc::FilterTargets,
...
class spell_sha_bloodlust : public SpellScript
{
  ...
  void Register() override
  {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_sha_bloodlust::RemoveInvalidTargets, EFFECT_ALL,
...
class spell_sha_chain_heal : public SpellScript
{
  ...
  void HandleHeal(SpellEffIndex /*effIndex*/)
  {
    if (firstHeal)
    {
      ...
      if (AuraEffect* aurEff = GetHitUnit()->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELLFAMILY_SHAMAN, 0, 0, 0x10,
                                                           GetCaster()->GetGUID()))
      {
        riptide = true;
        // Consume it
...
class spell_sha_cleansing_totem_pulse : public SpellScript
{
  ...
  void HandleDummy(SpellEffIndex /*effIndex*/)
  {
    ...
    if (GetCaster() && GetHitUnit() && GetOriginalCaster())
      GetCaster()->CastCustomSpell(GetHitUnit(), SPELL_SHAMAN_CLEANSING_TOTEM_EFFECT, nullptr, &bp, nullptr, true,
                                   nullptr, nullptr, GetOriginalCaster()->GetGUID());
  }
...
class spell_sha_earth_shield : public AuraScript
{
  ...
  {
    PreventDefaultAction();
    GetTarget()->CastCustomSpell(SPELL_SHAMAN_EARTH_SHIELD_HEAL, SPELLVALUE_BASE_POINT0, aurEff->GetAmount(),
                                 GetTarget(), true, nullptr, aurEff, GetCasterGUID());
...
class spell_sha_earthbind_totem : public AuraScript
{
  ...
  void Register() override
  {
    OnEffectPeriodic += AuraEffectPeriodicFn(spell_sha_earthbind_totem::HandleEffectPeriodic, EFFECT_0,
                                             SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    OnEffectApply += AuraEffectApplyFn(spell_sha_earthbind_totem::Apply, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL,
                                       AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_sha_earthliving_weapon : public AuraScript
{
  ...
  bool CheckProc(ProcEventInfo& eventInfo)
  {
    ...
    if (AuraEffect const* aurEff =
            caster->GetAuraEffectOfRankedSpell(SPELL_SHAMAN_BLESSING_OF_THE_ETERNALS_R1, EFFECT_1, caster->GetGUID()))
    {
  ...
  void Register() override { DoCheckProc += AuraCheckProcFn(spell_sha_earthliving_weapon::CheckProc); }
};
...
class spell_sha_flame_shock : public AuraScript
{
  ...
  void Register() override { AfterDispel += AuraDispelFn(spell_sha_flame_shock::HandleDispel); }
};
    ...
    if (Unit* target = GetHitUnit())
      if (Unit* caster = GetCaster())
      {
        if (Unit* owner = caster->GetOwner())
        {
          ...
          // Restorative Totems
          if (AuraEffect* dummy =
                  owner->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN, SHAMAN_ICON_ID_RESTORATIVE_TOTEMS, 1))
        ...
        caster->CastCustomSpell(target, SPELL_SHAMAN_TOTEM_HEALING_STREAM_HEAL, &damage, 0, 0, true, 0, 0,
...
class spell_sha_heroism : public SpellScript
{
  ...
  {
    OnObjectAreaTargetSelect +=
        SpellObjectAreaTargetSelectFn(spell_sha_heroism::RemoveInvalidTargets, EFFECT_0, TARGET_UNIT_CASTER_AREA_RAID);
    OnObjectAreaTargetSelect +=
        SpellObjectAreaTargetSelectFn(spell_sha_heroism::RemoveInvalidTargets, EFFECT_1, TARGET_UNIT_CASTER_AREA_RAID);
    OnObjectAreaTargetSelect +=
        SpellObjectAreaTargetSelectFn(spell_sha_heroism::RemoveInvalidTargets, EFFECT_2, TARGET_UNIT_CASTER_AREA_RAID);
...
class spell_sha_item_lightning_shield_trigger : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_SHAMAN_ITEM_MANA_SURGE}); }
  ...
  {
    OnEffectProc +=
        AuraEffectProcFn(spell_sha_item_lightning_shield_trigger::HandleProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
  }
};
...
class spell_sha_item_mana_surge : public AuraScript
{
  ...
  bool CheckProc(ProcEventInfo& eventInfo) { return eventInfo.GetSpellInfo() != nullptr; }
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    GetTarget()->CastCustomSpell(SPELL_SHAMAN_ITEM_MANA_SURGE, SPELLVALUE_BASE_POINT0, damage, GetTarget(), true,
                                 nullptr, aurEff);
  }
...
class spell_sha_item_t10_elemental_2p_bonus : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_SHAMAN_ELEMENTAL_MASTERY}); }
    ...
    if (Unit* target = GetHitUnit())
      if (Unit* caster = GetCaster())
        if (target->HasActivePowerType(POWER_MANA))
          caster->CastCustomSpell(target, SPELL_SHAMAN_MANA_SPRING_TOTEM_ENERGIZE, &damage, 0, 0, true, 0, 0,
                                  GetOriginalCaster()->GetGUID());
  }
    ...
    if (Unit* caster = GetCaster())
      if (Unit* unitTarget = GetHitUnit())
      {
        if (unitTarget->HasActivePowerType(POWER_MANA))
        {
          ...
          caster->CastCustomSpell(unitTarget, SPELL_SHAMAN_MANA_TIDE_TOTEM, &effBasePoints0, nullptr, nullptr, true,
                                  nullptr, nullptr, GetOriginalCaster()->GetGUID());
        }
      }
  }
...
class spell_sha_sentry_totem : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spell*/) override { return ValidateSpellInfo({SPELL_SHAMAN_BIND_SIGHT}); }
  ...
  {
    AfterEffectRemove +=
        AuraEffectRemoveFn(spell_sha_sentry_totem::AfterRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_sha_flurry_proc : public AuraScript
{
  ...
  void Register() override { DoCheckProc += AuraCheckProcFn(spell_sha_flurry_proc::CheckProc); }
};
...
class spell_sha_t8_electrified : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_SHAMAN_ELECTRIFIED}); }
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    eventInfo.GetProcTarget()->CastDelayedSpellWithPeriodicAmount(eventInfo.GetActor(), SPELL_SHAMAN_ELECTRIFIED,
...
class spell_sha_acenstral_winds : public AuraScript
{
  ...
  void Register() override { DoCheckProc += AuraCheckProcFn(spell_sha_acenstral_winds::CheckProc); }
};
