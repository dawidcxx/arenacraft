...
static uint32 argentPonyService[2][3] = {{ARGENT_PONY_STATE_MAILBOX, ARGENT_PONY_STATE_VENDOR, ARGENT_PONY_STATE_BANK},
...
static argentPonyBanner argentBanners[MAX_RACES] = {{0, 0, ""},
...
struct npc_pet_gen_argent_pony_bridle : public ScriptedAI
{
  ...
  bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action)
  {
    ...
    if (spellId && !creature->HasAura(spellId))
    {
      ...
      player->AddSpellCooldown(player->GetTeamId(true) ? SPELL_AURA_TIRED_G : SPELL_AURA_TIRED_S, 0,
...
{
  npc_pet_gen_gnomish_flame_turret(Creature* c) : ScriptedAI(c) { checkTimer = 0; }
...
class spell_pet_gen_valkyr_guardian_smite : public SpellScript
{
  ...
  void Register() override { OnHit += SpellHitFn(spell_pet_gen_valkyr_guardian_smite::RecalculateDamage); }
};
    ...
    if (TempSummon* summon = me->ToTempSummon())
      if (Unit* owner = summon->GetSummonerUnit())
        if (owner->IsPlayer())
        {
          ...
          {
            _hasParty               = true;
            std::string const& text = sCreatureTextMgr->GetLocalizedChatString(me->GetEntry(), 0, 0 /*text group*/,
                                                                               urand(0, 60) /*text id*/, LOCALE_enUS);
