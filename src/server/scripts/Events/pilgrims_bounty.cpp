      ...
      if (timerSpawnPlate >= 1000)
      {
        if (Vehicle* table = me->GetVehicle())
          if (Unit* plateHolder = table->GetPassenger(6))
          {
            ...
            Creature* plate =
                me->SummonCreature(NPC_STRUDY_PLATE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f);
            if (!plate)
              return;
        ...
        if (Player* player = target->GetCharmerOrOwnerPlayerOrPlayerItself())
        {
          ...
          if (AuraEffect* aur = target->GetAuraEffectDummy(spellId))
          {
            if (aur->GetBase()->GetStackAmount() >= 5)
            {
              ...
              {
                casterPlayer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL2,
...
class spell_pilgrims_bounty_turkey_tracker : public SpellScript
{
  ...
  {
    OnEffectHitTarget +=
        SpellEffectFn(spell_pilgrims_bounty_turkey_tracker::HandleScriptEffect, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
  }
};
...
class spell_pilgrims_bounty_serve_generic : public AuraScript
{
  ...
  void Register() override
  {
    OnEffectRemove += AuraEffectRemoveFn(spell_pilgrims_bounty_serve_generic::OnAuraRemove, EFFECT_0, SPELL_AURA_DUMMY,
...
class spell_pilgrims_bounty_food_aura : public AuraScript
{
  ...
  void Register() override
  {
    if (m_scriptSpellId == 66041)
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pilgrims_bounty_food_aura::RecalculateHook, EFFECT_0,
                                                   SPELL_AURA_MOD_POWER_REGEN);
    else if (m_scriptSpellId != 66477)
      DoEffectCalcAmount +=
          AuraEffectCalcAmountFn(spell_pilgrims_bounty_food_aura::RecalculateHook, EFFECT_0, SPELL_AURA_MOD_REGEN);
  }
};
