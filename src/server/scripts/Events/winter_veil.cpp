...
class spell_winter_wondervolt_trap : public SpellScript
{
  ...
  void HandleScriptEffect(SpellEffIndex /*effIndex*/)
  {
    if (Player* target = GetHitPlayer())
    {
      ...
      {
        spellId =
            target->GetTeamId() == TEAM_ALLIANCE ? SPELL_WINTER_WONDERVOLT_GREEN_MAN : SPELL_WINTER_WONDERVOLT_RED_MAN;
      }
      else
      {
...
class spell_winter_veil_racer_rocket_slam : public SpellScript
{
  ...
  {
    OnEffectLaunch +=
        SpellEffectFn(spell_winter_veil_racer_rocket_slam::HandleTriggerSpell, EFFECT_1, SPELL_EFFECT_TRIGGER_SPELL);
  }
};
...
class spell_winter_veil_racer_slam_hit : public SpellScript
{
  ...
  void HandleScriptEffect(SpellEffIndex /*effIndex*/)
  {
    ...
    target->CastSpell(target->GetPositionX() + irand(-10, 10), target->GetPositionY() + irand(-10, 10),
                      target->GetPositionZ(), SPELL_RACER_DEATH_VISUAL, true);
    ...
    {
      // targetSummoner->RemoveCriteriaProgress(sAchievementCriteriaStore.LookupEntry(RACER_ACHI_CRITERIA)); !ZOMG, ADD
      // ACCESSOR
...
class spell_winter_veil_shoot_air_rifle : public SpellScript
{
  ...
  void Register() override
  {
    if (m_scriptSpellId == SPELL_AIR_RIFLE_HIT_TRIGGER)
      OnEffectHitTarget +=
          SpellEffectFn(spell_winter_veil_shoot_air_rifle::HandleScriptEffect, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
    else
      OnEffectHitTarget +=
          SpellEffectFn(spell_winter_veil_shoot_air_rifle::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
  }
};
