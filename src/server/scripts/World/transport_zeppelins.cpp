...
struct go_transport_the_iron_eagle : GameObjectAI
{
  ...
  {
    sWorldState->HandleConditionStateChange(WORLD_STATE_CONDITION_THE_IRON_EAGLE,
                                            static_cast<WorldStateConditionState>(eventId));
    switch (eventId)
    {
...
struct go_transport_the_thundercaller : GameObjectAI
{
  ...
  void EventInform(uint32 eventId) override
  {
    sWorldState->HandleConditionStateChange(WORLD_STATE_CONDITION_THE_THUNDERCALLER,
                                            static_cast<WorldStateConditionState>(eventId));
    switch (eventId)
    {
...
struct go_transport_the_purple_princess : GameObjectAI
{
  ...
  void EventInform(uint32 eventId) override
  {
    sWorldState->HandleConditionStateChange(WORLD_STATE_CONDITION_THE_PURPLE_PRINCESS,
                                            static_cast<WorldStateConditionState>(eventId));
    switch (eventId)
    {
