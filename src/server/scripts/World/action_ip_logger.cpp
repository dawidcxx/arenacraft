...
public:
  ...
  // ACCOUNT_LOGIN = 0
  void OnAccountLogin(uint32 accountId) override { AccountIPLogAction(accountId, ACCOUNT_LOGIN); }

  // We log last_attempt_ip instead of last_ip, as failed login doesn't necessarily mean approperiate user
  // ACCOUNT_FAIL_LOGIN = 1
  void OnFailedAccountLogin(uint32 accountId) override { AccountIPLogAction(accountId, ACCOUNT_FAIL_LOGIN); }

  // ACCOUNT_CHANGE_PW = 2
  void OnPasswordChange(uint32 accountId) override { AccountIPLogAction(accountId, ACCOUNT_CHANGE_PW); }

  // ACCOUNT_CHANGE_PW_FAIL = 3
  void OnFailedPasswordChange(uint32 accountId) override { AccountIPLogAction(accountId, ACCOUNT_CHANGE_PW_FAIL); }
  ...
  void OnFailedEmailChange(uint32 accountId) override { AccountIPLogAction(accountId, ACCOUNT_CHANGE_EMAIL_FAIL); }
  ...
  void AccountIPLogAction(uint32 accountId, IPLoggingTypes aType)
  {
    ...
    /*case ACCOUNT_LOGOUT:
        ...
        break;*/
    // Neither should happen. Ever. Period. If it does, call Ghostbusters and all your local software defences to
    // investigate.
    case UNKNOWN_ACTION:
    default:
...
public:
  ...
  // CHARACTER_CREATE = 7
  void OnCreate(Player* player) override { CharacterIPLogAction(player, CHARACTER_CREATE); }

  // CHARACTER_LOGIN = 8
  void OnLogin(Player* player) override { CharacterIPLogAction(player, CHARACTER_LOGIN); }

  // CHARACTER_LOGOUT = 9
  void OnLogout(Player* player) override { CharacterIPLogAction(player, CHARACTER_LOGOUT); }
...
public:
  ...
  // CHARACTER_DELETE = 10
  void OnDelete(ObjectGuid guid, uint32 accountId) override { DeleteIPLogAction(guid, accountId, CHARACTER_DELETE); }
  ...
  void DeleteIPLogAction(ObjectGuid guid, ObjectGuid::LowType playerGuid, IPLoggingTypes aType)
  {
    ...
    // We declare all the required variables
    ObjectGuid::LowType characterGuid = guid.GetCounter(); // We have no access to any member function of Player* or
                                                           // WorldSession*. So use old-fashioned way.
