...
engineering. SDCategory: NPCs EndScriptData */
...
#define BOX_UNLEARN_ENGIN_SPEC                                                                                         \
  "Do you really want to unlearn your engineering specialty and lose all associated recipes? \n Cost: "
  ...
  for (uint8 i = 0; i < 3; ++i)
  {
    ...
    for (uint8 j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j)
    {
      ...
      if (item && item->GetTemplate()->RequiredSpell == reqSpell)
      {
        ...
        LOG_DEBUG("scripts.ai", "Player attempt to unlearn spell {}, but item {} is equipped.", reqSpell,
                  item->GetEntry());
        return false;
      }
    }
...
public:
          ...
          player->GetQuestRewardStatus(Q_MASTER_POTION))
      {
        ...
        case N_TRAINER_TRANSMUTE: // Zarevhi
          if (!HasAlchemySpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_TRANSMUTE, GOSSIP_SENDER_LEARN,
        ...
        case N_TRAINER_ELIXIR: // Lorokeem
          if (!HasAlchemySpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_ELIXIR, GOSSIP_SENDER_LEARN,
        ...
        case N_TRAINER_POTION: // Lauranna Thar'well
          if (!HasAlchemySpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_POTION, GOSSIP_SENDER_LEARN,
  ...
  void SendConfirmUnlearn(Player* player, Creature* creature, uint32 action)
  {
    if (action)
    {
      ...
      {
      case N_TRAINER_TRANSMUTE: // Zarevhi
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_TRANSMUTE, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_ALCHEMY_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
      case N_TRAINER_ELIXIR: // Lorokeem
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_ELIXIR, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_ALCHEMY_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
      case N_TRAINER_POTION: // Lauranna Thar'well
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_POTION, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_ALCHEMY_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
...
public:
          ...
          player->GetQuestRewardStatus(5283) || player->GetQuestStatus(5301))
      {
        ...
        case N_TRAINER_SMITHOMNI2: // Krathok Moltenfist
          if (!player->HasSpell(S_ARMOR) && !player->HasSpell(S_WEAPON))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_ARMOR_LEARN, GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF + 1);
          if (!player->HasSpell(S_WEAPON) && !player->HasSpell(S_ARMOR))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_WEAPON_LEARN, GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF + 2);
          break;
    ...
    if (player->HasSpell(S_WEAPON) && player->GetLevel() > 49 && player->GetBaseSkillValue(SKILL_BLACKSMITHING) >= 250)
    {
      ...
      case N_TRAINER_HAMMER: // Lilith the Lithe
        if (!HasWeaponSub(player))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_HAMMER, GOSSIP_SENDER_LEARN,
                           GOSSIP_ACTION_INFO_DEF + 5);
      ...
      case N_TRAINER_SWORD: // Seril Scourgebane
        if (!HasWeaponSub(player))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_SWORD, GOSSIP_SENDER_LEARN,
  ...
  void SendConfirmUnlearn(Player* player, Creature* creature, uint32 action)
  {
    if (action)
    {
      ...
      case N_TRAINER_ARMOR1:  // Grumnus Steelshaper
      case N_TRAINER_ARMOR2:  // Okothos Ironrager
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_SMITH_SPEC, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_ARMORORWEAPON,
      ...
      case N_TRAINER_HAMMER:
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_HAMMER, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_WEAPON_SPEC,
                         DoMedUnlearnCost(player), false);
        // unknown textID (TALK_HAMMER_UNLEARN)
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
      case N_TRAINER_AXE:
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_AXE, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_WEAPON_SPEC,
                         DoMedUnlearnCost(player), false);
        // unknown textID (TALK_AXE_UNLEARN)
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
      case N_TRAINER_SWORD:
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_SWORD, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_WEAPON_SPEC,
                         DoMedUnlearnCost(player), false);
...
public:
  ...
  bool OnGossipHello(Player* player, Creature* creature) override
  {
    ...
    case N_TRAINER_DRAGON2: // Thorkaf Dragoneye
      AddGossipItemFor(
          player, creature->GetEntry() == N_TRAINER_DRAGON1 ? GOSSIP_MENU_PETER_GALEN : GOSSIP_MENU_THORKAF_DRAGONEYE,
          GOSSIP_MENU_OPTION_TRAIN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRAIN);
      break;
        ...
        player->GetLevel() > 40)
    {
      ...
      case N_TRAINER_DRAGON2: // Thorkaf Dragoneye
        if (player->HasSpell(S_DRAGON))
          AddGossipItemFor(player,
                           ...
                           GOSSIP_MENU_OPTION_CONFIRM_UNLEARN_DRAGONSCALE, GOSSIP_SENDER_MAIN,
      ...
      case N_TRAINER_ELEMENTAL2: // Brumn Winterhoof
        if (player->HasSpell(S_ELEMENTAL))
          AddGossipItemFor(
              player,
              creature->GetEntry() == N_TRAINER_ELEMENTAL1 ? GOSSIP_MENU_SARAH_TANNER : GOSSIP_MENU_BRUMN_WINTERHOOF,
              GOSSIP_MENU_OPTION_CONFIRM_UNLEARN_ELEMENTAL, GOSSIP_SENDER_MAIN, GOSSIP_MENU_UNLEARN_CONFIRM_ELEMENTAL);
        break;
      case N_TRAINER_TRIBAL1: // Caryssia Moonhunter
      case N_TRAINER_TRIBAL2: // Se'Jib
        if (player->HasSpell(S_TRIBAL))
          AddGossipItemFor(
              player, creature->GetEntry() == N_TRAINER_TRIBAL1 ? GOSSIP_MENU_CARYSSIA_MOONHUNTER : GOSSIP_MENU_SEJIB,
              GOSSIP_MENU_OPTION_CONFIRM_UNLEARN_TRIBAL, GOSSIP_SENDER_MAIN, GOSSIP_MENU_UNLEARN_CONFIRM_TRIBAL);
        break;
      }
    }
  ...
  bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
  {
    ...
    case GOSSIP_MENU_UNLEARN_CONFIRM_DRAGONSCALE:
      AddGossipItemFor(player, GOSSIP_MENU_UNLEARN_CONFIRM_DRAGONSCALE, GOSSIP_MENU_OPTION_CONFIRM_UNLEARN_DRAGONSCALE,
                       GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1, DoMedUnlearnCost(player));
      SendGossipMenuFor(player, GOSSIP_TEXT_UNLEARN_CONFIRM_DRAGONSCALE, creature);
      break;
    case GOSSIP_MENU_UNLEARN_CONFIRM_ELEMENTAL:
      AddGossipItemFor(player, GOSSIP_MENU_UNLEARN_CONFIRM_ELEMENTAL, GOSSIP_MENU_OPTION_CONFIRM_UNLEARN_ELEMENTAL,
                       GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2, DoMedUnlearnCost(player));
      SendGossipMenuFor(player, GOSSIP_TEXT_UNLEARN_CONFIRM_ELEMENTAL, creature);
      break;
    case GOSSIP_MENU_UNLEARN_CONFIRM_TRIBAL:
      AddGossipItemFor(player, GOSSIP_MENU_UNLEARN_CONFIRM_TRIBAL, GOSSIP_MENU_OPTION_CONFIRM_UNLEARN_TRIBAL,
                       GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3, DoMedUnlearnCost(player));
...
public:
          ...
          player->GetQuestRewardStatus(10833))
      {
        ...
        case N_TRAINER_SPELLFIRE: // Gidge Spellweaver
          if (!HasTailorSpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_SPELLFIRE, GOSSIP_SENDER_LEARN,
                             GOSSIP_ACTION_INFO_DEF + 1);
        ...
        case N_TRAINER_MOONCLOTH: // Nasmara Moonsong
          if (!HasTailorSpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_MOONCLOTH, GOSSIP_SENDER_LEARN,
                             GOSSIP_ACTION_INFO_DEF + 2);
        ...
        case N_TRAINER_SHADOWEAVE: // Andrion Darkspinner
          if (!HasTailorSpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_SHADOWEAVE, GOSSIP_SENDER_LEARN,
                             GOSSIP_ACTION_INFO_DEF + 3);
  ...
  void SendConfirmUnlearn(Player* player, Creature* creature, uint32 action)
  {
    if (action)
    {
      ...
      {
      case N_TRAINER_SPELLFIRE: // Gidge Spellweaver
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_SPELLFIRE, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_TAILOR_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
      case N_TRAINER_MOONCLOTH: // Nasmara Moonsong
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_MOONCLOTH, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_TAILOR_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
      case N_TRAINER_SHADOWEAVE: // Andrion Darkspinner
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_SHADOWEAVE, GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_TAILOR_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
        break;
...
public:
  ...
  bool OnGossipHello(Player* player, GameObject* gameobject) override
  {
        ...
        player->GetLevel() >= 35)
    {
          ...
          player->GetQuestRewardStatus(3639))
      {
        ...
        {
          AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GOBLIN, GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 3,
                           BOX_UNLEARN_ENGIN_SPEC, DoHighUnlearnCost(player), false);
        }
        else if (player->HasSpell(S_GNOMISH)) // Has Gnomish specialization
        {
          AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GNOMISH, GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 4,
                           BOX_UNLEARN_ENGIN_SPEC, DoHighUnlearnCost(player), false);
        }
        else // does not have any specialization
        {
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_GOBLIN, GOSSIP_SENDER_LEARN,
                           GOSSIP_ACTION_INFO_DEF + 1, BOX_LEARN_ENGIN_SPEC, DoLearnCost(player), false);
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_GNOMISH, GOSSIP_SENDER_LEARN,
                           GOSSIP_ACTION_INFO_DEF + 2, BOX_LEARN_ENGIN_SPEC, DoLearnCost(player), false);
        }
      }
    }
    ...
    if (player->HasSkill(SKILL_LEATHERWORKING) && player->GetBaseSkillValue(SKILL_LEATHERWORKING) >= 225 &&
        player->GetLevel() >= 40)
    {
      if (!HasLeatherSpecialty(player) && (player->GetQuestRewardStatus(5141) || player->GetQuestRewardStatus(5143) ||
      ...
      {
        AddGossipItemFor(player, GOSSIP_MENU_GO_SOOTHSAYING_FOR_DUMMIES, GOSSIP_MENU_OPTION_GO_LEARN_DRAGONSCALE,
                         GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 5);
        AddGossipItemFor(player, GOSSIP_MENU_GO_SOOTHSAYING_FOR_DUMMIES, GOSSIP_MENU_OPTION_GO_LEARN_ELEMENTAL,
                         GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 6);
        AddGossipItemFor(player, GOSSIP_MENU_GO_SOOTHSAYING_FOR_DUMMIES, GOSSIP_MENU_OPTION_GO_LEARN_TRIBAL,
  ...
  void SendConfirmUnlearn(Player* player, GameObject* gameobject, uint32 uiAction)
  {
    ...
    // Goblin
    case GOSSIP_ACTION_INFO_DEF + 3:
      AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GOBLIN, GOSSIP_SENDER_CHECK, uiAction, BOX_UNLEARN_ENGIN_SPEC,
                       DoHighUnlearnCost(player), false);
      SendGossipMenuFor(player, player->GetGossipTextId(gameobject), gameobject->GetGUID());
      break;
    // Gnomish
    case GOSSIP_ACTION_INFO_DEF + 4:
      AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GNOMISH, GOSSIP_SENDER_CHECK, uiAction, BOX_UNLEARN_ENGIN_SPEC,
                       DoHighUnlearnCost(player), false);
      SendGossipMenuFor(player, player->GetGossipTextId(gameobject), gameobject->GetGUID());
      break;
    }
