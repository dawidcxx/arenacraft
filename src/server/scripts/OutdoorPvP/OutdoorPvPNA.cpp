...
void OutdoorPvPNA::HandleKill(Player* killer, Unit* killed)
{
  ...
  else
  {
    // creature kills must be notified, even if not inside objective / not outdoor pvp active
    if (killer &&
        ((killer->IsOutdoorPvPActive() && killer->ToPlayer()->GetAreaId() == NA_HALAA_ZONE_ID) || killed->IsCreature()))
    {
...
void OutdoorPvPNA::HandleKillImpl(Player* player, Unit* killed)
{
  if (killed->IsPlayer() && player->GetTeamId() != killed->ToPlayer()->GetTeamId())
  {
    ...
    player->CastSpell(player, player->GetTeamId() == TEAM_ALLIANCE ? NA_KILL_TOKEN_ALLIANCE : NA_KILL_TOKEN_HORDE,
                      true);
  }
}
...
TeamId OPvPCapturePointNA::GetControllingFaction() const { return m_ControllingFaction; }
...
void OPvPCapturePointNA::SpawnGOsForTeam(TeamId teamId)
{
  ...
  for (int i = 0; i < NA_CONTROL_GO_NUM; ++i)
  {
    ...
    AddObject(i, gos[i].entry, gos[i].map, gos[i].x, gos[i].y, gos[i].z, gos[i].o, gos[i].rot0, gos[i].rot1,
              gos[i].rot2, gos[i].rot3);
  }
}
...
OPvPCapturePointNA::OPvPCapturePointNA(OutdoorPvP* pvp)
    : OPvPCapturePoint(pvp), m_capturable(true), m_GuardsAlive(0), m_ControllingFaction(TEAM_NEUTRAL),
      m_WyvernStateNorth(0), m_WyvernStateSouth(0), m_WyvernStateEast(0), m_WyvernStateWest(0), m_HalaaState(HALAA_N),
      m_RespawnTimer(NA_RESPAWN_TIME), m_GuardCheckTimer(NA_GUARD_CHECK_TIME), m_canRecap(true)
{
...
void OutdoorPvPNA::FillInitialWorldStates(WorldPacket& data) { m_obj->FillInitialWorldStates(data); }
...
int32 OPvPCapturePointNA::HandleOpenGo(Player* player, GameObject* go)
{
  ...
  if (retval >= 0)
  {
    ...
    if (add > -1)
      AddObject(add, gos[add].entry, gos[add].map, gos[add].x, gos[add].y, gos[add].z, gos[add].o, gos[add].rot0,
                gos[add].rot1, gos[add].rot2, gos[add].rot3);

    if (add2 > -1)
      AddObject(add2, gos[add2].entry, gos[add2].map, gos[add2].x, gos[add2].y, gos[add2].z, gos[add2].o,
                gos[add2].rot0, gos[add2].rot1, gos[add2].rot2, gos[add2].rot3);
...
bool OPvPCapturePointNA::Update(uint32 diff)
{
  ...
  if (m_capturable)
  {
    ...
    if (_value < -_minValue) // red
    {
      ...
      {
        // When the point goes through neutral, the same faction can recapture again to respawn the guards, still need
        // check blizzlike
        m_canRecap = true;
        DespawnGOs();
        DespawnCreatures(GetControllingFaction() == TEAM_HORDE ? halaaNPCHorde : halaaNPCAlly);
