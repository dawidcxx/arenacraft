...
public:
  ...
  static bool HandleQuestAdd(ChatHandler* handler, Quest const* quest, Optional<PlayerIdentifier> playerTarget)
  {
    ...
    else
    {
      ...
      QueryResult         result =
          CharacterDatabase.Query("SELECT 1 FROM character_queststatus WHERE guid = {} AND quest = {}", guid, entry);
      ...
      if (uint32 mail_template_id = quest->GetRewMailTemplateId())
      {
        if (quest->GetRewMailSenderEntry() != 0)
        {
          MailDraft(mail_template_id)
              .SendMailTo(trans, MailReceiver(nullptr, guid), quest->GetRewMailSenderEntry(), MAIL_CHECK_MASK_HAS_BODY,
