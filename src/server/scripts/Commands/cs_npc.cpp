...
NpcFlagText const npcFlagTexts[NPCFLAG_COUNT] = {{UNIT_NPC_FLAG_AUCTIONEER, LANG_NPCINFO_AUCTIONEER},
...
SpellSchoolImmune const spellSchoolImmunes[MAX_SPELL_SCHOOL] = {
    {SPELL_SCHOOL_NORMAL, "SPELL_SCHOOL_NORMAL"}, {SPELL_SCHOOL_HOLY, "SPELL_SCHOOL_HOLY"},
    {SPELL_SCHOOL_FIRE, "SPELL_SCHOOL_FIRE"},     {SPELL_SCHOOL_NATURE, "SPELL_SCHOOL_NATURE"},
    {SPELL_SCHOOL_FROST, "SPELL_SCHOOL_FROST"},   {SPELL_SCHOOL_SHADOW, "SPELL_SCHOOL_SHADOW"},
...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable npcFollowCommandTable = {{"stop", HandleNpcUnFollowCommand, SEC_GAMEMASTER, Console::No},
    ...
    static ChatCommandTable npcCommandTable = {{"info", HandleNpcInfoCommand, SEC_GAMEMASTER, Console::No},
  ...
  static bool HandleNpcAddCommand(ChatHandler* handler, CreatureEntry id)
  {
    ...
    // current "creature" variable is deleted and created fresh new, otherwise old values might trigger asserts or cause
    // undefined behavior
  ...
  static bool HandleNpcSetDataCommand(ChatHandler* handler, uint32 data_1, uint32 data_2)
  {
    ...
    handler->PSendSysMessage(LANG_NPC_SETDATA, creature->GetGUID().ToString(), creature->GetEntry(),
  ...
  static bool HandleNpcInfoCommand(ChatHandler* handler)
  {
    ...
    handler->PSendSysMessage(LANG_NPCINFO_CHAR, target->GetSpawnId(), target->GetGUID().ToString(), entry, id1, id2,
                             id3, displayid, nativeid, faction, npcflags);
    handler->PSendSysMessage(LANG_NPCINFO_LEVEL, target->GetLevel());
    handler->PSendSysMessage(LANG_NPCINFO_EQUIPMENT, target->GetCurrentEquipmentId(), target->GetOriginalEquipmentId());
    handler->PSendSysMessage(LANG_NPCINFO_HEALTH, target->GetCreateHealth(), target->GetMaxHealth(),
                             target->GetHealth());
    handler->PSendSysMessage(LANG_NPCINFO_FLAGS, target->GetUnitFlags(), target->GetUnitFlags2(),
                             target->GetDynamicFlags(), target->GetFaction());
   ...
   */
  static bool
  HandleNpcSetMoveTypeCommand(ChatHandler* handler, Optional<CreatureSpawnId> lowGuid,
  ...
  static bool HandleNpcSetLinkCommand(ChatHandler* handler, ObjectGuid::LowType linkguid)
  {
    ...
    handler->PSendSysMessage("LinkGUID '{}' added to creature with DBTableGUID: '{}'", linkguid,
                             creature->GetSpawnId());
    return true;
  }
};
