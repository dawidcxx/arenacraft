...
public:
  ...
  {
    // latched onto HandleReloadSkillExtraItemTemplateCommand as it's part of that table group (and i don't want to
    // chance all the command IDs)
  ...
  static bool HandleReloadPageTextsCommand(ChatHandler* handler)
  {
    ...
    handler->SendGlobalGMSysMessage("You need to delete your client cache or change the cache number in config in "
                                    "order for your players see the changes.");
    return true;
  }
