...
public:
  ...
  static bool HandleGMTicketCloseByIdCommand(ChatHandler* handler, uint32 ticketId)
  {
    ...
    std::string msg = ticket->FormatMessageString(*handler, player ? player->GetName().c_str() : "Console", nullptr,
                                                  nullptr, nullptr);
    handler->SendGlobalGMSysMessage(msg.c_str());
  ...
  static bool HandleGMTicketCommentCommand(ChatHandler* handler, uint32 ticketId, Tail comment)
  {
    ...
    std::string       msg = ticket->FormatMessageString(*handler, assignedName.empty() ? nullptr : assignedName.c_str(),
                                                        nullptr, nullptr, nullptr);

    msg += handler->PGetParseString(LANG_COMMAND_TICKETLISTADDCOMMENT, player ? player->GetName().c_str() : "Console",
                                    comment.data());
    handler->SendGlobalGMSysMessage(msg.c_str());
  ...
  static bool HandleGMTicketUnAssignCommand(ChatHandler* handler, uint32 ticketId)
  {
    ...
    std::string assignedTo =
        ticket->GetAssignedToName(); // copy assignedto name because we need it after the ticket has been unnassigned
    CharacterDatabaseTransaction trans = CharacterDatabaseTransaction(nullptr);
