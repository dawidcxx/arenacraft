...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable commandTable = {{"gobject", gobjectCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleGameObjectAddCommand(ChatHandler* handler, GameObjectEntry objectId, Optional<int32> spawnTimeSecs)
  {
    ...
    if (objectInfo->displayId && !sGameObjectDisplayInfoStore.LookupEntry(objectInfo->displayId))
    {
      ...
      LOG_ERROR("sql.sql", "Gameobject (Entry {} GoType: {}) have invalid displayId ({}), not spawned.", *objectId,
    ...
    if (!object->Create(guidLow, objectInfo->entry, map, player->GetPhaseMaskForSpawn(), x, y, z, o, G3D::Quat(), 0,
                        GO_STATE_READY))
    {
      delete object;
      return false;
    ...
    handler->PSendSysMessage(LANG_GAMEOBJECT_ADD, uint32(objectId), objectInfo->name, guidLow, player->GetPositionX(),
                                            ...
                                            Optional<Variant<GameObjectEntry, std::string_view>> objectId)
  {
    ...
    if (objectId)
    {
      ...
      {
        result =
            WorldDatabase.Query("SELECT guid, id, position_x, position_y, position_z, orientation, map, phaseMask, "
                                "(POW(position_x - '{}', 2) + POW(position_y - '{}', 2) + POW(position_z - '{}', 2)) "
                                "AS order_ FROM gameobject WHERE map = '{}' AND id = '{}' ORDER BY order_ ASC LIMIT 1",
                                player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(),
                                player->GetMapId(), static_cast<uint32>(objectId->get<GameObjectEntry>()));
      }
      else
      {
        ...
        result = WorldDatabase.Query(
            "SELECT guid, id, position_x, position_y, position_z, orientation, map, phaseMask, (POW(position_x - {}, "
            "2) + POW(position_y - {}, 2) + POW(position_z - {}, 2)) AS order_ "
            "FROM gameobject LEFT JOIN gameobject_template ON gameobject_template.entry = gameobject.id WHERE map = {} "
    ...
    else
    {
      ...
      result = WorldDatabase.Query(
          ...
          "LEFT OUTER JOIN game_event_gameobject on gameobject.guid = game_event_gameobject.guid WHERE map = '{}' {} "
  ...
  static bool HandleGameObjectTurnCommand(ChatHandler* handler, GameObjectSpawnId guidLow, Optional<float> oz,
                                          Optional<float> oy, Optional<float> ox)
  {
    if (!guidLow)
      return false;
    ...
    handler->PSendSysMessage(LANG_COMMAND_TURNOBJMESSAGE, object->GetSpawnId(), object->GetGOInfo()->name,
                             object->GetSpawnId());
    return true;
  }

  // move selected object
  static bool HandleGameObjectMoveCommand(ChatHandler* handler, GameObjectSpawnId guidLow,
                                          Optional<std::array<float, 3>> xyz)
  {
    ...
    if (xyz)
    {
      ...
      {
        handler->SendErrorMessage(LANG_INVALID_TARGET_COORD, pos.GetPositionX(), pos.GetPositionY(),
                                  object->GetMapId());
        return false;
      }
    }
    ...
    handler->PSendSysMessage(LANG_COMMAND_MOVEOBJMESSAGE, object->GetSpawnId(), object->GetGOInfo()->name,
                             object->GetSpawnId());
    return true;
  }
  ...
  static bool HandleGameObjectRespawn(ChatHandler* handler, GameObjectSpawnId guidLow)
  {
    ...
    object->Respawn();
    handler->PSendSysMessage(LANG_CMD_GO_RESPAWN, object->GetNameForLocaleIdx(handler->GetSessionDbcLocale()),
                             object->GetEntry(), object->GetSpawnId());
    return true;
  }
};
