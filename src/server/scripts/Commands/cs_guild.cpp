...
public:
  ...
  {
    static ChatCommandTable guildCommandTable = {{"create", HandleGuildCreateCommand, SEC_GAMEMASTER, Console::Yes},
  ...
  static bool HandleGuildInfoCommand(ChatHandler*                                                handler,
                                     Optional<Variant<ObjectGuid::LowType, QuotedString>> const& guildIdentifier)
  {
    ...
    if (sCharacterCache->GetCharacterNameByGuid(guild->GetLeaderGUID(), guildMasterName))
      handler->PSendSysMessage(LANG_GUILD_INFO_GUILD_MASTER, guildMasterName,
                               guild->GetLeaderGUID().ToString()); // Guild Master
    ...
    handler->PSendSysMessage(LANG_GUILD_INFO_BANK_GOLD,
                             guild->GetTotalBankMoney() / 100 / 100);       // Bank Gold (in gold coins)
