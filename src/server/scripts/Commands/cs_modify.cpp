...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable morphCommandTable = {{"reset", HandleMorphResetCommand, SEC_MODERATOR, Console::No},
                                                 {"target", HandleMorphTargetCommand, SEC_MODERATOR, Console::No},
                                                 {"mount", HandleMorphMountCommand, SEC_MODERATOR, Console::No}};

    static ChatCommandTable commandTable = {{"morph", morphCommandTable}, {"modify", modifyCommandTable}};

    return commandTable;
  }

  template <typename... Args>
  static void NotifyModification(ChatHandler* handler, Unit* target, AcoreStrings resourceMessage,
                                 AcoreStrings resourceReportMessage, Args&&... args)
  {
  ...
  static bool HandleModifyEnergyCommand(ChatHandler* handler, int32 energyPoints)
  {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_ENERGY, handler->GetNameLink(target), energyPoints / 10,
                             energyPoints / 10);

    if (handler->needReportToTarget(target))
    {
      ChatHandler(target->GetSession())
          .PSendSysMessage(LANG_YOURS_ENERGY_CHANGED, handler->GetNameLink(), energyPoints / 10, energyPoints / 10);
    }
  ...
  static bool HandleModifyRunicPowerCommand(ChatHandler* handler, int32 runePoints)
  {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_RUNIC_POWER, handler->GetNameLink(target), runePoints / 10,
                             runePoints / 10);

    if (handler->needReportToTarget(target))
    {
      ChatHandler(target->GetSession())
          .PSendSysMessage(LANG_YOURS_RUNIC_POWER_CHANGED, handler->GetNameLink(), runePoints / 10, runePoints / 10);
    }
  ...
  // Edit Player Spell
  static bool HandleModifySpellCommand(ChatHandler* handler, uint8 spellFlatID, uint8 op, uint16 val,
                                       Optional<uint16> mark)
  {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_SPELLFLATID, spellFlatID, val, mark ? *mark : 65535,
                             handler->GetNameLink(target));
    if (handler->needReportToTarget(target))
    {
      ChatHandler(target->GetSession())
          .PSendSysMessage(LANG_YOURS_SPELLFLATID_CHANGED, handler->GetNameLink(), spellFlatID, val,
                           mark ? *mark : 65535);
    }
  ...
  static bool CheckModifySpeed(ChatHandler* handler, Unit* target, float speed, float minimumBound, float maximumBound,
  ...
  static bool HandleModifyRepCommand(ChatHandler* handler, uint32 factionId, Variant<int32, std::string> rank,
                                     Optional<int32> delta)
  {
    ...
    {
      handler->SendErrorMessage(LANG_COMMAND_FACTION_NOREP_ERROR, factionEntry->name[handler->GetSessionDbcLocale()],
                                factionId);
      return false;
    }
