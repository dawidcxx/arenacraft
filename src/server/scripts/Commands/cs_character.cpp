...
public:
  ...
  {
    static ChatCommandTable pdumpCommandTable = {{"load", HandlePDumpLoadCommand, SEC_ADMINISTRATOR, Console::Yes},
    ...
    static ChatCommandTable commandTable = {{"character", characterCommandTable},
  ...
  static void HandleCharacterDeletedListHelper(DeletedInfoList const& foundList, ChatHandler* handler)
  {
    ...
    for (DeletedInfoList::const_iterator itr = foundList.begin(); itr != foundList.end(); ++itr)
    {
      ...
      if (!handler->GetSession())
        handler->PSendSysMessage(LANG_CHARACTER_DELETED_LIST_LINE_CONSOLE, itr->lowGuid, itr->name,
                                 itr->accountName.empty() ? "<Not existing>" : itr->accountName, itr->accountId,
                                 dateStr);
      else
        handler->PSendSysMessage(LANG_CHARACTER_DELETED_LIST_LINE_CHAT, itr->lowGuid, itr->name,
                                 itr->accountName.empty() ? "<Not existing>" : itr->accountName, itr->accountId,
                                 dateStr);
    }
  ...
  static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo, ChatHandler* handler)
  {
    ...
    {
      sCharacterCache->AddCharacterCacheEntry(ObjectGuid(HighGuid::Player, delInfo.lowGuid), delInfo.accountId,
                                              delInfo.name, (*result)[2].Get<uint8>(), (*result)[0].Get<uint8>(),
                                              (*result)[1].Get<uint8>(), (*result)[3].Get<uint8>());
    }
  }

  static void HandleCharacterLevel(Player* player, ObjectGuid playerGuid, uint32 oldLevel, uint32 newLevel,
                                   ChatHandler* handler)
  {
    if (player)
    {
    ...
    else
    {
      ...
      sAchievementMgr->UpdateAchievementCriteriaForOfflinePlayer(playerGuid.GetCounter(),
  ...
  static bool HandleCharacterTitlesCommand(ChatHandler* handler, Optional<PlayerIdentifier> player)
  {
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
      ...
      if (titleInfo && target->HasTitle(titleInfo))
      {
        ...
        if (handler->GetSession())
          handler->PSendSysMessage(LANG_TITLE_LIST_CHAT, id, titleInfo->bit_index, id, titleName, localeNames[loc],
                                   knownStr, activeStr);
        else
          handler->PSendSysMessage(LANG_TITLE_LIST_CONSOLE, id, titleInfo->bit_index, name, localeNames[loc], knownStr,
                                   activeStr);
      }
    }
  ...
  static bool HandleCharacterReputationCommand(ChatHandler* handler, Optional<PlayerIdentifier> player)
  {
    ...
    for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
    {
      ...
      if (handler->GetSession())
        ss << faction.ID << " - |cffffffff|Hfaction:" << faction.ID << "|h[" << factionName << ' ' << localeNames[loc]
           << "]|h|r";
      else
  ...
  /**
   * Handles the '.character deleted old' command, which completely deletes all deleted characters deleted with some
   * days ago
   *
  ...
  static bool HandleCharacterEraseCommand(ChatHandler* handler, PlayerIdentifier player)
  {
    ...
    handler->PSendSysMessage(LANG_CHARACTER_DELETED, player.GetName(), player.GetGUID().ToString(), accountName,
                             accountId);
    ...
    if (BagSlot == 1)
    {
      for (uint32 i = 23; i < 39; i++)
      {
        if (Item* item = target->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
        {
          ...
          handler->PSendSysMessage("{} - |c{}|Hitem:{}:0:0:0:0:0:0:0:0:0|h[{}]|h|r - {}", Counter, ItemString.str(),
                                   item->GetEntry(), item->GetTemplate()->Name1, item->GetCount());
        }
      }
    }
    else
  ...
  static bool HandleCharacterChangeAccountCommand(ChatHandler* handler, std::string accountName,
                                                  Optional<PlayerIdentifier> player)
  {
    ...
    if (uint32 accountId = AccountMgr::GetId(accountName))
    {
      ...
      if (CharacterCacheEntry const* cache = sCharacterCache->GetCharacterCacheByName(player->GetName()))
      {
        ...
        handler->PSendSysMessage(LANG_CMD_CHAR_CHANGE_ACC_SUCCESS, player->GetName(), player->GetGUID().ToString(),
                                 accName, cache->AccountId, accountName, accountId);
      }
      ...
      CharacterDatabase.Query("UPDATE characters SET account = {} WHERE guid = {}", accountId,
