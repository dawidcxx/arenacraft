...
static Optional<BigNumber> GetHexFromConfig(char const* configKey, int bits)
{
  ...
  if (!secret.SetHexStr(str.c_str()))
  {
    LOG_FATAL("server.loading",
              "Invalid value for '{}' - specify a hexadecimal integer of up to {} bits with no prefix.", configKey,
              bits);
    ABORT();
  }
...
void SecretMgr::AttemptLoad(Secrets i, LogLevel errorLevel, std::unique_lock<std::mutex> const&)
{
  ...
  if (((!oldDigest) != (!currentValue)) || // there is an old digest, but no current secret (or vice versa)
      (oldDigest &&
       !Acore::Crypto::Argon2::Verify(currentValue->AsHexStr(),
  ...
  )
  {
    ...
    if (error)
    {
      LOG_MESSAGE_BODY("server.loading", errorLevel,
...
Optional<std::string> SecretMgr::AttemptTransition(Secrets i, Optional<BigNumber> const& newSecret,
                                                   Optional<BigNumber> const& oldSecret, bool hadOldSecret) const
{
  ...
  case SECRET_TOTP_MASTER_KEY:
  {
    ...
    if (result)
      do
      {
        ...
        {
          if (!oldSecret)
            return Acore::StringFormat("Cannot decrypt old TOTP tokens - add config key '{}' to authserver.conf!",
          ...
          if (!success)
            return Acore::StringFormat("Cannot decrypt old TOTP tokens - value of '{}' is incorrect for some users!",
                                       secret_info[i].oldKey);
        }
