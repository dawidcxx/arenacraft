...
enum AchievementFlags
{
  ...
  ACHIEVEMENT_FLAG_AVERAGE   = 0x00000040,         // Show as average value (value / time_in_days) depend from
                                                   // other flag (by def use last criteria value)
  ACHIEVEMENT_FLAG_BAR = 0x00000080,               // Show as progress bar (value / max vale) depend from other
...
enum AchievementCriteriaCondition
{
  ...
  ACHIEVEMENT_CRITERIA_CONDITION_UNK1     = 2, // only used in "Complete a daily quest every day for five consecutive
                                               // days"
...
enum AchievementCriteriaTimedTypes
{
  ...
  ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET = 6, // Timer is started by being target of spell with entry in
                                           // timerStartEvent
...
enum AchievementCriteriaTypes
{
  ...
  ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA = 37, /// @todo: the archievements 1162 and 1163 requires a special rating
  ...
  ACHIEVEMENT_CRITERIA_TYPE_VISIT_BARBER_SHOP = 48, // note: rewarded as soon as the player payed, not at taking place
                                                    // at the seat
  ...
  ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS  = 56, /// @todo: in some cases map not present, and in some cases need do
                                                     /// without die
...
enum AreaFlags
{
  ...
  AREA_FLAG_OUTLAND        = 0x00000400,     // expansion zones? (only Eye of the Storm not
  ...
  AREA_FLAG_OUTLAND2  = 0x00004000,          // expansion zones? (only Circle of Blood Arena not have this
                                             // flag, but have 0x00000400 flag)
  AREA_FLAG_OUTDOOR_PVP = 0x00008000,        // pvp objective area? (Death's Door also has this flag
                                             // although it's no pvp object area)
  AREA_FLAG_ARENA_INSTANCE = 0x00010000,     // used by instanced arenas only
  AREA_FLAG_UNUSED2        = 0x00020000,     // Unused in 3.3.5a
  AREA_FLAG_CONTESTED_AREA = 0x00040000,     // On PvP servers these areas are considered contested, even
  ...
  AREA_FLAG_REST_ZONE_HORDE = 0x00400000,    // Instead of using areatriggers, the zone will act as one for
                                             // Horde players (Warsong Hold, Acherus: The Ebon Hold, New
                                             // Agamand Inn, Vengeance Landing Inn, Sunreaver Pavilion,
                                             // etc)
  AREA_FLAG_REST_ZONE_ALLIANCE = 0x00800000, // Instead of using areatriggers, the zone will act as one for
                                             // Alliance players (Valgarde, Acherus: The Ebon Hold,
                                             // Westguard Inn, Silver Covenant Pavilion, etc)
  AREA_FLAG_WINTERGRASP = 0x01000000,        // Wintergrasp and it's subzones
  AREA_FLAG_INSIDE      = 0x02000000,        // used for determinating spell related
                                             // inside/outside questions in Map::IsOutdoors
  AREA_FLAG_OUTSIDE = 0x04000000,            // used for determinating spell related
                                             // inside/outside questions in Map::IsOutdoors
...
enum FactionTemplateFlags
{
  ...
  FACTION_TEMPLATE_FLAG_ASSIST_PLAYERS = 0x0800, // Old title: FACTION_TEMPLATE_FLAG_ASSIST_PLAYERS, Old comment:
                                                 // flagged for PvP //@todo: Should see if this is implemented
                                                 // correctly.
  FACTION_TEMPLATE_FLAG_ATTACK_PVP_ACTIVE_PLAYERS =
      0x1000, // Old title: FACTION_TEMPLATE_FLAG_ATTACK_PVP_ACTIVE_PLAYERS, Old
              // comment: faction will attack players that were involved in PvP
              // combats //@todo: Should see if this is implemented correctly.
  FACTION_TEMPLATE_FLAG_HATES_ALL_EXCEPT_FRIENDS =
      0x2000, // Old title: FACTION_TEMPLATE_FLAG_HATES_ALL_EXCEPT_FRIENDS, Old
              // comment: //@todo: Should see if this is implemented correctly.
};
...
enum SkillRaceClassInfoFlags
{
  ...
  SKILL_FLAG_INCLUDE_IN_SORT    = 0x80, // Spells belonging to a skill with this flag will additionally
                                        // compare skill ids when sorting spellbook in client
  SKILL_FLAG_NOT_TRAINABLE = 0x100,
  SKILL_FLAG_MONO_VALUE    = 0x400 // Skill always has value 1 - clientside display
                                   // flag, real value can be different
};
...
enum SummonPropFlags
{
  ...
  SUMMON_PROP_FLAG_USE_SUMMONER_FACTION      = 0x00001000, /// @todo: NYI Lightwell, Jeeves, Gnomish Alarm-o-bot, Build
                                                           /// vehicles(wintergrasp)
  SUMMON_PROP_FLAG_DO_NOT_FOLLOW_MOUNTED_SUMMONER = 0x00002000, /// @todo: NYI Guides, player follows
  SUMMON_PROP_FLAG_SAVE_PET_AUTOCAST = 0x00004000,     /// @todo: NYI Force of Nature, Shadowfiend, Feral Spirit,
                                                       /// Summon Water Elemental
  SUMMON_PROP_FLAG_IGNORE_SUMMONER_PHASE = 0x00008000, /// @todo: NYI Light/Dark Bullet, Soul/Fiery Consumption,
...
enum VehicleSeatFlags
{
  ...
  VEHICLE_SEAT_FLAG_CAN_CAST_MOUNT_SPELL = 0x00001000, // Can cast spells with SPELL_AURA_MOUNTED from seat
...
enum VehicleSeatFlagsB
{
  ...
  VEHICLE_SEAT_FLAG_B_VEHICLE_PLAYERFRAME_UI = 0x80000000, // Lua_UnitHasVehiclePlayerFrameUI - actually checked for
                                                           // flagsb &~ 0x80000000
};
