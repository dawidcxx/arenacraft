...
public:
  ...
  explicit ByteBuffer(std::size_t reserve) : _rpos(0), _wpos(0) { _storage.reserve(reserve); }
  ...
  template <typename T> void put(std::size_t pos, T value)
  {
    static_assert(std::is_fundamental<T>::value, "append(compound)");
    EndianConvert(value);
  ...
  ByteBuffer& operator<<(std::string const& str) { return operator<<(std::string_view(str)); }

  ByteBuffer& operator<<(char const* str) { return operator<<(std::string_view(str ? str : "")); }
  ...
  template <std::size_t Size> void read(std::array<uint8, Size>& arr) { read(arr.data(), Size); }
  ...
  void append(char const* src, std::size_t cnt) { return append((uint8 const*)src, cnt); }

  template <class T> void append(const T* src, std::size_t cnt) { return append((uint8 const*)src, cnt * sizeof(T)); }
  ...
  template <std::size_t Size> void append(std::array<uint8, Size> const& arr) { append(arr.data(), Size); }
