...
bool readCamera(M2Camera const* cam, uint32 buffSize, M2Header const* header, CinematicCameraEntry const* dbcentry)
{
  ...
  // Read camera positions and timestamps (translating first position of 3 only, we don't need to translate the whole
  // spline)
...
void LoadM2Cameras(std::string const& dataPath)
{
  ...
  for (CinematicCameraEntry const* dbcentry : sCinematicCameraStore)
  {
    ...
    {
      LOG_ERROR("server.loading", "Camera file {} is damaged. Camera references position beyond file end",
                filename.string());
      continue;
    }
  ...
  LOG_INFO("server.loading", ">> Loaded {} Cinematic Waypoint Sets in {} ms", (uint32)sFlyByCameraStore.size(),
