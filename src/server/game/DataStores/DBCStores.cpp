...
{
  LOG_ERROR("dbc", "Size of '{}' set by format string ({}) not equal size of C++ structure ({}).", filename, fsize,
            rsize);
...
#define LOAD_DBC(store, file, dbtable) LoadDBC(availableDbcLocales, bad_dbc_files, store, dbcPath, file, dbtable)
  ...
  // LOAD_DBC(sGtOCTRegenMPStore,                  "gtOCTRegenMP.dbc",                     "gtoctregenmp_dbc");       --
  // not used currently
...
#undef LOAD_DBC
    ...
    for (uint8 x = 0; x < MAX_DIFFICULTY; ++x)
    {
      if (spellDiff->SpellID[x] <= 0 || !sSpellStore.LookupEntry(spellDiff->SpellID[x]))
      {
        if (spellDiff->SpellID[x] >
          ...
          LOG_ERROR("sql.sql",
                    "spelldifficulty_dbc: spell {} at field id: {} at spellid {} does not exist in SpellStore "
  ...
  // include existed nodes that have at least single not spell base (scripted) path
  {
    ...
    for (uint32 i = 1; i < sTaxiNodesStore.GetNumRows(); ++i)
    {
      ...
      // old continent node (+ nodes virtually at old continents, check explicitly to avoid loading map files for zone
      // info)
...
/// Returns LFGDungeonEntry for a specific map and difficulty. Will return first found entry if multiple dungeons use
