...
enum GroupUpdateFlags
{
  ...
  GROUP_UPDATE_FLAG_PET_AURAS      = 0x00040000, // uint64 mask, for each bit set uint32 spellid + uint8 unk,
                                                 // pet auras...
...
public:
  ...
  bool RemoveMember(ObjectGuid guid, const RemoveMethod& method = GROUP_REMOVEMETHOD_DEFAULT,
  ...
  GroupJoinBattlegroundResult CanJoinBattlegroundQueue(Battleground const*     bgTemplate,
                                                       BattlegroundQueueTypeId bgQueueTypeId, uint32 MinPlayerCount,
                                                       uint32 MaxPlayerCount, bool isRated, uint32 arenaSlot);
