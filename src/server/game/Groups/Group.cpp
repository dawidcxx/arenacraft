...
Group::Group()
    : m_leaderName(""), m_groupType(GROUPTYPE_NORMAL), m_dungeonDifficulty(DUNGEON_DIFFICULTY_NORMAL),
      m_raidDifficulty(RAID_DIFFICULTY_10MAN_NORMAL), m_bfGroup(nullptr), m_bgGroup(nullptr),
      m_lootMethod(FREE_FOR_ALL), m_lootThreshold(ITEM_QUALITY_UNCOMMON), m_subGroupsCounts(nullptr), m_counter(0),
      m_maxEnchantingLevel(0), _difficultyChangePreventionTime(0),
...
bool Group::RemoveMember(ObjectGuid guid, const RemoveMethod& method /*= GROUP_REMOVEMETHOD_DEFAULT*/,
                         ObjectGuid kicker /*= ObjectGuid::Empty*/, const char* reason /*= nullptr*/)
{
  ...
  // remove member and change leader (if need) only if strong more 2 members _before_ member remove (BG/BF allow 1
  // member group)
  if (GetMembersCount() > ((isBGGroup() || isLFGGroup() || isBFGroup()) ? 1u : 2u))
  {
    ...
    for (Rolls::iterator it = RollId.begin(); it != RollId.end();)
    {
      ...
      // Xinef: player is removed from all vote lists so it will not pass above playerVote == playerVote.end statement
...
void Group::SendLootRoll(ObjectGuid sourceGuid, ObjectGuid targetGuid, uint8 rollNumber, uint8 rollType,
                         Roll const& roll, bool autoPass)
{
...
void Group::SendLootRollWon(ObjectGuid sourceGuid, ObjectGuid targetGuid, uint8 rollNumber, uint8 rollType,
                            Roll const& roll)
{
      ...
      if (r->totalPlayersRolling > 0)
      {
        ...
        for (Roll::PlayerVote::const_iterator itr = r->playerVote.begin(); itr != r->playerVote.end(); ++itr)
        {
          ...
          if (itr->second == PASS)
            ...
            SendLootStartRollToPlayer(60000, lootedObject->GetMapId(), p,
...
void Group::CountTheRoll(Rolls::iterator rollI, Map* allowedMap)
{
  ...
  if (roll->totalNeed == 0 &&
      roll->totalGreed > 0) // pussywizard: if (roll->totalNeed == 0 && ...), not else if, because numbers can be
                            // modified above if player is on a different map
  {
    if (!roll->playerVote.empty())
    {
      uint8      maxresul = 0;
...
void Group::SendUpdateToPlayer(ObjectGuid playerGUID, MemberSlot* slot)
{
  ...
  WorldPacket data(SMSG_GROUP_LIST, (1 + 1 + 1 + 1 + 1 + 4 + 8 + 4 + 4 +
                                     (GetMembersCount() - 1) * (13 + 8 + 1 + 1 + 1 + 1) + 8 + 1 + 8 + 1 + 1 + 1 + 1));
  ...
  {
    data << uint8(
        sLFGMgr->GetState(m_guid) == lfg::LFG_STATE_FINISHED_DUNGEON ? 2 : 0); // FIXME - Dungeon save status? 2 = done
...
void Group::ChangeMembersGroup(ObjectGuid guid, uint8 group)
{
  ...
  if (Player* player = ObjectAccessor::FindConnectedPlayer(guid))
  {
    ...
    else // If player is in BG raid, it is possible that he is also in normal raid - and that normal raid is stored in
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(Battleground const*     bgTemplate,
                                                            ...
                                                            uint32 MinPlayerCount, uint32 /*MaxPlayerCount*/,
                                                            bool isRated, uint32 arenaSlot)
{
...
void Group::SetLootThreshold(ItemQualities threshold) { m_lootThreshold = threshold; }
...
Player* Group::GetLeader() { return ObjectAccessor::FindConnectedPlayer(m_leaderGuid); }
...
bool Group::IsMember(ObjectGuid guid) const { return _getMemberCSlot(guid) != m_memberSlots.end(); }

bool Group::IsLeader(ObjectGuid guid) const { return (GetLeaderGUID() == guid); }
...
Difficulty Group::GetDifficulty(bool isRaid) const { return isRaid ? m_raidDifficulty : m_dungeonDifficulty; }
