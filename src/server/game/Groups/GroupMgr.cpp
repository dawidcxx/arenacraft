...
void GroupMgr::AddGroup(Group* group) { GroupStore[group->GetGUID().GetCounter()] = group; }

void GroupMgr::RemoveGroup(Group* group) { GroupStore.erase(group->GetGUID().GetCounter()); }
...
{
  {
    ...
    CharacterDatabase.DirectExecute("DELETE lfg_data FROM lfg_data LEFT JOIN `groups` ON lfg_data.guid = groups.guid "
                                    "WHERE groups.guid IS NULL OR groups.groupType <> 12");
    // CharacterDatabase.DirectExecute("DELETE `groups` FROM `groups` LEFT JOIN lfg_data ON groups.guid = lfg_data.guid
    // WHERE groups.groupType=12 AND lfg_data.guid IS NULL"); // group should be left so binds are cleared when
    // disbanded
    ...
    QueryResult result = CharacterDatabase.Query(
        "SELECT g.leaderGuid, g.lootMethod, g.looterGuid, g.lootThreshold, g.icon1, g.icon2, g.icon3, g.icon4, "
        "g.icon5, g.icon6"
        //  10         11          12         13              14                  15            16        17          18
        ", g.icon7, g.icon8, g.groupType, g.difficulty, g.raidDifficulty, g.masterLooterGuid, g.guid, lfg.dungeon, "
        "lfg.state FROM `groups` g LEFT JOIN lfg_data lfg ON lfg.guid = g.guid ORDER BY g.guid ASC");
  ...
  LOG_INFO("server.loading", "Loading Group Members...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        if (group)
          group->LoadMemberFromDB(fields[1].Get<uint32>(), fields[2].Get<uint8>(), fields[3].Get<uint8>(),
                                  fields[4].Get<uint8>());
