...
void VisibleNotifier::SendToSelf()
{
  ...
  for (GuidUnorderedSet::const_iterator it = vis_guids.begin(); it != vis_guids.end(); ++it)
  {
    ...
    // pussywizard: static transports are removed only in RemovePlayerFromMap and here if can no longer detect (eg.
    // phase changed)
    if ((*it).IsTransport())
...
void PlayerRelocationNotifier::Visit(PlayerMapType& m)
{
  for (PlayerMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
  {
    ...
    player->UpdateVisibilityOf(
        &i_player); // this notifier with different Visit(PlayerMapType&) than VisibleNotifier is needed to update
                    // visibility of self for other players when we move (eg. stealth detection changes)
  }
}
...
void CreatureRelocationNotifier::Visit(PlayerMapType& m)
{
  for (PlayerMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
  {
    ...
    // NOTIFY_VISIBILITY_CHANGED does not guarantee that player will do it himself (because distance is also checked),
    // but screw it, it's not that important
    if (!player->m_seer->isNeedNotify(NOTIFY_VISIBILITY_CHANGED))
      player->UpdateVisibilityOf(&i_creature);

    // NOTIFY_AI_RELOCATION does not guarantee that player will do it himself (because distance is also checked), but
...
void AIRelocationNotifier::Visit(CreatureMapType& m)
{
  ...
  for (CreatureMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
  {
    ...
    // NOTIFY_VISIBILITY_CHANGED | NOTIFY_AI_RELOCATION does not guarantee that unit will do it itself (because distance
    // is also checked), but screw it, it's not that important
