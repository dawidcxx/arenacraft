...
struct LootStoreItem
{
  ...
  float  chance;                 // chance to drop for both quest and non-quest items, chance to
                                 // be used for refs
  ...
  LootStoreItem(uint32 _itemid, int32 _reference, float _chance, bool _needs_quest, uint16 _lootmode, uint8 _groupid,
                int32 _mincount, uint8 _maxcount)
      : itemid(_itemid), reference(_reference), chance(_chance), needs_quest(_needs_quest), lootmode(_lootmode),
        groupid(_groupid), mincount(_mincount), maxcount(_maxcount)
  {
  }

  bool               Roll(bool rate, Player const* player, Loot& loot,
                          LootStore const& store) const; // Checks if the entry takes it's chance (at loot generation)
...
struct LootItem
{
  ...
  ObjectGuid       rollWinnerGUID; // Stores the guid of person who won loot, if his bags are
                                   // full only he can see the item in loot list!
  uint8 count : 8;
  bool  is_looted : 1;
  bool  is_blocked : 1;
  ...
  // Constructor, copies most fields from LootStoreItem, generates random count
  // and random suffixes/properties Should be called for non-reference
...
public:
  ...
  void ReportInvalidCount(uint32 lootId, const char* ownerType, uint32 ownerId, uint32 itemId, uint8 minCount,
...
public:
  ...
  void Process(Loot& loot, LootStore const& store, uint16 lootMode, Player const* player, uint8 groupId = 0,
...
private:
  ...
  LootGroups        Groups;  // groups have own (optimised) processing, grouped entries
                             // go there
...
struct Loot
{
  ...
  [[nodiscard]] QuestItemMap const& GetPlayerNonQuestNonFFAConditionalItems() const
  {
  ...
  ObjectGuid            roundRobinPlayer; // GUID of the player having the Round-Robin ownership
  ...
  // GUID of container that holds this loot (item_instance.entry), set for items
  // that can be looted
  ...
  void clear()
  {
    ...
    for (QuestItemMap::const_iterator itr = PlayerNonQuestNonFFAConditionalItems.begin();
  ...
  LootItem* LootItemInSlot(uint32 lootslot, Player* player, QuestItem** qitem = nullptr, QuestItem** ffaitem = nullptr,
