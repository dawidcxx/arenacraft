      ...
      for (std::vector<LootItem>::const_iterator itr = _loot.items.begin(); itr != _loot.items.end(); ++itr)
        if (itr->itemid == item->itemid && itr->groupid == item->groupid)
        {
          ...
          if (_proto->InventoryType == 0 && foundDuplicates == 3 &&
              _proto->ItemId != 47242 /*Trophy of the Crusade*/) // Non-equippable items are limited to 3 drops
            return true;
...
public:
  ...
  void
        Process(Loot& loot, Player const* player, LootStore const& lootstore, uint16 lootMode,
...
uint32 LootStore::LoadLootTable()
{
  ...
  QueryResult result = WorldDatabase.Query(
      "SELECT Entry, Item, Reference, Chance, QuestRequired, LootMode, GroupId, MinCount, MaxCount FROM {}", GetName());
  ...
  do
  {
    ...
    {
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: MaxCount value ({}) to large. must be less {} - skipped",
                GetName(), entry, item, maxcount, std::numeric_limits<uint8>::max());
      continue; // error already printed to log/console.
    }

    if (lootmode == 0)
    {
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: LootMode is equal to 0, item will never drop - setting mode 1",
                GetName(), entry, item);
      lootmode = 1;
    }
...
void LootStore::ReportUnusedIds(LootIdSet const& lootIdSet) const
{
  ...
  for (LootIdSet::const_iterator itr = lootIdSet.begin(); itr != lootIdSet.end(); ++itr)
    LOG_ERROR("sql.sql", "Table '{}' Entry {} isn't {} and not referenced from loot, and thus useless.", GetName(),
              *itr, GetEntryName());
}
...
{
  LOG_ERROR("sql.sql", "Table '{}' Entry {} does not exist but it is used by {} {}", GetName(), lootId, ownerType,
            ownerId);
}

void LootStore::ReportInvalidCount(uint32 lootId, const char* ownerType, uint32 ownerId, uint32 itemId, uint8 minCount,
                                   uint8 maxCount) const
{
  LOG_ERROR("sql.sql",
            "Table '{}' Entry {} used by {} entry {} item {} has minCount ( {} ) != maxCount ( {} ) which is not "
            "supported for this loot type.",
            GetName(), lootId, ownerType, ownerId, itemId, minCount, maxCount);
}
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
  ...
  {
    LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: GroupId ({}) must be less {} - skipped", store.GetName(), entry,
              itemid, groupid, 1 << 7);
    return false;
  }
  ...
  if (mincount == 0)
  {
    LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: wrong MinCount ({}) - skipped", store.GetName(), entry, itemid,
              mincount);
    return false;
  }
  ...
  if (!reference) // item (quest or non-quest) entry, maybe grouped
  {
    ...
    if (!proto)
    {
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: item entry not listed in `item_template` - skipped",
    ...
    {
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: equal-chanced grouped entry, but group not defined - skipped",
                store.GetName(), entry, itemid);
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: low chance ({}) - skipped", store.GetName(), entry, itemid,
                chance);
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: MaxCount ({}) less that MinCount ({}) - skipped",
  ...
  {
    if (needs_quest)
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: quest required will be ignored", store.GetName(), entry,
                itemid);
    else if (chance == 0) // no chance for the reference
    {
      LOG_ERROR("sql.sql", "Table '{}' Entry {} Item {}: zero chance is specified for a reference, skipped",
                store.GetName(), entry, itemid);
      return false;
    }
  }
...
bool LootItem::AllowedForPlayer(Player const* player, ObjectGuid source) const
{
  ...
  if (pProto->Class == ITEM_CLASS_RECIPE && pProto->Bonding == BIND_WHEN_PICKED_UP && pProto->Spells[1].SpellId != 0 &&
  ...
  if (!pProto->HasFlagCu(ITEM_FLAGS_CU_IGNORE_QUEST_STATUS))
  {
    ...
    {
      // Don't drop the item if the player has already finished the quest OR player already has the item in their
      // inventory, and that item is unique OR the player has not finished a prerequisite quest
...
void LootItem::AddAllowedLooter(Player const* player) { allowedGUIDs.insert(player->GetGUID()); }
                    ...
                    uint16 lootMode /*= LOOT_MODE_DEFAULT*/, WorldObject* lootSource /*= nullptr*/)
{
  ...
  if (!personal && group)
  {
    ...
    {
      if (Player* player = itr->GetSource()) // should actually be looted object instead of lootOwner but looter has to
    ...
    if (itr != PlayerQuestItems.end() && questSlot < itr->second->size())
    {
      ...
      if (item->follow_loot_rules &&
          !item->AllowedForPlayer(player,
                                  sourceWorldObjectGUID)) // pussywizard: such items (follow_loot_rules) are added to
                                                          // every player, but not everyone is allowed, check it here
        return nullptr;
...
ByteBuffer& operator<<(ByteBuffer& b, LootView const& lv)
{
  ...
  case RESTRICTED_PERMISSION:
  {
    ...
    {
      if (!l.items[i].is_looted && !l.items[i].freeforall && (l.items[i].conditions.empty() || isMasterLooter) &&
...
LootStoreItem const* LootTemplate::LootGroup::Roll(Loot& loot, Player const* player, LootStore const& store,
...
void LootTemplate::LootGroup::Process(Loot& loot, Player const* player, LootStore const& store, uint16 lootMode,
...
void LootTemplate::LootGroup::Verify(LootStore const& lootstore, uint32 id, uint8 group_id) const
{
  ...
  {
    LOG_ERROR("sql.sql", "Table '{}' entry {} group {} has total chance > 100% ({})", lootstore.GetName(), id, group_id,
              chance);
  }

  if (chance >= 100.0f && !EqualChanced.empty())
  {
    LOG_ERROR("sql.sql", "Table '{}' entry {} group {} has items with chance=0% but group total chance >= 100% ({})",
  ...
  for (LootStoreItemList::const_iterator ieItr = ExplicitlyChanced.begin(); ieItr != ExplicitlyChanced.end(); ++ieItr)
  {
    ...
    if (item->reference)
    {
      if (item->mincount != item->maxcount)
        LootTemplates_Reference.ReportInvalidCount(std::abs(item->reference), lootstore.GetName(), Id, item->itemid,
...
void LootTemplate::Process(Loot& loot, LootStore const& store, uint16 lootMode, Player const* player, uint8 groupId,
...
void LoadLootTemplates_Disenchant()
{
  ...
  if (count)
    LOG_INFO("server.loading", ">> Loaded {} disenchanting loot templates in {} ms", count,
...
void LoadLootTemplates_Pickpocketing()
{
  ...
  if (count)
    LOG_INFO("server.loading", ">> Loaded {} pickpocketing loot templates in {} ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_WARN("server.loading",
...
void LoadLootTemplates_Prospecting()
{
  ...
  if (count)
    LOG_INFO("server.loading", ">> Loaded {} prospecting loot templates in {} ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
