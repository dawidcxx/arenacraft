...
void AutobroadcastMgr::LoadAutobroadcasts()
{
  ...
  if (_announceType < AnnounceType::World || _announceType > AnnounceType::Both)
  {
    LOG_ERROR("autobroadcast",
              "AutobroadcastMgr::LoadAutobroadcasts: Config option AutoBroadcast.Center set to not allowed value {}. "
...
void AutobroadcastMgr::SendAutobroadcasts()
{
  ...
  for (AutobroadcastsWeightMap::const_iterator it = _autobroadcastsWeights.begin(); it != _autobroadcastsWeights.end();
       ++it)
  {
    if (it->second)
    {
