...
template <class T, class U> T* EnsureAI(U* ai)
{
...
struct DefaultTargetSelector : public Acore::unary_function<Unit*, bool>
{
  ...
  DefaultTargetSelector(Unit const* unit, float dist, bool playerOnly, bool withMainTank, int32 aura)
      : me(unit), m_dist(dist), except(!withMainTank ? me->GetThreatMgr().GetCurrentVictim() : nullptr),
...
struct FarthestTargetSelector : public Acore::unary_function<Unit*, bool>
{
  FarthestTargetSelector(Unit const* unit, float maxDist, bool playerOnly, bool inLos, float minDist = 0.f)
      : _me(unit), _minDist(minDist), _maxDist(maxDist), _playerOnly(playerOnly), _inLos(inLos)
  {
  }
...
public:
  ...
  virtual void AttackStart(Unit* /*target*/); /// @brief Use to start attacking a target.
                                              /// Called just before JustEngagedWith()
  ...
  Unit* SelectTarget(SelectTargetMethod targetType, uint32 position = 0, float dist = 0.0f, bool playerOnly = false,
                     bool withTank = true, int32 aura = 0);

  // Select the best target (in <targetType> order) satisfying <predicate> from
  // the threat list. If <offset> is nonzero, the first <offset> entries in
  // <targetType> order (or SelectTargetMethod::MaxThreat order, if <targetType>
  // is SelectTargetMethod::Random) are skipped.
  template <class PREDICATE>
  Unit* SelectTarget(SelectTargetMethod targetType, uint32 position, PREDICATE const& predicate)
  {
    ThreatMgr& mgr = GetThreatMgr();
    // shortcut: if we ignore the first <offset> elements, and there are at most
    // <offset> elements, then we ignore ALL elements
  ...
  /** @brief Select the best (up to) <num> targets (in <targetType> order) from
   * the threat list that fulfill the following:
   * - Not among the first <offset> entries in <targetType> order (or
   * SelectTargetMethod::MaxThreat order, if <targetType> is
   ...
   */
  void SelectTargetList(std::list<Unit*>& targetList, uint32 num, SelectTargetMethod targetType, uint32 position = 0,
                        float dist = 0.0f, bool playerOnly = false, bool withTank = true, int32 aura = 0);

  // Select the best (up to) <num> targets (in <targetType> order) satisfying
  // <predicate> from the threat list and stores them in <targetList> (which is
  // cleared first). If <offset> is nonzero, the first <offset> entries in
  // <targetType> order (or SelectTargetMethod::MaxThreat order, if <targetType>
  // is SelectTargetMethod::Random) are skipped.
  template <class PREDICATE>
  void SelectTargetList(std::list<Unit*>& targetList, uint32 num, SelectTargetMethod targetType, uint32 position,
                        PREDICATE const& predicate)
  {
    targetList.clear();
    ThreatMgr& mgr = GetThreatMgr();
    // shortcut: we're gonna ignore the first <offset> elements, and there's at
    // most <offset> elements, so we ignore them all - nothing to do here
    if (mgr.GetThreatListSize() <= position)
      return;
   ...
   * @note NOTE: Creature engage logic should NOT be here, but in
   * JustEngagedWith, which happens once threat is established!)
   *
   ...
   *  for attack reaction use AttackedBy called for non DOT damage in
   * Unit::DealDamage also
   */
  virtual void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/, DamageEffectType /*damagetype*/,
  ...
  /// @brief Called during calculation when receiving periodic healing or damage
  /// (DoT or HoT)
  ...
  } /// @brief To specify the caster as target if the spell is self-cast
  SpellCastResult DoCastToAllHostilePlayers(uint32 spellid, bool triggered = false);
  SpellCastResult DoCastVictim(uint32 spellId, bool triggered = false);
  SpellCastResult DoCastAOE(uint32 spellId, bool triggered = false);
  SpellCastResult DoCastRandomTarget(uint32 spellId, uint32 threatTablePosition = 0, float dist = 0.0f,
                                     bool playerOnly = true, bool triggered = false, bool withTank = true);

  /// @brief Cast spell on the top threat target, which may not be the current
  /// victim.
  SpellCastResult DoCastMaxThreat(uint32 spellId, uint32 threatTablePosition = 0, float dist = 0.0f,
                                  bool playerOnly = true, bool triggered = false);

  float DoGetSpellMaxRange(uint32 spellId, bool positive = false);

  void DoMeleeAttackIfReady();
  bool DoSpellAttackIfReady(uint32 spell);
  void DoSpellAttackToRandomTargetIfReady(uint32 spell, uint32 threatTablePosition = 0, float dist = 0.f,
