 ...
 * @param WorldObject target The target of the speech, in case it has elements such as $n, where the target's name will
 * be referrenced.
...
void CreatureAI::DoZoneInCombat(Creature* creature /*= nullptr*/, float maxRangeToNearestTarget /*= 250.0f*/)
{
  ...
  {
    LOG_ERROR("entities.unit.ai", "DoZoneInCombat call for map {} that isn't a dungeon (creature entry = {})",
              map->GetId(), creature->IsCreature() ? creature->ToCreature()->GetEntry() : 0);
    return;
  }
...
void CreatureAI::MoveInLineOfSight(Unit* who)
{
  ...
  if (me->IsMoveInLineOfSightDisabled())
    if (me->GetCreatureType() == CREATURE_TYPE_NON_COMBAT_PET || // nothing more to do, return
        ...
        !me->IsWithinDist(who, ATTACK_DISTANCE, true,
...
void CreatureAI::MoveCircleChecks()
{
  ...
  if (!victim || !me->IsFreeToMove() || me->HasUnitMovementFlag(MOVEMENTFLAG_ROOT) || !me->IsWithinMeleeRange(victim) ||
      me == victim->GetVictim() || (!victim->IsPlayer() && !victim->IsPet()) // only player & pets to save CPU
  )
  {
    return;
  }
...
Creature* CreatureAI::DoSummon(uint32 entry, WorldObject* obj, float radius, uint32 despawnTime,
...
Creature* CreatureAI::DoSummonFlyer(uint32 entry, WorldObject* obj, float flightZ, float radius, uint32 despawnTime,
                                    TempSummonType summonType)
{
  Position pos = obj->GetRandomNearPosition(radius);
