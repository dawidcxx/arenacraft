...
public:
  ...
  virtual void UpdateFollowerAI(uint32);  // used when it's needed to add code in update
                                          // (abilities, scripted events, etc)

  void StartFollow(Player* player, uint32 factionForFollower = 0, const Quest* quest = nullptr);

  void SetFollowPaused(bool bPaused); // if special event require follow mode to
                                      // hold/resume during the follow
