...
void ScriptedAI::DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damagetype*/,
...
void ScriptedAI::DoPlaySoundToSet(WorldObject* source, uint32 soundId)
{
  ...
  {
    LOG_ERROR("entities.unit.ai", "Invalid soundId {} used in DoPlaySoundToSet (Source: {})", soundId,
...
Creature* ScriptedAI::DoSpawnCreature(uint32 entry, float offsetX, float offsetY, float offsetZ, float angle,
                                      uint32 type, uint32 despawntime)
{
  return me->SummonCreature(entry, me->GetPositionX() + offsetX, me->GetPositionY() + offsetY,
                                    ...
                                    Milliseconds repeatMin, Milliseconds repeatMax, uint32 uniqueId)
{
  ...
  scheduler.Schedule(timerMin == 0s ? timerMax : timerMin, timerMax,
...
void ScriptedAI::DoResetThreatList()
{
  if (!me->CanHaveThreatList() || me->GetThreatMgr().isThreatListEmpty())
  {
    LOG_ERROR("entities.unit.ai",
              "DoResetThreatList called for creature that either cannot have threat list or has empty threat list (me "
              "entry = {})",
              me->GetEntry());
    return;
  }
...
BossAI::BossAI(Creature* creature, uint32 bossId)
    : ScriptedAI(creature), instance(creature->GetInstanceScript()), summons(creature), _bossId(bossId),
...
void BossAI::SummonedCreatureDespawn(Creature* summon) { summons.Despawn(summon); }
...
void WorldBossAI::SummonedCreatureDespawn(Creature* summon) { summons.Despawn(summon); }
