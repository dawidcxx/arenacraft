...
bool SmartScript::IsSmart(GameObject* g, bool silent) const
{
  ...
  if (!smart && !silent)
    LOG_ERROR("sql.sql",
              "SmartScript: Action target GameObject(entry: {}) is not using SmartGameObjectAI, action skipped to "
              "prevent crash.",
...
void SmartScript::ProcessEventsFor(SMART_EVENT e, Unit* unit, uint32 var0, uint32 var1, bool bvar,
                                   SpellInfo const* spell, GameObject* gob)
{
  for (SmartAIEventList::iterator i = mEvents.begin(); i != mEvents.end(); ++i)
  {
                                ...
                                SpellInfo const* spell, GameObject* gob)
{
  ...
  if (WorldObject* tempInvoker = GetLastInvoker())
    LOG_DEBUG("sql.sql", "SmartScript::ProcessAction: Invoker: {} ({})", tempInvoker->GetName(),
  ...
  case SMART_ACTION_TALK:
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartScript::ProcessAction: SMART_ACTION_TALK: EntryOrGuid {} SourceType {} EventType {} TargetType "
                "{} using non-existent Text id {} for talker {}, ignored.",
                e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetTargetType(), e.action.talk.textGroupID,
                talker->GetEntry());
      break;
    }
    ...
    LOG_DEBUG("sql.sql", "SmartScript::ProcessAction: SMART_ACTION_TALK: talker: {} ({}), textId: {}",
              talker->GetName(), talker->GetGUID().ToString(), mLastTextID);
    break;
  }
  case SMART_ACTION_SIMPLE_TALK:
  {
    for (WorldObject* target : targets)
    {
      if (IsCreature(target))
        sCreatureTextMgr->SendChat(target->ToCreature(), uint8(e.action.simpleTalk.textGroupID),
                                   IsPlayer(GetLastInvoker()) ? GetLastInvoker() : 0);
      else if (IsPlayer(target) && me)
      {
        WorldObject* templastInvoker = GetLastInvoker();
        sCreatureTextMgr->SendChat(me, uint8(e.action.simpleTalk.textGroupID),
                                   IsPlayer(templastInvoker) ? templastInvoker : 0, CHAT_MSG_ADDON, LANG_ADDON,
                                   TEXT_RANGE_NORMAL, 0, TEAM_NEUTRAL, false, target->ToPlayer());
      }
      ...
      for (WorldObject* target : targets)
      {
        if (IsUnit(target))
        {
          ...
          LOG_DEBUG(
              ...
              target->GetName(), target->GetGUID().ToString(), e.action.music.sound, e.action.music.onlySelf,
              e.action.music.type);
        }
      }
    }
  ...
  case SMART_ACTION_RANDOM_MUSIC:
  {
    ...
    for (WorldObject* target : targets)
    {
      if (IsUnit(target))
      {
        ...
        LOG_DEBUG("sql.sql",
                  "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_MUSIC: target: {} ({}), sound: {}, onlyself: {}, "
                  "type: {}",
                  target->GetName(), target->GetGUID().ToString(), sound, e.action.randomMusic.onlySelf,
          ...
          if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(target->ToCreature()->GetEntry()))
          {
            if (target->ToCreature()->GetFaction() != ci->faction)
            {
              ...
              LOG_DEBUG("scripts.ai",
                        "SmartScript::ProcessAction:: SMART_ACTION_SET_FACTION: Creature entry {}, GuidLow {} set "
                        "faction to {}",
      ...
      if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
      {
        ...
        if (e.action.morphOrMount.creature)
        {
          if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(e.action.morphOrMount.creature))
          {
            ...
            LOG_DEBUG("scripts.ai",
                      "SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry {}, GuidLow "
                      "{} set displayid to {}",
  ...
  case SMART_ACTION_RANDOM_EMOTE:
  {
    ...
    std::copy_if(e.action.randomEmote.emotes.begin(), e.action.randomEmote.emotes.end(), std::back_inserter(emotes),
  ...
  case SMART_ACTION_THREAT_ALL_PCT:
  {
    ...
    for (ThreatContainer::StorageType::const_iterator i = threatList.begin(); i != threatList.end(); ++i)
    {
      ...
      {
        me->GetThreatMgr().ModifyThreatByPercent(target, e.action.threatPCT.threatINC
  ...
  case SMART_ACTION_THREAT_SINGLE_PCT:
  {
    ...
    for (WorldObject* target : targets)
    {
      if (IsUnit(target))
      {
        me->GetThreatMgr().ModifyThreatByPercent(target->ToUnit(), e.action.threatPCT.threatINC
                                                                       ? (int32)e.action.threatPCT.threatINC
                                                                       : -(int32)e.action.threatPCT.threatDEC);
        LOG_DEBUG("scripts.ai",
                  "SmartScript::ProcessAction:: SMART_ACTION_THREAT_SINGLE_PCT: Creature guidLow {} modify threat for "
                  "unit {}, value {}",
                  me->GetGUID().ToString(), target->GetGUID().ToString(),
  ...
  case SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS:
  {
    for (WorldObject* target : targets)
    {
      ...
      if (IsPlayer(target))
      {
        ...
        LOG_DEBUG("scripts.ai",
                  "SmartScript::ProcessAction:: SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS: Player guidLow {} "
                  "credited quest {}",
                  target->GetGUID().ToString(), e.action.quest.quest);
      }
    }
  ...
  case SMART_ACTION_CAST:
  {
    ...
    if (e.GetScriptType() == SMART_SCRIPT_TYPE_AREATRIGGER)
      caster = unit->SummonTrigger(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ(),
                                   unit->GetOrientation(), 5000);
    ...
    for (WorldObject* target : targets)
    {
      ...
      else if (me)
      {
        ...
        {
          // If cast flag SMARTCAST_COMBAT_MOVE is set combat movement will not be allowed unless target is outside
          // spell range, out of mana, or LOS.
          if (result == SPELL_FAILED_OUT_OF_RANGE)
            // if we are just out of range, we only chase until we are back in spell range.
            CAST_AI(SmartAI, me->AI())->SetCombatMove(true, std::max(spellMaxRange - NOMINAL_MELEE_RANGE, 0.0f));
          else
            // if spell fail for any other reason, we chase to melee range, or stay where we are if spellcast was
            // successful.
  ...
  case SMART_ACTION_INVOKER_CAST:
  {
    ...
    for (WorldObject* target : targets)
    {
      ...
      if (!(e.action.cast.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !target->ToUnit()->HasAura(e.action.cast.spell))
      {
        ...
        LOG_DEBUG("scripts.ai",
                  "SmartScript::ProcessAction:: SMART_ACTION_INVOKER_CAST: Invoker {} casts spell {} on target {} with "
                  "castflags {}",
                  tempLastInvoker->GetGUID().ToString(), e.action.cast.spell, target->GetGUID().ToString(),
    ...
    for (WorldObject* target : targets)
    {
      if (IsUnit(target))
      {
        ...
        else
        {
          ...
          LOG_DEBUG("scripts.ai",
                    "SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. Unit {} removed flag {} to "
                    "UNIT_FIELD_FLAGS_2",
                    target->GetGUID().ToString(), e.action.unitFlag.flag);
        }
      }
  ...
  case SMART_ACTION_INC_EVENT_PHASE:
  {
    ...
    LOG_DEBUG("sql.sql",
              ...
              "decrease by {}",
              GetBaseObject()->GetGUID().ToString(), e.action.incEventPhase.inc, e.action.incEventPhase.dec);
    break;
  }
  ...
  case SMART_ACTION_FLEE_FOR_ASSIST:
  {
    ...
    LOG_DEBUG("sql.sql", "SmartScript::ProcessAction:: SMART_ACTION_FLEE_FOR_ASSIST: Creature {} DoFleeToGetAssistance",
              me->GetGUID().ToString());
    break;
  }
    ...
    for (WorldObject* target : targets)
    {
      if (IsUnit(target))
      {
        ...
        CAST_AI(SmartAI, me->AI())
            ->SetFollow(target->ToUnit(), float(e.action.follow.dist) + 0.1f, angle, e.action.follow.credit,
                        e.action.follow.entry, e.action.follow.creditType, e.action.follow.aliveState);
  ...
  case SMART_ACTION_RANDOM_PHASE:
  {
    ...
    std::copy_if(e.action.randomPhase.phases.begin(), e.action.randomPhase.phases.end(), std::back_inserter(phases),
  ...
  case SMART_ACTION_SET_INST_DATA:
  {
    ...
    case 0:
    {
      ...
      LOG_DEBUG("scripts.ai.sai", "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA: Field: {}, data: {}",
                e.action.setInstanceData.field, e.action.setInstanceData.data);
    }
    break;
    case 1:
    {
      instance->SetBossState(e.action.setInstanceData.field,
  ...
  case SMART_ACTION_SET_INST_DATA64:
  {
    ...
    if (!instance)
    {
      LOG_ERROR("sql.sql", "SmartScript: Event {} attempt to set instance data without instance script. EntryOrGuid {}",
                e.GetEventType(), e.entryOrGuid);
      break;
    }
  ...
  case SMART_ACTION_UPDATE_TEMPLATE:
  {
    for (WorldObject* target : targets)
      if (IsCreature(target))
        target->ToCreature()->UpdateEntry(e.action.updateTemplate.creature, target->ToCreature()->GetCreatureData(),
      ...
      if (me && !me->isDead())
      {
        me->m_Events.AddEventAtOffset(
            [&]
            {
              ...
              {
                me->KillSelf();
                LOG_DEBUG("sql.sql", "SmartScript::ProcessAction: SMART_ACTION_DIE: Creature {}",
                          me->GetGUID().ToString());
              }
            },
  ...
  case SMART_ACTION_CALL_FOR_HELP:
  {
    for (WorldObject* target : targets)
    {
      if (IsCreature(target))
      {
        ...
        {
          Acore::BroadcastTextBuilder builder(target, CHAT_MSG_MONSTER_EMOTE, BROADCAST_TEXT_CALL_FOR_HELP,
    ...
    for (WorldObject* target : targets)
    {
      if (e.action.forceDespawn.removeObjectFromWorld)
      {
        if (e.action.forceDespawn.delay || e.action.forceDespawn.forceRespawnTimer)
          LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_FORCE_DESPAWN has removeObjectFromWorld set. delay and "
                               "forceRespawnTimer ignored.");
  ...
  case SMART_ACTION_RISE_UP:
  {
    ...
    me->GetMotionMaster()->MovePoint(SMART_RANDOM_POINT, me->GetPositionX(), me->GetPositionY(),
  ...
  case SMART_ACTION_SUMMON_CREATURE:
  {
    ...
    if (e.GetTargetType() == SMART_TARGET_RANDOM_POINT)
    {
      ...
      for (uint32 i = 0; i < e.target.randomPoint.amount; i++)
      {
        ...
        if (Creature* summon = summoner->SummonCreature(e.action.summonCreature.creature, randomPoint,
                                                        (TempSummonType)e.action.summonCreature.type,
                                                        e.action.summonCreature.duration, 0, nullptr, personalSpawn))
        {
    ...
    for (WorldObject* target : targets)
    {
      ...
      z += e.target.z;
      o += e.target.o;
      if (Creature* summon = summoner->SummonCreature(e.action.summonCreature.creature, x, y, z, o,
                                                      (TempSummonType)e.action.summonCreature.type,
                                                      e.action.summonCreature.duration, nullptr, personalSpawn))
      {
    ...
    if (Creature* summon =
            summoner->SummonCreature(e.action.summonCreature.creature, e.target.x, e.target.y, e.target.z, e.target.o,
                                     (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
    {
      if (unit && e.action.summonCreature.attackInvoker)
        summon->AI()->AttackStart(unit);
      else if (me && e.action.summonCreature.attackScriptOwner)
  ...
  case SMART_ACTION_SUMMON_GO:
  {
    ...
    if (!targets.empty())
    {
      ...
      for (WorldObject* target : targets)
      {
        ...
        if (!e.action.summonGO.targetsummon)
          GetBaseObject()->SummonGameObject(e.action.summonGO.entry, x, y, z, o, 0, 0, 0, 0,
                                            e.action.summonGO.despawnTime);
        else
          target->SummonGameObject(e.action.summonGO.entry, GetBaseObject()->GetPositionX(),
                                   GetBaseObject()->GetPositionY(), GetBaseObject()->GetPositionZ(),
                                   GetBaseObject()->GetOrientation(), 0, 0, 0, 0, e.action.summonGO.despawnTime);
      }
    }
    ...
    GetBaseObject()->SummonGameObject(e.action.summonGO.entry, e.target.x, e.target.y, e.target.z, e.target.o, 0, 0, 0,
                                      0, e.action.summonGO.despawnTime);
    break;
  }
      ...
      for (WorldObject* target : targets)
      {
        if (IsCreature(target))
        {
          if (SmartAI* ai = CAST_AI(SmartAI, target->ToCreature()->AI()))
            ai->GetScript()->StoreCounter(e.action.setCounter.counterId, e.action.setCounter.value,
                                          e.action.setCounter.reset, e.action.setCounter.subtract);
          else
            LOG_ERROR("sql.sql",
  ...
  case SMART_ACTION_MOVE_TO_POS:
  {
    ...
    case SMART_TARGET_RANDOM_POINT:
    {
      if (me)
      {
        ...
        me->GetMotionMaster()->MovePoint(e.action.moveToPos.pointId, randomPoint.m_positionX, randomPoint.m_positionY,
                                         randomPoint.m_positionZ, true, true,
    ...
    default:
    {
      ...
      if (e.action.moveToPos.combatReach)
        target->GetNearPoint(me, x, y, z, target->GetCombatReach() + e.action.moveToPos.ContactDistance, 0,
                             target->GetAngle(me));
      ...
      me->GetMotionMaster()->MovePoint(e.action.moveToPos.pointId, x + e.target.x, y + e.target.y, z + e.target.z, true,
                                       true, isControlled ? MOTION_SLOT_CONTROLLED : MOTION_SLOT_ACTIVE);
  ...
  case SMART_ACTION_MOVE_TO_POS_TARGET:
  {
    for (WorldObject* target : targets)
    {
      if (IsCreature(target))
      {
        ...
        ctarget->GetMotionMaster()->MovePoint(e.action.moveToPos.pointId, e.target.x, e.target.y, e.target.z, true,
                                              true, isControlled ? MOTION_SLOT_CONTROLLED : MOTION_SLOT_ACTIVE);
      }
    }
  ...
  case SMART_ACTION_CALL_TIMED_ACTIONLIST:
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "SmartScript: Entry {} SourceType {} Event {} Action {} is using TARGET_NONE(0) for Script9 target. "
                "Please correct target_type in database.",
  ...
  case SMART_ACTION_CROSS_CAST:
  {
    ...
    GetTargets(casters,
               CreateSmartEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, 0, 0, 0, 0, SMART_ACTION_NONE, 0, 0, 0, 0, 0, 0,
                                (SMARTAI_TARGETS)e.action.crossCast.targetType, e.action.crossCast.targetParam1,
    ...
    for (WorldObject* caster : casters)
    {
      ...
      for (WorldObject* target : targets)
      {
            ...
            !target->ToUnit()->HasAura(e.action.crossCast.spell))
        {
          ...
          casterUnit->CastSpell(target->ToUnit(), e.action.crossCast.spell,
                                (e.action.crossCast.flags & SMARTCAST_TRIGGERED) != 0);
        }
        else
          LOG_DEBUG("scripts.ai",
                    "Spell {} not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target ({}) already has "
                    "the aura",
                    e.action.crossCast.spell, target->GetGUID().ToString());
      }
    }
    break;
  }
  ...
  case SMART_ACTION_JUMP_TO_POS:
  {
    ...
    if (e.action.jump.selfJump)
    {
      if (WorldObject* target = Acore::Containers::SelectRandomContainerElement(targets))
        if (me)
          me->GetMotionMaster()->MoveJump(target->GetPositionX() + e.target.x, target->GetPositionY() + e.target.y,
                                          target->GetPositionZ() + e.target.z, (float)e.action.jump.speedxy,
      ...
      for (WorldObject* target : targets)
        if (WorldObject* obj = (target))
        {
          if (Creature* creature = obj->ToCreature())
            creature->GetMotionMaster()->MoveJump(e.target.x, e.target.y, e.target.z, (float)e.action.jump.speedxy,
                                                  (float)e.action.jump.speedz);
        }
    }
    ...
    for (WorldObject* target : targets)
    {
      ...
      else if (IsGameObject(target))
      {
        if (SmartGameObjectAI* ai = CAST_AI(SmartGameObjectAI, target->ToGameObject()->AI()))
          ...
          LOG_ERROR("sql.sql", "SmartScript: Action target for SMART_ACTION_SEND_TARGET_TO_TARGET is not using "
      ...
      for (WorldObject* target : targets)
        if (IsCreature(target))
        {
          if (e.action.setHomePos.spawnPos)
          {
            ...
            target->ToCreature()->SetHomePosition(target->GetPositionX(), target->GetPositionY(),
  ...
  case SMART_ACTION_REMOVE_IMMUNITY:
  {
    for (WorldObject* target : targets)
      if (IsUnit(target))
        target->ToUnit()->ApplySpellImmune(e.action.immunity.id, e.action.immunity.type, e.action.immunity.value,
                                           false);
    break;
  }
    ...
    for (WorldObject* target : targets)
    {
      if (IsUnit(target))
      {
            ...
            !target->ToUnit()->HasAura(e.action.castCustom.spell))
        {
          ...
          me->CastCustomSpell(e.action.castCustom.spell, values, target->ToUnit(),
  ...
  case SMART_ACTION_VORTEX_SUMMON:
  {
    ...
    for (WorldObject* target : targets)
    {
      ...
      do
      {
        ...
        me->SummonCreature(e.action.summonVortex.summonEntry, summonPosition, summon_type,
  ...
  case SMART_ACTION_CONE_SUMMON:
  {
         ...
         radius += e.action.coneSummon.distanceBetweenRings)
    {
      ...
      for (uint32 index = 0; index < count; ++index)
      {
        ...
        me->SummonCreature(e.action.coneSummon.summonEntry, spawnPosition, spawnType,
  ...
  case SMART_ACTION_SET_MOVEMENT_SPEED:
  {
    ...
    float  speed =
        float(speedInteger) +
        float(speedFraction) / std::pow(10, std::floor(std::log10(float(speedFraction ? speedFraction : 1)) + 1));
  ...
  case SMART_ACTION_SUMMON_RADIAL:
  {
    ...
    if (e.action.radialSummon.dist)
    {
      for (uint32 itr = 0; itr < e.action.radialSummon.repetitions; itr++)
      {
        ...
        me->SummonCreature(e.action.radialSummon.summonEntry, summonPos, spawnType,
    ...
    for (uint32 itr = 0; itr < e.action.radialSummon.repetitions; itr++)
    {
      ...
      me->SummonCreature(e.action.radialSummon.summonEntry, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(),
                         currentAngle, spawnType, e.action.radialSummon.summonDuration);
    }
  ...
  case SMART_ACTION_FOLLOW_GROUP:
  {
    ...
    case FOLLOW_TYPE_SEMI_CIRCLE_FRONT:
    {
      for (WorldObject* target : targets)
      {
        if (IsCreature(target))
        {
          target->ToCreature()->GetMotionMaster()->MoveFollow(me, dist,
                                                              (M_PI + (M_PI / 2.0f) + (M_PI / membCount) * (itr - 1)));
          itr++;
        }
      }
      break;
    ...
    case FOLLOW_TYPE_COLUMN:
    {
      for (WorldObject* target : targets)
      {
        if (IsCreature(target))
        {
          target->ToCreature()->GetMotionMaster()->MoveFollow(me, dist * (((itr - 1) / 2) + 1),
    ...
    case 0: // Reset
    {
      for (WorldObject* target : targets)
        target->ToCreature()->SetFacingTo(
            (target->ToCreature()->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) && target->ToCreature()->GetTransGUID()
    ...
    case 3: // Target parameters
    {
      ...
      GetTargets(facingTargets,
                 CreateSmartEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, 0, 0, 0, 0, SMART_ACTION_NONE, 0, 0, 0, 0, 0, 0,
                                  ...
                                  e.action.orientationTarget.targetParam1, e.action.orientationTarget.targetParam2,
                                  e.action.orientationTarget.targetParam3, e.action.orientationTarget.targetParam4, 0),
                 unit);
  ...
  {
    sWorldState->HandleExternalEvent(static_cast<WorldStateEvent>(e.action.worldStateScript.eventId),
                                     e.action.worldStateScript.param);
    break;
  }
  default:
    LOG_ERROR("sql.sql", "SmartScript::ProcessAction: Entry {} SourceType {}, Event {}, Unhandled Action type {}",
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
  ...
  {
    AddEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, e.action.installTtemplate.param2, e.action.installTtemplate.param3, 0, 0,
             SMART_ACTION_CAST, e.action.installTtemplate.param1, e.target.raw.param1, 0, 0, 0, 0, SMART_TARGET_VICTIM,
             0, 0, 0, 0, 1);
    AddEvent(SMART_EVENT_RANGE, 0, e.action.installTtemplate.param4, 300, 0, 0, 0, 0,
             SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 1, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 1);
    AddEvent(SMART_EVENT_RANGE, 0, 0, e.action.installTtemplate.param4 > 10 ? e.action.installTtemplate.param4 - 10 : 0,
             0, 0, 0, 0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 0, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 1);
    AddEvent(SMART_EVENT_MANA_PCT, 0,
             e.action.installTtemplate.param5 - 15 > 100 ? 100 : e.action.installTtemplate.param5 + 15, 100, 1000, 1000,
             0, 0, SMART_ACTION_SET_EVENT_PHASE, 1, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_MANA_PCT, 0, 0, e.action.installTtemplate.param5, 1000, 1000, 0, 0,
             SMART_ACTION_SET_EVENT_PHASE, 0, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_MANA_PCT, 0, 0, e.action.installTtemplate.param5, 1000, 1000, 0, 0,
             SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 1, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    break;
  }
  case SMARTAI_TEMPLATE_TURRET:
  {
    AddEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, e.action.installTtemplate.param2, e.action.installTtemplate.param3, 0, 0,
             SMART_ACTION_CAST, e.action.installTtemplate.param1, e.target.raw.param1, 0, 0, 0, 0, SMART_TARGET_VICTIM,
             0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_JUST_CREATED, 0, 0, 0, 0, 0, 0, 0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 0, 0, 0, 0, 0, 0,
  ...
  case SMARTAI_TEMPLATE_CAGED_NPC_PART:
  {
    if (!me)
      return;
    // store cage as id1
    AddEvent(SMART_EVENT_DATA_SET, 0, 0, 0, 0, 0, 0, 0, SMART_ACTION_STORE_TARGET_LIST, 1, 0, 0, 0, 0, 0,
             SMART_TARGET_CLOSEST_GAMEOBJECT, e.action.installTtemplate.param1, 10, 0, 0, 0);

    // reset(close) cage on hostage(me) respawn
    AddEvent(SMART_EVENT_UPDATE, SMART_EVENT_FLAG_NOT_REPEATABLE, 0, 0, 0, 0, 0, 0, SMART_ACTION_RESET_GOBJECT, 0, 0, 0,
             0, 0, 0, SMART_TARGET_GAMEOBJECT_DISTANCE, e.action.installTtemplate.param1, 5, 0, 0, 0);

    AddEvent(SMART_EVENT_DATA_SET, 0, 0, 0, 0, 0, 0, 0, SMART_ACTION_SET_RUN, e.action.installTtemplate.param3, 0, 0, 0,
             0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_DATA_SET, 0, 0, 0, 0, 0, 0, 0, SMART_ACTION_SET_EVENT_PHASE, 1, 0, 0, 0, 0, 0,
             SMART_TARGET_NONE, 0, 0, 0, 0, 0);

    AddEvent(SMART_EVENT_UPDATE, SMART_EVENT_FLAG_NOT_REPEATABLE, 1000, 1000, 0, 0, 0, 0, SMART_ACTION_MOVE_FORWARD,
             e.action.installTtemplate.param4, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 1);
    // phase 1: give quest credit on movepoint reached
    AddEvent(SMART_EVENT_MOVEMENTINFORM, 0, POINT_MOTION_TYPE, SMART_RANDOM_POINT, 0, 0, 0, 0, SMART_ACTION_SET_DATA, 0,
             0, 0, 0, 0, 0, SMART_TARGET_STORED, 1, 0, 0, 0, 1);
    // phase 1: despawn after time on movepoint reached
    AddEvent(SMART_EVENT_MOVEMENTINFORM, 0, POINT_MOTION_TYPE, SMART_RANDOM_POINT, 0, 0, 0, 0,
             SMART_ACTION_FORCE_DESPAWN, e.action.installTtemplate.param2, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0,
             1);

    if (sCreatureTextMgr->TextExist(me->GetEntry(), (uint8)e.action.installTtemplate.param5))
      AddEvent(SMART_EVENT_MOVEMENTINFORM, 0, POINT_MOTION_TYPE, SMART_RANDOM_POINT, 0, 0, 0, 0, SMART_ACTION_TALK,
               e.action.installTtemplate.param5, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 1);
    break;
  }
  case SMARTAI_TEMPLATE_CAGED_GO_PART:
  {
    ...
    AddEvent(SMART_EVENT_GO_STATE_CHANGED, 0, 2, 0, 0, 0, 0, 0, SMART_ACTION_STORE_TARGET_LIST, 1, 0, 0, 0, 0, 0,
             SMART_TARGET_CLOSEST_CREATURE, e.action.installTtemplate.param1, 10, 0, 0, 0);
    // store invoker as id2
    AddEvent(SMART_EVENT_GO_STATE_CHANGED, 0, 2, 0, 0, 0, 0, 0, SMART_ACTION_STORE_TARGET_LIST, 2, 0, 0, 0, 0, 0,
             SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    // signal hostage
    AddEvent(SMART_EVENT_GO_STATE_CHANGED, 0, 2, 0, 0, 0, 0, 0, SMART_ACTION_SET_DATA, 0, 0, 0, 0, 0, 0,
             SMART_TARGET_STORED, 1, 0, 0, 0, 0);
    // when hostage raeched end point, give credit to invoker
    if (e.action.installTtemplate.param2)
      AddEvent(SMART_EVENT_DATA_SET, 0, 0, 0, 0, 0, 0, 0, SMART_ACTION_CALL_KILLEDMONSTER,
               e.action.installTtemplate.param1, 0, 0, 0, 0, 0, SMART_TARGET_STORED, 2, 0, 0, 0, 0);
    else
      AddEvent(SMART_EVENT_GO_STATE_CHANGED, 0, 2, 0, 0, 0, 0, 0, SMART_ACTION_CALL_KILLEDMONSTER,
               e.action.installTtemplate.param1, 0, 0, 0, 0, 0, SMART_TARGET_STORED, 2, 0, 0, 0, 0);
    break;
  }
...
void SmartScript::AddEvent(SMART_EVENT e, uint32 event_flags, uint32 event_param1, uint32 event_param2,
                           uint32 event_param3, uint32 event_param4, uint32 event_param5, uint32 event_param6,
                           SMART_ACTION action, uint32 action_param1, uint32 action_param2, uint32 action_param3,
                           uint32 action_param4, uint32 action_param5, uint32 action_param6, SMARTAI_TARGETS t,
                           uint32 target_param1, uint32 target_param2, uint32 target_param3, uint32 target_param4,
                           uint32 phaseMask)
{
  mInstallEvents.push_back(CreateSmartEvent(e, event_flags, event_param1, event_param2, event_param3, event_param4,
                                            event_param5, event_param6, action, action_param1, action_param2,
                                            action_param3, action_param4, action_param5, action_param6, t,
                                            target_param1, target_param2, target_param3, target_param4, phaseMask));
}
...
SmartScriptHolder SmartScript::CreateSmartEvent(SMART_EVENT e, uint32 event_flags, uint32 event_param1,
                                                ...
                                                uint32 event_param5, uint32 event_param6, SMART_ACTION action,
                                                uint32 action_param1, uint32 action_param2, uint32 action_param3,
                                                uint32 action_param4, uint32 action_param5, uint32 action_param6,
                                                SMARTAI_TARGETS t, uint32 target_param1, uint32 target_param2,
...
void SmartScript::GetTargets(ObjectVector& targets, SmartScriptHolder const& e,
                             WorldObject* invoker /*= nullptr*/) const
{
  ...
  case SMART_TARGET_HOSTILE_SECOND_AGGRO:
    if (me)
    {
      ...
      {
        if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::MaxThreat, 1,
                                             PowerUsersSelector(me, Powers(e.target.hostileRandom.powerType - 1),
                                                                (float)e.target.hostileRandom.maxDist,
                                                                e.target.hostileRandom.playerOnly)))
          targets.push_back(u);
      }
      else if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::MaxThreat, 1, (float)e.target.hostileRandom.maxDist,
                                                e.target.hostileRandom.playerOnly, true, -e.target.hostileRandom.aura))
        targets.push_back(u);
    }
    break;
  case SMART_TARGET_HOSTILE_LAST_AGGRO:
    if (me)
    {
      if (e.target.hostileRandom.powerType)
      {
        if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::MinThreat, 0,
                                             PowerUsersSelector(me, Powers(e.target.hostileRandom.powerType - 1),
                                                                (float)e.target.hostileRandom.maxDist,
                                                                e.target.hostileRandom.playerOnly)))
          targets.push_back(u);
      }
      else if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::MinThreat, 0, (float)e.target.hostileRandom.maxDist,
                                                e.target.hostileRandom.playerOnly, true, -e.target.hostileRandom.aura))
        targets.push_back(u);
    }
    break;
  case SMART_TARGET_HOSTILE_RANDOM:
    if (me)
    {
      if (e.target.hostileRandom.powerType)
      {
        if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::Random, 0,
                                             PowerUsersSelector(me, Powers(e.target.hostileRandom.powerType - 1),
                                                                (float)e.target.hostileRandom.maxDist,
                                                                e.target.hostileRandom.playerOnly)))
          targets.push_back(u);
      }
      else if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::Random, 0, (float)e.target.hostileRandom.maxDist,
                                                e.target.hostileRandom.playerOnly, true, -e.target.hostileRandom.aura))
        targets.push_back(u);
    }
    break;
  case SMART_TARGET_HOSTILE_RANDOM_NOT_TOP:
    if (me)
    {
      if (e.target.hostileRandom.powerType)
      {
        if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::Random, 1,
                                             PowerUsersSelector(me, Powers(e.target.hostileRandom.powerType - 1),
                                                                (float)e.target.hostileRandom.maxDist,
                                                                e.target.hostileRandom.playerOnly)))
          targets.push_back(u);
      }
      else if (Unit* u = me->AI()->SelectTarget(SelectTargetMethod::Random, 1, (float)e.target.hostileRandom.maxDist,
                                                e.target.hostileRandom.playerOnly, true, -e.target.hostileRandom.aura))
        targets.push_back(u);
    }
    break;
  case SMART_TARGET_FARTHEST:
    ...
    {
      if (Unit* u =
              me->AI()->SelectTarget(SelectTargetMethod::MinDistance, 0,
                                     FarthestTargetSelector(me, e.target.farthest.maxDist, e.target.farthest.playerOnly,
                                                            e.target.farthest.isInLos, e.target.farthest.minDist)))
        targets.push_back(u);
    }
    break;
  ...
  case SMART_TARGET_CREATURE_RANGE:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_CREATURE_RANGE: Entry {} SourceType {} Event {} Action {} Target {} is missing base "
                "object or invoker",
  ...
  case SMART_TARGET_GAMEOBJECT_RANGE:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_GAMEOBJECT_RANGE: Entry: {} SourceType {} Event {} Action {} Target {} is missing base "
                "object or invoker.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
      break;
    }
  ...
  case SMART_TARGET_CREATURE_GUID:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_CREATURE_GUID: Entry {} SourceType {} Event {} Action {} Target {} is missing base "
                "object or invoker.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
      break;
    }
  ...
  case SMART_TARGET_GAMEOBJECT_GUID:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_GAMEOBJECT_GUID: Entry {} SourceType {} Event {} Action {} Target {} is missing base "
                "object or invoker.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
      break;
    }
  ...
  case SMART_TARGET_PLAYER_RANGE:
  {
    ...
    if (!units.empty() && baseObject)
      for (WorldObject* unit : units)
        if (IsPlayer(unit) &&
            baseObject->IsInRange(unit, float(e.target.playerRange.minDist), float(e.target.playerRange.maxDist)))
          targets.push_back(unit);
  ...
  case SMART_TARGET_CLOSEST_CREATURE:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_CLOSEST_CREATURE: Entry {} SourceType {} Event {} Action {} Target {} is missing base "
                "object or invoker.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
      break;
    }

    Creature* target = GetClosestCreatureWithEntry(ref, e.target.unitClosest.entry,
  ...
  case SMART_TARGET_CLOSEST_GAMEOBJECT:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_CLOSEST_GAMEOBJECT: Entry {} SourceType {} Event {} Action {} Target {} is missing base "
                "object or invoker.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
      break;
    }

    GameObject* target = GetClosestGameObjectWithEntry(ref, e.target.goClosest.entry,
  ...
  case SMART_TARGET_CLOSEST_PLAYER:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_CLOSEST_PLAYER: Entry {} SourceType {} Event {} Action {} Target {} is missing base "
                "object or invoker.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
      break;
    }
  ...
  case SMART_TARGET_CLOSEST_ENEMY:
  {
    if (me)
      if (Unit* target =
              me->SelectNearestTarget(e.target.closestAttackable.maxDist, e.target.closestAttackable.playerOnly))
        targets.push_back(target);
  ...
  case SMART_TARGET_CLOSEST_FRIENDLY:
  {
    if (me)
      if (Unit* target =
              DoFindClosestFriendlyInRange(e.target.closestFriendly.maxDist, e.target.closestFriendly.playerOnly))
        targets.push_back(target);
  ...
  case SMART_TARGET_PLAYER_WITH_AURA:
  {
    ...
    for (WorldObject* unit : units)
      if (IsPlayer(unit) && unit->ToPlayer()->IsAlive() && !unit->ToPlayer()->IsGameMaster())
        if (GetBaseObject()->IsInRange(unit, (float)e.target.playerWithAura.distMin,
  ...
  case SMART_TARGET_INSTANCE_STORAGE:
  {
    ...
    {
      LOG_ERROR("scripts.ai.sai",
                "SMART_TARGET_INSTANCE_STORAGE: Entry {} SourceType {} Event {} Action {} Target {} called outside an "
                "instance map.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
    }
                               ...
                               SpellInfo const* spell, GameObject* gob)
{
  ...
  case SMART_EVENT_FRIENDLY_HEALTH:
  {
    ...
    {
      // Xinef: if there are at least two same npcs, they will perform the same action immediately even if this is
      // useless...
  ...
  case SMART_EVENT_FRIENDLY_IS_CC:
  {
    ...
    ProcessTimedAction(e, e.event.friendlyCC.repeatMin, e.event.friendlyCC.repeatMax,
  ...
  case SMART_EVENT_FRIENDLY_MISSING_BUFF:
  {
    ...
    ProcessTimedAction(e, e.event.missingBuff.repeatMin, e.event.missingBuff.repeatMax,
                       Acore::Containers::SelectRandomContainerElement(creatures));
    break;
  }
  ...
  case SMART_EVENT_FRIENDLY_HEALTH_PCT:
  {
    ...
    case SMART_TARGET_ACTION_INVOKER:
      unitTarget =
          DoSelectLowestHpPercentFriendly((float)e.event.friendlyHealthPct.radius, 0, e.event.friendlyHealthPct.hpPct);
      break;
    default:
      return;
    }
...
void SmartScript::SortEvents(SmartAIEventList& events) { std::sort(events.begin(), events.end()); }
...
void SmartScript::RetryLater(SmartScriptHolder& e, bool ignoreChanceRoll)
{
  ...
  // This allows to retry the action later without rolling again the chance roll (which might fail and end up not
...
void SmartScript::SetScript9(SmartScriptHolder& e, uint32 entry)
{
  ...
  {
    LOG_ERROR("scripts.ai.sai",
              "Entry {} SourceType {} Event {} Action {} is trying to overwrite timed action list from a timed action, "
              "this is not allowed!.",
              e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
    return;
  }

  // Do NOT allow to start a new actionlist if a previous one is already running, unless explicitly allowed. We need to
  // always finish the current actionlist
...
bool SmartScript::IsUnit(WorldObject* obj) { return obj && (obj->IsCreature() || obj->IsPlayer()); }

bool SmartScript::IsPlayer(WorldObject* obj) { return obj && obj->IsPlayer(); }

bool SmartScript::IsCreature(WorldObject* obj) { return obj && obj->IsCreature(); }
...
bool SmartScript::IsGameObject(WorldObject* obj) { return obj && obj->IsGameObject(); }
