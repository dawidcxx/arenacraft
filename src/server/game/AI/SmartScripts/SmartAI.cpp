...
WayPoint* SmartAI::GetNextWayPoint()
{
  ...
  if (itr != mWayPoints->end())
  {
    ...
    if (mLastWP->id != mCurrentWPID)
      LOG_ERROR("scripts.ai.sai", "SmartAI::GetNextWayPoint: Got not expected waypoint id {}, expected {}", mLastWP->id,
                mCurrentWPID);
...
void SmartAI::EndPath(bool fail)
{
  ...
  // Xinef: End Path events should be only processed if it was SUCCESSFUL stop or stop called by
  // SMART_ACTION_WAYPOINT_STOP
  if (fail)
  {
    mCurrentWPID = 0;
    return;
...
void SmartAI::UpdatePath(const uint32 diff)
{
  ...
  if (mEscortInvokerCheckTimer < diff)
  {
    ...
    if (!IsEscortInvokerInRange())
    {
      ...
      // Xinef: allow to properly hook out of range despawn action, which in most cases should perform the same
      // operation as dying
  ...
  if (!me->HasUnitState(UNIT_STATE_NOT_MOVE) && me->movespline->Finalized()) // reached WP
  {
    ...
    {
      SetRun(mRun);
      // xinef: if we have reached waypoint, and there is no working spline movement it means our splitted array has
      // ended, make new one
...
void SmartAI::MovementInform(uint32 MovementType, uint32 Data)
{
  ...
  if (MovementType == WAYPOINT_MOTION_TYPE)
    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_DATA_REACHED, nullptr,
...
bool SmartAI::CanAIAttack(Unit const* /*who*/) const { return !(me->GetReactState() == REACT_PASSIVE); }
...
void SmartAI::DamageTaken(Unit* doneBy, uint32& damage, DamageEffectType damagetype,
...
{
  GetScript()->ProcessEventsFor(SMART_EVENT_JUST_SUMMONED, summoner->ToUnit(), 0, 0, false, nullptr,
...
{
  GetScript()->ProcessEventsFor(apply ? SMART_EVENT_PASSENGER_BOARDED : SMART_EVENT_PASSENGER_REMOVED, who,
...
void SmartAI::DoAction(int32 param) { GetScript()->ProcessEventsFor(SMART_EVENT_ACTION_DONE, nullptr, param); }
