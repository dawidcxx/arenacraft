...
public:
  ...
  // instructions
  //
  // Function in which handler functions are registered, must be implemented in
  // script
  virtual void Register() = 0;
  // Function called on server startup, if returns false script won't be used in
  // core use for: dbc/template data presence/correctness checks
  ...
  template <class T> static bool ValidateSpellInfo(T const& spellIds)
  {
    return _ValidateSpellInfo(std::begin(spellIds), std::end(spellIds));
  }

private:
  template <typename Iterator> static bool _ValidateSpellInfo(Iterator begin, Iterator end)
  {
    bool allValid = true;
    while (begin != end)
    {
...
#define SPELLSCRIPT_FUNCTION_TYPE_DEFINES(CLASSNAME)                                                                   \
  ...
  {
  public:
    ObjectAreaTargetSelectHandler(SpellObjectAreaTargetSelectFnType _pObjectAreaTargetSelectHandlerScript,
  ...
  {
  public:
    ObjectTargetSelectHandler(SpellObjectTargetSelectFnType _pObjectTargetSelectHandlerScript, uint8 _effIndex,
                              uint16 _targetType);
  ...
  {
  public:
    DestinationTargetSelectHandler(SpellDestinationTargetSelectFnType _DestinationTargetSelectHandlerScript,
                                   uint8 _effIndex, uint16 _targetType);
...
#define SPELLSCRIPT_FUNCTION_CAST_DEFINES(CLASSNAME)                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    explicit BeforeHitHandlerFunction(SpellBeforeHitFnType pBeforeHitHandlerScript)                                    \
        : SpellScript::BeforeHitHandler((SpellScript::SpellBeforeHitFnType)pBeforeHitHandlerScript)                    \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    ObjectAreaTargetSelectHandlerFunction(SpellObjectAreaTargetSelectFnType _pObjectAreaTargetSelectHandlerScript,     \
                                          uint8 _effIndex, uint16 _targetType)                                         \
        : SpellScript::ObjectAreaTargetSelectHandler(                                                                  \
              (SpellScript::SpellObjectAreaTargetSelectFnType)_pObjectAreaTargetSelectHandlerScript, _effIndex,        \
              _targetType)                                                                                             \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    ObjectTargetSelectHandlerFunction(SpellObjectTargetSelectFnType _pObjectTargetSelectHandlerScript,                 \
                                      uint8 _effIndex, uint16 _targetType)                                             \
        : SpellScript::ObjectTargetSelectHandler(                                                                      \
              (SpellScript::SpellObjectTargetSelectFnType)_pObjectTargetSelectHandlerScript, _effIndex, _targetType)   \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    DestinationTargetSelectHandlerFunction(SpellDestinationTargetSelectFnType _DestinationTargetSelectHandlerScript,   \
                                           uint8 _effIndex, uint16 _targetType)                                        \
        : SpellScript::DestinationTargetSelectHandler(                                                                 \
              (SpellScript::SpellDestinationTargetSelectFnType)_DestinationTargetSelectHandlerScript, _effIndex,       \
              _targetType)                                                                                             \
    {                                                                                                                  \
    }                                                                                                                  \
  };
...
#define SpellObjectAreaTargetSelectFn(F, I, N) ObjectAreaTargetSelectHandlerFunction(&F, I, N)
  ...
  // example: OnObjectTargetSelect += SpellObjectTargetSelectFn(class::function,
  // EffectIndexSpecifier, TargetsNameSpecifier); where function is void
...
#define SpellDestinationTargetSelectFn(F, I, N) DestinationTargetSelectHandlerFunction(&F, I, N)
  ...
  // 1. BeforeCast - executed when spell preparation is finished (when cast bar
  // becomes full) before cast is handled
  // 2. OnCheckCast - allows to override result of CheckCast function
  // 3a. OnObjectAreaTargetSelect - executed just before adding selected targets
  // to final target list (for area targets) 3b. OnObjectTargetSelect - executed
  // just before adding selected target to final target list (for single unit
  // targets) 3c. OnDestinationTargetSelect - executed just before adding
  // selected target to final target list (for destination targets)
  // 4. OnCast - executed just before spell is launched (creates missile) or
  // executed
  // 5. AfterCast - executed after spell missile is launched and immediate spell
  // actions are done
  ...
  // target map
  // 8. OnEffectHit - executed just before specified effect handler call - when
  // spell missile hits dest
  // 9. BeforeHit - executed just before spell hits a target - called for each
  // target from spell target map
  // 10. OnEffectHitTarget - executed just before specified effect handler call
  // - called for each target from spell target map
  // 11. OnHit - executed just before spell deals damage and procs auras - when
  // spell hits target - called for each target from spell target map
  // 12. AfterHit - executed just after spell finishes all it's jobs for target
  // - called for each target from spell target map
  ...
  // explicit target - target selected by caster (player, game client, or script
  // - DoCast(explicitTarget, ...), required for spell to be cast examples:
  // -shadowstep - explicit target is the unit you want to go behind of
  // -chain heal - explicit target is the unit to be healed first
  // -holy nova/arcane explosion - explicit target = nullptr because target you
  // are selecting doesn't affect how spell targets are selected you can
  ...
  // returns: Unit which was selected as an explicit spell target or nullptr if
  // there's no target
  ...
  // returns: Item which was selected as an explicit spell target or nullptr if
  // there's no target
  Item* GetExplTargetItem();

  // methods useable only during spell hit on target, or during spell launch on
  // target: returns: target of current effect if it was Unit otherwise nullptr
  Unit* GetHitUnit();
...
#define AURASCRIPT_FUNCTION_TYPE_DEFINES(CLASSNAME)                                                                    \
  ...
  {
  public:
    EffectApplyHandler(AuraEffectApplicationModeFnType _pEffectHandlerScript, uint8 _effIndex, uint16 _effName,
...
#define AURASCRIPT_FUNCTION_CAST_DEFINES(CLASSNAME)                                                                    \
  ...
  public:                                                                                                              \
    EffectPeriodicHandlerFunction(AuraEffectPeriodicFnType _pEffectHandlerScript, uint8 _effIndex, uint16 _effName)    \
        : AuraScript::EffectPeriodicHandler((AuraScript::AuraEffectPeriodicFnType)_pEffectHandlerScript, _effIndex,    \
                                            _effName)                                                                  \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    EffectUpdatePeriodicHandlerFunction(AuraEffectUpdatePeriodicFnType _pEffectHandlerScript, uint8 _effIndex,         \
                                        uint16 _effName)                                                               \
        : AuraScript::EffectUpdatePeriodicHandler((AuraScript::AuraEffectUpdatePeriodicFnType)_pEffectHandlerScript,   \
                                                  _effIndex, _effName)                                                 \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    EffectCalcAmountHandlerFunction(AuraEffectCalcAmountFnType _pEffectHandlerScript, uint8 _effIndex,                 \
                                    uint16 _effName)                                                                   \
        : AuraScript::EffectCalcAmountHandler((AuraScript::AuraEffectCalcAmountFnType)_pEffectHandlerScript,           \
                                              _effIndex, _effName)                                                     \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    EffectCalcPeriodicHandlerFunction(AuraEffectCalcPeriodicFnType _pEffectHandlerScript, uint8 _effIndex,             \
                                      uint16 _effName)                                                                 \
        : AuraScript::EffectCalcPeriodicHandler((AuraScript::AuraEffectCalcPeriodicFnType)_pEffectHandlerScript,       \
                                                _effIndex, _effName)                                                   \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    EffectCalcSpellModHandlerFunction(AuraEffectCalcSpellModFnType _pEffectHandlerScript, uint8 _effIndex,             \
                                      uint16 _effName)                                                                 \
        : AuraScript::EffectCalcSpellModHandler((AuraScript::AuraEffectCalcSpellModFnType)_pEffectHandlerScript,       \
                                                _effIndex, _effName)                                                   \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  {                                                                                                                    \
  public:                                                                                                              \
    EffectApplyHandlerFunction(AuraEffectApplicationModeFnType _pEffectHandlerScript, uint8 _effIndex,                 \
                               uint16 _effName, AuraEffectHandleModes _mode)                                           \
        : AuraScript::EffectApplyHandler((AuraScript::AuraEffectApplicationModeFnType)_pEffectHandlerScript,           \
                                         _effIndex, _effName, _mode)                                                   \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
  ...
  class AfterCheckProcHandlerFunction : public AuraScript::AfterCheckProcHandler                                       \
  {                                                                                                                    \
  public:                                                                                                              \
    AfterCheckProcHandlerFunction(AuraAfterCheckProcFnType handlerScript)                                              \
        : AuraScript::AfterCheckProcHandler((AuraScript::AuraAfterCheckProcFnType)handlerScript)                       \
    {                                                                                                                  \
    }                                                                                                                  \
  };                                                                                                                   \
...
private:
  ...
  public:
    ...
    ScriptStateStore(uint8 currentScriptState, AuraApplication const* auraApplication, bool defaultActionPrevented)
        : _auraApplication(auraApplication), _currentScriptState(currentScriptState),
...
#define AuraDispelFn(F) AuraDispelFunction(&F)
  ...
  // should be used when when effect handler preventing/replacing is needed, do
  // not use this hook for triggering spellcasts/removing auras etc - may be
  // unsafe example: OnEffectApply += AuraEffectApplyFn(class::function,
...
#define AuraEffectApplyFn(F, I, N, M) EffectApplyHandlerFunction(&F, I, N, M)
  ...
  // should be used when when effect handler preventing/replacing is needed, do
  // not use this hook for triggering spellcasts/removing auras etc - may be
  // unsafe example: OnEffectRemove += AuraEffectRemoveFn(class::function,
  // EffectIndexSpecifier, EffectAuraNameSpecifier, AuraEffectHandleModes);
  // where function is: void function (AuraEffect const* aurEff,
  // AuraEffectHandleModes mode);
...
#define AuraEffectCalcAmountFn(F, I, N) EffectCalcAmountHandlerFunction(&F, I, N)
  ...
  // example: DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(class::function,
  // EffectIndexSpecifier, EffectAuraNameSpecifier); where function is: void
...
#define AuraEffectCalcPeriodicFn(F, I, N) EffectCalcPeriodicHandlerFunction(&F, I, N)
  ...
  // example: DoEffectCalcSpellMod += AuraEffectCalcSpellModFn(class::function,
  // EffectIndexSpecifier, EffectAuraNameSpecifier); where function is: void
...
#define AuraEffectCalcSpellModFn(F, I, N) EffectCalcSpellModHandlerFunction(&F, I, N)
  ...
  // EffectIndexSpecifier); where function is: void function (AuraEffect const*
...
#define AuraEffectManaShieldFn(F, I) EffectManaShieldFunction(&F, I)
  ...
  // absorbAmount is real amount absorbed by aura example: AfterEffectManaShield
  // += AuraEffectAbsorbFn(class::function, EffectIndexSpecifier); where
  // function is: void function (AuraEffect* aurEff, DamageInfo& dmgInfo,
  // uint32& absorbAmount);
...
#define AuraEffectProcFn(F, I, N) EffectProcHandlerFunction(&F, I, N)
  ...
  // returns guid of object which casted the aura (m_originalCaster of the Spell
  // class)
  ...
  // returns owner if it's unit or unit derived object, nullptr otherwise (only
  // for persistent area auras nullptr is returned)
  ...
  // class Do not call these in hooks in which AuraApplication is not avalible,
  // otherwise result will differ from expected (the functions will return
  // nullptr)
