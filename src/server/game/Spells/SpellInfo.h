...
enum SpellCastTargetFlags
{
  ...
  TARGET_FLAG_DEST_TARGET     = 0x00040000, // sometimes appears with DEST_TARGET spells (may appear or
  ...
  TARGET_FLAG_UNIT_MASK = TARGET_FLAG_UNIT | TARGET_FLAG_UNIT_RAID | TARGET_FLAG_UNIT_PARTY | TARGET_FLAG_UNIT_ENEMY |
                          TARGET_FLAG_UNIT_ALLY | TARGET_FLAG_UNIT_DEAD | TARGET_FLAG_UNIT_MINIPET |
...
private:
  struct StaticData
  {
    ...
    SpellTargetReferenceTypes ReferenceType; // defines which object is used as a reference when
                                             // selecting target
...
public:
  ...
  SpellEffectInfo()
      : _spellInfo(nullptr), _effIndex(0), Effect(0), ApplyAuraName(0), Amplitude(0), DieSides(0),
...
private:
  struct StaticData
  {
    SpellEffectImplicitTargetTypes ImplicitTargetType; // defines what target can be added to effect target
                                                       // list if there's no valid target type provided for
                                                       // effect
    SpellTargetObjectTypes UsedTargetObjectType;       // defines valid target object
                                                       // type for spell effect
  };
...
public:
  ...
  SpellCastResult CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player* player = nullptr,
                                bool strict = true) const;
  SpellCastResult CheckTarget(Unit const* caster, WorldObject const* target, bool implicit = true) const;
  SpellCastResult CheckExplicitTarget(Unit const* caster, WorldObject const* target,
