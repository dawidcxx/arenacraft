...
struct ItemLinkData
{
  ...
  uint32                           RandomSuffixBaseAmount; /* ITEM_FIELD_PROPERTY_SEED - only nonzero for
                                                              RandomSuffix items, AllocationPct from DBC
                                                              are multiplied with this, then floored, to
                                                              get stat value */
  uint8 RenderLevel;
...
struct base_tag
{
  ...
  template <typename T> static std::enable_if_t<std::is_integral_v<T>, bool> StoreTo(T& val, std::string_view data)
  {
