...
public:
  ...
  static std::size_t BuildChatPacket(WorldPacket& data, ChatMsg chatType, Language language, ObjectGuid senderGUID,
  ...
  // Builds chat packet and returns receiver guid position in the packet to
  // substitute in whisper builders
  static std::size_t BuildChatPacket(WorldPacket& data, ChatMsg chatType, Language language, WorldObject const* sender,
                                     WorldObject const* receiver, std::string_view message, uint32 achievementId = 0,
  ...
  template <typename... Args> void SendNotification(uint32 strId, Args&&... args)
  {
    if (HasSession())
      SendNotification(Acore::StringFormat(GetAcoreString(strId), std::forward<Args>(args)...));
  }
  template <typename... Args> void SendNotification(char const* fmt, Args&&... args)
  {
    if (HasSession())
  ...
  template <typename... Args> void SendGMText(uint32 strId, Args&&... args)
  {
  ...
  template <typename... Args> void SendGMText(char const* fmt, Args&&... args)
  {
    // GMText should be sent to all sessions
    DoForAllValidSessions(
        [&](Player* player)
  ...
  template <typename... Args> void SendWorldText(uint32 strId, Args&&... args)
  {
  ...
  template <typename... Args> void SendWorldText(char const* fmt, Args&&... args)
  {
    // WorldText should be sent to all sessions
    DoForAllValidSessions(
        [&](Player* player)
  ...
  template <typename... Args> void SendWorldTextOptional(uint32 strId, uint32 flag, Args&&... args)
  {
  ...
  template <typename... Args> void SendWorldTextOptional(char const* fmt, uint32 flag, Args&&... args)
  {
    // WorldTextOptional should be sent to all sessions
    DoForAllValidSessions(
        [&](Player* player)
  ...
  template <typename... Args> void PSendSysMessage(char const* fmt, Args&&... args)
  {
    if (HasSession())
      SendSysMessage(Acore::StringFormat(fmt, std::forward<Args>(args)...));
  }

  template <typename... Args> void PSendSysMessage(uint32 entry, Args&&... args)
  {
    if (HasSession())
      SendSysMessage(PGetParseString(entry, std::forward<Args>(args)...));
  }

  template <typename... Args> std::string PGetParseString(uint32 entry, Args&&... args) const
  {
  ...
  template <typename... Args> void PSendModuleSysMessage(std::string module, uint32 id, Args&&... args)
  {
    if (HasSession())
      SendSysMessage(PGetParseModuleString(module, id, std::forward<Args>(args)...));
  }

  template <typename... Args> std::string PGetParseModuleString(std::string module, uint32 id, Args&&... args) const
  {
  ...
  template <typename... Args> void SendErrorMessage(char const* fmt, Args&&... args)
  {
    PSendSysMessage(fmt, std::forward<Args>(args)...);
    SetSentErrorMessage(true);
  }

  template <typename... Args> void SendErrorMessage(uint32 entry, Args&&... args)
  {
  ...
  // Do whatever you want to all the players with a valid session [including
  // GameMasters], i.e.: param exec = [&](Player* p) { p->Whatever(); } A
  // "valid" session requires player->IsInWorld() to be true
  ...
  bool                GetPlayerGroupAndGUIDByName(const char* cname, Player*& player, Group*& group, ObjectGuid& guid,
                                                  bool offline = false);
...
public:
  ...
  // CLI does not have a session, so we override it to always be true to output
  // SendNotification and PSendSysMessage to console
