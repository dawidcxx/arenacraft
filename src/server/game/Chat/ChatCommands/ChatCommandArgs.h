...
|* *|
\****************************************************************************************/
template <typename T, typename = void> struct ArgInfo
{
  static_assert(Acore::dependant_false_v<T>, "Invalid command parameter type - see ChatCommandArgs.h for "
                                             "possible types");
};

// catch-all for number types
template <typename T> struct ArgInfo<T, std::enable_if_t<std::is_integral_v<T> || std::is_floating_point_v<T>>>
{
...
// string_view
template <> struct ArgInfo<std::string_view, void>
{
...
// string
template <> struct ArgInfo<std::string, void>
{
...
// wstring
template <> struct ArgInfo<std::wstring, void>
{
...
template <typename T> struct ArgInfo<T, std::enable_if_t<std::is_enum_v<T>>>
{
  ...
  static ChatCommandResult TryConsume(T& val, ChatHandler const* handler, std::string_view args)
  {
    ...
    // Value not found. Try to parse arg as underlying type and cast it to enum
    // type
...
// a container tag
template <typename T> struct ArgInfo<T, std::enable_if_t<std::is_base_of_v<ContainerTag, T>>>
{
...
// non-empty vector
template <typename T> struct ArgInfo<std::vector<T>, void>
{
...
// fixed-size array
template <typename T, std::size_t N> struct ArgInfo<std::array<T, N>, void>
{
...
// variant
template <typename... Ts> struct ArgInfo<Acore::ChatCommands::Variant<Ts...>>
{
  using V                        = std::variant<Ts...>;
  static constexpr std::size_t N = std::variant_size_v<V>;

  template <std::size_t I>
  static ChatCommandResult TryAtIndex([[maybe_unused]] Acore::ChatCommands::Variant<Ts...>& val,
                                      ...
                                      [[maybe_unused]] std::string_view                     args)
  {
    if constexpr (I < N)
    {
      ChatCommandResult thisResult =
          ArgInfo<std::variant_alternative_t<I, V>>::TryConsume(val.template emplace<I>(), handler, args);
      if (thisResult)
        return thisResult;
      else
      {
        ...
        if (StringStartsWith(nestedResult.GetErrorMessage(), "\""))
          return Acore::StringFormat("\"{}\"\n{} {}", thisResult.GetErrorMessage(),
                                     GetAcoreString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage());
        else
          return Acore::StringFormat("\"{}\"\n{} \"{}\"", thisResult.GetErrorMessage(),
                                     GetAcoreString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage());
      }
  ...
  static ChatCommandResult TryConsume(Acore::ChatCommands::Variant<Ts...>& val, ChatHandler const* handler,
...
template <> struct AC_GAME_API ArgInfo<AchievementEntry const*>
{
