...
void ChannelMgr::LoadChannels()
{
  ...
  do
  {
    ...
    {
      LOG_ERROR("server.loading", "Failed to load channel '{}' from database - invalid utf8 sequence? Deleted.",
                channelName);
    ...
    if (!mgr)
    {
      LOG_ERROR("server.loading", "Failed to load custom chat channel '{}' from database - invalid team {}. Deleted.",
                channelName, team);
      toDelete.emplace_back(channelName, team);
      continue;
    }
    ...
    if (QueryResult banResult =
            CharacterDatabase.Query("SELECT playerGUID, banTime FROM channels_bans WHERE channelId = {}", channelDBId))
    {
      do
      {
        ...
        if (!banFields)
          break;
        newChannel->AddBan(ObjectGuid::Create<HighGuid::Player>(banFields[0].Get<uint32>()),
...
void ChannelMgr::LoadChannelRights()
{
  ...
  do
  {
    ...
    SetChannelRightsFor(fields[0].Get<std::string>(), fields[1].Get<uint32>(), fields[2].Get<uint32>(),
                        fields[3].Get<std::string>(), fields[4].Get<std::string>(), moderators);
...
void ChannelMgr::SetChannelRightsFor(const std::string& name, const uint32& flags, const uint32& speakDelay,
                                     const std::string& joinmessage, const std::string& speakmessage,
