...
enum ChatNotify : uint8
{
  ...
  CHAT_THROTTLED_NOTICE             = 0x1F, //+ "[%s] The number of messages that can be sent to this channel is
                                            // limited, please wait to send another message.";
  CHAT_NOT_IN_AREA_NOTICE = 0x20,           //+ "[%s] You are not in the correct area for this channel."; -- The
                                            // user is trying to send a chat to a zone specific channel, and
                                            // they're not physically in that zone.
  CHAT_NOT_IN_LFG_NOTICE = 0x21,            //+ "[%s] You must be queued in looking for group before joining
...
public:
  ...
  ChannelRights(const uint32& f, const uint32& d, std::string jm, std::string sm, std::set<uint32> ml)
      : flags(f), speakDelay(d), joinMessage(std::move(jm)), speakMessage(std::move(sm)), moderators(std::move(ml))
  {
  }
  uint32           flags{0};
  uint32           speakDelay{0};
...
public:
  Channel(std::string const& name, uint32 channel_id, uint32 channelDBId, TeamId teamId = TEAM_NEUTRAL,
          bool announce = true, bool ownership = true);
...
private:
  ...
  void MakeModeChange(WorldPacket* data, ObjectGuid guid,
                      uint8 oldflags);                           //+ 0x0C
  ...
  void MakePlayerKicked(WorldPacket* data, ObjectGuid bad,
                        ObjectGuid good); //? 0x12
  void MakeBanned(WorldPacket* data);     //? 0x13
  void MakePlayerBanned(WorldPacket* data, ObjectGuid bad,
                        ObjectGuid good); //? 0x14
  void MakePlayerUnbanned(WorldPacket* data, ObjectGuid bad,
