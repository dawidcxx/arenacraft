...
void OutdoorPvPMgr::InitOutdoorPvP()
{
  ...
  for (uint8 i = 1; i < MAX_OUTDOORPVP_TYPES; ++i)
  {
    ...
    if (!pvp)
    {
      LOG_ERROR("outdoorpvp", "Could not initialize OutdoorPvP object for type ID {}; got nullptr pointer from script.",
                uint32(i));
      continue;
    }
    ...
    {
      LOG_ERROR("outdoorpvp", "Could not initialize OutdoorPvP object for type ID {}; SetupOutdoorPvP failed.",
                uint32(i));
      continue;
    }
...
void OutdoorPvPMgr::AddZone(uint32 zoneid, OutdoorPvP* handle) { m_OutdoorPvPMap[zoneid] = handle; }
