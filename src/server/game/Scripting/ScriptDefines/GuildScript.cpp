...
{
  CALL_ENABLED_HOOKS(GuildScript, GUILDHOOK_ON_REMOVE_MEMBER,
...
{
  CALL_ENABLED_HOOKS(GuildScript, GUILDHOOK_ON_MEMBER_WITDRAW_MONEY,
...
{
  CALL_ENABLED_HOOKS(GuildScript, GUILDHOOK_ON_MEMBER_DEPOSIT_MONEY,
...
{
  CALL_ENABLED_HOOKS(GuildScript, GUILDHOOK_ON_ITEM_MOVE,
                     script->OnItemMove(guild, player, pItem, isSrcBank, srcContainer, srcSlotId, isDestBank,
                                        destContainer, destSlotId));
}

void ScriptMgr::OnGuildEvent(Guild* guild, uint8 eventType, ObjectGuid::LowType playerGuid1,
                             ObjectGuid::LowType playerGuid2, uint8 newRank)
{
  CALL_ENABLED_HOOKS(GuildScript, GUILDHOOK_ON_EVENT,
...
{
  CALL_ENABLED_HOOKS(GuildScript, GUILDHOOK_ON_BANK_EVENT,
...
{
  CALL_ENABLED_BOOLEAN_HOOKS(GuildScript, GUILDHOOK_CAN_GUILD_SEND_BANK_LIST,
                             !script->CanGuildSendBankList(guild, session, tabId, sendAllSlots));
}

GuildScript::GuildScript(const char* name, std::vector<uint16> enabledHooks) : ScriptObject(name, GUILDHOOK_END)
{
