...
void ScriptMgr::OnGroupRemoveMember(Group* group, ObjectGuid guid, RemoveMethod method, ObjectGuid kicker,
                                    const char* reason)
{
  ASSERT(group);

  CALL_ENABLED_HOOKS(GroupScript, GROUPHOOK_ON_REMOVE_MEMBER,
...
void ScriptMgr::OnGroupChangeLeader(Group* group, ObjectGuid newLeaderGuid, ObjectGuid oldLeaderGuid)
{
  ...
  CALL_ENABLED_HOOKS(GroupScript, GROUPHOOK_ON_CHANGE_LEADER,
                                              ...
                                              uint32 MinPlayerCount, bool isRated, uint32 arenaSlot)
{
  CALL_ENABLED_BOOLEAN_HOOKS(
      GroupScript, GROUPHOOK_CAN_GROUP_JOIN_BATTLEGROUND_QUEUE,
...
GroupScript::GroupScript(const char* name, std::vector<uint16> enabledHooks) : ScriptObject(name, GROUPHOOK_END)
{
