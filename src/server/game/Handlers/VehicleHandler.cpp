...
void WorldSession::HandleChangeSeatsOnControlledVehicle(WorldPacket& recvData)
{
  ...
  if (!seat->CanSwitchFromSeat())
  {
    ...
    LOG_ERROR("network.opcode",
              "HandleChangeSeatsOnControlledVehicle, Opcode: {}, Player {} tried to switch seats but current seatflags "
              "{} don't permit that.",
...
void WorldSession::HandleEjectPassenger(WorldPacket& data)
{
  ...
  if (!vehicle)
  {
    ...
    LOG_ERROR("network.opcode", "HandleEjectPassenger: Player {} is not in a vehicle!",
  ...
  if (guid.IsPlayer())
  {
    ...
    if (!player)
    {
      LOG_ERROR("network.opcode",
                ...
                GetPlayer()->GetGUID().ToString(), guid.ToString());
      return;
    }

    if (!player->IsOnVehicle(vehicle->GetBase()))
    {
      LOG_ERROR("network.opcode", "Player {} tried to eject player {}, but they are not in the same vehicle",
                GetPlayer()->GetGUID().ToString(), guid.ToString());
      return;
    }
    ...
    if (seat->IsEjectable())
      ...
      LOG_ERROR("network.opcode", "Player {} attempted to eject player {} from non-ejectable seat.",
                GetPlayer()->GetGUID().ToString(), guid.ToString());
  }
  else if (guid.IsCreature())
  {
    ...
    {
      LOG_ERROR("network.opcode", "Player {} tried to eject unit {}, but they are not in the same vehicle",
                GetPlayer()->GetGUID().ToString(), guid.ToString());
      return;
    }
    ...
    if (seat->IsEjectable())
    {
      ...
      LOG_ERROR("network.opcode", "Player {} attempted to eject creature {} from non-ejectable seat.",
                GetPlayer()->GetGUID().ToString(), guid.ToString());
  }
  else
    LOG_ERROR("network.opcode", "HandleEjectPassenger: Player {} tried to eject invalid {}",
              GetPlayer()->GetGUID().ToString(), guid.ToString());
}
