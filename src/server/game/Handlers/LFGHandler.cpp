...
void WorldSession::HandleLfgProposalResultOpcode(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "CMSG_LFG_PROPOSAL_RESULT [{}] proposal: {} accept: {}", GetPlayer()->GetGUID().ToString(),
...
void WorldSession::HandleLfgSetRolesOpcode(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "CMSG_LFG_SET_ROLES: Group [{}], Player [{}], Roles: {}", gguid.ToString(), guid.ToString(),
            roles);
...
void WorldSession::HandleLfgPlayerLockInfoRequestOpcode(WorldPacket& /*recvData*/)
{
  ...
  WorldPacket data(SMSG_LFG_PLAYER_INFO, 1 + rsize * (4 + 1 + 4 + 4 + 4 + 4 + 1 + 4 + 4 + 4) + 4 +
...
void WorldSession::SendLfgUpdatePlayer(lfg::LfgUpdateData const& updateData)
{
  ...
  WorldPacket data(SMSG_LFG_UPDATE_PLAYER,
                   1 + 1 + (size > 0 ? 1 : 0) * (1 + 1 + 1 + 1 + size * 4 + updateData.comment.length()));
...
{
  LOG_DEBUG("network", "SMSG_LFG_ROLE_CHOSEN [{}] guid: [{}] roles: {}", GetPlayer()->GetGUID().ToString(),
            guid.ToString(), roles);
...
void WorldSession::SendLfgJoinResult(lfg::LfgJoinResultData const& joinData)
{
  ...
  LOG_DEBUG("network", "SMSG_LFG_JOIN_RESULT [{}] checkResult: {} checkValue: {}", GetPlayer()->GetGUID().ToString(),
            joinData.result, joinData.state);
...
{
  LOG_DEBUG("network",
            "SMSG_LFG_QUEUE_STATUS [{}] dungeon: {} - waitTime: {} - avgWaitTime: {} - waitTimeTanks: {} - "
            "waitTimeHealer: {} - waitTimeDps: {} - queuedTime: {} - tanks: {} - healers: {} - dps: {}",
            GetPlayer()->GetGUID().ToString(), queueData.dungeonId, queueData.waitTime, queueData.waitTimeAvg,
            queueData.waitTimeTank, queueData.waitTimeHealer, queueData.waitTimeDps, queueData.queuedTime,
            queueData.tanks, queueData.healers, queueData.dps);
...
void WorldSession::SendLfgPlayerReward(lfg::LfgPlayerRewardData const& rewardData)
{
  ...
  LOG_DEBUG("lfg", "SMSG_LFG_PLAYER_REWARD {} rdungeonEntry: {}, sdungeonEntry: {}, done: {}", GetPlayerInfo(),
            rewardData.rdungeonEntry, rewardData.sdungeonEntry, rewardData.done);
...
void WorldSession::SendLfgBootProposalUpdate(lfg::LfgPlayerBoot const& boot)
{
  ...
  LOG_DEBUG("network",
            "SMSG_LFG_BOOT_PROPOSAL_UPDATE [{}] inProgress: {} - didVote: {} - agree: {} - victim: [{}] votes: {} - "
            "agrees: {} - left: {} - needed: {} - reason {}",
            guid.ToString(), uint8(boot.inProgress), uint8(playerVote != lfg::LFG_ANSWER_PENDING),
            uint8(playerVote == lfg::LFG_ANSWER_AGREE), boot.victim.ToString(), votesNum, agreeNum, secsleft,
            lfg::LFG_GROUP_KICK_VOTES_NEEDED, boot.reason);
...
{
  LOG_DEBUG("network", "SMSG_LFG_OFFER_CONTINUE [{}] dungeon entry: {}", GetPlayer()->GetGUID().ToString(),
            dungeonEntry);
