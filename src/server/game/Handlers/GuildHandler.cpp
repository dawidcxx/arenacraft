...
void WorldSession::HandleGuildCreateOpcode(WorldPackets::Guild::GuildCreate& packet)
{
  LOG_ERROR("network.opcode",
            ...
            GetPlayerInfo(), packet.GuildName);
}
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_SET_PUBLIC_NOTE [{}]: Target: {}, Note: {}", GetPlayerInfo(), packet.NoteeName,
            packet.Note);
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_SET_OFFICER_NOTE [{}]: Target: {}, Note: {}", GetPlayerInfo(), packet.NoteeName,
            packet.Note);
...
void WorldSession::HandleGuildRankOpcode(WorldPackets::Guild::GuildSetRankPermissions& packet)
{
  ...
  {
    // For some reason the client is sending - 1 for guildmaster tab withdraw item limit, that's ilegal for us because
    // we expect unsigned int. Probably core handling for this should be changed.
  ...
  guild->HandleSetRankInfo(this, packet.RankID, packet.RankName, packet.Flags, packet.WithdrawGoldLimit,
                           rightsAndSlots);
}
...
void WorldSession::HandleSaveGuildEmblemOpcode(WorldPackets::Guild::SaveGuildEmblem& packet)
{
  ...
  LOG_DEBUG("guild",
            "MSG_SAVE_GUILD_EMBLEM [{}]: Guid: [{}] Style: {}, Color: {}, BorderStyle: {}, BorderColor: {}, "
            "BackgroundColor: {}",
            GetPlayerInfo(), packet.Vendor.ToString(), emblemInfo.GetStyle(), emblemInfo.GetColor(),
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_BANKER_ACTIVATE [{}]: Go: [{}] AllSlots: {}", GetPlayerInfo(),
            packet.Banker.ToString(), packet.FullUpdate);
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_QUERY_TAB [{}]: Go: [{}], TabId: {}, ShowTabs: {}", GetPlayerInfo(),
            packet.Banker.ToString(), packet.Tab, packet.FullUpdate);
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_DEPOSIT_MONEY [{}]: Go: [{}], money: {}", GetPlayerInfo(),
            packet.Banker.ToString(), packet.Money);
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_WITHDRAW_MONEY [{}]: Go: [{}], money: {}", GetPlayerInfo(),
            packet.Banker.ToString(), packet.Money);
...
void WorldSession::HandleGuildBankSwapItems(WorldPackets::Guild::GuildBankSwapItems& packet)
{
  ...
  if (packet.BankOnly)
    guild->SwapItems(GetPlayer(), packet.BankTab1, packet.BankSlot1, packet.BankTab, packet.BankSlot,
  ...
  else
  {
    ...
    if (!Player::IsInventoryPos(playerBag, playerSlotId) && !(playerBag == NULL_BAG && playerSlotId == NULL_SLOT))
      ...
      guild->SwapItemsWithInventory(GetPlayer(), toChar != 0, packet.BankTab, packet.BankSlot, playerBag, playerSlotId,
                                    splitedAmount);
  }
}
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_BUY_TAB [{}]: [{}[, TabId: {}", GetPlayerInfo(), packet.Banker.ToString(),
            packet.BankTab);
...
{
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_UPDATE_TAB [{}]: Go: [{}], TabId: {}, Name: {}, Icon: {}", GetPlayerInfo(),
            packet.Banker.ToString(), packet.BankTab, packet.Name, packet.Icon);
