...
void WorldSession::SendUpdateTrade(bool trader_data /*= true*/)
{
  ...
  data << uint32(0); // added in 2.4.0, this value must be equal to value from TRADE_STATUS_OPEN_WINDOW status packet
                     // (different value for different players to block multiple trades?)
        ...
        if (myItems[i]->IsBOPTradable())
          myItems[i]->SetUInt32Value(
              ...
              trader->GetTotalPlayedTime() -
                  (_player->GetTotalPlayedTime() - myItems[i]->GetUInt32Value(ITEM_FIELD_CREATE_PLAYED_TIME)));
        // store
        ...
        if (hisItems[i]->IsBOPTradable())
          hisItems[i]->SetUInt32Value(
              ...
              _player->GetTotalPlayedTime() -
                  (trader->GetTotalPlayedTime() - hisItems[i]->GetUInt32Value(ITEM_FIELD_CREATE_PLAYED_TIME)));
        // store
...
static void setAcceptTradeMode(TradeData* myTrade, TradeData* hisTrade, Item** myItems, Item** hisItems)
{
  ...
  for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
  {
    ...
    {
      LOG_DEBUG("network.opcode", "player trade item {} bag: {} slot: {}", item->GetGUID().ToString(),
    ...
    {
      LOG_DEBUG("network.opcode", "partner trade item {} bag: {} slot: {}", item->GetGUID().ToString(),
                item->GetBagSlot(), item->GetSlot());
      hisItems[i] = item;
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
  ...
  for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
  {
    ...
    if (Item* item = his_trade->GetItem(TradeSlots(i)))
    {
      ...
      // if (item->IsBindedNotWith(_player))   // dont mark as invalid when his item isnt good (not exploitable because
      // if item is invalid trade will fail anyway later on the same check)
      //{
      //     SendTradeStatus(TRADE_STATUS_NOT_ELIGIBLE);
      //     his_trade->SetAccepted(false, true);
      //     return;
  ...
  if (his_trade->IsAccepted())
  {
    ...
    if (my_trade->GetMoney() >= 10 * GOLD)
    {
      CharacterDatabase.Execute(
          "INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", {}, \"goods\", NOW(), {})", GetAccountId(),
          _player->GetGUID().GetCounter(), _player->GetName(), GetRemoteAddress(), trader->GetSession()->GetAccountId(),
          trader->GetName(), my_trade->GetMoney(), 6);
    }
    if (his_trade->GetMoney() >= 10 * GOLD)
    {
      CharacterDatabase.Execute(
          "INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", {}, \"goods\", NOW(), {})",
          trader->GetSession()->GetAccountId(), trader->GetGUID().GetCounter(), trader->GetName(),
          trader->GetSession()->GetRemoteAddress(), GetAccountId(), _player->GetName(), his_trade->GetMoney(), 6);
    }
...
void WorldSession::HandleSetTradeItemOpcode(WorldPacket& recvPacket)
{
  ...
  {
    // Do not send TRADE_STATUS_TRADE_CANCELED because it will cause double display of "Transaction canceled"
    // notification On the trade initiator screen
