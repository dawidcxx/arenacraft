...
void WorldSession::HandleArenaTeamAcceptOpcode(WorldPacket& /*recvData*/)
{
  ...
  // Broadcast event
  arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_JOIN_SS, _player->GetGUID(), 2, _player->GetName().c_str(),
                            arenaTeam->GetName(), "");
}
...
void WorldSession::HandleArenaTeamLeaveOpcode(WorldPacket& recvData)
{
  ...
  // Broadcast event
  arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_LEAVE_SS, _player->GetGUID(), 2, _player->GetName().c_str(),
                            arenaTeam->GetName(), "");
...
void WorldSession::HandleArenaTeamRemoveOpcode(WorldPacket& recvData)
{
  ...
  // Broadcast event
  arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_REMOVE_SSS, ObjectGuid::Empty, 3, name, arenaTeam->GetName(),
                            _player->GetName());
}
...
void WorldSession::HandleArenaTeamLeaderOpcode(WorldPacket& recvData)
{
  ...
  // Broadcast event
  arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_LEADER_CHANGED_SSS, ObjectGuid::Empty, 3, _player->GetName().c_str(), name,
                            arenaTeam->GetName());
}

void WorldSession::SendArenaTeamCommandResult(uint32 teamAction, const std::string& team, const std::string& player,
                                              uint32 errorId)
{
