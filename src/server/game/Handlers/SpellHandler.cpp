...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
  ...
  LOG_DEBUG("network",
            "WORLD: CMSG_USE_ITEM packet, bagIndex: {}, slot: {}, castCount: {}, spellId: {}, Item: {}, glyphIndex: "
            "{}, data length = {}",
            bagIndex, slot, castCount, spellId, pItem->GetEntry(), glyphIndex, (uint32)recvPacket.size());
...
void WorldSession::HandleOpenItemOpcode(WorldPacket& recvPacket)
{
  ...
  if (lockId)
  {
    ...
    if (!lockInfo)
    {
      ...
      LOG_ERROR("network.opcode", "WORLD::OpenItem: item [{}] has an unknown lockId: {}!", item->GetGUID().ToString(),
                lockId);
      return;
    }
  ...
  if (sScriptMgr->OnBeforeOpenItem(pUser, item))
  {
    if (item->IsWrapped()) // wrapped?
    {
      ...
      _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
          std::bind(&WorldSession::HandleOpenWrappedItemCallback, this, bagIndex, slot, item->GetGUID().GetCounter(),
                    std::placeholders::_1)));
    }
    else
    {
...
void WorldSession::HandleOpenWrappedItemCallback(uint8 bagIndex, uint8 slot, ObjectGuid::LowType itemLowGUID,
                                                 PreparedQueryResult result)
{
  ...
  if (!result)
  {
    LOG_ERROR("network", "Wrapped item {} don't have record in character_gifts table and will deleted",
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
  ...
  LOG_DEBUG("network", "WORLD: got cast spell packet, castCount: {}, spellId: {}, castFlags: {}, data length = {}",
  ...
  // pussywizard: HandleClientCastFlags calls HandleMovementOpcodes, which can result in pretty much anything. Caster
  // not in map will crash at GetMap() for spell difficulty in Spell constructor.
  if (!mover->FindMap())
  {
    recvPacket.rfinish(); // prevent spam at ignore packet
    return;
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket& recvPacket)
{
  ...
  if (!pet)
  {
    LOG_ERROR("network.opcode", "HandlePetCancelAura: Attempt to cancel an aura for non-existant pet {} by player {}",
              guid.ToString(), GetPlayer()->GetName());
    return;
  }

  if (pet != GetPlayer()->GetGuardianPet() && pet != GetPlayer()->GetCharm())
  {
    LOG_ERROR("network.opcode", "HandlePetCancelAura: Pet {} is not a pet of player {}", guid.ToString(),
              GetPlayer()->GetName());
    return;
  }
