...
struct CreatureTextId
{
  ...
  bool operator<(CreatureTextId const& right) const { return memcmp(this, &right, sizeof(CreatureTextId)) < 0; }
...
public:
  ...
  void SendSound(Creature* source, uint32 sound, ChatMsg msgType, WorldObject const* target, CreatureTextRange range,
                 TeamId teamId, bool gmOnly);
  void SendEmote(Unit* source, uint32 emote);

  // if sent, returns the 'duration' of the text else 0 if error
  uint32      SendChat(Creature* source, uint8 textGroup, WorldObject const* target = nullptr,
  ...
  std::string GetLocalizedChatString(uint32 entry, uint8 gender, uint8 textGroup, uint32 id,
                                     LocaleConstant locale) const;

  template <class Builder>
  void SendChatPacket(WorldObject* source, Builder const& builder, ChatMsg msgType, WorldObject const* target = nullptr,
                      CreatureTextRange range = TEXT_RANGE_NORMAL, TeamId teamId = TEAM_NEUTRAL,
...
private:
  ...
  void  SendNonChatPacket(WorldObject* source, WorldPacket const* data, ChatMsg msgType, WorldObject const* target,
...
template <class Builder> class CreatureTextLocalizer
{
public:
...
template <class Builder>
void CreatureTextMgr::SendChatPacket(WorldObject* source, Builder const& builder, ChatMsg msgType,
