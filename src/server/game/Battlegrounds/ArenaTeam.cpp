...
ArenaTeam::ArenaTeam()
    : TeamId(0), Type(0), TeamName(), BackgroundColor(0), EmblemStyle(0), EmblemColor(0), BorderStyle(0), BorderColor(0)
{
  Stats.WeekGames   = 0;
  Stats.SeasonGames = 0;
  Stats.Rank        = 0;
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
  ...
  {
    LOG_ERROR("bg.arena", "Arena: Player {} ({}) already has an arena team of type {}", playerName,
              playerGuid.ToString(), GetType());
    return false;
  }
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result)
{
  ...
  do
  {
    ...
    // Prevent crash if db records are broken when all members in result are already processed and current team doesn't
    // have any members
    if (!fields)
      break;
    ...
    if (fields[6].Get<std::string>().empty())
    {
      LOG_ERROR(
          ...
          arenaTeamId, newMember.Guid.ToString());
  ...
  if (Empty() || !captainPresentInTeam)
  {
    ...
    LOG_DEBUG("bg.battleground",
              "ArenaTeam {} does not have any members or its captain is not in team, disbanding it...", TeamId);
    return false;
  }
          ...
          if (queue.GetPlayerGroupInfoData(playerMember->GetGUID(), &ginfo))
          {
            if (!ginfo.IsInvitedToBGInstanceGUID)
            {
              ...
              sBattlegroundMgr->BuildBattlegroundStatusPacket(
                  &data, nullptr, playerMember->GetBattlegroundQueueIndex(bgQueue), STATUS_NONE, 0, 0, 0, TEAM_NEUTRAL);
...
int32 ArenaTeam::GetMatchmakerRatingMod(uint32 ownRating, uint32 opponentRating,
  ...
  if (won)
  {
    if (ownRating < 1300)
    {
      ...
      if (ownRating < 1000)
        ...
        mod =
            ((win_rating_modifier1 / 2.0f) + ((win_rating_modifier1 / 2.0f) * (1300.0f - float(ownRating)) / 300.0f)) *
            (1.0f - chance);
    }
    else
...
std::unordered_map<uint32, uint8> ArenaTeam::ArenaSlotByType = {
    {ARENA_TEAM_2v2, ARENA_SLOT_2v2}, {ARENA_TEAM_3v3, ARENA_SLOT_3v3}, {ARENA_TEAM_5v5, ARENA_SLOT_5v5}};
