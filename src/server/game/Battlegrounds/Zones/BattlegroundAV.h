...
enum BG_AV_Sounds
{
  ...
  8212:
      ...
      drek "mlanzenabschaum! In meiner Burg?! Toetet sie all" - nicht immer der
  sound 8333: galv "sterbt fuer euch ist kein platz hier"
  ...
  Unterst" 8173: ally grave capture/defend tower destroy mine capture ally wins
  8192:
  ...
  AV_SOUND_ALLIANCE_GOOD     = 8173, // if something good happens for the team:  wins(maybe only through
                                     // killing the boss), captures mine or grave, destroys tower and
                                     // defends grave
  AV_SOUND_HORDE_GOOD        = 8213,
  AV_SOUND_BOTH_TOWER_DEFEND = 8192,

  AV_SOUND_ALLIANCE_CAPTAIN = 8232, // gets called when someone attacks them and at the beginning after
                                    // 3min+rand(x)*10sec (maybe buff)
...
{
  // cause the moongose-system is a bit different, we don't use the right go-ids
  // for every node.. if we want to be 100% like another big server, we must
  // take one object for every node snowfall 4flags as eyecandy 179424 (alliance
  // neutral) Banners - stolen from battleground_AB.h ;-)
...
enum BG_AV_ObjectTypes
{
  ...
  // big flags on top of towers 2 flags on each (contested, (alliance | horde))
  // + 2 auras
...
const float BG_AV_ObjectPos[AV_OPLACE_MAX][4] = {{638.592f, -32.422f, 46.0608f, -1.62316f},   // firstaid station
...
std::tuple<uint32, Position, std::array<float, 4>> const BG_AV_GenericObjectPos[BG_AV_OBJECT_GENERIC_MAX -
                                                                                BG_AV_OBJECT_GENERIC_MIN + 1] = {
    // Burning Embers
...
// GO template ID, Position, rotation0-3, VerifiedBuild
std::tuple<uint32, Position, std::array<float, 4>> const
    BG_AV_HallowsEndObjectPos[BG_AV_OBJECT_HALLOWS_END_MAX - BG_AV_OBJECT_HALLOWS_END_MIN + 1] = {
        // G_Pumpkin_01
...
enum BG_AV_CreaturePlace
{
  ...
  // i don't will add for all 4 positions a variable.. i think one is enough to
  // compute the rest
...
const uint32 BG_AV_GraveyardIds[9] = {AV_GRAVE_STORM_AID, AV_GRAVE_STORM_GRAVE,   AV_GRAVE_STONE_GRAVE,
                                      AV_GRAVE_SNOWFALL,  AV_GRAVE_ICE_GRAVE,     AV_GRAVE_FROSTWOLF,
                                      AV_GRAVE_FROST_HUT, AV_GRAVE_MAIN_ALLIANCE, AV_GRAVE_MAIN_HORDE};
...
enum BG_AV_WorldStates
{
  ...
  /*
      //the comments behind the state shows which icon overlaps the other.. but
  is, until now, unused and maybe not a good solution (but give few performance
  (:)
...
const uint32 BG_AV_MineWorldStates[2][3] = {{1358, 1360, 1359}, {1355, 1357, 1356}};
