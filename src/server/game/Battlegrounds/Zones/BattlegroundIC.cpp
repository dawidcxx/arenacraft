  ...
  for (uint8 i = NODE_TYPE_REFINERY; i < MAX_NODE_TYPES; ++i)
  {
    if (nodePoint[i].nodeType == NODE_TYPE_DOCKS)
    {
      if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A || nodePoint[i].nodeState == NODE_STATE_CONTROLLED_H)
      {
        if (nodePoint[i].timer <= diff)
        {
          ...
          for (uint8 j = 0; j < MAX_CATAPULTS_SPAWNS_PER_FACTION; ++j)
          {
            ...
            if (Creature* catapult = GetBgMap()->GetCreature(BgCreatures[type]))
              if (!catapult->IsAlive())
              {
                ...
                catapult->Relocate(
                    BG_IC_DocksVehiclesCatapults[j].GetPositionX(), BG_IC_DocksVehiclesCatapults[j].GetPositionY(),
                    BG_IC_DocksVehiclesCatapults[j].GetPositionZ(), BG_IC_DocksVehiclesCatapults[j].GetOrientation());
          ...
          {
            uint8 type =
                (nodePoint[i].faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_1_A : BG_IC_NPC_GLAIVE_THROWER_1_H) +
                j;
            if (Creature* glaiveThrower = GetBgMap()->GetCreature(BgCreatures[type]))
              if (!glaiveThrower->IsAlive())
              {
                ...
                glaiveThrower->Relocate(
                    BG_IC_DocksVehiclesGlaives[j].GetPositionX(), BG_IC_DocksVehiclesGlaives[j].GetPositionY(),
                    BG_IC_DocksVehiclesGlaives[j].GetPositionZ(), BG_IC_DocksVehiclesGlaives[j].GetOrientation());
        ...
        if (siegeEngineWorkshopTimer <= diff)
        {
          ...
          if (Creature* siege = GetBGCreature(siegeType)) // this always should be true
            if (!siege->IsAlive())
            {
              ...
              siege->Relocate(BG_IC_WorkshopVehicles[4].GetPositionX(), BG_IC_WorkshopVehicles[4].GetPositionY(),
          ...
          {
            uint8 type =
                (nodePoint[i].faction == TEAM_ALLIANCE ? BG_IC_NPC_DEMOLISHER_1_A : BG_IC_NPC_DEMOLISHER_1_H) + u;
            if (Creature* demolisher = GetBgMap()->GetCreature(BgCreatures[type]))
              if (!demolisher->IsAlive())
              {
                ...
                demolisher->Relocate(BG_IC_WorkshopVehicles[u].GetPositionX(), BG_IC_WorkshopVehicles[u].GetPositionY(),
    ...
    if (nodePoint[i].needChange)
    {
      if (nodePoint[i].timer <= diff)
      {
        ...
        float cords[4] = {banner->GetPositionX(), banner->GetPositionY(), banner->GetPositionZ(),
                          banner->GetOrientation()};

        DelObject(nodePoint[i].gameobject_type);
        AddObject(nodePoint[i].gameobject_type, nodePoint[i].gameobject_entry, cords[0], cords[1], cords[2], cords[3],
...
bool BattlegroundIC::SetupBattleground()
{
  ...
  {
    if (!AddObject(BG_IC_ObjSpawnlocs[i].type, BG_IC_ObjSpawnlocs[i].entry, BG_IC_ObjSpawnlocs[i].x,
                   BG_IC_ObjSpawnlocs[i].y, BG_IC_ObjSpawnlocs[i].z, BG_IC_ObjSpawnlocs[i].o, 0, 0, 0, 0,
                   RESPAWN_ONE_DAY))
    {
      LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning gameobject {}",
                BG_IC_ObjSpawnlocs[i].entry);
      return false;
    }
  ...
  for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTERS_SPAWNS; ++i)
  {
    if (!AddObject(BG_IC_Teleporters[i].type, BG_IC_Teleporters[i].entry, BG_IC_Teleporters[i].x,
                   BG_IC_Teleporters[i].y, BG_IC_Teleporters[i].z, BG_IC_Teleporters[i].o, 0, 0, 0, 0, RESPAWN_ONE_DAY))
    {
  ...
  for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTER_EFFECTS_SPAWNS; ++i)
  {
    if (!AddObject(BG_IC_TeleporterEffects[i].type, BG_IC_TeleporterEffects[i].entry, BG_IC_TeleporterEffects[i].x,
                   BG_IC_TeleporterEffects[i].y, BG_IC_TeleporterEffects[i].z, BG_IC_TeleporterEffects[i].o, 0, 0, 0, 0,
                   RESPAWN_ONE_DAY))
    {
      LOG_ERROR("bg.battleground",
                "Isle of Conquest | Starting Event Open Doors: There was an error spawning gameobject {}",
  ...
  {
    if (!AddCreature(BG_IC_NpcSpawnlocs[i].entry, BG_IC_NpcSpawnlocs[i].type, BG_IC_NpcSpawnlocs[i].x,
                     BG_IC_NpcSpawnlocs[i].y, BG_IC_NpcSpawnlocs[i].z, BG_IC_NpcSpawnlocs[i].o, RESPAWN_ONE_DAY))
    {
      LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning creature {}",
                BG_IC_NpcSpawnlocs[i].entry);
      return false;
    }
  }

  if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 3, BG_IC_SpiritGuidePos[5][0], BG_IC_SpiritGuidePos[5][1],
                      BG_IC_SpiritGuidePos[5][2], BG_IC_SpiritGuidePos[5][3], TEAM_ALLIANCE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 4, BG_IC_SpiritGuidePos[6][0], BG_IC_SpiritGuidePos[6][1],
                      BG_IC_SpiritGuidePos[6][2], BG_IC_SpiritGuidePos[6][3], TEAM_HORDE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 5, BG_IC_SpiritGuidePos[7][0], BG_IC_SpiritGuidePos[7][1],
                      BG_IC_SpiritGuidePos[7][2], BG_IC_SpiritGuidePos[7][3], TEAM_ALLIANCE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 6, BG_IC_SpiritGuidePos[8][0], BG_IC_SpiritGuidePos[8][1],
                      BG_IC_SpiritGuidePos[8][2], BG_IC_SpiritGuidePos[8][3], TEAM_HORDE))
  {
...
void BattlegroundIC::EventPlayerClickedOnFlag(Player* player, GameObject* gameObject)
{
  ...
  for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
  {
    ...
    if (point.gameobject_entry == gameObject->GetEntry())
    {
      ...
      {
        if (GateStatus[BG_IC_A_FRONT] != BG_IC_GATE_DESTROYED && GateStatus[BG_IC_A_WEST] != BG_IC_GATE_DESTROYED &&
      ...
      {
        if (GateStatus[BG_IC_H_FRONT] != BG_IC_GATE_DESTROYED && GateStatus[BG_IC_H_WEST] != BG_IC_GATE_DESTROYED &&
      ...
      AddObject(point.gameobject_type, point.gameobject_entry, cords[0], cords[1], cords[2], cords[3], 0, 0, 0, 0,
                RESPAWN_ONE_DAY);

      GetBGObject(point.gameobject_type)
          ->SetUInt32Value(GAMEOBJECT_FACTION, point.faction == TEAM_ALLIANCE ? BG_IC_Factions[1] : BG_IC_Factions[0]);
...
uint32 BattlegroundIC::GetNextBanner(ICNodePoint* nodePoint, uint32 team, bool returnDefinitve)
{
  ...
  if (returnDefinitve)
    // here is a special case, here we must return the definitve faction banner after the grey banner was spawned 1
    // minute
...
void BattlegroundIC::HandleCapturedNodes(ICNodePoint* nodePoint, bool recapture)
{
  if (nodePoint->nodeType != NODE_TYPE_REFINERY && nodePoint->nodeType != NODE_TYPE_QUARRY)
  {
    if (!AddSpiritGuide(static_cast<uint16>(BG_IC_NPC_SPIRIT_GUIDE_1) + nodePoint->nodeType - 2,
                        ...
                        nodePoint->faction))
      LOG_ERROR("bg.battleground", "Isle of Conquest: Failed to spawn spirit guide! point: {}, team: {}, ",
  ...
  case BG_IC_GO_HANGAR_BANNER:
  {
    ...
    for (uint8 u = 0; u < MAX_HANGAR_TELEPORTERS_SPAWNS; ++u)
    {
      ...
      if (!AddObject(type, (nodePoint->faction == TEAM_ALLIANCE ? GO_ALLIANCE_GUNSHIP_PORTAL : GO_HORDE_GUNSHIP_PORTAL),
                     ...
                     BG_IC_HangarTeleporters[u].GetPositionZ(), BG_IC_HangarTeleporters[u].GetOrientation(), 0, 0, 0, 0,
                     RESPAWN_ONE_DAY))
        LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning a gunship portal. Type: {}",
    ...
    for (uint8 u = 0; u < MAX_HANGAR_TELEPORTER_EFFECTS_SPAWNS; ++u)
    {
      ...
      if (!AddObject(type,
        ...
        LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning a gunship portal effects. Type: {}",
                  BG_IC_GO_HANGAR_TELEPORTER_1 + u);
    }

    for (uint8 u = 0; u < MAX_TRIGGER_SPAWNS_PER_FACTION; ++u)
    {
      if (!AddCreature(NPC_WORLD_TRIGGER_NOT_FLOATING, BG_IC_NPC_WORLD_TRIGGER_NOT_FLOATING,
                       ...
                       BG_IC_HangarTrigger[nodePoint->faction].GetOrientation(), RESPAWN_ONE_DAY,
                       nodePoint->faction == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde))
        LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning a world trigger. Type: {}",
    ...
    for (uint8 u = 0; u < MAX_CAPTAIN_SPAWNS_PER_FACTION; ++u)
    {
      ...
      if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_1)
        if (AddCreature(nodePoint->faction == TEAM_ALLIANCE ? NPC_ALLIANCE_GUNSHIP_CAPTAIN : NPC_HORDE_GUNSHIP_CAPTAIN,
                        type, BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE ? 2 : 0].GetPositionX(),
      ...
      if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_2)
        if (!AddCreature(nodePoint->faction == TEAM_ALLIANCE ? NPC_ALLIANCE_GUNSHIP_CAPTAIN : NPC_HORDE_GUNSHIP_CAPTAIN,
                         type, BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE ? 3 : 1].GetPositionX(),
                         ...
                         RESPAWN_ONE_DAY, nodePoint->faction == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde))
          LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning a world trigger. Type: {}",
  ...
  case BG_IC_GO_DOCKS_BANNER:
    ...
    {
      uint8 type =
          (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_1_A : BG_IC_NPC_GLAIVE_THROWER_1_H) + i;
    ...
    for (uint8 i = 0; i < MAX_CATAPULTS_SPAWNS_PER_FACTION; ++i)
    {
      ...
      if (AddCreature(NPC_CATAPULT, type, BG_IC_DocksVehiclesCatapults[i].GetPositionX(),
                      BG_IC_DocksVehiclesCatapults[i].GetPositionY(), BG_IC_DocksVehiclesCatapults[i].GetPositionZ(),
                      BG_IC_DocksVehiclesCatapults[i].GetOrientation(), RESPAWN_ONE_DAY))
  ...
  case BG_IC_GO_WORKSHOP_BANNER:
  {
    ...
    if (!recapture)
    {
      for (uint8 i = 0; i < MAX_DEMOLISHERS_SPAWNS_PER_FACTION; ++i)
      {
        ...
        if (AddCreature(NPC_DEMOLISHER, type, BG_IC_WorkshopVehicles[i].GetPositionX(),
                        BG_IC_WorkshopVehicles[i].GetPositionY(), BG_IC_WorkshopVehicles[i].GetPositionZ(),
    ...
    {
      AddObject(BG_IC_GO_SEAFORIUM_BOMBS_1 + i, GO_SEAFORIUM_BOMBS, workshopBombs[i].GetPositionX(),
                workshopBombs[i].GetPositionY(), workshopBombs[i].GetPositionZ(), workshopBombs[i].GetOrientation(), 0,
                0, 0, 0, 10);

      if (GameObject* seaforiumBombs = GetBGObject(BG_IC_GO_SEAFORIUM_BOMBS_1 + i))
      {
        seaforiumBombs->SetRespawnTime(10);
        seaforiumBombs->SetUInt32Value(GAMEOBJECT_FACTION,
