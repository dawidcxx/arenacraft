...
void LoadSkillPerfectItemTable()
{
  ...
  do /* fetch data and run sanity checks */
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "Skill perfection data for spell {} has non-existent required specialization spell id {} in "
    ...
    {
      LOG_ERROR("sql.sql",
                "Skill perfection data for spell {} references non-existent perfect item id {} in "
                "`skill_perfect_item_template`!",
  ...
  LOG_INFO("server.loading", ">> Loaded {} spell perfection definitions in {} ms", count,
...
void LoadSkillExtraItemTable()
{
  ...
  do
  {
    ...
    if (!sSpellMgr->GetSpellInfo(spellId))
    {
      LOG_ERROR("sql.sql", "Skill specialization {} has non-existent spell id in `skill_extra_item_template`!",
                spellId);
      continue;
    }
    ...
    {
      LOG_ERROR("sql.sql",
                "Skill specialization {} have not existed required specialization spell id {} in "
    ...
    if (!newMaxOrEntry)
    {
      LOG_ERROR("sql.sql", "Skill specialization {} has 0 max number of extra items in `skill_extra_item_template`!",
                spellId);
      continue;
    }
  ...
  LOG_INFO("server.loading", ">> Loaded {} spell specialization definitions in {} ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server.loading", " ");
}
