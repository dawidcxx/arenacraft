...
void GossipMenu::AddMenuItem(uint32 menuId, uint32 menuItemId, uint32 sender, uint32 action, uint32 boxMoney)
{
  ...
  for (GossipMenuItemsContainer::const_iterator itr = bounds.first; itr != bounds.second; ++itr)
  {
    ...
    AddMenuItem(-1, itr->second.OptionIcon, strOptionText, sender, action, strBoxText,
                boxMoney ? boxMoney : itr->second.BoxMoney, itr->second.BoxCoded);
  }
}
...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const* quest, ObjectGuid npcGUID, bool activateAccept) const
{
  ...
  LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTGIVER_QUEST_DETAILS {}, questid={}", npcGUID.ToString(),
            quest->GetQuestId());
}
...
void PlayerMenu::SendQuestQueryResponse(Quest const* quest) const
{
  ...
  data << uint32(quest->GetQuestLevel()); // may be -1, static data, in other cases must be used dynamic level:
                                          // Player::GetQuestLevel (0 is not known, but assuming this is no longer valid
                                          // for quest intended for client)
...
void PlayerMenu::SendQuestGiverOfferReward(Quest const* quest, ObjectGuid npcGUID, bool enableNext) const
{
  ...
  _session->SendPacket(&data);
  LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTGIVER_OFFER_REWARD {}, questid={}", npcGUID.ToString(),
            quest->GetQuestId());
}

void PlayerMenu::SendQuestGiverRequestItems(Quest const* quest, ObjectGuid npcGUID, bool canComplete,
                                            bool closeOnCancel) const
{
  ...
  _session->SendPacket(&data);
  LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTGIVER_REQUEST_ITEMS {}, questid={}", npcGUID.ToString(),
            quest->GetQuestId());
}
