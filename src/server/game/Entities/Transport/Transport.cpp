...
MotionTransport::MotionTransport()
    : Transport(), _transportInfo(nullptr), _isMoving(true), _pendingStop(false), _triggeredArrivalEvent(false),
      _triggeredDepartureEvent(false), _passengersLoaded(false), _delayedTeleport(false)
{
...
bool MotionTransport::CreateMoTrans(ObjectGuid::LowType guidlow, uint32 entry, uint32 mapid, float x, float y, float z,
                                    float ang, uint32 animprogress)
{
  ...
  if (!goinfo)
  {
    LOG_ERROR("entities.transport",
              "Transport not created: entry in `gameobject_template` not found, guidlow: {} map: {}  (X: {} Y: {} Z: "
              "{}) ang: {}",
  ...
  if (!tInfo)
  {
    LOG_ERROR("entities.transport", "Transport {} (name: {}) will not be created, missing `transport_template` entry.",
              entry, goinfo->name);
    return false;
  }
...
void MotionTransport::Update(uint32 diff)
{
  ...
  for (;;)
  {
    ...
    sScriptMgr->OnRelocate(this, _currentFrame->Node->index, _currentFrame->Node->mapid, _currentFrame->Node->x,
                           _currentFrame->Node->y, _currentFrame->Node->z);

    // LOG_DEBUG("entities.transport", "Transport {} ({}) moved to node {} {} {} {} {}", GetEntry(), GetName(),
    // _currentFrame->Node->index, _currentFrame->Node->mapid, _currentFrame->Node->x, _currentFrame->Node->y,
...
void MotionTransport::UpdatePassengerPositions(PassengerSet& passengers)
{
  for (PassengerSet::iterator itr = passengers.begin(); itr != passengers.end(); ++itr)
  {
    ...
    // if passenger is on vehicle we have to assume the vehicle is also on transport and its the vehicle that will be
    // updating its passengers
...
void MotionTransport::DoEventIfAny(KeyFrame const& node, bool departure)
{
  ...
  {
    // LOG_DEBUG("maps.script", "Taxi {} event {} of node {} of {} path", departure ? "departure" : "arrival", eventid,
    // node.Node->index, GetName());
                             ...
                             uint32 artKit)
{
  ...
  {
    LOG_ERROR("vehicle", "StaticTransport::Create: No AnimationInfo was found for GameObject entry ({})",
              goinfo->entry);
    return false;
  }
  ...
  {
    LOG_ERROR("vehicle", "StaticTransport::Create: AnimationInfo->TotalTime is 0 for GameObject entry ({})",
              goinfo->entry);
    return false;
  }
...
void StaticTransport::RelocateToProgress(uint32 progress)
{
  ...
  if (m_goValue.Transport.AnimationInfo->GetAnimNode(progress, curr, next, percPos))
  {
    ...
    // pussywizard: PathRotation in db is only simple orientation rotation, so don't use sophisticated and not working
    // code reminder: WorldRotation only influences model rotation, not the path
    ...
    // pussywizard: AnimRotation in dbc is only simple orientation rotation, so don't use sophisticated and not working
    // code
