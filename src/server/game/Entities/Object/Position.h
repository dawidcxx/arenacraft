...
struct Position
{
  ...
  operator G3D::Vector3() const { return {m_positionX, m_positionY, m_positionZ}; }
  ...
  {
    return NormalizeOrientation(std::atan2(static_cast<float>(y - m_positionY), static_cast<float>(x - m_positionX)));
  }
  ...
  float GetRelativeAngle(const Position* pos) const { return NormalizeOrientation(GetAngle(pos) - m_orientation); }
  ...
  [[nodiscard]] bool IsInDist2d(float x, float y, float dist) const { return GetExactDist2dSq(x, y) < dist * dist; }
...
public:
  ...
  WorldLocation(uint32 mapId, Position const& position) : Position(position), m_mapId(mapId) {}
