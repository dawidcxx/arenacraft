...
enum TempSummonType
{
  ...
  TEMPSUMMON_TIMED_DESPAWN_OOC_ALIVE     = 10, // despawns after a specified time after the creature is out of combat
                                               // and alive
};
...
template <class T_VALUES, class T_FLAGS, class FLAG_TYPE, uint8 ARRAY_SIZE> class FlaggedValuesArray32
{
public:
...
{
protected:
  explicit WorldObject(bool isWorldObject); // note: here it means if it is in grid object list
...
public:
  ...
  void GetNearPoint2D(WorldObject const* searcher, float& x, float& y, float distance, float absAngle,
                      Position const* startPos = nullptr) const;
  void GetNearPoint2D(float& x, float& y, float distance, float absAngle, Position const* startPos = nullptr) const;
  void GetNearPoint(WorldObject const* searcher, float& x, float& y, float& z, float searcher_size, float distance2d,
                    float absAngle, float controlZ = 0, Position const* startPos = nullptr) const;
  void GetVoidClosePoint(float& x, float& y, float& z, float size, float distance2d = 0, float relAngle = 0,
                         float controlZ = 0) const;
  bool GetClosePoint(float& x, float& y, float& z, float size, float distance2d = 0, float angle = 0,
                     WorldObject const* forWho = nullptr, bool force = false) const;
  ...
  void GetChargeContactPoint(WorldObject const* obj, float& x, float& y, float& z,
  ...
  {
    return m_useCombinedPhases ? GetPhaseMask() & phasemask : GetPhaseMask() == phasemask;
  }
  ...
  bool IsWithinDistInMap(WorldObject const* obj, float dist2compare, bool is3D = true,
                         bool useBoundingRadius = true) const;
  [[nodiscard]] bool     IsWithinLOS(float x, float y, float z,
                                     VMAP::ModelIgnoreFlags ignoreFlags = VMAP::ModelIgnoreFlags::Nothing,
                                     LineOfSightChecks      checks      = LINEOFSIGHT_ALL_CHECKS) const;
  [[nodiscard]] bool     IsWithinLOSInMap(WorldObject const*     obj,
                                          VMAP::ModelIgnoreFlags ignoreFlags = VMAP::ModelIgnoreFlags::Nothing,
                                          LineOfSightChecks      checks      = LINEOFSIGHT_ALL_CHECKS,
                                          Optional<float> collisionHeight = {}, Optional<float> combatReach = {}) const;
  [[nodiscard]] Position GetHitSpherePointFor(Position const& dest, Optional<float> collisionHeight = {},
                                              Optional<float> combatReach = {}) const;
  void GetHitSpherePointFor(Position const& dest, float& x, float& y, float& z, Optional<float> collisionHeight = {},
                            Optional<float> combatReach = {}) const;
  ...
  virtual void
  CleanupsBeforeDelete(bool finalCleanup = true); // used in destructor or explicitly before mass creature delete
  ...
  bool CanSeeOrDetect(WorldObject const* obj, bool ignoreStealth = false, bool distanceCheck = false,
  ...
  FlaggedValuesArray32<int32, uint32, ServerSideVisibilityType, TOTAL_SERVERSIDE_VISIBILITY_TYPES>
  ...
  TempSummon* SummonCreature(uint32 id, const Position& pos, TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN,
                             uint32 despwtime = 0, uint32 vehId = 0, SummonPropertiesEntry const* properties = nullptr,
                             bool visibleBySummonerOnly = false) const;
  TempSummon* SummonCreature(uint32 id, float x, float y, float z, float ang = 0,
                             TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN, uint32 despwtime = 0,
                             SummonPropertiesEntry const* properties = nullptr, bool visibleBySummonerOnly = false);
  ...
  Creature*   SummonTrigger(float x, float y, float z, float ang, uint32 dur, bool setLevel = false,
  ...
  {
    return GetMapId() == 571 && GetPositionX() > 3733.33331f && GetPositionX() < 5866.66663f &&
           GetPositionY() > 1599.99999f && GetPositionY() < 4799.99997f;
  }
  ...
  [[nodiscard]] float GetMapHeight(float x, float y, float z, bool vmap = true,
...
private:
  ...
  virtual bool _IsWithinDist(WorldObject const* obj, float dist2compare, bool is3D,
                             bool useBoundingRadius = true) const;
