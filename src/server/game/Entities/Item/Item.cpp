...
void Item::UpdateDuration(Player* owner, uint32 diff)
{
  ...
  LOG_DEBUG("entities.player.items", "Item::UpdateDuration Item (Entry: {} Duration {} Diff {})", GetEntry(),
...
void Item::SaveToDB(CharacterDatabaseTransaction trans)
{
  ...
  case ITEM_CHANGED:
  {
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(uState == ITEM_NEW ? CHAR_REP_ITEM_INSTANCE : CHAR_UPD_ITEM_INSTANCE);
...
bool Item::LoadFromDB(ObjectGuid::LowType guid, ObjectGuid owner_guid, Field* fields, uint32 entry)
{
  ...
  // result = CharacterDatabase.Query("SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags,
  // enchantments, randomPropertyId, durability, playedTime, text FROM item_instance WHERE guid = '{}'", guid);
  ...
  if (tokens.size() == MAX_ITEM_PROTO_SPELLS)
  {
    for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
      if (Optional<int32> charges = Acore::StringTo<int32>(tokens[i]))
        ...
        LOG_ERROR("entities.item", "Invalid charge info '{}' for item {}, charge data not loaded.", tokens.at(i),
                  GetGUID().ToString());
    }
  }
  ...
  {
    LOG_WARN("entities.item", "Invalid enchantment data '{}' for item {}. Forcing partial load.",
             fields[6].Get<std::string>(), GetGUID().ToString());
  }
...
void Item::DeleteFromDB(CharacterDatabaseTransaction trans) { DeleteFromDB(trans, GetGUID().GetCounter()); }
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id)
{
  ...
  if ((itemProto->RandomProperty) && (itemProto->RandomSuffix))
  {
    LOG_ERROR("sql.sql",
              ...
              itemProto->ItemId, itemProto->RandomProperty, itemProto->RandomSuffix);
    return 0;
  }
  ...
  if (itemProto->RandomProperty)
  {
    ...
    if (!random_id)
    {
      LOG_ERROR("sql.sql", "Enchantment id #{} used but it doesn't have records in 'ItemRandomProperties.dbc'",
                randomPropId);
      return 0;
    }
  ...
  else
  {
    ...
    if (!random_id)
    {
      LOG_ERROR("sql.sql", "Enchantment id #{} used but it doesn't have records in sItemRandomSuffixStore.",
                randomPropId);
      return 0;
    }
...
{
  // player CAN be nullptr in which case we must not update random properties because that accesses player's item update
  // queue
