    ...
    if (Item* item = m_bagslot[i])
    {
      if (item->IsInWorld())
      {
        LOG_FATAL("entities.item",
                  "Item {} (slot {}, bag slot {}) in bag {} (slot {}, bag slot {}, m_bagslot {}) is to be deleted but "
                  "is still in world.",
                  item->GetEntry(), (uint32)item->GetSlot(), (uint32)item->GetBagSlot(), GetEntry(), (uint32)GetSlot(),
...
void Bag::SaveToDB(CharacterDatabaseTransaction trans) { Item::SaveToDB(trans); }
