...
enum ItemSpelltriggerType
{
  ...
  ITEM_SPELLTRIGGER_LEARN_SPELL_ID  = 6  // used in item_template.spell_2 with spell_id with SPELL_GENERIC_LEARN
                                         // in spell_1
};
...
enum ItemFieldFlags : uint32
{
  ...
  ITEM_FIELD_FLAG_REFUNDABLE    = 0x00001000, // Item can be returned to vendor for
                                              // its original cost (extended cost)
...
enum ItemFlags : uint32
{
  ...
  ITEM_FLAG_NO_USER_DESTROY = 0x00000020,      // Item can not be destroyed, except by using spell (item can
                                               // be reagent for spell)
  ...
  ITEM_FLAG_ITEM_PURCHASE_RECORD = 0x00001000, // Item can be returned to vendor for its original cost
                                               // (extended cost)
  ...
  ITEM_FLAG_HIDE_UNUSABLE_RECIPE              = 0x02000000, // Profession recipes: can only be looted if you meet
                                                            // requirements and don't already know it
  ITEM_FLAG_NOT_USEABLE_IN_ARENA = 0x04000000,              // Item cannot be used in arena
  ITEM_FLAG_IS_BOUND_TO_ACCOUNT  = 0x08000000,              // Item binds to account and can be sent only to your own
                                                            // characters
...
enum ItemFlagsCustom : uint32
{
  ...
  ITEM_FLAGS_CU_FOLLOW_LOOT_RULES   = 0x0004, // Item will always follow group/master/need before greed looting
                                              // rules
};
...
const uint32 MaxItemSubclassValues[MAX_ITEM_CLASS] = {
    ...
    MAX_ITEM_SUBCLASS_JUNK,       MAX_ITEM_SUBCLASS_GLYPH};
...
struct ItemTemplate
{
  ...
  {
    return GetMaxStackSize() == 1 && Class != ITEM_CLASS_CONSUMABLE && Class != ITEM_CLASS_QUEST &&
           !HasFlag(ITEM_FLAG_NO_CREATOR) && ItemId != 6948; /*Hearthstone*/
  }
  ...
  {
    return Class == ITEM_CLASS_CONSUMABLE && HasFlag(ITEM_FLAG_CONJURED);
  }
