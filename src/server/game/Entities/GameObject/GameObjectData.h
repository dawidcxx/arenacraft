...
struct GameObjectTemplate
{
  ...
  union // different GO types have different data field
  {
    ...
    struct
    {
      uint32 startOpen;        // 0 used client side to determine GO_ACTIVATED means
                               // open/closed
    ...
    struct
    {
      ...
      uint32 consumable;       // 3
      uint32 minSuccessOpens;  // 4 Deprecated, pre 3.0 was used for mining nodes
                               // but since WotLK all mining nodes are usable
                               // once and grant all loot with a single use
      uint32 maxSuccessOpens;  // 5 Deprecated, pre 3.0 was used for mining nodes
                               // but since WotLK all mining nodes are usable
                               // once and grant all loot with a single use
      uint32 eventId;          // 6 lootedEvent
      uint32 linkedTrapId;     // 7
      uint32 questId;          // 8 not used currently but store quest required for GO
                               // activation for player
      uint32 level;            // 9
      uint32 losOK;            // 10
      uint32 leaveLoot;        // 11
    ...
    struct
    {
      ...
      uint32 spellId;       // 3
      uint32 type;          // 4 0 trap with no despawn after cast. 1 trap despawns after
                            // cast. 2 bomb casts on spawn.
  ...
  [[nodiscard]] uint32 GetAutoCloseTime() const
  {
    ...
    return autoCloseTime /* xinef: changed to milliseconds/ IN_MILLISECONDS*/
