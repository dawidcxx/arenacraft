...
enum PlayerFlags : uint32
{
  ...
  PLAYER_ALLOW_ONLY_ABILITY   = 0x00800000, // used by bladestorm and killing spree, allowed only spells
...
enum InstanceResetWarningType
{
  ...
  RAID_INSTANCE_WARNING_MIN_SOON = 3, // WARNING! %s is scheduled to reset in %d minute(s). Please exit the
...
public:
  ...
  {
    return TeleportTo(loc.GetMapId(), loc.GetPositionX(), loc.GetPositionY(), loc.GetPositionZ(), loc.GetOrientation(),
                      options, target);
  }
  ...
  /// Handles whispers from Addons and players based on sender, receiver's guid
  /// and language.
  ...
  [[nodiscard]] inline Item* GetUseableItemByPos(uint8 bag,
                                                 uint8 slot) const // Does additional check for disarmed weapons
  {
  ...
  {
    return (Unit::HasWeaponForAttack(type) && GetWeaponForAttack(type, true));
  }
  ...
  [[nodiscard]] bool  HasItemOrGemWithLimitCategoryEquipped(uint32 limitCategory, uint32 count,
  ...
  InventoryResult CanEquipUniqueItem(ItemTemplate const* itemProto, uint8 except_slot = NULL_SLOT,
  ...
  Item* StoreNewItem(ItemPosCountVec const& pos, uint32 item, bool update, int32 randomPropertyId,
  ...
  InventoryResult CanTakeMoreSimilarItems(uint32 entry, uint32 count, Item* pItem,
                                          uint32* no_space_count = nullptr) const;
  InventoryResult CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, uint32 entry, uint32 count,
  ...
  bool _StoreOrEquipNewItem(uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot, int32 price,
  ...
  {
    return (uint16)(GetUInt64Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_COUNTS_OFFSET) >>
                    (counter * 16));
  }
  ...
  [[nodiscard]] bool HasQuestForItem(uint32 itemId, uint32 excludeQuestId = 0, bool turnIn = false,
  ...
  void SendQuestUpdateAddCreatureOrGo(Quest const* quest, ObjectGuid guid, uint32 creatureOrGO_idx, uint16 old_count,
                                      uint16 add_count);
  ...
  static bool   LoadPositionFromDB(uint32& mapid, float& x, float& y, float& z, float& o, bool& in_flight,
  ...
  void
  SaveInventoryAndGoldToDB(CharacterDatabaseTransaction trans); // fast save function for item/money cheating preventing
  ...
  static void SavePositionInDB(WorldLocation const& loc, uint16 zoneId, ObjectGuid guid,
  ...
  void SendMailResult(uint32 mailId, MailResponseType mailAction, MailResponseResult mailError, uint32 equipError = 0,
                      ObjectGuid::LowType item_guid = 0, uint32 item_count = 0);
  void SendNewMail();
  ...
  void                             SendItemRetrievalMail(
                                  std::vector<std::pair<uint32, uint32>> mailItems); // Item retrieval mails sent by The Postmaster (34337)
  ...
  bool RemoveMItem(ObjectGuid::LowType itemLowGuid) { return !!mMitems.erase(itemLowGuid); }
  ...
  void                 AddSpellAndCategoryCooldowns(SpellInfo const* spellInfo, uint32 itemId, Spell* spell = nullptr,
                                                    bool infinityCooldown = false);
  void                 AddSpellCooldown(uint32 spell_id, uint32 itemid, uint32 end_time, bool needSendToClient = false,
                                        bool forceSendToSpectator = false) override;
  void                 _AddSpellCooldown(uint32 spell_id, uint16 categoryId, uint32 itemid, uint32 end_time,
  ...
  static void RemoveFromGroup(Group* group, ObjectGuid guid, RemoveMethod method = GROUP_REMOVEMETHOD_DEFAULT,
  ...
  void SendLogXPGain(uint32 GivenXP, Unit* victim, uint32 BonusXP, bool recruitAFriend = false,
  ...
  void SendMessageToSetInRange(WorldPacket const* data, float dist, bool self, bool includeMargin, bool ownTeamOnly,
                               bool required3dDist = false) const;
  ...
  float CalculateReputationGain(ReputationSource source, uint32 creatureOrQuestLevel, float rep, int32 faction,
  ...
  void                 ModifyHonorPoints(
                      int32                        value,
                      CharacterDatabaseTransaction trans = CharacterDatabaseTransaction(nullptr)); //! If trans is specified, honor save
                                                                                   //! query will be added to trans
  void ModifyArenaPoints(int32                        value,
                         CharacterDatabaseTransaction trans =
                             CharacterDatabaseTransaction(nullptr)); //! If trans is specified, arena point save query
                                                                     //! will be added to trans
  ...
  SpellSchoolMask GetMeleeDamageSchoolMask(WeaponAttackType attackType  = BASE_ATTACK,
  ...
  void CastItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item* item,
  ...
  void   SetBattlegroundId(uint32 id, BattlegroundTypeId bgTypeId, uint32 queueSlot, bool invited, bool isRandom,
                           TeamId teamId);
  ...
  template <class T> void UpdateVisibilityOf(T* target, UpdateData& data, std::vector<Unit*>& visibleNow);
  ...
  {
    return GetSession()->PlayerLogout() || !IsInWorld() || !IsAlive() || IsMounted() /*+in flight*/ || GetVehicle() ||
           IsBeingTeleported();
  }
  ...
  void
  PrettyPrintRequirementsAchievementsList(const std::vector<const ProgressionRequirement*>& missingAchievements) const;
  ...
  void               ResetAchievementCriteria(AchievementCriteriaCondition condition, uint32 value,
                                              bool evenIfCriteriaComplete = false);
  void UpdateAchievementCriteria(AchievementCriteriaTypes type, uint32 miscValue1 = 0, uint32 miscValue2 = 0,
                                 Unit* unit = nullptr);
...
private:
  ...
  InventoryResult CanStoreItem_InSpecificSlot(uint8 bag, uint8 slot, ItemPosCountVec& dest, ItemTemplate const* pProto,
                                              uint32& count, bool swap, Item* pSrcItem) const;
  InventoryResult CanStoreItem_InBag(uint8 bag, ItemPosCountVec& dest, ItemTemplate const* pProto, uint32& count,
                                     bool merge, bool non_specialized, Item* pSrcItem, uint8 skip_bag,
                                     uint8 skip_slot) const;
  InventoryResult CanStoreItem_InInventorySlots(uint8 slot_begin, uint8 slot_end, ItemPosCountVec& dest,
