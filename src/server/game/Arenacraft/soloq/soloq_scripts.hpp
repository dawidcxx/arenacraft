...
public:
  SoloQueueScript() : AllBattlegroundScript("SoloQueueScript"), matchMaker(250) {};
  ...
  void OnQueueUpdate(BattlegroundQueue* queue, uint32 diff, BattlegroundTypeId bgTypeId,
                     ...
                     uint32 /*arenaRatedTeamId*/) override
  {
    if (arenaType != ARENA_TYPE_5v5 || !isRated)
      return;
    ...
    for (auto& matchup : matchups)
    {
      ...
      //     if (p)
      //     {
      //         p->GetSession()->SendArenaTeamCommandResult(ArenaTeamCommandTypes::ERR_ARENA_TEAM_CREATE_S,
      //         p->GetName(), "", ERR_ALREADY_IN_ARENA_TEAM);
      //     }
      // });
    }
  }
...
public:
  ...
  {
    AddGossipItemFor(player, GossipOptionIcon::GOSSIP_ICON_BATTLE, "Join SoloQ", GOSSIP_SENDER_MAIN,
                     NpcSoloqAction::QUEUE);
    AddGossipItemFor(player, GossipOptionIcon::GOSSIP_ICON_INTERACT_1, "Create SoloQ", GOSSIP_SENDER_MAIN,
                     NpcSoloqAction::CREATE_TEAM);
...
private:
  ...
  bool CreateSoloTeam(Player* player, Creature* creature)
  {
    if (arenacraft::soloq::io::HasSoloTeam(player))
    {
      player->GetSession()->SendArenaTeamCommandResult(ArenaTeamCommandTypes::ERR_ARENA_TEAM_CREATE_S,
                                                       player->GetName(), "", ERR_ALREADY_IN_ARENA_TEAM);
      return false;
    }
