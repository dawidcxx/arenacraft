...
enum AchievementCriteriaDataType
{
  ...
  ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_DEAD = 4, // own_team       0             not corpse (not released body),
  ...
  ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE  = 8, // minvalue                     value provided with achievement update
                                             // must be not less that limit
  ...
  ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY = 12, // difficulty                   normal/heroic difficulty for
                                                      // current event map
  ...
  ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_LOSS_TEAM_SCORE = 17, // min_score      max_score     player's team win bg and
                                                          // opposition team have team score in range
  ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT = 18, // 0              0             maker instance script call for
                                                       // check current criteria requirements fit
  ACHIEVEMENT_CRITERIA_DATA_TYPE_S_EQUIPPED_ITEM = 19, // item_level     item_quality  for equipped item in slot to
                                                       // check item level and quality
  ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_ID = 20,              // map_id         0 player must be
                                                           // on map with id in map_id
  ...
  ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_TEAMS_SCORES = 24, // winner_score   loser score   player's team win bg and their
                                                       // teams have exact scores
...
private:
  ...
  void SendCriteriaUpdate(AchievementCriteriaEntry const* entry, CriteriaProgress const* progress, uint32 timeElapsed,
                          bool timedCompleted) const;
  void SetCriteriaProgress(AchievementCriteriaEntry const* entry, uint32 changeValue,
...
public:
  ...
  AchievementCriteriaEntryList const* GetAchievementCriteriaByCondition(AchievementCriteriaCondition condition,
                                                                        uint32                       val)
  {
  ...
  void UpdateAchievementCriteriaForOfflinePlayer(ObjectGuid::LowType playerLowGuid, AchievementCriteriaTypes type,
