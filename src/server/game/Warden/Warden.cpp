...
Warden::Warden()
    : _session(nullptr), _checkTimer(10000 /*10 sec*/), _clientResponseTimer(0), _dataSent(false), _module(nullptr),
      _initialized(false), _interrupted(false), _checkInProgress(false)
{
...
void Warden::DecryptData(uint8* buffer, uint32 length) { _inputCrypto.UpdateData(buffer, length); }

void Warden::EncryptData(uint8* buffer, uint32 length) { _outputCrypto.UpdateData(buffer, length); }
...
void Warden::ApplyPenalty(uint16 checkId, std::string const& reason)
{
  ...
  if (checkId)
  {
    if (Player const* plr = _session->GetPlayer())
    {
      ...
      reportMsg                      = Acore::StringFormat(
          reportFormat, plr->GetName(), plr->GetGUID().GetCounter(), _session->GetAccountId(), checkId,
  ...
  else
  {
    if (Player const* plr = _session->GetPlayer())
    {
      ...
      reportMsg = Acore::StringFormat(reportFormat, plr->GetName(), plr->GetGUID().GetCounter(),
                                      _session->GetAccountId(), causeMsg, GetWardenActionStr(action));
    }
    else
    {
      std::string const reportFormat = "Account id: {} failed warden {} check. Action: {}";
