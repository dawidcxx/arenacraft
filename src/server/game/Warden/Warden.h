...
enum WardenOpcodes
{
  ...
  WARDEN_SMSG_MEM_CHECKS_REQUEST   = 4, // byte len; while(!EOF) { byte unk(1); byte index(++); string
...
{
  MEM_CHECK = 0xF3,      // 243: byte moduleNameIndex + uint Offset + byte Len (check
                         // to ensure memory isn't modified)
  PAGE_CHECK_A = 0xB2,   // 178: uint Seed + byte[20] SHA1 + uint Addr + byte Len
                         // (scans all pages for specified hash)
  PAGE_CHECK_B = 0xBF,   // 191: uint Seed + byte[20] SHA1 + uint Addr + byte Len (scans only
                         // pages starts with MZ+PE headers for specified hash)
  ...
  DRIVER_CHECK   = 0x71, // 113: uint Seed + byte[20] SHA1 + byte driverNameIndex
  ...
  MODULE_CHECK = 0xD9,   // 217: uint Seed + byte[20] SHA1 (check to ensure module
                         // isn't injected)
};
