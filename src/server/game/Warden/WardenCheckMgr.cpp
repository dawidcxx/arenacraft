...
void WardenCheckMgr::LoadWardenChecks()
{
  ...
  do
  {
    ...
    {
      LOG_ERROR("warden",
                "sql.sql: Warden Lua check with id {} found in `warden_checks`. Lua checks may have four-digit IDs at "
                "most. Skipped.",
                id);
      continue;
    }
    ...
    if (checkType == MEM_CHECK || checkType == MPQ_CHECK || checkType == LUA_EVAL_CHECK || checkType == DRIVER_CHECK ||
    ...
    case LUA_EVAL_CHECK:
    {
      ...
      {
        LOG_ERROR("warden",
                  "sql.sql: Found over-long Lua check for Warden check with id {} in `warden_checks`. Max length is "
                  "{}. Skipped.",
...
void WardenCheckMgr::LoadWardenOverrides()
{
  ...
  do
  {
    ...
    // Check if check actually exists before accessing the CheckStore vector
      ...
      LOG_ERROR("warden", "Warden check action override for non-existing check (ID: {}, action: {}), skipped", checkId,
                action);
    else
    {
