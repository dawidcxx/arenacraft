...
void Battlefield::RegisterZone(uint32 zoneId) { sBattlefieldMgr->AddZone(zoneId, this); }
...
void Battlefield::HideNpc(Creature* creature)
{
  ...
  creature->SetPhaseMask(2, false); // pussywizard: false because UpdateObjectVisibility(true) is called below in
                                    // SetVisible(), no need to have it here
...
{
  creature->SetPhaseMask(1, false); // pussywizard: false because UpdateObjectVisibility(true) is called below in
                                    // SetVisible(), no need to have it here
...
bool Battlefield::AddOrSetPlayerToCorrectBfGroup(Player* player)
{
  ...
  {
    LOG_INFO("misc", "Battlefield::AddOrSetPlayerToCorrectBfGroup - player is already in {} group!",
...
void BfGraveyard::RelocateDeadPlayers()
{
  ...
  for (GuidUnorderedSet::const_iterator itr = m_ResurrectQueue.begin(); itr != m_ResurrectQueue.end(); ++itr)
  {
    ...
    if (closestGrave)
      player->TeleportTo(player->GetMapId(), closestGrave->x, closestGrave->y, closestGrave->z,
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o)
{
  ...
  if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(), entry, map, PHASEMASK_NORMAL, x, y, z, o, G3D::Quat(),
                  100, GO_STATE_READY))
  {
    LOG_ERROR("sql.sql",
    ...
    LOG_ERROR("bg.battlefield",
              "Battlefield::SpawnGameObject: Cannot create gameobject template {}! Battlefield not created!", entry);
    delete go;
    return nullptr;
  }
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint, TeamId team)
{
  ...
  // At first call using TEAM_NEUTRAL as a checker but never using it, after first call we reset the capturepoints to
  // the new winner of the last WG war
...
GameObject* BfCapturePoint::GetCapturePointGo() { return m_Bf->GetGameObject(m_capturePoint); }
...
bool BfCapturePoint::Update(uint32 diff)
{
  ...
  else // grey, did not go through mid point
  {
    ...
    else if (ChallengerId == TEAM_HORDE && (m_OldState == BF_CAPTUREPOINT_OBJECTIVESTATE_ALLIANCE ||
