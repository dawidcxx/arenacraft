...
void BattlefieldMgr::AddZone(uint32 zoneid, Battlefield* handle) { m_BattlefieldMap[zoneid] = handle; }
...
void BattlefieldMgr::HandlePlayerEnterZone(Player* player, uint32 zoneid)
{
  ...
  LOG_DEBUG("bg.battlefield", "Player {} entered outdoorpvp id {}", player->GetGUID().ToString(),
...
void BattlefieldMgr::HandlePlayerLeaveZone(Player* player, uint32 zoneid)
{
  ...
  LOG_DEBUG("bg.battlefield", "Player {} left outdoorpvp id {}", player->GetGUID().ToString(),
            itr->second->GetTypeId());
}
