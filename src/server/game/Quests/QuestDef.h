...
enum QuestFailedReason : uint32
{
  ...
  INVALIDREASON_QUEST_FAILED_MISSING_ITEMS = 21,    // DESCRIPTION You don't have the required items with you. Check
                                                    // storage.
  ...
  INVALIDREASON_QUEST_FAILED_CAIS = 27,          // DESCRIPTION You cannot complete quests once you have reached tired
                                                 // time.
...
enum QuestFlags
{
  ...
  QUEST_FLAGS_PARTY_ACCEPT = 0x00000002,            // Not used currently. If player in party, all players that
  ...
  QUEST_FLAGS_NO_MONEY_FROM_XP = 0x00000100,        // Not used currently: Experience is not converted to gold at
                                                    // max level
  QUEST_FLAGS_HIDDEN_REWARDS = 0x00000200,          // Items and money rewarded only sent in
                                                    // SMSG_QUESTGIVER_OFFER_REWARD (not in
                                                    // SMSG_QUESTGIVER_QUEST_DETAILS or in client quest
                                                    // log(SMSG_QUEST_QUERY_RESPONSE))
  QUEST_FLAGS_TRACKING = 0x00000400,                // These quests are automatically rewarded on quest complete
  ...
  QUEST_FLAGS_AUTO_ACCEPT             = 0x00080000, // The client recognizes this flag as auto-accept. However,
...
enum QuestSpecialFlags
{
  ...
  QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT = 0x0002,  // Set by 2 in SpecialFlags from DB (if required area explore,
  ...
  QUEST_SPECIAL_FLAGS_AUTO_ACCEPT = 0x0004,           // Set by 4 in SpecialFlags in DB if
                                                      // the quest is to be auto-accepted.
  QUEST_SPECIAL_FLAGS_DF_QUEST = 0x0008,              // Set by 8 in SpecialFlags in DB if
                                                      // the quest is used by Dungeon Finder.
  QUEST_SPECIAL_FLAGS_MONTHLY = 0x0010,               // Set by 16 in SpecialFlags in DB if the quest is reset at the
                                                      // begining of the month
  QUEST_SPECIAL_FLAGS_CAST = 0x0020,                  // Set by 32 in SpecialFlags in DB if the quest requires
                                                      // RequiredOrNpcGo killcredit but NOT kill (a spell cast)
  QUEST_SPECIAL_FLAGS_NO_REP_SPILLOVER = 0x0040,      // Set by 64 in SpecialFlags in DB if the quest does not share
                                                      // rewarded reputation with other allied factions
  QUEST_SPECIAL_FLAGS_CAN_FAIL_IN_ANY_STATE = 0x0080, // Set by 128 in SpecialFlags in DB if the quest is allowed to
                                                      // fail in Player::FailQuest() independant of its current state
  QUEST_SPECIAL_FLAGS_NO_LOREMASTER_COUNT = 0x0100,   // Set by 256 in SpecialFlags in DB if the quest should not count
                                                      // towards Loremaster Achievement
  ...
  QUEST_SPECIAL_FLAGS_DB_ALLOWED = QUEST_SPECIAL_FLAGS_REPEATABLE | QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT |
                                   ...
                                   QUEST_SPECIAL_FLAGS_NO_REP_SPILLOVER | QUEST_SPECIAL_FLAGS_CAN_FAIL_IN_ANY_STATE |
...
public:
  ...
  {
    return (ZoneOrSort == -QUEST_SORT_SEASONAL || ZoneOrSort == -QUEST_SORT_SPECIAL ||
            ZoneOrSort == -QUEST_SORT_LUNAR_FESTIVAL || ZoneOrSort == -QUEST_SORT_MIDSUMMER ||
            ZoneOrSort == -QUEST_SORT_BREWFEST || ZoneOrSort == -QUEST_SORT_LOVE_IS_IN_THE_AIR ||
