...
int main(int argc, char** argv)
{
  ...
  // Set signal handlers (this must be done before starting IoContext threads, because otherwise they would unblock and
  // exit)
  ...
  {
    soapThread.reset(new std::thread(ACSoapThread, sConfigMgr->GetOption<std::string>("SOAP.IP", "127.0.0.1"),
...
void ShutdownCLIThread(std::thread* cliThread)
{
  if (cliThread)
  {
    ...
    if (!CancelSynchronousIo(cliThread->native_handle()))
    {
      ...
      LOG_DEBUG("server.worldserver", "Error cancelling I/O of CliThread, error code {}, detail: {}", uint32(errorCode),
                errorBuffer);
...
{
  QueryResult result =
      LoginDatabase.Query("SELECT id, name, address, localAddress, localSubnetMask, port, icon, flag, timezone, "
                          "allowedSecurityLevel, population, gamebuild FROM realmlist WHERE id = {}",
                          realm.Id.Realm);
  if (!result)
    return false;
                                  ...
                                  [[maybe_unused]] std::string& configService)
{
  ...
  win.add_options()("service,s", value<std::string>(&configService)->default_value(""),
