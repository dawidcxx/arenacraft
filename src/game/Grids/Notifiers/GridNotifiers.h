...
struct MessageDistDeliverer
{
  ...
  bool               required3dDist;
  MessageDistDeliverer(WorldObject const* src, WorldPacket const* msg, float dist, bool own_team_only = false,
                       Player const* skipped = nullptr, bool req3dDist = false)
...
template <class Check> struct WorldObjectLastSearcher
{
  uint32        i_mapTypeMask;
  uint32        i_phaseMask;
  WorldObject*& i_object;
...
template <class Check> struct WorldObjectListSearcher : ContainerInserter<WorldObject*>
{
  uint32 i_mapTypeMask;
  uint32 i_phaseMask;
  Check& i_check;
  ...
  WorldObjectListSearcher(WorldObject const* searcher, Container& container, Check& check,
                          uint32 mapTypeMask = GRID_MAP_TYPE_MASK_ALL)
      : ContainerInserter<WorldObject*>(container), i_mapTypeMask(mapTypeMask), i_phaseMask(searcher->GetPhaseMask()),
        i_check(check)
  {
  }
...
template <class Check> struct GameObjectListSearcher : ContainerInserter<GameObject*>
{
  uint32 i_phaseMask;
  Check& i_check;
  ...
  GameObjectListSearcher(WorldObject const* searcher, Container& container, Check& check)
      : ContainerInserter<GameObject*>(container), i_phaseMask(searcher->GetPhaseMask()), i_check(check)
  {
  }
...
template <class Functor> struct GameObjectWorker
{
  GameObjectWorker(WorldObject const* searcher, Functor& func) : _func(func), _phaseMask(searcher->GetPhaseMask()) {}
...
template <class Check> struct UnitListSearcher : ContainerInserter<Unit*>
{
  uint32 i_phaseMask;
  Check& i_check;
  ...
  UnitListSearcher(WorldObject const* searcher, Container& container, Check& check)
      : ContainerInserter<Unit*>(container), i_phaseMask(searcher->GetPhaseMask()), i_check(check)
  {
  }
...
template <class Check> struct CreatureListSearcher : ContainerInserter<Creature*>
{
  uint32 i_phaseMask;
  Check& i_check;
  ...
  CreatureListSearcher(WorldObject const* searcher, Container& container, Check& check)
      : ContainerInserter<Creature*>(container), i_phaseMask(searcher->GetPhaseMask()), i_check(check)
  {
  }
...
template <class Check> struct PlayerListSearcher : ContainerInserter<Player*>
{
  uint32 i_phaseMask;
  Check& i_check;
  ...
  PlayerListSearcher(WorldObject const* searcher, Container& container, Check& check)
      : ContainerInserter<Player*>(container), i_phaseMask(searcher->GetPhaseMask()), i_check(check)
  {
  }
...
template <class Check> struct PlayerListSearcherWithSharedVision
{
  uint32              i_phaseMask;
...
template <class Check> struct PlayerLastSearcher
{
  uint32   i_phaseMask;
  Player*& i_object;
  Check&   i_check;
...
template <class Do> struct PlayerDistWorker
{
...
{
public:
  AnyDeadUnitSpellTargetInRangeCheck(Unit* searchObj, float range, SpellInfo const* spellInfo,
...
public:
  ...
  bool operator()(GameObject* go)
  {
    if (go->GetGOInfo()->type == GAMEOBJECT_TYPE_FISHINGHOLE && go->isSpawned() &&
        i_obj.IsWithinDistInMap(go, i_range) && i_obj.IsWithinDistInMap(go, (float)go->GetGOInfo()->fishinghole.radius))
    {
...
public:
  ...
  {
    if (u->IsAlive() && u->IsInCombat() && !i_obj->IsHostileTo(u) && i_obj->IsWithinDistInMap(u, i_range) &&
        (u->HasFearAura() || u->IsCharmed() || u->isFrozen() || u->HasUnitState(UNIT_STATE_STUNNED) ||
...
public:
  ...
  bool operator()(Unit* u)
  {
    ...
    if (u->IsCreature() && (u->ToCreature()->IsTotem() || u->ToCreature()->IsTrigger() ||
                            u->ToCreature()->IsAvoidingAOE())) // pussywizard: added IsAvoidingAOE()
      return false;
...
public:
  ...
  {
    return u->IsAlive() && i_funit->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u) &&
           i_funit->IsValidAttackTarget(u) && !u->IsCritter() && !u->IsTotem() // xinef: dont attack totems
        /*&& i_funit->CanSeeOrDetect(u)*/;                                     // pussywizard: already checked in
                                                                               // IsValidAttackTarget(u)
  }
...
{
public:
  AnyFriendlyNotSelfUnitInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range,
  ...
  bool operator()(Unit* u)
  {
    if (u != i_obj && u->IsAlive() && i_obj->IsWithinDistInMap(u, i_range) && i_funit->IsFriendlyTo(u) &&
        (!i_playerOnly || u->IsPlayer()))
      return true;
    else
      return false;
...
public:
  ...
  bool operator()(Unit* u)
  {
    // Check contains checks for: live, non-selectable, non-attackable flags,
    // flight check and GM check, ignore totems
    ...
    if (i_funit->_IsValidAttackTarget(u, _spellInfo, i_obj->IsDynamicObject() ? i_obj : nullptr) &&
        i_obj->IsWithinDistInMap(u, i_range, true, false))
...
public:
  ...
  bool operator()(Unit* u)
  {
    if (!u->IsAlive() || u->HasUnitFlag(UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE) ||
...
public:
  ...
  {
    // No threat list check, must be done explicit if expected to be in combat
    // with creature
...
public:
  ...
  bool operator()(WorldObject const* object) { return (object->GetTypeId() == _typeId) == _equals; }
...
public:
  ...
  bool operator()(WorldObject const* object) { return (object->GetGUID() == _GUID) == _equals; }
...
public:
  ...
  bool operator()(Unit const* unit) const { return unit->HasAura(_spellId, _casterGUID) == _present; }
...
public:
  ...
  bool operator()(WorldObject const* object) { return (_object->GetExactDist2d(object) > _range) == _equals; }
...
public:
  ...
  bool operator()(WorldObject const* /*object*/) const { return roll_chance_i(_chance); }
