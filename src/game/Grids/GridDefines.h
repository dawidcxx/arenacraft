...
typedef TYPELIST_5(GameObject, Player, Creature /*pets*/, Corpse /*resurrectable*/,
...
template <uint32 LIMIT> struct CoordPair
{
  ...
  [[nodiscard]] bool IsCoordValid() const { return x_coord < LIMIT && y_coord < LIMIT; }
...
template <uint32 LIMIT> bool operator==(const CoordPair<LIMIT>& p1, const CoordPair<LIMIT>& p2)
{
  return (p1.x_coord == p2.x_coord && p1.y_coord == p2.y_coord);
}

template <uint32 LIMIT> bool operator!=(const CoordPair<LIMIT>& p1, const CoordPair<LIMIT>& p2) { return !(p1 == p2); }
...
inline bool IsValidMapCoord(float c) { return std::isfinite(c) && (std::fabs(c) <= MAP_HALFSIZE - 0.5f); }

inline bool IsValidMapCoord(float x, float y) { return IsValidMapCoord(x) && IsValidMapCoord(y); }

inline bool IsValidMapCoord(float x, float y, float z) { return IsValidMapCoord(x, y) && IsValidMapCoord(z); }
