...
inline void Cell::Visit(CellCoord const& standing_cell, TypeContainerVisitor<T, CONTAINER>& visitor, Map& map,
                        WorldObject const& obj, float radius) const
{
...
template <class T, class CONTAINER>
inline void Cell::Visit(CellCoord const& standing_cell, TypeContainerVisitor<T, CONTAINER>& visitor, Map& map,
                        float x_off, float y_off, float radius) const
{
...
template <class T, class CONTAINER>
inline void Cell::VisitCircle(TypeContainerVisitor<T, CONTAINER>& visitor, Map& map, CellCoord const& begin_cell,
                              CellCoord const& end_cell) const
{
  ...
  for (uint32 step = 1; step <= (x_start - begin_cell.x_coord); ++step)
  {
    ...
    {
      // we visit cells symmetrically from both sides, heading from center to
      // sides and from up to bottom e.g. filling 2 trapezoids after filling
      // central cell strip...
