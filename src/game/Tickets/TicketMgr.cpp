...
// GM ticket
GmTicket::GmTicket()
    : _id(0), _type(TICKET_TYPE_OPEN), _posX(0), _posY(0), _posZ(0), _mapId(0), _createTime(0), _lastModifiedTime(0),
...
GmTicket::GmTicket(Player* player)
    : _type(TICKET_TYPE_OPEN), _createTime(GameTime::GetGameTime().count()),
      _lastModifiedTime(GameTime::GetGameTime().count()), _completed(false), _escalatedStatus(TICKET_UNASSIGNED),
...
bool GmTicket::LoadFromDB(Field* fields)
{
  ...
  //  15         16        17        18         19
  // id, type, playerGuid, name, message, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, assignedTo,
  // comment, response, completed, escalated, viewed, haveTicket, resolvedBy
  uint8 index       = 0;
...
void GmTicket::SaveToDB(CharacterDatabaseTransaction trans) const
{
  ...
  //  14        15         16        17         18          19
  // id, type, playerGuid, name, description, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy,
  // assignedTo, comment, response, completed, escalated, viewed, needMoreHelp, resolvedBy
  uint8                               index = 0;
...
std::string GmTicket::FormatMessageString(ChatHandler& handler, bool detailed) const
{
  ...
  ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTAGE,
...
std::string GmTicket::FormatMessageString(ChatHandler& handler, const char* szClosedName, const char* szAssignedToName,
                                          const char* szUnassignedName, const char* szDeletedName) const
{
  std::stringstream ss;
  ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTGUID, _id);
  ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTNAME, _playerName.c_str());
...
void GmTicket::SetGmAction(uint32 needResponse, bool needMoreHelp)
{
  ...
  _needMoreHelp = needMoreHelp; // Requests further GM interaction on a ticket to which a GM has already responded.
...
void TicketMgr::UpdateLastChange() { _lastChange = GameTime::GetGameTime().count(); }
