...
public:
  ...
  std::string FormatMessageString(ChatHandler& handler, const char* szClosedName, const char* szAssignedToName,
...
private:
  ...
  ObjectGuid _resolvedBy; // 0 = Open, -1 = Resolved by Console, GM who resolved
                          // it by closing or completing the ticket.
  ObjectGuid               _assignedTo;
  std::string              _comment;
  bool                     _completed;
  GMTicketEscalationStatus _escalatedStatus;
  bool                     _viewed;
  bool                     _needResponse; /// @todo: find out the use of this, and then store it in
                                          /// DB
  bool        _needMoreHelp;
...
public:
  ...
  void ResolveAndCloseTicket(uint32     ticketId,
                             ObjectGuid source); // used when GM resolves a ticket by simply closing it
