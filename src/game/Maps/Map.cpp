...
ZoneDynamicInfo::ZoneDynamicInfo()
    : MusicId(0), WeatherId(WEATHER_STATE_FINE), WeatherGrade(0.0f), OverrideLightId(0), LightFadeInTime(0)
{
}
...
bool Map::ExistMap(uint32 mapid, int gx, int gy)
{
  ...
  else
  {
    ...
    if (fread(&header, sizeof(header), 1, pf) == 1)
    {
      ...
      {
        LOG_ERROR("maps",
                  "Map file '{}' is from an incompatible map version (%.*u v{}), %.*s v{} is expected. Please pull "
                  "your source, recompile tools and recreate maps using the updated mapextractor, then replace your "
                  "old map files with new files.",
  ...
  if (VMAP::IVMapMgr* vmgr = VMAP::VMapFactory::createOrGetVMapMgr())
  {
    if (vmgr->isMapLoadingEnabled())
    {
      ...
      case VMAP::LoadResult::FileNotFound:
        ...
        LOG_ERROR("maps",
                  "Please place VMAP files (*.vmtree and *.vmtile) in the vmap directory ({}), or correct the DataDir "
      ...
      case VMAP::LoadResult::VersionMismatch:
        ...
        LOG_ERROR("maps", "This is because the version of the VMap file and the version of this module are different, "
                          "please re-extract the maps with the tools compiled with this module.");
        return false;
      }
    }
  }
...
void Map::LoadMMap(int gx, int gy)
{
  ...
  case MMAP::MMAP_LOAD_RESULT_OK:
    LOG_DEBUG("maps", "MMAP loaded name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})", GetMapName(), GetId(), gx, gy,
              gx, gy);
    break;
  case MMAP::MMAP_LOAD_RESULT_ERROR:
    LOG_DEBUG("maps", "Could not load MMAP name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})", GetMapName(), GetId(),
              gx, gy, gx, gy);
    break;
  case MMAP::MMAP_LOAD_RESULT_IGNORED:
    LOG_DEBUG("maps", "Ignored MMAP name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})", GetMapName(), GetId(), gx, gy,
              gx, gy);
    break;
  }
}
...
void Map::LoadVMap(int gx, int gy)
{
  ...
  case VMAP::VMAP_LOAD_RESULT_OK:
    LOG_DEBUG("maps", "VMAP loaded name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})", GetMapName(), GetId(), gx, gy,
              gx, gy);
    break;
  case VMAP::VMAP_LOAD_RESULT_ERROR:
    LOG_DEBUG("maps", "Could not load VMAP name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})", GetMapName(), GetId(),
              gx, gy, gx, gy);
    break;
  case VMAP::VMAP_LOAD_RESULT_IGNORED:
    LOG_DEBUG("maps", "Ignored VMAP name:{}, id:{}, x:{}, y:{} (vmap rep.: x:{}, y:{})", GetMapName(), GetId(), gx, gy,
              gx, gy);
    break;
  }
}
...
void Map::LoadMap(int gx, int gy, bool reload)
{
  ...
  // map already load, delete it before reloading (Is it necessary? Do we really need the ability the reload maps during
  // runtime?)
  if (GridMaps[gx][gy])
  {
...
Map::Map(uint32 id, uint32 InstanceId, uint8 SpawnMode, Map* _parent)
    : i_mapEntry(sMapStore.LookupEntry(id)), i_spawnMode(SpawnMode), i_InstanceId(InstanceId), m_unloadTimer(0),
      ...
      m_activeNonPlayersIter(m_activeNonPlayers.end()), _transportsUpdateIter(_transports.end()), i_scriptLock(false),
...
template <> void Map::AddToGrid(Corpse* obj, Cell const& cell)
{
  ...
  // so we need to explicitly check it here (Map::AddToGrid is only called from Player::BuildPlayerRepop, not from
  // ObjectGridLoader) to avoid failing an assertion in GridObject::AddToGrid
...
template <> void Map::SwitchGridContainers(GameObject* obj, bool on)
{
  ...
  // LOG_DEBUG(LOG_FILTER_MAPS, "Switch object {} from grid[{}, {}] {}", obj->GetGUID().ToString(),
  // cell.data.Part.grid_x, cell.data.Part.grid_y, on);
  NGridType* ngrid = getNGrid(cell.GridX(), cell.GridY());
  ASSERT(ngrid);
...
bool Map::AddPlayerToMap(Player* player)
{
  ...
  if (!cellCoord.IsCoordValid())
  {
    LOG_ERROR("maps", "Map::Add: Player ({}) has invalid coordinates X:{} Y:{} grid cell [{}:{}]",
              player->GetGUID().ToString(), player->GetPositionX(), player->GetPositionY(), cellCoord.x_coord,
              cellCoord.y_coord);
    return false;
  }
...
template <class T> bool Map::AddToMap(T* obj, bool checkTransport)
{
  ...
  if (!cellCoord.IsCoordValid())
  {
    LOG_ERROR("maps", "Map::AddToMap: Object {} has invalid coordinates X:{} Y:{} grid cell [{}:{}]",
              obj->GetGUID().ToString(), obj->GetPositionX(), obj->GetPositionY(), cellCoord.x_coord,
              cellCoord.y_coord);
  ...
  // Xinef: little hack for vehicles, accessories have to be added after visibility update so they wont fall off the
  // vehicle, moved from Creature::AIM_Initialize Initialize vehicle, this is done only for summoned npcs, DB creatures
  // are handled by grid loaders
  if (obj->IsCreature())
...
void Map::Update(const uint32 t_diff, const uint32 s_diff, bool /*thread*/)
{
  ...
  for (m_mapRefIter = m_mapRefMgr.begin(); m_mapRefIter != m_mapRefMgr.end(); ++m_mapRefIter)
  {
    ...
    if (WorldObject* viewPoint = player->GetViewpoint())
    {
      ...
      {
        VisitNearbyCellsOf(viewCreature, grid_object_update, world_object_update, grid_large_object_update,
                           world_large_object_update);
      }
      else if (DynamicObject* viewObject = viewPoint->ToDynObject())
      {
        VisitNearbyCellsOf(viewObject, grid_object_update, world_object_update, grid_large_object_update,
                           world_large_object_update);
      }
    }
...
void Map::CreatureRelocation(Creature* creature, float x, float y, float z, float o)
{
  ...
  if (cellChanged || creature->IsInCombat() || creature->IsInEvadeMode())
  {
...
{
  // pussywizard: UnloadGrid only done when whole map is unloaded, no need to worry about moving npcs between grids,
  // etc.
  ...
  if (HavePlayers())
  {
    for (MapRefMgr::iterator itr = m_mapRefMgr.begin(); itr != m_mapRefMgr.end(); ++itr)
    {
      ...
      if (!player->IsBeingTeleportedFar())
      {
        ...
        LOG_ERROR("maps", "Map::UnloadAll: player {} is still in map {} during unload, this should not happen!",
...
bool GridMap::loadData(char* filename)
{
  ...
  LOG_ERROR("maps", "Map file '{}' is from an incompatible clientversion. Please recreate using the mapextractor.",
            filename);
  fclose(in);
  return false;
}
...
float GridMap::getMinHeight(float x, float y) const
{
  ...
  static float const boundGridCoords[] = {0.0f,        0.0f, 0.0f,        -266.66666f, 0.0f,        -533.33331f,
  ...
  return G3D::Plane(
             G3D::Vector3(boundGridCoords[indices[quarterIndex + 0] * 2 + 0],
                          boundGridCoords[indices[quarterIndex + 0] * 2 + 1], _minHeight[indices[quarterIndex + 0]]),
             G3D::Vector3(boundGridCoords[indices[quarterIndex + 1] * 2 + 0],
                          boundGridCoords[indices[quarterIndex + 1] * 2 + 1], _minHeight[indices[quarterIndex + 1]]),
             G3D::Vector3(boundGridCoords[indices[quarterIndex + 2] * 2 + 0],
                          boundGridCoords[indices[quarterIndex + 2] * 2 + 1], _minHeight[indices[quarterIndex + 2]]))
...
inline LiquidData const GridMap::GetLiquidData(float x, float y, float z, float collisionHeight,
...
bool Map::GetAreaInfo(uint32 phaseMask, float x, float y, float z, uint32& flags, int32& adtId, int32& rootId,
                      int32& groupId) const
{
  float           vmap_z    = z;
  float           dynamic_z = z;
...
LiquidData const Map::GetLiquidData(uint32 phaseMask, float x, float y, float z, float collisionHeight,
                                    uint8 ReqLiquidType)
{
  LiquidData liquidData;
  ...
  if (vmgr->GetLiquidLevel(GetId(), x, y, z, ReqLiquidType, liquid_level, ground_level, liquid_type, mogpFlags))
  {
    ...
    LOG_DEBUG("maps", "GetLiquidStatus(): vmap liquid level: {} ground: {} type: {}", liquid_level, ground_level,
              liquid_type);
...
{
  LiquidData const& liquidData = const_cast<Map*>(this)->GetLiquidData(phaseMask, x, y, z, collisionHeight,
...
{
  LiquidData const& liquidData = const_cast<Map*>(this)->GetLiquidData(searcher->GetPhaseMask(), x, y, z,
                                                                       searcher->GetCollisionHeight(), MAP_ALL_LIQUIDS);
...
template <class T> void Map::RemoveFromActive(T* obj) { RemoveFromActiveHelper(obj); }

template <> void Map::RemoveFromActive(Creature* c) { RemoveFromActiveHelper(c); }

template <> void Map::RemoveFromActive(DynamicObject* obj) { RemoveFromActiveHelper(obj); }

template <> void Map::RemoveFromActive(GameObject* obj) { RemoveFromActiveHelper(obj); }
...
InstanceMap::InstanceMap(uint32 id, uint32 InstanceId, uint8 SpawnMode, Map* _parent)
    : Map(id, InstanceId, SpawnMode, _parent), m_resetAfterUnload(false), m_unloadWhenEmpty(false),
...
Map::EnterState InstanceMap::CannotEnter(Player* player, bool loginCheck)
{
  ...
  {
    LOG_ERROR("maps", "InstanceMap::CanEnter - player {} ({}) already in map {}, {}, {}!", player->GetName(),
              player->GetGUID().ToString(), GetId(), GetInstanceId(), GetSpawnMode());
  ...
  {
    LOG_DEBUG("maps", "MAP: Instance '{}' of map '{}' cannot have more than '{}' players. Player '{}' rejected",
...
bool InstanceMap::AddPlayerToMap(Player* player)
{
  ...
  if (IsDungeon())
  {
    ...
    if (!mapSave)
    {
      LOG_ERROR("maps", "InstanceMap::Add: InstanceSave does not exist for map {} spawnmode {} with instance id {}",
    ...
    if (playerBind && playerBind->perm)
    {
      ...
      {
        LOG_ERROR("maps",
                  "InstanceMap::Add: player {} ({}) is permanently bound to instance {}, {}, {}, {} but he is being "
                  "put into instance {}, {}, {}, {}",
                  player->GetName(), player->GetGUID().ToString(), playerBind->save->GetMapId(),
                  playerBind->save->GetInstanceId(), playerBind->save->GetDifficulty(), playerBind->save->CanReset(),
                  mapSave->GetMapId(), mapSave->GetInstanceId(), mapSave->GetDifficulty(), mapSave->CanReset());
        return false;
      }
    }
    ...
    else
    {
      ...
      if (Group* g = player->GetGroup())
        if (g->GetLeaderGUID() != player->GetGUID())
          if (!sInstanceSaveMgr->PlayerGetBoundInstance(g->GetLeaderGUID(), mapSave->GetMapId(),
                                                        mapSave->GetDifficulty()))
          {
            sInstanceSaveMgr->PlayerCreateBoundInstancesMaps(g->GetLeaderGUID());
            sInstanceSaveMgr->PlayerBindToInstance(g->GetLeaderGUID(), mapSave, false,
...
{
  // pussywizard: moved m_unloadTimer to InstanceMap::AfterPlayerUnlinkFromMap(), in this function if 2 players run out
  // at the same time the instance won't close
...
bool InstanceMap::Reset(uint8 method, GuidList* globalResetSkipList)
{
  if (method == INSTANCE_RESET_GLOBAL)
  {
    ...
    for (MapRefMgr::iterator itr = m_mapRefMgr.begin(); itr != m_mapRefMgr.end(); ++itr)
    {
      ...
      if (!globalResetSkipList || std::find(globalResetSkipList->begin(), globalResetSkipList->end(),
                                            itr->GetSource()->GetGUID()) == globalResetSkipList->end())
...
void InstanceMap::PermBindAllPlayers()
{
  ...
  {
    LOG_ERROR("maps",
              "Cannot bind players because no instance save is available for instance map (Name: {}, Entry: {}, "
              "InstanceId: {})!",
...
MapDifficulty const* Map::GetMapDifficulty() const { return GetMapDifficultyData(GetId(), GetDifficulty()); }
...
void BattlegroundMap::InitVisibilityDistance()
{
  ...
  if (IsBattleArena()) // pussywizard: start with 30yd visibility range on arenas to ensure players can't get
                       // informations about the opponents in any way
...
DynamicObject* Map::GetDynamicObject(ObjectGuid guid) { return _objectsStore.Find<DynamicObject>(guid); }
...
void Map::UpdateEncounterState(EncounterCreditType type, uint32 creditEntry, Unit* source)
{
  ...
  // 631 : ICC - 724 : Ruby Sanctum --- For heroic difficulties, for some reason, we don't have an encounter list, so we
  // get the encounter list from normal diff. We shouldn't change difficulty_fixed variable.
  if ((GetId() == 631 || GetId() == 724) && IsHeroic())
  {
    encounters = sObjectMgr->GetDungeonEncounterList(GetId(), !Is25ManRaid() ? RAID_DIFFICULTY_10MAN_NORMAL
  ...
  for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
    if (Player* p = itr->GetSource())
    {
      ...
      snprintf(buffer, 16384, "%s (%s, acc: %u, ip: %s, guild: %u), xyz: (%.1f, %.1f, %.1f), auras: %s\n",
               p->GetName().c_str(), p->GetGUID().ToString().c_str(), p->GetSession()->GetAccountId(),
               p->GetSession()->GetRemoteAddress().c_str(), p->GetGuildId(), p->GetPositionX(), p->GetPositionY(),
               p->GetPositionZ(), auraStr.c_str());
      playersInfo += buffer;
    }
  CleanStringForMysqlQuery(playersInfo);
  CharacterDatabase.Execute("INSERT INTO log_encounter VALUES(NOW(), {}, {}, {}, {}, '{}')", GetId(),
                            (uint32)GetDifficulty(), type, creditEntry, playersInfo);
}
...
bool Map::CanReachPositionAndGetValidCoords(WorldObject const* source, float& destX, float& destY, float& destZ,
                                            bool failOnCollision, bool failOnSlopes) const
{
  return CanReachPositionAndGetValidCoords(source, source->GetPositionX(), source->GetPositionY(),
                                           source->GetPositionZ(), destX, destY, destZ, failOnCollision, failOnSlopes);
}
                                          ...
                                          float& destX, float& destY, float& destZ, bool failOnCollision) const
{
  ...
  {
    LOG_FATAL("maps",
              "Map::CheckCollisionAndGetValidCoords invalid coordinates startX: {}, startY: {}, startZ: {}, destX: {}, "
              "destY: {}, destZ: {}",
  ...
  if (notOnGround)
  {
    bool col = VMAP::VMapFactory::createOrGetVMapMgr()->GetObjectHitPos(
        source->GetMapId(), startX, startY, startZ + halfHeight, destX, destY, destZ + halfHeight, destX, destY, destZ,
        -CONTACT_DISTANCE);
  ...
  bool col = source->GetMap()->GetObjectHitPos(source->GetPhaseMask(), startX, startY, startZ + halfHeight, destX,
                                               destY, destZ + halfHeight, destX, destY, destZ, -CONTACT_DISTANCE);
...
void Map::LoadCorpseData()
{
  ...
  //        14          15         16
  // SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1, bytes2, guildId, flags, dynFlags, time,
  // corpseType, instanceId, phaseMask, guid FROM corpse WHERE mapId = ? AND instanceId = ?
