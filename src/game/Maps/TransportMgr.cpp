...
void TransportMgr::GeneratePath(GameObjectTemplate const* goInfo, TransportTemplate* transport)
{
  ...
  if (splinePath.size() >= 2)
  {
    ...
    // if (!keyFrames.front().IsStopFrame() && !keyFrames.front().Node->arrivalEventID &&
    // !keyFrames.front().Node->departureEventID)
    {
      splinePath.erase(splinePath.begin());
      keyFrames.erase(keyFrames.begin());
    }
    // if (!keyFrames.back().IsStopFrame() && !keyFrames.back().Node->arrivalEventID &&
...
void TransportMgr::SpawnContinentTransports()
{
  ...
  if (sWorld->getBoolConfig(CONFIG_ENABLE_CONTINENT_TRANSPORT))
  {
    ...
    LOG_INFO("server.loading", ">> Spawned {} continent motion transports in {} ms", count,
  ...
  if (sWorld->getBoolConfig(CONFIG_ENABLE_CONTINENT_TRANSPORT_PRELOADING))
  {
    ...
    QueryResult result2 = WorldDatabase.Query("SELECT map, position_x, position_y FROM gameobject g JOIN "
    ...
    LOG_INFO("server.loading", ">> Preloaded grids for {} continent static transports in {} ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  }
}
...
bool TransportAnimation::GetAnimNode(uint32 time, TransportAnimationEntry const*& curr,
                                     TransportAnimationEntry const*& next, float& percPos) const
{
  if (Path.empty())
    return false;
...
void TransportAnimation::GetAnimRotation(uint32 time, G3D::Quat& curr, G3D::Quat& next, float& percRot) const
{
  ...
  for (TransportPathRotationContainer::const_reverse_iterator itr = Rotations.rbegin(); itr != Rotations.rend(); ++itr)
    if (time >= itr->first)
    {
      ...
      {
        next    = G3D::Quat(Rotations.begin()->second->X, Rotations.begin()->second->Y, Rotations.begin()->second->Z,
