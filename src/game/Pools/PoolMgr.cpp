...
// Same on one pool
template <> void PoolGroup<Pool>::Despawn1Object(uint32 child_pool_id) { sPoolMgr->DespawnPool(child_pool_id); }
...
template <class T> void PoolGroup<T>::SpawnObject(ActivePoolData& spawns, uint32 limit, uint32 triggerFrom)
{
  ...
  if (count > 0)
  {
    ...
    {
      std::copy_if(EqualChanced.begin(), EqualChanced.end(), std::back_inserter(rolledObjects),
...
// Same for 1 pool
template <> void PoolGroup<Pool>::Spawn1Object(PoolObject* obj) { sPoolMgr->SpawnPool(obj->guid); }
...
void PoolMgr::LoadFromDB()
{
  ...
  LOG_INFO("server.loading", "Loading Gameobjects Pooling Data...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          LOG_ERROR("sql.sql",
                    "`pool_gameobject` has a not lootable gameobject spawn (GUID: {}, type: {}) defined for pool id "
                    "({}), skipped.",
    ...
    else
    {
      ...
      do
      {
        ...
        }
        {
          ...
          if (it == mPoolTemplate.end())
          {
            LOG_ERROR("sql.sql", "`pool_pool` included pool_id ({}) is not in `pool_template`, skipped.",
                      child_pool_id);
            continue;
          }
        }
        ...
        {
          LOG_ERROR("sql.sql",
                    "`pool_pool` has an invalid chance ({}) for pool id ({}) in mother pool id ({}), skipped.", chance,
                    child_pool_id, mother_pool_id);
          continue;
        }
  ...
  LOG_INFO("server.loading", "Loading Quest Pooling Data...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          LOG_ERROR("sql.sql",
                    "`pool_quest` has an quest ({}) which is not daily or weekly in pool id ({}), use ExclusiveGroup "
                    "instead, skipped.",
                    entry, pool_id);
          continue;
        }
        ...
        if (poolTypeMap[pool_id] != currType)
        {
          LOG_ERROR("sql.sql",
                    ...
                    entry, currType == QUEST_DAILY ? "QUEST_DAILY" : "QUEST_WEEKLY", pool_id,
        ...
        {
          LOG_ERROR("sql.sql",
                    "`pool_quest` lists entry ({}) as member of pool ({}) but is not started anywhere, skipped.", entry,
                    pool_id);
          continue;
        }
  ...
  // The initialize method will spawn all pools not in an event and not in another pool, this is why there is 2 left
  // joins with 2 null checks
  LOG_INFO("server.loading", "Starting Objects Pooling System...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          if (pool_pool_id)
            // The pool is a child pool in pool_pool table. Ideally we should remove it from the pool handler to ensure
            // it never gets spawned, however that could recursively invalidate entire chain of mother pools. It can be
            // done in the future but for now we'll do nothing.
            LOG_ERROR("sql.sql",
                      "Pool Id {} has no equal chance pooled entites defined and explicit chance sum is not 100. This "
                      "broken pool is a child pool of Id {} and cannot be safely removed.",
                      pool_entry, fields[2].Get<uint32>());
          else
            LOG_ERROR("sql.sql",
                      "Pool Id {} has no equal chance pooled entites defined and explicit chance sum is not 100. The "
                      "pool will not be spawned.",
                      pool_entry);
          continue;
        }
      ...
      LOG_INFO("pool", "Pool handling system initialized, {} pools spawned in {} ms", count,
