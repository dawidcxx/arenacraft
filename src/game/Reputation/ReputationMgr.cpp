...
bool ReputationMgr::IsAtWar(uint32 faction_id) const
{
  ...
  if (!factionEntry)
  {
    LOG_ERROR("reputation", "ReputationMgr::IsAtWar: Can't get AtWar flag of {} for unknown faction (faction id) #{}.",
              _player->GetName(), faction_id);
    return 0;
  }
...
int32 ReputationMgr::GetBaseReputation(FactionEntry const* factionEntry) const
{
  ...
  // in faction.dbc exist factions with (RepListId >=0, listed in character reputation list) with all BaseRepRaceMask[i]
  // == 0
  return 0;
}
...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const* factionEntry, float stand, bool incremental,
                                            Optional<ReputationRank> repMaxCap)
{
...
void ReputationMgr::LoadFromDB(PreparedQueryResult result)
{
  ...
  // QueryResult* result = CharacterDatabase.Query("SELECT faction, standing, flags FROM character_reputation WHERE guid
  // = '{}'", GetGUID().GetCounter());
