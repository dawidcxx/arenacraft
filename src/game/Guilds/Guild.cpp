...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode,
                              std::string_view param)
{
  ...
  LOG_DEBUG("guild", "SMSG_GUILD_COMMAND_RESULT [{}]: Type: {}, code: {}, param: {}", session->GetPlayerInfo(), type,
            errCode, resultPacket.Name);
}
...
Guild::LogHolder<Entry>::LogHolder()
    : m_maxRecords(sWorld->getIntConfig(std::is_same_v<Entry, BankEventLogEntry> ? CONFIG_GUILD_BANK_EVENT_LOG_COUNT
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(uint8 tabs, CharacterDatabaseTransaction trans,
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
  ...
  {
    LOG_ERROR("guild", "Item (GUID {}, id: {}) not found in item_instance, deleting from guild bank!", itemGuid,
              itemEntry);
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const
{
  ...
  if (session)
  {
    LOG_DEBUG("guild", "MSG_QUERY_GUILD_BANK_TEXT [{}]: Tabid: {}, Text: {}", session->GetPlayerInfo(), m_tabId,
              m_text);
...
void Guild::Member::SetStats(std::string_view name, uint8 level, uint8 _class, uint8 gender, uint32 zoneId,
                             uint32 accountId)
{
  m_name      = name;
  m_level     = level;
...
void Guild::Member::UpdateLogoutTime() { m_logoutTime = GameTime::GetGameTime().count(); }
...
bool Guild::Member::CheckStats() const
{
  if (m_level < 1)
  {
    LOG_ERROR("guild", "Player ({}) has a broken data in field `characters`.`level`, deleting him from guild!",
              m_guid.ToString());
    return false;
  }
  ...
  {
    LOG_ERROR("guild", "Player ({}) has a broken data in field `characters`.`class`, deleting him from guild!",
              m_guid.ToString());
    return false;
  }
  return true;
...
void Guild::MoveItemData::LogAction(MoveItemData* pFrom) const
{
  ...
  sScriptMgr->OnGuildItemMove(m_pGuild, m_pPlayer, pFrom->GetItem(), pFrom->IsBank(), pFrom->GetContainer(),
                              pFrom->GetSlotId(), IsBank(), GetContainer(), GetSlotId());
}
...
void Guild::PlayerMoveItemData::RemoveItem(CharacterDatabaseTransaction trans, MoveItemData* /*pOther*/,
...
void Guild::PlayerMoveItemData::LogBankEvent(CharacterDatabaseTransaction trans, MoveItemData* pFrom,
                                             uint32 count) const
{
  ASSERT(pFrom);
  // Bank -> Char
...
Item* Guild::BankMoveItemData::StoreItem(CharacterDatabaseTransaction trans, Item* pItem)
{
  ...
  for (ItemPosCountVec::const_iterator itr = m_vec.begin(); itr != m_vec.end();)
  {
    ...
    LOG_DEBUG("guild", "GUILD STORAGE: StoreItem tab = {}, slot = {}, item = {}, count = {}", m_container, m_slotId,
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const { MoveItemData::LogAction(pFrom); }

Item* Guild::BankMoveItemData::_StoreItem(CharacterDatabaseTransaction trans, BankTab* pTab, Item* pItem,
...
{
  LOG_DEBUG("guild", "GUILD STORAGE: CanStore() tab = {}, slot = {}, item = {}, count = {}", m_container, m_slotId,
            pItem->GetEntry(), pItem->GetCount());
...
bool Guild::Create(Player* pLeader, std::string_view name)
{
  ...
  LOG_DEBUG("guild", "GUILD: creating guild [{}] for leader {} ({})", m_name, pLeader->GetName(),
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name)
{
  ...
  if (memberLimit > 0 && player->GetGuild()->GetMemberCount() >= memberLimit)
  {
    ChatHandler(player->GetSession())
        .PSendSysMessage("Your guild has reached the maximum amount of members ({}). You cannot send another invite "
...
void Guild::HandleUpdateMemberRank(WorldSession* session, std::string_view name, bool demote)
{
  ...
  else if (Member* member = GetMember(name))
  {
    ...
    _LogEvent(demote ? GUILD_EVENT_LOG_DEMOTE_PLAYER : GUILD_EVENT_LOG_PROMOTE_PLAYER, player->GetGUID(),
              member->GetGUID(), newRankId);
    _BroadcastEvent(demote ? GE_DEMOTION : GE_PROMOTION, ObjectGuid::Empty, player->GetName(), member->GetName(),
...
void Guild::HandleRemoveLowestRank(WorldSession* session) { HandleRemoveRank(session, _GetLowestRankId()); }
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
  ...
  if (amount > 10 * GOLD) // receiver_acc = Guild id, receiver_name = Guild name
    CharacterDatabase.Execute("INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", {}, \"(guild members: "
                              "{}, new amount: {}, leader guid low: {}, sender level: {})\", NOW(), {})",
                              session->GetAccountId(), player->GetGUID().GetCounter(), player->GetName(),
                              session->GetRemoteAddress(), GetId(), GetName(), amount, GetMemberCount(),
                              GetTotalBankMoney(), GetLeaderGUID().GetCounter(), player->GetLevel(), 3);
}
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint32 amount, bool repair)
{
  ...
  _LogBankEvent(trans, repair ? GUILD_BANK_LOG_REPAIR_MONEY : GUILD_BANK_LOG_WITHDRAW_MONEY, uint8(0),
                player->GetGUID(), amount);
  CharacterDatabase.CommitTransaction(trans);

  if (amount > 10 * GOLD) // sender_acc = 0 (guild has no account), sender_guid = Guild id, sender_name = Guild name
    CharacterDatabase.Execute("INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", {}, \"(guild, members: "
                              "{}, new amount: {}, leader guid low: {}, withdrawer level: {})\", NOW(), {})",
                              0, GetId(), GetName(), session->GetRemoteAddress(), session->GetAccountId(),
                              player->GetName(), amount, GetMemberCount(), GetTotalBankMoney(),
                              GetLeaderGUID().GetCounter(), player->GetLevel(), 4);
  ...
  if (dbTabId < _GetPurchasedTabsSize() || isMoneyTab)
  {
    ...
    if (bankLog.CanInsert())
    {
      ...
      if (BankEventLogEntry::IsMoneyEvent(eventType))
      {
        ...
        {
          LOG_ERROR("guild",
                    "GuildBankEventLog ERROR: MoneyEvent(LogGuid: {}, Guild: {}) does not belong to money tab ({}), "
                    "ignoring...",
                    guid, m_id, dbTabId);
          return false;
        }
...
bool Guild::LoadBankItemFromDB(Field* fields)
{
  ...
  if (tabId >= _GetPurchasedTabsSize())
  {
    LOG_ERROR("guild", "Invalid tab for item (GUID: {}, id: #{}) in guild bank, skipped.", fields[14].Get<uint32>(),
...
void Guild::BroadcastToGuild(WorldSession* session, bool officerOnly, std::string_view msg, uint32 language) const
{
  ...
  {
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, officerOnly ? CHAT_MSG_OFFICER : CHAT_MSG_GUILD, Language(language),
                                 session->GetPlayer(), nullptr, msg);
    for (auto const& [guid, member] : m_members)
...
bool Guild::AddMember(ObjectGuid guid, uint8 rankId)
{
  ...
  else
  {
    ...
    if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
    {
      ...
      member.SetStats(name, fields[1].Get<uint8>(), fields[2].Get<uint8>(), fields[3].Get<uint8>(),
                      fields[4].Get<uint16>(), fields[5].Get<uint32>());
...
// Bank (items move)
void Guild::SwapItems(Player* player, uint8 tabId, uint8 slotId, uint8 destTabId, uint8 destSlotId,
                      uint32 splitedAmount)
{
...
void Guild::SwapItemsWithInventory(Player* player, bool toChar, uint8 tabId, uint8 slotId, uint8 playerBag,
...
inline void Guild::_LogEvent(GuildEventLogTypes eventType, ObjectGuid playerGuid1, ObjectGuid playerGuid2,
                             uint8 newRank)
{
  CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
                          ...
                          ObjectGuid guid, uint32 itemOrMoney, uint16 itemStackCount, uint8 destTabId)
{
  ...
  sScriptMgr->OnGuildBankEvent(this, uint8(eventType), tabId, guid.GetCounter(), itemOrMoney, itemStackCount,
                               destTabId);
}
...
void Guild::_SendBankMoneyUpdate(WorldSession* session) const { _SendBankList(session); }
...
void Guild::_SendBankContentUpdate(uint8 tabId, SlotIds slots) const { _SendBankList(nullptr, tabId, false, &slots); }
                          ...
                          SlotIds* slots /*= nullptr*/) const
{
  ...
  if (BankTab const* tab = GetBankTab(tabId))
  {
    ...
    if (sendAllSlots)
      fillItems(boost::make_counting_iterator(uint8(0)), boost::make_counting_iterator(uint8(GUILD_BANK_MAX_SLOTS)),
                true);
  ...
  if (session)
  {
    ...
    session->SendPacket(packet.Write());
    LOG_DEBUG("guild", "SMSG_GUILD_BANK_LIST [{}]: TabId: {}, FullSlots: {}, slots: {}", session->GetPlayerInfo(),
              tabId, sendAllSlots, packet.WithdrawalsRemaining);
  }
  else
  {
    packet.Write();
    for (auto const& [guid, member] : m_members)
    {
      ...
      LOG_DEBUG("guild", "SMSG_GUILD_BANK_LIST [{}]: TabId: {}, FullSlots: {}, slots: {}", player->GetName(), tabId,
                sendAllSlots, packet.WithdrawalsRemaining);
    }
  }
}
