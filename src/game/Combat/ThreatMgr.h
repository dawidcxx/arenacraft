...
#define THREAT_UPDATE_INTERVAL                                                                                         \
  2 * IN_MILLISECONDS // Server should send threat update to client periodically
                      // each second
...
{
  static float calcThreat(Unit* hatedUnit, float threat, SpellSchoolMask schoolMask = SPELL_SCHOOL_MASK_NORMAL,
...
public:
  ...
  [[nodiscard]] HostileReference* getMostHated() const { return iThreatList.empty() ? nullptr : iThreatList.front(); }
...
public:
  ...
  // methods to access the lists from the outside to do some dirty manipulation
  // (scriping and such) I hope they are used as little as possible.
  [[nodiscard]] ThreatContainer::StorageType const& GetThreatList() const { return iThreatContainer.GetThreatList(); }
  [[nodiscard]] ThreatContainer::StorageType const& GetOfflineThreatList() const
  {
