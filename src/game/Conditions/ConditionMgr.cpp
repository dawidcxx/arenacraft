...
bool Condition::Meets(ConditionSourceInfo& sourceInfo)
{
  ...
  {
    LOG_ERROR("condition",
              "ConditionTarget {} for for condition (Entry: {} Type: {} Group: {}) is greater or equal than "
              "MAX_CONDITION_TARGETS",
              ConditionTarget, SourceEntry, SourceType, SourceGroup);
    return false;
  }
  ...
  if (!object)
  {
    LOG_DEBUG("condition", "Condition object not found for condition (Entry: {} Type: {} Group: {})", SourceEntry,
  ...
  {
    if (Unit* unit = object->ToUnit())
      condMeets = CompareValues(static_cast<ComparisionType>(ConditionValue2), static_cast<uint32>(unit->GetLevel()),
                                ConditionValue1);
    break;
  }
  ...
  case CONDITION_NEAR_CREATURE:
  {
    condMeets = static_cast<bool>(
        GetClosestCreatureWithEntry(object, ConditionValue1, static_cast<float>(ConditionValue2), !ConditionValue3));
    break;
  }
  ...
  {
    if (WorldObject* toObject = sourceInfo.mConditionTargets[ConditionValue1])
      condMeets = CompareValues(static_cast<ComparisionType>(ConditionValue3), object->GetDistance(toObject),
  ...
  {
    if (Unit* unit = object->ToUnit())
      condMeets = CompareValues(static_cast<ComparisionType>(ConditionValue2), unit->GetHealth(),
  ...
  {
    if (Unit* unit = object->ToUnit())
      condMeets = CompareValues(static_cast<ComparisionType>(ConditionValue2), unit->GetHealthPct(),
  ...
  // bool script = sScriptMgr->OnConditionCheck(this, sourceInfo); // Returns true by default. // pussywizard:
  // optimization
...
bool ConditionMgr::IsObjectMeetToConditionList(ConditionSourceInfo& sourceInfo, ConditionList const& conditions)
{
  ...
  for (ConditionList::const_iterator i = conditions.begin(); i != conditions.end(); ++i)
  {
    LOG_DEBUG("condition", "ConditionMgr::IsPlayerMeetToConditionList condType: {} val1: {}", (*i)->ConditionType,
              (*i)->ConditionValue1);
    if ((*i)->isLoaded())
    {
...
bool ConditionMgr::CanHaveSourceGroupSet(ConditionSourceType sourceType) const
{
  return (sourceType == CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE ||
          ...
          sourceType == CONDITION_SOURCE_TYPE_SPELL_LOOT_TEMPLATE || sourceType == CONDITION_SOURCE_TYPE_GOSSIP_MENU ||
          sourceType == CONDITION_SOURCE_TYPE_GOSSIP_MENU_OPTION || sourceType == CONDITION_SOURCE_TYPE_VEHICLE_SPELL ||
...
ConditionList ConditionMgr::GetConditionsForNotGroupedEntry(ConditionSourceType sourceType, uint32 entry)
{
  ...
  if (sourceType > CONDITION_SOURCE_TYPE_NONE && sourceType < CONDITION_SOURCE_TYPE_MAX)
  {
    ...
    if (itr != ConditionStore.end())
    {
      ...
      {
        spellCond = (*i).second;
        LOG_DEBUG("condition", "GetConditionsForNotGroupedEntry: found conditions for type {} and entry {}",
...
ConditionList ConditionMgr::GetConditionsForSpellClickEvent(uint32 creatureId, uint32 spellId)
{
  ...
  if (itr != SpellClickEventConditionStore.end())
  {
    ...
    {
      cond = (*i).second;
      LOG_DEBUG("condition", "GetConditionsForSpellClickEvent: found conditions for Vehicle entry {} spell {}",
                creatureId, spellId);
    }
  }
  return cond;
...
ConditionList ConditionMgr::GetConditionsForVehicleSpell(uint32 creatureId, uint32 spellId)
{
  ...
  if (itr != VehicleSpellConditionStore.end())
  {
    ...
    {
      cond = (*i).second;
      LOG_DEBUG("condition", "GetConditionsForVehicleSpell: found conditions for Vehicle entry {} spell {}", creatureId,
                spellId);
    }
  }
  return cond;
...
ConditionList ConditionMgr::GetConditionsForNpcVendorEvent(uint32 creatureId, uint32 itemId)
{
  ...
  if (itr != NpcVendorConditionContainerStore.end())
  {
    ...
    if (i != (*itr).second.end())
    {
      ...
      if (itemId)
      {
        LOG_DEBUG("condition", "GetConditionsForNpcVendorEvent: found conditions for creature entry {} item {}",
                  creatureId, itemId);
      }
      else
      {
...
void ConditionMgr::LoadConditions(bool isReload)
{
  ...
  QueryResult result = WorldDatabase.Query("SELECT SourceTypeOrReferenceId, SourceGroup, SourceEntry, SourceId, "
  ...
  do
  {
    ...
    if (iConditionTypeOrReference < 0) // it has a reference
    {
      ...
      if (cond->ConditionTarget)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in ConditionTarget ({})!", rowType,
                  iSourceTypeOrReferenceId, cond->ConditionTarget);
      if (cond->ConditionValue1)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in value1 ({})!", rowType, iSourceTypeOrReferenceId,
                  cond->ConditionValue1);
      if (cond->ConditionValue2)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in value2 ({})!", rowType, iSourceTypeOrReferenceId,
                  cond->ConditionValue2);
      if (cond->ConditionValue3)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in value3 ({})!", rowType, iSourceTypeOrReferenceId,
                  cond->ConditionValue3);
      if (cond->NegativeCondition)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in NegativeCondition ({})!", rowType,
                  iSourceTypeOrReferenceId, cond->NegativeCondition);
      if (cond->SourceGroup && iSourceTypeOrReferenceId < 0)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in SourceGroup ({})!", rowType, iSourceTypeOrReferenceId,
                  cond->SourceGroup);
      if (cond->SourceEntry && iSourceTypeOrReferenceId < 0)
        LOG_ERROR("sql.sql", "Condition {} {} has useless data in SourceEntry ({})!", rowType, iSourceTypeOrReferenceId,
                  cond->SourceEntry);
    }
    ...
    {
      LOG_ERROR("sql.sql", "Condition type {} has not allowed value of SourceGroup = {}!", uint32(cond->SourceType),
                cond->SourceGroup);
      delete cond;
      continue;
    }
    ...
    {
      LOG_ERROR("sql.sql", "Condition type {} has not allowed value of SourceId = {}!", uint32(cond->SourceType),
                cond->SourceId);
      delete cond;
      continue;
    }
    ...
    {
      LOG_ERROR("condition", "Condition type {} entry {} can't have ErrorType ({}), set to 0!",
    ...
    {
      LOG_ERROR("condition", "Condition type {} entry {} has any ErrorType, ErrorTextId ({}) is set, set to 0!",
...
bool ConditionMgr::isSourceTypeValid(Condition* cond)
{
  ...
  {
    LOG_ERROR("sql.sql", "ConditionSourceType {} in `condition` table is not supported on 3.3.5a, ignoring.",
  ...
  case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `creature_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
  ...
  case CONDITION_SOURCE_TYPE_FISHING_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `fishing_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
  ...
  case CONDITION_SOURCE_TYPE_ITEM_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `item_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
  ...
  case CONDITION_SOURCE_TYPE_MAIL_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `mail_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
  ...
  case CONDITION_SOURCE_TYPE_MILLING_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `milling_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
  ...
  case CONDITION_SOURCE_TYPE_SKINNING_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `skinning_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
  ...
  case CONDITION_SOURCE_TYPE_SPELL_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `spell_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
  ...
  case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
  {
    ...
    if (!spellInfo)
    {
      LOG_ERROR("sql.sql", "SourceEntry {} in `condition` table, does not exist in `spell.dbc`, ignoring.",
                cond->SourceEntry);
      return false;
    }
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
      ...
      LOG_ERROR("sql.sql",
                "SourceEntry {} SourceGroup {} in `condition` table - spell {} does not have implicit targets of "
                "types: _AREA_, _CONE_, _NEARBY_ for effect {}, SourceGroup needs correction, ignoring.",
  ...
  case CONDITION_SOURCE_TYPE_CREATURE_TEMPLATE_VEHICLE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceEntry {} in `condition` table, does not exist in `creature_template`, ignoring.",
                cond->SourceEntry);
      return false;
    }
    break;
  ...
  case CONDITION_SOURCE_TYPE_QUEST_AVAILABLE:
    ...
    {
      LOG_ERROR("sql.sql", "CONDITION_SOURCE_TYPE_QUEST_AVAILABLE specifies non-existing quest ({}), skipped",
                cond->SourceEntry);
      return false;
    }
    break;
  case CONDITION_SOURCE_TYPE_UNUSED_20:
    LOG_ERROR("sql.sql", "CONDITION_SOURCE_TYPE_UNUSED_20 is not in use. SourceEntry = ({}), skipped",
              cond->SourceEntry);
    break;
  case CONDITION_SOURCE_TYPE_VEHICLE_SPELL:
  case CONDITION_SOURCE_TYPE_SPELL_CLICK_EVENT:
  ...
  case CONDITION_SOURCE_TYPE_NPC_VENDOR:
  {
    if (!sObjectMgr->GetCreatureTemplate(cond->SourceGroup))
    {
      LOG_ERROR("condition", "SourceEntry {} in `condition` table, does not exist in `creature_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
    if (cond->SourceEntry)
    {
      ...
      if (!itemTemplate)
      {
        LOG_ERROR("condition", "SourceEntry {} in `condition` table, does not exist in `item_template`, ignoring.",
                  cond->SourceEntry);
        return false;
      }
    }
  ...
  case CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "SourceGroup {} in `condition` table, does not exist in `player_loot_template`, ignoring.",
                cond->SourceGroup);
      return false;
    }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
  ...
  case CONDITION_AURA:
  {
    ...
    {
      LOG_ERROR("sql.sql", "Aura condition has non existing effect index ({}) (must be 0..2), skipped",
                cond->ConditionValue2);
      return false;
    }
    if (cond->ConditionValue3)
  ...
  case CONDITION_ZONEID:
  {
    ...
    if (areaEntry->zone != 0)
    {
      LOG_ERROR("sql.sql", "ZoneID condition requires to be in area ({}) which is a subzone but zone expected, skipped",
                cond->ConditionValue1);
      return false;
    }
  ...
  case CONDITION_QUESTSTATE:
    ...
    {
      LOG_ERROR("sql.sql", "ConditionType ({}) has invalid state mask ({}), skipped.", cond->ConditionType,
                cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_CLASS:
  {
    ...
    {
      LOG_ERROR("sql.sql", "Class condition has non existing classmask ({}), skipped",
  ...
  case CONDITION_RACE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "Race condition has non existing racemask ({}), skipped",
  ...
  case CONDITION_NEAR_CREATURE:
  {
    ...
    {
      LOG_ERROR("sql.sql", "NearCreature condition has non existing creature template entry ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_NEAR_GAMEOBJECT:
  {
    ...
    {
      LOG_ERROR("sql.sql", "NearGameObject condition has non existing gameobject template entry ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    if (cond->ConditionValue3 > 2)
      LOG_ERROR("sql.sql", "NearGameObject condition for gameobject ID ({}) has data over 2 for value3 ({})!",
                cond->ConditionValue1, cond->ConditionValue3);
    break;
  }
  case CONDITION_OBJECT_ENTRY_GUID:
  {
    ...
    case TYPEID_UNIT:
      ...
      {
        LOG_ERROR("sql.sql", "ObjectEntryGuid condition has non existing creature template entry ({}), skipped",
                  cond->ConditionValue2);
        return false;
      }
      if (cond->ConditionValue3 > 1)
      {
        ...
        {
          LOG_ERROR("sql.sql", "ObjectEntryGuid condition has non existing creature guid ({}), skipped",
                    cond->ConditionValue3);
          return false;
        }
      }
    ...
    case TYPEID_GAMEOBJECT:
      ...
      {
        LOG_ERROR("sql.sql", "ObjectEntryGuid condition has non existing gameobject template entry ({}), skipped",
                  cond->ConditionValue2);
        return false;
      }
      if (cond->ConditionValue3)
      {
        ...
        {
          LOG_ERROR("sql.sql", "ObjectEntryGuid condition has non existing gameobject guid ({}), skipped",
                    cond->ConditionValue3);
          return false;
        }
      }
  ...
  case CONDITION_RELATION_TO:
  {
    ...
    {
      LOG_ERROR("sql.sql", "RelationTo condition has invalid ConditionValue1(ConditionTarget selection) ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql", "RelationTo condition has invalid ConditionValue2(RelationType) ({}), skipped",
                cond->ConditionValue2);
      return false;
    }
    if (cond->ConditionValue3)
  ...
  case CONDITION_REACTION_TO:
  {
    if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets())
    {
      LOG_ERROR("sql.sql", "ReactionTo condition has invalid ConditionValue1(ConditionTarget selection) ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    if (cond->ConditionValue1 == cond->ConditionTarget)
    ...
    if (!cond->ConditionValue2)
    {
      LOG_ERROR("sql.sql", "mConditionValue2 condition has invalid ConditionValue2(rankMask) ({}), skipped",
                cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_DISTANCE_TO:
  {
    if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets())
    {
      LOG_ERROR("sql.sql", "DistanceTo condition has invalid ConditionValue1(ConditionTarget selection) ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    if (cond->ConditionValue1 == cond->ConditionTarget)
  ...
  case CONDITION_SPAWNMASK:
  {
    ...
    {
      LOG_ERROR("condition", "SpawnMask condition has non existing SpawnMask in value1 ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_UNIT_STATE:
  {
    ...
    {
      LOG_ERROR("condition", "UnitState condition has non existing UnitState in value1 ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_CREATURE_TYPE:
  {
    ...
    {
      LOG_ERROR("condition", "CreatureType condition has non existing CreatureType in value1 ({}), skipped",
                cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_REALM_ACHIEVEMENT:
  {
    ...
    if (!achievement)
    {
      LOG_ERROR("condition", "CONDITION_REALM_ACHIEVEMENT has non existing realm first achivement id ({}), skipped.",
                cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_QUEST_OBJECTIVE_PROGRESS:
  {
    ...
    if (!quest)
    {
      LOG_ERROR("sql.sql", "CONDITION_QUEST_OBJECTIVE_PROGRESS points to non-existing quest ({}), skipped.",
                cond->ConditionValue1);
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql",
                "CONDITION_QUEST_OBJECTIVE_PROGRESS has out-of-range quest objective index specified ({}), it must be "
                "a number between 0 and 3. skipped.",
                cond->ConditionValue2);
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql",
                "CONDITION_QUEST_OBJECTIVE_PROGRESS has quest objective {} for quest {}, but the field "
                "RequiredNPCOrGo{} is 0, skipped.",
                cond->ConditionValue2, cond->ConditionValue1, cond->ConditionValue2);
      return false;
    }
    ...
    {
      LOG_ERROR("sql.sql",
                "CONDITION_QUEST_OBJECTIVE_PROGRESS has quest objective count {} in value3, but quest {} has a maximum "
                "objective count of {} in RequiredNPCOrGOCount{}, skipped.",
                cond->ConditionValue3, cond->ConditionValue2, quest->RequiredNpcOrGoCount[cond->ConditionValue2],
                cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_STAND_STATE:
  {
    ...
    if (!valid)
    {
      LOG_ERROR("sql.sql", "CONDITION_STAND_STATE has non-existing stand state ({},{}), skipped.",
                cond->ConditionValue1, cond->ConditionValue2);
      return false;
    }
    break;
  }
  case CONDITION_DIFFICULTY_ID:
    ...
    {
      LOG_ERROR("sql.sql", "CONDITION_DIFFICULTY_ID has non existing difficulty in value1 ({}), skipped.",
                cond->ConditionValue1);
      return false;
    }
    break;
