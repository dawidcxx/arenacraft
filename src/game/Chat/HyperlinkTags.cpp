...
bool Acore::Hyperlinks::LinkTags::item::StoreTo(ItemLinkData& val, std::string_view text)
{
  ...
  // however, there is also a client bug in inspect packet handling that causes a int16 to be cast to uint16, then int32
  // (dropping sign extension along the way) this results in the wrong value being sent in the link; DBC lookup
  // clientside fails, so it sends the link without suffix to detect and allow these invalid links, we first read
