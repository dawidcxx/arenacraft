...
template <typename Tuple, typename NextType, std::size_t offset> struct MultiConsumer
{
  ...
  static ChatCommandResult TryConsumeTo(Tuple& tuple, ChatHandler const* handler, std::string_view args)
  {
    ...
    if (result1.HasErrorMessage() && result2.HasErrorMessage())
    {
      return Acore::StringFormat("{} \"{}\"\n{} \"{}\"", GetAcoreString(handler, LANG_CMDPARSER_EITHER),
                                 result2.GetErrorMessage(), GetAcoreString(handler, LANG_CMDPARSER_OR),
...
template <typename Tuple, std::size_t offset>
ChatCommandResult ConsumeFromOffset([[maybe_unused]] Tuple& tuple, [[maybe_unused]] ChatHandler const* handler,
...
template <typename T> struct HandlerToTuple
{
  static_assert(Acore::dependant_false_v<T>, "Invalid command handler signature");
};
template <typename... Ts> struct HandlerToTuple<bool(ChatHandler*, Ts...)>
{
...
struct ChatCommandBuilder
{
  ...
  ChatCommandBuilder(char const* name, TypedHandler& handler, AcoreStrings help, uint32 securityLevel,
                     Acore::ChatCommands::Console allowConsole)
  ...
  template <typename TypedHandler>
  ChatCommandBuilder(char const* name, TypedHandler& handler, uint32 securityLevel,
                     Acore::ChatCommands::Console allowConsole)
...
using ChatCommand [[deprecated("std::vector<ChatCommand> should be ChatCommandTable! (using "
                               "namespace Acore::ChatCommands)")]] = Acore::ChatCommands::ChatCommandBuilder;
