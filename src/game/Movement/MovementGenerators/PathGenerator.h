...
enum PathType
{
  ...
  PATHFIND_FARFROMPOLY       = PATHFIND_FARFROMPOLY_START | PATHFIND_FARFROMPOLY_END, // start or end positions are
                                                                                      // far from the mmap poligon
};
...
public:
  ...
  [[nodiscard]] static bool  IsWalkableClimb(float x, float y, float z, float destX, float destY, float destZ,
                                             float sourceHeight);
  ...
  [[nodiscard]] bool         IsSwimmableSegment(float x, float y, float z, float destX, float destY, float destZ,
                                                bool checkSwim = true) const;
  [[nodiscard]] static float GetRequiredHeightToClimb(float x, float y, float z, float destX, float destY, float destZ,
                                                      float sourceHeight);
  ...
  // shortens the path until the destination is the specified distance from the
  // target point
...
private:
  ...
  G3D::Vector3 _actualEndPosition; // {x, y, z} of the closest possible point to
                                   // given destination
  ...
  dtPolyRef          GetPathPolyByPosition(dtPolyRef const* polyPath, uint32 polyPathSize, float const* Point,
  ...
  dtStatus FindSmoothPath(float const* startPos, float const* endPos, dtPolyRef const* polyPath, uint32 polyPathSize,
