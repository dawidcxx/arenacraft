...
// Blade's Edge Arena Ropes normalization
namespace
{
constexpr uint32 MAP_BLADES_EDGE_ARENA = 562;
...
static const std::array<BladeEdgeArenaRope, 2> BladeEdgeArenaRopes = {{
    {{6243.1523f, 267.53094f, 10.929295f}, {6245.9717f, 271.29346f, 10.879172f}, 0.43f},
    {{6234.3213f, 256.29733f, 11.002348f}, {6231.3247f, 252.58781f, 10.976968f}, 0.46f},
}};
...
bool GetClosestPointOnBladeEdgeArenaRope(G3D::Vector3 const& point, BladeEdgeArenaRope const& rope,
                                         G3D::Vector3& closestPoint)
{
  ...
  float const sagZ    = rope.Sag * std::sin(static_cast<float>(M_PI) * t);
...
PathGenerator::PathGenerator(WorldObject const* owner)
    : _polyLength(0), _type(PATHFIND_BLANK), _useStraightPath(false), _forceDestination(false), _slopeCheck(false),
      _pointPathLimit(MAX_POINT_PATH_LENGTH), _useRaycast(false), _endPosition(G3D::Vector3::zero()), _source(owner),
      _navMesh(nullptr), _navMeshQuery(nullptr)
{
...
dtPolyRef PathGenerator::GetPathPolyByPosition(dtPolyRef const* polyPath, uint32 polyPathSize, float const* point,
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
  ...
  if (startFarFromPoly || endFarFromPoly)
  {
    ...
    auto liquidDataStart = _source->GetMap()->GetLiquidData(_source->GetPhaseMask(), startPos.x, startPos.y, startPos.z,
                                                            _source->GetCollisionHeight(), MAP_ALL_LIQUIDS);
    auto liquidDataEnd   = _source->GetMap()->GetLiquidData(_source->GetPhaseMask(), endPos.x, endPos.y, endPos.z,
                                                            _source->GetCollisionHeight(), MAP_ALL_LIQUIDS);

    bool startUnderWaterEndInWater =
        liquidDataStart.Status == LIQUID_MAP_UNDER_WATER && (liquidDataEnd.Status & MAP_LIQUID_STATUS_IN_CONTACT) != 0;
    bool startInWaterEndUnderWater =
        (liquidDataStart.Status & MAP_LIQUID_STATUS_IN_CONTACT) != 0 && liquidDataEnd.Status == LIQUID_MAP_UNDER_WATER;
  ...
  else
  {
    ...
    if (_useRaycast)
    {
      ...
      dtResult = _navMeshQuery->raycast(startPoly, startPoint, endPoint, &_filter, &hit, hitNormal, _pathPolyRefs,
                                        (int*)&_polyLength, MAX_PATH_LENGTH);
      ...
      if (hit != FLT_MAX)
      {
        ...
        // Walk back a bit from the hit point to make sure it's in the mesh (sometimes the point is actually outside of
        // the polygons due to float precision issues)
        hit *= 0.99f;
    ...
    if (!_polyLength || dtStatusFailed(dtResult))
    {
      ...
      LOG_ERROR("movement", "PathGenerator::BuildPolyPath: {} Path Build failed: 0 length path",
...
void PathGenerator::BuildPointPath(const float* startPoint, const float* endPoint)
{
  ...
  if (_useRaycast)
  {
    // _straightLine uses raycast and it currently doesn't support building a point path, only a 2-point path with start
    // and hitpoint/end is returned
    LOG_ERROR("movement", "PathGenerator::BuildPointPath() called with _useRaycast for unit {}",
              _source->GetGUID().ToString());
    BuildShortcut();
    _type = PATHFIND_NOPATH;
    return;
...
{
  LiquidData const& liquidData = _source->GetMap()->GetLiquidData(_source->GetPhaseMask(), x, y, z,
                                                                  _source->GetCollisionHeight(), MAP_ALL_LIQUIDS);
...
uint32 PathGenerator::FixupCorridor(dtPolyRef* path, uint32 npath, uint32 maxPath, dtPolyRef const* visited,
                                    uint32 nvisited)
{
...
bool PathGenerator::GetSteerTarget(float const* startPos, float const* endPos, float minTargetDist,
                                   dtPolyRef const* path, uint32 pathSize, float* steerPos, unsigned char& steerPosFlag,
                                   dtPolyRef& steerPosRef)
{
  ...
  uint32              nsteerPath = 0;
  dtStatus dtResult = _navMeshQuery->findStraightPath(startPos, endPos, path, pathSize, steerPath, steerPathFlags,
                                       ...
                                       uint32 maxSmoothPathSize)
{
  *smoothPathSize    = 0;
  ...
  while (npolys && nsmoothPath < maxSmoothPathSize)
  {
    ...
    uint32 nvisited = 0;
    if (dtStatusFailed(_navMeshQuery->moveAlongSurface(polys[0], iterPos, moveTgt, &_filter, result, visited,
 ...
 */
bool PathGenerator::IsWalkableClimb(float x, float y, float z, float destX, float destY, float destZ,
                                    float sourceHeight)
{
 ...
 */
float PathGenerator::GetRequiredHeightToClimb(float x, float y, float z, float destX, float destY, float destZ,
                                              float sourceHeight)
{
...
void PathGenerator::ShortenPathUntilDist(G3D::Vector3 const& target, float dist)
{
  ...
  while (1)
  {
    ...
    _source->GetHitSpherePointFor({_pathPoints[i - 1].x, _pathPoints[i - 1].y, _pathPoints[i - 1].z + collisionHeight},
                                  x, y, z);
    if (!_source->GetMap()->isInLineOfSight(x, y, z, _pathPoints[i - 1].x, _pathPoints[i - 1].y,
                                            _pathPoints[i - 1].z + collisionHeight, _source->GetPhaseMask(),
                                            LINEOFSIGHT_ALL_CHECKS, VMAP::ModelIgnoreFlags::Nothing) ||
        (canCheckSlope &&
         !IsSwimmableSegment(_source->GetPositionX(), _source->GetPositionY(), _source->GetPositionZ(),
                             _pathPoints[i - 1].x, _pathPoints[i - 1].y, _pathPoints[i - 1].z) &&
         !IsWalkableClimb(_source->GetPositionX(), _source->GetPositionY(), _source->GetPositionZ(),
                          _pathPoints[i - 1].x, _pathPoints[i - 1].y, _pathPoints[i - 1].z)))
    {
 ...
 */
bool PathGenerator::IsSwimmableSegment(float x, float y, float z, float destX, float destY, float destZ,
