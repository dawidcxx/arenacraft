...
bool WaypointMovementGenerator<Creature>::DoUpdate(Creature* creature, uint32 diff)
{
  ...
  else
  {
    ...
    else
    {
      ...
      //    finished = (creature->movespline->_Spline().length(creature->movespline->_currentSplineIdx() + 1) -
      //    creature->movespline->timePassed()) < 200;
...
void WaypointMovementGenerator<Creature>::Resume(uint32 /*overrideTimer/*/) { stalled = false; }
...
void FlightPathMovementGenerator::LoadPath(Player* player)
{
  ...
  if (GetCurrentNode() >= i_path.size())
  {
    ...
    LOG_ERROR(
        ...
        player->GetGUID().ToString(), GetCurrentNode(), i_path.size(), paths, player->GetPosition().ToString());
...
{
  // remove flag to prevent send object build movement packets for flight state and crash (movement generator already
  // not at top of stack)
  ...
  if (pointId > i_currentNode && i_currentNode < i_path.size() - 1)
  {
    ...
    do
    {
      ...
      while (!_pointsForPathSwitch.empty() && _pointsForPathSwitch.front().PathIndex <= i_currentNode)
      {
        ...
        {
          player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GOLD_SPENT_FOR_TRAVELLING,
...
void FlightPathMovementGenerator::DoEventIfAny(Player* player, TaxiPathNodeEntry const* node, bool departure)
{
  ...
  {
    LOG_DEBUG("maps.script", "Taxi {} event {} of node {} of path {} for player {}",
              departure ? "departure" : "arrival", eventid, node->index, node->path, player->GetName());
...
void FlightPathMovementGenerator::PreloadEndGrid()
{
  ...
  if (endMap)
  {
    LOG_DEBUG("misc", "Preloading grid ({}, {}) for map {} at node index {}/{}", _endGridX, _endGridY, _endMapId,
              _preloadTargetNode, (uint32)(i_path.size() - 1));
