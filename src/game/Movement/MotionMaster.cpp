...
ChaseRange::ChaseRange(float range)
    : MinRange(range > CONTACT_DISTANCE ? 0 : range - CONTACT_DISTANCE), MinTolerance(range),
      MaxRange(range + CONTACT_DISTANCE), MaxTolerance(range)
{
}
ChaseRange::ChaseRange(float _minRange, float _maxRange)
    : MinRange(_minRange), MinTolerance(std::min(_minRange + CONTACT_DISTANCE, (_minRange + _maxRange) / 2)),
      MaxRange(_maxRange), MaxTolerance(std::max(_maxRange - CONTACT_DISTANCE, MinTolerance))
{
}
ChaseRange::ChaseRange(float _minRange, float _minTolerance, float _maxTolerance, float _maxRange)
    : MinRange(_minRange), MinTolerance(_minTolerance), MaxRange(_maxRange), MaxTolerance(_maxTolerance)
{
}
...
ChaseAngle::ChaseAngle(float angle, float _tolerance /* = M_PI_4*/)
    : RelativeAngle(Position::NormalizeOrientation(angle)), Tolerance(_tolerance)
{
}
...
inline bool isStatic(MovementGenerator* movement) { return (movement == GetIdleMovementGenerator()); }
...
void MotionMaster::InitDefault() { Mutate(FactorySelector::SelectMovementGenerator(_owner), MOTION_SLOT_IDLE); }
...
/**
 * @brief The unit will return this initial position (owner for pets and summoned creatures). Doesn't work with
...
void MotionMaster::MoveTargetedHome(bool walk /*= false*/)
{
  ...
  else if (_owner->IsCreature() && _owner->ToCreature()->GetCharmerOrOwnerGUID())
  {
    ...
    if (target)
    {
      LOG_DEBUG("movement.motionmaster", "Following {} ({})", target->IsPlayer() ? "player" : "creature",
...
void MotionMaster::MoveChase(Unit* target, std::optional<ChaseRange> dist, std::optional<ChaseAngle> angle)
{
  ...
  if (_owner->IsPlayer())
  {
    LOG_DEBUG("movement.motionmaster", "Player ({}) chase to {} ({})", _owner->GetGUID().ToString(),
              target->IsPlayer() ? "player" : "creature", target->GetGUID().ToString());
    Mutate(new ChaseMovementGenerator<Player>(target, dist, angle), MOTION_SLOT_ACTIVE);
  }
  else
  {
    LOG_DEBUG("movement.motionmaster", "Creature ({}) chase to {} ({})", _owner->GetGUID().ToString(),
              target->IsPlayer() ? "player" : "creature", target->GetGUID().ToString());
...
void MotionMaster::MoveFollow(Unit* target, float dist, float angle, MovementSlot slot, bool inheritWalkState)
{
  ...
  if (_owner->IsPlayer())
  {
    LOG_DEBUG("movement.motionmaster", "Player ({}) follow to {} ({})", _owner->GetGUID().ToString(),
              target->IsPlayer() ? "player" : "creature", target->GetGUID().ToString());
    Mutate(new FollowMovementGenerator<Player>(target, dist, angle, inheritWalkState), slot);
  }
  else
  {
    LOG_DEBUG("movement.motionmaster", "Creature ({}) follow to {} ({})", _owner->GetGUID().ToString(),
              target->IsPlayer() ? "player" : "creature", target->GetGUID().ToString());
                             ...
                             MovementSlot slot, float orientation /* = 0.0f*/)
{
  ...
  if (_owner->IsPlayer())
  {
    LOG_DEBUG("movement.motionmaster", "Player ({}) targeted point (Id: {} X: {} Y: {} Z: {})",
              _owner->GetGUID().ToString(), id, x, y, z);
    Mutate(new PointMovementGenerator<Player>(id, x, y, z, 0.0f, orientation, nullptr, generatePath, forceDestination),
           slot);
  }
  else
  {
    LOG_DEBUG("movement.motionmaster", "Creature ({}) targeted point (ID: {} X: {} Y: {} Z: {})",
              _owner->GetGUID().ToString(), id, x, y, z);
    Mutate(
        new PointMovementGenerator<Creature>(id, x, y, z, 0.0f, orientation, nullptr, generatePath, forceDestination),
        slot);
  }
}
...
void MotionMaster::MoveLand(uint32 id, Position const& pos, float speed /* = 0.0f*/)
{
  ...
  LOG_DEBUG("movement.motionmaster", "Creature (Entry: {}) landing point (ID: {} X: {} Y: {} Z: {})",
...
{
  LOG_DEBUG("movement.motionmaster", "Unit ({}) jump to point (X: {} Y: {} Z: {})", _owner->GetGUID().ToString(), x, y,
            z);
...
void MotionMaster::MoveFall(uint32 id /*=0*/, bool addFlagForNPC)
{
  ...
  float tz = _owner->GetMapHeight(_owner->GetPositionX(), _owner->GetPositionY(), _owner->GetPositionZ(), true,
                                  MAX_FALL_DISTANCE);
  if (tz <= INVALID_HEIGHT)
  {
    LOG_DEBUG("movement.motionmaster",
              "MotionMaster::MoveFall: unable retrive a proper height at map {} (x: {}, y: {}, z: {}).",
              _owner->GetMap()->GetId(), _owner->GetPositionX(), _owner->GetPositionX(),
 ...
 */
void MotionMaster::MoveCharge(float x, float y, float z, float speed, uint32 id, const Movement::PointsArray* path,
                              bool generatePath, float orientation /* = 0.0f*/,
                              ObjectGuid targetGUID /*= ObjectGuid::Empty*/)
{
  ...
  if (_owner->IsPlayer())
  {
    LOG_DEBUG("movement.motionmaster", "Player ({}) charge point (X: {} Y: {} Z: {})", _owner->GetGUID().ToString(), x,
              y, z);
    Mutate(new PointMovementGenerator<Player>(id, x, y, z, speed, orientation, path, generatePath, generatePath,
                                              targetGUID),
           MOTION_SLOT_CONTROLLED);
  }
  else
  {
    LOG_DEBUG("movement.motionmaster", "Creature ({}) charge point (X: {} Y: {} Z: {})", _owner->GetGUID().ToString(),
              x, y, z);
    Mutate(new PointMovementGenerator<Creature>(id, x, y, z, speed, orientation, path, generatePath, generatePath,
                                                targetGUID),
           MOTION_SLOT_CONTROLLED);
  }
}
...
void MotionMaster::MoveSeekAssistance(float x, float y, float z)
{
  ...
  {
    LOG_DEBUG("movement.motionmaster", "Creature ({}) seek assistance (X: {} Y: {} Z: {})",
              _owner->GetGUID().ToString(), x, y, z);
    _owner->AttackStop();
...
void MotionMaster::MoveSeekAssistanceDistract(uint32 time)
{
  ...
  if (_owner->IsPlayer())
  {
    LOG_ERROR("movement.motionmaster", "Player ({}) attempt to call distract after assistance",
              _owner->GetGUID().ToString());
  }
  else
  {
    LOG_DEBUG("movement.motionmaster", "Creature ({}) is distracted after assistance call (Time: {})",
              _owner->GetGUID().ToString(), time);
...
void MotionMaster::MoveFleeing(Unit* enemy, uint32 time)
{
  ...
  if (_owner->IsPlayer())
  {
    LOG_DEBUG("movement.motionmaster", "Player ({}) flee from {} ({})", _owner->GetGUID().ToString(),
              enemy->IsPlayer() ? "player" : "creature", enemy->GetGUID().ToString());
    Mutate(new FleeingMovementGenerator<Player>(enemy->GetGUID()), MOTION_SLOT_CONTROLLED);
  }
  else
  {
    LOG_DEBUG("movement.motionmaster", "Creature ({}) flee from {} ({}) {}", _owner->GetGUID().ToString(),
              enemy->IsPlayer() ? "player" : "creature", enemy->GetGUID().ToString(),
...
void MotionMaster::MoveTaxiFlight(uint32 path, uint32 pathnode)
{
  if (_owner->IsPlayer())
  {
    ...
    {
      LOG_ERROR("movement.motionmaster", "{} attempt taxi to (not existed Path {} node {})", _owner->GetName(), path,
                pathnode);
    }
  }
  else
  {
    LOG_ERROR("movement.motionmaster", "Creature ({}) attempt taxi to (Path {} node {})", _owner->GetGUID().ToString(),
              path, pathnode);
  }
}
...
void MotionMaster::MovePath(uint32 path_id, bool repeatable)
{
  ...
  LOG_DEBUG("movement.motionmaster", "{} ({}) start moving over path(Id:{}, repeatable: {})",
            _owner->IsPlayer() ? "Player" : "Creature", _owner->GetGUID().ToString(), path_id,
            repeatable ? "YES" : "NO");
}
...
{
  LOG_DEBUG("movement.motionmaster", "Unit (Entry {}) is trying to delete its updating MG (Type {})!",
