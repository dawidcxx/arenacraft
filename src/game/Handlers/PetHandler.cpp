...
void WorldSession::HandlePetAction(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network.opcode", "HandlePetAction: Pet {} - flag: {}, spellId: {}, target: {}.", guid1.ToString(),
            uint32(flag), spellId, guid2.ToString());

  if (!pet)
  {
    LOG_ERROR("network.opcode", "HandlePetAction: Pet ({}) doesn't exist for player {}", guid1.ToString(),
              GetPlayer()->GetName());
    return;
  }
  ...
  {
    LOG_ERROR("network.opcode", "HandlePetAction: Pet ({}) does not belong to player {}", guid1.ToString(),
              GetPlayer()->GetName());
    return;
  }
...
void WorldSession::HandlePetStopAttack(WorldPackets::Pet::PetStopAttack& packet)
{
  ...
  {
    LOG_ERROR("network.opcode", "HandlePetStopAttack: Pet {} isn't a pet or charmed creature of player {}",
...
void WorldSession::HandlePetActionHelper(Unit* pet, ObjectGuid guid1, uint32 spellId, uint16 flag, ObjectGuid guid2)
{
  ...
  if (!charmInfo)
  {
    LOG_ERROR("network.opcode",
              "WorldSession::HandlePetAction(petGuid: {}, tagGuid: {}, spellId: {}, flag: {}): object ({}) is "
  ...
  case ACT_COMMAND: // 0x07
    ...
    case COMMAND_ATTACK: // spellId=1792  //ATTACK
    {
      ...
      bool checkLos = !DisableMgr::IsPathfindingEnabled(pet->GetMap()) ||
                      (TargetUnit->IsCreature() &&
                       (TargetUnit->ToCreature()->isWorldBoss() || TargetUnit->ToCreature()->IsDungeonBoss()));
  ...
  case ACT_ENABLED:  // 0xC1    spell
  {
    ...
    if (result == SPELL_CAST_OK)
    {
      ...
      // spell->prepare() can delete charm info, refresh the pointer.
...
void WorldSession::HandlePetSetAction(WorldPacket& recvData)
{
  ...
  {
    LOG_ERROR("network.opcode", "HandlePetSetAction: Unknown pet ({}) or pet owner ({})", petguid.ToString(),
  ...
  for (Unit::ControlSet::const_iterator itr = petsSet.begin(); itr != petsSet.end(); ++itr)
  {
    ...
    {
      LOG_ERROR("network.opcode",
                "WorldSession::HandlePetSetAction: object ({} TypeId: {}) is considered pet-like but doesn't have a "
                "charminfo!",
            ...
            !pet->HasSpell(spell_id)))
      {
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell_id))
        {
          ...
          if (act_state == ACT_ENABLED)
          {
            ...
            {
              for (auto iterator = GetPlayer()->m_Controlled.begin(); iterator != GetPlayer()->m_Controlled.end();
                   ++iterator)
              {
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPackets::Pet::PetSpellAutocast& packet)
{
  ...
  if (!checkPet)
  {
    LOG_ERROR("entities.pet", "WorldSession::HandlePetSpellAutocastOpcode: Pet {} not found.",
  ...
  if (!spellInfo)
  {
    LOG_ERROR("spells.pet", "WorldSession::HandlePetSpellAutocastOpcode: Unknown spell id {} used by {}.",
              packet.SpellID, packet.PetGUID.ToString());
    return;
  }
  ...
  for (Unit* pet : petsSet)
  {
    ...
    {
      LOG_ERROR("network.opcode",
                "WorldSession::HandlePetSpellAutocastOpcode: object ({} TypeId: {}) is considered pet-like but doesn't "
                "have a charminfo!",
                pet->GetGUID().ToString(), pet->GetTypeId());
      continue;
    }
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& recvPacket)
{
  ...
  LOG_DEBUG("network", "WORLD: CMSG_PET_CAST_SPELL, guid: {}, castCount: {}, spellId {}, castFlags {}", guid.ToString(),
            castCount, spellId, castFlags);
  ...
  {
    LOG_ERROR("network.opcode", "HandlePetCastSpellOpcode: Pet {} isn't pet of player {} .", guid.ToString(),
              GetPlayer()->GetName());
    return;
  }
