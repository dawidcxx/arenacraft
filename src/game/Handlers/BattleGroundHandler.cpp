...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket& recvData)
{
  ...
  {
    LOG_ERROR("network", "Battleground: invalid bgtype ({}) received. possible cheater? player {}", bgTypeId_,
  ...
  if (!joinAsGroup)
  {
    ...
    // send status packet
    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_QUEUE, avgWaitTime, 0, 0,
                                                    TEAM_NEUTRAL);
    SendPacket(&data);
  ...
  else
  {
    ...
    grp->DoForAllMembers(
        [&err, bgQueueTypeId, bgQueueTypeIdRandom, bgTypeId](Player* member)
        {
    ...
    grp->DoForAllMembers(
        [bg, err, bgQueueTypeId, avgWaitTime](Player* member)
        {
          WorldPacket data;

          // send status packet
          sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, member->AddBattlegroundQueueId(bgQueueTypeId),
...
void WorldSession::HandlePVPLogDataOpcode(WorldPacket& /*recvData*/)
{
  ...
  // Prevent players from sending BuildPvpLogDataPacket in an arena except for when sent in
...
void WorldSession::HandleBattlefieldListOpcode(WorldPacket& recvData)
{
  ...
  if (!bl)
  {
    LOG_DEBUG("bg.battleground", "BattlegroundHandler: invalid bgtype ({}) with player (Name: {}, {}) received.",
              bgTypeId, _player->GetName(), _player->GetGUID().ToString());
    return;
  }
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket& recvData)
{
  ...
  {
    LOG_DEBUG("bg.battleground",
              "CMSG_BATTLEFIELD_PORT {} ArenaType: {}, Unk: {}, BgType: {}, Action: {}. Player not in queue (No player "
              "Group Info)!",
              GetPlayerInfo(), arenaType, unk2, bgTypeId_, action);
    return;
  }
  ...
  if (action == 1 && ginfo.ArenaType == 0)
  {
    ...
    if (!_player->CanJoinToBattleground())
    {
      ...
      SendPacket(&data);
      action = 0;
      LOG_DEBUG("bg.battleground", "Player {} {} has a deserter debuff, do not port him to battleground!",
    ...
    {
      LOG_ERROR("network",
                "Player {} {} has level ({}) higher than maxlevel ({}) of battleground ({})! Do not port him to "
                "battleground!",
                _player->GetName(), _player->GetGUID().ToString(), _player->GetLevel(), bg->GetMaxLevel(),
                bg->GetBgTypeID());
      action = 0;
    }
  }
  ...
  if (action) // accept
  {
    ...
    _player->SetBattlegroundId(bg->GetInstanceID(), bg->GetBgTypeID(), queueSlot, true, bgTypeId == BATTLEGROUND_RB,
                               teamId);
    sBattlegroundMgr->SendToBattleground(_player, ginfo.IsInvitedToBGInstanceGUID, bgTypeId);

    LOG_DEBUG("bg.battleground", "Battleground: player {} {} joined battle for bg {}, bgtype {}, queue type {}.",
              _player->GetName(), _player->GetGUID().ToString(), bg->GetInstanceID(), bg->GetBgTypeID(), bgQueueTypeId);
  }
  else // leave queue
  {
    for (auto const& playerGuid : ginfo.Players)
    {
      ...
      LOG_DEBUG("bg.battleground", "Battleground: player {} {} left queue for bgtype {}, queue type {}.",
                player->GetName(), playerGuid.ToString(), bg->GetBgTypeID(), bgQueueTypeId);
    }
...
void WorldSession::HandleBattlefieldStatusOpcode(WorldPacket& /*recvData*/)
{
  ...
  if (Battleground* bg = _player->GetBattleground())
    ...
    {
      sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, _player->GetCurrentBattlegroundQueueSlot(),
  ...
  for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
  {
    ...
    if (ginfo.IsInvitedToBGInstanceGUID)
    {
      ...
      uint32 remainingTime = (GameTime::GetGameTimeMS().count() < ginfo.RemoveInviteTime
                                  ...
                                  : 1);
      sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, i, STATUS_WAIT_JOIN, remainingTime, 0, ginfo.ArenaType,
    ...
    else
    {
      ...
      uint32 avgWaitTime = bgQueue.GetAverageQueueWaitTime(&ginfo);
      sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bgt, i, STATUS_WAIT_QUEUE, avgWaitTime,
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recvData)
{
  ...
  if (!asGroup)
  {
    ...
    GroupQueueInfo* ginfo = bgQueue.AddGroup(_player, nullptr, bgTypeId, bracketEntry, arenatype, isRated != 0, false,
                                             arenaRating, matchmakerRating, ateamId, previousOpponents);
    uint32          avgWaitTime = bgQueue.GetAverageQueueWaitTime(ginfo);
    uint32          queueSlot   = _player->AddBattlegroundQueueId(bgQueueTypeId);

    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bgt, queueSlot, STATUS_WAIT_QUEUE, avgWaitTime, 0, arenatype,
                                                    TEAM_NEUTRAL);
    SendPacket(&data);
    ...
    if (err > 0)
    {
      ...
      if (isRated)
      {
        LOG_DEBUG("bg.battleground",
                  ...
                  _player->GetArenaTeamId(arenaslot), _player->GetName(), matchmakerRating, arenatype);
        bgt->SetRated(true);
      }
      else
      {
      ...
      GroupQueueInfo* ginfo = bgQueue.AddGroup(_player, grp, bgTypeId, bracketEntry, arenatype, isRated != 0, false,
                                               arenaRating, matchmakerRating, ateamId, previousOpponents);
      avgWaitTime           = bgQueue.GetAverageQueueWaitTime(ginfo);
    }
...
void WorldSession::HandleReportPvPAFK(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("bg.battleground", "WorldSession::HandleReportPvPAFK: {} reported {}", _player->GetName(),
