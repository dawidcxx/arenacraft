...
void WorldSession::HandleSwapInvItemOpcode(WorldPacket& recvData)
{
  ...
  {
    // LOG_DEBUG("network", "WORLD: HandleSwapInvItemOpcode - Unit ({}) not found or you can't interact with him.",
...
void WorldSession::HandleSwapItem(WorldPacket& recvData)
{
  ...
  {
    // LOG_DEBUG("network", "WORLD: HandleSwapItem - Unit ({}) not found or you can't interact with him.",
    // m_currentBankerGUID.ToString());
    return;
  }
...
void WorldSession::HandleSellItemOpcode(WorldPacket& recvData)
{
  ...
  if (!creature)
  {
    LOG_DEBUG("network", "WORLD: HandleSellItemOpcode - Unit ({}) not found or you can not interact with him.",
              vendorguid.ToString());
      ...
      if (pProto->SellPrice > 0)
      {
        ...
        if (maxDurability)
        {
          ...
          if (LostDurability > 0)
          {
            ...
            {
              _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, creature, itemguid, 0);
              LOG_ERROR("network.opcode", "WORLD: HandleSellItemOpcode - Wrong item lvl {} for item {} count = {}",
                        pProto->ItemLevel, pItem->GetEntry(), count);
              return;
            }
            ...
            if (!dQualitymodEntry)
            {
              ...
              LOG_ERROR("network.opcode",
                        ...
                        dQualitymodEntryId, pItem->GetEntry(), count);
              return;
            }
        ...
        if (count < pItem->GetCount()) // need split items
        {
          ...
          if (!pNewItem)
          {
            LOG_ERROR("network.opcode", "WORLD: HandleSellItemOpcode - could not create clone of item {}; count = {}",
...
void WorldSession::HandleBuybackItem(WorldPacket& recvData)
{
  ...
  if (!creature)
  {
    LOG_DEBUG("network", "WORLD: HandleBuybackItem - Unit ({}) not found or you can not interact with him.",
              vendorguid.ToString());
...
void WorldSession::SendListInventory(ObjectGuid vendorGuid, uint32 vendorEntry)
{
  ...
  if (!vendor)
  {
    LOG_DEBUG("network", "WORLD: SendListInventory - Unit ({}) not found or you can not interact with him.",
              vendorGuid.ToString());
    _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid::Empty, 0);
    return;
  }
  ...
  for (uint8 slot = 0; slot < itemCount; ++slot)
  {
    if (VendorItem const* item = items->GetItem(slot))
    {
      if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(item->item))
      {
        ...
        {
          LOG_DEBUG("network", "SendListInventory: conditions not met for creature entry {} item {}",
...
void WorldSession::HandleWrapItemOpcode(WorldPacket& recvData)
{
  ...
  LOG_DEBUG("network", "WRAP: receive gift_bag = {}, gift_slot = {}, item_bag = {}, item_slot = {}", gift_bag,
            gift_slot, item_bag, item_slot);
...
void WorldSession::HandleSocketOpcode(WorldPacket& recvData)
{
  ...
  if (SocketBonusActivated ^ SocketBonusToBeActivated)          // if there was a change...
  {
    ...
    itemTarget->SetEnchantment(BONUS_ENCHANTMENT_SLOT,
                               (SocketBonusToBeActivated ? itemTarget->GetTemplate()->socketBonus : 0), 0, 0,
                               _player->GetGUID());
