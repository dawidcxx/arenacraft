...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
  ...
  else
  {
    ...
    if (type != CHAT_MSG_AFK && type != CHAT_MSG_DND)
      sender->UpdateSpeakTime(lang == LANG_ADDON ? Player::ChatFloodThrottle::ADDON
  ...
  // Our Warden module also uses SendAddonMessage as a way to communicate Lua check results to the server, see if this
  // is that
  ...
  if (lang != LANG_ADDON)
  {
    ...
    for (uint8 c : msg)
    {
      if (isNasty(c))
      {
        LOG_ERROR("network", "Player {} {} sent a message containing invalid character {} - blocked",
                  GetPlayer()->GetName(), GetPlayer()->GetGUID().ToString(), uint8(c));
        return;
      }
    }
  ...
  case CHAT_MSG_WHISPER:
  {
    ...
    // If player is a Gamemaster and doesn't accept whisper, we auto-whitelist every player that the Gamemaster is
    // talking to
