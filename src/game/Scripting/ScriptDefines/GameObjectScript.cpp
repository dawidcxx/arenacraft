...
void ScriptMgr::OnGameObjectModifyHealth(GameObject* go, Unit* attackerOrHealer, int32& change,
...
void ScriptMgr::OnGameObjectStateChanged(GameObject* go, uint32 state)
{
  ...
  ExecuteScript<AllGameObjectScript>([&](AllGameObjectScript* script) { script->OnGameObjectStateChanged(go, state); });
