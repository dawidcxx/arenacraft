  ...
  else
  {
    ...
    if (!player)
      LOG_ERROR("maps.script",
                "{} neither source nor target object is player (source: TypeId: {}, Entry: {}, GUID: {}; target: "
                "TypeId: {}, Entry: {}, GUID: {}), skipping.",
                scriptInfo->GetDebugInfo(), source ? source->GetTypeId() : 0, source ? source->GetEntry() : 0,
                source ? source->GetGUID().ToString() : "", target ? target->GetTypeId() : 0,
                target ? target->GetEntry() : 0, target ? target->GetGUID().ToString() : "");
  }
  return player;
}

inline Creature* Map::_GetScriptCreatureSourceOrTarget(Object* source, Object* target, const ScriptInfo* scriptInfo,
                                                       bool bReverse) const
{
  ...
  else
  {
    ...
    if (!creature)
      LOG_ERROR("maps.script",
                "{} neither source nor target are creatures (source: TypeId: {}, Entry: {}, GUID: {}; target: TypeId: "
                "{}, Entry: {}, GUID: {}), skipping.",
                scriptInfo->GetDebugInfo(), source ? source->GetTypeId() : 0, source ? source->GetEntry() : 0,
                source ? source->GetGUID().ToString() : "", target ? target->GetTypeId() : 0,
                target ? target->GetEntry() : 0, target ? target->GetGUID().ToString() : "");
...
inline Unit* Map::_GetScriptUnit(Object* obj, bool isSource, const ScriptInfo* scriptInfo) const
{
  ...
  if (!obj)
    ...
    LOG_ERROR("maps.script", "{} {} object is not unit (TypeId: {}, Entry: {}, GUID: {}), skipping.",
              scriptInfo->GetDebugInfo(), isSource ? "source" : "target", obj->GetTypeId(), obj->GetEntry(),
              obj->GetGUID().ToString());
  else
  {
    unit = obj->ToUnit();
    if (!unit)
      LOG_ERROR("maps.script", "{} {} object could not be casted to unit.", scriptInfo->GetDebugInfo(),
...
inline Player* Map::_GetScriptPlayer(Object* obj, bool isSource, const ScriptInfo* scriptInfo) const
{
  ...
  else
  {
    player = obj->ToPlayer();
    if (!player)
      LOG_ERROR("maps.script", "{} {} object is not a player ({}).", scriptInfo->GetDebugInfo(),
...
inline WorldObject* Map::_GetScriptWorldObject(Object* obj, bool isSource, const ScriptInfo* scriptInfo) const
{
  ...
  else
  {
    ...
    if (!pWorldObject)
      LOG_ERROR("maps.script", "{} {} object is not a world object ({}).", scriptInfo->GetDebugInfo(),
                isSource ? "source" : "target", obj->GetGUID().ToString());
  }
  return pWorldObject;
}
...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, const ScriptInfo* scriptInfo) const
{
  ...
  if (!guid)
    ...
    LOG_ERROR("maps.script", "{} source object is not unit ({}), skipping.", scriptInfo->GetDebugInfo(),
              source->GetGUID().ToString());
  else
  {
    WorldObject* wSource = dynamic_cast<WorldObject*>(source);
    if (!wSource)
      LOG_ERROR("maps.script", "{} source object could not be casted to world object ({}), skipping.",
    ...
    else
    {
      ...
      if (!pDoor)
        ...
        LOG_ERROR("maps.script", "{} gameobject is not a door ({}).", scriptInfo->GetDebugInfo(),
  ...
  while (!m_scriptSchedule.empty() && (iter->first <= GameTime::GetGameTime().count()))
  {
    ...
    if (step.sourceGUID)
    {
      ...
      case HighGuid::Mo_Transport:
        ...
        break;
      default:
        LOG_ERROR("maps.script", "{} source with unsupported high guid ({}).", step.script->GetDebugInfo(),
    ...
    if (step.targetGUID)
    {
      ...
      case HighGuid::Mo_Transport:
        ...
        break;
      default:
        LOG_ERROR("maps.script", "{} target with unsupported high guid ({}).", step.script->GetDebugInfo(),
    ...
    case SCRIPT_COMMAND_TALK:
    {
      ...
      {
        LOG_ERROR("maps.script", "{} invalid chat type ({}) specified, skipping.", step.script->GetDebugInfo(),
      ...
      if (source)
      {
        ...
        if (!sourceUnit)
        {
          LOG_ERROR("scripts", "{} source object ({}) is not an unit, skipping.", step.script->GetDebugInfo(),
                    source->GetGUID().ToString());
          break;
        }
        ...
        case CHAT_MSG_RAID_BOSS_WHISPER:
        {
          ...
          if (!receiver)
            ...
            sourceUnit->Whisper(step.script->Talk.TextID, receiver,
      ...
      if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
      {
        ...
        if (step.script->FieldSet.FieldID <= OBJECT_FIELD_ENTRY ||
          ...
          LOG_ERROR("maps.script", "{} wrong field {} (max count: {}) in object ({}) specified, skipping.",
                    step.script->GetDebugInfo(), step.script->FieldSet.FieldID, cSource->GetValuesCount(),
    ...
    case SCRIPT_COMMAND_MOVE_TO:
      ...
      if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
      {
        ...
        {
          float speed =
              unit->GetDistance(step.script->MoveTo.DestX, step.script->MoveTo.DestY, step.script->MoveTo.DestZ) /
              ((float)step.script->MoveTo.TravelTime * 0.001f);
          unit->MonsterMoveWithSpeed(step.script->MoveTo.DestX, step.script->MoveTo.DestY, step.script->MoveTo.DestZ,
                                     speed);
        }
        else
          unit->NearTeleportTo(step.script->MoveTo.DestX, step.script->MoveTo.DestY, step.script->MoveTo.DestZ,
                               unit->GetOrientation());
      }
      break;
    ...
    case SCRIPT_COMMAND_FLAG_SET:
      ...
      if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
      {
        ...
        if (step.script->FlagToggle.FieldID <= OBJECT_FIELD_ENTRY ||
          ...
          LOG_ERROR("maps.script", "{} wrong field {} (max count: {}) in object ({}) specified, skipping.",
                    step.script->GetDebugInfo(), step.script->FlagToggle.FieldID, cSource->GetValuesCount(),
                    cSource->GetGUID().ToString());
        else
    ...
    case SCRIPT_COMMAND_QUEST_EXPLORED:
    {
      ...
      if (player)
      {
        ...
        {
          LOG_ERROR("maps.script", "{} source is not unit, gameobject or player ({}), skipping.",
      ...
      else
      {
        ...
        if (player)
        {
          ...
          {
            LOG_ERROR("maps.script", "{} target is not unit, gameobject or player ({}), skipping.",
    ...
    case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
      ...
      if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
      {
        ...
        if (!pGO)
        {
          LOG_ERROR("maps.script", "{} gameobject was not found (guid: {}).", step.script->GetDebugInfo(),
    ...
    case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE:
    {
      ...
      if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
      {
        ...
        else
        {
          ...
          if (!pSummoner->SummonCreature(entry, x, y, z, o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
                                         step.script->TempSummonCreature.DespawnDelay))
            LOG_ERROR("maps.script", "{} creature was not spawned (entry: {}).", step.script->GetDebugInfo(),
    ...
    case SCRIPT_COMMAND_ACTIVATE_OBJECT:
      ...
      if (Unit* unit = _GetScriptUnit(source, true, step.script))
      {
        ...
        {
          LOG_ERROR("maps.script", "{} target object is not gameobject ({}), skipping.", step.script->GetDebugInfo(),
    ...
    case SCRIPT_COMMAND_CAST_SPELL:
    {
      ...
      case SF_CASTSPELL_SEARCH_CREATURE: // source -> creature with entry
        uSource = source ? source->ToUnit() : nullptr;
        uTarget = uSource ? GetClosestCreatureWithEntry(uSource, std::abs(step.script->CastSpell.CreatureEntry),
      ...
      {
        LOG_ERROR("maps.script", "{} no source unit found for spell {}", step.script->GetDebugInfo(),
      ...
      {
        LOG_ERROR("maps.script", "{} no target unit found for spell {}", step.script->GetDebugInfo(),
                  step.script->CastSpell.SpellID);
        break;
      }
    ...
    case SCRIPT_COMMAND_CREATE_ITEM:
      ...
      if (Player* pReceiver = _GetScriptPlayerSourceOrTarget(source, target, step.script))
      {
        ...
        InventoryResult msg = pReceiver->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, step.script->CreateItem.ItemEntry,
    ...
    case SCRIPT_COMMAND_LOAD_PATH:
      ...
      if (Unit* unit = _GetScriptUnit(source, true, step.script))
      {
        if (!sWaypointMgr->GetPath(step.script->LoadPath.PathID))
          LOG_ERROR("maps.script", "{} source object has an invalid path ({}), skipping.", step.script->GetDebugInfo(),
    ...
    case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
    {
      ...
      if (creatureBounds.first != creatureBounds.second)
      {
        ...
        auto creatureItr = std::find_if(creatureBounds.first, creatureBounds.second,
      ...
      if (!cTarget)
      {
        LOG_ERROR("maps.script", "{} target was not found (entry: {})", step.script->GetDebugInfo(),
      ...
      if (!datamap)
      {
        LOG_ERROR("maps.script", "{} unknown scriptmap ({}) specified, skipping.", step.script->GetDebugInfo(),
    ...
    case SCRIPT_COMMAND_KILL:
      ...
      {
        if (cSource->isDead())
          LOG_ERROR("maps.script", "{} creature is already dead ({})", step.script->GetDebugInfo(),
                    cSource->GetGUID().ToString());
        else
        {
