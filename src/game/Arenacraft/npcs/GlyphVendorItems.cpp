...
// ai-docs/class_guides.md), then the remaining glyphs alphabetically. Vendor
// order follows row order, so moving a row up moves it up in the vendor.
//
...
std::vector<GlyphEntry> const& AllGlyphs()
{
  static std::vector<GlyphEntry> const glyphs = {
      // == Warrior ==
      // Guide-recommended glyphs first (see ai-docs/class_guides.md);
      // reorder these rows to change the vendor order.
