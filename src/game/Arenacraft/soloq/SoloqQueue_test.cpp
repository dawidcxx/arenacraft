...
{
  return QueuedPlayer{id, classId, specIndex, InitialRating, mmr};
}

QueuedPlayer melee(PlayerId id, uint32_t mmr = InitialMmr) { return player(id, CLASS_WARRIOR, 0, mmr); }
QueuedPlayer caster(PlayerId id, uint32_t mmr = InitialMmr) { return player(id, CLASS_MAGE, 0, mmr); }
QueuedPlayer healer(PlayerId id, uint32_t mmr = InitialMmr) { return player(id, CLASS_PRIEST, 1, mmr); }
