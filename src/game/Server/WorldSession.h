...
{
public:
  WorldSession(uint32 id, std::string&& name, std::shared_ptr<WorldSocket> sock, AccountTypes sec, uint8 expansion,
               time_t mute_time, LocaleConstant locale, uint32 recruiter, bool isARecruiter, bool skipQueue,
               uint32 TotalTime);
  ~WorldSession();
  ...
  // Guild/Arena Team
  void SendArenaTeamCommandResult(uint32 team_action, std::string const& team, std::string const& player,
                                  uint32 error_id = 0);
...
public:                                               // opcodes handlers
  ...
  void HandleCharFactionOrRaceChangeCallback(std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo,
  ...
  void HandleOpenWrappedItemCallback(uint8 bagIndex, uint8 slot, ObjectGuid::LowType itemLowGUID,
                                     PreparedQueryResult result);
...
private:
  ...
  // EnumData helpers
  bool IsLegitCharacterForAccount(ObjectGuid guid) { return _legitCharacters.find(guid) != _legitCharacters.end(); }

  // this stores the GUIDs of the characters who can login
  // characters who failed on Player::BuildEnumData shouldn't login
  GuidSet _legitCharacters;

  ObjectGuid::LowType m_GUIDLow; // set logined or recently logout player (while
                                 // m_playerRecentlyLogout set)
  Player*                      _player;
