...
struct GameEventData
{
  ...
  uint8          holidayStage;
  GameEventState state{GAMEEVENT_NORMAL};                // state of the game event, these are saved into the
                                                         // game_event table on change!
  GameEventConditionMap             conditions;          // conditions to finish
  std::set<uint16 /*gameevent id*/> prerequisite_events; // events that must be completed before starting this
                                                         // event
...
private:
  ...
  void RunSmartAIScripts(uint16 event_id,
