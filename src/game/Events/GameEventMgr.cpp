...
uint32 GameEventMgr::NextCheck(uint16 entry) const
{
  ...
  // for NEXTPHASE state world events, return the delay to start the next event, so the followup event will be checked
  // correctly
  ...
  // for CONDITIONS state world events, return the length of the wait period, so if the conditions are met, this check
  // will be called again to set the timer as NEXTPHASE event
...
{
  {
    ...
    //                                                    7             8            9          10
    QueryResult result =
        WorldDatabase.Query("SELECT eventEntry, UNIX_TIMESTAMP(start_time), UNIX_TIMESTAMP(end_time), occurence, "
                            "length, holiday, holidayStage, description, world_event, announce FROM game_event");
    if (!result)
    {
      mGameEvent.clear();
    ...
    do
    {
      ...
      if (pGameEvent.holiday_id != HOLIDAY_NONE)
      {
        ...
        {
          LOG_ERROR("sql.sql", "`game_event` game event id ({}) have not existed holiday id {}.", event_id,
  ...
  LOG_INFO("server.loading", "Loading Game Event Saves Data...");
  {
    ...
    else
    {
      ...
      LOG_INFO("server.loading", ">> Loaded {} Game Event Saves In Game Events in {} ms", count,
    ...
    else
    {
      ...
      do
      {
        ...
        if (mGameEvent[event_id].state != GAMEEVENT_NORMAL && mGameEvent[event_id].state != GAMEEVENT_INTERNAL)
        {
          ...
          {
            LOG_ERROR("sql.sql",
                      "`game_event_prerequisite` game event prerequisite id ({}) is out of range compared to max event "
                      "id in `game_event`",
                      prerequisite_event);
            continue;
          }
      ...
      LOG_INFO("server.loading", ">> Loaded {} game event prerequisites in Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Creature Data...");
  {
    ...
    else
    {
      ...
      LOG_INFO("server.loading", ">> Loaded {} Creatures In Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event GO Data...");
  {
    ...
    else
    {
      ...
      LOG_INFO("server.loading", ">> Loaded {} Gameobjects In Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Model/Equipment Change Data...");
  {
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT creature.guid, creature.id1, creature.id2, creature.id3, game_event_model_equip.eventEntry, "
    ...
    else
    {
      ...
      do
      {
        ...
        if (newModelEquipSet.equipment_id > 0)
        {
          ...
          {
            LOG_ERROR("sql.sql",
                      "Table `game_event_model_equip` have creature (Guid: {}) with equipment_id {} not found in table "
      ...
      LOG_INFO("server.loading", ">> Loaded {} Model/Equipment Changes In Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Quest Data...");
  {
    ...
    else
    {
      ...
      LOG_INFO("server.loading", ">> Loaded {} Quests Additions In Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event GO Quest Data...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          LOG_ERROR("sql.sql",
                    "`game_event_gameobject_quest` game event id ({}) is out of range compared to max event id in "
                    "`game_event`",
                    event_id);
          continue;
        }
  ...
  LOG_INFO("server.loading", "Loading Game Event Quest Condition Data...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          LOG_ERROR("sql.sql",
                    "`game_event_quest_condition` game event id ({}) is out of range compared to max event id in "
                    "`game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server.loading", ">> Loaded {} quest event conditions in Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Condition Data...");
  {
    ...
    //                                                  0          1            2             3                      4
    QueryResult result = WorldDatabase.Query("SELECT eventEntry, condition_id, req_num, max_world_state_field, "
    ...
    else
    {
      ...
      LOG_INFO("server.loading", ">> Loaded {} conditions in Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Condition Save Data...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          LOG_ERROR("sql.sql", "game_event_condition_save contains not present condition evt id {} cond id {}",
                    event_id, condition);
          continue;
        }
      ...
      LOG_INFO("server.loading", ">> Loaded {} Condition Saves In Game Events In {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Seasonal Quest Relations...");
  {
    ...
    if (!result)
    {
      LOG_WARN("server.loading", ">> Loaded 0 Seasonal Quests Additions In Game Events. DB Table "
    ...
    else
    {
      ...
      do
      {
        ...
        if (!questTemplate)
        {
          LOG_ERROR("sql.sql", "`game_event_seasonal_questrelation` quest id ({}) does not exist in `quest_template`",
                    questId);
          continue;
        }
  ...
  LOG_INFO("server.loading", "Loading Game Event Vendor Additions Data...");
  {
    ...
    //                                                    0        1    2       3         4          5
    QueryResult result = WorldDatabase.Query("SELECT eventEntry, guid, item, maxcount, incrtime, ExtendedCost FROM "
    ...
    else
    {
      ...
      do
      {
        ...
        if (!sObjectMgr->IsVendorItemValid(newEntry.entry, newEntry.item, newEntry.maxcount, newEntry.incrtime,
                                           newEntry.ExtendedCost, nullptr, nullptr, event_npc_flag))
          continue;
      ...
      LOG_INFO("server.loading", ">> Loaded {} Vendor Additions In Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Battleground Data...");
  {
    ...
    else
    {
      ...
      do
      {
        ...
        {
          LOG_ERROR("sql.sql",
                    "`game_event_battleground_holiday` game event id ({}) is out of range compared to max event id in "
                    "`game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server.loading", ">> Loaded {} Battleground Holidays In Game Events in {} ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server.loading", " ");
    }
  }
  ...
  LOG_INFO("server.loading", "Loading Game Event Pool Data...");
  {
    ...
    //                                                               0                         1
    QueryResult result =
        WorldDatabase.Query("SELECT pool_template.entry, game_event_pool.eventEntry FROM pool_template"
    ...
    else
    {
      ...
      do
      {
        ...
        {
          LOG_ERROR("sql.sql",
                    "Pool Id ({}) has all creatures or gameobjects with explicit chance sum <>100 and no equal chance "
...
uint32 GameEventMgr::Update() // return the next event delay in ms
{
  ...
  // following the activate - deactivate order, deactivating the first event later will leave the spawn in (wont
  // disappear then reappear clientside)
...
void GameEventMgr::ApplyNewEvent(uint16 event_id)
{
  ...
  // When event ends (if it expires or if it's stopped via commands) worldstate will be set to 0 again, ready for
