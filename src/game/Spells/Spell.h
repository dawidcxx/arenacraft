...
public:
  Spell(Unit* caster, SpellInfo const* info, TriggerCastFlags triggerFlags,
        ObjectGuid originalCasterGUID = ObjectGuid::Empty, bool skipCheck = false);
  ~Spell();
  ...
  void SelectEffectImplicitTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType,
  ...
  void SelectImplicitChainTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType,
  ...
  WorldObject* SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType,
                                  ConditionList* condList = nullptr);
  void         SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer,
                                 SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType,
                                 ConditionList* condList);
  void         SearchChainTargets(std::list<WorldObject*>& targets, uint32 chainTargets, WorldObject* target,
  ...
  static void WriteCastResultInfo(WorldPacket& data, Player* caster, SpellInfo const* spellInfo, uint8 castCount,
  ...
  void        SendLogExecute();
  void        ExecuteLogEffectTakeTargetPower(uint8 effIndex, Unit* target, uint32 PowerType, uint32 powerTaken,
...
protected:
  ...
  // e.g. damage around area spell trigered by victim aura and damage enemies of
  // aura caster
  ...
  // Spell data
  SpellSchoolMask m_spellSchoolMask; // Spell school (can be overwrite for some
                                     // spells (wand shoot for example)
  ...
  int32 m_casttime;                  // Calculated spell cast time initialized only in
                                     // Spell::prepare
  int32 m_channeledDuration;         // Calculated channeled spell duration in order to
                                     // calculate correct pushback.
  bool m_canReflect;                 // can reflect this spell?

  uint8 m_spellFlags; // for spells whose target was changed in cast i.e. due to
                      // reflect
  ...
  bool m_executedCurrently;          // mark as executed to prevent deleted and access by
                                     // dead pointers
  ...
  SpellCastResult CanOpenLock(uint32 effIndex, uint32 lockid, SkillType& skillid, int32& reqSkillValue,
                              int32& skillValue);
  // -------------------------------------------
...
struct WorldObjectSpellAreaTargetCheck : public WorldObjectSpellTargetCheck
{
  float           _range;
  Position const* _position;
  WorldObjectSpellAreaTargetCheck(float range, Position const* position, Unit* caster, Unit* referer,
...
{
  float _coneAngle;
  WorldObjectSpellConeTargetCheck(float coneAngle, float range, Unit* caster, SpellInfo const* spellInfo,
                                  SpellTargetCheckTypes selectionType, ConditionList* condList);
  bool operator()(WorldObject* target);
};
