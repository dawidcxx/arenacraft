...
SpellDestination::SpellDestination(WorldObject const& wObj)
{
  ...
  _transportOffset.Relocate(wObj.GetTransOffsetX(), wObj.GetTransOffsetY(), wObj.GetTransOffsetZ(),
...
void SpellCastTargets::Read(ByteBuffer& data, Unit* caster)
{
  ...
  if (m_targetMask & (TARGET_FLAG_UNIT | TARGET_FLAG_UNIT_MINIPET | TARGET_FLAG_GAMEOBJECT | TARGET_FLAG_CORPSE_ENEMY |
  ...
  else
  {
    ...
    if (m_src._transportGUID)
      m_src._transportOffset.Relocate(caster->GetTransOffsetX(), caster->GetTransOffsetY(), caster->GetTransOffsetZ(),
  ...
  else
  {
    ...
    if (m_dst._transportGUID)
      m_dst._transportOffset.Relocate(caster->GetTransOffsetX(), caster->GetTransOffsetY(), caster->GetTransOffsetZ(),
                                      caster->GetTransOffsetO());
    else
...
// Xinef: Channel Data
void SpellCastTargets::SetObjectTargetChannel(ObjectGuid targetGUID) { m_objectTargetGUIDChannel = targetGUID; }

void SpellCastTargets::SetDstChannel(SpellDestination const& spellDest) { m_dstChannel = spellDest; }
...
bool SpellCastTargets::HasDstChannel() const { return m_dstChannel._position.GetExactDist(0, 0, 0) > 0.001f; }

SpellDestination const* SpellCastTargets::GetDstChannel() const { return &m_dstChannel; }
...
Spell::~Spell()
{
  ...
  if (m_referencedFromCurrentSpell && m_selfContainer && *m_selfContainer == this)
  {
    ...
    LOG_ERROR("spells", "Spell::~Spell: deleting spell for spell ID {}. However, spell still referenced.",
              m_spellInfo->Id);
    *m_selfContainer = nullptr;
  }
...
void Spell::SelectSpellTargets()
{
  ...
  for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
  {
    ...
    // some spell effects don't add anything to target map (confirmed with sniffs) (like
    ...
    if (m_spellInfo->IsChanneled())
    {
      ...
      if (!focusObject && m_UniqueTargetInfo.empty() && m_UniqueGOTargetInfo.empty() && m_UniqueItemInfo.empty() &&
          !m_targets.HasDst())
      {
...
uint64 Spell::CalculateDelayMomentForDst() const
{
  if (m_targets.HasDst())
  {
    ...
    {
      // We should not subtract caster size from dist calculation (fixes execution time desync with animation on client,
      // eg. Malleable Goo cast by PP)
...
void Spell::SelectEffectImplicitTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType,
                                        uint32& processedEffectMask)
{
  ...
  case TARGET_SELECT_CATEGORY_DEFAULT:
    ...
    case TARGET_OBJECT_TYPE_SRC:
      ...
      case TARGET_REFERENCE_TYPE_CASTER:
        ...
        break;
      default:
        ASSERT(false && "Spell::SelectEffectImplicitTargets: received not implemented select target reference type for "
                        "TARGET_TYPE_OBJECT_SRC");
        break;
      }
      break;
  ...
  case TARGET_SELECT_CATEGORY_NYI:
    LOG_DEBUG("spells.aura", "SPELL: target type {}, found in spellID {}, effect {} is not implemented yet!",
...
void Spell::SelectImplicitChannelTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
  ...
  case TARGET_UNIT_CHANNEL_TARGET:
  {
    ...
    {
      LOG_DEBUG("spells.aura", "SPELL: cannot find channel spell target for spell ID {}, effect {}", m_spellInfo->Id,
                effIndex);
    }
    break;
  }
  case TARGET_DEST_CHANNEL_TARGET:
    ...
    {
      LOG_DEBUG("spells.aura", "SPELL: cannot find channel spell destination for spell ID {}, effect {}",
...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType,
                                        uint32 effMask)
{
  if (targetType.GetReferenceType() != TARGET_REFERENCE_TYPE_CASTER)
  {
  ...
  if (targetType.GetCheckType() == TARGET_CHECK_ENTRY && (!condList || condList->empty()))
  {
    LOG_DEBUG("spells.aura",
              "Spell::SelectImplicitNearbyTargets: no conditions entry for target with TARGET_CHECK_ENTRY of spell ID "
              "{}, effect {} - selecting default targets",
              m_spellInfo->Id, effIndex);
    switch (targetType.GetObjectType())
    {
  ...
  if (!target)
  {
    LOG_DEBUG("spells.aura", "Spell::SelectImplicitNearbyTargets: cannot find nearby target for spell ID {}, effect {}",
              m_spellInfo->Id, effIndex);
    return;
  }
  ...
  if (!target)
  {
    // LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id {} set
    // nullptr target, effect {}", m_spellInfo->Id, effIndex);
    return;
  }
  ...
  case TARGET_OBJECT_TYPE_UNIT:
  {
    ...
    {
      // LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id {} set
      // object of wrong type, expected unit, got {}, effect {}", m_spellInfo->Id, target->GetGUID().GetTypeName(),
      // effMask);
      return;
    }
    break;
  ...
  case TARGET_OBJECT_TYPE_GOBJ:
    ...
    {
      // LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id {} set
      // object of wrong type, expected gameobject, got {}, effect {}", m_spellInfo->Id,
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
  ...
  if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList))
  {
    ...
    Acore::WorldObjectListSearcher<Acore::WorldObjectSpellConeTargetCheck> searcher(m_caster, targets, check,
                                                                                    containerTypeMask);
    SearchTargets<Acore::WorldObjectListSearcher<Acore::WorldObjectSpellConeTargetCheck>>(searcher, containerTypeMask,
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
  ...
  case TARGET_REFERENCE_TYPE_LAST:
  {
    ...
    for (std::list<TargetInfo>::reverse_iterator ihit = m_UniqueTargetInfo.rbegin(); ihit != m_UniqueTargetInfo.rend();
         ++ihit)
    {
  ...
  std::list<WorldObject*> targets;
  float                   radius = m_spellInfo->Effects[effIndex].CalcRadius(m_caster) * m_spellValue->RadiusMod;
  SearchAreaTargets(targets, radius, center, referer, targetType.GetObjectType(), targetType.GetCheckType(),
...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
  ...
  case TARGET_DEST_HOME:
    if (Player* playerCaster = m_caster->ToPlayer())
      dest = SpellDestination(playerCaster->m_homebindX, playerCaster->m_homebindY, playerCaster->m_homebindZ,
                              playerCaster->GetOrientation(), playerCaster->m_homebindMapId);
    break;
  case TARGET_DEST_DB:
    if (SpellTargetPosition const* st = sSpellMgr->GetSpellTargetPosition(m_spellInfo->Id, effIndex))
    {
      ...
      if (m_spellInfo->HasEffect(SPELL_EFFECT_TELEPORT_UNITS) || m_spellInfo->HasEffect(SPELL_EFFECT_BIND))
        dest =
            SpellDestination(st->target_X, st->target_Y, st->target_Z, st->target_Orientation, (int32)st->target_mapId);
  ...
  case TARGET_DEST_CASTER_FISHING:
  {
    ...
    // m_caster->GetClosePoint(x, y, z, DEFAULT_WORLD_OBJECT_SIZE, dis, angle); this contains extra code that breaks
    // fishing
    ...
    LiquidData const& liquidData  = m_caster->GetMap()->GetLiquidData(m_caster->GetPhaseMask(), x, y, z,
                                                                      m_caster->GetCollisionHeight(), MAP_ALL_LIQUIDS);
  ...
  case TARGET_DEST_CASTER_FRONT_LEAP:
  {
      ...
      for (; j < (numChecks + 1); j++)
      {
        ...
        if (!isCasterInWater)
        {
          if (map->IsInWater(phasemask, tstX, tstY, tstZ, collisionHeight))
          {
            ...
            // LOG_ERROR("spells", "(start was from land) step in water , number of cycle = {} , distance of step = {},
            // total path = {}, Z = {}", j, srange, totalpath, tstZ);
          }
        }
        else if (map->IsInWater(phasemask, tstX, tstY, tstZ, collisionHeight))
        {
          ...
          {
            wcol = true;
            // LOG_ERROR("spells", "step in water with collide and use standart check (for continue way after possible
            // collide), number of cycle = {} ", j);
          }

          // if (j < 2)
          //    LOG_ERROR("spells", "(start in water) step in water, number of cycle = {} , distance of step = {}, total
          //    path = {}", j, srange, totalpath);
          // else
          //    LOG_ERROR("spells", "step in water, number of cycle = {} , distance of step = {}, total path = {}", j,
          //    srange, totalpath);
        }
        ...
        if ((!IsInWater && tstZ != beforewaterz) || wcol) // second safety check z for blink way if on the ground
        {
          ...
          // LOG_ERROR("spells", "step on ground, number of cycle = {} , distance of step = {}, total path = {}", j,
          // srange, totalpath);
        }
        ...
        if (totalpath > distance)
        {
          ...
          // LOG_ERROR("spells", "total path > than distance in 3D , need to move back a bit for save distance, total
          // path = {}, overdistance = {}", totalpath, overdistance);
        }

        bool col = VMAP::VMapFactory::createOrGetVMapMgr()->GetObjectHitPos(mapid, prevX, prevY, prevZ + 0.5f, tstX,
                                                                            tstY, tstZ + 0.5f, tstX, tstY, tstZ, -0.5f);
        // check dynamic collision
        bool dcol = m_caster->GetMap()->GetObjectHitPos(phasemask, prevX, prevY, prevZ + 0.5f, tstX, tstY, tstZ + 0.5f,
                                                        tstX, tstY, tstZ, -0.5f);

        // collision occured
        if (col || dcol || (overdistance > 0.0f && !map->IsInWater(phasemask, tstX, tstY, ground, collisionHeight)) ||
            (fabs(prevZ - tstZ) > maxtravelDistZ && (tstZ > prevZ)))
        {
          ...
          if ((DELTA_X > 0 && startx > destx) || (DELTA_X < 0 && startx < destx) || (DELTA_Y > 0 && starty > desty) ||
    ...
    else
    {
      ...
      bool col = VMAP::VMapFactory::createOrGetVMapMgr()->GetObjectHitPos(mapid, pos.GetPositionX(), pos.GetPositionY(),
                                                                          z, destx, desty, z, destx, desty, z, -0.5f);
      // check dynamic collision
      bool dcol = m_caster->GetMap()->GetObjectHitPos(phasemask, pos.GetPositionX(), pos.GetPositionY(), z, destx,
                                                      desty, z, destx, desty, z, -0.5f);
      ...
      dest = SpellDestination(lastpos);
      // float range = sqrt((desty - pos.GetPositionY())*(desty - pos.GetPositionY()) + (destx -
      // pos.GetPositionX())*(destx - pos.GetPositionX())); LOG_ERROR("spells", "Blink number 2, in falling but at a
      // hight, distance of blink = {}", range);
    }
    break;
  }
  default:
...
void Spell::SelectImplicitChainTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType,
                                       WorldObject* target, uint32 effMask)
{
  ...
  if (maxTargets > 1)
  {
    ...
    SearchChainTargets(targets, maxTargets - 1, target, targetType.GetObjectType(), targetType.GetCheckType(),
                       targetType.GetSelectionCategory(), m_spellInfo->Effects[effIndex].ImplicitTargetConditions,
...
void Spell::SelectImplicitTrajTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
  ...
  std::list<WorldObject*>                targets;
  Acore::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo,
                                               TARGET_CHECK_ENEMY /*targetCheckType*/,
                                               m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
  Acore::WorldObjectListSearcher<Acore::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check,
  ...
  case EFFECT_IMPLICIT_TARGET_EXPLICIT:
    ...
    if (targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK))
    {
      ...
      else if (targetMask & TARGET_FLAG_CORPSE_MASK)
      {
        ...
        {
          /// @todo: this is a workaround - corpses should be added to spell target map too, but we can't do that so we
          /// add owner instead
                                       ...
                                       SpellTargetCheckTypes selectionType, ConditionList* condList)
{
  ...
  Acore::WorldObjectLastSearcher<Acore::WorldObjectSpellNearbyTargetCheck> searcher(m_caster, target, check,
                                                                                    containerTypeMask);
  SearchTargets<Acore::WorldObjectLastSearcher<Acore::WorldObjectSpellNearbyTargetCheck>>(searcher, containerTypeMask,
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer,
                              SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType,
                              ConditionList* condList)
{
  uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList);
  if (!containerTypeMask)
    return;
  Acore::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType,
                                               condList);
  Acore::WorldObjectListSearcher<Acore::WorldObjectSpellAreaTargetCheck> searcher(m_caster, targets, check,
                                                                                  containerTypeMask);
  SearchTargets<Acore::WorldObjectListSearcher<Acore::WorldObjectSpellAreaTargetCheck>>(searcher, containerTypeMask,
    ...
    if (isChainHeal)
    {
      ...
      for (std::list<WorldObject*>::iterator itr = tempTargets.begin(); itr != tempTargets.end(); ++itr)
      {
        if (Unit* unit = (*itr)->ToUnit())
        {
          ...
          if ((deficit > maxHPDeficit || foundItr == tempTargets.end()) && target->IsWithinDist(unit, jumpRadius) &&
...
void Spell::prepareDataForTriggerSystem(AuraEffect const* /*triggeredByAura*/)
{
  ...
  if (m_originalCaster && m_caster != m_originalCaster && m_caster->IsCreature() && m_caster->ToCreature()->IsTotem() &&
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
  ...
  if (m_healing > 0)
  {
    ...
    if (canEffectTrigger)
      Unit::ProcDamageAndSpell(caster, unitTarget, procAttacker, procVictim, procEx, addhealth, m_attackType,
                               m_spellInfo, m_triggeredByAuraSpell.spellInfo, m_triggeredByAuraSpell.effectIndex, this,
                               nullptr, &healInfo);
  }
  ...
  else if (m_damage > 0)
  {
    ...
    if (canEffectTrigger)
    {
      ...
      Unit::ProcDamageAndSpell(caster, unitTarget, procAttacker, procVictim, procEx, damageInfo.damage, m_attackType,
                               m_spellInfo, m_triggeredByAuraSpell.spellInfo, m_triggeredByAuraSpell.effectIndex, this,
                               &dmgInfo);
  ...
  else
  {
    ...
    if (canEffectTrigger)
    {
      ...
      Unit::ProcDamageAndSpell(caster, unitTarget, procAttacker, procVictim, procEx, 0, m_attackType, m_spellInfo,
                               m_triggeredByAuraSpell.spellInfo, m_triggeredByAuraSpell.effectIndex, this, &dmgInfo);
  ...
  if (m_caster)
  {
        ...
        (!m_spellInfo->IsPositive() || m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)))
    {
      ...
      // Patch 3.0.8: All player spells which cause a creature to become aggressive to you will now also immediately
      // cause the creature to be tapped.
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS; ++effectNumber)
  {
    if (effectMask & (1 << effectNumber))
    {
      ...
      // Xinef: And why we check every spell effect basing on rand and generic dispel info? some effects will be
      // appliend and some wont?
      /*else if (m_spellInfo->Effects[effectNumber].IsAura() && !m_spellInfo->IsPositiveEffect(effectNumber))
      {
          int32 debuff_resist_chance = unit->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE,
      int32(m_spellInfo->Dispel)); debuff_resist_chance +=
      unit->GetMaxNegativeAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(m_spellInfo->Dispel));
  ...
  if (unit->IsPlayer())
  {
    ...
    unit->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, m_spellInfo->Id, 0,
                                                m_caster);
    unit->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2, m_spellInfo->Id, 0,
                                                m_caster);
  }
  ...
  if (aura_effmask)
  {
    ...
    // Unit has to be target only of aura effect, both caster and target have to be players, target has to be other than
    // unit target
    ...
    if (m_originalCaster)
    {
      ...
      m_spellAura          = Aura::TryRefreshStackOrCreate(aurSpellInfo, effectMask, unit, m_originalCaster,
                                                  (aurSpellInfo == m_spellInfo) ? &m_spellValue->EffectBasePoints[0]
                                                                                         : &basePoints[0],
                                                           m_CastItem, ObjectGuid::Empty, &refresh, refreshPeriodic);
      ...
      if (m_spellAura)
      {
        ...
        if (unit == m_caster && m_targets.GetUnitTarget())
          ...
          diminishMod = unit->ApplyDiminishingToDuration(m_diminishGroup, duration, m_originalCaster, m_diminishLevel,
                                                         limitduration);
  ...
  if (!m_hitTriggerSpells.empty())
  {
    ...
    for (HitTriggerSpellList::const_iterator i = m_hitTriggerSpells.begin(); i != m_hitTriggerSpells.end(); ++i)
    {
      if (CanExecuteTriggersOnHit(effMask, i->triggeredByAura) && roll_chance_i(i->chance))
      {
        ...
        LOG_DEBUG("spells.aura", "Spell {} triggered spell {} by SPELL_AURA_ADD_TARGET_TRIGGER aura", m_spellInfo->Id,
                  i->triggeredSpell->Id);
...
bool Spell::UpdateChanneledTargetList()
{
  ...
  for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
  {
    ...
    {
      Unit* unit =
          m_caster->GetGUID() == ihit->targetGUID ? m_caster : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID);
  ...
  if (channelTargetEffectMask != 0)
  {
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
      if (ihit->missCondition == SPELL_MISS_NONE && (channelAuraMask & ihit->effectMask))
        if (Unit* unit = m_caster->GetGUID() == ihit->targetGUID ? m_caster
...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
  ...
  // (even if they are interrupted on moving, spells with almost immediate effect get to have their effect processed
  // before movement interrupter kicks in)
  ...
  LOG_DEBUG("spells.aura", "Spell::prepare: spell id {} source {} caster {} customCastFlags {} mask {}",
            m_spellInfo->Id, m_caster->GetEntry(), m_originalCaster ? m_originalCaster->GetEntry() : -1,
            _triggeredCastFlags, m_targets.GetTargetMask());

  if (!(m_spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED) &&
      !(m_spellInfo->Attributes & SPELL_ATTR0_ALLOW_WHILE_SITTING) && !m_triggeredByAuraSpell && m_caster->IsSitState())
  {
  ...
  else
  {
    ...
    if ((m_casttime || m_spellInfo->IsChanneled()) && !HasTriggeredCastFlag(TRIGGERED_IGNORE_SET_FACING))
    {
          ...
          ((m_targets.GetObjectTarget() && m_caster != m_targets.GetObjectTarget()) || m_spellInfo->IsPositive()))
      {
        ...
        m_caster->ToCreature()->FocusTarget(this, m_targets.GetObjectTarget() != nullptr ? m_targets.GetObjectTarget()
                                                                                         : m_caster);
      }
    }
...
void Spell::cancel(bool bySelf)
{
  ...
  case SPELL_STATE_PREPARING:
    ...
    if (m_caster->IsPlayer())
    {
      if (m_caster->ToPlayer()->NeedSendSpectatorData())
        ArenaSpectator::SendCommand_Spell(m_caster->FindMap(), m_caster->GetGUID(), "SPE", m_spellInfo->Id,
                                          bySelf ? 99998 : 99999);
    }
    [[fallthrough]];
  ...
  // Do not remove current far sight object (already done in Spell::EffectAddFarsight) to prevent from reset viewpoint
  // to player
...
void Spell::_cast(bool skipCheck)
{
  ...
  // Xinef: implement attribute SPELL_ATTR1_DISMISS_PET_FIRST, on spell cast current pet is dismissed and charms are
  // removed
  ...
  if (m_caster->IsPlayer())
  {
    ...
    m_caster->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL, m_spellInfo->Id, 0,
  ...
  if (m_originalCaster)
  {
    ...
    Unit::ProcDamageAndSpell(m_originalCaster, m_originalCaster, procAttacker, PROC_FLAG_NONE, procEx, 1, BASE_ATTACK,
                             m_spellInfo, m_triggeredByAuraSpell.spellInfo, m_triggeredByAuraSpell.effectIndex, this,
                             nullptr, nullptr, PROC_SPELL_PHASE_CAST);
  }
  ...
  if (Unit* target = m_targets.GetUnitTarget())
    if (m_caster->IsPlayer() || (m_caster->IsPet() && m_caster->IsControlledByPlayer()))
      if (GetDelayMoment() > 0 && !m_caster->IsFriendlyTo(target) && !m_spellInfo->HasAura(SPELL_AURA_BIND_SIGHT) &&
          (!m_spellInfo->IsPositive() || m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)))
        m_caster->CombatStartOnCast(target, !m_spellInfo->HasAttribute(SPELL_ATTR3_SUPPRESS_TARGET_PROCS),
                                    GetDelayMoment() +
                                        500); // xinef: increase this time so we dont leave and enter combat in a moment
...
void Spell::_handle_finish_phase()
{
  ...
  if (m_comboTarget && m_comboPointGain)
  {
    ...
    // (this Aura removes the already-added CP when it expires from duration - now that we've added CP, this shouldn't
    // happen anymore!)
  ...
  if (m_originalCaster)
  {
    ...
    Unit::ProcDamageAndSpell(m_originalCaster, m_originalCaster, procAttacker, PROC_FLAG_NONE, procEx, 1, BASE_ATTACK,
                             m_spellInfo, m_triggeredByAuraSpell.spellInfo, m_triggeredByAuraSpell.effectIndex, this,
                             nullptr, nullptr, PROC_SPELL_PHASE_FINISH);
  }
}
...
void Spell::update(uint32 difftime)
{
  ...
  case SPELL_STATE_PREPARING:
  {
    ...
    if (m_timer == 0 && !IsNextMeleeSwingSpell() && !IsAutoRepeat())
      // don't CheckCast for instant spells - done in spell::prepare, skip duplicate checks, needed for range checks for
      // example
      cast(!m_casttime);
    break;
  }
  case SPELL_STATE_CASTING:
  {
    ...
    // Xinef: If creature gets new aura in 800ms and in 840ms its updated with diff 270, not 40 is removed from duration
    // but 270 Xinef: so the aura can be removed in different updates for all units
...
void Spell::finish(bool ok)
{
  ...
  if (m_caster->IsCreature() && m_caster->ToCreature()->IsSummon())
  {
    ...
    {
      LOG_DEBUG("spells.aura", "Statue {} is unsummoned in spell {} finish", m_caster->GetGUID().ToString(),
                m_spellInfo->Id);
...
void Spell::SendCastResult(SpellCastResult result)
{
  ...
  // Xinef: override every possible result, except for gm fail result... breaks many things and goes unnoticed because
  // of this and makes me rage when i find this out
...
void Spell::ExecuteLogEffectTakeTargetPower(uint8 effIndex, Unit* target, uint32 PowerType, uint32 powerTaken,
...
void Spell::SendChannelStart(uint32 duration)
{
  ...
  if (!channelTarget && !m_spellInfo->NeedsExplicitUnitTarget())
    if (m_UniqueTargetInfo.size() + m_UniqueGOTargetInfo.size() == 1) // this is for TARGET_SELECT_CATEGORY_NEARBY
      channelTarget =
          !m_UniqueTargetInfo.empty() ? m_UniqueTargetInfo.front().targetGUID : m_UniqueGOTargetInfo.front().targetGUID;
...
SpellCastResult Spell::CheckRuneCost(uint32 RuneCostID)
{
  ...
  // If we are in .cheat power mode we dont need to check the cost as we are expected to be able to use it anyways
  // (infinite power)
...
void Spell::HandleThreatSpells()
{
  ...
  LOG_DEBUG("spells.aura", "Spell {}, added an additional {} threat for {} {} target(s)", m_spellInfo->Id, threat,
            m_spellInfo->_IsPositiveSpell() ? "assisting" : "harming", uint32(m_UniqueTargetInfo.size()));
}
...
SpellCastResult Spell::CheckCast(bool strict)
{
  ...
  if (!m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE))
  {
    if (m_caster->IsPlayer())
    {
      ...
      // check if we are using a potion in combat for the 2nd+ time. Cooldown is added only after caster gets out of
      // combat
  ...
  if (m_caster->IsPlayer() /*&& VMAP::VMapFactory::createOrGetVMapMgr()->isLineOfSightCalcEnabled()*/) // pussywizard:
                                                                                                       // optimization
                                                                                                       // (commented)
  {
  ...
  // check spell cast conditions from database
  {
    ...
    {
      // mLastFailedCondition can be nullptr if there was an error processing the condition in Condition::Meets (i.e.
      // wrong data for ConditionTarget or others)
  ...
  // such spells when learned are not targeting anyone using targeting system, they should apply directly to caster
  // instead also, such casts shouldn't be sent to client Xinef: do not check explicit casts for self cast of triggered
  // spells (eg. reflect case)
  if (!(m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE) &&
        (!m_targets.GetUnitTarget() || m_targets.GetUnitTarget() == m_caster)))
  {
       ...
       m_triggeredByAuraSpell.spellInfo->IsPositive()))
  {
    ...
    // xinef: Enraged Regeneration: While this is active, the warrior is blocked from using abilities that trigger being
    // enraged (which would do nothing and waste the cooldowns).
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
  {
    ...
    case SPELL_EFFECT_CHARGE:
    {
      ...
      {
        // Warbringer - can't be handled in proc system - should be done before checkcast root check and charge effect
        // process
      ...
      if (DisableMgr::IsPathfindingEnabled(m_caster->FindMap()) && m_spellInfo->NeedsExplicitUnitTarget())
      {
        ...
        // first we must check to see if the target is in LoS. A path can usually be built but LoS matters for charge
        // spells
        ...
        bool result = m_preGeneratedPath->CalculatePath(target->GetPositionX(), target->GetPositionY(),
        ...
        m_preGeneratedPath->ShortenPathUntilDist(
            G3D::Vector3(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ()), objSize); // move back
      }
      if (Player* player = m_caster->ToPlayer())
    ...
    case SPELL_EFFECT_SUMMON_RAF_FRIEND:
    {
      ...
      if (!target || !(target->GetSession()->GetRecruiterId() == playerCaster->GetSession()->GetAccountId() ||
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
  {
    ...
    {
      if (m_caster && m_caster->HasAura(23397)) // Nefarian Class Call (Warrior): Berserk -- Nefertum: I don't really
                                                // like this but I didn't find another way.
      {
        return SPELL_FAILED_NOT_SHAPESHIFT;
      }
      break;
...
SpellCastResult Spell::CheckCasterAuras(bool preventionOnly) const
{
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR1_IMMUNITY_PURGES_EFFECT))
  {
    ...
    //         PVP trinket                   EMFH                   TOC PVP trinket               Bullheaded Bestial
    //         Wrath             // Beath Within         // Medalion of Immunity
    if (m_spellInfo->Id == 42292 || m_spellInfo->Id == 59752 || m_spellInfo->Id == 65547 || m_spellInfo->Id == 53490 ||
        m_spellInfo->Id == 19574 || m_spellInfo->Id == 34471 || m_spellInfo->Id == 46227)
  ...
  if (!preventionOnly)
  {
    ...
    {
      // spell is usable while stunned, check if caster has only mechanic stun auras, another stun types must prevent
      // cast spell
      if (usableInStun)
      {
    ...
    if (target != m_caster)
    {
      ...
      if (range_type == SPELL_RANGE_MELEE)
      {
        ...
        if (!m_caster->IsCreature() && m_caster->isMoving() && target->isMoving() && !m_caster->IsWalking() &&
            !target->IsWalking())
          real_max_range -= MIN_MELEE_REACH; // Because of lag, we can not check too strictly here (is only used if both
...
SpellCastResult Spell::CheckItems()
{
  ...
  else
  {
    ...
    if (proto->Class == ITEM_CLASS_CONSUMABLE && m_targets.GetUnitTarget())
    {
      ...
      for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
      {
        // skip check, pet not required like checks, and for TARGET_UNIT_PET m_targets.GetUnitTarget() is not the real
  ...
  if (!(m_CastItem && m_CastItem->GetTemplate()->HasFlag(ITEM_FLAG_NO_REAGENT_COST)))
  {
    ...
    // check reagents (ignore triggered spells with reagents processed by original spell) and special reagent ignore
    // case.
    if (checkReagents)
    {
      for (uint32 i = 0; i < MAX_SPELL_REAGENTS; i++)
  ...
  for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
  {
    ...
    case SPELL_EFFECT_CREATE_ITEM_2:
    {
      ...
      {
        // SPELL_EFFECT_CREATE_ITEM_2 differs from SPELL_EFFECT_CREATE_ITEM in that it picks the random item to create
        // from a pool of potential items, so we need to make sure there is at least one free space in the player's
        // inventory
        ...
        if (m_spellInfo->Effects[i].ItemType)
        {
          ...
          uint32 createCount =
              std::clamp<uint32>(m_spellInfo->Effects[i].CalcValue(), 1u, itemTemplate->GetMaxStackSize());
          ItemPosCountVec dest;
          InventoryResult msg = target->ToPlayer()->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest,
                                                                    m_spellInfo->Effects[i].ItemType, createCount);
          if (msg != EQUIP_ERR_OK)
          {
            /// @todo Needs review
...
SpellCastResult Spell::CheckSpellFocus()
{
  ...
  if (m_spellInfo->RequiresSpellFocus)
  {
    ...
    TypeContainerVisitor<Acore::GameObjectSearcher<Acore::GameObjectFocusCheck>, GridTypeMapContainer> object_checker(
        checker);
    Map& map = *m_caster->GetMap();
...
void Spell::DelayedChannel()
{
  ...
  LOG_DEBUG("spells.aura", "Spell {} partially interrupted for {} ms, new duration: {} ms", m_spellInfo->Id, delaytime,
            m_timer);
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
  ...
  // this is only for target addition and target has to have unselectable flag, this is valid for FLAG_EXTRA_TRIGGER and
  // quest triggers however there are some without this flag, used not_selectable
  ...
  case SPELL_EFFECT_SUMMON_RAF_FRIEND:
    ...
    if (m_caster->ToPlayer()->GetSession()->IsARecruiter() &&
        target->ToPlayer()->GetSession()->GetRecruiterId() != m_caster->ToPlayer()->GetSession()->GetAccountId())
      return false;
    if (m_caster->ToPlayer()->GetSession()->GetRecruiterId() != target->ToPlayer()->GetSession()->GetAccountId() &&
...
bool Spell::IsNextMeleeSwingSpell() const { return m_spellInfo->HasAttribute(SPELL_ATTR0_ON_NEXT_SWING_NO_DAMAGE); }

bool Spell::IsIgnoringCooldowns() const { return HasTriggeredCastFlag(TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD) != 0; }

bool Spell::IsAutoActionResetSpell() const
{
  /// @todo changed SPELL_INTERRUPT_FLAG_AUTOATTACK -> SPELL_INTERRUPT_FLAG_INTERRUPT to fix compile - is this check
  /// correct at all?
...
SpellEvent::~SpellEvent()
{
  ...
  else
  {
    LOG_ERROR("spells",
              ...
              (m_Spell->GetCaster()->IsPlayer() ? "Player" : "Creature"), m_Spell->GetCaster()->GetGUID().ToString(),
...
bool ReflectEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
  ...
  if (target && _caster->IsInMap(target))
    Unit::ProcDamageAndSpell(_caster, target, PROC_FLAG_NONE, PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG,
...
SpellCastResult Spell::CanOpenLock(uint32 effIndex, uint32 lockId, SkillType& skillId, int32& reqSkillValue,
                                   int32& skillValue)
{
...
void Spell::LoadScripts()
{
  ...
  for (std::list<SpellScript*>::iterator itr = m_loadedScripts.begin(); itr != m_loadedScripts.end();)
  {
    ...
    LOG_DEBUG("spells.aura", "Spell::LoadScripts: Script `{}` for spell `{}` is loaded now",
                                                      ...
                                                      SpellImplicitTargetInfo const& targetType)
{
  for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
  {
    ...
    std::list<SpellScript::DestinationTargetSelectHandler>::iterator hookItrEnd =
                                                                         (*scritr)->OnDestinationTargetSelect.end(),
                                                                     hookItr =
                                                                         (*scritr)->OnDestinationTargetSelect.begin();
    for (; hookItr != hookItrEnd; ++hookItr)
      if (hookItr->IsEffectAffected(m_spellInfo, effIndex) && targetType.GetTarget() == hookItr->GetTarget())
        hookItr->Call(*scritr, target);
...
bool Spell::CanExecuteTriggersOnHit(uint8 effMask, SpellInfo const* triggeredByAura) const
{
  ...
  // If triggeredByAura has SPELL_ATTR4_CLASS_TRIGGER_ONLY_ON_TARGET then it can only proc on a casted spell with
  // TARGET_UNIT_CASTER
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
  {
    if ((effMask & (1 << i)) &&
...
WorldObjectSpellTargetCheck::WorldObjectSpellTargetCheck(Unit* caster, Unit* referer, SpellInfo const* spellInfo,
                                                         SpellTargetCheckTypes selectionType, ConditionList* condList)
    : _caster(caster), _referer(referer), _spellInfo(spellInfo), _targetSelectionType(selectionType),
      _condList(condList)
{
  if (condList)
...
WorldObjectSpellAreaTargetCheck::WorldObjectSpellAreaTargetCheck(float range, Position const* position, Unit* caster,
                                                                 Unit* referer, SpellInfo const* spellInfo,
                                                                 SpellTargetCheckTypes selectionType,
                                                                 ConditionList*        condList)
    : WorldObjectSpellTargetCheck(caster, referer, spellInfo, selectionType, condList), _range(range),
      _position(position)
{
}
...
bool WorldObjectSpellAreaTargetCheck::operator()(WorldObject* target)
{
  ...
  {
    if (!target->ToGameObject()->IsInRange(_position->GetPositionX(), _position->GetPositionY(),
