...
std::array<SpellImplicitTargetInfo::StaticData, TOTAL_SPELL_TARGETS> SpellImplicitTargetInfo::_data = {{
    ...
    {TARGET_OBJECT_TYPE_UNIT, TARGET_REFERENCE_TYPE_NONE, TARGET_SELECT_CATEGORY_NYI, TARGET_CHECK_DEFAULT,
     TARGET_DIR_NONE}, // 105 TARGET_UNIT_UNK_105
    {TARGET_OBJECT_TYPE_DEST, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_CHANNEL, TARGET_CHECK_DEFAULT,
     TARGET_DIR_NONE}, // 106 TARGET_DEST_CHANNEL_CASTER
    {TARGET_OBJECT_TYPE_NONE, TARGET_REFERENCE_TYPE_DEST, TARGET_SELECT_CATEGORY_NYI, TARGET_CHECK_DEFAULT,
     TARGET_DIR_NONE}, // 107 TARGET_UNK_DEST_AREA_UNK_107
    {TARGET_OBJECT_TYPE_GOBJ, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_CONE, TARGET_CHECK_DEFAULT,
     TARGET_DIR_FRONT}, // 108 TARGET_GAMEOBJECT_CONE
    {TARGET_OBJECT_TYPE_NONE, TARGET_REFERENCE_TYPE_NONE, TARGET_SELECT_CATEGORY_NYI, TARGET_CHECK_DEFAULT,
     TARGET_DIR_NONE}, // 109
    {TARGET_OBJECT_TYPE_DEST, TARGET_REFERENCE_TYPE_NONE, TARGET_SELECT_CATEGORY_NYI, TARGET_CHECK_DEFAULT,
...
bool SpellEffectInfo::IsEffect(SpellEffects effectName) const { return Effect == effectName; }
...
// checks if spell targets are selected from area, doesn't include spell effects in check (like area wide auras for
// example)
...
bool SpellInfo::IsDeathPersistent() const { return AttributesEx3 & SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; }

bool SpellInfo::IsRequiringDeadTarget() const { return AttributesEx3 & SPELL_ATTR3_ONLY_ON_GHOSTS; }
...
bool SpellInfo::IsBreakingStealth() const { return !(AttributesEx & SPELL_ATTR1_ALLOW_WHILE_STEALTHED); }
...
bool SpellInfo::IsAutoRepeatRangedSpell() const { return AttributesEx2 & SPELL_ATTR2_AUTO_REPEAT; }

bool SpellInfo::IsAffectedBySpellMods() const { return !(AttributesEx3 & SPELL_ATTR3_IGNORE_CASTER_MODIFIERS); }
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player* player /*= nullptr*/,
  ...
  for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
  {
    ...
    for (Unit::AuraEffectList::const_iterator iter = auraList.begin(); iter != auraList.end(); ++iter)
    {
      ...
      if (sFlag >= SPELL_GROUP_SPECIAL_FLAG_PRIORITY1 && sFlag <= SPELL_GROUP_SPECIAL_FLAG_PRIORITY4)
      {
        ...
        if (sFlagCurr >= SPELL_GROUP_SPECIAL_FLAG_PRIORITY1 && sFlagCurr <= SPELL_GROUP_SPECIAL_FLAG_PRIORITY4 &&
            sFlagCurr < sFlag)
        {
          return true;
        }
      ...
      // xinef: check aura effect equal auras only, some auras have different effects on different ranks - check rank
      // also
      ...
      // xinef: have to fix spell mods for triggered spell, turn off current spellmodtakingspell for preparing and
      // restore after
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
  ...
  // check this flag only for implicit targets (chain and area), allow to explicitly target units for spells like Shield
  // of Righteousness
  ...
  Spell examples: [ID - 52864 Devour Water, ID - 52862 Devour Wind, ID - 49370 Wyrmrest Defender: Destabilize Azure
  Dragonshrine Effect] */
...
SpellCastResult SpellInfo::CheckExplicitTarget(Unit const* caster, WorldObject const* target,
...
SpellSpecificType SpellInfo::LoadSpellSpecific() const
{
  ...
  case SPELLFAMILY_SHAMAN:
  {
    ...
    if (SpellFamilyFlags[1] & 0x420 || (SpellFamilyFlags[0] & 0x00000400 && HasAttribute(SPELL_ATTR1_NO_THREAT)) ||
        Id == 23552)
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell) const
{
  ...
  powerCost =
      int32(powerCost * (1.0f + caster->GetFloatValue(static_cast<uint16>(UNIT_FIELD_POWER_COST_MULTIPLIER) + school)));
  if (powerCost < 0)
    powerCost = 0;
  return powerCost;
}
...
bool SpellInfo::_IsPositiveEffect(uint8 effIndex, bool deep) const
{
  ...
  case SPELL_EFFECT_DUMMY:
    ...
    break;
  // always positive effects (check before target checks that provided non-positive result in some case for positive
  // effects)
  case SPELL_EFFECT_HEAL:
  ...
  case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
  {
    ...
    case SPELL_AURA_PROC_TRIGGER_SPELL:
      // many positive auras have negative triggered spells at damage for example and this not make it negative (it can
