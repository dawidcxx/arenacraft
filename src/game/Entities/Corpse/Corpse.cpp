...
bool Corpse::Create(ObjectGuid::LowType guidlow, Player* owner)
{
  ...
  if (!IsPositionValid())
  {
    LOG_ERROR("entities.player",
              "Corpse (guidlow {}, owner {}) not created. Suggested coordinates isn't valid (X: {} Y: {})", guidlow,
...
void Corpse::DeleteFromDB(CharacterDatabaseTransaction trans) { DeleteFromDB(GetOwnerGUID(), trans); }
  ...
  for (uint16 index = 0; index < m_valuesCount; ++index)
  {
         ...
         (flags[index] & visibleFlag)))
    {
      ...
      if (index == CORPSE_FIELD_BYTES_1 || index == CORPSE_FIELD_BYTES_2)
      {
        ...
        if (owner && owner != target && owner->IsInRaidWith(target) &&
