...
Vehicle::~Vehicle()
{
  ...
  for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
    if (itr->second.Passenger.Guid)
    {
      ...
      {
        LOG_FATAL("vehicles", "Vehicle(), unit: {}, entry: {}, typeid: {}, this_entry: {}, this_typeid: {}!",
                  unit->GetName(), unit->GetEntry(), unit->GetTypeId(), _me ? _me->GetEntry() : 0,
                  _me ? _me->GetTypeId() : 0);
        unit->_ExitVehicle();
      }
      else
...
void Vehicle::ApplyAllImmunities()
{
  ...
  // Mechanical units & vehicles ( which are not Bosses, they have own immunities in DB ) should be also immune on
  // healing ( exceptions in switch below )
...
void Vehicle::InstallAccessory(uint32 entry, int8 seatId, bool minion, uint8 type, uint32 summonTime)
{
  ...
  if (_status == STATUS_UNINSTALLING)
  {
    LOG_ERROR("vehicles",
              ...
              "Check Uninstall/PassengerBoarded script hooks for errors.",
              _me->GetGUID().ToString(), entry, (int32)seatId);
    return;
  }

  LOG_DEBUG("vehicles", "Vehicle: Installing accessory entry {} on vehicle entry {} (seat:{})", entry,
            GetCreatureEntry(), seatId);
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
  ...
  LOG_DEBUG("vehicles", "Unit {} enter vehicle entry {} id {} ({}) seat {}", unit->GetName(), _me->GetEntry(),
            _vehicleInfo->m_ID, _me->GetGUID().ToString(), (int32)seat->first);
  ...
  if (_me->IsCreature() && unit->IsPlayer() && seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
  {
    ...
    if (!_me->SetCharmedBy(unit, CHARM_TYPE_VEHICLE))
    {
      ...
      LOG_INFO("vehicles",
               "Crash recovered in Unit::SetCharmedBy(). Unit {}, typeid: {}, in world: {}, duringremove: {} has wrong "
               "CharmType! Charmer {}, typeid: {}, in world: {}, duringremove: {}.",
               _me->GetName(), _me->GetTypeId(), _me->IsInWorld(), _me->IsDuringRemoveFromWorld(), unit->GetName(),
               unit->GetTypeId(), unit->IsInWorld(), unit->IsDuringRemoveFromWorld());
      return false;
    }
  }
  ...
  if (_me->IsInWorld())
  {
    ...
    unit->SetControlled(
        true, UNIT_STATE_ROOT); // SMSG_FORCE_ROOT - In some cases we send SMSG_SPLINE_MOVE_ROOT here (for creatures)
...
void Vehicle::RemovePassenger(Unit* unit)
{
  ...
  {
    LOG_ERROR("vehicles",
              "Vehicle::RemovePassenger: Vehicle entry ({}) id ({}) is dissmised and removed all existing passangers, "
              "but the unit ({}) was not on the vehicle!",
              _me->GetEntry(), _vehicleInfo->m_ID, unit->GetName());
    return;
  }

  LOG_DEBUG("vehicles", "Unit {} exit vehicle entry {} id {} ({}) seat {}", unit->GetName(), _me->GetEntry(),
            _vehicleInfo->m_ID, _me->GetGUID().ToString(), (int32)seat->first);
