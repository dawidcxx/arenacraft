...
enum CreatureFlagsExtra : uint32
{
  ...
  CREATURE_FLAG_EXTRA_WORLDEVENT           = 0x00004000, // custom flag for world event creatures (left room for
                                                         // merging)
  ...
  CREATURE_FLAG_EXTRA_ALL_DIMINISH = 0x00100000,         // creature is subject to all diminishing returns as players
                                                         // are
  CREATURE_FLAG_EXTRA_NO_PLAYER_DAMAGE_REQ = 0x00200000, // creature does not need to take player damage for kill
                                                         // credit
  CREATURE_FLAG_EXTRA_AVOID_AOE = 0x00400000,            // pussywizard: ignored by aoe attacks (for icc blood prince
  ...
  CREATURE_FLAG_EXTRA_DONT_CALL_ASSISTANCE = 0x02000000, // Prevent creatures from calling for assistance on initial
                                                         // aggro
  ...
  CREATURE_FLAG_EXTRA_DUNGEON_BOSS = 0x10000000,       // creature is a dungeon boss (SET DYNAMICALLY, DO NOT ADD IN
                                                       // DB)
...
struct CreatureModel
{
  ...
  CreatureModel() : CreatureDisplayID(0), DisplayScale(0.0f), Probability(0.0f) {}
...
struct CreatureTemplate
{
  ...
  [[nodiscard]] bool IsExotic() const { return (type_flags & CREATURE_TYPE_FLAG_TAMEABLE_EXOTIC) != 0; }
...
struct CreatureBaseStats
{
  ...
  float GenerateArmor(CreatureTemplate const* info) const { return std::ceil(BaseArmor * info->ModArmor); }
...
struct VendorItem
{
  ...
  {
    return pProto->HasFlag2(ITEM_FLAG2_DONT_IGNORE_BUY_PRICE) || !ExtendedCost;
  }
};
