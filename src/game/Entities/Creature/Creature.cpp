...
// movement flags. Combat / cell-change creatures refresh immediately in Map::CreatureRelocation() and
// never use this.
constexpr float CREATURE_MOVEMENT_FLAGS_REFRESH_DIST_SQ = 2.0f * 2.0f;

CreatureMovementData::CreatureMovementData()
    : Ground(CreatureGroundMovementType::Run), Flight(CreatureFlightMovementType::None), Swim(true), Rooted(false),
      Chase(CreatureChaseMovementType::Run), Random(CreatureRandomMovementType::Walk),
...
bool TemporaryThreatModifierEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
  if (Unit* victim = ObjectAccessor::GetUnit(m_owner, m_threatVictimGUID))
  {
    if (m_owner.IsInCombatWith(victim))
    {
      ...
      m_owner.GetThreatMgr().AddThreat(victim,
                                       m_threatValue); // Set to the previous value it had, first before modification.
    }
  }
...
Creature::Creature(bool isWorldObject)
    : Unit(isWorldObject), MovableMapObject(), m_groupLootTimer(0), lootingGroupLowGUID(0), m_lootRecipientGroup(0),
      m_corpseRemoveTime(0), m_respawnTime(0), m_respawnDelay(300), m_corpseDelay(60), m_wanderDistance(0.0f),
      m_boundaryCheckTime(2500), m_transportCheckTimer(1000), lootPickPocketRestoreTime(0), m_combatPulseTime(0),
      m_combatPulseDelay(0), m_reactState(REACT_AGGRESSIVE), m_defaultMovementType(IDLE_MOTION_TYPE), m_spawnId(0),
      m_equipmentId(0), m_originalEquipmentId(0), m_AlreadyCallAssistance(false), m_AlreadySearchedAssistance(false),
      m_regenHealth(true), m_regenPower(true), m_AI_locked(false), m_meleeDamageSchoolMask(SPELL_SCHOOL_MASK_NORMAL),
      m_originalEntry(0), m_moveInLineOfSightDisabled(false), m_moveInLineOfSightStrictlyDisabled(false),
      m_homePosition(), m_transportHomePosition(), m_creatureInfo(nullptr), m_creatureData(nullptr),
      m_detectionDistance(20.0f), m_waypointID(0), m_path_id(0), m_formation(nullptr),
      m_lastLeashExtensionTime(nullptr), m_cannotReachTimer(0), _isMissingSwimmingFlagOutOfCombat(false),
      m_assistanceTimer(0), _playerDamageReq(0), _damagedByPlayer(false), _isCombatMovementAllowed(true)
{
...
void Creature::AddToWorld()
{
  ...
  if (!IsInWorld())
  {
    // pussywizard: motion master needs to be initialized before OnCreatureCreate, which may set death state to
    // JUST_DIED, to prevent crash it's also initialized in AIM_Initialize(), few lines below, but it's not a problem
    Motion_Initialize();
...
void Creature::RemoveCorpse(bool setSpawnTime, bool skipVisibility)
{
  ...
  // pussywizard: if corpse was removed during falling then the falling will continue after respawn, so stop falling is
  // such case
  if (IsFalling())
    StopMoving();
}
...
bool Creature::InitEntry(uint32 Entry, const CreatureData* data)
{
  ...
  {
    LOG_ERROR("sql.sql", "Creature (Entry: {}) has no model defined in table `creature_template_model`, can't load. ",
              Entry);
    return false;
  }
  ...
  {
    LOG_ERROR("sql.sql",
              "Creature (Entry: {}) has no model {} defined in table `creature_template_model`, can't load. ", Entry,
...
bool Creature::UpdateEntry(uint32 Entry, const CreatureData* data, bool changelevel, bool updateAI)
{
  ...
  // updates spell bars for vehicles and set player's faction - should be called here, to overwrite faction that is set
...
void Creature::Update(uint32 diff)
{
  ...
  case DeathState::Alive:
  {
    ...
    // could have changed. (Creatures in combat or changing grid cell re-derive immediately in
    // CreatureRelocation and never reach this.)
    if (IsPositionDataUpdatePending() &&
        (!isMoving() || GetExactDistSq(LastMovementFlagsPos) >= CREATURE_MOVEMENT_FLAGS_REFRESH_DIST_SQ))
    {
      UpdatePositionData(); // -> ProcessTerrainStatusUpdate() -> UpdateMovementFlags() re-baselines
                            // LastMovementFlagsPos
    }
...
void Creature::DoFleeToGetAssistance()
{
  ...
  if (radius > 0)
  {
    ...
    if (!creature)
      ...
      GetMotionMaster()->MoveSeekAssistance(creature->GetPositionX(), creature->GetPositionY(),
...
bool Creature::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, float x,
                      float y, float z, float ang, const CreatureData* data)
{
  ASSERT(map);
  SetMap(map);
  ...
  if (!cinfo)
  {
    LOG_ERROR("sql.sql", "Creature::Create(): creature template (guidlow: {}, entry: {}) does not exist.", guidlow,
              Entry);
    return false;
  }
  ...
  if (!IsPositionValid())
  {
    LOG_ERROR("entities.unit",
              "Creature::Create(): given coordinates for creature (guidlow {}, entry {}) are not valid (X: {}, Y: {}, "
              "Z: {}, O: {})",
              guidlow, Entry, x, y, z, ang);
    return false;
  }

  // area/zone id is needed immediately for ZoneScript::GetCreatureEntry hook before it is known which creature template
  // to load (no model/scale available yet)
  PositionFullTerrainStatus terrainData;
  GetMap()->GetFullTerrainStatusForPosition(GetPhaseMask(), GetPositionX(), GetPositionY(), GetPositionZ(),
                                            DEFAULT_COLLISION_HEIGHT, terrainData);
...
void Creature::SaveToDB()
{
  ...
  uint32 mapId = GetTransport() && GetTransport()->ToMotionTransport() ? GetTransport()->GetGOInfo()->moTransport.mapID
                                                                       : GetMapId();
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
  ...
  // prevent add data integrity problems
  data.movementType =
      !m_wanderDistance && GetDefaultMovementType() == RANDOM_MOTION_TYPE ? IDLE_MOTION_TYPE : GetDefaultMovementType();
...
bool Creature::CreateFromProto(ObjectGuid::LowType guidlow, uint32 Entry, uint32 vehId, const CreatureData* data)
{
  ...
  if (!normalInfo)
  {
    LOG_ERROR("sql.sql", "Creature::CreateFromProto(): creature template (guidlow: {}, entry: {}) does not exist.",
              guidlow, Entry);
    return false;
  }
  ...
  if (!vehId)
  {
    ...
    if (cinfo->VehicleId)
      CreateVehicleKit(cinfo->VehicleId,
                       (cinfo->VehicleId != normalInfo->VehicleId ? cinfo->Entry : normalInfo->Entry));
  }
  else
...
bool Creature::LoadCreatureFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap, bool allowDuplicate /*= false*/)
{
  if (!allowDuplicate)
  {
    ...
    if (creatureBounds.first != creatureBounds.second)
    {
      for (auto itr = creatureBounds.first; itr != creatureBounds.second; ++itr)
      {
        ...
        {
          LOG_DEBUG("maps", "Would have spawned {} but {} already exists", spawnId,
  ...
  if (!Create(map->GenerateLowGuid<HighGuid::Unit>(), map, data->phaseMask, entry, 0, data->posX, data->posY,
...
bool Creature::CanStartAttack(Unit const* who) const
{
  ...
  if (!CanFly() &&
      (GetDistanceZ(who) >
       CREATURE_Z_ATTACK_RANGE + m_CombatDistance)) // too much Z difference, skip very costy vmap calculations here
    return false;
  ...
  // pussywizard: if who is in combat and has an attacker, help him if the distance is right (help because who is
  // hostile or help because attacker is friendly)
  bool assist = false;
  ...
  if (!assist)
    if (IsNeutralToAll() || !IsWithinDistInMap(who, GetAggroRange(who) + m_CombatDistance, true,
...
/**
 * @brief A creature can be in 4 different states: Alive, JustDied, Corpse, and JustRespawned. The cycle follows the
 * next order:
...
void Creature::Respawn(bool force)
{
  ...
  {
    // Creature should not respawn, reset respawn timer. Conditions will be checked again the next time it tries to
    // respawn.
  ...
  if (!linkedRespawntime || (cInfo && cInfo->HasFlagsExtra(CREATURE_FLAG_EXTRA_HARD_RESET)) || force) // Should respawn
  {
    ...
    if (getDeathState() == DeathState::Dead)
    {
      ...
      LOG_DEBUG("entities.unit", "Respawning creature {} (SpawnId: {}, {})", GetName(), GetSpawnId(),
                GetGUID().ToString());
      m_respawnTime = 0;
...
bool Creature::IsImmunedToSpell(SpellInfo const* spellInfo, Spell const* spell)
{
  ...
  // This check must be done instead of 'if (GetCreatureTemplate()->MechanicImmuneMask & (1 << (spellInfo->Mechanic -
  // 1)))' for not break the check of mechanic immunity on DB (tested) because GetCreatureTemplate()->MechanicImmuneMask
  // and m_spellImmune[IMMUNITY_MECHANIC] don't have same data.
...
/**
 * @brief Select nearest hostile unit within the given attack distance (i.e. distance is ignored if > than
...
bool Creature::LoadCreaturesAddon(bool reload)
{
  ...
  if (!cainfo->auras.empty())
  {
    for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin(); itr != cainfo->auras.end(); ++itr)
    {
      ...
      {
        if (!reload)
          LOG_ERROR("sql.sql", "Creature ({}) has duplicate aura (spell {}) in `auras` field.", GetGUID().ToString(),
                    *itr);
...
std::string const& Creature::GetAIName() const { return sObjectMgr->GetCreatureTemplate(GetEntry())->AIName; }

std::string Creature::GetScriptName() const { return sObjectMgr->GetScriptName(GetScriptId()); }
...
uint32 Creature::GetScriptId() const
{
  ...
  if (CreatureData const* creatureData = GetCreatureData())
    if (creatureData->ScriptId && entry == creatureData->id1)
...
bool Creature::SetCanFly(bool enable, bool /*packetOnly*/ /* = false */)
{
  ...
  if (m_movedByPlayer)
  {
    ...
    if (!enable)
      m_movedByPlayer->ToPlayer()->SetFallInformation(GameTime::GetGameTime().count(),
...
void Creature::UpdateMovementFlags()
{
  ...
  bool isInAir =
      (G3D::fuzzyGt(GetPositionZ(),
                    ground + (canHover ? GetFloatValue(UNIT_FIELD_HOVERHEIGHT) : 0.0f) + GROUND_HEIGHT_TOLERANCE) ||
       G3D::fuzzyLt(GetPositionZ(), ground - GROUND_HEIGHT_TOLERANCE)); // Can be underground too, prevent the falling
...
float Creature::GetNativeObjectScale() const { return ObjectMgr::ChooseDisplayId(GetCreatureTemplate())->DisplayScale; }
...
void Creature::UpdateLeashExtensionTime() { (*GetLastLeashExtensionTimePtr()) = GameTime::GetGameTime().count(); }
...
void Creature::SetCorpseRemoveTime(uint32 delay) { m_corpseRemoveTime = GameTime::GetGameTime().count() + delay; }
...
void Creature::SetCombatMovement(bool allowMovement) { _isCombatMovementAllowed = allowMovement; }
...
ObjectGuid Creature::GetSummonerGUID() const
{
  ...
  LOG_DEBUG("entities.unit", "Creature::GetSummonerGUID() called by creature that is not a summon. Creature: {} ({})",
            GetEntry(), GetName());
