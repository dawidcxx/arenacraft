...
public:
  ...
  bool Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, float x, float y,
              float z, float ang, const CreatureData* data = nullptr);
  ...
  } /// @brief Check if the creature has the specified ReactState
  ...
  uint8 getLevelForTarget(
      WorldObject const* target) const override; // overwrite Unit::getLevelForTarget for boss level support
  ...
  [[nodiscard]] SpellSchoolMask GetMeleeDamageSchoolMask(WeaponAttackType /*attackType*/ = BASE_ATTACK,
  ...
  bool                              isTappedBy(Player const* player) const; // return true if the creature is tapped by
  ...
  {
    return (GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_IGNORE_FEIGN_DEATH) != 0;
  }
  ...
  void   SetCombatPulseDelay(uint32 delay) // (secs) interval at which the creature pulses the entire
  ...
  // Used to control if MoveChase() is to be used or not in AttackStart(). Some
  // creatures does not chase victims NOTE: If you use SetCombatMovement while
  // the creature is in combat, it will do NOTHING - This only affects
  // AttackStart
  //       You should make the necessary to make it happen so.
  //       Remember that if you modified _isCombatMovementAllowed (e.g: using
  //       SetCombatMovement) it will not be reset at Reset(). It will keep the
  //       last value you set.
...
protected:
  ...
  uint32 m_boundaryCheckTime; // (msecs) remaining time for next evade boundary
                              // check
  ...
  CreatureTemplate const* m_creatureInfo; // in difficulty mode > 0 can different from
...
public:
  TemporaryThreatModifierEvent(Creature& owner, ObjectGuid threatVictimGUID, float threatValue)
      : BasicEvent(), m_owner(owner), m_threatVictimGUID(threatVictimGUID), m_threatValue(threatValue)
  {
  }
  bool Execute(uint64 e_time, uint32 p_time) override;
