...
enum UnitState
{
  ...
  // UNIT_STATE_MELEE_ATTACK_BY        = 0x00000004,       // player is melee
  // attack by someone
  ...
  UNIT_STATE_ALL_STATE_SUPPORTED =
      UNIT_STATE_DIED | UNIT_STATE_MELEE_ATTACKING | UNIT_STATE_STUNNED | UNIT_STATE_ROAMING | UNIT_STATE_CHASE |
  ...
  UNIT_STATE_MOVING = UNIT_STATE_ROAMING_MOVE | UNIT_STATE_CONFUSED_MOVE | UNIT_STATE_FLEEING_MOVE |
  ...
  UNIT_STATE_IGNORE_ANTISPEEDHACK =
      UNIT_STATE_FLEEING | UNIT_STATE_CONFUSED | UNIT_STATE_CHARGING | UNIT_STATE_DISTRACTED | UNIT_STATE_POSSESSED,
...
enum UnitFlags : uint32
{
  ...
  UNIT_FLAG_SERVER_CONTROLLED = 0x00000001, // set only when unit movement is controlled by server - by
                                            // SPLINE/MONSTER_MOVE packets, together with
                                            // UNIT_FLAG_STUNNED; only set to units controlled by client;
                                            // client function CGUnit_C::IsClientControlled returns false
                                            // when set for owner
  UNIT_FLAG_NON_ATTACKABLE    = 0x00000002, // not attackable
  UNIT_FLAG_DISABLE_MOVE      = 0x00000004,
  UNIT_FLAG_PLAYER_CONTROLLED = 0x00000008, // controlled by player, use _IMMUNE_TO_PC instead of
                                            // _IMMUNE_TO_NPC
  UNIT_FLAG_RENAME      = 0x00000010,
  UNIT_FLAG_PREPARATION = 0x00000020, // don't take reagents for spells with
                                      // SPELL_ATTR5_NO_REAGENT_COST_WITH_AURA
  UNIT_FLAG_UNK_6            = 0x00000040,
  UNIT_FLAG_NOT_ATTACKABLE_1 = 0x00000080, // ?? (UNIT_FLAG_PLAYER_CONTROLLED |
                                           // UNIT_FLAG_NOT_ATTACKABLE_1) is NON_PVP_ATTACKABLE
  UNIT_FLAG_IMMUNE_TO_PC = 0x00000100,     // disables combat/assistance with PlayerCharacters (PC) - see
                                           // Unit::_IsValidAttackTarget, Unit::_IsValidAssistTarget
  UNIT_FLAG_IMMUNE_TO_NPC = 0x00000200,    // disables combat/assistance with NonPlayerCharacters (NPC) -
  ...
  UNIT_FLAG_NON_ATTACKABLE_2 = 0x00010000, // removes attackable icon, if on yourself, cannot assist self
                                           // but can cast TARGET_SELF spells - added by
                                           // SPELL_AURA_MOD_UNATTACKABLE
  UNIT_FLAG_PACIFIED    = 0x00020000,      // 3.0.3 ok
  UNIT_FLAG_STUNNED     = 0x00040000,      // 3.0.3 ok
  UNIT_FLAG_IN_COMBAT   = 0x00080000,
  UNIT_FLAG_TAXI_FLIGHT = 0x00100000, // disable casting at client side spell not allowed by taxi
                                      // flight (mounted?), probably used with 0x4 flag
  UNIT_FLAG_DISARMED = 0x00200000,    // 3.0.3, disable melee spells casting..., "Required melee
  ...
  UNIT_FLAG_PREVENT_EMOTES_FROM_CHAT_TEXT = 0x20000000, // Prevent automatically playing emotes from parsing chat
...
enum UnitFlags2 : uint32
{
  ...
  UNIT_FLAG2_DISARM_RANGED      = 0x00000400, // this does not disable ranged weapon display (maybe
  ...
  UNIT_FLAG2_ALLOW_CHEAT_SPELLS         = 0x00040000, // Allows casting spells with AttributesEx7 &
...
enum NPCFlags : uint32
{
  ...
  UNIT_NPC_FLAG_VENDOR_AMMO        = 0x00000100, // TITLE is vendor (ammo) DESCRIPTION
  ...
  UNIT_NPC_FLAG_PETITIONER     = 0x00040000,     // TITLE handles guild/arena petitions DESCRIPTION 100%
  ...
  UNIT_NPC_FLAG_SPELLCLICK = 0x01000000,         // TITLE has spell click enabled DESCRIPTION cause client to
                                                 // send 1015 opcode (spell click)
  UNIT_NPC_FLAG_PLAYER_VEHICLE = 0x02000000,     // TITLE is player vehicle DESCRIPTION players with mounts
                                                 // that have vehicle data should have it set
  UNIT_NPC_FLAG_MAILBOX = 0x04000000,            // TITLE is mailbox

  UNIT_NPC_FLAG_VENDOR_MASK =
      UNIT_NPC_FLAG_VENDOR | UNIT_NPC_FLAG_VENDOR_AMMO | UNIT_NPC_FLAG_VENDOR_POISON | UNIT_NPC_FLAG_VENDOR_REAGENT
};
...
enum MovementFlags
{
  ...
  MOVEMENTFLAG_DISABLE_GRAVITY = 0x00000400,      // Former MOVEMENTFLAG_LEVITATING. This is used when walking
                                                  // is not possible.
  ...
  MOVEMENTFLAG_FLYING               = 0x02000000, // unit is actually flying. pretty sure this is only used for
                                                  // players. creatures use disable_gravity
  ...
  MOVEMENTFLAG_MASK_MOVING = MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_BACKWARD | MOVEMENTFLAG_STRAFE_LEFT |
                             MOVEMENTFLAG_STRAFE_RIGHT | MOVEMENTFLAG_PITCH_UP | MOVEMENTFLAG_PITCH_DOWN |
                             MOVEMENTFLAG_FALLING | MOVEMENTFLAG_FALLING_FAR | MOVEMENTFLAG_ASCENDING |
  ...
  MOVEMENTFLAG_MASK_HAS_PLAYER_STATUS_OPCODE = MOVEMENTFLAG_DISABLE_GRAVITY | MOVEMENTFLAG_ROOT | MOVEMENTFLAG_CAN_FLY |
                                               MOVEMENTFLAG_WATERWALKING | MOVEMENTFLAG_FALLING_SLOW |
                                               MOVEMENTFLAG_HOVER
};
