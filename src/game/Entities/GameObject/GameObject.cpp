...
void GameObject::RemoveFromOwner()
{
  ...
  LOG_DEBUG("entities.gameobject",
            ...
            GetGUID().ToString(), GetGOInfo()->entry, m_spellId, GetGOInfo()->GetLinkedGameObjectEntry(),
            ownerGUID.ToString());
...
bool GameObject::Create(ObjectGuid::LowType guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y,
                        float z, float ang, G3D::Quat const& rotation, uint32 animprogress, GOState go_state,
                        uint32 artKit)
{
  ASSERT(map);
  SetMap(map);
  ...
  if (!IsPositionValid())
  {
    LOG_ERROR("entities.gameobject",
              "Gameobject (GUID: {} Entry: {}) not created. Suggested coordinates isn't valid (X: {} Y: {})", guidlow,
              name_id, x, y);
    return false;
  }
  ...
  {
    LOG_ERROR("sql.sql",
              "Gameobject (GUID: {} Entry: {}) not created: non-existing entry in `gameobject_template`. Map: {} (X: "
              "{} Y: {} Z: {})",
  ...
  {
    LOG_ERROR("sql.sql",
              "Gameobject (GUID: {} Entry: {}) not created: non-existing GO type '{}' in `gameobject_template`. It "
              "will crash client if created.",
  ...
  if (uint32 linkedEntry = GetGOInfo()->GetLinkedGameObjectEntry())
  {
    ...
    if (linkedGO->Create(map->GenerateLowGuid<HighGuid::GameObject>(), linkedEntry, map, phaseMask, x, y, z, ang,
                         rotation, 255, GO_STATE_READY))
    {
...
void GameObject::Update(uint32 diff)
{
    ...
    case GAMEOBJECT_TYPE_SUMMONING_RITUAL:
    {
      ...
      if (info->summoningRitual.casterTargetSpell &&
        ...
        for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
          ...
          if (Player* target =
                  ObjectAccessor::GetPlayer(*this, Acore::Containers::SelectRandomContainerElement(m_unique_users)))
  ...
  case GO_READY:
  {
    ...
    if (isSpawned())
    {
      ...
      if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
      {
        ...
        {
          // environmental damage spells already have around enemies targeting but this not help in case not existed GO
  ...
  case GO_JUST_DEACTIVATED:
  {
    ...
    // Do not delete chests or goobers that are not consumed on loot, while still allowing them to despawn when they
    // expire if summoned
...
void GameObject::GetFishLootJunk(Loot* fishloot, Player* loot_owner)
{
  ...
  if (fishloot->empty()) // use this becase if zone or subzone has normal mask drop, then fishloot->FillLoot return
                         // true.
  {
    // use zone loot
...
bool GameObject::LoadGameObjectFromDB(ObjectGuid::LowType spawnId, Map* map, bool addToMap)
{
  ...
  if (!Create(map->GenerateLowGuid<HighGuid::GameObject>(), entry, map, phaseMask, x, y, z, ang, data->rotation,
...
void GameObject::SetRespawnDelay(int32 respawn) { m_respawnDelayTime = respawn > 0 ? respawn : 0; }
...
void GameObject::Use(Unit* user)
{
  ...
  case GAMEOBJECT_TYPE_TRAP: // 6
  {
    ...
    m_cooldownTime = GameTime::GetGameTimeMS().count() + (goInfo->trap.cooldown ? goInfo->trap.cooldown : uint32(4)) *
  ...
  case GAMEOBJECT_TYPE_GOOBER: // 10
  {
    ...
    if (user->IsPlayer())
    {
      ...
      {
        LOG_DEBUG("maps.script", "Goober ScriptStart id {} for GO entry {} (spawnId {}).", info->goober.eventId,
                  GetEntry(), m_spawnId);
  ...
  case GAMEOBJECT_TYPE_FISHINGNODE: // 17
  {
    ...
    case GO_READY: // ready for loot
    {
      ...
      LOG_DEBUG("entities.gameobject", "Fishing check (skill: {} zone min skill: {} chance {} roll: {}", skill,
                zone_skill, chance, roll);
  ...
  case GAMEOBJECT_TYPE_SUMMONING_RITUAL: // 18
  {
    ...
    if (info->summoningRitual.animSpell)
      ...
      player->CastSpell(player, GetSpellId(),
                        TriggerCastFlags(TRIGGERED_IGNORE_EFFECTS | TRIGGERED_IGNORE_POWER_AND_REAGENT_COST |
  ...
  if (!spellInfo)
  {
    if (!user->IsPlayer() || !sOutdoorPvPMgr->HandleCustomSpell(user->ToPlayer(), spellId, this))
      LOG_ERROR("entities.gameobject", "WORLD: unknown spell id {} at use action for gameobject (Entry: {} GoType: {})",
...
void GameObject::SetDestructibleState(GameObjectDestructibleState state, Player* eventInvoker /*= nullptr*/,
...
void GameObject::SetLootState(LootState state, Unit* unit)
{
  ...
  // pussywizard: lootState has nothing to do with collision, it depends entirely on GOState. Loot state is for timed
  // close/open door and respawning, which then sets GOState
  /*if (m_model)
  {
      // startOpen determines whether we are going to add or remove the LoS on activation
      bool startOpen = (GetGoType() == GAMEOBJECT_TYPE_DOOR || GetGoType() == GAMEOBJECT_TYPE_BUTTON ?
  GetGOInfo()->door.startOpen : false);
...
void GameObject::SetLootGenerationTime() { m_lootGenerationTime = GameTime::GetGameTime().count(); }
...
public:
  ...
  {
    const_cast<GameObject*>(_owner)->SummonCreature(1, corner.x, corner.y, corner.z, 0.0f, TEMPSUMMON_TIMED_DESPAWN,
                                                    10000);
  }
...
std::string GameObject::GetDebugInfo() const
{
  ...
  sstr << WorldObject::GetDebugInfo() << "\n"
       << "SpawnId: " << GetSpawnId() << " GoState: " << std::to_string(GetGoState()) << " ScriptId: " << GetScriptId()
       << " AIName: " << GetAIName();
  return sstr.str();
}
