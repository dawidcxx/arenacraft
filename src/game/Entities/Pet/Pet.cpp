...
Pet::Pet(Player* owner, PetType type)
    : Guardian(nullptr, owner ? owner->GetGUID() : ObjectGuid::Empty, true), m_usedTalentCount(0), m_removed(false),
      m_owner(owner), m_happinessTimer(PET_LOSE_HAPPINES_INTERVAL), m_petType(type), m_duration(0),
      m_auraRaidUpdateMask(0), m_loading(false), m_petRegenTimer(PET_FOCUS_REGEN_INTERVAL), m_tempspellTarget(),
...
std::pair<PetStable::PetInfo const*, PetSaveMode> Pet::GetLoadPetInfo(PetStable const& stable, uint32 petEntry,
                        ...
                        bool fullMana /*= false*/)
{
  ...
  if (slot == PET_SAVE_NOT_IN_SLOT)
  {
    ...
    auto unslottedPetItr =
        std::find_if(petStable->UnslottedPets.begin(), petStable->UnslottedPets.end(),
  ...
  {
    auto stabledPet = std::find_if(petStable->StabledPets.begin(), petStable->StabledPets.end(),
  ...
  owner->GetSession()
      ...
      .AfterComplete(
          [this, owner, session = owner->GetSession(), isTemporarySummon, current, lastSaveTime = petInfo->LastSaveTime,
           savedhealth = petInfo->Health, savedmana = petInfo->Mana, healthPct,
...
void Pet::setDeathState(DeathState s,
                        bool /*despawn = false*/) // overwrite virtual Creature::setDeathState and Unit::setDeathState
{
...
void Pet::Update(uint32 diff)
{
  ...
  case DeathState::Alive:
  {
    ...
    // if (!owner || (!IsWithinDistInMap(owner, GetMap()->GetVisibilityDistance()) && (owner->GetCharmGUID() &&
    // (owner->GetCharmGUID() != GetGUID()))) || (isControlled() && !owner->GetPetGUID()))
    {
    ...
    if (isControlled())
    {
      if (owner->GetPetGUID() != GetGUID())
      {
        ...
        ASSERT(getPetType() != HUNTER_PET, "Unexpected unlinked pet found for owner {}",
...
void Pet::Remove(PetSaveMode mode, bool returnreagent) { GetOwner()->RemovePet(this, mode, returnreagent); }
...
void Pet::_LoadSpellCooldowns(PreparedQueryResult result)
{
  ...
  if (result)
  {
    ...
    do
    {
      ...
      {
        LOG_ERROR("entities.pet", "Pet {} have unknown spell {} in `pet_spell_cooldown`, skipping.",
      ...
      LOG_DEBUG("entities.pet", "Pet (Number: {}) spell {} cooldown loaded ({} secs).", m_charmInfo->GetPetNumber(),
                spell_id, uint32(db_time - curTime));
...
void Pet::_SaveAuras(CharacterDatabaseTransaction trans)
{
  ...
  for (AuraMap::const_iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end(); ++itr)
  {
    ...
    // don't save guid of caster in case we are caster of the spell - guid for pet is generated every pet load, so it
    // won't match saved guid anyways
...
void Pet::InitLevelupSpellsForLevel()
{
  ...
  // default spells (can be not learned if pet level (as owner level decrease result for example) less first possible in
  // normal game)
