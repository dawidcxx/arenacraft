...
public:
  ...
  ChatCommandTable GetCommands() const override
  {
    ...
    static ChatCommandTable deserterCommandTable = {{"instance", deserterInstanceCommandTable},
                                                    {"bg", deserterBGCommandTable}};
    static ChatCommandTable commandTable         = {{"deserter", deserterCommandTable}};
    return commandTable;
  }
   ...
   * @endcode
   */
  static bool HandleDeserterAdd(ChatHandler* handler, Optional<std::string> playerName, Optional<std::string> time,
                                bool isInstance)
  {
    ...
    if (target)
    {
      ...
      {
        handler->PSendSysMessage("Player {} already has a longer {} Deserter active.", handler->playerLink(*playerName),
    ...
    else
    {
      ...
      if (QueryResult result = CharacterDatabase.Query(
              "SELECT remainTime FROM character_aura WHERE guid = {} AND spell = {}", guid.GetCounter(), deserterSpell))
      {
        ...
        if (remainTime < 0 || remainTime >= duration * IN_MILLISECONDS)
        {
          handler->PSendSysMessage("Player {} already has a longer {} Deserter active.",
                                   handler->playerLink(*playerName), isInstance ? "Instance" : "Battleground");
          return true;
        }
        CharacterDatabase.Query("DELETE FROM character_aura WHERE guid = {} AND spell = {}", guid.GetCounter(),
                                deserterSpell);
      }
    ...
    handler->PSendSysMessage("{} of {} Deserter has been added to player {}.", secsToTimeString(duration),
                             isInstance ? "Instance" : "Battleground", handler->playerLink(*playerName));
    return true;
  }
  ...
  static bool HandleDeserterRemove(ChatHandler* handler, Optional<PlayerIdentifier> player, bool isInstance)
  {
    ...
    else
    {
      if (QueryResult result =
              CharacterDatabase.Query("SELECT remainTime FROM character_aura WHERE guid = {} AND spell = {}",
                                      player->GetGUID().GetCounter(), deserterSpell))
      {
        Field* fields = result->Fetch();
    ...
    if (duration == 0)
    {
      handler->SendErrorMessage("Player {} does not have {} Deserter.", handler->playerLink(player->GetName()),
                                isInstance ? "Instance" : "Battleground");
      return true;
    }
    ...
    if (duration < 0)
    {
      handler->SendErrorMessage("Permanent {} Deserter has been removed from player {} (GUID {}).",
                                isInstance ? "Instance" : "Battleground", handler->playerLink(player->GetName()),
                                player->GetGUID().ToString());
      return true;
    }

    handler->PSendSysMessage("{} of {} Deserter has been removed from player {} (GUID {}).",
                             secsToTimeString(duration / IN_MILLISECONDS), isInstance ? "Instance" : "Battleground",
                             handler->playerLink(player->GetName()), player->GetGUID().ToString());
    return true;
  }
  ...
  static bool HandleDeserterRemoveAll(ChatHandler* handler, bool isInstance, Optional<std::string> maxTime)
  {
    ...
    if (remainTime > 0)
    {
      result = CharacterDatabase.Query("SELECT COUNT(guid) FROM character_aura WHERE spell = {} AND remainTime <= {}",
                                       deserterSpell, remainTime * IN_MILLISECONDS);
    }
    else
    {
    ...
    if (deserterCount > 0)
    {
      ...
      if (remainTime > 0)
      {
        CharacterDatabase.Execute("DELETE FROM character_aura WHERE spell = {} AND remainTime <= {}", deserterSpell,
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = onlinePlayerList.begin(); itr != onlinePlayerList.end();
         ++itr)
    {
    ...
    if (deserterCount == 0)
    {
      handler->PSendSysMessage("No player on this realm has {} Deserter with a duration of {} or less.",
                               isInstance ? "Instance" : "Battleground", remainTimeStr);
      return true;
    }

    handler->PSendSysMessage("{} Deserter has been removed from {} player(s) with a duration of {} or less.",
                             isInstance ? "Instance" : "Battleground", deserterCount, remainTimeStr);
    return true;
  }

  /// @sa HandleDeserterAdd()
  static bool HandleDeserterInstanceAdd(ChatHandler* handler, Optional<std::string> playerName,
