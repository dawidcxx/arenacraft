...
public:
  ...
  {
    static ChatCommandTable gmCommandTable = {{"chat", HandleGMChatCommand, SEC_GAMEMASTER, Console::No},
  ...
  static bool HandleGMListIngameCommand(ChatHandler* handler)
  {
    ...
    for (auto const& [playerGuid, player] : ObjectAccessor::GetPlayers())
    {
      ...
      if ((player->IsGameMaster() || (!AccountMgr::IsPlayerAccount(playerSec) &&
  ...
  static bool HandleGMVisibleCommand(ChatHandler* handler, Optional<bool> visibleArg)
  {
    ...
    if (!visibleArg)
    {
      handler->PSendSysMessage(LANG_YOU_ARE, _player->isGMVisible() ? handler->GetAcoreString(LANG_VISIBLE)
