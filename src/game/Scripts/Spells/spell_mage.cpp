...
class spell_mage_arcane_blast : public SpellScript
{
  ...
  void HandleAfterCast() { GetCaster()->CastSpell(GetCaster(), _triggerSpellId, TRIGGERED_FULL_MASK); }
...
class spell_mage_molten_armor : public AuraScript
{
  ...
  bool CheckProc(ProcEventInfo& eventInfo)
  {
    ...
    if (AuraEffect* aurEff =
            eventInfo.GetActionTarget()->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_MAGE, 16, EFFECT_0))
      return roll_chance_i(aurEff->GetSpellInfo()->GetRank() * 50);

    return false;
  }

  void Register() override { DoCheckProc += AuraCheckProcFn(spell_mage_molten_armor::CheckProc); }
};
...
class spell_mage_mirror_image : public AuraScript
{
  ...
  void CalcPeriodic(AuraEffect const* /*effect*/, bool& isPeriodic, int32& /*amplitude*/) { isPeriodic = false; }

  void Register() override
  {
    OnEffectApply += AuraEffectApplyFn(spell_mage_mirror_image::HandleEffectApply, EFFECT_2, SPELL_AURA_PERIODIC_DUMMY,
...
class spell_mage_burnout : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_MAGE_BURNOUT_TRIGGER}); }
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    GetTarget()->CastCustomSpell(SPELL_MAGE_BURNOUT_TRIGGER, SPELLVALUE_BASE_POINT0, mana, GetTarget(), true, nullptr,
                                 aurEff);
  }
...
class spell_mage_pet_scaling : public AuraScript
{
  ...
  void Register() override
  {
    if (m_scriptSpellId != 35657)
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_mage_pet_scaling::CalculateResistanceAmount, EFFECT_ALL,
    ...
    {
      DoEffectCalcAmount +=
          AuraEffectCalcAmountFn(spell_mage_pet_scaling::CalculateAPAmount, EFFECT_ALL, SPELL_AURA_MOD_ATTACK_POWER);
      DoEffectCalcAmount +=
          AuraEffectCalcAmountFn(spell_mage_pet_scaling::CalculateSPAmount, EFFECT_ALL, SPELL_AURA_MOD_DAMAGE_DONE);
    }

    OnEffectApply += AuraEffectApplyFn(spell_mage_pet_scaling::HandleEffectApply, EFFECT_ALL, SPELL_AURA_ANY,
                                       AURA_EFFECT_HANDLE_REAL);
...
class spell_mage_brain_freeze : public AuraScript
{
  ...
  void Register() override { DoCheckProc += AuraCheckProcFn(spell_mage_brain_freeze::CheckProc); }
};
...
class spell_mage_glyph_of_eternal_water : public AuraScript
{
  ...
  void Register() override
  {
    OnEffectRemove += AuraEffectRemoveFn(spell_mage_glyph_of_eternal_water::OnRemove, EFFECT_0, SPELL_AURA_DUMMY,
                                         AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_mage_combustion_proc : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_MAGE_COMBUSTION}); }
  ...
  void Trigger(AuraEffect* aurEff, DamageInfo& /*dmgInfo*/, uint32& absorbAmount)
  {
    ...
    if (AuraEffect* talentAurEff = target->GetAuraEffectOfRankedSpell(SPELL_MAGE_INCANTERS_ABSORBTION_R1, EFFECT_0))
    {
      ...
      if (AuraEffect* currentAura = target->GetAuraEffect(SPELL_AURA_MOD_DAMAGE_DONE, SPELLFAMILY_MAGE, 2941, EFFECT_0))
      {
        ...
        target->CastCustomSpell(target, SPELL_MAGE_INCANTERS_ABSORBTION_TRIGGERED, &bp, nullptr, nullptr, true, nullptr,
                                aurEff);
    }
  }
};
...
class spell_mage_blast_wave : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_MAGE_GLYPH_OF_BLAST_WAVE}); }
  ...
  void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
  {
    ...
    if (AuraEffect* talentAurEff = target->GetAuraEffectOfRankedSpell(SPELL_MAGE_FROST_WARDING_R1, EFFECT_0))
    {
      ...
      if (roll_chance_i(chance))
      {
        ...
        target->CastCustomSpell(target, SPELL_MAGE_FROST_WARDING_TRIGGERED, &bp, nullptr, nullptr, true, nullptr,
                                aurEff);
        absorbAmount = 0;
...
class spell_mage_focus_magic : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_MAGE_FOCUS_MAGIC_PROC}); }
...
class spell_mage_ice_barrier_aura : public spell_mage_incanters_absorbtion_base_AuraScript
{
  ...
  static int32 CalculateSpellAmount(Unit* caster, int32 amount, SpellInfo const* spellInfo, const AuraEffect* aurEff)
  {
    ...
    // Glyph of Ice Barrier is only applied at the spell damage bonus because it was already applied to the base value
    // in CalculateSpellDamage
...
class spell_mage_ice_barrier : public SpellScript
{
  ...
  SpellCastResult CheckCast()
  {
    ...
    if (AuraEffect* aurEff =
            caster->GetAuraEffect(SPELL_AURA_SCHOOL_ABSORB, (SpellFamilyNames)GetSpellInfo()->SpellFamilyName,
                                  GetSpellInfo()->SpellIconID, EFFECT_0))
    {
  ...
  void Register() override { OnCheckCast += SpellCheckCastFn(spell_mage_ice_barrier::CheckCast); }
};
...
class spell_mage_ignite : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_MAGE_IGNITE}); }
  ...
  void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
  {
    ...
    eventInfo.GetProcTarget()->CastDelayedSpellWithPeriodicAmount(eventInfo.GetActor(), SPELL_MAGE_IGNITE,
                                                                  SPELL_AURA_PERIODIC_DAMAGE, amount);
    // GetTarget()->CastCustomSpell(SPELL_MAGE_IGNITE, SPELLVALUE_BASE_POINT0, amount, eventInfo.GetProcTarget(), true,
    // nullptr, aurEff);
  }
...
class spell_mage_living_bomb : public AuraScript
{
  ...
  {
    AfterEffectRemove +=
        AuraEffectRemoveFn(spell_mage_living_bomb::AfterRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_mage_master_of_elements : public AuraScript
{
      ...
      if (Unit* caster = GetCaster())
      {
        if (Unit* target = (selectCaster ? eventInfo.GetActor() : eventInfo.GetActionTarget()))
        {
          if (AuraEffect const* aurEff =
                  target->GetAuraEffect(_spellInfo->Id, eventInfo.GetTriggerAuraEffectIndex(), caster->GetGUID()))
          {
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    if (Unit* target = GetTarget())
    {
      ...
      if (mana > 0)
      {
        target->CastCustomSpell(SPELL_MAGE_MASTER_OF_ELEMENTS_ENERGIZE, SPELLVALUE_BASE_POINT0, mana, target, true,
                                nullptr, aurEff);
      }
    }
  }
...
const uint32 spell_mage_polymorph_cast_visual::spell_mage_polymorph_cast_visual::PolymorhForms[6] = {
    SPELL_MAGE_SQUIRREL_FORM,   SPELL_MAGE_GIRAFFE_FORM, SPELL_MAGE_SERPENT_FORM,
    SPELL_MAGE_DRAGONHAWK_FORM, SPELL_MAGE_WORGEN_FORM,  SPELL_MAGE_SHEEP_FORM};
...
class spell_mage_summon_water_elemental : public SpellScript
{
  ...
  {
    return ValidateSpellInfo({SPELL_MAGE_GLYPH_OF_ETERNAL_WATER, SPELL_MAGE_SUMMON_WATER_ELEMENTAL_TEMPORARY,
...
class spell_mage_fingers_of_frost_proc_aura : public AuraScript
{
  ...
  void HandleOnEffectProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
  {
    ...
    else
    {
      if (eventInfo.GetSpellPhaseMask() == PROC_SPELL_PHASE_FINISH ||
          (_procSpellDelayMoment.value_or(0) > 0 || !eventInfo.GetDamageInfo()))
      {
        PreventDefaultAction();
      }
  ...
  void HandleAfterEffectProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
  {
    ...
    {
    case PROC_SPELL_PHASE_HIT:
      _chance = 100.f;
      break;
    case PROC_SPELL_PHASE_FINISH:
      ResetProcState();
      break;
    default:
      break;
  ...
  void Register()
  {
    ...
    OnEffectProc += AuraEffectProcFn(spell_mage_fingers_of_frost_proc_aura::HandleOnEffectProc, EFFECT_0,
                                     SPELL_AURA_PROC_TRIGGER_SPELL);
    AfterEffectProc += AuraEffectProcFn(spell_mage_fingers_of_frost_proc_aura::HandleAfterEffectProc, EFFECT_0,
                                        SPELL_AURA_PROC_TRIGGER_SPELL);
  }
...
class spell_mage_fingers_of_frost_proc : public AuraScript
{
  ...
  void Register() { DoCheckProc += AuraCheckProcFn(spell_mage_fingers_of_frost_proc::CheckProc); }
};
