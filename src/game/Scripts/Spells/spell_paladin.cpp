  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    if (target->IsAlive())
    {
      eventInfo.GetActor()->CastCustomSpell(aurEff->GetSpellInfo()->Effects[EFFECT_0].TriggerSpell,
                                            SPELLVALUE_MAX_TARGETS, targets, target, false, nullptr, aurEff);
    }
  }
  ...
  void Register() override
  {
    if (m_scriptSpellId == 20424)
      OnObjectAreaTargetSelect +=
          SpellObjectAreaTargetSelectFn(spell_pal_seal_of_command::FilterTargets, EFFECT_0, TARGET_UNIT_TARGET_ENEMY);
  }
};
...
class spell_pal_divine_intervention : public AuraScript
{
  ...
  void Register() override
  {
    OnEffectRemove += AuraEffectRemoveFn(spell_pal_divine_intervention::HandleRemove, EFFECT_0,
                                         SPELL_AURA_SCHOOL_IMMUNITY, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_pal_seal_of_light : public AuraScript
{
  ...
  void Register() override { DoCheckProc += AuraCheckProcFn(spell_pal_seal_of_light::CheckProc); }
};
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    if (eventInfo.GetTypeMask() & PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS)
    {
      ...
      if (caster && procSpell->SpellFamilyName == SPELLFAMILY_PALADIN &&
          ...
          caster->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_PALADIN, 3021, 0)) // need infusion of light
      {
        ...
        caster->CastCustomSpell(eventInfo.GetActionTarget(), 66922, &basepoints, nullptr, nullptr, true, nullptr,
    ...
    eventInfo.GetActionTarget()->CastCustomSpell(eventInfo.GetActionTarget(), triggered_spell_id, &basepoints, nullptr,
...
private:
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({PAL_SPELL_ARDENT_DEFENDER_HEAL}); }
  ...
  void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
  {
    ...
    if (remainingHealth <= 0 && !victim->ToPlayer()->HasAura(PAL_SPELL_ARDENT_DEFENDER_DEBUFF))
    {
      ...
      victim->CastCustomSpell(PAL_SPELL_ARDENT_DEFENDER_HEAL, SPELLVALUE_BASE_POINT0, healAmount, victim, true, nullptr,
                              aurEff);
    }
...
class spell_pal_avenging_wrath : public AuraScript
{
  ...
  void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
  {
    ...
    {
      int32 basepoints = aurEff->GetAmount();
      target->CastCustomSpell(target, SPELL_PALADIN_SANCTIFIED_WRATH, &basepoints, &basepoints, nullptr, true, nullptr,
                              aurEff);
    }
  }
  ...
  void Register() override
  {
    OnEffectApply += AuraEffectApplyFn(spell_pal_avenging_wrath::HandleApply, EFFECT_0,
                                       SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
    AfterEffectRemove += AuraEffectRemoveFn(spell_pal_avenging_wrath::HandleRemove, EFFECT_0,
                                            SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_pal_blessing_of_sanctuary : public AuraScript
{
  ...
  bool CheckProc(ProcEventInfo& /*eventInfo*/) { return GetTarget()->HasActivePowerType(POWER_MANA); }
  ...
  void Register() override
  {
    AfterEffectApply += AuraEffectApplyFn(spell_pal_blessing_of_sanctuary::HandleEffectApply, EFFECT_0,
                                          SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    AfterEffectRemove += AuraEffectRemoveFn(spell_pal_blessing_of_sanctuary::HandleEffectRemove, EFFECT_0,
                                            SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
...
class spell_pal_divine_sacrifice : public AuraScript
{
  ...
  void Register() override { OnEffectSplit += AuraEffectSplitFn(spell_pal_divine_sacrifice::Split, EFFECT_0); }
};
...
class spell_pal_divine_storm : public SpellScript
{
  ...
  void TriggerHeal()
  {
    ...
    if (GetHitUnit() != caster)
      caster->CastCustomSpell(SPELL_PALADIN_DIVINE_STORM_DUMMY, SPELLVALUE_BASE_POINT0,
                              (GetHitDamage() * healPct) / 100, caster, true);
  }

  void Register() override { AfterHit += SpellHitFn(spell_pal_divine_storm::TriggerHeal); }
};
...
private:
  ...
  void Register() override
  {
    ...
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_pal_divine_storm_dummy::CountTargets, EFFECT_0,
...
class spell_pal_exorcism_and_holy_wrath_damage : public AuraScript
{
  ...
  void Register() override
  {
    DoEffectCalcSpellMod += AuraEffectCalcSpellModFn(spell_pal_exorcism_and_holy_wrath_damage::HandleEffectCalcSpellMod,
...
class spell_pal_glyph_of_holy_light : public SpellScript
{
  ...
  void Register() override
  {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_pal_glyph_of_holy_light::FilterTargets, EFFECT_0,
...
class spell_pal_guarded_by_the_light : public SpellScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_PALADIN_DIVINE_PLEA}); }
...
class spell_pal_hand_of_sacrifice_aura : public AuraScript
{
  ...
  void Register() override { OnEffectSplit += AuraEffectSplitFn(spell_pal_hand_of_sacrifice_aura::Split, EFFECT_0); }
};
...
class spell_pal_hand_of_sacrifice : public SpellScript
{
  ...
  void Register() override { OnCheckCast += SpellCheckCastFn(spell_pal_hand_of_sacrifice::CheckTarget); }
};
...
class spell_pal_hand_of_salvation : public AuraScript
{
  ...
  void Register() override
  {
    DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pal_hand_of_salvation::CalculateAmount, EFFECT_1,
  ...
  void HandleScriptEffect(SpellEffIndex /*effIndex*/)
  {
    ...
    if (GetCaster()->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_PALADIN, 3015, 0))
    {
          ...
          (spellId2 == SPELL_JUDGEMENT_OF_VENGEANCE_EFFECT || spellId2 == SPELL_JUDGEMENT_OF_CORRUPTION_EFFECT))
      {
        ...
        GetCaster()->CastSpell(GetHitUnit(), SealApplication(spellId2),
...
class spell_pal_seal_of_righteousness : public AuraScript
{
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
  {
    ...
    GetTarget()->CastCustomSpell(SPELL_PALADIN_SEAL_OF_RIGHTEOUSNESS, SPELLVALUE_BASE_POINT0, bp,
                                 eventInfo.GetProcTarget(), true, nullptr, aurEff);
  }
...
class spell_pal_seal_of_vengeance : public SpellScript
{
  ...
  {
    OnEffectHitTarget +=
        SpellEffectFn(spell_pal_seal_of_vengeance::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_WEAPON_PERCENT_DAMAGE);
  }
};
