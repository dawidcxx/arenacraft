...
class spell_hun_check_pet_los : public SpellScript
{
  ...
  void Register() override { OnCheckCast += SpellCheckCastFn(spell_hun_check_pet_los::CheckCast); }
};
      ...
      void HandleEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
  {
    if (Unit* caster = GetCaster())
      caster->CastSpell(GetTarget(),
                        sSpellMgr->GetSpellWithRank(SPELL_HUNTER_WYVERN_STING_DOT, GetSpellInfo()->GetRank()), true);
  }

  void Register() override
  {
    AfterEffectRemove += AuraEffectRemoveFn(spell_hun_wyvern_sting::HandleEffectRemove, EFFECT_0, SPELL_AURA_MOD_STUN,
...
class spell_hun_animal_handler : public AuraScript
{
  ...
  {
    DoEffectCalcAmount +=
        AuraEffectCalcAmountFn(spell_hun_animal_handler::CalculateAmount, EFFECT_0, SPELL_AURA_MOD_ATTACK_POWER_PCT);
  }
};
...
class spell_hun_generic_scaling : public AuraScript
{
  ...
  void CalculateStatAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
  {
    if (Unit* owner = GetUnitOwner()->GetOwner())
    {
      ...
      amount = CalculatePct(
          std::max<int32>(0, owner->GetStat(Stats(aurEff->GetSpellInfo()->Effects[aurEff->GetEffIndex()].MiscValue))),
          modifier);
  ...
  void CalculateAPAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
  {
    if (Unit* owner = GetUnitOwner()->GetOwner())
    {
      ...
      if (AuraEffect* HvWEff =
              owner->GetAuraEffect(SPELL_AURA_MOD_ATTACK_POWER_OF_STAT_PERCENT, SPELLFAMILY_HUNTER, 3647, EFFECT_0))
  ...
  void Register() override
  {
    if (m_scriptSpellId != 34902)
      DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_generic_scaling::CalculateResistanceAmount, EFFECT_ALL,
    ...
    else
    {
      ...
      DoEffectCalcAmount +=
          AuraEffectCalcAmountFn(spell_hun_generic_scaling::CalculateAPAmount, EFFECT_ALL, SPELL_AURA_MOD_ATTACK_POWER);
      DoEffectCalcAmount +=
          AuraEffectCalcAmountFn(spell_hun_generic_scaling::CalculateSPAmount, EFFECT_ALL, SPELL_AURA_MOD_DAMAGE_DONE);
    }
...
class spell_hun_taming_the_beast : public AuraScript
{
  ...
  void Register() override
  {
    OnEffectApply += AuraEffectApplyFn(spell_hun_taming_the_beast::HandleOnEffectApply, EFFECT_0, SPELL_AURA_MOD_CHARM,
                                       AURA_EFFECT_HANDLE_REAL);
    OnEffectRemove += AuraEffectRemoveFn(spell_hun_taming_the_beast::HandleOnEffectRemove, EFFECT_0,
...
class spell_hun_aspect_of_the_beast : public AuraScript
{
  ...
  void Register() override
  {
    ...
    {
      AfterEffectApply += AuraEffectApplyFn(spell_hun_aspect_of_the_beast::OnApply, EFFECT_0, SPELL_AURA_UNTRACKABLE,
                                            AURA_EFFECT_HANDLE_REAL);
      AfterEffectRemove += AuraEffectRemoveFn(spell_hun_aspect_of_the_beast::OnRemove, EFFECT_0, SPELL_AURA_UNTRACKABLE,
                                              AURA_EFFECT_HANDLE_REAL);
    }
    else
      AfterEffectApply += AuraEffectApplyFn(spell_hun_aspect_of_the_beast::OnPetApply, EFFECT_0, SPELL_AURA_UNTRACKABLE,
                                            AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_hun_ascpect_of_the_viper : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override
  {
    return ValidateSpellInfo({SPELL_HUNTER_ASPECT_OF_THE_VIPER_ENERGIZE, SPELL_HUNTER_GLYPH_OF_ASPECT_OF_THE_VIPER,
                              SPELL_HUNTER_VIPER_ATTACK_SPEED, SPELL_HUNTER_VICIOUS_VIPER});
  }
  ...
  void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
  {
    ...
    GetTarget()->CastCustomSpell(SPELL_HUNTER_ASPECT_OF_THE_VIPER_ENERGIZE, SPELLVALUE_BASE_POINT0, mana, GetTarget(),
                                 true, nullptr, aurEff);
  }
  ...
  void Register() override
  {
    ...
    AfterEffectApply += AuraEffectApplyFn(spell_hun_ascpect_of_the_viper::OnApply, EFFECT_0, SPELL_AURA_OBS_MOD_POWER,
                                          AURA_EFFECT_HANDLE_REAL);
    AfterEffectRemove += AuraEffectRemoveFn(spell_hun_ascpect_of_the_viper::OnRemove, EFFECT_0,
                                            SPELL_AURA_OBS_MOD_POWER, AURA_EFFECT_HANDLE_REAL);
  }
};
      ...
      for (Unit::AuraApplicationMap::iterator i = Auras.begin(); i != Auras.end(); ++i)
      {
        ...
        if (AuraEffect* aurEff = aura->GetEffect(0))
        {
          ...
          if (familyFlag[0] & 0x4000)
          {
            ...
            basePoint =
                unitTarget->SpellDamageBonusTaken(caster, aura->GetSpellInfo(), basePoint, DOT, aura->GetStackAmount());
          }
          ...
          else if (familyFlag[1] & 0x00000080)
          {
            ...
            int32 casterBasePoint =
                aurEff->GetAmount() * unitTarget->GetMaxPower(POWER_MANA) / 50; /// @todo: Caster uses unitTarget?
            if (basePoint > casterBasePoint)
              basePoint = casterBasePoint;
            ApplyPct(basePoint, TickCount * 60);
          }
          // Scorpid Sting - Attempts to Disarm the target for 10 sec. This effect cannot occur more than once per 1
          // minute.
...
class spell_hun_improved_mend_pet : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_HUNTER_IMPROVED_MEND_PET}); }

  bool CheckProc(ProcEventInfo& /*eventInfo*/) { return roll_chance_i(GetEffect(EFFECT_0)->GetAmount()); }
...
class spell_hun_last_stand_pet : public SpellScript
{
  ...
  void HandleDummy(SpellEffIndex /*effIndex*/)
  {
    ...
    caster->CastCustomSpell(caster, SPELL_HUNTER_PET_LAST_STAND_TRIGGERED, &healthModSpellBasePoints0, nullptr, nullptr,
                            true, nullptr);
  }
...
class spell_hun_readiness : public SpellScript
{
  ...
  void HandleDummy(SpellEffIndex /*effIndex*/)
  {
    ...
    for (auto const& [spellId, cooldown] : cooldowns)
    {
      ...
      if (spellInfo && spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && spellInfo->Id != SPELL_HUNTER_READINESS &&
          spellInfo->Id != SPELL_HUNTER_BESTIAL_WRATH && spellInfo->Id != SPELL_DRAENEI_GIFT_OF_THE_NAARU)
      {
...
class spell_hun_sniper_training : public AuraScript
{
  ...
  {
    OnEffectPeriodic +=
        AuraEffectPeriodicFn(spell_hun_sniper_training::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_hun_sniper_training::HandleUpdatePeriodic, EFFECT_0,
...
class spell_hun_pet_heart_of_the_phoenix : public SpellScript
{
  ...
  void HandleScript(SpellEffIndex /*effIndex*/)
  {
    ...
    if (Unit* owner = caster->GetOwner())
      if (!caster->HasAura(SPELL_HUNTER_PET_HEART_OF_THE_PHOENIX_DEBUFF))
      {
        owner->CastCustomSpell(SPELL_HUNTER_PET_HEART_OF_THE_PHOENIX_TRIGGERED, SPELLVALUE_BASE_POINT0, 100, caster,
                               true);
...
class spell_hun_misdirection : public AuraScript
{
  ...
  bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_HUNTER_MISDIRECTION_PROC}); }
  ...
  bool CheckProc(ProcEventInfo& /*eventInfo*/) { return GetTarget()->GetRedirectThreatTarget(); }
...
class spell_hun_misdirection_proc : public AuraScript
{
  ...
  void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/) { GetTarget()->ResetRedirectThreat(); }

  void Register() override
  {
    AfterEffectRemove +=
        AuraEffectRemoveFn(spell_hun_misdirection_proc::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_hun_disengage : public SpellScript
{
  ...
  void Register() override { OnCheckCast += SpellCheckCastFn(spell_hun_disengage::CheckCast); }
};
...
class spell_hun_tame_beast : public SpellScript
{
  ...
  void Register() override { OnCheckCast += SpellCheckCastFn(spell_hun_tame_beast::CheckCast); }
};
...
class spell_hun_viper_attack_speed : public AuraScript
{
  ...
  {
    AfterEffectApply +=
        AuraEffectApplyFn(spell_hun_viper_attack_speed::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    AfterEffectRemove +=
        AuraEffectRemoveFn(spell_hun_viper_attack_speed::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
  }
};
...
class spell_hun_glyph_of_arcane_shot : public AuraScript
{
  ...
  bool CheckProc(ProcEventInfo& eventInfo)
  {
    if (Unit* procTarget = eventInfo.GetProcTarget())
    {
      ...
      const auto found =
          std::find_if(std::begin(procTarget->GetAppliedAuras()), std::end(procTarget->GetAppliedAuras()),
...
class spell_hun_volley_trigger : public SpellScript
{
  ...
  void Register() override
  {
    OnObjectAreaTargetSelect +=
        SpellObjectAreaTargetSelectFn(spell_hun_volley_trigger::SelectTarget, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
...
class spell_hun_lock_and_load : public AuraScript
{
  ...
  void Register() override
  {
    ...
    OnEffectProc += AuraEffectProcFn(spell_hun_lock_and_load::HandleProcs<PROC_FLAG_DONE_TRAP_ACTIVATION>, EFFECT_0,
...
class spell_hun_intimidation : public AuraScript
{
  ...
  void Register() override { DoCheckProc += AuraCheckProcFn(spell_hun_intimidation::CheckProc); }
};
...
class spell_hun_bestial_wrath : public SpellScript
{
  ...
  void Register() override { OnCheckCast += SpellCheckCastFn(spell_hun_bestial_wrath::CheckCast); }
};
...
class spell_hun_target_self_and_pet : public SpellScript
{
  ...
  void Register() override
  {
    OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_hun_target_self_and_pet::FilterTargets, EFFECT_ALL,
