...
*/
InstanceSave* InstanceSaveMgr::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty,
                                               bool startup /*=false*/)
{
  ...
  if (!entry)
  {
    LOG_ERROR("instance.save", "InstanceSaveMgr::AddInstanceSave: wrong mapid = {}, instanceid = {}!", mapId,
              instanceId);
    return nullptr;
  }
  ...
  if (instanceId == 0)
  {
    LOG_ERROR("instance.save", "InstanceSaveMgr::AddInstanceSave: mapid = {}, wrong instanceid = {}!", mapId,
              instanceId);
    return nullptr;
  }
  ...
  {
    LOG_ERROR("instance.save", "InstanceSaveMgr::AddInstanceSave: mapid = {}, instanceid = {}, wrong difficulty {}!",
...
InstanceSave::InstanceSave(uint16 MapId, uint32 InstanceId, Difficulty difficulty, time_t resetTime,
                           time_t extendedResetTime)
    : m_resetTime(resetTime), m_extendedResetTime(extendedResetTime), m_instanceid(InstanceId), m_mapid(MapId),
      m_difficulty(IsSharedDifficultyMap(MapId) ? Difficulty(difficulty % 2) : difficulty), m_canReset(true),
      m_instanceData(""), m_completedEncounterMask(0)
{
...
InstanceTemplate const* InstanceSave::GetTemplate() { return sObjectMgr->GetInstanceTemplate(m_mapid); }

MapEntry const* InstanceSave::GetMapEntry() { return sMapStore.LookupEntry(m_mapid); }
...
void InstanceSaveMgr::LoadInstances()
{
  ...
  // Delete expired normal instances (normals expire after 3 days even if someone is still bound to them, cleared on
  // startup)
  ...
  CharacterDatabase.DirectExecute("DELETE tmp.* FROM character_instance AS tmp LEFT JOIN instance ON tmp.instance = "
                                  "instance.id WHERE tmp.instance > 0 AND instance.id IS NULL");
  ...
  CharacterDatabase.DirectExecute("UPDATE characters AS tmp LEFT JOIN instance ON tmp.instance_id = instance.id SET "
                                  "tmp.instance_id = 0 WHERE tmp.instance_id > 0 AND instance.id IS NULL");
...
void InstanceSaveMgr::LoadResetTimes()
{
    ...
    do
    {
      ...
      if (!mapDiff)
      {
        LOG_ERROR("instance.save",
                  "InstanceSaveMgr::LoadResetTimes: invalid mapid({})/difficulty({}) pair in instance_reset!", mapid,
                  difficulty);
        CharacterDatabase.DirectExecute("DELETE FROM instance_reset WHERE mapid = '{}' AND difficulty = '{}'", mapid,
                                        difficulty);
        continue;
      }
  ...
  for (MapDifficultyMap::const_iterator itr = sMapDifficultyMap.begin(); itr != sMapDifficultyMap.end(); ++itr)
  {
    ...
    if (!t)
    {
      ...
      CharacterDatabase.DirectExecute("INSERT INTO instance_reset VALUES ('{}', '{}', '{}')", mapid, difficulty,
                                      (uint32)t);
    }
    ...
    if (t < now)
    {
      ...
      CharacterDatabase.DirectExecute(
          "UPDATE instance_reset SET resettime = '{}' WHERE mapid = '{}' AND difficulty = '{}'", (uint32)t, mapid,
          difficulty);
    }
  ...
  if (result)
  {
    do
    {
      ...
      if (InstanceSave* save = GetInstanceSave(instanceId))
      {
        ...
        if (bind.save) // pussywizard: another bind for the same map and difficulty! may happen because of mysql thread
                       // races
        {
...
void InstanceSaveMgr::_ResetSave(InstanceSaveHashMap::iterator& itr)
{
  ...
  {
    iter2 = iter++;
    PlayerUnbindInstanceNotExtended(*iter2, itr->second->GetMapId(), itr->second->GetDifficulty(),
  ...
  {
    // delete character_instance per id where extended = 0, transtaction with set extended = 0, transaction is used to
    // avoid mysql thread races
...
void InstanceSaveMgr::_ResetOrWarnAll(uint32 mapid, Difficulty difficulty, bool warn, time_t resetTime)
{
  ...
  if (!warn)
  {
    ...
    {
      LOG_ERROR("instance.save", "InstanceSaveMgr::ResetOrWarnAll: not valid difficulty or no reset delay for map {}",
                mapid);
      return;
    }
...
InstancePlayerBind* InstanceSaveMgr::PlayerBindToInstance(ObjectGuid guid, InstanceSave* save, bool permanent,
                                                          Player* player /*= nullptr*/)
{
  ...
  else
  {
    ...
    CharacterDatabasePreparedStatement* stmt =
    CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_INSTANCE_BY_GUID_MAP_DIFF); // DELETE ci FROM
    character_instance ci JOIN instance i ON ci.instance = i.id WHERE ci.guid = ? AND i.map = ? AND i.difficulty = ?
...
void InstanceSaveMgr::PlayerUnbindInstance(ObjectGuid guid, uint32 mapid, Difficulty difficulty, bool deleteFromDB,
                                           Player* player /*= nullptr*/)
{
...
void InstanceSaveMgr::PlayerUnbindInstanceNotExtended(ObjectGuid guid, uint32 mapid, Difficulty difficulty,
                                                      Player* player /*= nullptr*/)
{
  BoundInstancesMapWrapper*   w   = playerBindStorage[guid];
  BoundInstancesMap::iterator itr = w->m[difficulty].find(mapid);
