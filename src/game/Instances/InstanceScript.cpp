...
ObjectGuid InstanceScript::GetGuidData(uint32 type) const { return GetObjectGuid(type); }

Creature* InstanceScript::GetCreature(uint32 type) { return instance->GetCreature(GetObjectGuid(type)); }

GameObject* InstanceScript::GetGameObject(uint32 type) { return instance->GetGameObject(GetObjectGuid(type)); }
...
void InstanceScript::LoadObjectData(ObjectData const* creatureData, ObjectData const* gameObjectData)
{
  ...
  LOG_DEBUG("scripts", "InstanceScript::LoadObjectData: {} objects loaded.",
...
void InstanceScript::UpdateDoorState(GameObject* door)
{
  ...
  // xinef: doors can be assigned to few bosses, if any of them demands doors closed - they should be closed (added &
...
void InstanceScript::RemoveObject(WorldObject* obj, uint32 type) { AddObject(obj, type, false); }
...
void InstanceScript::RemoveMinion(Creature* minion) { AddMinion(minion, false); }
...
void InstanceScript::StorePersistentData(uint32 index, uint32 data)
{
  if (index > persistentData.size())
  {
    LOG_ERROR("scripts",
              ...
              index, persistentData.size(), data);
    return;
  }
...
void InstanceScript::DoUseDoorOrButton(ObjectGuid uiGuid, uint32 uiWithRestoreTime, bool bUseAlternativeState)
{
  ...
  if (go)
  {
    if (go->GetGoType() == GAMEOBJECT_TYPE_DOOR || go->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
    {
      ...
      LOG_ERROR("scripts.ai", "SD2: Script call DoUseDoorOrButton, but gameobject entry {} is type {}.", go->GetEntry(),
                go->GetGoType());
  }
}
...
void InstanceScript::DoRespawnGameObject(ObjectGuid uiGuid, uint32 uiTimeToDespawn)
{
  if (GameObject* go = instance->GetGameObject(uiGuid))
  {
    ...
    case GAMEOBJECT_TYPE_FISHINGNODE:
      ...
      LOG_ERROR("scripts", "InstanceScript: DoRespawnGameObject can't respawn gameobject entry {}, because type is {}.",
...
void InstanceScript::DoUpdateAchievementCriteria(AchievementCriteriaTypes type, uint32 miscValue1 /*= 0*/,
...
void InstanceScript::DoCastSpellOnPlayer(Player* player, uint32 spell, bool includePets /*= false*/,
...
bool InstanceScript::CheckAchievementCriteriaMeet(uint32 criteria_id, Player const* /*source*/,
                                                  Unit const* /*target*/ /*= nullptr*/, uint32 /*miscvalue1*/ /*= 0*/)
{
  LOG_ERROR("scripts.ai",
            "Achievement system call InstanceScript::CheckAchievementCriteriaMeet but instance script for map {} not "
            "have implementation for achievement criteria {}",
...
void InstanceScript::SendEncounterUnit(uint32 type, Unit* unit /*= nullptr*/, uint8 param1 /*= 0*/,
                                       uint8 param2 /*= 0*/)
{
