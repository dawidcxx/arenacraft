...
void WardenWin::RequestChecks()
{
  ...
  for (uint16 const checkId : _CurrentChecks)
  {
    ...
    case LUA_EVAL_CHECK:
    {
      buff << uint8(sizeof(_luaEvalPrefix) - 1 + check->Str.size() + sizeof(_luaEvalMidfix) - 1 + check->IdStr.size() +
...
void WardenWin::HandleData(ByteBuffer& buff)
{
  ...
  // TIMING_CHECK
  {
    ...
    if (result == 0x00)
    {
      ...
      // ApplyPenalty(0, "TIMING CHECK FAIL result"); Commented out because of too many false postives. Mostly caused by
      // client stutter.
      return;
    }
  ...
  for (uint16 const checkId : _CurrentChecks)
  {
    ...
    case LUA_EVAL_CHECK:
    {
      ...
      LOG_DEBUG("warden", "LUA_EVAL_CHECK CheckId {} account Id {} got in-warden dummy response", checkId,
    ...
    case MPQ_CHECK:
    {
      ...
      WardenCheckResult const* rs = sWardenCheckMgr->GetWardenResultById(checkId);
      if (memcmp(buff.contents() + buff.rpos(), rs->Result.ToByteArray<20>(false).data(),
