...
void MailDraft::prepareItems(Player* receiver, CharacterDatabaseTransaction trans)
{
  ...
  // Only quest in the game up to BFA which sends raw gold through mail. So would just be overkill to introduce a new
                                   ...
                                   ObjectGuid::LowType receiver_guid, CharacterDatabaseTransaction trans)
{
  ...
  SendMailTo(trans, MailReceiver(receiver, receiver_guid), MailSender(MAIL_NORMAL, sender_guid),
             MAIL_CHECK_MASK_RETURNED, 0);
}
...
{
  sScriptMgr->OnBeforeMailDraftSendMailTo(this, receiver, sender, checked, deliver_delay, custom_expiration,
