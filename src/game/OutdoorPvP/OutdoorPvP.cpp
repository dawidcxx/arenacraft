...
bool OPvPCapturePoint::AddCreature(uint32 type, uint32 entry, uint32 map, float x, float y, float z, float o,
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
  ...
  // beats me why this is needed, but with the recent removal "cleanup" some creatures stay in the map if "properly"
  // deleted so this is a big fat workaround, if AddObjectToRemoveList and DoDelayedMovesAndRemoves worked correctly,
...
bool OPvPCapturePoint::HandleCustomSpell(Player* /*player*/, uint32 /*spellId*/, GameObject* /*go*/) { return false; }
...
bool OPvPCapturePoint::HandleGossipOption(Player* /*player*/, Creature* /*creature*/, uint32 /*id*/) { return false; }

bool OPvPCapturePoint::CanTalkTo(Player* /*player*/, Creature* /*c*/, GossipMenuItems const& /*gso*/) { return false; }

bool OPvPCapturePoint::HandleDropFlag(Player* /*player*/, uint32 /*id*/) { return false; }
...
void OPvPCapturePoint::SetSlider(float slider) { _value = std::clamp<float>(slider, -_maxValue, _maxValue); }

bool OutdoorPvP::HandleAreaTrigger(Player* /*player*/, uint32 /*trigger*/) { return false; }
...
void OutdoorPvP::RegisterZone(uint32 zoneId) { sOutdoorPvPMgr->AddZone(zoneId, this); }
...
void OutdoorPvP::TeamApplyBuff(TeamId teamId, uint32 spellId, uint32 spellId2, Player* sameMapPlr)
{
  ...
  TeamCastSpell(teamId == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE, spellId2 ? -(int32)spellId2 : -(int32)spellId,
                sameMapPlr);
}
...
void OutdoorPvP::OnCreatureRemove(Creature* creature) { _creatureScriptStore.erase(creature->GetGUID().GetCounter()); }
