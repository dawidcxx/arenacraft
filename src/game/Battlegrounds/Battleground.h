...
enum BattlegroundSpells
{
  ...
  SPELL_HONORABLE_DEFENDER_25Y = 68652, // +50% honor when standing at a capture point that you control,
                                        // 25yards radius (added in 3.2)
  SPELL_HONORABLE_DEFENDER_60Y = 66157  // +50% honor when standing at a capture point that you control,
...
public:
  ...
  [[nodiscard]] uint32 GetMinPlayersPerTeam() const
  {
    ...
    return (lowLevelsOverride && !isTemplate() && !isMaxLevel() && !isArena()) ? lowLevelsOverride
                                                                               : m_MinPlayersPerTeam;
  }
  ...
  virtual void EndBattleground(TeamId winnerTeamId) { EndBattleground(GetPvPTeamId(winnerTeamId)); }
  ...
  [[nodiscard]] uint32
  GetAlivePlayersCountByTeam(TeamId teamId) const; // used in arenas to correctly handle death in spirit of redemption
  ...
  bool      AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1,
                      float rotation2, float rotation3, uint32 respawnTime = 0, GOState goState = GO_STATE_READY);
  ...
  [[nodiscard]] virtual ObjectGuid GetFlagPickerGUID(TeamId /*teamId*/ = TEAM_NEUTRAL) const
  {
...
private:
  ...
  uint32             m_ClientInstanceID; // the instance-id which is sent to the client and
                                         // without any other internal use
  uint32 m_StartTime;
  ...
  bool                  _InBGFreeSlotQueue{false}; // used to make sure that BG is only once inserted into the
  ...
  // Invited counters are useful for player invitation to BG - do not allow, if
  // BG is started to one faction to have 2 more players than another faction
  // Invited counters will be changed only when removing already invited player
  // from queue, removing player from battleground and inviting player to BG
