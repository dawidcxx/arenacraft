...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* group, BattlegroundTypeId bgTypeId,
                                            ...
                                            uint32 arenaTeamId /*= 0*/, uint32 opponentsArenaTeamId /*= 0*/)
{
  ...
  sScriptMgr->OnAddGroup(this, ginfo, index, leader, group, bgTypeId, bracketEntry, arenaType, isRated, isPremade,
                         arenaRating, matchmakerRating, arenaTeamId, opponentsArenaTeamId);

  LOG_DEBUG("bg.battleground", "Adding Group to BattlegroundQueue bgTypeId: {}, bracket_id: {}, index: {}", bgTypeId,
            bracketId, index);
  ...
  if (group)
  {
    group->DoForAllMembers(
        [this, ginfo](Player* member)
        {
...
void BattlegroundQueue::PlayerInvitedToBGUpdateAverageWaitTime(GroupQueueInfo* ginfo)
{
  ...
  // team_index: bg alliance - TEAM_ALLIANCE, bg horde - TEAM_HORDE, arena skirmish - TEAM_ALLIANCE, arena rated -
  // TEAM_HORDE
  uint8 team_index;
  if (!ginfo->ArenaType)
...
void BattlegroundQueue::RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount)
{
  ...
  if (groupInfo->IsInvitedToBGInstanceGUID && groupInfo->IsRated && decreaseInvitedCount)
  {
    ...
    {
      LOG_DEBUG("bg.battleground", "UPDATING memberLost's personal arena rating for {} by opponents rating: {}",
                guid.ToString(), groupInfo->OpponentsTeamRating);
    ...
    if (diffAli < diffHorde)
    {
      ...
      if (m_SelectionPools[TEAM_ALLIANCE].KickGroup(diffHorde - diffAli))
      {
        for (; aliIndex < aliCount &&
               m_SelectionPools[TEAM_ALLIANCE].AddGroup((*Ali_itr), (aliFree >= diffHorde) ? aliFree - diffHorde : 0);
             aliIndex++)
          ++Ali_itr;
      }
    ...
    else
    {
      ...
      if (m_SelectionPools[TEAM_HORDE].KickGroup(diffAli - diffHorde))
      {
        for (; hordeIndex < hordeCount &&
               m_SelectionPools[TEAM_HORDE].AddGroup((*Horde_itr), (hordeFree >= diffAli) ? hordeFree - diffAli : 0);
             hordeIndex++)
          ++Horde_itr;
      }
                                          ...
                                          uint32 MaxPlayersPerTeam)
{
      ...
      !m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].empty())
  {
        ...
        horde_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].end())
    {
      ...
      uint32 maxPlayers =
          std::min(m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount(), m_SelectionPools[TEAM_HORDE].GetPlayerCount());
...
bool BattlegroundQueue::CheckNormalMatch(Battleground* bgTemplate, BattlegroundBracketId bracket_id, uint32 minPlayers,
                                         uint32 maxPlayers)
{
                                                ...
                                                uint32 arenaRating)
{
  ...
  if (!bracketEntry)
  {
    LOG_ERROR("bg.battleground", "Battleground: Update: bg bracket entry not found for map {} bracket id {}",
  ...
  else if (bg_template->isArena())
  {
    ...
    // into account else leave the discard time on 0, this way all ratings will be discarded this has to be signed value
    // - when the server starts, this value would be negative and thus overflow
    ...
    if (found == 2)
    {
      ...
      LOG_DEBUG("bg.battleground", "setting oposite teamrating for team {} to {}", aTeam->ArenaTeamId,
                aTeam->OpponentsTeamRating);
      LOG_DEBUG("bg.battleground", "setting oposite teamrating for team {} to {}", hTeam->ArenaTeamId,
...
void BattlegroundQueue::BattlegroundQueueAnnouncerUpdate(uint32 diff, BattlegroundQueueTypeId bgQueueTypeId,
    ...
    if (_queueAnnouncementTimer[bracket_id] >= 0)
    {
      if (_queueAnnouncementTimer[bracket_id] <= static_cast<int32>(diff))
      {
        ...
        ChatHandler(nullptr).SendWorldTextOptional(LANG_BG_QUEUE_ANNOUNCE_WORLD, ANNOUNCER_FLAG_DISABLE_BG_QUEUE,
                                                   bgName.c_str(), q_min_level, q_max_level, qPlayers, MaxPlayers);
      }
      else
      {
...
void BattlegroundQueue::SendMessageBGQueue(Player* leader, Battleground* bg, PvPDifficultyEntry const* bracketEntry)
{
  ...
  LOG_DEBUG("bg.battleground", "> Queue status for {} (Lvl: {} to {}) Queued: {} (Need at least {} more)", bgName,
            q_min_level, q_max_level, qAlliance + qHorde, MaxPlayers - qTotal);
  ...
  if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY))
  {
    ChatHandler(leader->GetSession())
        .PSendSysMessage(LANG_BG_QUEUE_ANNOUNCE_SELF, bgName, q_min_level, q_max_level, qAlliance,
  ...
  else // Show queue status to server (when joining battleground queue)
  {
    ...
    else
    {
      ...
      ChatHandler(nullptr).SendWorldTextOptional(LANG_BG_QUEUE_ANNOUNCE_WORLD, ANNOUNCER_FLAG_DISABLE_BG_QUEUE,
                                                 bgName.c_str(), q_min_level, q_max_level, qAlliance + qHorde,
                                                 MaxPlayers);
    }
  }
}

void BattlegroundQueue::SendJoinMessageArenaQueue(Player* leader, GroupQueueInfo* ginfo,
                                                  PvPDifficultyEntry const* bracketEntry, bool isRated)
{
  ...
  if (!isRated)
  {
    ...
    {
      ChatHandler(leader->GetSession())
          .PSendSysMessage(LANG_ARENA_QUEUE_ANNOUNCE_SELF, bgName, arenatype, q_min_level, q_max_level, qPlayers,
    ...
    else
    {
      ...
      ChatHandler(nullptr).SendWorldTextOptional(LANG_ARENA_QUEUE_ANNOUNCE_WORLD, ANNOUNCER_FLAG_DISABLE_ARENA_QUEUE,
                                                 bgName.c_str(), arenatype.c_str(), q_min_level, q_max_level, qPlayers,
                                                 playersNeed);
    }
  }
  else
  {
    ...
    case 3:
      ChatHandler(nullptr).SendWorldTextOptional(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN_NAME_RATING,
                                                 ANNOUNCER_FLAG_DISABLE_ARENA_QUEUE, TeamName.c_str(), ArenaType,
                                                 ArenaType, ArenaTeamRating);
      break;
    case 2:
      ChatHandler(nullptr).SendWorldTextOptional(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN_NAME,
                                                 ANNOUNCER_FLAG_DISABLE_ARENA_QUEUE, TeamName, ArenaType, ArenaType);
      break;
    case 1:
      ChatHandler(nullptr).SendWorldTextOptional(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN_RATING,
                                                 ANNOUNCER_FLAG_DISABLE_ARENA_QUEUE, ArenaType, ArenaType,
                                                 ArenaTeamRating);
      break;
    default:
      ChatHandler(nullptr).SendWorldTextOptional(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN,
                                                 ANNOUNCER_FLAG_DISABLE_ARENA_QUEUE, ArenaType, ArenaType);
      break;
    }
  }
...
void BattlegroundQueue::SendExitMessageArenaQueue(GroupQueueInfo* ginfo)
{
  ...
  if (ArenaType && ginfo->Players.empty())
  {
    ...
    case 1:
      ...
      break;
    default:
      ChatHandler(nullptr).SendWorldTextOptional(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_EXIT,
                                                 ANNOUNCER_FLAG_DISABLE_ARENA_QUEUE, ArenaType, ArenaType);
      break;
    }
  }
...
void BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, TeamId teamId)
{
  ...
  for (auto const& itr : ginfo->Players)
  {
    ...
    BGQueueInviteEvent* inviteEvent = new BGQueueInviteEvent(player->GetGUID(), ginfo->IsInvitedToBGInstanceGUID,
                                                             bgTypeId, ginfo->ArenaType, ginfo->RemoveInviteTime);
    bgQueue.AddEvent(inviteEvent, INVITATION_REMIND_TIME);

    // create automatic remove events
    BGQueueRemoveEvent* removeEvent = new BGQueueRemoveEvent(player->GetGUID(), ginfo->IsInvitedToBGInstanceGUID,
                                                             bgTypeId, bgQueueTypeId, ginfo->RemoveInviteTime);
    ...
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME, 0,
                                                    ginfo->ArenaType, TEAM_NEUTRAL, bg->isRated());
    player->GetSession()->SendPacket(&data);

    // pussywizard:
    if (bg->isArena() && bg->isRated())
      bg->ArenaLogEntries[player->GetGUID()].Fill(player->GetName(), player->GetGUID().GetCounter(),
  ...
  if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)
  {
    ...
    if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
    {
      ...
      sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN,
                                                      ...
                                                      TEAM_NEUTRAL, bg->isRated(), m_BgTypeId);
      player->GetSession()->SendPacket(&data);
    }
  }
  ...
  if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES) // player is in queue
  {
    ...
    if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
    {
      ...
      LOG_DEBUG("bg.battleground",
                "Battleground: removing player {} from bg queue for instance {} because of not pressing enter battle "
                "in time.",
                player->GetGUID().ToString(), m_BgInstanceGUID);
