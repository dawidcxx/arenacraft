  ...
  bool OnTrigger(Player* player, AreaTrigger const* /*trigger*/) override
  {
    if (!player->isDead() && player->GetQuestStatus(QUEST_SCENT_OF_LARKORWI) == QUEST_STATUS_INCOMPLETE)
    {
      if (!player->FindNearestCreature(NPC_LARKORWI_MATE, 15))
        player->SummonCreature(NPC_LARKORWI_MATE, player->GetPositionX() + 5, player->GetPositionY(),
                               player->GetPositionZ(), 3.3f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000);
    }
...
public:
  ...
  bool OnTrigger(Player* player, AreaTrigger const* /*trigger*/) override
  {
    ...
    if (player->GetQuestStatus(QUEST_NATS_BARGAIN) == QUEST_STATUS_INCOMPLETE)
    {
      ...
      {
        if (Creature* shark = player->SummonCreature(NPC_LURKING_SHARK, -4246.243f, -3922.356f, -7.488f, 5.0f,
...
public:
  ...
  bool OnTrigger(Player* player, AreaTrigger const* /*trigger*/) override
  {
    ...
    {
      if (Creature* tervosh = player->SummonCreature(NPC_TERVOSH, -3476.51f, -4105.94f, 17.1f, 5.3816f,
