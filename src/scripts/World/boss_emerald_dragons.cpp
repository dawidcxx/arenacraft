...
struct emerald_dragonAI : public WorldBossAI
{
  ...
  void ExecuteEvent(uint32 eventId) override
  {
    ...
    case EVENT_SEEPING_FOG:
      ...
      // Despawntime is 2 minutes, so reschedule it for new cast after 2 minutes + a minor "random time" (30 seconds at
      // max)
...
public:
    ...
    void ScheduleEvents()
    {
      ...
      scheduler.Schedule(1s,
                         [this](TaskContext context)
                         {
                           ...
                           // Seeping fog movement is slow enough for a player to be able to walk backwards and still
                           // outpace it
                           me->SetWalk(true);
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new npc_dream_fogAI(creature); }
};
...
public:
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new boss_ysondreAI(creature); }
};
...
public:
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new boss_lethonAI(creature); }
};
...
public:
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new npc_spirit_shadeAI(creature); }
};
...
public:
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new boss_emerissAI(creature); }
};
...
public:
  ...
  struct boss_taerarAI : public emerald_dragonAI
  {
    ...
    void SummonedCreatureDies(Creature* /*summon*/, Unit*) override { --_shades; }
    ...
    void UpdateAI(uint32 diff) override
    {
      ...
      if (_banished)
      {
        // If all three shades are dead, OR it has taken too long, end the current event and get Taerar back into
        // business
        ...
        // Update the events before we return (handled under emerald_dragonAI::UpdateAI(diff); if we're not inside this
        // check)
        events.Update(diff);
  ...
  CreatureAI* GetAI(Creature* creature) const override { return new boss_taerarAI(creature); }
};
...
class spell_dream_fog_sleep : public SpellScript
{
  ...
  void FilterTargets(std::list<WorldObject*>& targets) { targets.remove_if(Acore::UnitAuraCheck(true, SPELL_SLEEP)); }
...
class spell_shadow_bolt_whirl : public AuraScript
{
  ...
  {
    return ValidateSpellInfo({SPELL_SHADOW_BOLT_WHIRL1, SPELL_SHADOW_BOLT_WHIRL2, SPELL_SHADOW_BOLT_WHIRL3,
                              SPELL_SHADOW_BOLT_WHIRL4, SPELL_SHADOW_BOLT_WHIRL5, SPELL_SHADOW_BOLT_WHIRL6,
                              SPELL_SHADOW_BOLT_WHIRL7, SPELL_SHADOW_BOLT_WHIRL8});
  }
  ...
  void HandlePeriodic(AuraEffect const* aurEff)
  {
    ...
    if (!caster || !target)
      return;
    std::array<uint32, 8> spellForTick = {SPELL_SHADOW_BOLT_WHIRL1, SPELL_SHADOW_BOLT_WHIRL2, SPELL_SHADOW_BOLT_WHIRL3,
                                          SPELL_SHADOW_BOLT_WHIRL4, SPELL_SHADOW_BOLT_WHIRL5, SPELL_SHADOW_BOLT_WHIRL6,
                                          SPELL_SHADOW_BOLT_WHIRL7, SPELL_SHADOW_BOLT_WHIRL8};
