...
public:
  ...
  {
    static ChatCommandTable sendCommandTable = {{"items", HandleSendItemsCommand, SEC_GAMEMASTER, Console::Yes},
  ...
  static bool HandleSendItemsCommand(ChatHandler* handler, Optional<PlayerIdentifier> target, QuotedString subject,
                                     QuotedString text, Tail items)
  {
    if (!target)
    {
    ...
    ObjectGuid::LowType senderGuid = handler->GetSession() ? handler->GetSession()->GetPlayer()->GetGUID().GetCounter()
  ...
  static bool HandleSendMailCommand(ChatHandler* handler, Optional<PlayerIdentifier> target, QuotedString subject,
                                    QuotedString text)
  {
    if (!target)
    {
  ...
  static bool HandleSendMoneyCommand(ChatHandler* handler, Optional<PlayerIdentifier> target, QuotedString subject,
                                     QuotedString text, uint32 money)
  {
    if (!target)
    {
      target = PlayerIdentifier::FromTargetOrSelf(handler);
