...
WMOGroup::WMOGroup(std::string const& filename)
    : filename(std::move(filename)), MOPY(nullptr), MOVI(nullptr), MoviEx(nullptr), MOVT(nullptr), MOBA(nullptr),
      MobaEx(nullptr), hlq(nullptr), LiquEx(nullptr), LiquBytes(nullptr), groupName(0), descGroupName(0), mogpFlags(0),
      moprIdx(0), moprNItems(0), nBatchA(0), nBatchB(0), nBatchC(0), fogIdx(0), groupLiquid(0), groupWMOID(0),
...
int WMOGroup::ConvertToVMAPGroupWmo(FILE* output, bool preciseVectorData)
{
  ...
  if (liquflags & 3)
  {
    ...
    /* std::ofstream llog("Buildings/liquid.log", ios_base::out | ios_base::app);
    llog << filename;
    llog << ":\nliquidEntry: " << liquidEntry << " type: " << hlq->type << " (root:" << rootWMO->liquidType << " group:"
    << liquidType << ")\n"; llog.close(); */
...
bool WMOGroup::ShouldSkip(WMORoot const* root) const
{
  ...
  if (groupName >= 0 && groupName < int32(root->GroupNames.size()) &&
...
void MapObject::Extract(ADT::MODF const& mapObjDef, char const* WmoInstName, uint32 mapID, uint32 tileX, uint32 tileY,
                        FILE* pDirfile)
{
