...
int main(int argc, char** argv)
{
  ...
  Acore::Banner::Show(
      "dbimport", [](std::string_view text) { LOG_INFO("dbimport", text); },
      []()
      {
        LOG_INFO("dbimport", "> Using configuration file:       {}", sConfigMgr->GetFilename());
        LOG_INFO("dbimport", "> Using SSL version:              {} (library: {})", OPENSSL_VERSION_TEXT,
                 OpenSSL_version(OPENSSL_VERSION));
        LOG_INFO("dbimport", "> Using Boost version:            {}.{}.{}", BOOST_VERSION / 100000,
...
bool StartDB()
{
  ...
  DatabaseLoader loader = modules.empty() ? DatabaseLoader("dbimport")
                          : (modules == "all")
...
variables_map GetConsoleArguments(int argc, char** argv, fs::path& configFile)
{
  ...
  all.add_options()("help,h", "print usage message")("version,v", "print version build info")("dry-run,d", "Dry run")(
      ...
      value<fs::path>(&configFile)
          ->default_value(fs::path(sConfigMgr->GetConfigPath() + std::string(_ACORE_DB_IMPORT_CONFIG))),
