...
void AppenderConsole::InitColors(std::string const& name, std::string_view str)
{
  ...
  {
    throw InvalidAppenderArgsException(Acore::StringFormat(
  ...
  for (uint8 i = 0; i < NUM_ENABLED_LOG_LEVELS; ++i)
  {
    ...
    {
      throw InvalidAppenderArgsException(Acore::StringFormat(
          "Log::CreateAppenderFromConfig: Invalid color '{}' for log level {} on console appender {}", colorStrs[i],
          EnumUtils::ToTitle(static_cast<LogLevel>(i)), name));
    }
  }
...
{
#if AC_PLATFORM == AC_PLATFORM_WINDOWS
  static WORD WinColorFG[NUM_COLOR_TYPES] = {0,                                                   // BLACK
                                             FOREGROUND_RED,                                      // RED
