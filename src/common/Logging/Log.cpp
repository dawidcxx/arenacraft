...
void Log::ReadLoggersFromConfig()
{
  ...
  if (loggers.find(LOGGER_ROOT) == loggers.end())
  {
    ...
    AppenderConsole* appender =
        new AppenderConsole(NextAppenderId(), "Console", LOG_LEVEL_DEBUG, APPENDER_FLAGS_NONE, {});
    appenders[appender->getId()].reset(appender);
...
std::string Log::GetTimestampStr() { return Acore::Time::TimeToTimestampStr(GetEpochTime(), "%Y-%m-%d_%H_%M_%S"); }
