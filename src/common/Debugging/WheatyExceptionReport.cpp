...
{
  LPVOID lpMsgBuf;
  DWORD  formatResult = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, nullptr, dw,
...
LONG WINAPI WheatyExceptionReport::WheatyUnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo)
{
  ...
  sprintf(m_szDumpFileName, "%s\\%s_%s_[%u-%u_%u-%u-%u].dmp", crash_folder_path, GitRevision::GetHash(), pos,
          systime.wDay, systime.wMonth, systime.wHour, systime.wMinute, systime.wSecond);

  sprintf(m_szLogFileName, "%s\\%s_%s_[%u-%u_%u-%u-%u].txt", crash_folder_path, GitRevision::GetHash(), pos,
          systime.wDay, systime.wMonth, systime.wHour, systime.wMinute, systime.wSecond);
  ...
  if (m_hDumpFile)
  {
    ...
    if (pExceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_ASSERTION_FAILURE &&
        pExceptionInfo->ExceptionRecord->NumberParameters > 0)
    {
      ...
      additionalStream.BufferSize =
          strlen(reinterpret_cast<char const*>(pExceptionInfo->ExceptionRecord->ExceptionInformation[0])) + 1;

      additionalStreamInfo.UserStreamArray = &additionalStream;
      additionalStreamInfo.UserStreamCount = 1;
    }

    MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), m_hDumpFile, MiniDumpWithIndirectlyReferencedMemory,
                      &info, &additionalStreamInfo, 0);
...
void __cdecl WheatyExceptionReport::WheatyCrtHandler(wchar_t const* /*expression*/, wchar_t const* /*function*/,
...
BOOL WheatyExceptionReport::_GetProcessorName(TCHAR* sProcessorName, DWORD maxcount)
{
  ...
  HKEY hKey;
  LONG lRet;
  lRet = ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0"), 0,
...
template <std::size_t size> void ToTchar(wchar_t const* src, TCHAR (&dst)[size], std::true_type)
{
  wcstombs_s(nullptr, dst, src, size);
}

template <std::size_t size> void ToTchar(wchar_t const* src, TCHAR (&dst)[size], std::false_type)
{
  wcscpy_s(dst, src);
}
  ...
  *szVersion = _T('\0');
  ...
  case VER_PLATFORM_WIN32_NT:
  {
    ...
    if (osvi.dwMajorVersion == 4 && _tcsicmp(szCSDVersion, _T("Service Pack 6")) == 0)
    {
      ...
      if (lRet == ERROR_SUCCESS)
      {
        _stprintf(wszTmp, _T("Service Pack 6a (Version %d.%d, Build %d)"), osvi.dwMajorVersion, osvi.dwMinorVersion,
                  osvi.dwBuildNumber & 0xFFFF);
        _tcsncat(szVersion, wszTmp, cntMax);
      }
      else // Windows NT 4.0 prior to SP6a
      {
        _stprintf(wszTmp, _T("%s (Version %d.%d, Build %d)"), szCSDVersion, osvi.dwMajorVersion, osvi.dwMinorVersion,
                  osvi.dwBuildNumber & 0xFFFF);
        _tcsncat(szVersion, wszTmp, cntMax);
      }
      ::RegCloseKey(hKey);
...
//////////////////////////////////////////////////////////////////////////////
void WheatyExceptionReport::DumpTypeIndex(DWORD64 modBase, DWORD dwTypeIndex, DWORD_PTR offset, bool& bHandled,
                                          char const* Name, char* /*suffix*/, bool newSymbol, bool logChildren)
{
  bHandled = false;
  ...
  case SymTagData:
    if (SymGetTypeInfo(m_hProcess, modBase, dwTypeIndex, TI_GET_TYPEID, &innerTypeID))
    {
      ...
      case SymTagUDT:
        ...
        DumpTypeIndex(modBase, innerTypeID, offset, bHandled, symbolDetails.top().Name.c_str(), (char*)"", false,
                      logChildren);
        break;
      case SymTagPointerType:
        if (Name != nullptr && Name[0] != '\0')
  ...
  case SymTagArrayType:
    if (SymGetTypeInfo(m_hProcess, modBase, dwTypeIndex, TI_GET_TYPEID, &innerTypeID))
    {
      ...
      DumpTypeIndex(modBase, innerTypeID, offset, bHandled, Name, (char*)"", false, false);
  ...
  for (unsigned i = 0; i < dwChildrenCount; i++)
  {
    ...
    DumpTypeIndex(modBase, children.ChildId[i], dwFinalOffset, bHandled2, "" /*Name */, (char*)"", true, true);
...
void WheatyExceptionReport::FormatOutputValue(char* pszCurrBuffer, BasicType basicType, DWORD64 length, PVOID pAddress,
