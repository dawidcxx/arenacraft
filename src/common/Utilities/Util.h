...
template <class T> inline T RoundToInterval(T& num, T floor, T ceil)
{
...
inline bool isExtendedLatinCharacter(wchar_t wchar)
{
  ...
  if (wchar >= 0x00C0 && wchar <= 0x00D6) // LATIN CAPITAL LETTER A WITH GRAVE - LATIN CAPITAL
                                          // LETTER O WITH DIAERESIS
  {
    return true;
  }
  if (wchar >= 0x00D8 && wchar <= 0x00DE) // LATIN CAPITAL LETTER O WITH STROKE
  ...
  if (wchar >= 0x0100 && wchar <= 0x012F) // LATIN CAPITAL LETTER A WITH MACRON
...
inline wchar_t wcharToUpper(wchar_t wchar)
{
  ...
  if (wchar >= 0x0101 && wchar <= 0x012F) // LATIN SMALL LETTER A WITH MACRON - LATIN SMALL LETTER
                                          // I WITH OGONEK (only %2=1)
  {
    if (wchar % 2 == 1)
    {
...
inline wchar_t wcharToLower(wchar_t wchar)
{
  ...
  if (wchar >= 0x0100 && wchar <= 0x012E) // LATIN CAPITAL LETTER A WITH MACRON - LATIN CAPITAL
...
template <typename Container> std::string ByteArrayToHexStr(Container const& c, bool reverse = false)
{
...
template <std::size_t Size> std::array<uint8, Size> HexStrToByteArray(std::string_view str, bool reverse = false)
{
...
public:
  ...
  inline bool operator!=(flag96 const& right) const { return !(*this == right); }
...
template <typename Ret, typename T1, typename... T> Ret* Coalesce(T1* first, T*... rest)
{
...
template <typename T> std::string GetTypeName() { return GetTypeName(typeid(T)); }

template <typename T> std::enable_if_t<!std::is_same_v<std::decay_t<T>, std::type_info>, std::string> GetTypeName(T&& v)
{
