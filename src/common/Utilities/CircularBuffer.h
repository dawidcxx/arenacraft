...
{
public:
  explicit CircularBuffer(std::size_t size) : buf_(std::unique_ptr<T[]>(new T[size])), max_size_(size) {}
  ...
  [[nodiscard]] std::size_t capacity() const { return max_size_; }
  ...
  // will never be lower than tail_ when compared to the original implementation
  // of this class
