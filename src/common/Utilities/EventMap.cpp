...
void EventMap::ScheduleEvent(uint32 eventId, Milliseconds minTime, Milliseconds maxTime, uint32 group /*= 0*/,
                             uint32 phase /*= 0*/)
{
...
void EventMap::RescheduleEvent(uint32 eventId, Milliseconds minTime, Milliseconds maxTime, uint32 group /*= 0*/,
                               uint32 phase /*= 0*/)
{
  CancelEvent(eventId);
  ScheduleEvent(eventId, randtime(minTime, maxTime).count(), group, phase);
...
void EventMap::RepeatEvent(uint32 time) { _eventMap.emplace(_time + time, _lastEvent); }

void EventMap::Repeat(Milliseconds time) { RepeatEvent(time.count()); }

void EventMap::Repeat(Milliseconds minTime, Milliseconds maxTime) { RepeatEvent(randtime(minTime, maxTime).count()); }
...
void EventMap::DelayEvents(uint32 delay) { _time = delay < _time ? _time - delay : 0; }
...
uint32 EventMap::GetNextEventTime() const { return Empty() ? 0 : _eventMap.begin()->first; }

bool EventMap::IsInPhase(uint8 phase) { return phase <= 8 && (!phase || _phase & (1 << (phase - 1))); }
