...
{
template <GenericHashImpl::HashCreator HashCreator, std::size_t DigestLength> class GenericHMAC
{
public:
  static constexpr std::size_t DIGEST_LENGTH = DigestLength;
  using Digest                               = std::array<uint8, DIGEST_LENGTH>;

  template <typename Container> static Digest GetDigestOf(Container const& seed, uint8 const* data, std::size_t len)
  {
  ...
  GenericHMAC(uint8 const* seed, std::size_t len)
      : _ctx(GenericHashImpl::MakeCTX()), _key(EVP_PKEY_new_mac_key(EVP_PKEY_HMAC, nullptr, seed, len))
  {
  ...
  GenericHMAC(GenericHMAC const& right) : _ctx(GenericHashImpl::MakeCTX()) { *this = right; }
  ...
  GenericHMAC& operator=(GenericHMAC const& right)
  {
    ...
    ASSERT(result == 1);
    _key = right._key;     // EVP_PKEY uses reference counting internally, just copy
                           // the pointer
    EVP_PKEY_up_ref(_key); // Bump reference count for PKEY, as every instance
                           // of this class holds two references to PKEY and
                           // destructor decrements it twice
