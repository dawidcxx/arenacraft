...
{
  bool result = false;
  if (fwrite(&iTilesX, sizeof(uint32), 1, wf) == 1 && fwrite(&iTilesY, sizeof(uint32), 1, wf) == 1 &&
      fwrite(&iCorner, sizeof(Vector3), 1, wf) == 1 && fwrite(&iType, sizeof(uint32), 1, wf) == 1)
  {
    if (iTilesX && iTilesY)
    {
...
bool WmoLiquid::readFromFile(FILE* rf, WmoLiquid*& out)
{
  ...
  if (fread(&liquid->iTilesX, sizeof(uint32), 1, rf) == 1 && fread(&liquid->iTilesY, sizeof(uint32), 1, rf) == 1 &&
      fread(&liquid->iCorner, sizeof(Vector3), 1, rf) == 1 && fread(&liquid->iType, sizeof(uint32), 1, rf) == 1)
  {
...
void GroupModel::GetMeshData(std::vector<G3D::Vector3>& outVertices, std::vector<MeshTriangle>& outTriangles,
                             WmoLiquid*& liquid)
{
...
bool WorldModel::IntersectRay(const G3D::Ray& ray, float& distance, bool stopAtFirstHit,
...
void WorldModel::GetGroupModels(std::vector<GroupModel>& outGroupModels) { outGroupModels = groupModels; }
} // namespace VMAP
