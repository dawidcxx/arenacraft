...
bool StaticMapTree::GetIntersectionTime(const G3D::Ray& pRay, float& pMaxDist, bool StopAtFirstHit,
...
bool StaticMapTree::InitMap(const std::string& fname, VMapMgr2* vm)
{
  ...
  if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn))
  {
    ...
    {
      success = false;
      // VMAP_ERROR_LOG(LOG_FILTER_GENERAL, "StaticMapTree::InitMap() : could not acquire WorldModel pointer for '{}'",
      // spawn.name);
    }
  }
...
bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapMgr2* vm)
{
  ...
  if (tf)
  {
    ...
    for (uint32 i = 0; i < numSpawns && result; ++i)
    {
      ...
      if (result)
      {
        ...
        if (!model)
        {
          LOG_ERROR("maps", "StaticMapTree::LoadMapTile() : could not acquire WorldModel pointer [{}, {}]", tileX,
                    tileY);
        }
        ...
        if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1)
        {
          if (!iLoadedSpawns.count(referencedVal))
          {
            ...
            {
              LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : invalid tree element ({}/{})", referencedVal,
                        iNTreeValues);
              continue;
            }
#endif
  ...
  METRIC_EVENT("map_events", "LoadMapTile",
               "Map: " + std::to_string(iMapID) + " TileX: " + std::to_string(tileX) +
...
void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapMgr2* vm)
{
  ...
  {
    LOG_ERROR("maps", "StaticMapTree::UnloadMapTile() : trying to unload non-loaded tile - Map:{} X:{} Y:{}", iMapID,
              tileX, tileY);
    return;
  }
      ...
      for (uint32 i = 0; i < numSpawns && result; ++i)
      {
        ...
        if (result)
        {
          ...
          else
          {
            ...
            {
              LOG_ERROR("maps", "StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '{}' (ID:{})",
                        spawn.name, spawn.ID);
            }
