...
bool MMapMgr::loadMap(uint32 mapId, int32 x, int32 y)
{
  ...
  {
    LOG_ERROR("maps", "MMAP:loadMap: {:03}{:02}{:02}.mmtile was built with generator v{}, expected v{}", mapId, x, y,
  ...
  if (dtStatusSucceed(mmap->navMesh->addTile(data, fileHeader.size, DT_TILE_FREE_DATA, 0, &tileRef)))
  {
    ...
    LOG_DEBUG("maps", "MMAP:loadMap: Loaded mmtile {:03}[{:02},{:02}] into {:03}[{:02},{:02}]", mapId, x, y, mapId,
              header->x, header->y);
    return true;
  }
...
bool MMapMgr::unloadMapInstance(uint32 mapId, uint32 instanceId)
{
  ...
  {
    LOG_DEBUG("maps", "MMAP:unloadMapInstance: Asked to unload not loaded dtNavMeshQuery mapId {:03} instanceId {}",
              mapId, instanceId);
    return false;
  }
...
dtNavMeshQuery const* MMapMgr::GetNavMeshQuery(uint32 mapId, uint32 instanceId)
{
  ...
  if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
  {
    ...
    if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
    {
      ...
      {
        dtFreeNavMeshQuery(query);
        LOG_ERROR("maps", "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId {:03} instanceId {}",
                  mapId, instanceId);
        return nullptr;
      }

      LOG_DEBUG("maps", "MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId {:03} instanceId {}", mapId,
                instanceId);
