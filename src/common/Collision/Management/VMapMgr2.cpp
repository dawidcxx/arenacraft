...
bool VMapMgr2::_loadMap(uint32 mapId, const std::string& basePath, uint32 tileX, uint32 tileY)
{
  ...
  if (instanceTree == iInstanceMapTrees.end())
  {
    if (thread_safe_environment)
    {
      ...
      ABORT("Invalid mapId {} tile [{}, {}] passed to VMapMgr2 after startup in thread unsafe environment", mapId,
            tileX, tileY);
  }
...
*/
bool VMapMgr2::GetObjectHitPos(unsigned int mapId, float x1, float y1, float z1, float x2, float y2, float z2,
...
bool VMapMgr2::GetAreaInfo(uint32 mapId, float x, float y, float& z, uint32& flags, int32& adtId, int32& rootId,
                           int32& groupId) const
{
#if defined(ENABLE_VMAP_CHECKS)
...
bool VMapMgr2::GetLiquidLevel(uint32 mapId, float x, float y, float z, uint8 reqLiquidType, float& level, float& floor,
                              uint32& type, uint32& mogpFlags) const
{
#if defined(ENABLE_VMAP_CHECKS)
...
void VMapMgr2::GetAreaAndLiquidData(uint32 mapId, float x, float y, float z, uint8 reqLiquidType,
                                    AreaAndLiquidData& data) const
{
  ...
  if (instanceTree != iInstanceMapTrees.end())
  {
    ...
    if (instanceTree->second->GetLocationInfo(pos, info))
    {
      ...
      if (!IsVMAPDisabledForPtr(mapId, VMAP_DISABLE_AREAFLAG))
        data.areaInfo.emplace(info.hitInstance->adtId, info.rootId, info.hitModel->GetWmoID(),
...
void VMapMgr2::GetInstanceMapTree(InstanceTreeMap& instanceMapTree) { instanceMapTree = iInstanceMapTrees; }
