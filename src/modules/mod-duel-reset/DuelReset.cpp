...
{
    // module configuration is hardcoded (zig-build port): the values used to
    // live in duelreset.conf.dist but were identical to these defaults
    m_enableCooldowns = true;
    m_enableHealth    = true;
    m_cooldownAge     = 30;

    FillWhitelist("0", m_zoneWhitelist);
    FillWhitelist("12;14;809", m_areaWhitelist);
}
