...
void RealmList::LoadBuildInfo()
{
  //                                                              0             1              2              3      4
  //                                                              5                6
  if (auto result = LoginDatabase.Query("SELECT majorVersion, minorVersion, bugfixVersion, hotfixVersion, build, "
                                        "winChecksumSeed, macChecksumSeed FROM build_info ORDER BY build ASC"))
  {
...
void RealmList::UpdateRealm(RealmHandle const& id, uint32 build, std::string const& name,
                            ...
                            boost::asio::ip::address&& localSubmask, uint16 port, uint8 icon, RealmFlags flag,
                            uint8 realmTimezone, AccountTypes allowedSecurityLevel, float population)
{
  ...
  if (result)
  {
    for (auto const& fields : *result)
    {
      try
      {
        ...
        if (!externalAddress)
        {
          LOG_ERROR("server.authserver", "Could not resolve address {} for realm \"{}\" id {}", externalAddressString,
                    name, realmId);
          continue;
        }
        ...
        if (!localAddress)
        {
          LOG_ERROR("server.authserver", "Could not resolve localAddress {} for realm \"{}\" id {}", localAddressString,
                    name, realmId);
          continue;
        }
        ...
        if (!localSubmask)
        {
          LOG_ERROR("server.authserver", "Could not resolve localSubnetMask {} for realm \"{}\" id {}",
                    localSubmaskString, name, realmId);
          continue;
        }
        ...
        UpdateRealm(
            id, build, name, externalAddress->address(), localAddress->address(), localSubmask->address(), port, icon,
            flag, realmTimezone,
