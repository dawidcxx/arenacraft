...
struct AchievementEntry
{
  ...
  // uint32 parentAchievement;                             // 3 its Achievement
  // parent (can`t start while parent uncomplete, use its Criteria if don`t have
  // own, use its progress on begin)
...
struct AchievementCriteriaEntry
{
  ...
  union
  {
    ...
    {
      uint32 skillID;    // 3
      uint32 skillLevel; // 4 apprentice=1, journeyman=2, expert=3, artisan=4,
                         // master=5, grand master=6
    } learn_skill_level;
    ...
    struct
    {
      ...
      uint32 count;   // 4 count of emotes, always required special target or
                      // requirements
    } do_emote;
    ...
    /// @todo: are those special criteria stored in the dbc or do we have to add
    /// another sql table?
    struct
    {
      uint32 unused;    // 3
...
struct BarberShopStyleEntry
{
  ...
  // char const*  displayNameLang[16];                     // 2-17 name of hair
  // style char const*  displayNameLangMask;                     // 18 char
  // const*  descriptionLang[16];                     // 19-34, all empty char
  // const*  descriptionLangMask;                     // 35 float costModifier;
...
struct CharStartOutfitEntry
{
  ...
  // int32 ItemDisplayId[MAX_OUTFIT_ITEMS];                // 29-52 not required
  // at server side int32 ItemInventorySlot[MAX_OUTFIT_ITEMS];            //
...
struct CharTitlesEntry
{
  ...
  // uint32      conditionID;                              // 1  Never used by
  // the client. Should be used serverside?
  char const* nameMale[16]; // 2-17
  // uint32 nameLangMask                                   // 18 string flag,
  // unused
  char const* nameFemale[16]; // 19-34
  // uint32 nameLang1Mask                                  // 35 string flag,
  // unused
...
struct FactionEntry
{
  ...
  float  spilloverRateOut;    // 20       Faction outputs rep * spilloverRateOut as
                              // spillover reputation
  uint32 spilloverMaxRankIn;  // 21       The highest rank the faction will
                              // profit from incoming spillover
  // uint32    parentFactionCap;                           // 22       It does
  // not seem to be the max standing at which a faction outputs spillover ...so
  // no idea
...
struct FactionTemplateEntry
{
  ...
  uint32 friendFaction[MAX_FACTION_RELATIONS]; // 10
...
struct GlyphPropertiesEntry
{
  ...
  uint32 SpellId;
  uint32 TypeFlags;
  // uint32  spellIconID;                                       // GlyphIconId
  // (SpellIcon.dbc)
};
...
struct HolidaysEntry
{
  ...
  // (HolidayNames.dbc) uint32 holidayDescriptionId;                          //
  // 50       m_holidayDescriptionID (HolidayDescriptions.dbc)
...
struct ItemExtendedCostEntry
{
  ...
  uint32 reqitemcount[MAX_ITEM_EXTENDED_COST_REQUIREMENTS]; // 9-14 required
                                                            // count of 1st item
...
struct MapEntry
{
  ...
  // char const*     hordeIntro[16];                       // 23-38 text for PvP
  // Zones
  //  39 intro text flags
  // char const*     allianceIntro[16];                    // 40-55 text for PvP
  // Zones
  //  56 intro text flags
  ...
  // uint32  raidOffset;                                   // 64 some kind of
  // time?
  ...
  [[nodiscard]] bool IsArenacraftWhitelistedMap() const
  {
    return map_type == MAP_BATTLEGROUND || map_type == MAP_ARENA || MapID == 571;
  }
  ...
  [[nodiscard]] bool IsContinent() const { return MapID == 0 || MapID == 1 || MapID == 530 || MapID == 571; }
...
{
  uint32 Id; // 0        m_ID
  // uint32    Type;                                       // 1 m_soundType char
  // const*     InternalName;                         // 2        m_name char
  // const*     FileName[10];                         // 3-12     m_File[10]
...
struct SpellEntry
{
  ...
  std::array<int32, MAX_SPELL_EFFECTS>   EffectBasePoints;         // 80-82    m_effectBasePoints (must not be used in
  ...
  // 170-185  m_description_lang not used uint32    DescriptionFlags; // 186 not
  // used char const*     ToolTip[16];                                  //
  // 187-202  m_auraDescription_lang not used uint32    ToolTipFlags; // 203 not
  // used
  ...
  // m_minFactionID not used uint32    MinReputation; // 220 m_minReputation not
  // used uint32    RequiredAuraVision;                                 // 221
  ...
  // m_spellMissileID not used uint32  PowerDisplayId; // 228 PowerDisplay.dbc,
  // new in 3.1
...
struct SpellDifficultyEntry
{
  ...
  int32  SpellID[MAX_DIFFICULTY]; // 1-4 instance modes: 10N, 25N, 10H, 25H or
                                  // Normal/Heroic if only 1-2 is set, if 3-4 is
                                  // 0 then Mode-2
};
...
struct TalentEntry
{
  ...
  // uint32 spellRank [4]                                 // 9-12 not used,
  // always 0, maybe not used high ranks
  ...
  // uint64  categoryMask[2];                              // 21-22 its a 64 bit
  // mask for pet 1<<m_categoryEnumID in CreatureFamily.dbc
};
...
struct VehicleSeatEntry
{
  ...
  [[nodiscard]] bool CanEnterOrExit() const
  {
    return ((m_flags & VEHICLE_SEAT_FLAG_CAN_ENTER_OR_EXIT) != 0 ||
            // If it has anmation for enter/ride, means it can be entered/exited
            // by logic
...
struct WorldMapAreaEntry
{
  ...
  float x1;             // 6
  float x2;             // 7
  int32 virtual_map_id; // 8 -1 (map_id have correct map) other: virtual map where
                        // zone show (map_id - where zone in fact internally)
...
{
    uint32    ID;                                           // 0
    uint32    mapID;                                        // 1        Can be
-1 to show up everywhere. uint32    zone; // 2        Can be zero for
"everywhere". uint32    phaseMask;                                    // 3 Phase
this WorldState is avaliable in char const*     icon; // 4        The icon that
is used in the interface. char const*     text; // 5-20     The text displayed
in the UI. char const*     textLangMask                            // 21 char
const*     tooltip;                                // 22-37    Text shown when
hovering mouse on icon char const*     tooltipLangMask // 38 uint32
worldStateID;                                 // 39       This is the actual ID
used uint32    type;                                         // 40       0 =
unknown, 1 = unknown, 2 = battleground scores ("Captured flags", ...) char
const*     dynamicIcon;                            // 41       Only set for
warsong-gulch-flags. char const*     dynamicTooltip;                         //
42-57    Again, set for the flags. "Flag is picked up". char const*
dynamicTooltipLangMask;                 // 58 uint32    extendedUI; // 59 If
set, then its "CAPTUREPOINT". Used for points that need to be tapped over time.
(Progress: *%) uint32    extendedUIStateVariable;                      // 60
This worldstate tells how far the captureing proceeded. 100: Alliance / left, 0:
Horde / right uint32    extendedUIStateVariableNeutral                // 61 This
state defines the grey part of the bar. In percent of the whole bar. uint32
extendedUIStateVariable                       // 62       unused in 3.3.5
};
*/
