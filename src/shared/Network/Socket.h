...
public:
  explicit Socket(tcp::socket&& socket)
      : _socket(std::move(socket)), _remoteAddress(_socket.remote_endpoint().address()),
        ...
        _isWritingAsync(false), _proxyHeaderReadingState(PROXY_HEADER_READING_STATE_NOT_STARTED)
  {
  ...
  [[nodiscard]] boost::asio::ip::address GetRemoteIpAddress() const { return _remoteAddress; }
  ...
  void AsyncRead()
  {
    ...
    _socket.async_read_some(boost::asio::buffer(_readBuffer.GetWritePointer(), _readBuffer.GetRemainingSpace()),
                            std::bind(&Socket<T>::ReadHandlerInternal, this->shared_from_this(), std::placeholders::_1,
                                      std::placeholders::_2));
  }
  ...
  void AsyncReadProxyHeader()
  {
    ...
    _readBuffer.EnsureFreeSpace();
    _socket.async_read_some(boost::asio::buffer(_readBuffer.GetWritePointer(), _readBuffer.GetRemainingSpace()),
                            std::bind(&Socket<T>::ProxyReadHeaderHandler, this->shared_from_this(),
  ...
  void CloseSocket()
  {
    ...
    if (shutdownError)
      LOG_DEBUG("network", "Socket::CloseSocket: {} errored when shutting down socket: {} ({})",
                GetRemoteIpAddress().to_string(), shutdownError.value(), shutdownError.message());
...
protected:
  ...
  bool AsyncProcessQueue()
  {
    ...
    _socket.async_write_some(boost::asio::null_buffers(),
                             std::bind(&Socket<T>::WriteHandlerWrapper, this->shared_from_this(), std::placeholders::_1,
                                       std::placeholders::_2));
#endif
    return false;
  }
...
private:
  ...
  // ProxyReadHeaderHandler reads Proxy Protocol v2 header (v1 is not
  // supported). See https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt
  // (2.2. Binary header format (version 2)) for more details.
  void ProxyReadHeaderHandler(boost::system::error_code error, std::size_t transferredBytes)
  {
    ...
    const uint8 expectedSignature[signatureSize] = {0x0D, 0x0A, 0x0D, 0x0A, 0x00, 0x0D,
    ...
    if (memcmp(packet.GetReadPointer(), expectedSignature, signatureSize) != 0)
    {
      ...
      LOG_ERROR("network",
                "Socket::ProxyReadHeaderHandler: received bad PROXY Protocol "
                "v2 signature for {}",
    ...
    // Connection created by a proxy itself (health checks?), ignore and do
    // nothing.
    if (command == 0)
    {
    ...
    default:
      _proxyHeaderReadingState = PROXY_HEADER_READING_STATE_FAILED;
      LOG_ERROR("network", "Socket::ProxyReadHeaderHandler: unsupported address family type {}",
                GetRemoteIpAddress().to_string());
      return;
    }
