...
float const GROUND_HEIGHT_TOLERANCE = 0.05f; // Extra tolerance to z position to check if it is in air or on
                                             // ground.
...
#define RACEMASK_ALL_PLAYABLE                                                                                          \
  ((1 << (RACE_HUMAN - 1)) | (1 << (RACE_ORC - 1)) | (1 << (RACE_DWARF - 1)) | (1 << (RACE_NIGHTELF - 1)) |            \
   (1 << (RACE_UNDEAD_PLAYER - 1)) | (1 << (RACE_TAUREN - 1)) | (1 << (RACE_GNOME - 1)) | (1 << (RACE_TROLL - 1)) |    \
...
#define CLASSMASK_ALL_CREATURES                                                                                        \
  ((1 << (UNIT_CLASS_WARRIOR - 1)) | (1 << (UNIT_CLASS_PALADIN - 1)) | (1 << (UNIT_CLASS_ROGUE - 1)) |                 \
   (1 << (UNIT_CLASS_MAGE - 1)))
...
uint32 constexpr QuestDifficultyColors[MAX_QUEST_DIFFICULTY] = {0xff40c040, 0xff808080, 0xffffff00, 0xffff8040,
                                                                0xffff2020};
...
enum SpellAttr0 : uint32
{
  ...
  SPELL_ATTR0_USES_RANGED_SLOT          = 0x00000002,  // TITLE Treat as ranged attack DESCRIPTION Use ammo, ranged
                                                       // attack range modifiers, ranged haste, etc.
  SPELL_ATTR0_ON_NEXT_SWING_NO_DAMAGE = 0x00000004,    // TITLE On next melee (type 1) DESCRIPTION Both "on next
  ...
  SPELL_ATTR0_IS_ABILITY               = 0x00000010,   // TITLE Treat as ability DESCRIPTION Cannot be reflected, not
                                                       // affected by cast speed modifiers, etc.
  SPELL_ATTR0_IS_TRADESKILL = 0x00000020,              // TITLE Trade skill recipe DESCRIPTION Displayed in recipe
  ...
  SPELL_ATTR0_DO_NOT_DISPLAY = 0x00000080,             // TITLE Hidden in UI (client only) DESCRIPTION Not visible in
                                                       // spellbook or aura bar (Spellbook, Aura Icon, Combat Log)
  SPELL_ATTR0_DO_NOT_LOG = 0x00000100,                 // TITLE Hidden in combat log (client only) DESCRIPTION Spell
                                                       // will not appear in combat logs
  SPELL_ATTR0_HELD_ITEM_ONLY = 0x00000200,             // TITLE Auto-target mainhand item (client only) DESCRIPTION
                                                       // Client will automatically select main-hand item as cast
                                                       // target
  SPELL_ATTR0_ON_NEXT_SWING = 0x00000400,              // TITLE On next melee (type 2) DESCRIPTION Both "on next
                                                       // swing" attributes have identical handling in server &
                                                       // client
  ...
  SPELL_ATTR0_SCALES_WITH_CREATURE_LEVEL = 0x00080000, // TITLE Scale with caster level DESCRIPTION For non-player
                                                       // casts, scale impact and power cost with caster's level
  SPELL_ATTR0_CANCELS_AUTO_ATTACK_COMBAT = 0x00100000, // TITLE Stop attacking after cast DESCRIPTION After casting
                                                       // this, the current auto-attack will be interrupted
  SPELL_ATTR0_NO_ACTIVE_DEFENSE = 0x00200000,          // TITLE Prevent physical avoidance DESCRIPTION Spell cannot
  ...
  SPELL_ATTR0_ALLOW_CAST_WHILE_DEAD = 0x00800000,       // TITLE Can be cast while dead DESCRIPTION Spells without
  ...
  SPELL_ATTR0_COOLDOWN_ON_EVENT   = 0x02000000,         // TITLE Cooldown starts on expiry DESCRIPTION Spell is
                                                        // unusable while already active, and cooldown does not begin
                                                        // until the effects have worn off
  SPELL_ATTR0_AURA_IS_DEBUFF = 0x04000000,              // TITLE Is negative spell DESCRIPTION Forces the spell to be
  ...
  SPELL_ATTR0_NO_IMMUNITIES               = 0x20000000, // TITLE Pierce invulnerability DESCRIPTION Allows spell to
                                                        // pierce invulnerability, unless the invulnerability spell
                                                        // also has this attribute
  SPELL_ATTR0_HEARTBEAT_RESIST = 0x40000000,            // TITLE Periodic resistance checks DESCRIPTION Periodically
                                                        // re-rolls against resistance to potentially expire aura
                                                        // early
  SPELL_ATTR0_NO_AURA_CANCEL = 0x80000000               // TITLE Aura cannot be cancelled DESCRIPTION Prevents the
...
{
  SPELL_ATTR1_DISMISS_PET_FIRST = 0x00000001,       // TITLE Dismiss Pet on cast DESCRIPTION Without this
                                                    // attribute, summoning spells will fail if caster already has
                                                    // a pet
  SPELL_ATTR1_USE_ALL_MANA = 0x00000002,            // TITLE Drain all power DESCRIPTION Ignores listed power cost
                                                    // and drains entire pool instead
  SPELL_ATTR1_IS_CHANNELED = 0x00000004,            // TITLE Channeled (type 1) DESCRIPTION Both "channeled"
                                                    // attributes have identical handling in server & client
  SPELL_ATTR1_NO_REDIRECTION = 0x00000008,          // TITLE Ignore redirection effects DESCRIPTION Spell will not
                                                    // be attracted by SPELL_MAGNET auras (Grounding Totem)
  SPELL_ATTR1_NO_SKILL_INCREASE = 0x00000010,       // TITLE Unknown attribute 4@Attr1 DESCRIPTION stealth and
                                                    // whirlwind
  SPELL_ATTR1_ALLOW_WHILE_STEALTHED = 0x00000020,   // TITLE Does not break stealth
  SPELL_ATTR1_IS_SELF_CHANNELED     = 0x00000040,   // TITLE Channeled (type 2) DESCRIPTION Both "channeled"
                                                    // attributes have identical handling in server & client
  SPELL_ATTR1_NO_REFLECTION = 0x00000080,           // TITLE Ignore reflection effects DESCRIPTION Spell will
                                                    // pierce through Spell Reflection and similar
  SPELL_ATTR1_ONLY_PEACEFUL_TARGETS = 0x00000100,   // TITLE Target cannot be in combat
  SPELL_ATTR1_INITIATE_COMBAT       = 0x00000200,   // TITLE Enables Auto-Attack (client only) DESCRIPTION Caster
                                                    // will begin auto-attacking the target on cast
  SPELL_ATTR1_NO_THREAT = 0x00000400,               // TITLE Does not generate threat DESCRIPTION Also does not
  ...
  SPELL_ATTR1_TRACK_TARGET_IN_CHANNEL = 0x00004000, // TITLE Track target while channeling DESCRIPTION While
                                                    // channeling, adjust facing to face target
  SPELL_ATTR1_IMMUNITY_PURGES_EFFECT = 0x00008000,  // TITLE Immunity cancels preapplied auras DESCRIPTION For
                                                    // immunity spells, cancel all auras that this spell would
                                                    // make you immune to when the spell is applied
  SPELL_ATTR1_IMMUNITY_TO_HOSTILE_AND_FRIENDLY_EFFECTS = 0x00010000, // TITLE Unaffected by school immunities
                                                                     // DESCRIPTION Will not pierce Divine Shield, Ice
                                                                     // Block and other full invulnerabilities
  SPELL_ATTR1_NO_AUTOCAST_AI = 0x00020000,                           // TITLE Cannot be autocast by pet DESCRIPTION (AI)
  SPELL_ATTR1_PREVENTS_ANIM  = 0x00040000,                           // TITLE NYI, auras apply
  ...
  SPELL_ATTR1_AURA_STAYS_AFTER_COMBAT = 0x02000000, // TITLE Aura stays after combat DESCRIPTION Aura will not be
                                                    // removed when the unit leaves combat
  SPELL_ATTR1_REQUIRE_ALL_TARGETS = 0x04000000,     // TITLE Unknown attribute 26@Attr1 DESCRIPTION Related to
  ...
  SPELL_ATTR1_COMBO_ON_BLOCK         = 0x40000000,  // TITLE Enable at dodge DESCRIPTION
...
enum SpellAttr2 : uint32
{
  ...
  SPELL_ATTR2_NO_SHAPESHIFT_UI  = 0x00000002,              // TITLE Unknown attribute 1@Attr2 DESCRIPTION vanish,
  ...
  SPELL_ATTR2_CANNOT_CAST_ON_TAPPED = 0x00000040,          // TITLE Cannot target others' tapped units DESCRIPTION Can
  ...
  SPELL_ATTR2_INCLUDE_IN_ADVANCED_COMBAT_LOG = 0x00000100, // TITLE Unknown attribute 8@Attr2 DESCRIPTION not set
                                                           // in 3.0.3
  SPELL_ATTR2_ALWAYS_CAST_AS_UNIT = 0x00000200,            // TITLE Unknown attribute 9@Attr2
  SPELL_ATTR2_SPECIAL_TAMING_FLAG = 0x00000400,            // TITLE Unknown attribute 10@Attr2 DESCRIPTION Related to
                                                           // taming?
  SPELL_ATTR2_NO_TARGET_PER_SECOND_COST = 0x00000800,      // TITLE Health Funnel
  SPELL_ATTR2_CHAIN_FROM_CASTER         = 0x00001000,      // TITLE Unknown attribute 12@Attr2 DESCRIPTION Cleave, Heart
                                                           // Strike, Maul, Sunder Armor, Swipe
  ...
  SPELL_ATTR2_DO_NOT_CONSUME_IF_GAINED_DURING_CAST = 0x00008000, // TITLE Unknown attribute 15@Attr2 DESCRIPTION not set
                                                                 // in 3.0.3
  SPELL_ATTR2_NO_ACTIVE_PETS             = 0x00010000,           // TITLE Tame Beast
  SPELL_ATTR2_DO_NOT_RESET_COMBAT_TIMERS = 0x00020000,   // TITLE Don't reset swing timer DESCRIPTION Does not reset
                                                         // melee/ranged autoattack timer on cast
  SPELL_ATTR2_NO_JUMP_WHILE_CAST_PENDING   = 0x00040000, // TITLE Requires dead pet
  SPELL_ATTR2_ALLOW_WHILE_NOT_SHAPESHIFTED = 0x00080000, // TITLE Also allow outside shapeshift (caster form)
                                                         // DESCRIPTION Even if Stances are nonzero, allow spell to be
                                                         // cast outside of shapeshift (though not in a different
                                                         // shapeshift)
  SPELL_ATTR2_INITIATE_COMBAT_POST_CAST  = 0x00100000,   // TITLE (Enables Auto-Attack)
  SPELL_ATTR2_FAIL_ON_ALL_TARGETS_IMMUNE = 0x00200000,   // TITLE Damage reduction ability DESCRIPTION Causes BG flags
                                                         // to be dropped if combined with
                                                         // ATTR1_DISPEL_AURAS_ON_IMMUNITY
  SPELL_ATTR2_NO_INITIAL_THREAD = 0x00400000,            // TITLE Unknown attribute 22@Attr2 DESCRIPTION Ambush,
  ...
  SPELL_ATTR2_NO_SCHOOL_IMMUNITIES       = 0x04000000,   // TITLE Pierce aura application immunities DESCRIPTION Allow
  ...
  SPELL_ATTR2_ACTIVE_THREAT      = 0x40000000,           // TITLE Allow triggered spell to trigger (type 1) DESCRIPTION
                                                         // Without this attribute, any triggered spell will be unable
                                                         // to trigger other auras' procs
...
enum SpellAttr3 : uint32
{
  ...
  SPELL_ATTR3_NOT_A_PROC                = 0x00000200,  // TITLE Allow triggered spell to trigger (type 2) DESCRIPTION
                                                       // Without this attribute, any triggered spell will be unable
                                                       // to trigger other auras' procs
  ...
  SPELL_ATTR3_NORMAL_RANGED_ATTACK      = 0x00008000,  // TITLE Unknown attribute 15@Attr3 DESCRIPTION Auto Shoot,
  ...
  SPELL_ATTR3_ALWAYS_HIT            = 0x00040000,      // TITLE Ignore hit result DESCRIPTION Spell cannot miss, or
  ...
  SPELL_ATTR3_CASTING_CANCELS_AUTOREPEAT = 0x00400000, // TITLE Requires equipped Wand (Mainline: Do Not Trigger
                                                       // Target Stand)
...
enum SpellAttr4 : uint32
{
  ...
  SPELL_ATTR4_AURA_EXPIRES_OFFLINE         = 0x00000004,  // TITLE Buff expires while offline DESCRIPTION Debuffs
  ...
  SPELL_ATTR4_NO_HARMFUL_THREAT = 0x00000010,             // TITLE Treat as delayed spell DESCRIPTION This will no
  ...
  SPELL_ATTR4_ALLOW_CAST_WHILE_CASTING = 0x00000080,      // TITLE Can be cast while casting DESCRIPTION Ignores already
                                                          // in-progress cast and still casts
  ...
  SPELL_ATTR4_WEAPON_SPEED_COST_SCALING     = 0x00000400, // TITLE Attack speed modifies cost DESCRIPTION Adds 10 to
  ...
  SPELL_ATTR4_NOT_IN_ARENA_OR_RATED_BATTLEGROUND = 0x00010000, // TITLE Not usable in arena DESCRIPTION Makes spell
                                                               // unusable despite CD <= 10min
  SPELL_ATTR4_IGNORE_DEFAULT_ARENA_RESTRICTIONS = 0x00020000,  // TITLE Usable in arena DESCRIPTION Makes spell usable
                                                               // despite CD > 10min
  SPELL_ATTR4_BOUNCY_CHAIN_MISSILES = 0x00040000,              // TITLE Chain area targets DESCRIPTION [NYI] Hits area
                                                               // targets over time instead of all at once
  SPELL_ATTR4_ALLOW_PROC_WHILE_SITTING = 0x00080000, // TITLE Unknown attribute 19@Attr4 DESCRIPTION proc dalayed,
                                                     // after damage or don't proc on absorb?
  SPELL_ATTR4_AURA_NEVER_BOUNCES = 0x00100000,       // TITLE Allow self-cast to override stronger aura (client
                                                     // only)
  SPELL_ATTR4_ALLOW_ENETRING_ARENA     = 0x00200000, // TITLE Keep when entering arena
  SPELL_ATTR4_PROC_SUPPRESS_SWING_ANIM = 0x00400000, // TITLE Unknown attribute 22@Attr4 DESCRIPTION Seal of
                                                     // Command (42058,57770) and Gymer's Smash 55426
  SPELL_ATTR4_SUPPRESS_WEAPON_PROCS = 0x00800000,    // TITLE Cannot trigger item spells
  SPELL_ATTR4_AUTO_RANGED_COMBAT    = 0x01000000,    // TITLE Unknown attribute 24@Attr4 DESCRIPTION Shoot-type
                                                     // spell?
  ...
  SPELL_ATTR4_USE_FACING_FROM_SPELL   = 0x80000000   // TITLE Unknown attribute 31@Attr4 DESCRIPTION Polymorph
...
enum SpellAttr5 : uint32
{
  ...
  SPELL_ATTR5_REMOVE_ENTERING_ARENA       = 0x00000004,  // TITLE Remove when entering arena DESCRIPTION Force this
  ...
  SPELL_ATTR5_LIMIT_N             = 0x00000020,          // TITLE Single-target aura DESCRIPTION Remove previous
  ...
  SPELL_ATTR5_NOT_ON_PLAYER_CONTROLLED_NPC = 0x00000100, // TITLE Cannot target player controlled units but can target
                                                         // players
  ...
  SPELL_ATTR5_MELEE_CHAIN_TARGETING   = 0x00001000,      // TITLE Unknown attribute 12@Attr5 DESCRIPTION Cleave
                                                         // related?
  ...
  SPELL_ATTR5_TREAT_AS_AREA_EFFECT         = 0x00008000, // TITLE Unknown attribute 15@Attr5 DESCRIPTION Related to
                                                         // multi-target spells?
  SPELL_ATTR5_AURA_AFFECTS_NOT_JUST_REQ_EQUIPPED_ITEM = 0x00010000, // TITLE DESCRIPTION this allows spells with
                                                                    // EquippedItemClass to affect spells from other
                                                                    // items if the required item is equipped
  ...
  SPELL_ATTR5_ADD_MELEE_HIT_RATING       = 0x80000000  // TITLE Unknown attribute 31@Attr5 DESCRIPTION Forces nearby
...
enum SpellAttr6 : uint32
{
  ...
  SPELL_ATTR6_FLOATING_COMBAT_TEXT_ON_CAST   = 0x00000040, // TITLE Generate spell_cast event instead of aura_start
                                                           // (client only)
  SPELL_ATTR6_AURA_IS_WEAPON_PROC                      = 0x00000080, // TITLE Unknown attribute 7@Attr6
  SPELL_ATTR6_DO_NOT_CHAIN_TO_CROWD_CONTROLLED_TARGETS = 0x00000100, // TITLE Do not implicitly target in CC DESCRIPTION
                                                                     // Implicit targeting (chaining and area targeting)
                                                                     // will not impact crowd controlled targets
  ...
  SPELL_ATTR6_NO_PUSHBACK                = 0x00008000, // TITLE Not affected by pushback DESCRIPTION Casts/Channels
                                                       // will not affected by pushback
  ...
  SPELL_ATTR6_ORIGINATE_FROM_CONTROLLER = 0x00040000,  // TITLE Spell is cast by charmer DESCRIPTION Client will
                                                       // prevent casting if not possessed, charmer will be caster
                                                       // for all intents and purposes
  SPELL_ATTR6_DELAY_COMBAT_TIMER_DURING_CAST = 0x00080000, // TITLE Unknown attribute 19@Attr6 DESCRIPTION only 47488,
                                                           // 50782
  ...
  SPELL_ATTR6_VEHICLE_IMMUNITY_CATEGORY = 0x04000000, // TITLE Unknown attribute 26@Attr6 DESCRIPTION Player
                                                      // castable buff?
  SPELL_ATTR6_IGNORE_HEALTH_MODIFIERS = 0x08000000,   // TITLE Limit applicable %healing modifiers DESCRIPTION This
  ...
  SPELL_ATTR6_IGNORE_CASTER_DAMAGE_MODIFIERS = 0x20000000, // TITLE Limit applicable %damage modifiers DESCRIPTION This
...
{
  SPELL_ATTR7_ALLOW_SPELL_REFLECTION = 0x00000001,           // TITLE Unknown attribute 0@Attr7 DESCRIPTION Shaman's new
  ...
  SPELL_ATTR7_DEBUG_SPELL             = 0x00000008,          // TITLE Is cheat spell DESCRIPTION Cannot cast if caster
                                                             // doesn't have UnitFlag2 & UNIT_FLAG2_ALLOW_CHEAT_SPELLS
  SPELL_ATTR7_TREAT_AS_RAID_BUFF = 0x00000010,               // TITLE Unknown attribute 4@Attr7 DESCRIPTION Soulstone
                                                             // related?
  ...
  SPELL_ATTR7_PREPARE_FOR_VEHICLE_CONTROL_END = 0x00000080,  // TITLE Unknown attribute 7@Attr7 DESCRIPTION 66218
                                                             // (Launch) spell.
  ...
  SPELL_ATTR7_CAN_CAUSE_SILENCE       = 0x00001000,          // TITLE Unknown attribute 12@Attr7 DESCRIPTION Not set
                                                             // in 3.2.2a.
  SPELL_ATTR7_NO_UI_NOT_INTERRUPTIBLE = 0x00002000,          // TITLE Unknown attribute 13@Attr7 DESCRIPTION Not set
                                                             // in 3.2.2a.
  SPELL_ATTR7_RECAST_ON_RESUMMON = 0x00004000,               // TITLE Unknown attribute 14@Attr7 DESCRIPTION Only 52150
                                                             // (Raise Dead - Pet) spell.
  SPELL_ATTR7_RESET_SWING_TIMER_AT_SPELL_START = 0x00008000, // TITLE Unknown attribute 15@Attr7 DESCRIPTION Exorcism -
                                                             // guaranteed crit vs families?
  SPELL_ATTR7_ONLY_IN_SPELLBOOK_UNTIL_LEARNED = 0x00010000, // TITLE Can restore secondary power DESCRIPTION Only spells
                                                            // with this attribute can replenish a non-active power type
  SPELL_ATTR7_DO_NOT_LOG_PVP_KILL = 0x00020000,             // TITLE Unknown attribute 17@Attr7 DESCRIPTION Only 27965
                                                            // (Suicide) spell.
  ...
  SPELL_ATTR7_NO_CLIENT_FAIL_WHILE_STUNNED_FLEEING_CONFUSED = 0x00100000, // TITLE Unknown attribute 20@Attr7
                                                                          // DESCRIPTION Invulnerability related?
  SPELL_ATTR7_RETAIN_COOLDOWN_THROUGH_LOAD            = 0x00200000,       // TITLE Unknown attribute 21@Attr7
  SPELL_ATTR7_IGNORES_COLD_WEATHER_FLYING_REQUIREMENT = 0x00400000, // TITLE Ignore cold weather flying restriction
                                                                    // DESCRIPTION Set for loaner mounts, allows them to
                                                                    // be used despite lacking required flight skill
  SPELL_ATTR7_NO_ATTACK_DODGE = 0x00800000, // TITLE Spell cannot be dodged 23@Attr7 DESCRIPTION Motivate,
                                            // Mutilate, Shattering Throw
  SPELL_ATTR7_NO_ATTACK_PARRY = 0x01000000, // TITLE Spell cannot be parried 24@Attr7 DESCRIPTION
  ...
  SPELL_ATTR7_REFLECTION_ONLY_DEFENDS         = 0x20000000, // TITLE Unknown attribute 29@Attr7 DESCRIPTION only 69028,
                                                            // 71237
  SPELL_ATTR7_CAN_PROC_FROM_SUPPRESSED_TARGET_PROCS = 0x40000000, // TITLE Unknown attribute 30@Attr7 DESCRIPTION
                                                                  // Burning Determination, Divine Sacrifice, Earth
...
enum SpellCustomErrors
{
  ...
  SPELL_CUSTOM_ERROR_NEED_HELPLESS_VILLAGER = 3, // This spell may only be used on Helpless Wintergarde Villagers that
                                                 // have not been rescued.
  SPELL_CUSTOM_ERROR_NEED_WARSONG_DISGUISE = 4,  // Requires that you be wearing the Warsong Orc Disguise.
  SPELL_CUSTOM_ERROR_REQUIRES_PLAGUE_WAGON = 5,  // You must be closer to a plague wagon in order to drop off your 7th
  ...
  SPELL_CUSTOM_ERROR_MUST_BE_IN_ENKILAH = 8, // The Imbued Scourge Shroud will only work when equipped in the Temple
                                             // City of En'kilah.
  ...
  SPELL_CUSTOM_ERROR_WYRMREST_VANQUISHER = 20, // You may only call for the aid of a Wyrmrest Vanquisher in Wyrmrest
  ...
  SPELL_CUSTOM_ERROR_REQUIRES_GAMMOTH_MAGNATAUR = 24, // You may only use the Pouch of Crushed Bloodspore on Gammothra
                                                      // or other magnataur in the Bloodspore Plains and Gammoth.
  SPELL_CUSTOM_ERROR_MUST_BE_IN_RESURRECTION_CHAMBER = 25, // Requires the magmawyrm resurrection chamber in the back of
                                                           // the Maw of Neltharion.
  SPELL_CUSTOM_ERROR_CANT_CALL_WINTERGARDE_HERE = 26, // You may only call down a Wintergarde Gryphon in Wintergarde
                                                      // Keep or the Carrion Fields.
  ...
  SPELL_CUSTOM_ERROR_TOO_MANY_GHOULS     = 30,        // You've created enough ghouls. Return to Gothik the Harvester at
                                                      // Death's Breach.
  SPELL_CUSTOM_ERROR_GO_FURTHER_FROM_SUNDERED_SHARD = 31, // Your companion does not want to come here.  Go further from
                                                          // the Sundered Shard.
  ...
  SPELL_CUSTOM_ERROR_MUST_HAVE_USED_DALARAN_CRYSTAL   = 37, // This teleport crystal cannot be used until the teleport
                                                            // crystal in Dalaran has been used at least once.
  SPELL_CUSTOM_ERROR_YOU_ALREADY_HOLD_SOMETHING = 38, // You are already holding something in your hand. You must throw
                                                      // the creature in your hand before picking up another.
  SPELL_CUSTOM_ERROR_YOU_DONT_HOLD_ANYTHING = 39, // You don't have anything to throw! Find a Vargul and use Gymer Grab
                                                  // to pick one up!
  SPELL_CUSTOM_ERROR_MUST_BE_CLOSE_TO_VALDURAN = 40, // Bouldercrag's War Horn can only be used within 10 yards of
  ...
  SPELL_CUSTOM_ERROR_ALREADY_CARRYING_CRUSADER = 43, // You are already carrying a captured Argent Crusader. You must
...
#define IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK                                                            \
  ((1 << MECHANIC_CHARM) | (1 << MECHANIC_DISORIENTED) | (1 << MECHANIC_FEAR) | (1 << MECHANIC_ROOT) |                 \
   (1 << MECHANIC_SLEEP) | (1 << MECHANIC_SNARE) | (1 << MECHANIC_STUN) | (1 << MECHANIC_FREEZE) |                     \
   (1 << MECHANIC_KNOCKOUT) | (1 << MECHANIC_POLYMORPH) | (1 << MECHANIC_BANISH) | (1 << MECHANIC_SHACKLE) |           \
...
enum GameObjectFlags : uint32
{
  ...
  GO_FLAG_TRANSPORT     = 0x00000008,  // any kind of transport? Object can transport
                                       // (elevator, boat, car)
...
{
  CREATURE_TYPE_FLAG_TAMEABLE = 0x00000001,               // Makes the mob tameable (must also
                                                          // be a beast and have family set)
  CREATURE_TYPE_FLAG_VISIBLE_TO_GHOSTS = 0x00000002,      // Creature is also visible for not alive player. Allows
                                                          // gossip interaction if npcflag allows?
  CREATURE_TYPE_FLAG_BOSS_MOB = 0x00000004,               // Changes creature's visible level to "??" in the creature's
  ...
  CREATURE_TYPE_FLAG_INTERACT_WHILE_DEAD    = 0x00000080,   // Player can interact with the creature if creature is dead
  ...
  CREATURE_TYPE_FLAG_COLLIDE_WITH_MISSILES = 0x00080000, // Projectiles can collide with this creature - interacts with
                                                         // TARGET_DEST_TRAJ
  ...
  CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT = 0x04000000, // ! Creature can be targeted by spells that require target to
                                                      // be in caster's party/raid
...
enum GroupJoinBattlegroundResult
{
  ...
  ERR_GROUP_JOIN_BATTLEGROUND_FAIL = 0,         // Your group has joined a battleground queue, but you are not eligible
                                                // (showed for non existing BattlemasterList.dbc indexes)
  ERR_BATTLEGROUND_NONE                 = -1,   // not show anything
  ERR_GROUP_JOIN_BATTLEGROUND_DESERTERS = -2,   // You cannot join the battleground yet because you or one of your
  ...
  ERR_BATTLEGROUND_QUEUED_FOR_RATED       = -6, // You cannot queue for another battle while queued for a rated arena
                                                // match
  ERR_BATTLEGROUND_TEAM_LEFT_QUEUE     = -7,    // Your team has left the arena queue
  ERR_BATTLEGROUND_NOT_IN_BATTLEGROUND = -8,    // You can't do that in a battleground.
  ERR_BATTLEGROUND_JOIN_XP_GAIN        = -9,    // doesn't exist in client...
  ERR_BATTLEGROUND_JOIN_RANGE_INDEX    = -10,   // Cannot join the queue unless all members of your party are in the
                                                // same battleground level range.
  ERR_BATTLEGROUND_JOIN_TIMED_OUT = -11,        // %s was unavailable to join the queue. (ObjectGuid guid exist in
                                                // client cache)
  ERR_BATTLEGROUND_JOIN_FAILED = -12,           // Join as a group failed (ObjectGuid guid
                                                // doesn't exist in client cache)
  ERR_LFG_CANT_USE_BATTLEGROUND = -13,          // You cannot queue for a battleground or
                                                // arena while using the dungeon system.
...
enum PartyResult
{
  ...
  ERR_GROUP_SWAP_FAILED        = 14, // if (PartyOperation == PARTY_OP_SWAP)
                                     // ERR_GROUP_SWAP_FAILED else ERR_INVITE_IN_COMBAT
